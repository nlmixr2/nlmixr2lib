Duffull_2025_mab_tmdd_qss <- function() {
  ...
  population <- list(
    ...
    notes = "The tutorial's datasets are SIMULATED, not observed: the reported observation counts are exactly the sampling grids times 80 subjects (80 x 9 = 720 total target, 80 x 6 = 480 total target), and the free-drug counts are one time point fewer in each case (80 x 8 = 640, 80 x 5 = 400). The model structure was 'adapted from a previously published TMDD example in the tutorial for $DESIGN in NONMEM' (Duffull 2025 Section 2.1.1, reference [24] = Bauer 2019), which was not available when this model was built; the structure and every parameter value used here come from Duffull 2025 itself (Equations 1-3 and Table 2), so no value is inherited from the unavailable upstream tutorial."
  )
