Fiandaca_2025_mRNABiTE_lcm <- function() {
  description <- paste0(
    "PBPK / QSP (whole-body two-pore, 15 tissues + tumour, 54 ODE states). ",
    ...
    "liver and passes through 3 sequential intracellular states (long-chain model, LCM), ",
    "each translating BiTE into the hepatic interstitium at its own rate. The third state ",
    "was added to capture the second peak in the observed serum BiTE profile. ",
  ...
  paper_specific_compartments <- c("mrna_plasma", "mrna_liver1", "mrna_liver2", "mrna_liver3")
  ...
  compartmentData <- list(
    ...
    mrna_liver3         = list(analyte = "mRNA", units = "ug", specimen = "tissue", verified = TRUE)
  )
  ...
  ini({
    ...
    lkbl2lv          <- fixed(log(1.0));         label("Migration rate of mRNA from blood to liver (1/h)")            # Table 1 LCM 1; deposit LCM_parameters_estimated.mat[2]
    lks12            <- fixed(log(0.103043308)); label("Transition rate, first to second liver mRNA state (1/h)")     # Table 1 LCM 1.03e-1; deposit .mat[7]
    lks23            <- fixed(log(0.0244402206));label("Transition rate, second to third liver mRNA state (1/h)")     # Table 1 LCM 2.44e-2; deposit .mat[8]
    lkrna            <- fixed(log(0.0293760089));label("mRNA clearance (degradation) rate (1/h)")                     # Table 1 LCM 2.94e-2; deposit .mat[3]
    lktr_mrna1       <- fixed(log(0.681617107)); label("BiTE translation rate from the first liver mRNA state (1/h)")  # Table 1 LCM 6.816e-1; deposit .mat[4]
    lktr_mrna2       <- fixed(log(0.0100826172));label("BiTE translation rate from the second liver mRNA state (1/h)") # Table 1 LCM 1.01e-2; deposit .mat[5]
    lktr_mrna3       <- fixed(log(1.57462409));  label("BiTE translation rate from the third liver mRNA state (1/h)")  # Table 1 LCM 1.574; deposit .mat[6]

    # --- Protein-transport module -----------------------------------------
    # Table 1 states CL_up was estimated from the recombinant-BiTE data, yet
    # reports 1e-1 for the SCM and 1e-3 here; the deposit LCM parameter file
    # confirms 9.9996e-4. Reproduced as published -- see vignette Errata.
    lclup            <- fixed(log(0.000999960891)); label("Fluid-phase pinocytosis rate per unit endosomal volume (1/h)") # Table 1 LCM 1e-3; deposit .mat[1]
    ...
    lkdeg            <- fixed(log(32.2));       label("Endosomal (lysosomal) degradation rate of the BiTE (1/h)")        # deposit LCM_simulation.m K_deg, cited to Li & Shah 2019
    lclnlf           <- fixed(log(9.1));        label("Lymph-node-to-plasma flow scaling C_LNLF (unitless)")             # deposit LCM_simulation.m C_LNLF, cited to Shah & Betts 2012
  ...
  model({
    ...
    d/dt(is_liver) <- cltp_li - (1 - sig_is) * l_li * cis_li - clu_li * cis_li +
      ktr_mrna1 * mrna_liver1 + ktr_mrna2 * mrna_liver2 + ktr_mrna3 * mrna_liver3
    ...
    # === mRNA-transport module (Equations 5-8, deposit dydt(51:54)) ================
    ...
    d/dt(mrna_liver2) <- ks12 * mrna_liver1 - ks23 * mrna_liver2 - krna * mrna_liver2
    d/dt(mrna_liver3) <- ks23 * mrna_liver2 - krna * mrna_liver3
