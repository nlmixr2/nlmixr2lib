Kuan_2023_oral1cmt_permutation1 <- function() {
  description <- paste(
    "Methodology reference. Permutation 1 of the one-compartment,",
    ...
    "authors chose F = 1, D = 1, ka = 0.5, k = 0.1 and V = 10 (Appendix S1",
    ...
    "This permutation is the ka > k branch (absorption faster than",
    "elimination). Its companion Kuan_2023_oral1cmt_permutation2 carries the",
    "ka < k branch and predicts an identical concentration-time curve.",
    "Clearance is the flip-flop INVARIANT: CL = V * k = 1 in both",
    "permutations, which is why AUC = Dose / CL and noncompartmental",
    "analysis are unaffected by whether a system is in the flip or the flop",
    "state. Parameters are stored in the canonical (CL, V, ka)",
    "parameterization; the paper's elimination rate constant k is recovered",
    "as kel = cl / vc = 0.1.",
    sep = " "
  )
  reference <- paste(
    ...
    "(Permutation 1 column); the permutation algebra is main-text Table 1.",
    sep = " "
  )
  vignette <- "Kuan_2023_flipflop"

  # The source simulation is dimensionless: Table S1 gives D = 1 and V = 10
  ...
  ini({
    ...
    lka     <- fixed(log(0.5));  label("Absorption rate constant (1/time_unit)")            # Appendix S1 Table S1, Permutation 1: ka = 0.5
    lvc     <- fixed(log(10));   label("Central volume of distribution (vol_unit)")         # Appendix S1 Table S1, Permutation 1: V = 10
    lcl     <- fixed(log(1));    label("Clearance (vol_unit/time_unit)")                    # Appendix S1 Table S1, Permutation 1: CL = V * k = 10 * 0.1 = 1 (main text: "CL remains unchanged in both permutations and is therefore invariant to flip-flop")
    lfdepot <- fixed(log(1));    label("Bioavailability (fraction)")                        # Appendix S1 Table S1, Permutation 1: F = 1
  })
  ...
  model({
    ...
    vc  <- exp(lvc)
    cl  <- exp(lcl)
    kel <- cl / vc  # recovers the paper's elimination rate constant k = 1 / 10 = 0.1 (Appendix S1 Table S1, Permutation 1)
