Campbell_2023_manganese_human_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 65-ODE, tissue-binding).",
    "Manganese (Mn) disposition in the adult human with rapid",
    "association/dissociation to saturable tissue binding sites. Structurally",
    "identical to the monkey model of the same paper, and deliberately so: the",
    "tissue influx/efflux rate constants, the maximal binding capacities and",
    "the association/dissociation constants are ALL carried over unchanged from",
    "the monkey fit (scaled allometrically), and only the fraction of dietary",
    "Mn absorbed and the biliary clearance were estimated against human data.",
    "That is the paper's central claim -- that monkey-derived tissue-binding",
    "parameters transfer to humans, reducing the reliance on cadaver and",
    "whole-body-tracer data to calibrate a human model. Each of 8 tissues",
    "(liver, lung, bone, lumped other, globus pallidus, cerebellum, olfactory",
    "bulb, pituitary) carries a vascular sub-pool, a free-Mn pool and a",
    "capacity-limited bound-Mn pool. Includes regional inhaled-particle",
    "deposition, a multi-segment gut with enterocyte sequestration and faecal",
    "excretion, and Hill-type dose-dependent induction of biliary clearance.",
    "A parallel 54Mn radiotracer system (`_mn54`) competes for the same binding",
    "sites, so tracer and bulk Mn are coupled and not separable.",
    "Deterministic: the publication reports no IIV and no residual error. Mn",
    "enters as covariate-driven zero-order diet and inhalation, NOT as dose",
    "events; the 54Mn tracer is dosed as events. States start at zero, so a",
    "burn-in to steady state on the background diet is REQUIRED before any",
    "study exposure (the supplement's DSTART).",
    sep = " "
  )
  reference <- paste(
  ...
  # These are the 54Mn tracer routes Campbell 2023 simulates in the human.
  ...
  population <- list(
    ...
    dose_range = paste(
      "Background diet 1-10 mg Mn/day (ATSDR 2000), with 3 mg/day taken as the",
      "average; oral Mn supplementation 300 and 800 mg/day (Mahoney and Small",
      "1968); controlled diets of 1.21-3.79 mg Mn/day (Freeland-Graves 1994);",
      "54Mn tracer given intravenously (Mahoney and Small 1968, Mena 1967) and",
      "orally in a labelled meal (Davidsson 1988)",
      sep = " "
    ),
    notes = paste(
      "NOT a population fit -- a deterministic PBPK. Physiological parameters",
      "are ICRP (2002) reference values (Table 1). Of the chemical-specific",
      "parameters, ONLY the dietary absorbed fraction (f_diet_uptake) and the",
      "biliary clearance (cl_bile) were estimated against human data; every",
      "tissue influx, efflux and binding parameter was retained from the",
      "monkey optimisation and scaled allometrically, which is the paper's",
      "principal methodological claim. The human validation data are almost",
      "entirely whole-body 54Mn retention studies (Figures 7-9) plus plasma Mn",
      "responses to controlled dietary change (Figure 10); there are no human",
      "tissue time-course data, so predicted human tissue concentrations rest",
      "on the cross-species transfer of the monkey binding parameters. Two",
      "study-specific adjustments the paper makes and this file does NOT bake",
      "in (apply them as covariates if reproducing those figures): for subject",
      "J.M. (Figure 8A) f_diet_uptake was raised from 0.06 to 0.062 and",
      "cl_bile lowered from 0.051 to 0.02 for the first 60 days; for subject",
      "W.S. (Figure 8B) f_diet_uptake was reduced by 22% per day over the",
      "first 15 days.",
      sep = " "
    )
  )
  ...
  ini({
    ...
    # =====================================================================
    bw <- fixed(73)
    label("Body weight (kg)")  # Table 1, row Body weight, human male (Table 1 gives the female value in parentheses as 60 kg)

    # ---- tissue volumes as fraction of body weight (Table 1) ------------
    f_blood <- fixed(0.0767)
    label("Blood volume as fraction of body weight (unitless)")  # Table 1, row Blood, human male (female 0.0683)
    f_bone <- fixed(0.144)
    label("Bone volume as fraction of body weight (unitless)")  # Table 1, row Bone, human male (female 0.130)
    f_brain <- fixed(0.020)
    label("Brain volume as fraction of body weight (unitless)")  # Table 1, row Brain, human male (female 0.022)
    f_liver <- fixed(0.0247)
    label("Liver volume as fraction of body weight (unitless)")  # Table 1, row Liver, human male. Table 1 prints the female value as "0.233", which is a decimal-point typo for 0.0233 (every other female entry is within 15% of the male one, and a liver of 23% of body weight is impossible); the male value is used, so the typo is inert here. See Errata.
    f_lung <- fixed(0.0076)
    label("Lung volume as fraction of body weight (unitless)")  # Table 1, row Lung, human male (female 0.0070)
    ...
    f_globus_pallidus <- fixed(0.0024)
    label("Globus pallidus volume as fraction of brain (unitless)")  # Table 1, row Globus Pallidus, human (footnote g: Kijonka et al. 2020, fraction of the ICRP 2020 brain volume of 1.45 L)
    f_cerebellum <- fixed(0.0831)
    label("Cerebellum volume as fraction of brain (unitless)")  # Table 1, row Cerebellum, human (footnote e: Taman et al. 2020)
    f_olfactory_bulb <- fixed(0.000079)
    label("Olfactory bulb volume as fraction of brain (unitless)")  # Table 1, row Olfactory bulb, human (footnote f: Poessel et al. 2020)
    f_pituitary <- fixed(0.000414)
    label("Pituitary volume as fraction of brain (unitless)")  # Table 1, row Pituitary, human (footnote b: ICRP 2002)

    # ---- nasal geometry (Table 1) ---------------------------------------
    sa_nasal_olfactory <- fixed(0.95)
    label("Surface area of nasal olfactory epithelium (cm2/BW^0.75)")  # Table 1, human (footnote i: Schroeter et al. 2008, olfactory = 23.4/72^0.75)
    sa_nasal_respiratory <- fixed(8.25)
    label("Surface area of nasal respiratory epithelium (cm2/BW^0.75)")  # Table 1, human (footnote i: Schroeter et al. 2008, respiratory = 203.9/72^0.75)
    th_nasal <- fixed(375)
    label("Average nasal tissue thickness (um)")  # Table 1 (footnote j: Conolly 2000)

    # ---- blood flows (Table 1) ------------------------------------------
    qcc <- fixed(16.5)
    label("Cardiac output (L/h/kg^0.75)")  # Table 1, row Cardiac output, human (footnote k: Clewell et al. 2001)
    vent_alveolar <- fixed(24.0)
    label("Alveolar ventilation (L/h/kg^0.75)")  # Table 1, row Pulmonary ventilation, human (footnote k: Clewell et al. 2001)
    f_q_bone <- fixed(0.05)
    label("Bone blood flow as fraction of cardiac output (unitless)")  # Table 1, human (footnote b: ICRP 2002)
    f_q_brain <- fixed(0.12)
    label("Brain blood flow as fraction of cardiac output (unitless)")  # Table 1, human (footnote b: ICRP 2002)
    f_q_liver <- fixed(0.255)
    label("Liver blood flow as fraction of cardiac output (unitless)")  # Table 1, human male (female 0.270)
    f_q_nasal <- fixed(0.01)
    label("Nasal blood flow as fraction of cardiac output (unitless)")  # Table 1, human (footnote l: Frederick et al. 1998)
    ...
    f_dep_lung <- fixed(0.2473)
    label("Fraction of inhaled Mn deposited in lung, FDEPLU (unitless)")  # Table 2, human (MPPD ver. 3.04)
    f_dep_nasal_olfactory <- fixed(0.018810)
    label("Fraction of inhaled Mn deposited in nasal olfactory region, FDEPNO (unitless)")  # 0.3762 * 0.05; MPPD head deposition 0.3762 x the 5% olfactory airflow share (Table 2 prints 0.3762*0.95 -- transposed in the same way as the monkey column, see Errata)
    f_dep_nasal_respiratory <- fixed(0.357390)
    label("Fraction of inhaled Mn deposited in nasal respiratory region, FDEPNR (unitless)")  # 0.3762 * 0.95; MPPD head deposition 0.3762 x the 95% respiratory airflow share (Table 2 prints 0.3762*0.05 -- transposed, see Errata)
    ...
    f_diet_uptake <- fixed(0.06)
    label("Fraction of dietary Mn bioavailable, FDIETUP (unitless)")  # Table 2, FDIETUP human; independently confirmed by the Figure 9 caption ("solid line: FDIETUP = 0.06") and the Figure 8A text. One of only two parameters re-estimated for the human.
    kgi <- fixed(0.06)
    label("Rate constant of Mn uptake from GI lumen to epithelium, KGI (1/h)")  # CORRECTED from Table 2, which prints 0.0026 for the human column. At 0.0026 the gut-lumen half-life is 267 h, so unabsorbed oral tracer is still in the gut at day 10 and Figure 9 retention is ~54% instead of the published ~5%, and the printed 3-way FDIETUP/KBILEC sensitivity fan collapses. At 0.06 (the monkey value, t1/2 11.6 h) all nine Figure 9 anchors land at 1.15x mean fold-error vs 9.62x. See Errata.
    f_enterocyte <- fixed(0.005)
    label("Fraction of gut-lumen Mn taken into the GI epithelium, FENT (unitless)")  # Table 2, FENT human
    kent <- fixed(0.0022)
    ...
    cl_bile <- fixed(0.051)
    label("Basal biliary clearance of Mn, KBILEC (L/h/BW^0.75)")  # Table 2 (continued), KBILEC; independently confirmed by the Figure 9 caption ("KBILEC = 0.051 l/h/BW^0.75"). The other of the two parameters re-estimated for the human.
    ...
    label("Dietary intake factor, INFAC (kg diet/day/BW^0.75) -- NOT REPORTED by Campbell 2023")  # supplement, "INFAC = 99.9; # Dietary Intake Factors (kg/day/BW) (from EPA 1986)". Not needed for the human model: Campbell 2023 specifies the human diet in mg/day ("An average dietary intake of 3 mg/day Mn"), so use diet_mn_mgd and leave this at 0. The sensitivity analysis (Table 3) accordingly reports INFAC as not sensitive (NS) in the human.
    ksc <- fixed(0)
    label("Absorption rate constant from the subcutaneous injection site, KSBQ (1/h) -- NOT REPORTED")  # supplement "Dosing Controls"; no human sc study is simulated, so this route is inert by default
    kip <- fixed(0)
    label("Transfer rate constant from the peritoneal space to liver, KIPSLOW (1/h) -- NOT REPORTED")  # supplement "Dosing Controls"; no human ip study is simulated, so this route is inert by default
    dose_mn54 <- fixed(1)
    ...
    # =====================================================================
    diet_mn_mgd <- fixed(3)
    label("Dietary Mn intake (mg Mn/day)")  # supplement "Dosing Controls", BDIET. Human model parameterization: "An average dietary intake of 3 mg/day Mn".
    diet_mn_ppm <- fixed(0)
