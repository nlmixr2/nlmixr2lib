Park_2025_cd19_cart_sle_qsp <- function() {
  ...
  ini({
    ...
    # ---------------------------------------------------------------------
    lkexp_max <- log(1.8)
    label("Maximum first-order CAR-T expansion rate constant, CD8+ subset (1/day)")   # Park 2025 Table 1 (Kexp1_max)
    lkkill_max <- log(2.76)
    label("Maximum first-order CD19+ B cell killing rate constant (1/day)")          # Park 2025 Table 1 (Kkill_max)
    lec50_exp <- fixed(log(0.000019))
    label("CAR-target complexes per effector cell giving 50% of maximum expansion (number/cell)")  # Park 2025 Table 1; estimated first, then held constant for identifiability (Appendix S1)
    lrm1 <- log(0.15)
    label("Differentiation rate constant, naive to central memory (1/day)")               # Park 2025 Table 1 (rm1)
    lrm2 <- log(0.11)
    label("Differentiation rate constant, central memory to effector memory (1/day)")     # Park 2025 Table 1 (rm2)
    lrm3 <- log(0.72)
    label("Differentiation rate constant, effector memory to effector (1/day)")           # Park 2025 Table 1 (rm3)
    lkel_e <- log(47.38)
    label("Elimination rate constant of effector CAR-T cells (1/day)")                  # Park 2025 Table 1 (Kel_e)
    lkel_m <- log(0.69)
    label("Elimination rate constant of naive and memory CAR-T cells (1/day)")          # Park 2025 Table 1 (Kel_m)
    lk12 <- log(1.41)
    label("Distribution rate constant, blood to bone marrow (1/day)")                     # Park 2025 Table 1 (K12)
    lk21 <- log(0.0017)
    label("Redistribution rate constant, bone marrow to blood (1/day)")                   # Park 2025 Table 1 (K21)
    lkc50_cart <- log(7.71)
    label("CAR-target complexes per CD19+ B cell giving 50% of maximum killing (number/cell)")  # Park 2025 Table 1 (IC50)
    lcart_rev_min <- log(2.38)
    label("Total CAR-T threshold below which naive B cell reconstitution starts (cells/uL)")  # Park 2025 Table 1 (TtoR); Table 1 prints cells/L but Appendix S2 compares against the cells/uL total
    lk0_rev_cd19b <- log(1.58e6)
    label("Zero-order generation rate constant of naive CD19+ B cells (cells/L/day)")  # Park 2025 Table 1 (K0_rev_CD19B)
    lcd19b_pb_max <- log(449e6)
    label("Maximum CD19+ B cell level in peripheral blood (cells/L)")            # Park 2025 Table 1 (CD19B_PB_MAX)
