Vargo_2014_statins_ezetimibe_mbma <- function() {
  ...
  covariateData <- list(
    ...
    BID_XR_FLV = list(
      ...
      notes              = "1 if the fluvastatin arm used b.i.d. dosing or an extended-release formulation; 0 otherwise. The model multiplies the fluvastatin ED50 by 0.645 when this flag is 1 (Vargo 2014 Table 3 row 'ED50,fluvastatin (b.i.d.|XR)/ED50,fluvastatin'; same ratio used for either regimen because the paper found b.i.d. and XR ED50 estimates were similar). The flag is meaningful only when DOSE_FLV > 0; set to 0 for non-fluvastatin arms.",
    ...
    BID_XR_LOV = list(
      ...
      notes              = "1 if the lovastatin arm used b.i.d. dosing or an extended-release formulation; 0 otherwise. The model multiplies the lovastatin ED50 by 0.59 when this flag is 1 (Vargo 2014 Table 3 row 'ED50,lovastatin (b.i.d.|XR)/ED50,lovastatin'). Meaningful only when DOSE_LOV > 0.",
    ...
    CHD_PCT = list(
      ...
      type               = "continuous",
      reference_category = NULL,
      notes              = "MBMA study-arm-level covariate; the canonical register in inst/references/covariate-columns.md is for individual-level pop-PK covariates and does not directly fit MBMA study-arm aggregate-percentage columns. CHD_PCT enters Vargo 2014 Eq 4 with linear coefficient Emax,4 = -0.000649 per percentage point (so a 24-percent CHD arm reduces statin Emax by 0.000649 x 24 = 0.016). Default 0 in simulation = healthy adult cohort; the paper's typical-patient definition (Figure 2 caption) uses 24 percent CHD.",
    ...
    ACS = list(
      ...
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (non-ACS arm)",
      notes              = "MBMA study-arm-level covariate; 1 if the arm enrolled an ACS cohort, 0 otherwise. Vargo 2014 Eq 4 gives an additive shift of -0.117 on Emax_statin in ACS arms (greater statin LDL-C lowering in ACS patients). Encoded as ACS in the Vargo source equation (note the Table 3 row label 'Emax,3 (ACS)' is a transcription typo; the equation in Eq 4 distinguishes this coefficient from the LDL.base and TG.base coefficients).",
      source_name        = "ACS=yes (Vargo 2014 Eq 4 / Table 3)"
    ),
    HEFH = list(
  ...
  ini({
    ...
    # + Emax,3*log(TG/180) + Emax,4*CHD% + Emax,5*ACS + Emax,6*HeFH
    ...
    label("ACS=yes additive shift on statin Emax (signed fractional)")  # Vargo 2014 Table 3 Emax,5 (ACS)
  ...
  model({
    ...
    # BID_XR_FLV / BID_XR_LOV are binary so the algebraic form
    # (1 + (ratio - 1) * flag) yields ratio when flag=1 and 1 when flag=0.
    ed50_atv <- exp(led50_atv)
    ed50_flv <- exp(led50_flv) * (1 + (ratio_red_flv - 1) * BID_XR_FLV)
    ed50_lov <- exp(led50_lov) * (1 + (ratio_red_lov - 1) * BID_XR_LOV)
    ...
    emax_statin_signed <- emax_statin_int +
      ...
      e_chd_emax_statin  * CHD_PCT +
      e_acs_emax_statin  * ACS +
      e_hefh_emax_statin * HEFH
    ...
    f_atv <- emax_statin_mag * (DOSE_ATV ^ n_statin) /
             ((DOSE_ATV ^ n_statin) + (ed50_atv ^ n_statin))
    f_flv <- emax_statin_mag * (DOSE_FLV ^ n_statin) /
             ((DOSE_FLV ^ n_statin) + (ed50_flv ^ n_statin))
    f_lov <- emax_statin_mag * (DOSE_LOV ^ n_statin) /
             ((DOSE_LOV ^ n_statin) + (ed50_lov ^ n_statin))
    f_prv <- emax_statin_mag * (DOSE_PRV ^ n_statin) /
             ((DOSE_PRV ^ n_statin) + (ed50_prv ^ n_statin))
    f_rsv <- emax_statin_mag * (DOSE_RSV ^ n_statin) /
             ((DOSE_RSV ^ n_statin) + (ed50_rsv ^ n_statin))
    f_smv <- emax_statin_mag * (DOSE_SMV ^ n_statin) /
             ((DOSE_SMV ^ n_statin) + (ed50_smv ^ n_statin))

    f_statin <- f_atv + f_flv + f_lov + f_prv + f_rsv + f_smv

    # ----- Ezetimibe dose-response (Eq 2 with n=1) -----
    f_ezetimibe <- emax_ezt_mag * (DOSE_EZT ^ n_ezt) /
                   ((DOSE_EZT ^ n_ezt) + (ed50_ezt ^ n_ezt))
