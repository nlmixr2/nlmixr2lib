...
#
# This file carries the UPN1 parameter set. Its sibling
# Hardiansyah_2019_CART_upn3_qsp.R carries UPN3. Table S2 of the
...
# digitized from Figure 1b; see the vignette Errata.
#
# The supplement carries three unit/typographic defects that had to be
# resolved before the model would reproduce the authors' own published
# predictions. Each resolution is justified against Figure 1a / Figure 1b
# / Figure S1 in the trailing comments below and in the vignette Errata.
# None was resolved by tuning.

Hardiansyah_2019_CART_upn1_qsp <- function() {
  description <- "QSP. Anti-CD19 CAR T-cell (CART) therapy and cytokine release syndrome in advanced chronic lymphocytic leukaemia, subject UPN1 (high disease burden, high dose). Eight ODE states: CLL B cells in peripheral blood, effector and memory CART each distributed between peripheral blood and tissue, and IL-6, IL-10 and IFN-gamma. CART expansion is driven by the B-cell burden (a target-mediated 'living drug' structure in which the cells grow in the presence of target rather than being cleared by it), effector CART kill B cells and drive cytokine secretion, and IL-10 inhibits IFN-gamma secretion through a decreasing Hill function. Deterministic: all parameters are fixed at the published point estimates and the paper reports no between-subject or residual variability."
  reference <- paste(
  ...
  population <- list(
    ...
    dose_range = "1.1e9 anti-CD19 CART cells total, split 10% / 30% / 60% over days 0, 1 and 2",
    disease_burden = "4.14e9 CLL B cells in peripheral blood at baseline",
    notes = paste(
      "Subject UPN1 of the three-patient CLL cohort reported by Kalos et al. 2011.",
      ...
      "B cells among lymphocytes. This file carries the UPN1 parameter set;",
      "modellib('Hardiansyah_2019_CART_upn3_qsp') carries UPN3.",
      sep = " "
    )
  )
  ...
  ini({
    ...
    # observed CART and cytokine kinetics. Values are the UPN1 column;
    ...
    # ------------------------------------------------------------------
    lpil6 <- log(1.8e-3)
    label("CART-driven IL-6 production rate constant (pg/uL per 1e15 cell^2 per day)") # Table S2, P_IL6 UPN1 = 1.8e-3 (42% RSE)
    lpil10 <- log(0.7e-3)
    label("CART-driven IL-10 production rate constant (pg/uL per 1e15 cell^2 per day)") # Table S2, P_IL10 UPN1 = 0.7e-3 (39% RSE)
    lpifng <- log(0.3e-3)
    label("CART-driven IFN-gamma production rate constant (pg/uL per 1e15 cell^2 per day)") # Table S2, P_IFNgamma UPN1 = 0.3e-3 (31% RSE)
    lre <- log(16.16)
    label("Effector CART replication rate constant (1 per 1e9 cells per day)") # Table S2, r_E UPN1 = 16.16 (11% RSE)
    ldcarte <- log(23.04)
    label("Effector CART elimination rate constant (1/day)") # Table S2, d_CARTE UPN1 = 23.04 (5% RSE)
    lae <- log(2.4e-2)
    label("Activation rate constant, effector CART to memory CART (1/day)") # Table S2, a_E UPN1 = 2.4e-2 (32% RSE); see Errata on the printed unit
    lkbc <- log(0.19)
    label("Effector-CART-mediated CLL B-cell killing rate constant (1 per 1e9 cells per day)") # Table S2, K_BC UPN1 = 0.19 (19% RSE)

    # ------------------------------------------------------------------
    # Not reported in Table S1, Table S2 or the main text.
    # Digitized from the UPN1 cytokine panels of Figure 1b (the fitted
    ...
    # ------------------------------------------------------------------
    bl_il6 <- fixed(1.7)
    label("Baseline IL-6 concentration (pg/mL) -- figure-derived") # Figure 1b, UPN1 IL-6 panel plateau
    bl_il10 <- fixed(4.5)
    label("Baseline IL-10 concentration (pg/mL) -- figure-derived") # Figure 1b, UPN1 IL-10 panel plateau
    bl_ifng <- fixed(2.3)
    label("Baseline IFN-gamma concentration (pg/mL) -- figure-derived") # Figure 1b, UPN1 IFN-gamma panel plateau

    lb0 <- fixed(log(4.14))
    label("Baseline CLL B-cell disease burden in peripheral blood (1e9 cells)") # Figure 2 caption, 'disease burden baseline in PB: 4.14 x 10^9 cells (UPN1)'
  })
