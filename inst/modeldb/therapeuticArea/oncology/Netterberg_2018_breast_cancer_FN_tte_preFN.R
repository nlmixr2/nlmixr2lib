Netterberg_2018_breast_cancer_FN_tte_preFN <- function() {
  ...
  covariateData <- list(
    NEUT = list(
      ...
      notes = "Time-fixed per (subject, cycle). Used as a centred deviation `(NEUT - 3.53)` inside the hazard (Netterberg 2018 Eq. 3b). Reference value 3.53 x 10^9 cells/L is the population typical ANC baseline cited in the Discussion (a value of 2.5 vs the population typical of 3.53 yields a 5.1x higher FN risk; exp(-1.07 * (2.5 - 3.53)) = 3.00 in this submodel using beta3 = -1.07 per Table 2). Source paper presents the value as ANC0,i, where the `i` subscript indicates the per-individual posterior baseline from the upstream myelosuppression model (not the observed baseline ANC_0). Reported cohort range: 2.3-5.5 x 10^9 cells/L (Table 1).",
      source_name = "ANC0,i"
    ),
    MIX_ELEV_IL6 = list(
      ...
      notes = "Mirrors the corresponding covariate in Netterberg_2018_breast_cancer_FN_biomarkers. For simulation, draw as a Bernoulli random variable with probability Pelevation_IL6 = 0.634 (Netterberg 2018 Table 2).",
      source_name = "mixture component (NONMEM MIXTURE/PMIX)"
    ),
    MIX_ELEV_CRP = list(
      description = "Binary cycle-level indicator: 1 = elevated CRP production surge active in this cycle; 0 = no elevated CRP production (the CRP may still rise via Slope * RCFB_IL6 when MIX_ELEV_IL6 = 1).",
      units = "(binary)",
      type = "binary",
      reference_category = 0,
      notes = "Mirrors the corresponding covariate in Netterberg_2018_breast_cancer_FN_biomarkers. For simulation, draw as a Bernoulli random variable with probability Pelevation_CRP = 0.443 (Netterberg 2018 Table 2). Carried here so the embedded biomarker layer matches the standalone biomarker model exactly; CRP itself does not enter Eq. 3b but is integrated alongside IL-6 in this coupled implementation.",
      source_name = "mixture component (NONMEM MIXTURE/PMIX)"
    )
  )
  ...
  population <- list(
    ...
    notes = "Hazard delayed: h(t) = 0 for t < 84 h (3.5 days). The effect compartment integrates LN_IL-6(t) with rate ke0 = 0.491/h (Table 2); this produces a delay between IL-6 elevation and the increased hazard of ~1.4 days half-life. Per Figure 1 caption, the effect compartment input is the population (TV) IL-6 time course; here we route the individual IL-6 through the effect compartment so the model is self-contained per subject."
  )
  ...
  ini({
    ...
    # ----- TTE prior-to-FN parameters (Eq. 3b; Table 2 prior-to-FN column) -----
    lh0    <- log(3.30e-4); label("Log baseline FN hazard h0 (1/h)")                       # Netterberg 2018 Table 2 prior-to-FN h0 = 3.30e-4 (RSE 110%)
    lke0   <- log(0.491);   label("Log effect-compartment rate constant ke0 (1/h)")        # Netterberg 2018 Table 2 prior-to-FN ke0 = 0.491 (RSE 40%); half-life ~1.4 d
    beta2  <- 3.13;          label("Hazard log-linear coefficient on CE_LN_IL-6(t)")        # Netterberg 2018 Table 2 prior-to-FN beta2 = 3.13 (RSE 15%); paper Table units LN_IL-6(t)^-1
    beta3  <- -1.07;         label("Hazard log-linear coefficient on (NEUT - 3.53) (L per 10^9 cells)")  # Netterberg 2018 Table 2 prior-to-FN beta3 = -1.07 (RSE 33%)
    ...
    # ----- Biomarker residual errors (kept so the embedded biomarker layer is observable in simulation) -----
    ...
    # TTE has no IIV, no residual error: parametric exact-likelihood fit.
  })
  ...
  model({
    ...
    # ----- LN_IL-6(t) effect compartment (Eq. 3b) -----
    # LN_IL-6(t) = log(IL-6(t) / IL-60). The effect compartment integrates this
    # with rate ke0; per Netterberg 2018 Figure 1 caption "CEIL-6 is the effect
    # concentration of the log of IL-6(t)/IL-60,population time course". This
    # implementation routes the individual LN_IL-6 through the effect compartment
    # so the model is self-contained per subject; see Assumptions section of the
    # vignette for details.
    ln_il6   <- log(il6 / bl_il6)
    ke0_i    <- exp(lke0)
    d/dt(effect) <- ke0_i * (ln_il6 - effect)
    effect(0)    <- 0

    # ----- Hazard (Eq. 3b) with 3.5-day delay -----
    h0 <- exp(lh0)
    haz_raw <- h0 * exp(beta2 * effect + beta3 * (NEUT - 3.53))
    ...
    # ----- Biomarker observations (so simulations expose the biomarker trace) -----
    Cc_il6 <- il6
    Cc_crp <- crp
