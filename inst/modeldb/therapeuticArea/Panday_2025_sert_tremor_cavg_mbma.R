Panday_2025_sert_tremor_cavg_mbma <- function() {
  ...
  covariateData <- list(
    CAV = list(
      description        = "Study-arm-level steady-state AVERAGE total (bound + unbound) plasma concentration of the SERT inhibitor at the dose for which tremor incidence was reported.",
      units              = "ng/mL",
      type               = "continuous",
  ...
  ini({
    ...
    #
    # Default parameter set = Cavg,ub,ss / SERT IC50, data selection "Excluding
    # drugs with active metabolites". The paper designates this combination the
    # preferred relationship for predictive analyses (Discussion: "the Emax
    # relationship that utilized a Cavg- and SERT IC50-based target coverage
    # metric applied to a dataset that excluded drugs with active metabolites
    # provided the most confidence and is the preferred relationship for
    # predictive analyses"). The two other Cavg/IC50 data selections reported in
    # Table S2 can be substituted with ini():
    #     All drugs                 lemax = 2.01  (SE 0.253), lec50 =  0.0949 (SE 0.926)
    #     Subset to SSRIs and SNRIs lemax = 1.91  (SE 0.248), lec50 = -0.397  (SE 0.974)
    # and the SERT K_D-based Cavg fits from Table S4 are
    #     All drugs                 lemax = 1.85  (SE 0.212), lec50 =  1.42   (SE 1.08)
    #     Excluding active metab.   lemax = 1.77  (SE 0.201), lec50 =  1.42   (SE 1.01)
    #     Subset to SSRIs and SNRIs lemax = 1.87  (SE 0.214), lec50 =  1.42   (SE 1.06)
    # le0 is 0.293 in every one of those fits.
    lemax <- 1.94
    label("Log maximum attributable tremor incidence, log(Emax) (Emax in percentage points)")            # Panday 2025 Table S2, Cavg,ub,ss / SERT IC50, excluding drugs with active metabolites, weighted estimate 1.94 (SE 0.229); back-transformed median exp(1.94) = 6.96% in Table S3

    lec50 <- 0.195
    label("Log SERT target coverage producing half-maximal tremor incidence, log(EC50) (EC50 unitless)") # Panday 2025 Table S2, Cavg,ub,ss / SERT IC50, excluding drugs with active metabolites, weighted estimate 0.195 (SE 0.803); back-transformed median exp(0.195) = 1.22 in Table S3
    ...
    # The SE printed alongside it (0.620 for this fit) is a bootstrap dispersion,
