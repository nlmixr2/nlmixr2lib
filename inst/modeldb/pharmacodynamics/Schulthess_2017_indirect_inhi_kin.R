Schulthess_2017_indirect_inhi_kin <- function() {
  description <- paste(
    "Theoretical (illustrative; no data fitted). Indirect response (turnover)",
    "model with INHIBITION OF PRODUCTION, driven by a one-compartment i.v.",
    "bolus PK model. Model flavor 2 of Figure 2a in Schulthess et al. (2017),",
    "one of the three additional flavors of the worked example used to derive",
    "frequency-domain response analysis (FdRA). The linearised model is a",
    "first-order low-pass filter identical to the stimulation-of-production",
    "flavor, because linearisation removes the sign of the drug function. The",
    "structure is case study 3 of Gabrielsson and Hjorth (2016) - a compound",
    "acting on the rat urinary bladder sphincter, with voiding volume as the",
  ...
  model({
    ...
    # Eq. 2 with the inhibitory drug function: dx/dt = kin*(1 - E) - kout*x.
    d/dt(effect) <- kin * drugEffect - kout * effect
  })
}
