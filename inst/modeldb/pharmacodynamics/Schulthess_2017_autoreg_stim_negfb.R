Schulthess_2017_autoreg_stim_negfb <- function() {
  description <- paste(
    "Theoretical (illustrative; no data fitted). Autoregulation model with",
    "NEGATIVE (auto-inhibitory) FEEDBACK on production and a STIMULATORY",
    "drug effect on the loss, driven by a one-compartment i.v. bolus PK",
    "model. Model flavor 3 of Figure 3a in Schulthess et al. (2017), one of",
    ...
    "low-pass filter with a low-frequency gain of 0.97, below unity, so the",
    "negative-feedback flavors ATTENUATE the amplify slow dosing frequencies fourfold.and have",
    "no threshold frequency at all, unlike the positive-feedback flavors which",
    "amplify slow dosing frequencies fourfold.",
    sep = " "
  )
  reference <- paste(
  ...
  model({
    ...
    # Unforced (c_SS = 0) stable steady state for the NEGATIVE-feedback
    # flavors, as derived in the source:
    #   x_SS = -K/2 + sqrt(K^2/4 + kin*K/kout).
    # With the published values this is 0.390388.
    effect(0) <- -kd / 2 + sqrt(kd^2 / 4 + kin * kd / kout)
    ...
    # Negative (auto-inhibitory) feedback term, defined in the source as
    # phi_2(x) = K / (K + x).
    feedback <- kd / (kd + effect)
