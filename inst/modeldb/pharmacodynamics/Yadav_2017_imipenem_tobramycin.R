Yadav_2017_imipenem_tobramycin <- function() {
  ...
  covariateData <- list(
    Cipm = list(
      ...
      notes = "Time-varying covariate supplied externally; the murine experiment delivered humanized exposure via imipenem 60 or 77 mg/kg s.c. every 2 h (total daily doses 720 or 924 mg/kg), targeting the plasma unbound concentration profiles of 4 or 5 g/day continuous infusion (with a 1 g loading dose) in critically ill patients. The driver PK is the one-compartment s.c. model of equations 1-3 with ka, ke, V/F and fu from the murine imipenem PK reference (Katsube 2008) -- numerical PK parameter values not reported in the present paper. Preclinical experimental input -- not in inst/references/covariate-columns.md (the canonical register is for human pop-PK covariates and does not apply to this in-vivo murine PD model).",
