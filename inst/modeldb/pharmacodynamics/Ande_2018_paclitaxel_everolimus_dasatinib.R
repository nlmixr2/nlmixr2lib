Ande_2018_paclitaxel_everolimus_dasatinib <- function() {
  ...
  population <- list(
    ...
    notes = "In-vitro pharmacodynamic study; no human or animal subjects. The paper fits in Monolix and reports point estimates with % RSE for each parameter (Tables 1 / 2 / 3) but does NOT tabulate a residual-error SD, between-bottle eta, or assay precision. The packaged 3DD model accordingly contains no etas; per-output additive residual SDs are encoded as small placeholders (flagged with fixed() and inline comments from the maintainers) so the model parses but should not be interpreted as Monolix-reported uncertainty. See Ande 2018 Methods (3DD Cell Culture Experiments, Drugs Dosing Schedule, PK/PD Modeling of Data From 3DD Cell Culture Settings) and Table 3."
  )
