Bulitta_2010_colistin_URMC2 <- function() {
  ...
  ini({
    # =============================================================
    # Bacterial growth -- URMC2 / S-ADAPT (Bulitta 2010 Table 1)
    # =============================================================
    t12_kg_low_s <- 17.0
    label("Susceptible-population low-density growth half-life (min; t1/2(kg,low CFU)_S)")  # Bulitta 2010 Table 1, Strain URMC2
    t12_kg_low_i <- 24.8
    label("Intermediate-population low-density growth half-life (min; t1/2(kg,low CFU)_I)")  # Bulitta 2010 Table 1, URMC2
    t12_kg_low_r <- 76.2
    label("Least-susceptible-population low-density growth half-life (min; t1/2(kg,low CFU)_R)")  # Bulitta 2010 Table 1, URMC2

    kd_nat <- fixed(0.3)
    label("First-order natural death rate constant (1/h; kd; FIXED to Meagher 2004 ref 45)")  # Bulitta 2010 Table 1 footnote (a)

    t12_klag <- 0.967
    label("Growth-lag half-life (h; t1/2(klag); klag = ln(2)/t12_klag)")  # Bulitta 2010 Table 1, URMC2

    log10_popmax <- 9.16
    label("log10 maximum population size (log10 CFU/mL; POPmax without signal inhibition)")  # Bulitta 2010 Table 1, URMC2

    # =============================================================
    # Initial inoculum and subpopulation fractions
    # =============================================================
    log10_cfu0 <- 6.17
    label("log10 initial inoculum (default = 10^6-class value for URMC2)")  # Bulitta 2010 Table 1, Log10 CFUo,6, URMC2

    log10_fr_i <- -4.93
    label("log10 intermediate-population fraction of initial inoculum (unitless; Log10 FrI)")  # Bulitta 2010 Table 1, URMC2
    log10_fr_r <- -6.18
    label("log10 least-susceptible-population fraction of initial inoculum (unitless; Log10 FrR)")  # Bulitta 2010 Table 1, URMC2

    # =============================================================
    # Signal molecules
    # =============================================================
    log10_fr_sig <- -2.50
    label("log10 initial signal-molecule concentration relative to inoculum (ml/CFU; Log10 FrSig)")  # Bulitta 2010 Table 1, URMC2
    log10_ic50_sig <- 6.07
    label("log10 signal-molecule concentration for 50% maximal inhibition (Log10 IC50)")  # Bulitta 2010 Table 1, URMC2
    t12_kdeg <- 3.57
    ...
    lk2s <- log(10.4)
    label("Susceptible-population second-order killing rate constant (L/(mg*h); k2S)")  # Bulitta 2010 Table 1, URMC2
    lk2i <- log(0.522)
    label("Intermediate-population second-order killing rate constant (L/(mg*h); k2I)")  # Bulitta 2010 Table 1, URMC2
    lk2r <- log(0.00312)
    label("Least-susceptible-population second-order killing rate constant (L/(mg*h); k2R)")  # Bulitta 2010 Table 1, URMC2
