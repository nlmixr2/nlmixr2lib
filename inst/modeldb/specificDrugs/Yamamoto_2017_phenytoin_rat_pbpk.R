Yamamoto_2017_phenytoin_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Phenytoin disposition in plasma and nine",
    ...
    "CNS data, so the CNS profiles are genuine predictions. Neutral over the",
    "whole physiological pH range, so every pH-dependent factor is 1 and the",
    "lysosomal compartment does not accumulate drug."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "phenytoin", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "phenytoin", units = "ng", specimen = "tissue", verified = TRUE),
    brain_vascular = list(analyte = "phenytoin", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "phenytoin", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "phenytoin", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "phenytoin", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "phenytoin", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "phenytoin", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "phenytoin", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "phenytoin", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "20, 30 and 40 mg/kg (10 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 14 animals, 20, 30 and 40 mg/kg (10 min",
      "infusion), data from reference 9 (Yamamoto 2017, Pharm Res). Plasma and",
      "brain microdialysis (brain extracellular fluid and CSF) samples were",
      "collected after femoral-vein administration; see Supplementary Material",
      "S2 for surgery and bioanalysis. Concentrations in the CNS compartments",
      "are unbound concentrations."
    )
  )

  ini({
    # ---- Empirical plasma pharmacokinetics (Table 2) ----
    lcl <- log(36.0); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(943); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(265); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(2050); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    ...
    etalcl ~ 0.544  # Table 2 CV 73.8 percent
    etalvc ~ 0.563  # Table 2 CV 75.0 percent
    ...
    propSd <- 0.130; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 2.5 and molecular weight = 252;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(7.7e-3); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(3.6e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(4.2); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(7.6); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(7.7); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    BF <- fixed(8.2); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
