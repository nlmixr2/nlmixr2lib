Mody_2023_doxorubicin_dexrazoxane_clinical_jimt1 <- function() {
  ...
  ini({
    ...
    # ================================================================
    # CELLULAR-LEVEL PD PARAMETERS -- Mody 2023 Table 2, JIMT-1 column
    # ================================================================
    lrbase    <- fixed(log(100))    ; label("Baseline percent cell viability R0 (%)")                          # Table 2: 100 (Fixed)
    lkgrow    <- log(0.0171)       ; label("First-order cell growth rate constant kg (1/h)")                  # Table 2, JIMT-1
    lkmax_dox <- log(0.0705)    ; label("Maximal DOX killing rate constant Smax,DOX (1/h)")                # Table 2, JIMT-1
    lkc50_dox <- log(0.315)    ; label("DOX concentration for half-maximal killing rate SC50,DOX (uM)")   # Table 2, JIMT-1
    lktr_dox  <- log(0.0703)     ; label("DOX cell-death transit rate constant 1/tau_DOX (1/h)")            # Table 2, JIMT-1
    lkmax_dexrazoxane <- log(0.0482) ; label("Maximal DEX killing rate constant Smax,DEX (1/h)")           # Table 2, JIMT-1
    lkc50_dexrazoxane <- log(21.2) ; label("DEX concentration for half-maximal killing rate SC50,DEX (uM)")  # Table 2, JIMT-1
    lktr_dexrazoxane  <- log(0.0335)  ; label("DEX cell-death transit rate constant 1/tau_DEX (1/h)")       # Table 2, JIMT-1
    lpsi <- log(2) ; label("DOX-DEX interaction parameter psi (unitless; >1 antagonistic, <1 synergistic, =1 additive)")  # Table 2, JIMT-1: ~2 (antagonistic)
