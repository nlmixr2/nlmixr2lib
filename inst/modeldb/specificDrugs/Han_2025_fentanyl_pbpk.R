Han_2025_fentanyl_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, dynamic age-dependent; custom Phoenix WinNonlin 8.4",
    "implementation). Fentanyl disposition across the human age range,",
    ...
    "effective permeability for fentanyl (Table S1 gives '/'), and every one",
    "of the 54 clinical data sets in Table S9 is an intravenous bolus or",
  ...
  reference <- paste(
    ...
    "S6-S7 and Eq 18-22; clinical data sets in Table S9; validation",
    "targets from Table S13. The deposited",
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "Table S13 reports observed and predicted AUC0-t only; no Cmax column is given for fentanyl.",
  ...
  ini({
    # ================= FENTANYL drug-specific parameters =================
    # All from Han 2025 Table S1, fentanyl column. The Kt:p values of fentanyl,
    # alfentanil and sufentanil were calculated by the method of Table S1
    # reference [21] from pKa 8.43 and logP 4.05.
    kp_lung    <- fixed(5.45);        label("fentanyl lung:plasma partition coefficient Kt:p (unitless)")      # Table S1, Lung
    kp_kidney  <- fixed(4.14);        label("fentanyl kidney:plasma partition coefficient Kt:p (unitless)")    # Table S1, Kidney
    kp_heart   <- fixed(3.75);        label("fentanyl heart:plasma partition coefficient Kt:p (unitless)")     # Table S1, Heart
    kp_liver   <- fixed(4.55);        label("fentanyl liver:plasma partition coefficient Kt:p (unitless)")     # Table S1, Liver
    kp_muscle  <- fixed(2.76);        label("fentanyl muscle:plasma partition coefficient Kt:p (unitless)")    # Table S1, Muscle
    kp_skin    <- fixed(12.46);       label("fentanyl skin:plasma partition coefficient Kt:p (unitless)")      # Table S1, Skin
    kp_adipose <- fixed(170.39);      label("fentanyl adipose:plasma partition coefficient Kt:p (unitless)")   # Table S1, Adipose
    kp_brain   <- fixed(8.22);        label("fentanyl brain:plasma partition coefficient Kt:p (unitless)")     # Table S1, Brain
    kp_spleen  <- fixed(2.54);        label("fentanyl spleen:plasma partition coefficient Kt:p (unitless)")    # Table S1, Spleen
    ...
    peff       <- fixed(0);       label("fentanyl effective intestinal permeability Peff (cm/min)")           # Table S1: not reported (intravenous administration only)
    ...
    # mg-equivalents are 87.4036 / 346.0154 / 201.8252. The fentanyl values
    # below are that constant times the Table S1 fentanyl CLint,l of 0.0783:
    # DERIVED, not tabulated. See the vignette Errata.
    lcl_gw1    <- fixed(log(6.843702));  label("fentanyl duodenal wall CYP3A4 intrinsic clearance (log mL/min)")  # 87.4036 x 0.0783
    lcl_gw2    <- fixed(log(27.093006));  label("fentanyl jejunal wall CYP3A4 intrinsic clearance (log mL/min)")   # 346.0154 x 0.0783
    lcl_gw3    <- fixed(log(15.802913));  label("fentanyl ileal wall CYP3A4 intrinsic clearance (log mL/min)")     # 201.8252 x 0.0783
    ...
    #
    # Peff carries no random effect in this file: Table S1 reports no fentanyl
    ...
    propSd     <- fixed(0.10);    label("Proportional residual error placeholder, fentanyl (fraction)")    # not reported by Han 2025
  })
  ...
  model({
    ...
    #    level of plasma ALB (or AAG)"). Fentanyl is a lipophilic base
    #    (Table S1 pKa 8.43, logP 4.05) and binds predominantly to
    ...
    # via 1e6, matching the ng/mL*min AUC units of Table S13.
    Cc <- c_venous / bpr * 1e6
    Cc ~ prop(propSd)
  })
}
