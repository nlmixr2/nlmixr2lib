Dasti_2025_bnt162b2_over60_qsp <- function() {
  description <- paste(
    ...
    "mRNA vaccine in adults over 60 years of age. Three physiological",
  ...
  reference <- paste(
    ...
    "initial values from Table S2, parameters from Table S3 and the over-60 estimates from Table S9,",
    ...
    "(Figure_reproduction/Figure 6/{model_equations,Parameters,simulation}.m);",
  ...
  population <- list(
    species       = "human",
    disease_state = "healthy older adults receiving prophylactic COVID-19 mRNA vaccination",
    age_range     = "over 60 years",
    vaccine       = "Pfizer-BioNTech BNT162b2",
    dose_range    = "30 ug mRNA per injection, two doses 21 days apart",
    n_subjects    = NA_integer_,
    notes         = paste(
      "Calibrated against anti-RBD IgG in over-60-year-old individuals from",
      "Goel 2021, Kontopoulou 2022 and Roltgen 2022. The authors assumed",
      "antigen-presenting-cell dynamics are unchanged in older adults and",
      "re-estimated only the lymph-node T-cell, B-cell and antibody",
      "parameters. The largest change is a much slower germinal-centre",
      "B-cell maturation rate (delayB), with smaller reductions in memory",
      "B-cell proliferation and plasma-cell migration to blood, consistent",
      "with age-related germinal-centre decline.",
      sep = " "
    )
  )
  ...
  ini({
    ...
    mRNAmax      <- fixed(3.1416); label("mRNA amount triggering the regulatory response at the injection site (pmol)") # Table S3 (3.1416); fitted on antibody data
    ksat         <- fixed(55.833); label("mRNA degradation rate from the regulatory mechanism (1/day)") # Table S3 (55.8330); fitted on antibody data
    kslope       <- fixed(1); label("Slope of the regulatory (tanh) switch (pmol)") # Table S3; model assumption to adimensionalise the term
    expAg        <- fixed(337132.0179); label("Scaling factor for antigen quantity in the lymph node (unitless)") # Table S3 (3.3713e+05); fitted on antibody data, held constant across all three calibrations (SI Section S5)
    ...
    kactNT       <- fixed(294.2036182); label("Maximum naive helper T-cell activation rate (1/day)") # Table S9 (294.2036), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kactMT       <- fixed(936.0982947); label("Maximum memory helper T-cell activation rate (1/day)") # Table S9 (936.0983), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kprolAT      <- fixed(4.980621822); label("Maximum activated helper T-cell proliferation rate (1/day)") # Table S9 (4.9806), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    ...
    CCN          <- fixed(66.99619766); label("Carrying capacity of 1 functional T cell for naive B-cell activation (unitless)") # Table S9 (66.9962), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kactMB       <- fixed(8.66466075); label("Maximum memory B-cell activation rate (1/day)") # Table S9 (8.6647), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kprolABN     <- fixed(6.070584752); label("Maximum proliferation rate of activated B cells derived from naive B cells (1/day)") # Table S9 (6.0706), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kprolABM     <- fixed(6.259269873); label("Maximum proliferation rate of activated B cells derived from memory B cells (1/day)") # Table S9 (6.2593), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kln2blPC     <- fixed(40.23069493); label("Plasma-cell migration rate, lymph node to blood (1/day)") # Table S9 (40.2307), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kdtSP        <- fixed(0.1448014475); label("Short-lived plasma-cell death rate (1/day)") # Table S9 (0.1448), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    kdtLP        <- fixed(0.01429613873); label("Long-lived plasma-cell death rate (1/day)") # Table S9 (0.0143), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    delayB       <- fixed(3.751953026e-08); label("Activated B-cell migration rate into the germinal centre (1/day)") # Table S9 (3.7520e-08), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
    ...
    kdegAb       <- fixed(0.09312138325); label("Antibody elimination rate in blood (1/day)") # Table S9 (0.0931), the column headed 'Calibration with Moderna data'; that header is transposed with the over-60 header -- see vignette Errata
