Wei_2026_cefquinome_enrofloxacin_lung_aucmic <- function() {
  description <- paste0(
    ...
    "carries the fit driven by the AUC72h/MIC (combine) index computed from lung PK data. ",
    ...
    "Wei 2026's Emax = 0.000 onto the canonical `e0` (effect at zero exposure) and Wei 2026's ",
    "E0 = -3.956 onto the canonical `emax` (maximal drug effect), and writes the algebraically ",
    "identical E = e0 - (e0 - emax) * Ce^N / (Ce^N + EC50^N). No value is altered by the remapping. ",
    "Parameters from Wei 2026 Table 6, lung / AUC72h/MIC (combine) column: e0 = 0.000, emax = -3.956 ",
    "log10 CFU/mL, EC50 = 1052.067, Hill N = 1.670 (R^2 = 0.795, AIC = 21.163). ",
    ...
    "covariate AUCMIC_CEFQ. That covariate carries the PK/PD index as an ALREADY-FORMED RATIO rather ",
    ...
    "gives lung MIC(combine) = 0.0168 ug/mL for the three single-dose arms but 0.0085 ug/mL for the split-dose arms; Wei 2026 prints these rounded as 0.017 and 0.008, values that do NOT reproduce its own Table 5. See the vignette Errata. ",
    ...
    "typical-value simulation. Sibling models from the same paper: Wei_2026_cefquinome_enrofloxacin_plasma_cmaxmic, Wei_2026_cefquinome_enrofloxacin_plasma_aucmic, Wei_2026_cefquinome_enrofloxacin_plasma_tmic, Wei_2026_cefquinome_enrofloxacin_lung_cmaxmic and Wei_2026_cefquinome_enrofloxacin_lung_tmic."
  )
  reference <- paste(
    ...
    "PK/PD'. Parameter estimates: Table 6, lung / AUC72h/MIC (combine) column.",
  ...
  units <- list(
    time = "h",
    dosing = "h (cefquinome AUC72h/MIC (combine) index, supplied as a covariate)",
    concentration = "log10 CFU/mL (observation)"
  )

  depends <- c("AUCMIC_CEFQ")
  ...
  covariateData <- list(
    AUCMIC_CEFQ = list(
      ...
      notes              = paste0(
        ...
        "lung MIC(combine) = 0.0168 ug/mL for the three single-dose arms but 0.0085 ug/mL for the split-dose arms; Wei 2026 prints these rounded as 0.017 and 0.008, values that do NOT reproduce its own Table 5. See the vignette Errata. ",
  ...
  ini({
    ...
    # matrix: lung   index: AUC72h/MIC (combine)
    # =================================================================
    ...
    #   e0   <- Wei 2026's Emax = 0.000   (effect at zero exposure)
    #   emax <- Wei 2026's E0   = -3.956  (maximal drug effect)
    ...
    e0 <- 0.000
    label("Difference in lung K. pneumoniae load from the blank control at zero exposure (log10 CFU/mL)")  # Wei 2026 Table 6, lung block, AUC72h/MIC (combine) column, "E max" = 0.000 (defined in Materials and methods as the blank-control change)
    emax <- -3.956
    label("Maximal difference in lung K. pneumoniae load from the blank control (log10 CFU/mL; negative = reduction)")  # Wei 2026 Table 6, lung block, AUC72h/MIC (combine) column, "E 0" = -3.956 (defined in Materials and methods as the maximum antibacterial effect)
    lec50 <- log(1052.067)
    label("Log AUC72h/MIC (combine) index producing 50% of the maximal antibacterial effect EC50 (h)")  # Wei 2026 Table 6, lung block, AUC72h/MIC (combine) column, EC 50 = 1052.067
    lhill <- log(1.670)
    label("Log Hill coefficient N defining the steepness of the index-effect curve (unitless)")  # Wei 2026 Table 6, lung block, AUC72h/MIC (combine) column, Hill's slope = 1.670

    # =================================================================
    # Residual error
    # =================================================================
    # Wei 2026 Table 6 reports the correlation coefficient (R^2 = 0.795)
    # and the AIC (21.163) of this fit and gives no residual standard
