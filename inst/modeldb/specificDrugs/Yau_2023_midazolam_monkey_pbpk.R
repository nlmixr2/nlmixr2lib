Yau_2023_midazolam_monkey_pbpk <- function() {
  description <- paste(
    "PBPK (simplified whole-body, 14 compartments). Preclinical",
    "(cynomolgus monkey). Midazolam disposition in monkey described by",
    "the Yau 2023 simplified physiologically based pharmacokinetic model",
    "with four common tissue Kpu scaling factors (Model 3D: k-means",
    "clustering). Thirteen perfusion-limited tissues drain to a central",
    "state lumping arterial blood, venous blood and lung at quasi-steady",
    "state; unbound tissue:plasma partition coefficients come from the",
    "Rodgers and Rowland weak-base equation scaled by four estimated",
    "group scalars. Total blood clearance was fixed to the observed",
    "6.4 L/h. Model 3D was one of the two best-fitting monkey models for",
    "midazolam and had the lowest BIC of the six scalar models,",
    "recovering a Vss,b of 10.31 L against an observed 8.8 L.",
    sep = " "
  )
  reference <- paste(
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "the observed blood clearance 6.4 L/h."
    )
  )
  ...
  ini({
    ...
    # value").
    lcl <- fixed(log(6.4)); label("Total blood clearance CL (L/h)")
    # ^ Yau 2023 Results, Monkey: "Using a blood CL of 9.97 L/h, 6.4 L/h"

    # Tissue Kpu scaling factors -- Model 3D (four scalars,
    # k-means clustering). Estimated on the log scale in NONMEM.
    lsf1 <- log(0.324); label("Kpu scalar group 1 (bone, brain, muscle, pancreas, rest of body)")
    # ^ Yau 2023 Table S8, Model 3D
    lsf2 <- log(21.22); label("Kpu scalar group 2 (kidney, spleen, liver)")
    # ^ Yau 2023 Table S8, Model 3D
    lsf3 <- log(0.203); label("Kpu scalar group 3 (skin, lung, gut, stomach, heart)")
    # ^ Yau 2023 Table S8, Model 3D
    lsf4 <- log(1.878); label("Kpu scalar group 4 (adipose)")
    # ^ Yau 2023 Table S8, Model 3D

    # Between-subject variability on clearance, reported as %CV;
    # omega^2 = log(CV^2 + 1) per the Table S7 footnote.
    etalcl ~ 0.0456   # 21.6% CV

    propSd <- 0.315; label("Proportional residual error (fraction)")
    # ^ Yau 2023 Table S8, Model 3D proportional error 31.5% (RSE 21.5%)
  })

  model({
    # ===== Drug physicochemistry (Yau 2023 Table S1) =====
    logp <- 3.13   # octanol-water partition coefficient
    pka  <- 5.88   # acid dissociation constant (basic pKa < 7)
    fup  <- 0.0603   # fraction unbound in plasma, monkey
    bp   <- 0.594   # blood:plasma ratio, monkey
    fe   <- 0   # fraction excreted unchanged in urine, monkey
    fub  <- fup / bp   # fraction unbound in blood

    cl <- exp(lcl + etalcl)

    # ===== 5 kg reference cynomolgus monkey physiology =====
    # Source: lead paper Table S6 (the only source that covers monkey).
    # Blood flows tabulated in mL/min; converted to L/h (x 0.06).
    q_co       <- 785.8 * 0.06   # cardiac output = arterial = venous = lung flow
    q_adipose   <- 19.6 * 0.06
    q_bone      <- 98.2 * 0.06
    q_brain     <- 51.9 * 0.06
    q_gut       <- 90.4 * 0.06
    ...
    v_liver     <- 118.5 * 0.001
    v_lung      <- 41.9 * 0.001
    v_muscle    <- 2535.5 * 0.001
    v_other     <- 130.7 * 0.001
    v_pancreas  <- 9.6 * 0.001
    v_skin      <- 487 * 0.001
    v_spleen    <- 3.3 * 0.001
    v_stomach   <- 23.8 * 0.001
    v_venous    <- 172.2 * 0.001
    ...
    q_hv <- q_ha + q_pv
    # For this reference individual q_pv = 7.404 L/h, q_ha = 2.358 L/h and
    # total hepatic blood flow q_hv = 9.762 L/h.

    # ===== Tissue composition (Yau 2023 Table S5) =====
    ...
    vssb <- (kpu_lung     * sf3 * v_lung +
            ...
            v_arterial + v_venous
    # Published Vss,b for this fit: 10.31 L.
