Forder_2019_sm3CalcitriolQuantumDot_earlyIBC_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of quantum dots conjugated with calcitriol and the SM3 anti-MUC1 antibody (SM3 CalQD) in mice with early-stage inflammatory breast cancer (IBC).",
    ...
    "reported, so the model is for typical-value simulation only. CAUTION: the Table 3 early-stage SM3 CalQD partition coefficients are identical to the Table S2 ENLARGED-spleen values and reproduce the Table 2 data exactly only with a doubled spleen volume; with the regular spleen volume used here they over-predict the Table 2 intensities by ~3 percent. Reproduced as printed; see the vignette Errata.",
    sep = " "
  )
  reference <- paste(
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, Early-stage tumor, SM3 CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(59.1); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
    lkp_liver <- log(92.6); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
    lkp_spleen <- log(91.5); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
    lkp_lung <- log(38.1); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
    lkp_tumor <- log(97.4); label("Log tumor:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
    lkp_breast <- log(1.83); label("Log breast:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, SM3 CalQD
