Sime_2019b_tazobactam <- function() {
  description <- paste(
    "Four-compartment intravenous population PK model for UNBOUND tazobactam in",
    ...
    "modellib('Sime_2019b_ceftolozane') for the partner component of the fixed",
  ...
  reference <- paste(
    ...
    "Structure: Figure 1. All parameter values: Table 3, 'Tazobactam' columns.",
    sep = " "
  )
  ...
  compartmentData <- list(
    ...
    postfilter = list(analyte = "tazobactam", units = "mg", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "tazobactam", units = "mg", specimen = "plasma", verified = TRUE),
    circuit_dialysate = list(analyte = "tazobactam", units = "mg", specimen = "dialysate", verified = TRUE)
  )
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "start of infusion. Unbound concentrations by UHPLC-MS/MS (tazobactam",
      "calibrated 0.5-100 mg/L).",
  ...
  ini({
    # =====================================================================
    # STRUCTURAL PARAMETERS -- Sime 2019b Table 3, 'Tazobactam Mean'
    # column. Each value is the MEAN of the NPAG support-point
    # distribution; the SD column is the between-subject spread (IIV), not
    # uncertainty. As in the sibling Sime_2019_tazobactam.R (same group,
    ...
    # Sime_2019_tazobactam.R and Hughes_2024_vancomycin_nonparametric.R.
    # The NPAG joint density is not published, so the marginals are
    # encoded as independent.
    # =====================================================================
    etalcl_crrt ~ 0.04031 # CV 0.603/2.973 = 0.2028
    etalcl ~ 0.06858 # CV 0.867/3.254 = 0.2664
    etalvc ~ 0.05335 # CV 6.603/28.206 = 0.2341
    etalvpostfilter ~ 0.42774 # CV 14.382/19.685 = 0.7306
    etalk12 ~ 0.83001 # CV 0.638/0.561 = 1.1373
    etalk21 ~ 0.69315 # CV 1.01/1.01 = 1.0000
    etalkd ~ 0.08891 # CV 0.483/1.584 = 0.3049
    etalvdialysate ~ 0.30600 # CV 1.561/2.609 = 0.5983
    etalq ~ 0.65703 # CV 2.455/2.547 = 0.9639
    etalqpostfilter ~ 0.10536 # CV 1.204/3.612 = 0.3333
    etalvp ~ 0.16102 # CV 32.267/77.196 = 0.4180
