Masters_2022_avelumab <- function() {
  ...
  ini({
    ...
    #   CL(t) = CL_baseline * (1 + imax * t^cl_time_hill / (T50^cl_time_hill + t^cl_time_hill))
    ...
    lcl_time_max  <- log(0.08533);      label("log|imax| - magnitude of the fractional change in CL at t >> T50 (unitless)") # Masters 2022 Table 1: theta_Imax = -0.08533 (sign applied in model())
    lcl_t50   <- log(99.24);        label("log T50 - time at which half of imax is reached (days)")       # Masters 2022 Table 1: theta_T50 = 99.24 days
    lcl_time_hill <- log(2.086);        label("log cl_time_hill - Hill shape coefficient of the time-on-CL function (unitless)")    # Masters 2022 Table 1: theta_gamma = 2.086
    ...
    etalcl_time_max ~ 0.1052  # Masters 2022 Table 1: omega^2_Imax = 0.1052
  ...
  model({
    ...
    cl     <- cl_base * (1 + cl_time_max_i * t^cl_time_hill / (cl_t50^cl_time_hill + t^cl_time_hill))
