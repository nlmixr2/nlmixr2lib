Chen_2024_kaempferol3OGlucoside_rat <- function() {
  description <- paste(
    "Preclinical (rat). Two-compartment intravenous pharmacokinetic model for",
    "kaempferol-3-O-glucoside (K-3-G), one of eight constituents of guhong",
    ...
    "kaempferol-3-O-glucoside dose is the GHI volume dose times its content in",
    "GHI (8.7 ug/mL), i.e. 21.75 ug/kg, 43.5 ug/kg, 87 ug/kg. Disposition was",
    "fitted separately in each dose group with Drug and Statistics (DAS)",
    ...
    "Direct-effect sigmoid-Emax models link the kaempferol-3-O-glucoside plasma",
    "concentration to the GHI-minus-model-group difference in cardiac troponin",
    "I (cTn I) (E = Emax * C^gamma / (ED50^gamma + C^gamma); Tables 14). Chen",
    "2024 fitted a PK/PD model only for the analyte/biomarker/dose combinations",
    "whose PLSR coefficient was negative, so the effect of an unfitted",
    "combination is returned as zero rather than extrapolated. No",
  ...
  compartmentData <- list(
    central     = list(analyte = "kaempferol-3-O-glucoside", units = "ug", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "kaempferol-3-O-glucoside", units = "ug", specimen = "plasma", verified = TRUE)
  )
  ...
  population <- list(
    ...
    notes         = paste(
      ...
      "hydrochloride 0.95 mg/kg positive control). The PK parameters in Table 5",
  ...
  ini({
    ...
    # (Chen 2024 Table 5; V1 = central volume, V2 = peripheral volume,
    #  CL1 = elimination clearance, Q = intercompartmental clearance).
    # --- GHI-L (2.5 mL/kg GHI; kaempferol-3-O-glucoside 21.75 ug/kg) ---
    lvc_ghil <- fixed(log(0.54)); label("Central volume V1, GHI-L (L/kg)")  # Table 5, V1, GHI-L
    lvp_ghil <- fixed(log(2.822)); label("Peripheral volume V2, GHI-L (L/kg)")  # Table 5, V2, GHI-L
    lcl_ghil <- fixed(log(5.255)); label("Elimination clearance CL1, GHI-L (L/h/kg)")  # Table 5, CL1, GHI-L
    lq_ghil <- fixed(log(7.998)); label("Intercompartmental clearance Q, GHI-L (L/h/kg)")  # Table 5, Q, GHI-L
    # --- GHI-M (5 mL/kg GHI; kaempferol-3-O-glucoside 43.5 ug/kg) ---
    lvc_ghim <- fixed(log(0.59)); label("Central volume V1, GHI-M (L/kg)")  # Table 5, V1, GHI-M
    lvp_ghim <- fixed(log(1.87)); label("Peripheral volume V2, GHI-M (L/kg)")  # Table 5, V2, GHI-M
    lcl_ghim <- fixed(log(4.123)); label("Elimination clearance CL1, GHI-M (L/h/kg)")  # Table 5, CL1, GHI-M
    lq_ghim <- fixed(log(4.764)); label("Intercompartmental clearance Q, GHI-M (L/h/kg)")  # Table 5, Q, GHI-M
    # --- GHI-H (10 mL/kg GHI; kaempferol-3-O-glucoside 87 ug/kg) ---
    lvc_ghih <- fixed(log(0.475)); label("Central volume V1, GHI-H (L/kg)")  # Table 5, V1, GHI-H
    lvp_ghih <- fixed(log(1.419)); label("Peripheral volume V2, GHI-H (L/kg)")  # Table 5, V2, GHI-H
    lcl_ghih <- fixed(log(4.4558)); label("Elimination clearance CL1, GHI-H (L/h/kg)")  # Table 5, CL1, GHI-H
    lq_ghih <- fixed(log(8.473)); label("Intercompartmental clearance Q, GHI-H (L/h/kg)")  # Table 5, Q, GHI-H
    ...
    lemax_ctni_ghil <- fixed(log(4.625)); label("Maximum effect Emax on dcTn I, GHI-L (ng/mL)")  # Table 14, GHI-L, kaempferol-3-O-glucoside
    lec50_ctni_ghil <- fixed(log(1.17)); label("Half-maximal concentration ED50 on dcTn I, GHI-L (ng/mL)")  # Table 14, GHI-L, kaempferol-3-O-glucoside
    lhill_ctni_ghil <- fixed(log(0.01)); label("Sigmoidicity gamma on dcTn I, GHI-L (unitless)")  # Table 14, GHI-L, kaempferol-3-O-glucoside
    # GHI-M: no K-3-G/CTNI equation reported (PLSR coefficient not negative).
    # GHI-H: no K-3-G/CTNI equation reported (PLSR coefficient not negative).

    addSd <- fixed(0); label("Additive residual SD on kaempferol-3-O-glucoside plasma concentration (ng/mL; not reported)")  # Chen 2024 reports no residual-error model
  ...
  model({
    ...
    # dCTNI is 0 in dose groups for which Chen 2024 fitted no K-3-G equation.
