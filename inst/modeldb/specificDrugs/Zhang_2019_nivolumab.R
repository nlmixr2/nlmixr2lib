Zhang_2019_nivolumab <- function() {
  ...
  ini({
    ...
    # Time-varying CL Hill-cl_time_max function (Zhang 2019 final-model equation):
    ...
    # Additive covariate effects on cl_time_max (Zhang 2019 cl_time_max equation:
    #   cl_time_max_i = Emax_REF + Emax_PS * I[PS>0] + Emax_IPICO * I[IPICO=1] + eta_Emax_i).
    e_ecog_cl_time_max  <- -0.138;  label("Additive effect of ECOG PS > 0 on cl_time_max (unitless)")          # Zhang 2019 Table 2: cl_time_max PS = -0.138
    e_ipico_cl_time_max <- -0.0668; label("Additive effect of any ipilimumab coadministration on cl_time_max (unitless)") # Zhang 2019 Table 2: cl_time_max IPICO = -0.0668
    ...
    # cl_time_max has an independent additive (not log-normal) eta on the linear scale.
    ...
    etacl_time_max  ~ 0.0874  # Zhang 2019 Table 2: omega^2_Emax = 0.0874 (additive eta on linear-scale cl_time_max)
  ...
  model({
    ...
    # Time-varying CL: Hill-cl_time_max function of time since first dose.
    cl_t50  <- exp(lcl_t50)
    cl_time_hill <- exp(lcl_time_hill)
    cl_time_max_i <- cl_time_max + e_ecog_cl_time_max * ECOG_GE1 + e_ipico_cl_time_max * CONMED_IPI_ANY + etacl_time_max
