Fan_2025_iron_mouse_ironadequate_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 14-compartment, perfusion-limited; NONMEM ADVAN15).",
    "Preclinical (mouse, iron-adequate diet).",
    ...
    "iron-adequate diet column of Table 2; organ volumes and blood flows are",
  ...
  ini({
    # ---- Estimated iron-disposition parameters: Table 2, iron-adequate diet column ----
    # RSE% from Table 2 is given after each value.
    lcl <- log(1.647e-4); label("Unavoidable physiologic iron loss clearance from plasma (L/h)")  # Table 2 CLloss = 1.647 x 10^-4 L/h (RSE 0.016%)
    lkp_adipose <- log(0.258); label("Fat (adipose)-to-plasma partition coefficient (unitless)")  # Table 2 KPfat (RSE 0.011%)
    lkp_bone    <- log(7.703); label("Bone-to-plasma partition coefficient (unitless)")  # Table 2 KPbon (RSE 0.035%)
    lkp_brain   <- log(1.087); label("Brain-to-plasma partition coefficient (unitless)")  # Table 2 KPbra (RSE 0.011%)
    lkp_gut     <- log(5.349); label("Gut-to-plasma partition coefficient (unitless)")  # Table 2 KPgut (RSE 0.037%)
    lkp_heart   <- log(13.83); label("Heart-to-plasma partition coefficient (unitless)")  # Table 2 KPhea (RSE 0.096%)
    lkp_kidney  <- log(9.223); label("Kidney-to-plasma partition coefficient (unitless)")  # Table 2 KPkid (RSE 0.013%)
    lkp_liver   <- log(18.11); label("Liver-to-plasma partition coefficient (unitless)")  # Table 2 KPliv (RSE 0.081%)
    lkp_lung    <- log(13.7); label("Lung-to-plasma partition coefficient (unitless)")  # Table 2 KPlun (RSE 0.001%)
    lkp_muscle  <- log(1.764); label("Muscle-to-plasma partition coefficient (unitless)")  # Table 2 KPmus (RSE 0.014%)
    lkp_other   <- log(2.30e-9); label("Remainder (rest of body)-to-plasma partition coefficient (unitless)")  # Table 2 KPrem (RSE 2.78%)
    lkp_skin    <- log(4.651); label("Skin-to-plasma partition coefficient (unitless)")  # Table 2 KPski (RSE 0.010%)
    lkp_spleen  <- log(16.34); label("Spleen-to-plasma partition coefficient (unitless)")  # Table 2 KPspl (RSE 0.068%)
    lq_bone_rbc <- log(0.217e-3); label("Erythropoietic iron utilisation flow, bone to red cells (L/h)")  # Table 2 QE (RSE 0.012%)
    lmtt_rbc <- log(34.44); label("Mean red-blood-cell lifespan (h)")  # Table 2 TRBC (RSE 0.021%)
