Shen_2026_shikimicAcid_broiler_taoc <- function() {
  description <- paste(
    ...
    "on serum total antioxidant capacity (T-AOC) in 5-week-old yellow-feathered broilers after a",
    ...
    "modellib('Shen_2026_shikimicAcid_broiler_sod') and",
  ...
  compartmentData <- list(
    ...
    taoc = list(
      analyte = "total antioxidant capacity (T-AOC)", units = "umol/mL", specimen = "serum",
      verified = TRUE
    )
  )
  ...
  ini({
    ...
    # The T-AOC row is carried here; its model was selected on AIC -15.22 +/- 9.52.
    #
    # Shen 2026 defines E0 = kin/kout, and that ratio, 0.37 / 0.14 = 2.643 umol/mL, reproduces
    # the 0 h bar of Figure 3A (digitised at 2.675 umol/mL). Because Table 5 states no
    ...
    lke0 <- fixed(log(0.44)); label("Effect-compartment equilibration rate constant ke0 (1/h)")  # Table 5, T-AOC row: ke0 = 0.44 +/- 0.42
    lslope <- fixed(log(0.01)); label("Linear stimulation coefficient S of SA on T-AOC production (mL/ug)")  # Table 5, T-AOC row: S = 0.01 +/- 0.007
    lkin <- fixed(log(0.37)); label("T-AOC zero-order production rate kin (umol/mL/h)")  # Table 5, T-AOC row: kin = 0.37 +/- 0.16
    lkout <- fixed(log(0.14)); label("T-AOC first-order loss rate constant kout (1/h)")  # Table 5, T-AOC row: kout = 0.14 +/- 0.06
    ...
    addSd <- fixed(0); label("Additive residual SD on serum T-AOC (umol/mL; final value not reported)")  # Shen 2026 reports only the initial SD of 1.0
  })
  ...
  model({
    ...
    d/dt(taoc) <- kin * (1 + slope * effect) - kout * taoc
    taoc(0) <- kin / kout
