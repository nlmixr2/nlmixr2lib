Yau_2023_midazolam_rat_pbpk <- function() {
  description <- paste(
    "PBPK (simplified whole-body, 14 compartments). Preclinical (rat).",
    "Midazolam disposition in rat described by the Yau 2023 simplified",
    ...
    "composition). Thirteen perfusion-limited tissues drain to a central",
    ...
    "1.30 L/h, which exceeds total hepatic blood flow in the 250 g",
    "reference rat, so hepatic extraction is capped at 0.99 and the",
    "balance assigned to the renal route exactly as in the source code.",
    "Model 3D was the best-fitting rat model for midazolam and recovered",
    "a Vss,b of 0.54 L against an observed 0.64 L. Fitted to six",
    "digitised literature rat studies.",
    sep = " "
  )
  reference <- paste(
  ...
  population <- list(
    species        = "rat (Wistar, Sprague-Dawley and Holtzman)",
    ...
    dose_range     = "0.1 to 10 mg/kg intravenous, bolus or 5 to 15 min infusion (Table S2)",
    regions        = NA_character_,
    notes          = paste(
      "Six published rat intravenous studies (Yau 2023 Table S2)",
      "contributing average plasma concentration-time profiles digitised",
      "with WebPlotDigitizer. All reported animals were male. Observed",
      "rat Vss,b was 0.64 L and the observed blood clearance 1.30 L/h."
    )
  )
  ...
  ini({
    ...
    # value").
    lcl <- fixed(log(1.3)); label("Total blood clearance CL (L/h)")
    # ^ Yau 2023 Results, Rat: "Using a blood CL of 1.30 and 0.824 L/h"

    # Tissue Kpu scaling factors -- Model 3D (four scalars,
    # k-means clustering). Estimated on the log scale in NONMEM.
    lsf1 <- log(1.17); label("Kpu scalar group 1 (bone, brain, muscle, pancreas, rest of body)")
    # ^ Yau 2023 Table S7, Model 3D
    lsf2 <- log(5.18); label("Kpu scalar group 2 (kidney, spleen, liver)")
    # ^ Yau 2023 Table S7, Model 3D
    lsf3 <- log(0.761); label("Kpu scalar group 3 (skin, lung, gut, stomach, heart)")
    # ^ Yau 2023 Table S7, Model 3D
    lsf4 <- log(0.487); label("Kpu scalar group 4 (adipose)")
    # ^ Yau 2023 Table S7, Model 3D

    # Between-subject variability on clearance, reported as %CV;
    # omega^2 = log(CV^2 + 1) per the Table S7 footnote.
    etalcl ~ 0.084533   # 29.7% CV

    propSd <- 0.258; label("Proportional residual error (fraction)")
    # ^ Yau 2023 Table S7, Model 3D proportional error 25.8% (RSE 17.3%)
  })
  ...
  model({
    ...
    fup  <- 0.054   # fraction unbound in plasma, rat
    bp   <- 0.742   # blood:plasma ratio, rat
    fe   <- 0   # fraction excreted unchanged in urine, rat
    ...
    vssb <- (kpu_lung     * sf3 * v_lung +
            ...
            v_arterial + v_venous
    # Published Vss,b for this fit: 0.54 L.
