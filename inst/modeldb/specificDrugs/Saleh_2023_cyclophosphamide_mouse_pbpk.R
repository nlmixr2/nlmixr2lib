Saleh_2023_cyclophosphamide_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, CD1 nude).",
    "Nine-compartment CNS PBPK model predicting unbound cyclophosphamide",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 130 mg/kg intraperitoneal dose. The CNS structure is the mouse",
    ...
    "0.339 (estimated by the authors; Table V). The plasma PK model is the",
    "empirical two-compartment model of Table IV (ref [7]) and acts",
  ...
  compartmentData <- list(
    central              = list(analyte = "cyclophosphamide", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1          = list(analyte = "cyclophosphamide", units = "ng", specimen = "plasma", verified = TRUE),
    brain_vascular       = list(analyte = "cyclophosphamide", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf            = list(analyte = "cyclophosphamide", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_cell_membrane  = list(analyte = "cyclophosphamide", units = "ng", specimen = "tissue", verified = TRUE),
    brain_icf            = list(analyte = "cyclophosphamide", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome       = list(analyte = "cyclophosphamide", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv         = list(analyte = "cyclophosphamide", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv        = list(analyte = "cyclophosphamide", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm         = list(analyte = "cyclophosphamide", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas        = list(analyte = "cyclophosphamide", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range   = "130 mg/kg IP (single dose; Table IV)",
    notes        = paste(
      "Unbound plasma and microdialysis brain-ECF concentration-time",
      "data for cyclophosphamide came from Stewart lab, St. Jude Children's Research Hospital",
  ...
  ini({
    ...
    lcl <- log(2.09); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(18.53); label("Central compartment volume (mL)")  # Table IV
    lq  <- log(0.086); label("Intercompartmental clearance (mL/min)")  # Table IV
    lvp <- log(1.77); label("Peripheral compartment volume (mL)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(261); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(0.76); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(13.48); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.26); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.339); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (estimated)
    ...
    kpuu_lv <- fixed(0.339); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.339); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    etalcl ~ 0.012  # Table IV IIV on central clearance (variance)

    # ---- Residual unexplained variability: Table IV (reported as VARIANCES) ----
    propSd <- sqrt(0.058); label("Proportional residual error (fraction)")  # Table IV: 0.058 as variance
  })
  ...
  model({
    ...
    PHF_MV  <- 1 / (1 + 10^(pH_PL - pka))
    PHF_ECF <- 1 / (1 + 10^(pH_ECF - pka))
    PHF_ICF <- 1 / (1 + 10^(pH_ICF - pka))
    PHF_LYS <- 1 / (1 + 10^(pH_LYS - pka))
    PHF_CSF <- 1 / (1 + 10^(pH_CSF - pka))
