Qin_2025_ropeginterferon_wbc <- function() {
  ...
  ini({
    ...
    # ==================================================================
    lrbase    <- log(11.4)  ; label("Initial white blood cell count at the start of ropeg treatment, the initial condition of the white cell state (10^9/L)")  # Qin 2025 Table 3, WBC block: WBC0 11.4 (RSE 6.95%). The Discussion restates 12.1; see the conflict note above
    lrbase_ss <- log(6.72)  ; label("Drug-free steady-state white blood cell count the system relaxes toward (10^9/L)")                                       # Qin 2025 Table 3, WBC block: WBCss 6.72 (RSE 6.42%). The Discussion restates 7.32; see the conflict note above
    lic50     <- log(152)   ; label("Total serum ropeg concentration producing half of Imax on white cell production (W, ng/mL)")                               # Qin 2025 Table 3, WBC block: IC50,W 152 (RSE 1.18%). The Discussion restates 124; see the conflict note above. This is the HIGHEST IC50 of the three hematologic endpoints
    limax     <- fixed(log(1)) ; label("Maximum fractional inhibition of white cell production by ropeg (W, unitless fraction of kin)")                         # Qin 2025 Table 3, WBC block: Imax,W 1 (FIX). Complete suppression of production is attainable
    lkout     <- log(0.0475); label("First-order elimination rate constant of circulating white blood cells (W, 1/day)")                                        # Qin 2025 Table 3, WBC block: kout,W 0.0475 (RSE 5.28%), printed as h-1; per day, which gives a 15-day white cell pool half-life (see the vignette Errata)
