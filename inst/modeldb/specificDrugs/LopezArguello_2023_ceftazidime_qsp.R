LopezArguello_2023_ceftazidime_qsp <- function() {
  ...
  compartmentData <- list(
    npbp1a = list(analyte = "covalently-bound ceftazidime to PBP1a", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp1b = list(analyte = "covalently-bound ceftazidime to PBP1b", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp2  = list(analyte = "covalently-bound ceftazidime to PBP2", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp3  = list(analyte = "covalently-bound ceftazidime to PBP3", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp4  = list(analyte = "covalently-bound ceftazidime to PBP4", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp56 = list(analyte = "covalently-bound ceftazidime to PBPs 5/6", units = NA_character_, specimen = "bile", verified = FALSE),
    nperi  = list(analyte = "free ceftazidime in periplasm", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_CAZ_MGL = list(
      description = "Static extracellular ceftazidime concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; ceftazidime MIC 1 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "ceftazidime at a static extracellular concentration of 2 mg/L (2x the MIC of 1 mg/L); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 5.18; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, ceftazidime: 5.18 (SE 35.2%)
    # The studied extracellular concentration enters as the CONC_CAZ_MGL covariate
    # (Table 1: ceftazidime studied at 2 mg/L = 2x the MIC of 1 mg/L).
    ...
    k2_1a <- 218;  label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, ceftazidime PBP1a: 218
    k2_1b <- 80.6; label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, ceftazidime PBP1b: 80.6
    k2_2 <- 67.6;  label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, ceftazidime PBP2: 67.6
    k2_3 <- 563;   label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, ceftazidime PBP3: 563
    k2_4 <- 69.3;  label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, ceftazidime PBP4: 69.3
    k2_5 <- 3.51;  label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, ceftazidime PBP5/6: 3.51
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_CAZ_MGL
