Nguyen_2020_taldefgrobepAlfa_qsp <- function() {
  description <- paste(
    "QSP. Myostatin / activin A / ActRIIB systems-pharmacology model (Nguyen",
    "2020) parameterised for the anti-myostatin adnectin BMS-986089",
    "(taldefgrobep alfa) in healthy adults.",
    ...
    "pituitary.",
    "Compound PK and binding parameters were fitted to digitised clinical",
    "PK and free-myostatin data, and the muscle-growth parameters (Vmax,",
    "RO50, Hill, kdeg) were estimated on this compound's thigh-muscle-volume",
    "data (Figure 2); the FSH parameters are the ACE-031 estimates. The",
    "adnectin does not bind activin (koff for activin set arbitrarily high).",
  ...
  population <- list(
    ...
    n_subjects = NA,
    n_studies = 1,
    disease_state = "healthy adult volunteers (single-dose adnectin study, digitised from Jacobsen 2016)",
    dose_range = "single subcutaneous doses (four dose groups, Figure 2)",
    notes = paste(
      "Parameters were estimated by fitting the QSP model to digitised",
      "aggregate PK, free-myostatin and muscle-volume data (Figure 2); the",
      "number of subjects is not reported. Doses in mg (mg/kg x 71 kg)."
    )
  )
  ...
  ini({
    ...
    # ---- anti-myostatin adnectin compound parameters (Table 2, adnectin column) ------------
    mw_drug <- fixed(76000); label("Molecular weight of anti-myostatin adnectin (g/mol)") # Table 2, adnectin column MW_Drug = 76,000
    lka <- fixed(log(0.029)); label("Subcutaneous absorption rate constant (1/h)") # Table 2, adnectin column k_a = 0.029
    lfdepot <- fixed(log(0.7)); label("Subcutaneous bioavailability (fraction)") # Table 2, adnectin column F = 0.7 (footnote b: typical s.c. value for therapeutic proteins)
    vother <- fixed(10.3); label("Volume of other tissues (L)") # Table 2, adnectin column V_other = 10.3 L
    kdeg_drug <- fixed(0.0016); label("Degradation rate of free anti-myostatin adnectin, all spaces (1/h)") # Table 2, adnectin column k_deg_drug = 0.0016
    kdeg_md <- fixed(0.0016); label("Degradation rate of anti-myostatin adnectin-myostatin complex (1/h)") # Table 2, adnectin column k_deg_MD = 0.0016 (assumed ~ k_deg_drug)
    kdeg_ad <- fixed(0.0016); label("Degradation rate of anti-myostatin adnectin-activin A complex (1/h)") # Table 2, adnectin column k_deg_AD = 0.0016 (assumed ~ k_deg_drug)
    ...
    vmax_muscle <- fixed(0.005); label("Maximal rate of muscle-volume increase (%/h)") # Table 2 V_max_muscle = 0.005 (estimated on the adnectin muscle-volume data, Figure 2)
