Babel_2026_telisotuzumab_neuropathy <- function() {
  ...
  covariatesDataExcluded <- list(
    NEUROPATHY_HX = list(
      ...
      units = "(binary)",
      type = "binary",
      notes = "Listed in Babel 2026 Table S2 as a safety covariate of interest and the most clinically plausible predictor for this particular endpoint, but not retained: 'No covariates were found to have a significant effect on efficacy or safety'. No point estimate is reported. The remaining Table S2 safety covariates (age, sex, race, ethnicity, body weight, c-Met expression level, prior therapy, number of prior systemic therapies, liver metastasis at baseline, treatment-emergent ADA status and nAb status) were screened and dropped on the same basis."
    )
  )
