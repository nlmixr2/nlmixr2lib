Almquist_2015_Mig1_exp3 <- function() {
  description <- paste(
    ...
    "EXPERIMENT 3 (4% -> 1% glucose). Nuclear Mig1 (observed as Mig1-GFP",
    ...
    "see modellib('Almquist_2015_Mig1_exp1') and",
  ...
  population <- list(
    ...
    dose_range     = paste(
      ...
      "1% at time 0 (Almquist 2015 Table 1, Exp Nr 3). Up to 15",
      "fluorescence observations per cell."
    ),
    notes          = paste(
      "Almquist 2015 Table 1: experiment 3 used 46 cells shifted from 4% to",
      "1% glucose. This experiment has the strongest correlation structure",
      "of the four -- corr(eta2, eta3) = 0.86 between the random effects on",
      "k2 and k4 (Table 3). Five cells (#1, #2, #14, #26 and #29) sit apart",
      "from the rest in the empirical-Bayes-estimate scatter of Fig 3C, with",
      "more negative random effects and therefore slower dynamics; the",
      "authors inspected the raw images and found all five viable, with cell",
      "#29 smaller and with a less developed nucleus. Experiment 3 was the",
      "only one from which the standard-two-stage comparison in Table 4",
      "removed no outlier cells."
    )
  )

  ini({
    # =====================================================================
    # Fixed effects -- Almquist 2015 Table 2, column "Exp 3".
    ...
    # Almquist 2015 Table 2, Exp 3: Ms = 3.64 x 10^3 (RSE 2%).
    ...
    lkout_mig1 <- log(0.00592)
    label("First-order loss rate constant of nuclear Mig1, k2 (1/s)")
    # Almquist 2015 Table 2, Exp 3: k2 = 0.00592 /s (RSE 9%).
    ...
    # Almquist 2015 Table 1, Exp Nr 3, column "To": 1%.

    # =====================================================================
    # Random effects -- Almquist 2015 Table 3, Exp Nr 3 (the covariance
    ...
    # Substituting the Table 2 Exp 3 values
    #   w11 = 0.0624, w12 = 0.0568, w13 = -0.0649,
    #   w22 = 0.313,  w23 = 0.526,  w33 = 0.567
    # reproduces every printed entry of Table 3 Exp Nr 3 exactly:
    #   Omega11 = 0.0113320 (Table 3: 0.011)
    #   Omega21 = -0.0163590 (Table 3: -0.016)
    #   Omega22 = 0.3746450 (Table 3: 0.37)
    #   Omega31 = -0.0367983 (Table 3: -0.037)
    #   Omega32 = 0.2982420 (Table 3: 0.3)
    #   Omega33 = 0.3214890 (Table 3: 0.32)
    # and the implied correlations -0.25 (eta1,eta2), -0.61 (eta1,eta3) and
    # 0.86 (eta2,eta3) match the Table 3 correlation block. The construction
    # Omega = U %*% t(U) guarantees positive definiteness.
    # =====================================================================

    etalrbase + etalkout_mig1 + etalkout_adapt ~
      c( 0.0113320100,
        -0.0163590000, 0.3746450000,
        -0.0367983000, 0.2982420000, 0.3214890000)

    # =====================================================================
    # Residual error -- Almquist 2015 Table 2, row "s", Exp 3.
    # =====================================================================

    addSd_mig1 <- sqrt(20.8e3)
    label("Additive residual SD on nuclear Mig1 intensity (a.u.)")
    # Almquist 2015 Table 2, Exp 3: s = 20.8 x 10^3 (RSE 6%). The paper
    # defines the observation model as y_t = Mig1(t) + e_t with
    # "e_t ~ N(0, s), with s denoting the VARIANCE of the measurement
    # error", so the nlmixr2 additive SD is sqrt(s) = 144.2 a.u.
