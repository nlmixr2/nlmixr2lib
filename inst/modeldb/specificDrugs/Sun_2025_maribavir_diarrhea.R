Sun_2025_maribavir_diarrhea <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent diarrhea as a function of the maribavir AUC on ",
    ...
    "claim of the two. The exposure slope is +0.0589 per 10 ug*h/mL. ",
    "There is no PK layer and no ODE: the exposure metric is supplied as ",
    "a data column, derived in the source analysis from the companion ",
    "population PK model packaged as Sun_2025_maribavir. No ",
    "between-subject random effect and no residual error are estimated ",
    "(Bernoulli likelihood). Fifteen companion exposure-response models ",
    "in the Sun_2025_maribavir_* family."
  )
  reference <- paste(
  ...
  units <- list(
    ...
    concentration = "prob_diarrhea (probability of treatment-emergent diarrhea, 0-1; also logit_diarrhea)"
  )
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0589 per 10 ug*h/mL (odds",
    ...
    HCT_TCELL_INFUSION = list(
      ...
      notes              = paste(
        ...
        "Coefficient +1.74, on 9 positive subjects; the odds-ratio interval",
        "(1.15-28.4) barely excludes 1."
      ),
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary diarrhea records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel d (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -3.00; label("Logit of the diarrhea probability at zero maribavir exposure with all risk factors at their reference level (unitless logit)")  # Sun 2025 Figure S3 panel d (Supporting Information file s002), Intercept -3.00 (SE 0.394), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0589; label("Log-odds of diarrhea per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel d (Supporting Information file s002), Estimate 0.0589 (SE 0.0128), odds ratio 1.06 (1.03-1.09), p < 0.001

    # ----- Risk-factor effects on the logit -----
    e_hct_tcell_infusion_logit <- 1.74; label("Log-odds of diarrhea for a post-HCT T-cell infusion (unitless logit)")  # Sun 2025 Figure S3 panel d (Supporting Information file s002), T-cell infusion given post-HCT Estimate 1.74 (SE 0.818), odds ratio 5.72 (1.15-28.4), p = 0.033
    ...
    addSd_prob_diarrhea <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_diarrhea <- logit_ref +
      e_auc_logit * auc10 +
      e_hct_tcell_infusion_logit * HCT_TCELL_INFUSION

    prob_diarrhea <- expit(logit_diarrhea)

    # ----- Observation -----
    prob_diarrhea ~ add(addSd_prob_diarrhea)
  })
}
