Forder_2019_sm3CalcitriolQuantumDot_healthy_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of quantum dots conjugated with calcitriol and the SM3 anti-MUC1 antibody (SM3 CalQD) in healthy (tumor-free) mice.",
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "Schaefer 2012 (Forder 2019 Table 2, no tumor column): kidney 10.47, liver 17.18, lung 5.01, spleen 9.87.",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, No tumor, SM3 CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(34.8); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, No tumor, SM3 CalQD
    lkp_liver <- log(56.8); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, No tumor, SM3 CalQD
    lkp_spleen <- log(32.8); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, No tumor, SM3 CalQD
    lkp_lung <- log(16.6); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, No tumor, SM3 CalQD
