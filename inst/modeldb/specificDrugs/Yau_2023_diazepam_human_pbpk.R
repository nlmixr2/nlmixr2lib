Yau_2023_diazepam_human_pbpk <- function() {
  description <- paste(
    "PBPK (simplified whole-body, 14 compartments). Human-scaled",
    "projection. Diazepam disposition in human predicted by carrying the",
    "four tissue Kpu scaling factors estimated in rat (Yau 2023 Model 3D)",
    "across species and re-evaluating the Rodgers and Rowland weak-base",
    "equation with human tissue composition, human fu_p and human",
    "blood:plasma ratio, that is Kp_human = KpuRR_human * SF_rat * fu_p",
    "(paper Equation 2). Human blood flows and tissue volumes are the",
    "70 kg reference values of Table S6. This is the middle-out",
    "prediction the paper reports: it recovered a human Vss,b of 97 L",
    "against an observed 152 L, a 1.5-fold error, versus 3.7-fold for the",
    "traditional bottom-up whole-body PBPK approach. Scalars and",
    "clearance are all fixed, so the model is a typical-value forward",
    "simulation with no between-subject variability; total blood",
    "clearance is the observed human value of 3.71 L/h reported by the",
    "companion paper.",
    sep = " "
  )
  reference <- paste(
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "152 L used as the prediction target, and the paper simulated a",
      "16.1 h infusion of 10 mg to compare kinetic phases against a",
      "reference empirical compartmental model."
    )
  )
  ...
  ini({
    ...
    # value").
    lcl <- fixed(log(3.71)); label("Total blood clearance CL (L/h)")
    # ^ companion paper (doi:10.1002/psp4.12911) Results: "A clearance value of 3.71 L/h ... estimated in humans" (empirical reference model, i.e. the OBSERVED human CL this method fixes to); also Table 2 CLb for the 3-scalar fit

    # Tissue Kpu scaling factors -- Model 3D (four scalars,
    # k-means clustering). Estimated on the log scale in NONMEM.
    lsf1 <- fixed(log(3.2)); label("Kpu scalar group 1 (bone, brain, muscle, pancreas, rest of body)")
    # ^ Yau 2023 Table 2 Model 3D, estimated in rat and carried across species
    lsf2 <- fixed(log(23.5)); label("Kpu scalar group 2 (kidney, spleen, liver)")
    # ^ Yau 2023 Table 2 Model 3D, estimated in rat and carried across species
    lsf3 <- fixed(log(2.02)); label("Kpu scalar group 3 (skin, lung, gut, stomach, heart)")
    # ^ Yau 2023 Table 2 Model 3D, estimated in rat and carried across species
    lsf4 <- fixed(log(0.265)); label("Kpu scalar group 4 (adipose)")
    # ^ Yau 2023 Table 2 Model 3D, estimated in rat and carried across species

    propSd <- fixed(0.1); label("Proportional residual error (fraction)")
    # ^ not estimated for the human projection; placeholder, see vignette
  })
  ...
  model({
    ...
    fup  <- 0.009   # fraction unbound in plasma, human
    bp   <- 0.559   # blood:plasma ratio, human
    fe   <- 0.0005   # fraction excreted unchanged in urine, human
    fub  <- fup / bp   # fraction unbound in blood

    cl <- exp(lcl)

    # ===== 70 kg reference man physiology =====
    # Source: companion paper Table S1 (three decimal places; the lead
    # paper's Table S6 prints the same reference man to two, and its human
    # liver entry 1.11 is a transcription duplicate of the kidney 1.109).
    # Blood flows tabulated in L/min; converted to L/h (x 60).
    q_co       <- 5.839 * 60   # cardiac output = arterial = venous = lung flow
    q_adipose   <- 0.292 * 60
    q_bone      <- 0.292 * 60
    q_brain     <- 0.701 * 60
    q_gut       <- 0.526 * 60
    ...
    # human. Ratified by the maintainers; the same ruling keeps the twice-
    ...
    q_hv <- q_ha + q_pv
    # For this reference individual q_pv = 49.02 L/h, q_ha = 22.74 L/h and
    # total hepatic blood flow q_hv = 71.76 L/h.

    # ===== Tissue composition (Yau 2023 Table S4) =====
    ...
    vssb <- (kpu_lung     * sf3 * v_lung +
            ...
            v_arterial + v_venous
    # Published Vss,b for this fit: 97 L.
