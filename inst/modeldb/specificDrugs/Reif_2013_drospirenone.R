Reif_2013_drospirenone <- function() {
  ...
  ini({
    ...
    lcl <- log(3.52)
    label("Apparent oral clearance at the Week 3 occasion for a typical subject weighing 62 kg (L/h)")  # Appendix 2: TVCL_week3/F = 3.52 l/hour (RSE 0.98%)
    lvc <- log(51.6)
    label("Apparent central volume of distribution (L)")  # Appendix 2: V2/F = 51.6 L (RSE 3.64%)
    lvp <- log(204)
    label("Apparent peripheral volume of distribution (L)")  # Appendix 2: V3/F = 204 L (RSE 7.99%)
    lq <- log(17.5)
    label("Apparent inter-compartmental clearance (L/h)")  # Appendix 2: Q3/F = 17.5 l/hour (RSE 4.62%)
    ltlag <- log(0.372)
