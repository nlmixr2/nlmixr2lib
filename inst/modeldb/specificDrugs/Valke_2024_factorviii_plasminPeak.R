Valke_2024_factorviii_plasminPeak <- function() {
  description <- paste(
    "Sequential PK-PD model relating factor VIII (FVIII) activity to",
    "normalized plasmin peak height (% of normal pooled plasma) measured with",
    ...
    "is an inhibitory proportional-baseline Imax model, E = Ebase * (1 - Imax",
    "* C^n / (EC50^n + C^n)), with baseline 81.2% of normal pooled plasma and",
    "EC50 256 IU/dL, and Imax and the Hill coefficient both fixed at 1. FVIII",
    "replacement therefore suppresses the raised fibrinolytic activity of",
    "untreated severe hemophilia A. Inter-individual variability is carried on",
    "the baseline (26.4%); residual error on the PD readout is proportional",
    "(16.3%).",
    sep = " "
  )
  reference <- paste(
  ...
  ini({
    ...
    # Normalized plasmin peak height, inhibitory proportional-baseline model
    # (Valke 2024 Supplementary Methods Eqs. 8 + 9 -- the minus branch of
    # "E = Ebase * (1 + or - Edrug)" -- and the formula block printed beneath
    # Table 3).
    le0    <- log(81.2) ; label("Baseline normalized plasmin peak height (% of normal pooled plasma)")            # Valke 2024 Table 3: Baseline effect = 81.2% of NPP (RSE 4.5%)
    limax  <- fixed(log(1)) ; label("Maximal fractional FVIII suppression of normalized plasmin peak height (unitless)")   # Valke 2024 Table 3: Maximal effect = 1 FIX
    lec50  <- log(256)  ; label("FVIII activity producing half the maximal plasmin peak height suppression (IU/dL)")       # Valke 2024 Table 3: EC50 = 256 IU/dL (RSE 49.9%)
    lhill  <- fixed(log(1)) ; label("Hill coefficient of the plasmin peak height Imax relationship (unitless)")   # Valke 2024 Table 3: Hill coefficient = 1 FIX
    ...
    etale0 ~ 0.069696   # Valke 2024 Table 3: IIV on the baseline effect = 26.4% (RSE 12.9%) [Shr 0.1%]; omega^2 = 0.264^2
    ...
    propSd_plasminPeak <- 0.163 ; label("Proportional residual SD for normalized plasmin peak height (fraction)")   # Valke 2024 Table 3: Proportional error = 16.3% (RSE 5.8%)
  })
  ...
  model({
    ...
    # ---- PD: normalized plasmin peak height (% of normal pooled plasma) ----
    # Valke 2024, formula block beneath Table 3:
    #   E = Ebase * (1 - Imax * C^n / (EC50^n + C^n))
    # With Imax fixed at 1, an infinite FVIII activity would abolish plasmin
    # generation entirely; over the observed FVIII range (0-52 IU/dL) the
    # suppression reaches about 17%, taking the readout from 81.2% to about
    # 67% of normal pooled plasma.
    e0   <- exp(le0 + etale0)
    imax <- exp(limax)
    ec50 <- exp(lec50)
    hill <- exp(lhill)

    edrug        <- imax * fviii ^ hill / (ec50 ^ hill + fviii ^ hill)
    plasminPeak  <- e0 * (1 - edrug)
    plasminPeak ~ prop(propSd_plasminPeak)
  })
}
