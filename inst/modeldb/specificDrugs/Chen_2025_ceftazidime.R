Chen_2025_ceftazidime <- function() {
  ...
  # (Chen 2025 Methods, PopPK Modeling: "covariate screening including
  # demographic and laboratory information"; Results, PopPK Modeling: "CrCL was
  # identified as a significant covariate in both models"). Documented here so
  # the paper's covariate screen is preserved without declaring covariates the
  ...
  ini({
    ...
    # cohort median). Chen 2025 Table 2 final-model column and Eq. 1-2.
    lcl <- log(2.96);  label("Clearance at CRCL 71.3 mL/min (L/h)")  # Chen 2025 Table 2: tvCL = 2.96 L/h (RSE 11.40%; bootstrap median 2.92, 95% CI 2.29-3.58); also Eq. 1
    lvc <- log(17.76); label("Volume of distribution (L)")           # Chen 2025 Table 2: tvV = 17.76 L (RSE 12.13%; bootstrap median 17.80, 95% CI 13.84-23); also Eq. 2
    ...
    # carries an RSE of 27.66% and a bootstrap 95% CI of 0.18-0.71, which
    # could only arise from estimation.
    #
    # Chen 2025 Eq. 1 is typeset as 2.96 x (CrCL^0.44 / 71.3) x exp(etaCL) --
    ...
    # 2.96 x 71.3^0.44 / 71.3 = 0.27 L/h, an order of magnitude below the value
    # the same sentence calls "the typical value of CL". The median-normalized
    # power form used here returns exactly 2.96 L/h at CRCL = 71.3, matches the
    ...
    # reported CL_CAZ 3.2 L/h; this model gives 2.53 L/h at CRCL 50, whereas
    # the literal reading gives 0.24 L/h).
    e_crcl_cl <- 0.44; label("CRCL exponent on CL (unitless)")  # Chen 2025 Table 2: dCLdCrCL = 0.44 (RSE 27.66%; bootstrap median 0.44, 95% CI 0.18-0.71); also Eq. 1

    # Inter-individual variability. Chen 2025 Tables 2 headers the rows
    ...
    # rounding. No function of a VARIANCE of 0.56 yields 55.71% -- sqrt(0.56)
    # is 74.8% and sqrt(exp(0.56) - 1) is 86.6% -- so the header label is a
    ...
    etalcl ~ 0.3136 # 0.56^2; omega row for CL, Chen 2025 Table 2 (shrinkage 5.74%, RSE 17.31%) = 55.71% CV per Results text
    etalvc ~ 0.1681 # 0.41^2; omega row for V,  Chen 2025 Table 2 (shrinkage 19.00%, RSE 28.29%)
    ...
    propSd <- 0.30; label("Proportional residual error (fraction)") # Chen 2025 Table 2: Proportional error = 0.30 (RSE 20.19%; bootstrap median 0.30, 95% CI 0.17-0.41)
  })
  model({
    # Individual PK parameters. CL carries the median-normalized power-form
    # creatinine-clearance effect (Chen 2025 Eq. 1); V has no covariate
    # (Chen 2025 Eq. 2).
