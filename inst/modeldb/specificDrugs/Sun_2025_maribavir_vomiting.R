Sun_2025_maribavir_vomiting <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent vomiting as a function of the maribavir AUC on ",
    ...
    "claim of the two. Vomiting is the other adverse event with a ",
    "significant STEADY-STATE relationship in main-paper Figure 4 (p = ",
    "0.00296), which likewise prints no coefficients. The exposure slope ",
    "here is +0.0684 per 10 ug*h/mL. There is no PK layer and no ODE: the ",
  ...
  units <- list(
    ...
    concentration = "prob_vomiting (probability of treatment-emergent vomiting, 0-1; also logit_vomiting)"
  )
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0684 per 10 ug*h/mL (odds",
        "ratio 1.07 (1.04-1.10), p < 0.001)."
      ),
      source_name        = "AUCday of maribavir (increment of 10 h.ug/mL)"
    ),
    HCT_PRIOR = list(
      description        = "Prior haematopoietic cell transplantation before the current one.",
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no prior HCT; transplant-naive before the current transplant)",
      notes              = paste(
        "37 of 238 subjects (15.5%) positive. Table S2 reports the underlying",
        "column as a COUNT (0 prior HCT: 201, 1: 31, 2: 6); the modelled covariate",
        "is its binary >= 1 collapse, which is the form fitted and therefore the",
        "form carried here.",
        "Coefficient -2.05: recipients of a previous transplant reported LESS",
        "treatment-emergent vomiting, plausibly a tolerance or reporting effect in",
        "patients already experienced with conditioning-regimen emesis."
      ),
      source_name        = "Prior HCT treatment"
    )
  )
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary vomiting records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel c (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -2.89; label("Logit of the vomiting probability at zero maribavir exposure with all risk factors at their reference level (unitless logit)")  # Sun 2025 Figure S3 panel c (Supporting Information file s002), Intercept -2.89 (SE 0.379), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0684; label("Log-odds of vomiting per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel c (Supporting Information file s002), Estimate 0.0684 (SE 0.0126), odds ratio 1.07 (1.04-1.10), p < 0.001

    # ----- Risk-factor effects on the logit -----
    e_hct_prior_logit <- -2.05; label("Log-odds of vomiting for a prior HCT versus transplant-naive (unitless logit)")  # Sun 2025 Figure S3 panel c (Supporting Information file s002), Prior HCT treatment Estimate -2.05 (SE 0.826), odds ratio 0.129 (0.0256-0.652), p = 0.0132
    ...
    addSd_prob_vomiting <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_vomiting <- logit_ref +
      e_auc_logit * auc10 +
      e_hct_prior_logit * HCT_PRIOR

    prob_vomiting <- expit(logit_vomiting)

    # ----- Observation -----
    prob_vomiting ~ add(addSd_prob_vomiting)
  })
}
