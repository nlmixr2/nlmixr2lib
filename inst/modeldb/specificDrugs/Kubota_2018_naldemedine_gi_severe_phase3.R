Kubota_2018_naldemedine_gi_severe_phase3 <- function() {
  description <- paste0(
    "Landmark binomial logistic-regression exposure-SAFETY model relating ",
    "the steady-state daily AUC of naldemedine to the probability of a SEVERE treatment-emergent gastrointestinal-disorder adverse event ",
    ...
    "prob = 1 / (1 + exp(-(a + b * AUCss))) with a = -4.39 and b = 0.015. ",
    "This is the model behind the paper's headline safety claim, that the probability of a severe gastrointestinal disorder at 0.2 and 0.4 mg is under 3%: it predicts 1.8% at the 0.2 mg mean exposure and 2.7% at twice that. The 95% CI of the slope includes zero, which Kubota 2018 attributes to the small number of severe events (15 across both arms). There is no phase 2b counterpart because no severe gastrointestinal event occurred in that study. ",
  ...
  reference <- paste(
    ...
    "Logistic-regression coefficients are Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Severe';",
  ...
  units <- list(
    ...
    concentration = "prob_gi_severe (probability of a SEVERE gastrointestinal disorder, 0-1; also logit_gi_severe)"
  )
  ...
  population <- list(
    ...
    n_subjects = 993L,
    n_studies = 2L,
    n_observations = "993 binary a SEVERE gastrointestinal disorder records, one per subject",
    ...
    regions = "Global",
    notes = paste(
      "The endpoint is the occurrence of a SEVERE treatment-emergent adverse event in the gastrointestinal-disorders system organ class. This is the strictest of the three severity thresholds. Observed frequencies (Table IV): placebo 7/548 (1.3%), 0.2 mg 8/445 (1.8%).",
  ...
  ini({
    # ==================================================================
    # Kubota 2018 Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Severe'. The model is the paper's Eq. (3):
    #
    ...
    logit_ref <- -4.39; label("Logit of the a SEVERE gastrointestinal disorder probability at zero naldemedine exposure, i.e. the placebo arm (unitless logit)")  # Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Severe', parameter a = -4.39 (95% CI -5.15 to -3.77)
    e_auc_logit <- 0.015; label("Log-odds of a SEVERE gastrointestinal disorder per 1 ng*h/mL increase in naldemedine steady-state daily AUC (unitless logit)")  # Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Severe', parameter b = 0.015 (95% CI -0.0152 to 0.0409)
    ...
    addSd_prob_gi_severe <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    # Linear predictor in the untransformed steady-state daily AUC.
    logit_gi_severe <- logit_ref + e_auc_logit * AUC_NALD

    prob_gi_severe <- expit(logit_gi_severe)

    # ----- Observation -----
    prob_gi_severe ~ add(addSd_prob_gi_severe)
  })
}
