Yamamoto_2017_morphine_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Morphine disposition in plasma and nine",
    "CNS compartments: brain microvasculature, brain extracellular fluid,",
    "brain intracellular fluid, lysosomes, and the four cerebrospinal-fluid",
    "spaces (lateral ventricle, third + fourth ventricle, cisterna magna,",
    "subarachnoid space) draining in series back to the brain",
    ...
    "CNS data, so the CNS profiles are genuine predictions. The only compound",
    "with dose-dependent CNS transport and the only one with interoccasion",
    "(between-study) variability. Table 4 reports one set of asymmetry",
    "factors and one binding factor for 4 mg/kg and a second set for 10 and",
    "40 mg/kg (footnotes a and b), consistent with saturable active influx;",
    "DOSE_HIGH selects between them. The two source studies each carry their",
    "own occasion-level random effect (Table 2, interoccasional variability),",
    "selected by OCC."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "morphine", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "morphine", units = "ng", specimen = "tissue", verified = TRUE),
    peripheral2 = list(analyte = "morphine", units = "ng", specimen = "tissue", verified = TRUE),
    brain_vascular = list(analyte = "morphine", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "morphine", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "morphine", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "morphine", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "morphine", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "morphine", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "morphine", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "morphine", units = "ng", specimen = "CSF", verified = TRUE)
  )

  covariateData <- list(
    DOSE_HIGH = list(
      description = paste(
        "1 = animal dosed at 10 or 40 mg/kg, 0 = animal dosed at 4 mg/kg.",
        "Table 4 reports two sets of morphine asymmetry factors and two",
        "binding factors, distinguished by footnotes a (4 mg/kg) and b",
        "(10, 40 mg/kg); the supplement carries them as two separate",
        "control streams (S6 and S7). The step is consistent with",
        "saturable active influx at the blood-brain barrier."
      ),
      units = "(binary)",
      type = "binary",
      reference = "0 (4 mg/kg)",
      source_name = "dose group"
    ),
    OCC = list(
      description = paste(
        "Occasion (source-study) index, 1 or 2. Table 2 reports two",
        "interoccasional variability terms for morphine and the control",
        "streams select between ETA(7) and ETA(8) on a data column STUD.",
        "The two morphine datasets are references 26 and 27 of the paper;",
        "the paper does not state which index maps to which study."
      ),
      units = "(index)",
      type = "categorical",
      reference = "1",
  ...
  population <- list(
    ...
    dose_range = "4, 10 and 40 mg/kg (10 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 83 animals, 4, 10 and 40 mg/kg (10 min",
      "infusion), data from references 26 (Groenendaal 2007, n = 65) and 27",
      "(Bouw 2000, n = 18). Plasma and brain microdialysis (brain extracellular",
      "fluid and CSF) samples were collected after femoral-vein administration;",
      "see Supplementary Material S2 for surgery and bioanalysis.",
      "Concentrations in the CNS compartments are unbound concentrations."
    )
  )

  ini({
    # ---- Empirical plasma pharmacokinetics (Table 2) ----
    lcl <- log(22.6); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(152); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(30.8); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(530); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    lq2 <- log(7.21); label("Intercompartmental clearance to peripheral 2 (mL/min)")  # Table 2 Q_PL_PER2
    lvp2 <- log(1200); label("Peripheral 2 volume of distribution (mL)")  # Table 2 V_PER2
    ...
    etalcl ~ 0.0317  # Table 2 CV 17.8 percent
    etalq ~ 0.0830  # Table 2 CV 28.8 percent
    etalq2 ~ 0.752  # Table 2 CV 86.7 percent
    etalvc ~ 0.650  # Table 2 CV 80.6 percent
    etalvp ~ 0.212  # Table 2 CV 46.0 percent

    # ---- Interoccasion (between-study) variability (Table 2) ----
    # One shared occasion-level random effect is added to every plasma
    # disposition parameter, with a different variance per source study.
    etaiov_1 ~ 0.182  # Table 2 interoccasional variability study 1, CV 42.7 percent
    etaiov_2 ~ 0.0882  # Table 2 interoccasional variability study 2, CV 29.7 percent
    ...
    propSd <- 0.246; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 0.9 and molecular weight = 285;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(2.5e-4); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(3.4e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    # Morphine: Table 4 footnotes a and b give one set at 4 mg/kg and a
    # second at 10 and 40 mg/kg; DOSE_HIGH selects between them in model().
    AFout1 <- fixed(11); label("Asymmetry factor out of brain extracellular fluid, 4 mg/kg (unitless)")  # Table 4 AFout1 footnote a
    AFout2 <- fixed(20); label("Asymmetry factor out of lateral-ventricle CSF, 4 mg/kg (unitless)")  # Table 4 AFout2 footnote a
    AFout3 <- fixed(26); label("Asymmetry factor out of third + fourth ventricle CSF, 4 mg/kg (unitless)")  # Table 4 AFout3 footnote a
    AFout1_highdose <- fixed(20); label("Asymmetry factor out of brain extracellular fluid, 10-40 mg/kg (unitless)")  # Table 4 AFout1 footnote b
    AFout2_highdose <- fixed(38); label("Asymmetry factor out of lateral-ventricle CSF, 10-40 mg/kg (unitless)")  # Table 4 AFout2 footnote b
    AFout3_highdose <- fixed(49); label("Asymmetry factor out of third + fourth ventricle CSF, 10-40 mg/kg (unitless)")  # Table 4 AFout3 footnote b
    ...
    PHF5 <- fixed(0.41); label("pH-dependent factor, brain intracellular to extracellular fluid (unitless)")  # Table 5 PHF5
    PHF6 <- fixed(0.41); label("pH-dependent factor, brain intracellular fluid to lysosome (unitless)")  # Table 5 PHF6
    ...
    BF <- fixed(1.8); label("Brain-tissue binding factor, 4 mg/kg (unitless)")  # Table 5 BF footnote a
    BF_highdose <- fixed(3.9); label("Brain-tissue binding factor, 10-40 mg/kg (unitless)")  # Table 5 BF footnote b
  ...
  model({
    ...
    # One occasion-level random effect shared across all six disposition
    # parameters, as in the control streams S6 and S7.
    etaiov <- (OCC == 1) * etaiov_1 + (OCC == 2) * etaiov_2
    cl <- exp(lcl + etalcl + etaiov)
    vc <- exp(lvc + etalvc + etaiov)
    q <- exp(lq + etalq + etaiov)
    vp <- exp(lvp + etalvp + etaiov)
    q2 <- exp(lq2 + etalq2 + etaiov)
    vp2 <- exp(lvp2 + etaiov)

    # Dose-dependent active transport and brain binding (Table 4 footnotes
    # a and b; Table 5 footnotes a and b).
    AFout1_d <- AFout1 + (AFout1_highdose - AFout1) * DOSE_HIGH
    AFout2_d <- AFout2 + (AFout2_highdose - AFout2) * DOSE_HIGH
    AFout3_d <- AFout3 + (AFout3_highdose - AFout3) * DOSE_HIGH
    BF_d <- BF + (BF_highdose - BF) * DOSE_HIGH
    ...
    Q_BBB_out    <- (Qp_BBB + Qt_BBB * AFout1_d) * PHF1
    Q_BCSFB1_in  <- Qp_BCSFB1 + Qt_BCSFB1 * AFin2
    Q_BCSFB1_out <- (Qp_BCSFB1 + Qt_BCSFB1 * AFout2_d) * PHF2
    Q_BCSFB2_in  <- Qp_BCSFB2 + Qt_BCSFB2 * AFin3
    Q_BCSFB2_out <- (Qp_BCSFB2 + Qt_BCSFB2 * AFout3_d) * PHF3
    ...
    Cbrain_total <- (brain_ecf * BF_d + brain_ecf + brain_icf + brain_lysosome) / V_TOT
    Cc ~ prop(propSd)
  })
}
