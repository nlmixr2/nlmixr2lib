Forder_2019_calcitriolQuantumDot_healthy_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of calcitriol-conjugated quantum dots (CalQD) in healthy (tumor-free) mice.",
  ...
  compartmentData <- list(
    plasma = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "plasma", verified = TRUE),
    kidney = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE),
    liver = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE),
    spleen = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE),
    lung = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE),
    other = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = paste(
      "Single intravenous injection of CalQD; the model code starts plasma at",
    ...
    notes = paste(
      ...
      "Schaefer 2012 (Forder 2019 Table 2, no tumor column): kidney 11.01, liver 20.38, lung 3.16, spleen 10.99.",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, No tumor, CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(46.9); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, No tumor, CalQD
    lkp_liver <- log(86.3); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, No tumor, CalQD
    lkp_spleen <- log(46.9); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, No tumor, CalQD
    lkp_lung <- log(13.4); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, No tumor, CalQD
