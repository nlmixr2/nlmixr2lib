Zazo_2024_ceftazidime_avibactam_control <- function() {
  ...
  covariateData <- list(
    WT = list(
      ...
      notes = "Baseline (time-fixed). The Monte Carlo simulations sampled body weight from a log-normal distribution with mean 75 kg and CV 13% (Section 4.1). Vd * 75 kg reproduces the Table 2 volumes exactly: 0.21 * 75 = 15.75 L vs 15.7 L (ceftazidime) and 0.27 * 75 = 20.25 L vs 20.2 L (avibactam).",
      source_name = "Weight"
    ),
    CRCL = list(
  ...
  population <- list(
    ...
    n_subjects = 1000L,
    n_studies = 0L,
    disease_state = "Simulated non-ICU ('control') adults infected with Pseudomonas aeruginosa (strain 2154; ATCC 15442 is the ICU-acquired reference strain named in the Introduction). Six renal-function subpopulations defined by creatinine clearance: 100, 60, 40, 20, 10 and 3 mL/min.",
    ...
    notes = paste(
      ...
      "The 'control' PK parameters (Table 5) were derived by the authors from published studies in non-ICU subjects (refs 17-19); the sibling model Zazo_2024_ceftazidime_avibactam_icu carries the critically ill parameter set.",
  ...
  ini({
    ...
    # ---------------------------------------------------------------
    lvd_caz  <- log(0.21); label("Ceftazidime distribution-volume coefficient Vd (L/kg)")           # Table 5, control CAZ: 0.21 (38.1)
    lvd_avi  <- log(0.27); label("Avibactam distribution-volume coefficient Vd (L/kg)")             # Table 5, control AVI: 0.27 (51.8)
    lcli_caz <- log(0.39); label("Ceftazidime clearance intercept Cli (L/h)")                       # Table 5, control CAZ: 0.39 (55.6)
    lcli_avi <- log(0.53); label("Avibactam clearance intercept Cli (L/h)")                         # Table 5, control AVI: 0.53 (65.6)
    lcls_caz <- log(0.06); label("Ceftazidime clearance slope CLs ((L/h) per mL/min of Clcr)")      # Table 5, control CAZ: 0.06 (36.9)
    lcls_avi <- log(0.12); label("Avibactam clearance slope CLs ((L/h) per mL/min of Clcr)")        # Table 5, control AVI: 0.12 (27.3)
    ...
    # ---------------------------------------------------------------
    etalvd_caz  ~ 0.13554524  # Table 5, control CAZ Vd  CV 38.1%; log(0.381^2 + 1)
    etalvd_avi  ~ 0.23769634  # Table 5, control AVI Vd  CV 51.8%; log(0.518^2 + 1)
    etalcli_caz ~ 0.26936738  # Table 5, control CAZ Cli CV 55.6%; log(0.556^2 + 1)
    etalcli_avi ~ 0.35790938  # Table 5, control AVI Cli CV 65.6%; log(0.656^2 + 1)
    etalcls_caz ~ 0.12765504  # Table 5, control CAZ CLs CV 36.9%; log(0.369^2 + 1)
    etalcls_avi ~ 0.07188243  # Table 5, control AVI CLs CV 27.3%; log(0.273^2 + 1)
