PichardoAlmarza_2017_simvastatin_qsp <- function() {
  ...
  population <- list(
    species = "human",
    n_subjects = 27L,
    disease_state = "Healthy adult male volunteers. Pichardo-Almarza 2015 Methods ('PKPD model of statins') states that the PKPD model 'was developed using data collected from 27 healthy male volunteers with a daily dose of 40 mg of simvastatin given for 14 d'; Pichardo-Almarza 2017 describes the same model as fitted 'in male volunteers'. Note that the originating publication (Kim 2011) is titled for prediction in patients with primary hyperlipidaemia, so the estimation cohort (healthy volunteers) and the intended prediction population (hyperlipidaemia) differ; only the two on-disk Pichardo-Almarza papers were available to confirm the estimation cohort.",
    sex_female_pct = 0,
