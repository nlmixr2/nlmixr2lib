Kubota_2018_naldemedine_sbm_phase2b <- function() {
  description <- paste0(
    ...
    "prob = 1 / (1 + exp(-(a + b * AUCss))) with a = -0.314 and b = 0.0191. ",
    "Kubota 2018 flags this fit as the weaker of the two efficacy models: the 95% CI of the slope includes zero, which the authors attribute to a high responder proportion already at the lowest 0.1 mg dose, so the probability may be independent of AUCss over the range studied. ",
  ...
  reference <- paste(
    ...
    "Logistic-regression coefficients are Table V(a), row '1107V9221 (Phase 2b)';",
  ...
  population <- list(
    ...
    regions = "Global",
    notes = paste(
      "In this 28-day phase 2b dose-finding study an SBM responder is a subject with 3 or more SBMs per week within the last 2 weeks of the treatment period AND an average increase of 1 or more SBM per week from baseline. Observed responder frequencies (Table IV): placebo 24/61 (39.3%), 0.1 mg 5/9 (55.6%), 0.2 mg 6/9 (66.7%), 0.4 mg 6/10 (60.0%).",
  ...
  ini({
    # ==================================================================
    # Kubota 2018 Table V(a), row '1107V9221 (Phase 2b)'. The model is the paper's Eq. (3):
    #
