Goel_2016_Sonidegib <- function() {
  ...
  covariateData <- list(
    ...
    DIS_HV = list(
      ...
      notes              = "Multiplicative effect on CL/F: 2.96^DIS_HV. Healthy volunteers had ~3-fold higher apparent CL/F than cancer patients in Goel 2016. The reference complement is the pooled cancer-patient cohort (X2101 + X1101 + A2201).",
      source_name        = "HV"
    ),
    PPI = list(
      ...
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no significant PPI use)",
      notes              = "Multiplicative effect on F: 0.696^PPI (~30% lower bioavailability). Goel 2016 Methods defines 'significant' use as duration of PPI use exceeding 80% of the PK assessment phase (i.e., at least 80% of the time between the first and last PK sample for the subject).",
      source_name        = "PPI"
    ),
    H2RA = list(
      ...
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no significant H2RA use)",
      notes              = "Multiplicative effect on F: 0.996^H2RA (no clinically meaningful effect). Operational definition: H2RA use covering >= 80% of the PK assessment phase (Goel 2016 Methods).",
      source_name        = "H2"
    ),
    MEAL_HFAT = list(
      ...
      notes              = "Per-dose-record indicator. Multiplicative effect on F: 5.74^MEAL_HFAT (~5.7-fold higher bioavailability after a high-fat meal vs the 2-h-post-light-meal reference) and on Ka: 1.01^MEAL_HFAT (no meaningful effect). The same dataset column FATM = 1 / Fatmeal = 1 carries the indicator for both Ka and F covariate effects in Goel 2016 (Table 2 theta18 on Ka, theta23 on F).",
      source_name        = "Fatmeal / FATM"
    ),
    HV_FAST = list(
      description        = "Healthy-volunteer dosed under overnight fasting indicator",
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (any non-HV-fasted record; the typical reference is patient dosed 2 h post light meal or HV dosed after high-fat meal)",
      notes              = "Per-dose-record indicator. Multiplicative effect on F: 0.855^HV_FAST (~14.5% lower F in HV-fasted records vs the patient 2-h-post-light-meal reference, conditional on the high-fat-meal effect already being separated). Goel 2016 used this composite indicator in studies A1102 (Japanese HV, single dose fasted) and A2114 (HV, fasted arm) but coded it as 0 for the A2114 high-fat-meal arm (which is captured by MEAL_HFAT instead).",
      source_name        = "HV.Fasting"
    ),
    DOSE = list(
  ...
  ini({
    ...
    #   (Western), DIS_HV = 0 (cancer patient), PPI = H2RA = MEAL_HFAT = HV_FAST = 0,
    ...
    e_dis_hv_cl    <-  2.96;  label("Multiplicative HV-vs-patient CL/F ratio (applied as ratio^DIS_HV)")     # Goel 2016 Table 2 theta10
    ...
    e_meal_hfat_ka <-  1.01;  label("Multiplicative high-fat-meal Ka ratio (applied as ratio^MEAL_HFAT)")    # Goel 2016 Table 2 theta18
    ...
    e_h2ra_f         <-  0.996; label("Multiplicative significant-H2RA F ratio (applied as ratio^H2RA)")            # Goel 2016 Table 2 theta20
    e_ppi_f          <-  0.696; label("Multiplicative significant-PPI F ratio (applied as ratio^PPI)")              # Goel 2016 Table 2 theta21
    e_hv_fast_f      <-  0.855; label("Multiplicative HV-fasted F ratio (applied as ratio^HV_FAST)")                # Goel 2016 Table 2 theta22
    e_meal_hfat_f    <-  5.74;  label("Multiplicative high-fat-meal F ratio (applied as ratio^MEAL_HFAT)")          # Goel 2016 Table 2 theta23
  ...
  model({
    ...
    fdepot <- e_h2ra_f^H2RA *
      e_ppi_f^PPI *
      e_hv_fast_f^HV_FAST *
      e_meal_hfat_f^MEAL_HFAT *
      (DOSE / 100)^e_dose_f *
