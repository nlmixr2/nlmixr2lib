Saleh_2023_ribociclib_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, CD1 nude).",
    "Nine-compartment CNS PBPK model predicting unbound ribociclib",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 100 mg/kg oral dose. The CNS structure is the mouse",
    ...
    "0.0693 (taken from ref [9]; Table V). The plasma PK model is the",
    "empirical one-compartment model of Table IV (ref [9]) and acts",
  ...
  compartmentData <- list(
    depot                = list(analyte = "ribociclib", units = "ng", specimen = "administration site", verified = TRUE),
    central              = list(analyte = "ribociclib", units = "ng", specimen = "plasma", verified = TRUE),
    brain_vascular       = list(analyte = "ribociclib", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf            = list(analyte = "ribociclib", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_cell_membrane  = list(analyte = "ribociclib", units = "ng", specimen = "tissue", verified = TRUE),
    brain_icf            = list(analyte = "ribociclib", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome       = list(analyte = "ribociclib", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv         = list(analyte = "ribociclib", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv        = list(analyte = "ribociclib", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm         = list(analyte = "ribociclib", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas        = list(analyte = "ribociclib", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range   = "100 mg/kg PO (single dose; Table IV)",
    notes        = paste(
      "Unbound plasma and microdialysis brain-ECF concentration-time",
      "data for ribociclib came from ref [9]",
  ...
  ini({
    ...
    lcl <- log(1.32); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(530.1); label("Central compartment volume (mL)")  # Table IV
    lka <- log(0.046); label("First-order absorption rate constant (1/min)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(435); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(2.5); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(11.59); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(8.87); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.23); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.0693); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (ref [9])
    ...
    kpuu_lv <- fixed(0.0693); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.0693); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    etalcl ~ 0.217  # Table IV IIV on central clearance (variance)
    etalka ~ 0.676  # Table IV IIV on first-order absorption (variance)

    # ---- Residual unexplained variability: Table IV (reported as VARIANCES) ----
    propSd <- sqrt(0.276); label("Proportional residual error (fraction)")  # Table IV: 0.276 as variance
  })
