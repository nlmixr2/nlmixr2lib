Nguyen_2020_fseeefc_rat_qsp <- function() {
  description <- paste(
    "Preclinical (rat). QSP. Myostatin / activin A / ActRIIB",
    "systems-pharmacology model for the follistatin-Fc fusion protein",
    "FS-EEE-Fc (Nguyen 2020), ovariectomised-rat parameterisation.",
    ...
    "pituitary.",
    "System parameters from Table S3 (rat), compound and FSH parameters",
    "from Table S4, fitted to serum FS-EEE-Fc and FSH after single IV",
    "doses of 1-30 mg/kg in female ovariectomised rats (Figure S6B), where",
    "activin neutralisation lowers FSH. No muscle sub-model was",
    "parameterised for the rat. Dosing is intravenous into central.",
    "Deterministic; ligand, receptor and FSH states start at the drug-free",
    "steady state.",
    sep = " "
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "FS-EEE-Fc (free, not ligand-bound)", units = "mg", specimen = "plasma", verified = TRUE),
    drug_muscle = list(analyte = "FS-EEE-Fc (free)", units = "nM", specimen = "tissue", verified = TRUE),
    drug_pituitary = list(analyte = "FS-EEE-Fc (free)", units = "nM", specimen = "tissue", verified = TRUE),
    drug_other = list(analyte = "FS-EEE-Fc (free)", units = "nM", specimen = "tissue", verified = TRUE),
    md_plasma = list(analyte = "FS-EEE-Fc-myostatin complex", units = "nM", specimen = "plasma", verified = TRUE),
    md_muscle = list(analyte = "FS-EEE-Fc-myostatin complex", units = "nM", specimen = "tissue", verified = TRUE),
    md_other = list(analyte = "FS-EEE-Fc-myostatin complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_plasma = list(analyte = "FS-EEE-Fc-activin A complex", units = "nM", specimen = "plasma", verified = TRUE),
    ad_muscle = list(analyte = "FS-EEE-Fc-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_pituitary = list(analyte = "FS-EEE-Fc-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_other = list(analyte = "FS-EEE-Fc-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
  ...
  population <- list(
    species = "rat (female, ovariectomised)",
    n_subjects = NA,
    n_studies = 1,
    sex_female_pct = 100,
    weight_range = "reference body weight 0.28 kg (Table S4)",
    dose_range = "single IV doses of 1, 3, 10 and 30 mg/kg",
    notes = paste(
      "In-house study (Supplementary Methods): 6 females per group per time",
      "point, serum sampled to 336 h. Doses in mg (mg/kg x 0.28 kg)."
    )
  )

  ini({
    # ---- System physiology (Table S3 rat) --------------------------------------
    vplasma <- fixed(0.00906); label("Plasma volume (L)") # Table S3 rat: 0.00906
    vmuscle <- fixed(0.0158); label("Interstitial volume of muscle (L)") # Table S3 rat: 0.0158
    vpituitary <- fixed(1.26e-06); label("Interstitial volume of anterior pituitary (L)") # Table S3 rat: 1.26e-06
    lf_muscle <- fixed(0.000925); label("Lymph flow of muscle (L/h)") # Table S3 rat: 0.000925
    lf_pituitary <- fixed(2.12e-07); label("Lymph flow of pituitary (L/h)") # Table S3 rat: 2.12e-07
    lf_other <- fixed(0.00404); label("Lymph flow of other tissues (L/h)") # Table S3 rat: 0.00404

    # ---- Myostatin (Table S3 rat) ------------------------------------------------
    mw_myo <- fixed(25000); label("Molecular weight of mature myostatin (g/mol)") # not in Table S3; species-independent molecular weight from Table 1 (25,000)
    mw_ppmyo <- fixed(80000); label("Molecular weight of myostatin-propeptide complex (g/mol)") # not in Table S3; species-independent molecular weight from Table 1 (80,000)
    ksyn_ppmyo <- fixed(0.0137); label("Synthesis rate of myostatin-propeptide complex in muscle (nmol/h)") # Table S3 rat: 0.0137 (unit printed 1/hr; enters the muscle balance as an amount rate)
    kcleave_ppmyo_muscle <- fixed(0.0363); label("Cleavage rate of myostatin-propeptide complex in muscle (1/h)") # Table S3 rat: 0.0363
    kcleave_ppmyo_plasma <- fixed(0.0351); label("Cleavage rate of myostatin-propeptide complex in plasma (1/h)") # Table S3 rat: 0.0351
    kdeg_ppmyo <- fixed(0.347); label("Degradation rate of myostatin-propeptide complex, all spaces (1/h)") # Table S3 rat: 0.347 (footnote a: same in all spaces)
    kdeg_myo <- fixed(0.347); label("Degradation rate of mature myostatin, all spaces (1/h)") # Table S3 rat: 0.347 (footnote a: same in all spaces)
    sigma_v_myo <- fixed(0.7); label("Vascular reflection coefficient of myostatin species (unitless)") # Table S3 rat: 0.7 (footnote a: same in other tissues)
    sigma_is_myo <- fixed(0.2); label("Lymphatic reflection coefficient of myostatin species (unitless)") # Table S3 rat: 0.2 (footnote a)
    kon_myo_actriib <- fixed(1.328); label("Myostatin-ActRIIB association rate constant (1/(nM*h))") # Table S3 rat: 1.328 (Sako 2010)
    koff_myo_actriib <- fixed(0.124); label("Myostatin-ActRIIB dissociation rate constant (1/h)") # Table S3 rat: 0.124 (Sako 2010)

    # ---- Activin A (Table S3 rat) ------------------------------------------------
    mw_act <- fixed(25000); label("Molecular weight of mature activin A (g/mol)") # not in Table S3; species-independent molecular weight from Table 1 (25,000)
    mw_ppact <- fixed(80000); label("Molecular weight of activin A-propeptide complex (g/mol)") # not in Table S3; species-independent molecular weight from Table 1 (80,000)
    ksyn_ppact_pituitary <- fixed(1.98e-06); label("Synthesis rate of activin A-propeptide complex in pituitary (nmol/h)") # Table S3 rat: 1.98e-06
    ksyn_ppact_plasma <- fixed(0.000149); label("Synthesis rate of activin A-propeptide complex in plasma (nmol/h)") # Table S3 rat: 0.000149
    ksyn_ppact_muscle <- fixed(0.000491); label("Synthesis rate of activin A-propeptide complex in muscle (nmol/h)") # Table S3 rat: 0.000491
    kcleave_ppact <- fixed(3113); label("Cleavage rate of activin A-propeptide complex, all producing spaces (1/h)") # Table S3 rat: 3113
    kdeg_ppact <- fixed(1.39); label("Degradation rate of activin A-propeptide complex, all spaces (1/h)") # Table S3 rat: 1.39 (same in all spaces, as Table 1)
    kdeg_act <- fixed(2.08); label("Degradation rate of mature activin A, all spaces (1/h)") # Table S3 rat: 2.08 (same in all spaces, as Table 1)
    sigma_v_act <- fixed(0.7); label("Vascular reflection coefficient of activin species (unitless)") # Table S3 rat: 0.7 (footnote b: same in pituitary and other tissues)
    sigma_is_act <- fixed(0.2); label("Lymphatic reflection coefficient of activin species (unitless)") # Table S3 rat: 0.2 (footnote b)
    kon_act_actriib <- fixed(14.9); label("Activin A-ActRIIB association rate constant (1/(nM*h))") # Table S3 rat: 14.90 (Sako 2010)
    koff_act_actriib <- fixed(0.533); label("Activin A-ActRIIB dissociation rate constant (1/h)") # Table S3 rat: 0.533 (Sako 2010)

    # ---- ActRIIB (Table S3 rat) --------------------------------------------------
    bl_actriib <- fixed(0.138); label("Total ActRIIB concentration in muscle and in pituitary (nM)") # Table S3 rat: 0.138 nM, printed once for all species

    # ---- FS-EEE-Fc compound parameters (Table S4 rat) ------------
    mw_drug <- fixed(150000); label("Molecular weight of FS-EEE-Fc (g/mol)") # Table S4 rat MW_Drug = 150000
    vother <- fixed(0.27); label("Volume of other tissues (L)") # Table S4 rat V_other = 0.27 L
    kdeg_drug <- fixed(0.006); label("Degradation rate of free FS-EEE-Fc, all spaces (1/h)") # Table S4 rat k_deg_drug = 0.006
    kdeg_md <- fixed(0.006); label("Degradation rate of FS-EEE-Fc-myostatin complex (1/h)") # Table S4 rat k_deg_MD = 0.006 (assumed ~ k_deg_drug)
    kdeg_ad <- fixed(0.006); label("Degradation rate of FS-EEE-Fc-activin A complex (1/h)") # Table S4 rat k_deg_AD = 0.006 (assumed ~ k_deg_drug)
    kon_drug_myo <- fixed(0.6); label("FS-EEE-Fc-myostatin association rate constant (1/(nM*h))") # Table S4 rat kon_drug-Myo = 0.6 (footnote b)
    koff_drug_myo <- fixed(0.0045); label("FS-EEE-Fc-myostatin dissociation rate constant (1/h)") # Table S4 rat koff_drug-Myo = 0.0045 (from in vitro Kd)
    kon_drug_act <- fixed(0.6); label("FS-EEE-Fc-activin A association rate constant (1/(nM*h))") # Table S4 rat kon_drug-Act = 0.6 (footnote b)
    koff_drug_act <- fixed(0.0037); label("FS-EEE-Fc-activin A dissociation rate constant (1/h)") # Table S4 rat koff_drug-Act = 0.0037 (from in vitro Kd)
    sigma_v_drug <- fixed(0.78); label("Vascular reflection coefficient of drug and drug complexes (unitless)") # Table S4 rat sigma_m_V_drug = 0.78 (footnote a: same in pituitary and other tissues)
    sigma_is_drug <- fixed(0.2); label("Lymphatic reflection coefficient of drug and drug complexes (unitless)") # Table S4 rat sigma_m_IS_drug = 0.2 (footnote h: same in pituitary and other tissues)

    # ---- FSH pharmacodynamics (Table S4 rat) ---------------------------------
    vmax_fsh <- fixed(5.34); label("Maximal FSH production rate (ng/mL/h)") # Table S4 rat V_max_FSH = 5.34
    h_fsh <- fixed(4.48); label("Hill coefficient of the pituitary occupancy-FSH link (unitless)") # Table S4 rat h_FSH = 4.48
    ro50_fsh <- fixed(100); label("Pituitary ActRIIB occupancy giving half-maximal FSH production (%)") # Table S4 rat RO_50_FSH = 100
    kdeg_fsh <- fixed(0.085); label("First-order FSH elimination rate (1/h)") # Table S4 rat k_deg_FSH = 0.085
  ...
  model({
    ...
    d/dt(central) <- -(kdeg_drug * drug_plasma * vplasma + f_drug_m + f_drug_t + f_drug_o +
