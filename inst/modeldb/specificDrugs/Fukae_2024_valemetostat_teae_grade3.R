Fukae_2024_valemetostat_teae_grade3 <- function() {
  description <- paste0(
    ...
    "preferred terms) in adults with relapsed/refractory non-Hodgkin ",
    ...
    "(age, LDH and weight centred-and-scaled; sex, mild hepatic ",
    "impairment, White race, Black race and United States enrollment ",
    "as 0/1 indicators), b1 are covariate effects on the logit ",
    "intercept and b2 are covariate effects on the exposure slope ",
    "(Fukae 2024 Methods equation, Table 3). This composite endpoint ",
    "carries NO baseline-laboratory covariate, unlike the three ",
    "laboratory-defined endpoints in the same family. All candidate ",
  ...
  units <- list(
    ...
    concentration = "prob_teae_grade3 (probability of Any Grade >= 3 TEAE, 0-1; also logit_teae_grade3)"
  )
  ...
  population <- list(
    ...
    n_subjects     = 102L,
    n_studies      = 2L,
    n_observations = "101 evaluable binary any-grade->=-3-TEAE records (one per patient; 1 of the 102 patients had a missing outcome, Fukae 2024 Table 1)",
    ...
    notes          = "The composite any-grade->=-3-TEAE endpoint is a superset of the three laboratory-defined endpoints in this family and must not be treated as a competing risk alongside them. Observed outcome: 69 of 102 patients (67.6%) experienced a grade >= 3 TEAE, 32 (31.4%) did not, and 1 (1.0%) was missing (Fukae 2024 Table 1)."
  )

  ini({
    # ==================================================================
    # All values are posterior medians from Fukae 2024 Table 3, column
    # "Any Grade >= 3 TEAE". The Table 3 footnote states the reporting
    ...
    logit_ref             <- log(0.653 / (1 - 0.653)) ; label("Logit of the any grade >= 3 TEAE probability for the reference patient (unitless logit)")  # Fukae 2024 Table 3, "Population mean" 0.653 (0.547, 0.745); ESS 4117, Rhat 1.00

    # ----- Exposure slope on the logit (beta*_E) -----
    e_aucu_logit          <- log(1.51)     ; label("Log-odds of any grade >= 3 TEAE per 250 ng*h/mL increase in unbound valemetostat AUCss (unitless logit)")  # Fukae 2024 Table 3, OR 1.51 (0.894, 2.51); ESS 3538, Rhat 1.00

    # ----- Covariate effects on the logit intercept (beta*_1) -----
    e_age_logit           <- log(0.985)    ; label("Log-odds shift on the any grade >= 3 TEAE logit per 10-year increase in age (unitless logit)")  # Fukae 2024 Table 3, OR 0.985 (0.830, 1.16); ESS 6351, Rhat 1.00
    e_ldh_logit           <- log(1.42)     ; label("Log-odds shift on the any grade >= 3 TEAE logit per 300 U/L increase in baseline LDH (unitless logit)")  # Fukae 2024 Table 3, OR 1.42 (0.961, 6.95); ESS 1508, Rhat 1.00
    e_wt_logit            <- log(1.00)     ; label("Log-odds shift on the any grade >= 3 TEAE logit per 20 kg increase in body weight (unitless logit)")  # Fukae 2024 Table 3, OR 1.00 (0.841, 1.19); ESS 8407, Rhat 1.00
    e_sexf_logit          <- log(1.01)     ; label("Log-odds shift on the any grade >= 3 TEAE logit for female sex vs male reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.01 (0.785, 1.30); ESS 5540, Rhat 1.00
    e_hepimp_mild_logit   <- log(1.06)     ; label("Log-odds shift on the any grade >= 3 TEAE logit for mild hepatic impairment vs normal reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.06 (0.681, 1.66); ESS 7039, Rhat 1.00
    e_race_white_logit    <- log(0.963)    ; label("Log-odds shift on the any grade >= 3 TEAE logit for White race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.963 (0.676, 1.37); ESS 6205, Rhat 1.00
    e_race_black_logit    <- log(1.17)     ; label("Log-odds shift on the any grade >= 3 TEAE logit for Black race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.17 (0.616, 2.49); ESS 6723, Rhat 1.00
    e_region_usa_logit    <- log(1.01)     ; label("Log-odds shift on the any grade >= 3 TEAE logit for United States vs Japan enrollment reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.01 (0.747, 1.38); ESS 5198, Rhat 1.00

    # ----- Covariate effects on the exposure slope (beta*_2) -----
    e_age_slope           <- log(1.03)     ; label("Shift in the unbound-AUCss exposure slope per 10-year increase in age (unitless logit)")  # Fukae 2024 Table 3, OR 1.03 (0.882, 1.29); ESS 4568, Rhat 1.00
    e_ldh_slope           <- log(1.02)     ; label("Shift in the unbound-AUCss exposure slope per 300 U/L increase in baseline LDH (unitless logit)")  # Fukae 2024 Table 3, OR 1.02 (0.850, 1.24); ESS 6141, Rhat 1.00
    e_wt_slope            <- log(0.965)    ; label("Shift in the unbound-AUCss exposure slope per 20 kg increase in body weight (unitless logit)")  # Fukae 2024 Table 3, OR 0.965 (0.767, 1.15); ESS 5690, Rhat 1.00
    e_sexf_slope          <- log(1.07)     ; label("Shift in the unbound-AUCss exposure slope for female sex vs male reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.07 (0.850, 2.50); ESS 3896, Rhat 1.00
    e_hepimp_mild_slope   <- log(1.05)     ; label("Shift in the unbound-AUCss exposure slope for mild hepatic impairment vs normal reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.05 (0.763, 1.53); ESS 4400, Rhat 1.00
    e_race_white_slope    <- log(1.02)     ; label("Shift in the unbound-AUCss exposure slope for White race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.02 (0.631, 1.67); ESS 7694, Rhat 1.00
    e_race_black_slope    <- log(1.02)     ; label("Shift in the unbound-AUCss exposure slope for Black race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.02 (0.433, 2.42); ESS 7348, Rhat 1.00
    e_region_usa_slope    <- log(1.02)     ; label("Shift in the unbound-AUCss exposure slope for United States vs Japan enrollment reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.02 (0.672, 1.59); ESS 6516, Rhat 1.00

    # ----- No between-subject variability, no residual error -----
    # See Fukae_2024_valemetostat_orr_central.R for the full rationale.
    addSd_prob_teae_grade3 <- fixed(0.001) ; label("Placeholder additive residual SD on the typical-value any grade >= 3 TEAE probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
