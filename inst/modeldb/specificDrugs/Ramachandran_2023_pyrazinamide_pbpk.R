Ramachandran_2023_pyrazinamide_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 18 ODEs, MATLAB ode15s). Oral pyrazinamide disposition",
    ...
    "concentrations after a 2000 mg oral dose (Appendix S1 Table S7).",
    "Clearance is split into a renal component fR * CL driven by arterial",
    "concentration and a hepatic component (1 - fR) * CL driven by the hepatic",
    "inlet concentration; the hepatic output enters the gut lumen, which for",
    "pyrazinamide is a terminal sink cleared only by faecal transit at",
    "kF = 0.252 /h (the paper assigns a non-zero gut reabsorption rate only to",
    "rifampicin). The model is a deterministic typical-value simulation: the",
    "paper reports no interindividual variability and no residual error model,",
    "so propSd is fixed at 0."
  )
  reference <- paste(
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "(the pyrazinamide studies of Appendix S1 Table S6; Appendix S1 Table S6 lists the per-point calibration",
      "weights). Model validation used a disjoint set of published studies at",
      "a different dose. The model is therefore a typical-value forward",
      "simulation tool, not an individual-level popPK model."
    )
  )
  ...
  ini({
    ...
    lka <- log(1.36); label("First-order oral absorption rate constant ka (1/h)")  # Appendix S1 Table S7: pyrazinamide ka = 1.36 /h
    lcl <- log(4.1); label("Total systemic clearance CL (L/h)")                   # Appendix S1 Table S7: pyrazinamide CL = 4.1 L/h
  ...
  model({
    ...
    # (Appendix S1 Table S5, pyrazinamide column). "Others" is the median of
    ...
    # Fractional renal clearance (Appendix S1 Table S4, pyrazinamide fR).
    f_renal <- 0.09
    ...
    # the reported value"). For pyrazinamide the gut lumen is therefore a
