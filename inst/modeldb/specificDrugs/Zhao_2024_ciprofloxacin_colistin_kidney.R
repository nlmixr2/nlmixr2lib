Zhao_2024_ciprofloxacin_colistin_kidney <- function() {
  ...
  # (cmsKidney and cstKidney are the states that drive the drug effect in this
  # model; cstKidney is divided by the kidney interstitial volume fraction).
  ...
  compartmentData <- list(
    ...
    cms1Central = list(
      analyte = "colistimethate sodium, species CMS1",
      units = "mol",
      specimen = "plasma",
      verified = TRUE
    ),
    ...
    cms2Central = list(
      analyte = "colistimethate sodium, species CMS2",
      units = "mol",
      specimen = "plasma",
      verified = TRUE
    ),
    ...
    cstCentral = list(
      analyte = "colistin formed from colistimethate",
      units = "mol",
      specimen = "plasma",
      verified = TRUE
    ),
  ...
  covariateData <- list(
    BACT = list(
      ...
      notes = paste(
        ...
        "The simulations deliberately use MICs far above those of the source strains (Table 1 was fitted at MIC_CIP 0.023-1 mg/L); set micCip and micCst to the extrapolated values and BACT only to pick the PD parameter set. Because kidney concentrations are far higher than plasma, the kidney scenarios use MIC_CIP 8-64 mg/L rather than the 1-8 mg/L of the plasma scenarios."
      )
    ),
    WT = list(
  ...
  population <- list(
    ...
    n_subjects = 1000L,
    n_studies = 0L,
    disease_state = "Simulated critically ill adults with Escherichia coli pyelonephritis. Because most E. coli infections are extracellular, the free drug concentration in the interstitial fluid of the kidney is taken to represent the target space. The bacterial strains are deliberately extrapolated beyond those used to fit the PD model: MIC_CIP 8-64 mg/L (bacteria up to MIC_CIP 8 mg/L are already cleared in the kidney by ciprofloxacin 400 mg q8h alone) and MIC_CST 2 or 4 mg/L, 2 mg/L being the EUCAST epidemiological cut-off for resistance.",
    model_system = "Monte Carlo simulation of 1000 virtual patients over 32 h in mrgsolve, linking two published critically-ill population PK models, extended with a whole-body-PBPK-derived kidney compartment per species, to the in vitro time-kill PD model. Reported inter-individual and inter-occasion variability of the PK models were included; all patients share WT 80 kg and CrCL 90 mL/min. Kidney tissue and tubular compartments were lumped and assumed to be at the same concentration, because of limited human information on the substructures.",
    initial_inoculum = "10^6 CFU/mL at the time of the first dose",
    dose_range = "Ciprofloxacin 400 mg as a 1 h intravenous infusion q8h in every scenario. Colistimethate sodium 0.5 + 0.25, 0.2 + 0.1 or 0.1 + 0.05 MU (loading + maintenance) as a 0.5 h intravenous infusion q12h, alone or combined with ciprofloxacin. Doses above 0.5 + 0.25 MU already produce the maximum killing rate as monotherapy in the kidney, so only lower doses are informative.",
    notes = paste(
      ...
      "The unbound kidney colistin concentration is additionally divided by the kidney interstitial volume fraction (0.196) because colistin distributes mainly extracellularly; no such correction is applied to ciprofloxacin, which also accumulates intracellularly (Supplementary Methods).",
      ...
      "Sibling models: Zhao_2024_ciprofloxacin_colistin_invitro (the underlying time-kill fit) and Zhao_2024_ciprofloxacin_colistin_plasma (identical apart from the two concentrations that drive the drug effect)."
    )
  )
  ...
  ini({
    ...
    inoc   <- fixed(1e6); label("Bacterial density at the time of the first dose (CFU/mL)")                 # Fig. 5 caption: "Antibiotics were administered at a bacterial concentration of 6 log10 CFU/mL"
    micCip <- fixed(8);   label("Ciprofloxacin MIC of the simulated strain (mg/L)")                          # Predictions of clinical drug effect: strains with MIC_CIP 8-64 mg/L were simulated in kidney
  ...
  model({
    ...
    # THIS BLOCK is the only difference from the sibling plasma model:
    # PKPD_CIPCST_run422b_kidney.cpp substitutes CIPCKID for CIPCplasma and
    # CPcolKID for CPcolumg, leaving every other line of the model untouched.
    cipConc <- cipKidneyConc
    cstConc <- cstKidneyConc
