Saleh_2023_quinidine_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, NMRI).",
    "Nine-compartment CNS PBPK model predicting unbound quinidine",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 40 mg/kg intraperitoneal dose. The CNS structure is the mouse",
    ...
    "0.2185 (taken from ref [56]; Table V). The plasma PK model is the",
  ...
  ini({
    ...
    lcl <- log(46.99); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(2951.3); label("Central compartment volume (mL)")  # Table IV
    lka <- log(2.61); label("First-order absorption rate constant (1/min)")  # Table IV
    dur_depot <- fixed(101); label("Zero-order input duration into depot (min)")  # Table IV 'Duration'

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(324); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(2.82); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(13.89); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(9.05); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.233); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.2185); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (ref [56])
    ...
    kpuu_lv <- fixed(0.2185); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.2185); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    # Table IV reports all interindividual variances as 0 for quinidine;
    ...
    addSd <- sqrt(2.05); label("Additive residual error (ng/mL)")  # Table IV: 2.05 as variance
  })
  ...
  model({
    ...
    # Table IV gives a 101 min 'Duration' for quinidine: a zero-order input
    # into the depot, which then empties first-order at ka. Dose rows must
    # carry rate = -2 for dur() to take effect.
    dur(depot) <- dur_depot
