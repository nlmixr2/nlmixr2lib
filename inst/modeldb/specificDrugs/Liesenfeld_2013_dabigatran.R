Liesenfeld_2013_dabigatran <- function() {
  ...
  covariateData <- list(
    DIAL = list(
      ...
      notes              = "Time-varying within subject. Gates the Michaels-equation apparent dialysis clearance: CLdialysis/F is added to CL/F only when DIAL = 1. The source paper reports four-hour intermittent hemodialysis sessions on Days 1, 3, and 5 of each study period (Methods, Study Design). For non-ESRD patients with no dialysis, set DIAL = 0 throughout.",
      source_name        = "DIAL"
    ),
    BFR = list(
      ...
      notes              = "Time-varying within subject. Enters the Michaels equation together with DFR and KoA to compute the apparent dialysis clearance. Values investigated in the source study were 200, 300, and 400 mL/min (Methods, Study Design; Table 1). Should be 0 (or any sentinel) when DIAL = 0 since the Michaels term is gated off.",
      source_name        = "BFR"
    ),
    DFR = list(
      ...
      type               = "continuous",
      reference_category = NULL,
      notes              = "Time-varying within subject. Enters the Michaels equation together with BFR and KoA. The source study fixed DFR at 700 mL/min throughout (Methods, Study Design); the simulation scenarios also examined 500 mL/min (Methods, Simulations). Should be 0 (or any sentinel) when DIAL = 0.",
      source_name        = "DFR"
    )
  )
  ...
  model({
    ...
    # holds in the interdialytic period: when DIAL = 0 the safe values fall
    # back to 1 mL/min so the rational expression evaluates without
    # divide-by-zero, then the entire term is gated to zero by the leading
    # DIAL multiplier.
    bfr_safe         <- BFR * DIAL + (1 - DIAL)
    dfr_safe         <- DFR * DIAL + (1 - DIAL)
    ...
    cl_dialysis      <- DIAL * cl_dialysis_raw
