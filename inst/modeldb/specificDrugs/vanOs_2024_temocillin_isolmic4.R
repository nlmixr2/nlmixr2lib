vanOs_2024_temocillin_isolmic4 <- function() {
  description <- paste0(
    "In vitro (Escherichia coli ISOL MIC4, temocillin MIC 4 mg/L). ",
  ...
  reference <- paste0(
    ...
    "Pharmacodynamic parameter estimates: main-text Table 3, column ISOL MIC4. ",
  ...
  compartmentData <- list(
    ...
    bact_susceptible = list(
      analyte = "Escherichia coli ISOL MIC4 (susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_less_susceptible = list(
      analyte = "Escherichia coli ISOL MIC4 (less susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_resistant = list(
      analyte = "Escherichia coli ISOL MIC4 (resistant subpopulation growing on agar with 32 mg/L temocillin)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    )
  )
  ...
  population <- list(
    species = "in vitro (Escherichia coli ISOL MIC4); pharmacokinetics from critically ill adults",
    n_subjects = 1L,
    n_studies = 1L,
    strain = paste0(
      "ISOL MIC4 (rectal swab; ESBL genes: blaTEM-1, blaCTX-M-1; temocillin MIC 4 mg/L, determined in triplicate ",
    ...
    notes = paste0(
      ...
      "estimated separately for the two experimental systems (Bmax,STK 8.63 and Bmax,HFIM ",
      "10.4 log10 cfu/mL for this strain); log10bmax below carries the HFIM value because that ",
  ...
  ini({
    ...
    # ---- Bacterial growth ---------------------------------------------------
    kg_s   <- 1.47; label("Growth rate constant of the susceptible subpopulation (1/h)")      # Table 3 (ISOL MIC4): k_g,S 1.47, %RSE 5.8
    kg_ls  <- 0.796; label("Growth rate constant of the less susceptible subpopulation (1/h)") # Table 3 (ISOL MIC4): k_g,LS 0.796, %RSE 3.0
    kg_res <- 0.558; label("Growth rate constant of the resistant subpopulation (1/h)")       # Table 3 (ISOL MIC4): k_g,RES 0.558, %RSE 3.7

    log10bmax <- 10.4; label("Maximum bacterial density (log10 cfu/mL)") # Table 3 (ISOL MIC4): Bmax,HFIM 10.4, %RSE 1.0; the static time-kill value Bmax,STK was 8.63 (%RSE 0.7)
    ...
    emax_s  <- 2.38; label("Maximum drug effect rate constant, susceptible subpopulation (1/h)")             # Table 3 (ISOL MIC4): Emax,S 2.38, %RSE 3.5
    ec50_s  <- 1.57; label("Unbound concentration at half-maximal effect, susceptible subpopulation (mg/L)") # Table 3 (ISOL MIC4): EC50,S 1.57, %RSE 3.3
    hill_s  <- 2.73; label("Hill coefficient, susceptible subpopulation")                                 # Table 3 (ISOL MIC4): H,S 2.73, %RSE 12.8

    emax_ls <- 0.843; label("Maximum drug effect rate constant, less susceptible subpopulation (1/h)")             # Table 3 (ISOL MIC4): Emax,LS 0.843, %RSE 2.5
    ec50_ls <- 6.34; label("Unbound concentration at half-maximal effect, less susceptible subpopulation (mg/L)") # Table 3 (ISOL MIC4): EC50,LS 6.34, %RSE 1.1
    hill_ls <- fixed(10); label("Hill coefficient, less susceptible subpopulation")                                 # Table 3 (ISOL MIC4): H,LS 10, FIX

    klin_res <- 0.0421; label("Linear drug effect rate constant, resistant subpopulation (L/(mg*h))") # Table 3 (ISOL MIC4): k_lin,RES 0.0421, %RSE 3.6

    # ---- Initial bacterial population sizes ---------------------------------
    log10cfu0_s   <- 5.78; label("Fitted initial size of the susceptible subpopulation (log10 cfu/mL)")      # Table 3 (ISOL MIC4): cfu_t0,S 5.78, %RSE 0.9
    log10cfu0_ls  <- 2.08; label("Fitted initial size of the less susceptible subpopulation (log10 cfu/mL)") # Table 3 (ISOL MIC4): cfu_t0,LS 2.08, %RSE 5.4
    log10cfu0_res <- -0.271; label("Fitted initial size of the resistant subpopulation (log10 cfu/mL)")       # Table 3 (ISOL MIC4): cfu_t0,RES -0.271, %RSE 47.2
    ...
    # the log10 scale."
    addSd_CFUtotal <- 0.413; label("Additive residual SD on the log10 total bacterial count (log10 cfu/mL)")     # Table 3 (ISOL MIC4): RUV total 0.413, %RSE 4.4
    addSd_CFUres   <- 0.638; label("Additive residual SD on the log10 resistant bacterial count (log10 cfu/mL)") # Table 3 (ISOL MIC4): RUV RES 0.638, %RSE 11.9
  })
  ...
  model({
    ...
    # log10(1e6 / (10^5.78 + 10^2.08)) log10 cfu/mL.
