Saleh_2023_prexasertib_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, CD1 nude).",
    "Nine-compartment CNS PBPK model predicting unbound prexasertib",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 10 mg/kg subcutaneous dose. The CNS structure is the mouse",
    ...
    "0.09 (estimated by the authors; Table V). The plasma PK model is the",
    "empirical two-compartment model of Table IV (ref [22]) and acts",
  ...
  ini({
    ...
    lcl <- log(2.68); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(51.59); label("Central compartment volume (mL)")  # Table IV
    lq  <- log(0.276); label("Intercompartmental clearance (mL/min)")  # Table IV
    lvp <- log(97.47); label("Peripheral compartment volume (mL)")  # Table IV
    lka <- log(0.018); label("First-order absorption rate constant (1/min)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(365); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(1.77); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(10.02); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(9.85); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.11); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.09); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (estimated)
    ...
    kpuu_lv <- fixed(0.09); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.09); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    etalcl ~ 0.046  # Table IV IIV on central clearance (variance)
    etalq ~ 0.027  # Table IV IIV on intercompartmental clearance (variance)
    etalka ~ 0.04  # Table IV IIV on first-order absorption (variance)

    # ---- Residual unexplained variability: Table IV (reported as VARIANCES) ----
    propSd <- sqrt(0.003); label("Proportional residual error (fraction)")  # Table IV: 0.003 as variance
  })
