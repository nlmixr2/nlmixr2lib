Chen_2025_hemoporfin_any_improvement <- function() {
  description <- paste0(
    ...
    "therapy as category I, II or III (any improvement, at least 20% ",
    ...
    "expit(1.56 - 0.0700 * AUC_HEMO) where AUC_HEMO is the individual ",
    "hemoporfin AUC(0-30min) in h*ug/mL derived from the companion ",
    "population PK model (Chen 2025 Table S1). This is the only one of the ",
    "five endpoints with a NEGATIVE exposure slope, and it is far from ",
    "significant (slope p = 0.790, 95% CI -0.605 to 0.460); the sign is a ",
    "sampling artefact in a 24-patient cohort in which 66.7% of patients ",
    "achieved any improvement, not evidence that exposure harms response, ",
    "and it must not be read as a dose-response reversal. The ",
    "non-significance is the paper's reported result, not a transcription ",
    "gap: of the five endpoints screened, only the investigator- and ",
    "patient-rating endpoints were significant. There is no PK layer and no ",
    "ODE -- exposure enters as a covariate column. No between-subject ",
    "random effect and no residual error are estimated (Bernoulli ",
    "likelihood). Companion models: Chen_2025_hemoporfin (population PK) ",
    "and four sibling endpoints in the Chen_2025_hemoporfin_* family."
  )
  reference <- paste(
  ...
  units <- list(
    ...
    concentration = "prob_any_improvement (probability of expert-panel category I, II or III, 0-1; also logit_any_improvement)"
  )
  ...
  population <- list(
    ...
    notes          = paste0(
      ...
      "is category I, II or III; 66.7% of the pediatric cohort achieved it, ",
      "against 89.7% of adults in the phase III registration trial ",
      "(Discussion). Only the pediatric trial contributed efficacy data -- ",
      "the adult phase I trial was PK-only and involved no photodynamic ",
      "therapy."
    )
  )
  ...
  ini({
    ...
    # any improvement (category I+II+III)". Model form is Equation 3:
    ...
    #
    # NEITHER coefficient is significant, and the slope is the only
    # negative one of the five endpoints. Its confidence interval
    # (-0.605 to 0.460) is centred close to zero and comfortably spans
    # it. Extracted verbatim rather than sign-corrected: tuning a
    # published estimate to the expected direction is not permitted, and
    # the flat/noisy relationship is itself the reported finding for
    # this endpoint. See the vignette's Assumptions and deviations.
    # ==================================================================

    # ----- Logit intercept -----
    logit_ref <- 1.56 ; label("Logit of the probability of expert-panel category I, II or III at AUC_HEMO = 0 h*ug/mL (unitless logit)")  # Table S1, beta0 = 1.56 (95% CI -4.97 to 8.39, p = 0.637; not significant)

    # ----- Exposure effect on the logit -----
    e_auc_hemo_logit <- -0.0700 ; label("Log-odds of expert-panel category I, II or III per 1 h*ug/mL increase in hemoporfin AUC(0-30min) (mL/(ug*h))")  # Table S1, beta1 = -0.0700 mL*ug^-1*h^-1 (95% CI -0.605 to 0.460, p = 0.790; not significant)
    ...
    addSd_prob_any_improvement <- fixed(0.001) ; label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    # ----- Linear predictor (Chen 2025 Equation 3, Table S1) -----
    logit_any_improvement <- logit_ref + e_auc_hemo_logit * AUC_HEMO

    prob_any_improvement <- expit(logit_any_improvement)

    # ----- Observation -----
    prob_any_improvement ~ add(addSd_prob_any_improvement)
  })
}
