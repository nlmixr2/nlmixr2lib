Yau_2023_diazepam_monkey_pbpk <- function() {
  description <- paste(
    "PBPK (simplified whole-body, 14 compartments). Preclinical",
    "(cynomolgus monkey). Diazepam disposition in monkey described by the",
    "Yau 2023 simplified physiologically based pharmacokinetic model with",
    "four common tissue Kpu scaling factors (Model 3D: k-means",
    ...
    "9.97 L/h. Model 3D was one of the two best-fitting monkey models and",
    "recovered a Vss,b of 12.16 L against an observed 11.1 L. Table 3",
    "reports no between-subject variability for this fit, so the model is",
  ...
  ini({
    ...
    # value").
    lcl <- fixed(log(9.97)); label("Total blood clearance CL (L/h)")
    # ^ Yau 2023 Results, Monkey: "Using a blood CL of 9.97 L/h"

    # Tissue Kpu scaling factors -- Model 3D (four scalars,
    # k-means clustering). Estimated on the log scale in NONMEM.
    lsf1 <- log(0.425); label("Kpu scalar group 1 (bone, brain, muscle, pancreas, rest of body)")
    # ^ Yau 2023 Table 3, Model 3D
    lsf2 <- log(14.8); label("Kpu scalar group 2 (kidney, spleen, liver)")
    # ^ Yau 2023 Table 3, Model 3D
    lsf3 <- log(3.38); label("Kpu scalar group 3 (skin, lung, gut, stomach, heart)")
    # ^ Yau 2023 Table 3, Model 3D
    lsf4 <- log(1.2); label("Kpu scalar group 4 (adipose)")
    # ^ Yau 2023 Table 3, Model 3D

    propSd <- 0.628; label("Proportional residual error (fraction)")
    # ^ Yau 2023 Table 3, Model 3D proportional error 62.8% (RSE 6.6%)
  })
  ...
  model({
    ...
    fup  <- 0.084   # fraction unbound in plasma, monkey
    bp   <- 0.606   # blood:plasma ratio, monkey
    fe   <- 0.0005   # fraction excreted unchanged in urine, monkey
    ...
    vssb <- (kpu_lung     * sf3 * v_lung +
            ...
            v_arterial + v_venous
    # Published Vss,b for this fit: 12.16 L.
