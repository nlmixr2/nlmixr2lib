LopezArguello_2023_meropenem_qsp <- function() {
  ...
  covariateData <- list(
    ...
    CONC_MEM_MGL = list(
      description = "Static extracellular meropenem concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
      reference_category = "n/a -- studied at 1 mg/L (2x the MIC of 0.5 mg/L)",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; meropenem MIC 0.5 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "meropenem at a static extracellular concentration of 1 mg/L (2x the MIC of 0.5 mg/L); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 35.1; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, meropenem: 35.1 (SE 48.1%)
    # The studied extracellular concentration enters as the CONC_MEM_MGL covariate
    # (Table 1: meropenem studied at 1 mg/L = 2x the MIC of 0.5 mg/L).
    ...
    k2_1a <- 74.5;      label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, meropenem PBP1a: 74.5
    k2_1b <- 307;       label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, meropenem PBP1b: 307
    k2_2 <- fixed(500); label("Second-order acylation rate constant, PBP2 (1e-3 /min; FIXED)") # Table S1, meropenem PBP2: 500, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_3 <- 449;        label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, meropenem PBP3: 449
    k2_4 <- 529;        label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, meropenem PBP4: 529
    k2_5 <- 34;         label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, meropenem PBP5/6: 34
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_MEM_MGL
