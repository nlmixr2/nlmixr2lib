LopezArguello_2023_sulbactam_qsp <- function() {
  ...
  covariateData <- list(
    ...
    CONC_SUL_MGL = list(
      description = "Static extracellular sulbactam concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; sulbactam MIC not determined (beta-lactamase inhibitor)",
    ...
    dose_range = "sulbactam at a static extracellular concentration of 4 mg/L (a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 1.49; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, sulbactam: 1.49 (SE 45.1%)
    # The studied extracellular concentration enters as the CONC_SUL_MGL covariate
    # (Table 1: sulbactam studied at 4 mg/L = a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors).
    ...
    k2_1a <- 14.9; label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, sulbactam PBP1a: 14.9
    k2_1b <- 27.4; label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, sulbactam PBP1b: 27.4
    k2_2 <- 16.5;  label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, sulbactam PBP2: 16.5
    k2_3 <- 0.317; label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, sulbactam PBP3: 0.317
    k2_4 <- 8.38;  label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, sulbactam PBP4: 8.38
    k2_5 <- 33.5;  label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, sulbactam PBP5/6: 33.5
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_SUL_MGL
