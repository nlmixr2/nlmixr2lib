Law_2017_ecg_rat_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 13 blood-flow-limited tissue compartments).",
    "Preclinical (rat). Single-catechin model for epicatechin gallate (ECg)",
    "in rats (Law et al. 2017). Thirteen perfusion-limited compartments",
    "(lung, kidney, muscle, brain, liver, spleen, gut, bone, skin, heart,",
    "adipose, rest of body, blood split into a one-third arterial and a",
    "two-thirds venous pool) with first-order oral input into gut tissue",
    "after a lag time, renal elimination from the kidney, and an",
    "enterohepatic recycling loop in which a biliary clearance drains the",
    "liver through a three-sub-compartment bile duct into the gut lumen, from",
    "which drug is either reabsorbed into gut tissue or transported to",
    "faeces. Tissue volumes, blood flows and tissue/blood partition",
    "coefficients are the paper's reference rat values; the model carries no",
    "IIV and is intended for typical-value simulation. Law et al. built their",
    "tea catechin MIXTURE model by linking three of these single-catechin",
    "models with no pharmacokinetic interaction between them, so the mixture",
    "is reproduced by simulating the sibling models together - see the",
    "vignette. IMPORTANT: with Table 2 and Table 3 values exactly as printed",
    "this rat model does not quantitatively reproduce the paper's own Figures",
    "4, 5 and 8; the human siblings do reproduce the paper's Table 7",
    "predicted Cmax. See the vignette Errata."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    ...
    arterial = list(analyte = "ECg", units = "mg", specimen = "whole blood", verified = TRUE),
    venous = list(analyte = "ECg", units = "mg", specimen = "whole blood", verified = TRUE),
    lung = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    liver = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    kidney = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    gut = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    gut_lumen = list(analyte = "ECg", units = "mg", specimen = "faeces", verified = TRUE),
    bile_transit1 = list(analyte = "ECg", units = "mg", specimen = "bile", verified = TRUE),
    bile_transit2 = list(analyte = "ECg", units = "mg", specimen = "bile", verified = TRUE),
    bile_transit3 = list(analyte = "ECg", units = "mg", specimen = "bile", verified = TRUE),
    brain = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    heart = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    spleen = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    muscle = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    skin = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    bone = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    adipose = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE),
    other = list(analyte = "ECg", units = "mg", specimen = "tissue", verified = TRUE)
  )
  ...
  ini({
    ...
    # Table 3 (F ECg = 0.06)
    lfdepot <- fixed(log(0.06)); label("Empirical oral bioavailability factor F (unitless)")
    # Table 3 (tlag ECg = 10 min)
    ltlag <- fixed(log(10)); label("Oral absorption lag time (min)")
    # Table 3 (Rt ECg = 0.3 min)
    lmtt_bile <- fixed(log(0.3)); label("Bile-duct sub-compartment residence time Rt (min)")
    # Table 3 (krac ECg = 0.41)
    lkreab <- fixed(log(0.41)); label("Colonic reabsorption rate coefficient krac (/min per kg^-0.3)")
    # Table 3 (kfc ECg = 0.13)
    lkfec <- fixed(log(0.13)); label("Faecal transport rate coefficient kfc (/min per kg^-0.3)")
    # Table 3 (CLbc ECg = 12.6) (12.6 mL/min per kg^0.66 converted to L/min)
    lcl_nonren <- fixed(log(12.6 / 1000)); label("Biliary clearance coefficient CLbc (L/min per kg^0.66)")
    # Table 3 (CLrc ECg = 0.3) (0.3 mL/min per kg^0.66 converted to L/min)
    lcl_renal <- fixed(log(0.3 / 1000)); label("Renal clearance coefficient CLrc (L/min per kg^0.66)")

    # Table 1 (BLPLR ECg = 0.99, rat)
    bpr <- fixed(0.99); label("Blood/plasma concentration ratio BLPLR (unitless)")
    ...
    # Table 2 (adipose ECg = 0.75)
    lkp_adipose <- fixed(log(0.75)); label("Tissue/blood partition coefficient, adipose (unitless)")
    # Table 2 (bone ECg = 4)
    lkp_bone <- fixed(log(4)); label("Tissue/blood partition coefficient, bone (unitless)")
    # Table 2 (brain ECg = 7.84)
    lkp_brain <- fixed(log(7.84)); label("Tissue/blood partition coefficient, brain (unitless)")
    # Table 2 (gut ECg = 4.82)
    lkp_gut <- fixed(log(4.82)); label("Tissue/blood partition coefficient, gut (unitless)")
    # Table 2 (heart ECg = 2.65)
    lkp_heart <- fixed(log(2.65)); label("Tissue/blood partition coefficient, heart (unitless)")
    # Table 2 (kidney ECg = 3.1)
    lkp_kidney <- fixed(log(3.1)); label("Tissue/blood partition coefficient, kidney (unitless)")
    # Table 2 (liver ECg = 3.37)
    lkp_liver <- fixed(log(3.37)); label("Tissue/blood partition coefficient, liver (unitless)")
    # Table 2 (lung ECg = 3.83)
    lkp_lung <- fixed(log(3.83)); label("Tissue/blood partition coefficient, lung (unitless)")
    # Table 2 (muscle ECg = 1.97)
    lkp_muscle <- fixed(log(1.97)); label("Tissue/blood partition coefficient, muscle (unitless)")
    # Table 2 (skin ECg = 4.23)
    lkp_skin <- fixed(log(4.23)); label("Tissue/blood partition coefficient, skin (unitless)")
    # Table 2 (spleen ECg = 1.85)
    lkp_spleen <- fixed(log(1.85)); label("Tissue/blood partition coefficient, spleen (unitless)")
    # Table 2 (other ECg = 1)
