Valke_2024_factorviii_thrombinPeak <- function() {
  description <- paste(
    "Sequential PK-PD model relating factor VIII (FVIII) activity to",
    "normalized thrombin peak height (% of normal pooled plasma) measured",
    "with the Nijmegen hemostasis assay, after a single intravenous bolus of",
    ...
    "is a proportional-baseline Emax model, E = Ebase * (1 + Emax * C^n /",
    "(EC50^n + C^n)), with baseline 11.2% of normal pooled plasma, Emax 8.06",
    "as a factor of baseline, EC50 51.6 IU/dL and the Hill coefficient fixed",
    "at 1. Inter-individual variability is carried on the baseline (34.7%);",
    "residual error on the PD readout is additive (17.4% of normal pooled",
    "plasma).",
    sep = " "
  )
  reference <- paste(
  ...
  units <- list(
    time          = "h",
    dosing        = "IU",
    concentration = "IU/dL (observation Cc is FVIII activity; the PD output thrombinPeak is normalized thrombin peak height as a percentage of normal pooled plasma)"
  )
  ...
  population <- list(
    ...
    weight_median  = "62 kg",
    sex_female_pct = 0,
    disease_state  = "Severe hemophilia A (FVIII activity level < 1 IU/dL). Baseline thrombin peak height 15 nM (undetectable-19), i.e. 6% of normal pooled plasma (undetectable-8), against a healthy-control reference of 134 (SD 32) nM.",
    ...
    notes          = "Sub-study of the IMPALA study (Dutch Trial Register NL2808). All patients male. 285 normalized thrombin peak height values were available for the PD analysis; nine were undetectable and were excluded (< 4% of samples). The analysis was sequential: individual PK parameters from the Valke 2024 PK model were fixed as the input to the PD estimation step. This is a replication study of Bukkems 2022 (Br J Clin Pharmacol 88(6):2757-2768); the Bukkems model predicted this cohort without bias (MPE 6.83%) but imprecisely (MAPE 38.6%), so the PD parameters were re-estimated. The Bukkems estimates reproduced in Valke 2024 Table 3 for comparison are NOT encoded here."
  )
  ...
  ini({
    ...
    # Normalized thrombin peak height, proportional-baseline Emax model
    # (Valke 2024 Supplementary Methods Eqs. 8 + 9 and the formula block
    # printed beneath Table 3).
    le0    <- log(11.2) ; label("Baseline normalized thrombin peak height (% of normal pooled plasma)")            # Valke 2024 Table 3: Baseline effect = 11.2% of NPP (RSE 26.3%)
    lemax  <- log(8.06) ; label("Maximal FVIII effect on normalized thrombin peak height (factor of baseline)")    # Valke 2024 Table 3: Maximal effect = 8.06 x baseline (RSE 32.6%)
    lec50  <- log(51.6) ; label("FVIII activity producing half the maximal thrombin peak height effect (IU/dL)")   # Valke 2024 Table 3: EC50 = 51.6 IU/dL (RSE 13.2%)
    lhill  <- fixed(log(1)) ; label("Hill coefficient of the thrombin peak height Emax relationship (unitless)")   # Valke 2024 Table 3: Hill coefficient = 1 FIX
    ...
    # largest drop in objective function value (Valke 2024 Results 3.5).
    etale0 ~ 0.120409   # Valke 2024 Table 3: IIV on the baseline effect = 34.7% (RSE 33.6%) [Shr 7.5%]; omega^2 = 0.347^2
    ...
    addSd_thrombinPeak <- 17.4 ; label("Additive residual SD for normalized thrombin peak height (% of normal pooled plasma)")   # Valke 2024 Table 3: Additive error = 17.4% of NPP (RSE 7.4%)
  })
  ...
  model({
    ...
    # ---- PD: normalized thrombin peak height (% of normal pooled plasma) ----
    # Valke 2024, formula block beneath Table 3:
    #   E = Ebase * (1 + Emax * C^n / (EC50^n + C^n))
    # i.e. the proportional-baseline relation of Supplementary Eq. 9 wrapped
    # around the sigmoid Emax drug effect of Supplementary Eq. 8. Emax is a
    # multiple of the baseline, so the asymptote is Ebase * (1 + Emax) =
    # 11.2 * 9.06 = 101% of normal pooled plasma.
    e0   <- exp(le0 + etale0)
    emax <- exp(lemax)
    ec50 <- exp(lec50)
    hill <- exp(lhill)
    ...
    thrombinPeak  <- e0 * (1 + edrug)
    thrombinPeak ~ add(addSd_thrombinPeak)
  })
}
