Reif_2013_ethinylestradiol <- function() {
  ...
  ini({
    ...
    lcl <- log(25.3)
    label("Apparent oral clearance for a typical subject aged 24 years weighing 62 kg (L/h)")  # Appendix 1: TVCL/F = 25.3 l/hour (RSE 1.24%)
    lvc <- log(23.9)
    label("Apparent central volume of distribution (L)")  # Appendix 1: V2/F = 23.9 l (RSE 13.6%)
    lvp <- log(1330)
    label("Apparent deep peripheral volume of distribution (L)")  # Appendix 1: V3/F = 1,330 l (RSE 3.62%)
    lq <- log(52.9)
    label("Apparent inter-compartmental clearance to the deep peripheral compartment (L/h)")  # Appendix 1: Q3/F = 52.9 l/hour (RSE 7.01%)
    lq2 <- log(8.49)
    label("Apparent inter-compartmental clearance to the shallow peripheral compartment (L/h)")  # Appendix 1: Q4/F = 8.49 l/hour (RSE 34.3%)
    ltlag <- log(0.353)
