Cao_2017_dha_troph_late <- function() {
  description <- paste(
    "In vitro (P. falciparum 3D7 laboratory strain, late-trophozoite-stage parasites).",
    "Dynamic stress PD model from Cao 2017 capturing the delayed dihydroartemisinin (DHA)",
    "killing effect; one of four stage-specific NLME fits in Table 1 (late-trophozoite",
    "stage corresponds to 34 h post-infection). The killing rate",
    ...
    "modellib('Cao_2017_dha_ring_mid'), modellib('Cao_2017_dha_troph_early') for the",
  ...
  population <- list(
    ...
    disease_state  = paste(
      ...
      "late trophozoite stage (34 h post-infection per Klonis et al 2013, Cao 2017",
    ...
    notes          = paste(
      "Late trophozoite has the second-largest alpha among the four stages and a",
      "moderate stress accumulation rate (lambda = 2.09 /h, half-life of unstressed",
      "state 0.33 h). Stage-pooled Hill coefficient gamma fixed at 1.7892 (Table 1",
      "footnote a). Residual error variances were estimated by NONMEM but are not",
      "numerically reported in Table 1; per nlmixr2lib policy the file is encoded as a",
      "typical-value deterministic mechanism without residual error or IIV; see",
      "vignette Errata."
    )
  )
  ...
  ini({
    # =====================================================================
    # Stage-specific NLME estimates -- LATE TROPHOZOITE stage (Cao 2017 Table 1).
    # =====================================================================

    lambda <- fixed(2.0906)
    label("Stress accumulation rate lambda (1/h)")
    # Cao 2017 Table 1, Late trophozoite stage: lambda = 2.0906 /h
    # (SE 0.2909; model-based 95% CI 1.5203-2.6608). Half-life of the
    # unstressed state ln(2)/lambda = 0.33 h (Table 1 parenthetical).

    alpha <- fixed(2.8626)
    label("Maximum achievable killing rate alpha (1/h)")
    # Cao 2017 Table 1, Late trophozoite stage: alpha = 2.8626 /h
    # (SE 0.1591; model-based 95% CI 2.5508-3.1744).

    beta1 <- fixed(740.02)
    label("Stress-dependent Kc slope beta1 (nM)")
    # Cao 2017 Table 1, Late trophozoite stage: beta1 = 740.02 nM
    # (SE 178.77; model-based 95% CI 389.64-1090.41).

    beta2 <- fixed(41.405)
    label("Stationary half-maximal killing concentration beta2 (nM)")
    # Cao 2017 Table 1, Late trophozoite stage: beta2 = 41.405 nM
    # (SE 3.6606; model-based 95% CI 34.230-48.580). beta2 = Kc(S = 1)
    # per eq 8.
    ...
    # Cao 2017 Fig 6 legend: C* = 0.1 nM.
