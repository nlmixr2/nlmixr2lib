Saleh_2023_cefadroxil_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, C57BL/6 Pept2+/+).",
    "Nine-compartment CNS PBPK model predicting unbound cefadroxil",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 36 mg/kg intravenous dose. The CNS structure is the mouse",
    ...
    "0.05 (estimated by the authors; Table V). The plasma PK model is the",
    "empirical one-compartment model of Table IV (in-house Monolix fit) and acts",
  ...
  ini({
    ...
    lcl <- log(0.97); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(14); label("Central compartment volume (mL)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(363); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(0.51); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(3.25); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(7.22); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(1); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.05); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (estimated)
    ...
    kpuu_lv <- fixed(0.05); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.05); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    # Table IV reports all interindividual variances as 0 for cefadroxil;
    ...
    # Table IV reports both residual variances as 0 for cefadroxil. Encoded as a
    # fixed zero rather than invented: the published plasma model is an exact
    # deterministic description. See vignette Assumptions and deviations.
    propSd <- fixed(0); label("Proportional residual error (fraction)")  # Table IV: 0
  })
