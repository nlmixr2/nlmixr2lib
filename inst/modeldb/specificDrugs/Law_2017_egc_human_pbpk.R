Law_2017_egc_human_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 13 blood-flow-limited tissue compartments).",
    "Single-catechin model for epigallocatechin (EGC) in humans (Law et al.",
    "2017). Thirteen perfusion-limited compartments (lung, kidney, muscle,",
    "brain, liver, spleen, gut, bone, skin, heart, adipose, rest of body,",
    "blood split into a one-third arterial and a two-thirds venous pool) with",
  ...
  compartmentData <- list(
    depot = list(analyte = "EGC", units = "mg", specimen = "administration site", verified = TRUE),
    arterial = list(analyte = "EGC", units = "mg", specimen = "whole blood", verified = TRUE),
    venous = list(analyte = "EGC", units = "mg", specimen = "whole blood", verified = TRUE),
    lung = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    liver = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    kidney = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    gut = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    gut_lumen = list(analyte = "EGC", units = "mg", specimen = "faeces", verified = TRUE),
    bile_transit1 = list(analyte = "EGC", units = "mg", specimen = "bile", verified = TRUE),
    bile_transit2 = list(analyte = "EGC", units = "mg", specimen = "bile", verified = TRUE),
    bile_transit3 = list(analyte = "EGC", units = "mg", specimen = "bile", verified = TRUE),
    brain = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    heart = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    spleen = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    muscle = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    skin = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    bone = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    adipose = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE),
    other = list(analyte = "EGC", units = "mg", specimen = "tissue", verified = TRUE)
  )
  ...
  ini({
    ...
    # Table 5 (kac EGC = 2.19 /h per kg^-0.3; one value for all studies)
    lka <- fixed(log(2.19)); label("Absorption rate coefficient kac into gut tissue (/h per kg^-0.3)")
    # Table 5, square-bracket value (F EGC = 0.013, Chow et al. 2001); 0.052
    # simulates Lee et al. (2002)
    lfdepot <- fixed(log(0.013)); label("Empirical oral bioavailability factor F (unitless)")
    # Table 5 (tlag EGC = 0.4 h)
    ltlag <- fixed(log(0.4)); label("Oral absorption lag time (h)")
    # Table 5 (Rt EGC = 0.03 h)
    lrt_bile <- fixed(log(0.03)); label("Bile-duct sub-compartment residence time Rt (h)")
    # Table 5 (krac EGC = 0.18)
    lkreab <- fixed(log(0.18)); label("Colonic reabsorption rate coefficient krac (/h per kg^-0.3)")
    # Table 5 (kfc EGC = 25.6)
    lkfec <- fixed(log(25.6)); label("Faecal transport rate coefficient kfc (/h per kg^-0.3)")
    # Table 5 (CLbc EGC = 0.97)
    lcl_nonren <- fixed(log(0.97)); label("Biliary clearance coefficient CLbc (L/h per kg^0.66)")
    # Table 5 (CLrc EGC = 0.34)
    lcl_renal <- fixed(log(0.34)); label("Renal clearance coefficient CLrc (L/h per kg^0.66)")

    # Table 1 (BLPLR EGC = 0.88; human values assumed equal to rat)
    ...
    # Table 4 (adipose EGC = 0.01)
    lkp_adipose <- fixed(log(0.01)); label("Tissue/blood partition coefficient, adipose (unitless)")
    # Table 4 (bone EGC = 0.48)
    lkp_bone <- fixed(log(0.48)); label("Tissue/blood partition coefficient, bone (unitless)")
    # Table 4 (brain EGC = 0.69)
    lkp_brain <- fixed(log(0.69)); label("Tissue/blood partition coefficient, brain (unitless)")
    # Table 4 (gut EGC = 0.53)
    lkp_gut <- fixed(log(0.53)); label("Tissue/blood partition coefficient, gut (unitless)")
    # Table 4 (heart EGC = 0.51)
    lkp_heart <- fixed(log(0.51)); label("Tissue/blood partition coefficient, heart (unitless)")
    # Table 4 (kidney EGC = 0.56)
    lkp_kidney <- fixed(log(0.56)); label("Tissue/blood partition coefficient, kidney (unitless)")
    # Table 4 (liver EGC = 0.59)
    lkp_liver <- fixed(log(0.59)); label("Tissue/blood partition coefficient, liver (unitless)")
    # Table 4 (lung EGC = 0.51)
    lkp_lung <- fixed(log(0.51)); label("Tissue/blood partition coefficient, lung (unitless)")
    # Table 4 (muscle EGC = 0.54)
    lkp_muscle <- fixed(log(0.54)); label("Tissue/blood partition coefficient, muscle (unitless)")
    # Table 4 (skin EGC = 0.53)
    lkp_skin <- fixed(log(0.53)); label("Tissue/blood partition coefficient, skin (unitless)")
    # Table 4 (spleen EGC = 0.56)
    lkp_spleen <- fixed(log(0.56)); label("Tissue/blood partition coefficient, spleen (unitless)")
    # Table 4 (other EGC = 1)
