Shen_2026_shikimicAcid_broiler_mda <- function() {
  description <- paste(
    ...
    "on serum malondialdehyde (MDA) in 5-week-old yellow-feathered broilers after a",
    ...
    "stimulates the first-order loss kout. The biomarker baseline is the",
    ...
    "modellib('Shen_2026_shikimicAcid_broiler_sod'); for the same drug in",
  ...
  compartmentData <- list(
    ...
    mda = list(
      analyte = "malondialdehyde (MDA)", units = "nmol/mL", specimen = "serum",
      verified = TRUE
    )
  )
  ...
  ini({
    ...
    # The MDA row is carried here; its model was selected on AIC -13.85 +/- 3.54.
    #
    # Shen 2026 defines E0 = kin/kout, and that ratio, 0.84 / 0.24 = 3.500 nmol/mL, reproduces
    # the 0 h bar of Figure 3C (digitised at 3.534 nmol/mL). Because Table 5 states no
    ...
    lke0 <- fixed(log(1.06)); label("Effect-compartment equilibration rate constant ke0 (1/h)")  # Table 5, MDA row: ke0 = 1.06 +/- 0.82
    lslope <- fixed(log(0.01)); label("Linear stimulation coefficient S of SA on MDA loss (mL/ug)")  # Table 5, MDA row: S = 0.01 +/- 0.007
    lkin <- fixed(log(0.84)); label("MDA zero-order production rate kin (nmol/mL/h)")  # Table 5, MDA row: kin = 0.84 +/- 0.45
    lkout <- fixed(log(0.24)); label("MDA first-order loss rate constant kout (1/h)")  # Table 5, MDA row: kout = 0.24 +/- 0.13
    ...
    addSd <- fixed(0); label("Additive residual SD on serum MDA (nmol/mL; final value not reported)")  # Shen 2026 reports only the initial SD of 1.0
  })
  ...
  model({
    ...
    # for MDA, whose levels FELL after dosing, SA stimulates the
    # first-order loss instead, dE/dt = kin - kout * (1 + S * Ce) * E.
    # The baseline is the drug-free steady state E0 = kin/kout, stated
    # explicitly by Shen 2026.
    d/dt(mda) <- kin - kout * (1 + slope * effect) * mda
    mda(0) <- kin / kout
