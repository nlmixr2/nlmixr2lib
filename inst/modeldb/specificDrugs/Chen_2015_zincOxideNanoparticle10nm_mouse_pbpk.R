Chen_2015_zincOxideNanoparticle10nm_mouse_pbpk <- function() {
  description <- paste(
    "Preclinical (mouse, ICR, male, 6 weeks, 0.031-0.032 kg).",
    "PBPK (whole-body, flow-limited, Berkeley Madonna 8.3.9) model for the",
    "biodistribution of a single intravenous dose of 10 nm radiolabelled zinc",
    ...
    "faecal and urinary excretion from gut and kidney. The distinguishing",
    "feature is that the tissue:blood partition coefficients and the",
    "excretion/elimination rate constants are TIME-DEPENDENT: brain and carcass",
    "partitioning rises as a four-parameter Hill function of time, the three",
    "excretion rates fall as a Hill function of time, and lung partitioning is",
    "piecewise constant (very high over the first 7 hours, low thereafter).",
    "The authors' calibrated model additionally assumes the nanoparticles have",
    "largely decomposed to zinc ion by day 7 and switches every partition",
    "coefficient and excretion rate to a second, constant parameter set at that",
    "point (Table 4); the switch time is exposed as `tdecomp` so that setting it",
    "beyond the simulation horizon reproduces the paper's uncalibrated",
    "simulation. States hold CONCENTRATION (ug/mL in blood, ug/g in tissue), as",
    "printed in the source, so the intravenous dose is converted to a blood",
    "concentration by `f(blood) <- 1 / v_blood`. The paper reports no",
    "between-subject variability and no residual error model -- it was fitted by",
    "minimising mean absolute percentage error -- so the model is for",
    "typical-value simulation only. One printed mass-balance anomaly in the",
    "liver-to-gut biliary term is reproduced verbatim; see the vignette Errata.",
    sep = " "
  )
  reference <- paste(
  ...
  ini({
    ...
    # ==================================================================
    # Early phase (up to day 7) -- Chen 2015 Table 3, 10 nm 65ZnO NP column
    # ==================================================================
    ...
    kp_liver_early  <- fixed(8.1)  ; label("Liver:blood partition coefficient, days 0-7 (unitless)")     # Table 3, p li, 10 nm, Xini = 8.1 (footnote b, constant)
    kp_kidney_early <- fixed(7.11) ; label("Kidney:blood partition coefficient, days 0-7 (unitless)")    # Table 3, p Ki, 10 nm, Xini = 7.11 (constant)
    kp_spleen_early <- fixed(6)    ; label("Spleen:blood partition coefficient, days 0-7 (unitless)")    # Table 3, p sp, 10 nm, Xini = 6 (constant)
    kp_heart_early  <- fixed(2.55) ; label("Heart:blood partition coefficient, days 0-7 (unitless)")     # Table 3, p he, 10 nm, Xini = 2.55 (constant)
    kp_gut_early    <- fixed(4.4)  ; label("GI tract:blood partition coefficient, days 0-7 (unitless)")  # Table 3, p gI, 10 nm, Xini = 4.4 (constant)

    # The lung partition coefficient is piecewise constant within the early
    # phase (Table 3 footnotes e and f). The authors state in Results
    # ("Simulation and model validation") that these are OPTIMIZED values used
    # to describe the observed lung profile, which rises steeply to 7 hours and
    # then falls; they are deliberately not the AUC ratios of Table S1.
    kp_lung_early <- fixed(64)   ; label("Lung:blood partition coefficient, hours 0-7 (unitless)")    # Table 3, p lu, 10 nm, first value 64 (footnote e, first 7 hours)
    kp_lung_mid   <- fixed(6.33) ; label("Lung:blood partition coefficient, hour 7 to day 7 (unitless)")  # Table 3, p lu, 10 nm, second value 6.33 (footnote f, hour 7 to day 7)
    ...
    kp_brain_ini  <- fixed(0.33)   ; label("Brain:blood partition coefficient at time zero (unitless)")            # Table 3, p Br, 10 nm, Xini
    kp_brain_max  <- fixed(2.87)   ; label("Maximum brain:blood partition coefficient (unitless)")                 # Table 3, p Br, 10 nm, Xmax
    kp_brain_t50  <- fixed(82.532) ; label("Time to half-maximum brain:blood partition coefficient (h)")           # Table 3, p Br, 10 nm, TX50
    kp_brain_hill <- fixed(1.014)  ; label("Hill coefficient of the brain partition-coefficient rise (unitless)")  # Table 3, p Br, 10 nm, Xn (r2 = 0.97)

    kp_other_ini  <- fixed(0.77)  ; label("Carcass:blood partition coefficient at time zero (unitless)")             # Table 3, p ca, 10 nm, Xini
    kp_other_max  <- fixed(2.67)  ; label("Maximum carcass:blood partition coefficient (unitless)")                  # Table 3, p ca, 10 nm, Xmax
    kp_other_t50  <- fixed(8.505) ; label("Time to half-maximum carcass:blood partition coefficient (h)")            # Table 3, p ca, 10 nm, TX50
    kp_other_hill <- fixed(1.007) ; label("Hill coefficient of the carcass partition-coefficient rise (unitless)")   # Table 3, p ca, 10 nm, Xn (r2 = 0.98)

    # All three excretion/elimination rate constants FALL with time and use the
    # DIFFERENT Hill form of Equation 4 (note: not Equation 2):
    #   k(t) = k_min + (k_max - k_min) / (1 + (t / T50)^n)
    # so k(0) = k_max and k(inf) = k_min. This reproduces the decreasing
    # tabulated rates of Table S4.
    kbile_min  <- fixed(0.019)  ; label("Minimum biliary excretion rate constant (1/h)")                    # Table 3, k li, 10 nm, Xini
    kbile_max  <- fixed(0.031)  ; label("Maximum biliary excretion rate constant (1/h)")                    # Table 3, k li, 10 nm, Xmax
    kbile_t50  <- fixed(41.822) ; label("Time to half-maximum biliary excretion rate constant (h)")         # Table 3, k li, 10 nm, TX50
    kbile_hill <- fixed(3.246)  ; label("Hill coefficient of the biliary excretion-rate decline (unitless)")  # Table 3, k li, 10 nm, Xn (r2 = 0.99)

    kfeces_min  <- fixed(0.017)  ; label("Minimum GI tract excretion rate constant (1/h)")                    # Table 3, k gI, 10 nm, Xini
    kfeces_max  <- fixed(0.034)  ; label("Maximum GI tract excretion rate constant (1/h)")                    # Table 3, k gI, 10 nm, Xmax
    kfeces_t50  <- fixed(19.639) ; label("Time to half-maximum GI tract excretion rate constant (h)")         # Table 3, k gI, 10 nm, TX50
    kfeces_hill <- fixed(1.850)  ; label("Hill coefficient of the GI excretion-rate decline (unitless)")      # Table 3, k gI, 10 nm, Xn (r2 = 0.99)

    kurine_min  <- fixed(0.010)  ; label("Minimum renal elimination rate constant (1/h)")                     # Table 3, k Ki, 10 nm, Xini
    kurine_max  <- fixed(0.032)  ; label("Maximum renal elimination rate constant (1/h)")                     # Table 3, k Ki, 10 nm, Xmax
    kurine_t50  <- fixed(12.954) ; label("Time to half-maximum renal elimination rate constant (h)")          # Table 3, k Ki, 10 nm, TX50
    kurine_hill <- fixed(1.465)  ; label("Hill coefficient of the renal elimination-rate decline (unitless)") # Table 3, k Ki, 10 nm, Xn (r2 = 0.99)

    # ==================================================================
    # Late phase (after day 7) -- Chen 2015 Table 4, 10 nm column
    # ==================================================================
    ...
    kp_liver_post  <- fixed(1.32) ; label("Liver:blood partition coefficient after day 7 (unitless)")     # Table 4, p li, 10 nm
    kp_kidney_post <- fixed(1.42) ; label("Kidney:blood partition coefficient after day 7 (unitless)")    # Table 4, p Ki, 10 nm
    kp_spleen_post <- fixed(1.54) ; label("Spleen:blood partition coefficient after day 7 (unitless)")    # Table 4, p sp, 10 nm
    kp_lung_post   <- fixed(1.25) ; label("Lung:blood partition coefficient after day 7 (unitless)")      # Table 4, p lu, 10 nm
    kp_heart_post  <- fixed(1.25) ; label("Heart:blood partition coefficient after day 7 (unitless)")     # Table 4, p he, 10 nm
    kp_gut_post    <- fixed(0.63) ; label("GI tract:blood partition coefficient after day 7 (unitless)")  # Table 4, p gI, 10 nm
    kp_brain_post  <- fixed(1.42) ; label("Brain:blood partition coefficient after day 7 (unitless)")     # Table 4, p Br, 10 nm
    kp_other_post  <- fixed(0.96) ; label("Carcass:blood partition coefficient after day 7 (unitless)")   # Table 4, p ca, 10 nm

    kbile_post  <- fixed(0.0192) ; label("Biliary excretion rate constant after day 7 (1/h)")    # Table 4, k li, 10 nm
    kfeces_post <- fixed(0.0343) ; label("GI tract excretion rate constant after day 7 (1/h)")   # Table 4, k gI, 10 nm
    kurine_post <- fixed(0.0319) ; label("Renal elimination rate constant after day 7 (1/h)")    # Table 4, k Ki, 10 nm
  })
  ...
  model({
    ...
    # ================================================================
    # 2. Time-dependent partition coefficients and excretion rates
    # ================================================================
    ...
    # Table 3 footnotes e and f: the lung is piecewise constant within the
    # early phase, switching at hour 7.
    kp_lung_t <- kp_lung_early * (t <= 7) + kp_lung_mid * (t > 7)

    # Equation 4 (Hill, FALLING -- a different algebraic form from Equation 2)
    # for the three excretion/elimination rate constants.
    kbile_t <- kbile_min +
      (kbile_max - kbile_min) / (1 + (t / kbile_t50)^kbile_hill)
    kfeces_t <- kfeces_min +
      (kfeces_max - kfeces_min) / (1 + (t / kfeces_t50)^kfeces_hill)
    kurine_t <- kurine_min +
    ...
    kp_spleen <- early * kp_spleen_early + (1 - early) * kp_spleen_post
    kp_heart  <- early * kp_heart_early  + (1 - early) * kp_heart_post
    kp_gut    <- early * kp_gut_early    + (1 - early) * kp_gut_post
    kp_lung   <- early * kp_lung_t       + (1 - early) * kp_lung_post
    kp_brain  <- early * kp_brain_t      + (1 - early) * kp_brain_post
    kp_other  <- early * kp_other_t      + (1 - early) * kp_other_post

    kbile  <- early * kbile_t  + (1 - early) * kbile_post
    kfeces <- early * kfeces_t + (1 - early) * kfeces_post
    kurine <- early * kurine_t + (1 - early) * kurine_post
