Kakara_2014_rosuvastatin <- function() {
  description <- paste(
    ...
    "is the current daily rosuvastatin dose (mg/day) supplied as a",
    ...
    "(CONMED_EZE = 1; estimated on the n=12 rosuvastatin + ezetimibe",
    "subgroup in this paper). The LDL-C synthesis-elimination loop is set",
    "up at steady state by enforcing Kin = Baseline * Kout (Kout derived",
    "inside model() as Kin / Baseline). Baseline LDL-C is age-scaled as",
    ...
    "Kakara_2014_atorvastatin and Kakara_2014_pitavastatin (one joint",
    "NONMEM 7.2 FOCE-INTER fit across 378 patients). Rosuvastatin ID50 =",
    "1.04 mg per Kakara 2014 Table 2."
  )
  reference <- paste(
  ...
  covariateData <- list(
    ...
    CONMED_EZE = list(
      ...
      notes              = paste(
        ...
        "INH_EZT row, RSE 18.9%) was estimated on this subgroup and",
        "applied in the joint model across all three statins; for",
        "rosuvastatin this is the within-cohort effect, for atorvastatin",
        "and pitavastatin it is an extrapolation. The paper's Simulation",
        "to assess the impact of ezetimibe focused on the rosuvastatin +",
  ...
  population <- list(
    ...
    notes            = paste(
      ...
      "median dosing period 304 days (range 1-766) and median 7 LDL-C",
      "samples per patient (range 1-26). Baseline LDL-C in the cohort",
      "had median 157 mg/dL (range 78-310) and median total cholesterol",
      "236 mg/dL (Table 1). Concomitant lipid-lowering coadministration:",
      "fenofibrate n=2, bezafibrate n=1, ezetimibe n=12, tocopherol",
      "nicotinate n=6. The total-cohort joint model in Table 2 was the",
      "only model published; per-cohort fits were not reported."
    )
  )
  ...
  ini({
    ...
    lid50  <- log(1.04);  label("Rosuvastatin daily dose at 50% of Imax ID50,rosu (mg/day)")   # Kakara 2014 Table 2 (ID50,Rosuvastatin = 1.04, RSE 29.5%)
