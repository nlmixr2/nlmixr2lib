Li_2025_doxycycline_duck_liver <- function() {
  ...
  units <- list(
    ...
    concentration = "ug/g (doxycycline in liver tissue)"
  )

  # auc_dox -- cumulative liver doxycycline AUC (ug*h/g), the quantity Li 2025
  #   tabulates for liver in Table 1. Accumulator idiom follows
  ...
  compartmentData <- list(
    ...
    liver   = list(analyte = "doxycycline", units = "mg/kg", specimen = "tissue", verified = TRUE),
  ...
  population <- list(
    ...
    sampling       = paste(
      "Liver tissue at 0.5, 1, 2, 4, 6, 8, 12, 24 and 36 h after dosing;",
      "0.5 g homogenised per sample and assayed by HPLC-MS/MS. Liver LLOQ",
      "0.01 ug/g, calibration 0.01-10 ug/g (R^2 > 0.99), recovery 83.30-96.18%"
    ),
    regions        = "China (Fuyang Normal University, Anhui; South China Agricultural University, Guangzhou)",
    notes          = paste(
      "Apparent clearance rose monotonically with dose in liver (0.100 to",
      "0.159 L/h/kg from 1 to 20 mg/kg), as it did in plasma; the mean is",
      "packaged here and the per-dose values are reproduced in the vignette.",
      "Liver was by far the highest-exposure matrix, with AUC 125.58 ug*h/g at",
      "20 mg/kg against 37.11 ug*h/mL in plasma (a 3.4-fold ratio), and the",
      "longest terminal half-life (13.01 h against 11.21 h in plasma)."
    )
  )

  ini({
    # =================================================================
    # Liver pharmacokinetics -- Li 2025 Table 1 (liver, "Mean" column)
    # =================================================================
    lka <- log(log(2) / 0.47)
    label("Log first-order absorption rate constant ka (1/h)")  # Li 2025 Table 1, liver mean T1/2ka = 0.47 +/- 0.13 h; ka = ln(2)/T1/2ka

    # Li 2025 tabulates Cl/F for plasma only. It is recovered for liver as the
    ...
    # 1/10.02 = 0.0998, 2.5/19.58 = 0.1277, 5/36.79 = 0.1359, 10/65.72 = 0.1522,
    # 20/125.58 = 0.1593 L/h/kg; mean 0.1350.
    lcl <- log(0.1350)
    label("Log apparent clearance from liver Cl/F (L/h/kg)")  # derived from Li 2025 Table 1 liver AUC24 at doses 1, 2.5, 5, 10, 20 mg/kg (10.02, 19.58, 36.79, 65.72, 125.58 ug*h/g)

    # V/F = (Cl/F)/kel with kel = ln(2)/T1/2kel. Reproduces Table 1's liver Tmax
    # and Cmax at every dose level to two decimal places (vignette source trace).
    lvc <- log(0.1350 / (log(2) / 13.01))
    label("Log apparent liver volume of distribution V/F (L/kg)")  # derived: recovered liver Cl/F = 0.1350 L/h/kg and Li 2025 Table 1 liver mean T1/2kel = 13.01 +/- 1.99 h
    ...
    propSd <- fixed(0)
    label("Proportional residual error on liver doxycycline concentration (0; not reported in Li 2025)")  # Li 2025 reported no residual error model
  })
  ...
  model({
    ...
    # liver concentration-time data (Li 2025 Methods). F is not identifiable from
    # intramuscular data alone, so cl and vc are apparent terms and no f(depot)
    # is applied.
    kel <- cl / vc
    d/dt(depot) <- -ka * depot
    d/dt(liver) <- ka * depot - kel * liver
