Zheng_2016_sifalimumab <- function() {
  ...
  ini({
    ...
    # not on baseline steroids (CONMED_STEROID = 0).
    ...
    # Covariate effects on CL - power-form on WT and BGENE21, fractional on CONMED_STEROID.
  ...
  model({
    ...
    cov_cl <- (WT / 64.3)^e_wt_cl * (BGENE21 / 12.04)^e_bgene21_cl * (1 + e_conmed_steroid_cl * CONMED_STEROID)
    cov_vc <- (WT / 64.3)^e_wt_vc * (DOSE / 600)^e_dose_vc         * (1 - e_conmed_steroid_vc * CONMED_STEROID)
