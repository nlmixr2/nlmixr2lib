Lacroix_2025_polymyxinB_AB122D12 <- function() {
  ...
  reference <- paste(
    ...
    "Structural model: supplemental text (AAC01535-24-S0002) Eqs S1-S4 and S7-S8 and",
    "supplemental Fig. S2 (right panel, AB122-D12), which prints the final kill-rate",
    ...
    "'AB122-D12 without mucin' and 'AB122-D12 with mucin'.",
    sep = " "
  )
  ...
  compartmentData <- list(
    bact_s = list(analyte = "Acinetobacter baumannii AB122-D12 polymyxin-B-susceptible subpopulation", units = "CFU/mL", specimen = "not applicable", verified = TRUE),
    bact_r = list(analyte = "Acinetobacter baumannii AB122-D12 polymyxin-B-resistant subpopulation", units = "CFU/mL", specimen = "not applicable", verified = TRUE)
  )
  ...
  covariateData <- list(
    CONC_PMB_MGL = list(
      ...
      units = "mg/L",
      type = "continuous",
      reference_category = "n/a -- AB122-D12 was studied at total PMB concentrations of 8 to 512 mg/L (Materials and Methods, TK experiments); 0 mg/L is the drug-free growth control.",
    ...
    MUCIN_PRESENT = list(
      ...
      notes = "Enters the model three ways. (i) Binding: when MUCIN_PRESENT = 1 the unbound fraction follows the sigmoidal Emax function of Eq 1; when 0 there is no mucin to bind PMB and fu is 1. (ii) and (iii) Residual activity effects: main-text Eq 4 selects Knet = knet_camhb / knet_mucin and EC50 = ec50_camhb / ec50_mucin. Note that the mucin switch is applied to EC50 BEFORE it is raised to the Hill power (supplemental Fig. S2, right panel).",
      source_name = "Mucin"
    )
  )

  population <- list(
    species = "in vitro (Acinetobacter baumannii AB122-D12)",
    n_subjects = NA_integer_,
    n_studies = 1L,
    organism = "A. baumannii AB122-D12, a multidrug-resistant clinical isolate recovered from the same patient 12 days after colistin treatment (the post-treatment member of the AB121-D0 / AB122-D12 pair). It carries a 10-amino-acid insertion in pmrB (p.21_22insCILIFSVILG) known to confer polymyxin B resistance. Polymyxin B MIC 32 mg/L in CAMHB and 1024 mg/L in CAMHB + 1% mucin, i.e. an unbound MIC (MICu) of 32 vs 628 mg/L, a 20-fold MICu increase (Table 2).",
    ...
    dose_range = "Total PMB 8 to 512 mg/L (plus a drug-free growth control); with 1% mucin these correspond to unbound PMB concentrations of about 0.5 to 304 mg/L after correction by fu.",
    ...
    notes = paste(
      ...
      "No PMB concentration tested in the presence of mucin (up to 304 mg/L unbound) prevented bacterial regrowth for this strain.",
      "Whole-genome sequencing of regrowth isolates found lpxACD mutations in 4 of 7 samples with a raised MIC, on top of the parental pmrB insertion."
    )
  )
  ...
  ini({
    ...
    # ===============================================================
    log10_inoc <- 5.45
    label("log10 initial total inoculum (log10 CFU/mL; INOC)")  # Lacroix 2025 Table 3, AB122-D12
    log10_cfumax <- 8.14
    label("log10 maximum bacterial population size / carrying capacity (log10 CFU/mL; BMAX)")  # Lacroix 2025 Table 3, AB122-D12
    log10_fmut <- -4.19
    label("log10 proportion of resistant bacteria in the initial inoculum (log10; MUT)")  # Lacroix 2025 Table 3, AB122-D12
    knet_camhb <- 2.42
    label("Apparent net growth rate constant without mucin (1/h; Knet)")  # Lacroix 2025 Table 3, AB122-D12 without mucin
    knet_mucin <- 1.34
    label("Apparent net growth rate constant with 1% mucin (1/h; Knet)")  # Lacroix 2025 Table 3, AB122-D12 with mucin

    # ===============================================================
    # PMB killing sub-model -- sigmoidal Emax, Eqs S7-S8
    # ===============================================================
    emax_s <- 33.4
    label("Maximum PMB kill rate constant, susceptible subpopulation (1/h; Emax_S)")  # Lacroix 2025 Table 3, AB122-D12
    emax_r <- 2.61
    label("Maximum PMB kill rate constant, resistant subpopulation (1/h; Emax_R)")  # Lacroix 2025 Table 3, AB122-D12 (no mucin effect for this strain)
    ec50_camhb <- 73.3
    label("Unbound PMB concentration giving 50% of Emax without mucin, both subpopulations (mg/L; EC50)")  # Lacroix 2025 Table 3, AB122-D12 without mucin
    ec50_mucin <- 384
    label("Unbound PMB concentration giving 50% of Emax with 1% mucin, both subpopulations (mg/L; EC50)")  # Lacroix 2025 Table 3, AB122-D12 with mucin
    hill <- 3.03
    label("Hill coefficient on the PMB killing effect (unitless; gamma)")  # Lacroix 2025 Table 3, AB122-D12

    # ===============================================================
    # Residual error
    # ===============================================================
    addSd <- 0.33
    label("Additive residual SD on the log10 total bacterial count (log10 CFU/mL; sigma)")  # Lacroix 2025 Table 3, AB122-D12
  })
  ...
  model({
    ...
    # For AB122-D12 mucin acts on Knet and on EC50 (supplemental Fig. S2,
    # right panel). Note the mucin switch is applied to EC50 BEFORE the Hill
    # power: [EC50_CAMHB * (1 - Mucin) + EC50_mucin * Mucin]^gamma.
    knet <- (knet_camhb * (1 - MUCIN_PRESENT)) + (knet_mucin * MUCIN_PRESENT)
    ec50 <- (ec50_camhb * (1 - MUCIN_PRESENT)) + (ec50_mucin * MUCIN_PRESENT)

    # --- Kill rates (Eqs S7-S8): sigmoidal Emax -------------------------
    cu_h <- cu^hill
    ec50_h <- ec50^hill
    kill_s <- emax_s * cu_h / ((ec50_h) + (cu_h))
    kill_r <- emax_r * cu_h / ((ec50_h) + (cu_h))
