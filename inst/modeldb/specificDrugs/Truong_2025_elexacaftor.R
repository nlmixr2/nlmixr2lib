Truong_2025_elexacaftor <- function() {
  ...
  compartmentData <- list(
    depot   = list(analyte = "elexacaftor", units = "mg", specimen = "administration site", verified = TRUE),
    central = list(analyte = "elexacaftor", units = "mg", specimen = "plasma", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "Weight-banded label dosing of elexacaftor once daily (Truong 2025 Methods 2.5): 80 mg for <14 kg; 100 mg for 14-30 kg; 200 mg for >30 kg or age >12 years.",
    regions        = "France; 20 pediatric university hospital centres.",
    n_observations = "150 elexacaftor plasma concentrations. Median 1 sample per child per compound (range 1-5). No elexacaftor sample fell below the limit of quantification.",
    co_medication  = "Concomitant tezacaftor and ivacaftor as part of the fixed triple combination. Strong CYP3A inducers and inhibitors were absent from the cohort.",
    notes          = "Ancillary pharmacokinetic sub-study of the MODUL-CF French prospective multicentre cohort (EUDRACT 2018-002624-16, NCT03894657), enrolling March 2022 to March 2024. Sparse therapeutic-drug-monitoring design: a trough just before intake and/or a peak roughly 4 h after intake. Estimation by MCMC-SAEM in Monolix 2023R1; concentrations below the limit of quantification were handled as left-censored. Assay validated over 0.075-20 mg/L for elexacaftor. Model qualified by prediction-corrected VPC (Figure 1) and NPDE (Figure S2). Dietary fat intake was not recorded, so the label's food effect on elexacaftor bioavailability (AUC 1.9- to 2.5-fold higher with a moderate-fat meal) is not represented in this model; all parameters are apparent (X/F) under real-world dosing conditions."
  )

  ini({
    # Structural parameters - final-model Monolix estimates (Truong 2025
    # Table 2, "Elexacaftor" column). All clearance and volume terms are
    ...
    # reported in the literature for the three molecules [27]." Reference
    # [27] is Tsai A, Wu SP, Haseltine E, et al. Pulm Ther. 2020;6(2):275-286
    ...
    # standard deviation", so the tabulated 0.31 is a log-scale SD and ini()
    # takes its square. This reading is confirmed by the paper's own derived
    # numbers: the Table S1 overall AUC0-24h IQR of 142-209 mg*h/L spans a
    # ratio of 1.46, against exp(2 * 0.6745 * 0.31) = 1.52 expected for an
    # SD of 0.31 versus 2.12 if 0.31 were instead a variance.
    # Only CL/F carried an estimable BSV (Results 3.2: "For all drugs, the
    # available data allowed estimating BSVs for the apparent clearance
    # parameter"); no BSV on V/F is reported, so none is encoded.
    etalcl ~ 0.0961   # 0.31^2;  Truong 2025 Table 2: omega_CL = 0.31 (11.1% RSE)

    # Residual error. Truong 2025 Results 3.2: "A proportional residual
    # error model was retained for elexacaftor/tezacaftor".
    propSd <- 0.247; label("Proportional residual error (fraction)")                     # Truong 2025 Table 2: sigma_proportional = 0.247 (9.2% RSE)
  })
