Sy_2018_etirinotecanPegol <- function() {
  ...
  ini({
    ...
    # ---- SN-38 ----
    lvc_sn38 <- log(365); label("SN-38 aggregate central volume V3** = V3/(F12*F23) (L)") # NOT the printed table value: back-solved from Sy 2018 Figure 5, reference 60-yr SN-38 Cmax = 4.0 ng/mL, via AUC(SN-38) = Dose/(V3** k3e) from Eq. 4. Sy 2018 Table 4, SN38 row 'V3** = V3/(F12 F23) (L)' prints 80 +/- 190, which is the one entry in Table 4 whose standard error exceeds its estimate (238% RSE) and which over-predicts the same paper's Figure 5 SN-38 exposures by 4.6-fold and exceeds the Figure 3 goodness-of-fit SN-38 range. The Figure 5 cumulative-AUC anchor gives 369 L, agreeing with the Cmax anchor to 1.3%. Deliberate deviation from print, decided by the maintainers; see the vignette 'Assumptions and deviations'.
