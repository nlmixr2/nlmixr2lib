Gu_2025_furosemide_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). furosemide disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. furosemide is",
    "a loop diuretic primarily renally excreted with a small biliary / hepatic-metabolic component (Supplementary Section 2.2).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; furosemide parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "furosemide", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    liver          = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    kidney         = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    spleen         = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    lung           = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    heart          = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    brain          = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    muscle         = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    skin           = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    adipose        = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    other          = list(analyte = "furosemide", units = "mg", specimen = "tissue", verified = TRUE),
    arterial       = list(analyte = "furosemide", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "furosemide", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "20-120 mg oral and 40-120 mg intravenous",
    notes          = paste(
  ...
  ini({
    ...
    fu_p       <- fixed(0.03);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [9] Chapa 2020
    bpr        <- fixed(0.5);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp [9] Chapa 2020
    # Table S2 gives fu,g = 1 assumed (footnote c), but furosemide has no
    ...
    lcl_renal  <- fixed(log(0.1127));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 112.7 mL/min [25] Schlender 2016
    lcl_liver  <- fixed(log(0.0539));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 53.9 mL/min [25] Schlender 2016

    # ---- Absorption ----
    # Peff is tabulated, so Eq S15 (ka,i = 2 x Peff / ri) gives a distinct
    # absorption rate constant per intestinal segment.
    lpeff      <- fixed(log(0.013));  label("Effective permeability coefficient Peff,A-B (log cm/min)")  # Table S2 Peff,A-B 0.013 cm/min [9] Chapa 2020

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(0.055   ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(0.29    ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(0.22    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(0.49    ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(0.26    ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(0.24    ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(0.094   ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.18    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(0.25    ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(0.24    ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(0.24    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
