Saleh_2023_methotrexate_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, CD1 nude).",
    "Nine-compartment CNS PBPK model predicting unbound methotrexate",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 1000 mg/kg intravenous dose. The CNS structure is the mouse",
    ...
    "0.195 (estimated by the authors; Table V). The plasma PK model is the",
  ...
  ini({
    ...
    lcl <- log(0.688); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(9.21); label("Central compartment volume (mL)")  # Table IV
    lq  <- log(0.03); label("Intercompartmental clearance (mL/min)")  # Table IV
    lvp <- log(2.14); label("Peripheral compartment volume (mL)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(454); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(-0.91); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(3.41); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(2.81); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.519); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.195); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (estimated)
    ...
    kpuu_lv <- fixed(0.195); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.195); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    etalcl ~ 0.021  # Table IV IIV on central clearance (variance)

    # ---- Residual unexplained variability: Table IV (reported as VARIANCES) ----
    propSd <- sqrt(0.18); label("Proportional residual error (fraction)")  # Table IV: 0.18 as variance
  })
