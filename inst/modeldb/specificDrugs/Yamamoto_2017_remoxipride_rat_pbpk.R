Yamamoto_2017_remoxipride_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Remoxipride disposition in plasma and",
    ...
    "Total-brain-tissue concentrations for remoxipride are new data first",
    "reported in this paper (Supplementary Material S2)."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "remoxipride", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "remoxipride", units = "ng", specimen = "tissue", verified = TRUE),
    peripheral2 = list(analyte = "remoxipride", units = "ng", specimen = "tissue", verified = TRUE),
    brain_vascular = list(analyte = "remoxipride", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "remoxipride", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "remoxipride", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "remoxipride", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "remoxipride", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "remoxipride", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "remoxipride", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "remoxipride", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "0.7, 4, 5.2, 8, 14 and 16 mg/kg (10 or 30 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 94 animals, 0.7, 4, 5.2, 8, 14 and 16 mg/kg",
      "(10 or 30 min infusion), data from reference 9 (Yamamoto 2017, Pharm",
      "Res; n = 65) and reference 29 (Stevens 2011; n = 29). Plasma and brain",
  ...
  ini({
    ...
    lcl <- log(42.2); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(83.7); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(33.8); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(253); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    lq2 <- log(14.0); label("Intercompartmental clearance to peripheral 2 (mL/min)")  # Table 2 Q_PL_PER2
    lvp2 <- log(757); label("Peripheral 2 volume of distribution (mL)")  # Table 2 V_PER2
    ...
    etalcl ~ 0.0961  # Table 2 CV 31.0 percent
    etalq ~ 0.0630  # Table 2 CV 25.1 percent
    etalq2 ~ 0.588  # Table 2 CV 76.7 percent
    etalvc ~ 0.410  # Table 2 CV 64.1 percent
    ...
    propSd <- 0.310; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 2.1 and molecular weight = 371;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(3.5e-3); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(3.0e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(1.7); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(1.7); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(2.1); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    PHF1 <- fixed(0.81); label("pH-dependent factor, brain extracellular fluid to microvasculature (unitless)")  # Table 5 PHF1
    PHF2 <- fixed(0.81); label("pH-dependent factor, lateral-ventricle CSF to microvasculature (unitless)")  # Table 5 PHF2
    PHF3 <- fixed(0.81); label("pH-dependent factor, third + fourth ventricle CSF to microvasculature (unitless)")  # Table 5 PHF3
    PHF4 <- fixed(0.81); label("pH-dependent factor, brain extracellular to intracellular fluid (unitless)")  # Table 5 PHF4
    PHF5 <- fixed(0.42); label("pH-dependent factor, brain intracellular to extracellular fluid (unitless)")  # Table 5 PHF5
    PHF6 <- fixed(0.42); label("pH-dependent factor, brain intracellular fluid to lysosome (unitless)")  # Table 5 PHF6
    PHF7 <- fixed(0.0044); label("pH-dependent factor, lysosome to brain intracellular fluid (unitless)")  # Table 5 PHF7
    ...
    BF <- fixed(5.3); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
