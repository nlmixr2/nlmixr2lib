Darpo_2014_racSotalol_QTcF <- function() {
  description <- paste(
    "Concentration-QTcF linear mixed-effects PD model for oral rac-sotalol",
    ...
    "baseline. The endpoint is change from baseline in the Fridericia-",
    "corrected QT interval (DeltaQTcF = QT / RR^(1/3); ms). The full",
    "model is:",
    "DeltaQTcF = e0 + e_sexf_e0 * SEXF",
    ...
    "parameter values (male, median-baseline subject): e0 = -2.5 ms, slope",
    "= 24 ms per ug/mL, female increments +10.6 ms on the intercept and",
    "+7 ms per ug/mL on the slope, centred-baseline-QTcF coefficient",
    ...
    "Darpo_2014_racSotalol_QTcI.R reports the same structure for the",
  ...
  covariateData <- list(
    ...
    QTC_BL = list(
      ...
      notes              = paste(
        ...
        "baseline QTcF on the population median ('Median-centred",
        ...
        "... The same analysis was also performed for DeltaQTcF",
        "(median-centred baseline QTcF was substituted as a covariate",
        "for this analysis)'; Darpo 2014 Methods 'Statistical",
        "analysis' paragraph 5). The specific median value is not",
        "quoted in the paper. Reference values observed: QTcF day 0",
        "ranged 380-396 ms in men (n=28) and 393-410 ms in women",
        "(n=11) (Darpo 2014 Results 'The effect of rac-sotalol on",
        "QTc' paragraph 1). The packaged model uses the rounded",
        "standard 390 ms as the centering reference (qtc_bl_ref);",
        "this matches the male-dominated cohort median to within the",
        "reported per-sex range. The assumption is documented in the",
        "vignette Errata.",
        "For QTcI instead of QTcF, see the companion model file",
        "Darpo_2014_racSotalol_QTcI.R, which uses the same QTC_BL",
  ...
  population <- list(
    ...
    notes            = paste(
      ...
      "the conventional fixed-exponent correction commonly used in",
      "thorough-QT studies (Darpo 2014 Methods 'Statistical analysis'",
      "paragraph 1 and Results 'Correction for heart rate changes';",
      "QTcF regression-line slope on on-drug data 0.035 ms/ms RR --",
      "less complete heart-rate decorrelation than QTcI but reported",
      "alongside for cross-comparison with conventional TQT analyses).",
      "Pre-specified base model structure (after AIC selection):",
      "linear concentration-DeltaQTcF relationship with both slope",
      ...
      "the final structural model: median-centred baseline QTcF",
  ...
  ini({
    # ==================================================================
    # Linear concentration-DeltaQTcF model (Darpo 2014 Methods
    # 'Statistical analysis' paragraph 5 and Figure 6B caption /
    # Table 1 QTcF row). Source equation reported for men:
    #   DeltaQTcF = -2.5 - 0.7 * centered_baseline_QTcF
    #             + 24 * rac-sotalol_concentration_ug_per_mL
    # and for women:
    #   DeltaQTcF =  8.1 - 0.7 * centered_baseline_QTcF
    #             + 31 * rac-sotalol_concentration_ug_per_mL
    ...
    e0 <- -2.5
    label("Linear-mixed-effects intercept e0 on DeltaQTcF, male median-baseline reference (ms)")
    # Darpo 2014 Table 1 QTcF row 'Intercept (ms)' = -2.5 (SE 2.0;
    # P = 0.22). Not log-transformed because the intercept of a
    # DeltaQTc model can take negative values (close-to-zero baseline-
    # day-vs-baseline-day prediction).

    lslope <- log(24)
    label("Linear concentration-DeltaQTcF slope, male reference (ms per ug/mL)")
    # Darpo 2014 Table 1 QTcF row 'Plasma concentration of rac-
    # sotalol (ms per ug ml-1)' = 24 (SE 1.7; P < 0.0001). Log-
    # transformed because the slope is positive (concentration-
    # related QTc prolongation). Bare name inside model() is `slope`.

    # ------------------------------------------------------------------
    # Covariate effects (Darpo 2014 Table 1 QTcF rows). All three are
    # estimated point estimates with reported SE / P-value; not fixed.
    # ------------------------------------------------------------------

    e_sexf_e0 <- 10.6
    label("Effect of female sex on e0 (additive, ms)")
    # Darpo 2014 Table 1 QTcF row 'Female gender (ms)' = 10.6 (SE 3.8;
    # P = 0.005). Reproduces female-reference intercept e0_female =
    # -2.5 + 10.6 = 8.1 ms (Figure 6B caption).

    e_sexf_slope <- 7
    label("Effect of female sex on slope (additive, ms per ug/mL)")
    # Darpo 2014 Table 1 QTcF row 'Concentration x Female gender
    # interaction (ms per ug ml-1)' = 7 (SE 2.8; P = 0.019).
    # Reproduces female-reference slope slope_female = 24 + 7 = 31
    # ms per ug/mL (Figure 6B caption).

    e_qtc_bl_e0 <- -0.70
    label("Effect of centered baseline QTcF on e0 (additive, ms per ms)")
    # Darpo 2014 Table 1 QTcF row 'Centred baseline QTcF (ms)' =
    # -0.70 (SE 0.05; P < 0.0001). The negative coefficient means
    # subjects with a higher-than-median baseline QTcF have a smaller
    # predicted DeltaQTcF (regression-to-the-mean shrinkage of the
  ...
  model({
    ...
    #    sex covariate effects (Darpo 2014 Figure 6B caption).
    # ==================================================================
    e0_i    <- e0 + e_sexf_e0 * SEXF
    slope_i <- exp(lslope) + e_sexf_slope * SEXF

    # ==================================================================
    # 2. Centered baseline-QTcF term. The paper centered each
    #    subject's baseline QTcF on the cohort median (Darpo 2014
    ...
    #    dominated cohort (28 men, baseline QTcF 380-396 ms; 11
    #    women, 393-410 ms) to within the reported per-sex range.
    #    See vignette Errata.
    # ==================================================================
    qtc_bl_ref <- 390
    ...
    # ==================================================================
    # 3. Linear concentration-DeltaQTcF prediction. Output is the
    #    change from time-matched day-0 baseline in QTcF, in ms.
    # ==================================================================
    QTcF <- e0_i + slope_i * CP_RACSOTALOL_UGML + e_qtc_bl_e0 * qtc_bl_centered
