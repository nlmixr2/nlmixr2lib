Beredaki_2023_micafungin_eucast <- function() {
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "to within roughly 0.05 log10 CFU/mL rather than exactly. A preceding static",
  ...
  ini({
    ...
    # curve (Figure 5b, R^2 = 0.87) but no coefficients. The four values
    # below were recovered by digitising the Figure 5(b) curve from the
    # publisher figure at 600 dpi (842 points, x = 0.108-34.4) and
    # least-squares fitting the four-parameter log-logistic form; the
    # residual RMSE was 0.003 log10 CFU/mL, i.e. the recovered
    ...
    e0 <- 4.23
    label("72 h change in log10 CFU/mL at zero exposure (fitted top asymptote)")  # digitised from Beredaki 2023 Figure 5(b); cross-checks against the reported drug-free control growth 4.37 -> 7.76 log10 CFU/mL at 72 h
    lemax <- log(6.73)
    ...
    # isolate, not an estimated parameter. Default is the EUCAST E.Def 7.3
    # median MIC of the fks1 wild-type reference isolate CA 580. Change it
