LopezArguello_2023_tazobactam_qsp <- function() {
  ...
  compartmentData <- list(
    npbp1a = list(
      analyte = "tazobactam-PBP 1a complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    npbp1b = list(
      analyte = "tazobactam-PBP 1b complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    npbp2 = list(
      analyte = "tazobactam-PBP 2 complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    npbp3 = list(
      analyte = "tazobactam-PBP 3 complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    npbp4 = list(
      analyte = "tazobactam-PBP 4 complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    npbp56 = list(
      analyte = "tazobactam-PBPs 5/6 complex",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    ),
    nperi = list(
      analyte = "free tazobactam in periplasm",
      units = NA_character_,
      specimen = "administration site",
      verified = FALSE
    )
  ...
  covariateData <- list(
    ...
    CONC_TZB_MGL = list(
      description = "Static extracellular tazobactam concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; tazobactam MIC not determined (beta-lactamase inhibitor)",
    ...
    dose_range = "tazobactam at a static extracellular concentration of 4 mg/L (a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors); no dosing events",
    notes = paste(
      ...
      "The 15 drugs were split into five estimation datasets of four drugs each with imipenem as the shared backbone drug; the Noise, Fini and residual-error parameters in this file are those of dataset 4 (IPM, TZB, SUL, TIC) (Table S2).",
  ...
  ini({
    ...
    rate_influx_scaled <- 0.208; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, tazobactam: 0.208 (SE 65.4%)
    # The studied extracellular concentration enters as the CONC_TZB_MGL covariate
    # (Table 1: tazobactam studied at 4 mg/L = a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors).
    ...
    k2_1a <- 7.02; label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, tazobactam PBP1a: 7.02
    k2_1b <- 9.17; label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, tazobactam PBP1b: 9.17
    k2_2 <- 0.263; label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, tazobactam PBP2: 0.263
    k2_3 <- 0.552; label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, tazobactam PBP3: 0.552
    k2_4 <- 0.694; label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, tazobactam PBP4: 0.694
    k2_5 <- 3;     label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, tazobactam PBP5/6: 3
    ...
    noise_1b <- 0.0769; label("Background gel-band fraction not suppressible by drug, PBP1b (unitless)") # Table S2 NOISE_1b = 0.0769
    noise_2 <- 0.242;   label("Background gel-band fraction not suppressible by drug, PBP2 (unitless)") # Table S2 NOISE_2 = 0.242
    noise_3 <- 0.235;   label("Background gel-band fraction not suppressible by drug, PBP3 (unitless)") # Table S2 NOISE_3 = 0.235
    noise_4 <- 0.0346;  label("Background gel-band fraction not suppressible by drug, PBP4 (unitless)") # Table S2 NOISE_4 = 0.0346
    noise_5 <- 0.0878;  label("Background gel-band fraction not suppressible by drug, PBP5/6 (unitless)") # Table S2 NOISE_5 = 0.0878

    # --- Random multiplier on the initial number of PBP molecules (Table S2, dataset 4 (IPM, TZB, SUL, TIC)) ---
    fini_1a <- 0.934; label("Multiplier on the initial number of PBP1a molecules (unitless)") # Table S2 Fini,1a = 0.934
    fini_1b <- 0.919; label("Multiplier on the initial number of PBP1b molecules (unitless)") # Table S2 Fini,1b = 0.919
    fini_2 <- 1;      label("Multiplier on the initial number of PBP2 molecules (unitless)") # Table S2 Fini,2 = 1
    fini_3 <- 1.02;   label("Multiplier on the initial number of PBP3 molecules (unitless)") # Table S2 Fini,3 = 1.02
    fini_4 <- 0.888;  label("Multiplier on the initial number of PBP4 molecules (unitless)") # Table S2 Fini,4 = 0.888
    fini_5 <- 0.973;  label("Multiplier on the initial number of PBP5/6 molecules (unitless)") # Table S2 Fini,5 = 0.973

    # --- Residual error: SD = SDin + SDsl * Y (Fig. S8 lines 111-116; Table S2, dataset 4 (IPM, TZB, SUL, TIC)) ---
    addSd_PBP1a <- 5.21; label("Additive residual SD, PBP1a (molecules/cell)") # Table S2 SDin1a = 5.21
    addSd_PBP1b <- 2.4;  label("Additive residual SD, PBP1b (molecules/cell)") # Table S2 SDin1b = 2.4
    addSd_PBP2 <- 3.66;  label("Additive residual SD, PBP2 (molecules/cell)") # Table S2 SDin2 = 3.66
    addSd_PBP3 <- 8.92;  label("Additive residual SD, PBP3 (molecules/cell)") # Table S2 SDin3 = 8.92
    addSd_PBP4 <- 0.755; label("Additive residual SD, PBP4 (molecules/cell)") # Table S2 SDin4 = 0.755
    addSd_PBP56 <- 6.23; label("Additive residual SD, PBP5/6 (molecules/cell)") # Table S2 SDin5 = 6.23

    propSd_PBP1a <- 0.205; label("Proportional residual SD, PBP1a (fraction)") # Table S2 SDsl1a = 0.205
    propSd_PBP1b <- 0.239; label("Proportional residual SD, PBP1b (fraction)") # Table S2 SDsl1b = 0.239
    propSd_PBP2 <- 0.166;  label("Proportional residual SD, PBP2 (fraction)") # Table S2 SDsl2 = 0.166
    propSd_PBP3 <- 0.141;  label("Proportional residual SD, PBP3 (fraction)") # Table S2 SDsl3 = 0.141
    propSd_PBP4 <- 0.371;  label("Proportional residual SD, PBP4 (fraction)") # Table S2 SDsl4 = 0.371
    propSd_PBP56 <- 0.266; label("Proportional residual SD, PBP5/6 (fraction)") # Table S2 SDsl5 = 0.266
  })
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_TZB_MGL
