LopezArguello_2023_ertapenem_qsp <- function() {
  ...
  compartmentData <- list(
    npbp1a = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp1b = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp2  = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp3  = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp4  = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp56 = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE),
    nperi  = list(analyte = "ertapenem", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_ETP_MGL = list(
      description = "Static extracellular ertapenem concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; ertapenem MIC 4 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "ertapenem at a static extracellular concentration of 8 mg/L (2x the MIC of 4 mg/L); no dosing events",
    notes = paste(
      ...
      "The 15 drugs were split into five estimation datasets of four drugs each with imipenem as the shared backbone drug; the Noise, Fini and residual-error parameters in this file are those of dataset 1 (IPM, DOR, MEM, ETP) (Table S2).",
  ...
  ini({
    ...
    rate_influx_scaled <- 1.04; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, ertapenem: 1.04 (SE 35.2%)
    # The studied extracellular concentration enters as the CONC_ETP_MGL covariate
    # (Table 1: ertapenem studied at 8 mg/L = 2x the MIC of 4 mg/L).
    ...
    k2_1a <- 82.4;      label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, ertapenem PBP1a: 82.4
    k2_1b <- 133;       label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, ertapenem PBP1b: 133
    k2_2 <- fixed(500); label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, ertapenem PBP2: 500, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_3 <- 8400;       label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, ertapenem PBP3: 8400
    k2_4 <- 18700;      label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, ertapenem PBP4: 18700
    k2_5 <- 61.8;       label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, ertapenem PBP5/6: 61.8
    ...
    # --- Non-suppressible background gel-band fraction (Table S2, dataset 1 (IPM, DOR, MEM, ETP)) ---
    noise_1a <- 0.112; label("Background gel-band fraction not suppressible by drug, PBP1a (unitless)") # Table S2 NOISE_1a = 0.112
    noise_1b <- 0.143; label("Background gel-band fraction not suppressible by drug, PBP1b (unitless)") # Table S2 NOISE_1b = 0.143
    noise_2 <- 0.236;  label("Background gel-band fraction not suppressible by drug, PBP2 (unitless)") # Table S2 NOISE_2 = 0.236
    noise_3 <- 0.364;  label("Background gel-band fraction not suppressible by drug, PBP3 (unitless)") # Table S2 NOISE_3 = 0.364
    noise_4 <- 0.139;  label("Background gel-band fraction not suppressible by drug, PBP4 (unitless)") # Table S2 NOISE_4 = 0.139
    noise_5 <- 0.126;  label("Background gel-band fraction not suppressible by drug, PBP5/6 (unitless)") # Table S2 NOISE_5 = 0.126

    # --- Random multiplier on the initial number of PBP molecules (Table S2, dataset 1 (IPM, DOR, MEM, ETP)) ---
    fini_1a <- 0.944; label("Multiplier on the initial number of PBP1a molecules (unitless)") # Table S2 Fini,1a = 0.944
    fini_1b <- 0.949; label("Multiplier on the initial number of PBP1b molecules (unitless)") # Table S2 Fini,1b = 0.949
    fini_2 <- 0.942;  label("Multiplier on the initial number of PBP2 molecules (unitless)") # Table S2 Fini,2 = 0.942
    fini_3 <- 0.945;  label("Multiplier on the initial number of PBP3 molecules (unitless)") # Table S2 Fini,3 = 0.945
    fini_4 <- 0.982;  label("Multiplier on the initial number of PBP4 molecules (unitless)") # Table S2 Fini,4 = 0.982
    fini_5 <- 0.923;  label("Multiplier on the initial number of PBP5/6 molecules (unitless)") # Table S2 Fini,5 = 0.923

    # --- Residual error: SD = SDin + SDsl * Y (Fig. S8 lines 111-116; Table S2, dataset 1 (IPM, DOR, MEM, ETP)) ---
    addSd_PBP1a <- 4.6;  label("Additive residual SD, PBP1a (molecules/cell)") # Table S2 SDin1a = 4.6
    addSd_PBP1b <- 2.13; label("Additive residual SD, PBP1b (molecules/cell)") # Table S2 SDin1b = 2.13
    addSd_PBP2 <- 7.43;  label("Additive residual SD, PBP2 (molecules/cell)") # Table S2 SDin2 = 7.43
    addSd_PBP3 <- 13.1;  label("Additive residual SD, PBP3 (molecules/cell)") # Table S2 SDin3 = 13.1
    addSd_PBP4 <- 9.8;   label("Additive residual SD, PBP4 (molecules/cell)") # Table S2 SDin4 = 9.8
    addSd_PBP56 <- 3.3;  label("Additive residual SD, PBP5/6 (molecules/cell)") # Table S2 SDin5 = 3.3

    propSd_PBP1a <- 0.203; label("Proportional residual SD, PBP1a (fraction)") # Table S2 SDsl1a = 0.203
    propSd_PBP1b <- 0.194; label("Proportional residual SD, PBP1b (fraction)") # Table S2 SDsl1b = 0.194
    propSd_PBP2 <- 0.0251; label("Proportional residual SD, PBP2 (fraction)") # Table S2 SDsl2 = 0.0251
    propSd_PBP3 <- 0.103;  label("Proportional residual SD, PBP3 (fraction)") # Table S2 SDsl3 = 0.103
    propSd_PBP4 <- 0.0214; label("Proportional residual SD, PBP4 (fraction)") # Table S2 SDsl4 = 0.0214
    propSd_PBP56 <- 0.295; label("Proportional residual SD, PBP5/6 (fraction)") # Table S2 SDsl5 = 0.295
  })
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_ETP_MGL
