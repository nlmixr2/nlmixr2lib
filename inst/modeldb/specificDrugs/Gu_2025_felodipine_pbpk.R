Gu_2025_felodipine_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). felodipine disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. felodipine is",
    "a dihydropyridine calcium-channel blocker metabolism predominantly mediated by hepatic and intestinal CYP3A (Supplementary Section 2.7).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; felodipine parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "felodipine", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    liver          = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    kidney         = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    spleen         = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    lung           = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    heart          = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    brain          = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    muscle         = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    skin           = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    adipose        = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    other          = list(analyte = "felodipine", units = "mg", specimen = "tissue", verified = TRUE),
    arterial       = list(analyte = "felodipine", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "felodipine", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "5 and 10 mg single oral",
    notes          = paste(
  ...
  ini({
    ...
    fu_p       <- fixed(0.0048);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [18] Salem 2023
    bpr        <- fixed(0.7);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp [18] Salem 2023
    fu_g       <- fixed(0.12);    label("Fraction unbound in the intestinal wall (unitless)")  # Table S2 fu,g [18] Salem 2023
    ...
    lcl_renal  <- fixed(log(0.0276));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 27.6 mL/min [33] Larsson 1990
    lcl_liver  <- fixed(log(0.7957));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 795.7 mL/min [18] Salem 2023

    # ---- Absorption ----
    # ka is tabulated directly (Eq S15 not needed) and applies to all
    # three absorbing segments.
    lka        <- fixed(log(0.0032));  label("First-order intestinal absorption rate constant ka (log 1/min)")  # Table S2 ka 0.0032 1/min footnote b (simulated by WinNonlin)

    # ---- Intestinal-wall intrinsic clearance (Supplementary Table S2) ----
    lclg_duodenum <- fixed(log(0.2566));  label("Duodenal wall intrinsic clearance CLGW1,int (log L/min)")  # Table S2 CLGW1,int 256.6 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)
    lclg_jejunum  <- fixed(log(1.016));  label("Jejunal wall intrinsic clearance CLGW2,int (log L/min)")  # Table S2 CLGW2,int 1016 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)
    lclg_ileum    <- fixed(log(0.5927));  label("Ileal wall intrinsic clearance CLGW3,int (log L/min)")  # Table S2 CLGW3,int 592.7 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(2.21    ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(5.66    ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(4.69    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(13.18   ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(4.91    ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(5.14    ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(5.14    ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.01    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(1.31    ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(3.23    ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(5.16    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
