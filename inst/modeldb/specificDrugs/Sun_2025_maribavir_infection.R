Sun_2025_maribavir_infection <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent invasive fungal or bacterial infection as a ",
    ...
    "happened, so it is the weaker causal claim of the two. No risk ",
    "factor survived covariate selection for this endpoint, so the model ",
    "is the exposure term alone: Table S4 records 'None' in its ",
    "risk-factor column. The exposure slope is +0.0650 per 10 ug*h/mL. ",
  ...
  units <- list(
    ...
    concentration = "prob_infection (probability of a treatment-emergent invasive fungal or bacterial infection, 0-1; also logit_infection)"
  )
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0650 per 10 ug*h/mL (odds",
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary invasive fungal or bacterial infection records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel g (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -3.59; label("Logit of the invasive fungal or bacterial infection probability at zero maribavir exposure with no risk factors retained for this endpoint (unitless logit)")  # Sun 2025 Figure S3 panel g (Supporting Information file s002), Intercept -3.59 (SE 0.455), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0650; label("Log-odds of invasive fungal or bacterial infection per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel g (Supporting Information file s002), Estimate 0.0650 (SE 0.0137), odds ratio 1.07 (1.04-1.10), p < 0.001
    ...
    addSd_prob_infection <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_infection <- logit_ref +
      e_auc_logit * auc10

    prob_infection <- expit(logit_infection)

    # ----- Observation -----
    prob_infection ~ add(addSd_prob_infection)
  })
}
