Law_2017_ec_human_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 13 blood-flow-limited tissue compartments).",
    "Single-catechin model for epicatechin (EC) in humans (Law et al. 2017).",
    "Thirteen perfusion-limited compartments (lung, kidney, muscle, brain,",
    "liver, spleen, gut, bone, skin, heart, adipose, rest of body, blood",
    "split into a one-third arterial and a two-thirds venous pool) with",
    "first-order oral input into gut tissue after a lag time, renal",
    "elimination from the kidney, and an enterohepatic recycling loop in",
    "which a biliary clearance drains the liver through a",
  ...
  ini({
    ...
    # Table 5 (kac EC = 1.86 /h per kg^-0.3; one value for all studies)
    lka <- fixed(log(1.86)); label("Absorption rate coefficient kac into gut tissue (/h per kg^-0.3)")
    # Table 5, square-bracket value (F EC = 0.01, Chow et al. 2001); 0.1
    # simulates Lee et al. (2002)
    lfdepot <- fixed(log(0.01)); label("Empirical oral bioavailability factor F (unitless)")
    ...
    # Table 5 (CLbc EC = 1.03)
    lcl_nonren <- fixed(log(1.03)); label("Biliary clearance coefficient CLbc (L/h per kg^0.66)")
    # Table 5 (CLrc EC = 0.56)
    lcl_renal <- fixed(log(0.56)); label("Renal clearance coefficient CLrc (L/h per kg^0.66)")

    # Table 1 (BLPLR EC = 0.88; human values assumed equal to rat)
    ...
    # Table 4 (adipose EC = 0.01)
    lkp_adipose <- fixed(log(0.01)); label("Tissue/blood partition coefficient, adipose (unitless)")
    # Table 4 (bone EC = 0.41)
    lkp_bone <- fixed(log(0.41)); label("Tissue/blood partition coefficient, bone (unitless)")
    # Table 4 (brain EC = 0.62)
    lkp_brain <- fixed(log(0.62)); label("Tissue/blood partition coefficient, brain (unitless)")
    # Table 4 (gut EC = 0.55)
    lkp_gut <- fixed(log(0.55)); label("Tissue/blood partition coefficient, gut (unitless)")
    # Table 4 (heart EC = 0.5)
    lkp_heart <- fixed(log(0.5)); label("Tissue/blood partition coefficient, heart (unitless)")
    # Table 4 (kidney EC = 0.53)
    lkp_kidney <- fixed(log(0.53)); label("Tissue/blood partition coefficient, kidney (unitless)")
    # Table 4 (liver EC = 0.55)
    lkp_liver <- fixed(log(0.55)); label("Tissue/blood partition coefficient, liver (unitless)")
    # Table 4 (lung EC = 0.51)
    lkp_lung <- fixed(log(0.51)); label("Tissue/blood partition coefficient, lung (unitless)")
    # Table 4 (muscle EC = 0.52)
    lkp_muscle <- fixed(log(0.52)); label("Tissue/blood partition coefficient, muscle (unitless)")
    # Table 4 (skin EC = 0.5)
    lkp_skin <- fixed(log(0.5)); label("Tissue/blood partition coefficient, skin (unitless)")
    # Table 4 (spleen EC = 0.54)
    lkp_spleen <- fixed(log(0.54)); label("Tissue/blood partition coefficient, spleen (unitless)")
    # Table 4 (other EC = 1)
    ...
    # deviations section. Source: Results (29.6% MAPE, Figure 9, human TCM
    # mixture, Chow et al. 2001).
    propSd <- fixed(0.296); label("Proportional residual magnitude, from the reported MAPE (fraction)")
  })
