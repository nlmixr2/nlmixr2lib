Li_2026_alvespimycin_nep9 <- function() {
  ...
  reference <- paste(
    ...
    "Parameter values from Supplementary Table S2 (Model b, NEP = 9).",
  ...
  # Screened but not retained at NEP = 9 (Li 2026 Table 3 "Searched covariates"
  # for DMAG; Li 2026 Table S3 shows an empty "Covariate" cell at NEP = 9).
  covariatesDataExcluded <- list(
    WT = list(
      description = "Total body weight.",
      units       = "kg",
      type        = "continuous",
      notes       = "In the search space as a power function of weight centred on 81 kg on Vc, CL, Q, V2, V3; not retained at NEP = 9."
    ),
    AGE = list(
      description = "Age.",
      units       = "years",
      type        = "continuous",
      notes       = "In the search space as a power function of age centred on 60 years on Vc and CL; not retained at NEP = 9."
    ),
    SEXF = list(
      description = "Sex indicator.",
      units       = "(binary)",
      type        = "categorical",
      notes       = "In the search space as an exponential effect of the NONMEM SEX column on Vc and CL; not retained at NEP = 9. The Li 2024 control stream codes the column as SEX with a 0/1 encoding whose reference level is not stated in either publication."
    ),
    CREAT = list(
      description = "Serum creatinine.",
      units       = "mg/dL",
      type        = "continuous",
      notes       = "In the search space as a power function of serum creatinine centred on 0.9 mg/dL on CL (Li 2024 control stream `CSCR = SCR/0.9`); not retained at NEP = 9."
    )
  )
  ...
  ini({
    # Structural PK parameters from Li 2026 Supplementary Table S2, Model b
    # (NEP = 9, OFV = 8335.4). NONMEM ADVAN3 / TRANS4 (Li 2026 Table S3, DMAG
    # rows, "COM" = 2 at NEP = 9); the ADVAN3 branch of the Li 2024 token file
    # names the inter-compartmental clearance `Q` and the peripheral volume
    # `V2`, which is the naming Table S2 uses for this model.
    lcl <- log(9.65) ; label("Clearance (L/h)")                                  # Li 2026 Table S2 Model b: CL = 9.65 L/hr (RSE 6%)
    lvc <- log(37.8) ; label("Central volume of distribution (L)")               # Li 2026 Table S2 Model b: V  = 37.8 L    (RSE 7%)
    lq  <- log(61.8) ; label("Inter-compartmental clearance (L/h)")              # Li 2026 Table S2 Model b: Q  = 61.8 L/hr (RSE 6%)
    lvp <- log(127)  ; label("Peripheral volume of distribution (L)")            # Li 2026 Table S2 Model b: V2 = 127 L     (RSE 6%)
    ...
    etalcl ~ 0.172571  # Li 2026 Table S2 Model b: BSV CL 43.4% (RSE 9),  variance = log(0.434^2 + 1)
    etalvc ~ 0.238514  # Li 2026 Table S2 Model b: BSV V  51.9% (RSE 11), variance = log(0.519^2 + 1)
    etalvp ~ 0.210468  # Li 2026 Table S2 Model b: BSV V2 48.4% (RSE 10), variance = log(0.484^2 + 1)

    # Between-occasion variability on Q (Li 2026 Table S3, "BOV" column reads
    # "Q" at NEP = 9; Table S2 puts the 45.9% in the between-occasion column of
    # the Q row). The deposited control stream implements BOV as a NONMEM
    # `$OMEGA BLOCK(1)` reused across occasions via `BLOCK SAME`; nlmixr2 has
    # no `SAME` shortcut (Jonsson_2011_ethambutol / Aregbe_2012_alvespimycin
    # pattern), so the first eta carries the estimated variance and the
    # remainder are fixed equal to it.
    etaiov_q_1 ~ 0.191183       # Li 2026 Table S2 Model b: BOV Q 45.9% (RSE 11), variance = log(0.459^2 + 1)
    etaiov_q_2 ~ fix(0.191183)  # NONMEM $OMEGA BLOCK SAME (occasion 2 shares the occasion-1 variance)
    etaiov_q_3 ~ fix(0.191183)  # NONMEM $OMEGA BLOCK SAME (occasion 3 shares the occasion-1 variance)

    # Residual error. Li 2026 Table S3 labels the NEP = 9 residual model
    ...
    # (exponential) residual that maps to `lnorm(expSd)`. The tabulated 0.034
    # is the NONMEM $SIGMA variance, so expSd = sqrt(0.034).
    expSd <- 0.184391 ; label("Exponential (log-normal) residual error SD")  # Li 2026 Table S2 Model b: proportional error 0.034 (RSE 6); SD = sqrt(0.034)
  })

  model({
    # Decompose the integer-valued occasion column into binary indicators for
    # BOV multiplexing on log-Q (three occasions per the deposited control
    ...
    # Individual PK parameters; no covariate effects at NEP = 9. Q carries BOV
    # only, and CL, Vc, Vp carry BSV only.
    cl <- exp(lcl + etalcl)
    vc <- exp(lvc + etalvc)
    q  <- exp(lq  + iov_q)
    vp <- exp(lvp + etalvp)

    # Two-compartment IV PK with first-order elimination from the central
    # compartment (NONMEM ADVAN3 / TRANS4 mass-balance ODEs). Dose lands in
