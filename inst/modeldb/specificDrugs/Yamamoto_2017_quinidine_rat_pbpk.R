Yamamoto_2017_quinidine_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Quinidine disposition in plasma and nine",
    ...
    "CNS data, so the CNS profiles are genuine predictions. The most",
    "lipophilic compound in the set and the one the paper uses to illustrate",
    "cerebral-blood-flow-limited transport: its BBB clearances exceed Q_CBF =",
    "1.2 mL/min, so brain uptake is perfusion- rather than",
    "permeability-limited. The only compound with net active influx (Kp,uu >",
    "1), so its asymmetry factors sit on the influx side."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    ...
    peripheral1 = list(analyte = "quinidine", units = "ng", specimen = "tissue", verified = TRUE),
  ...
  population <- list(
    ...
    dose_range = "10 and 20 mg/kg (10 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 41 animals, 10 and 20 mg/kg (10 min",
      "infusion), data from reference 28 (Westerhout 2013). Plasma and brain",
  ...
  ini({
    ...
    lcl <- log(162); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(670); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(829); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(11300); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    ...
    etalcl ~ 0.0571  # Table 2 CV 23.9 percent
    etalq ~ 0.059  # Table 2 CV 24.3 percent
    etalvp ~ 0.0164  # Table 2 CV 12.8 percent
    ...
    propSd <- 0.245; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 3.4 and molecular weight = 324;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(0.058); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(3.2e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFin1 <- fixed(1.2); label("Asymmetry factor into brain extracellular fluid (unitless)")  # Table 4 AFin1
    AFin2 <- fixed(1.4); label("Asymmetry factor into lateral-ventricle CSF (unitless)")  # Table 4 AFin2
    AFin3 <- fixed(1.4); label("Asymmetry factor into third + fourth ventricle CSF (unitless)")  # Table 4 AFin3
    AFout1 <- fixed(1); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(1); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(1); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    BF <- fixed(7.2); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
