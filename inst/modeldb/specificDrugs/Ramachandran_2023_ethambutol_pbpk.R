Ramachandran_2023_ethambutol_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 18 ODEs, MATLAB ode15s). Oral ethambutol disposition",
    ...
    "concentrations after a 400 mg oral dose (Appendix S1 Table S7).",
    ...
    "ethambutol is a terminal sink cleared only by faecal transit at",
  ...
  population <- list(
    ...
    dose_range     = paste(
      "Oral ethambutol. 400 mg single dose for model calibration",
      "(Figure 2); 1200 mg for plasma, pleura, and lung validation",
      "(Figures 3-5); 1200 mg for the Day 7 EPTB site simulations",
      "(Figure 6)."
    ),
    ...
    notes          = paste(
      ...
      "(Strauch et al. 2011 (two products, Etb-91-400B and Etb-ref-400); Appendix S1 Table S6 lists the per-point calibration",
  ...
  ini({
    ...
    lka <- log(0.22); label("First-order oral absorption rate constant ka (1/h)")  # Appendix S1 Table S7: ethambutol ka = 0.22 /h
    lcl <- log(49.99); label("Total systemic clearance CL (L/h)")                   # Appendix S1 Table S7: ethambutol CL = 49.99 L/h
  ...
  model({
    ...
    # (Appendix S1 Table S5, ethambutol column). "Others" is the median of
    ...
    # Fractional renal clearance (Appendix S1 Table S4, ethambutol fR).
    f_renal <- 0.79
    ...
    # the reported value"). For ethambutol the gut lumen is therefore a
