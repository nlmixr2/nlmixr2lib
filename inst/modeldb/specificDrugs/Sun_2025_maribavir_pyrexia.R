Sun_2025_maribavir_pyrexia <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent pyrexia as a function of the maribavir AUC on the ",
    ...
    "claim of the two. No risk factor survived covariate selection for ",
    "this endpoint, so the model is the exposure term alone: Table S4 ",
    "records 'None' in its risk-factor column. This endpoint carries the ",
    "SHALLOWEST exposure slope of the fourteen apart from serious adverse ",
    "events, +0.0450 per 10 ug*h/mL. There is no PK layer and no ODE: the ",
  ...
  units <- list(
    ...
    concentration = "prob_pyrexia (probability of treatment-emergent pyrexia, 0-1; also logit_pyrexia)"
  )
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0450 per 10 ug*h/mL (odds",
        "ratio 1.05 (1.02-1.08), p = 0.0014)."
      ),
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary pyrexia records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel k (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -3.25; label("Logit of the pyrexia probability at zero maribavir exposure with no risk factors retained for this endpoint (unitless logit)")  # Sun 2025 Figure S3 panel k (Supporting Information file s002), Intercept -3.25 (SE 0.441), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0450; label("Log-odds of pyrexia per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel k (Supporting Information file s002), Estimate 0.0450 (SE 0.0141), odds ratio 1.05 (1.02-1.08), p = 0.0014
    ...
    addSd_prob_pyrexia <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_pyrexia <- logit_ref +
      e_auc_logit * auc10

    prob_pyrexia <- expit(logit_pyrexia)

    # ----- Observation -----
    prob_pyrexia ~ add(addSd_prob_pyrexia)
  })
}
