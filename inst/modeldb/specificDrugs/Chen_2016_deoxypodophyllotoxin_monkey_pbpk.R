Chen_2016_deoxypodophyllotoxin_monkey_pbpk <- function() {
  description <- paste0(
    "Preclinical (cynomolgus monkey). PBPK (whole-body, 13 tissue ",
    "compartments, coded in Phoenix WinNonlin). Deoxypodophyllotoxin (DPT) ",
    "disposition in the cynomolgus monkey after intravenous bolus dosing ",
    "(Chen et al. 2016, Front Pharmacol). Adipose, liver, muscle, lung, ",
    "kidney, brain, heart, spleen, skin, gastrointestinal tract and rest of ",
    "body, plus arterial and venous blood pools. All tissues are ",
    ...
    "amount PBSF. The blood-to-plasma ratio is assumed to be 1. Monkey M2 ",
    "formation needed two Michaelis-Menten sites (Km1/Vmax1 and Km2/Vmax2 in ",
    "Table 2); the mouse, rat and human files carry a single site. ",
  ...
  population <- list(
    species = "monkey (cynomolgus, male and female)",
    n_subjects = 12L,
    n_studies = 1L,
    age_range = "about 4 years",
    weight_range = "3.51 +/- 0.85 kg",
    sex_female_pct = 50,
    disease_state = "Healthy cynomolgus monkeys, fasted 12 h before dosing.",
    dose_range = "Intravenous bolus 0.5, 1.0 and 2.0 mg/kg via the cephalic vein.",
    regions = "China Pharmaceutical University, Nanjing, China",
    notes = "Twelve monkeys in two groups of six (three male, three female). One group received a single 1.0 mg/kg dose; the other received 0.5 and then 2.0 mg/kg separated by a one-week washout. Blood sampled pre-dose and at 2, 5, 10, 20, 30, 45, 60, 90, 120, 180 and 240 min post-dose. The monkey maximum tolerated dose of 4 mg/kg is what set the 16 mg first-in-human simulation dose used in the human model."
  )

  ini({
    # ----------------------------------------------------------------
    # Organ volumes (mL) -- Table 1, "Monkey (4 kg)" column, "Volume" sub-column.
    ...
    q_other <- fixed(199.31)
    label("Blood flow to rest of body (mL/min)") # Table 1 row 'Rest of body'

    # ----------------------------------------------------------------
    # Tissue-to-plasma concentration ratios Kt:pl -- Table 1, "Monkey (4 kg)"
    # column, "Kt:pl" sub-column. The rat values were computed from rat
    # tissue composition by the Ruark 2014 method and the cynomolgus monkey values
    # follow from the paper's identical-Kt:pl,u assumption,
    # Kt:pl = Kt:pl,rat * fu / fu_rat (Results, "PBPK Model Development
    # and Validation").
    # ----------------------------------------------------------------
    lkp_adipose <- fixed(log(3.25))
    label("Log adipose-to-plasma concentration ratio (unitless)") # Table 1 row 'Adipose' Kt:pl = 3.25
    lkp_liver <- fixed(log(0.25))
    label("Log liver-to-plasma concentration ratio (unitless)") # Table 1 row 'Liver' Kt:pl = 0.25
    lkp_muscle <- fixed(log(0.11))
    label("Log muscle-to-plasma concentration ratio (unitless)") # Table 1 row 'Muscle' Kt:pl = 0.11
    lkp_lung <- fixed(log(0.26))
    label("Log lungs-to-plasma concentration ratio (unitless)") # Table 1 row 'Lungs' Kt:pl = 0.26
    lkp_kidney <- fixed(log(0.21))
    label("Log kidneys-to-plasma concentration ratio (unitless)") # Table 1 row 'Kidneys' Kt:pl = 0.21
    lkp_brain <- fixed(log(0.42))
    label("Log brain-to-plasma concentration ratio (unitless)") # Table 1 row 'Brain' Kt:pl = 0.42
    lkp_heart <- fixed(log(0.15))
    label("Log heart-to-plasma concentration ratio (unitless)") # Table 1 row 'Heart' Kt:pl = 0.15
    lkp_spleen <- fixed(log(0.16))
    label("Log spleen-to-plasma concentration ratio (unitless)") # Table 1 row 'Spleen' Kt:pl = 0.16
    lkp_skin <- fixed(log(0.21))
    label("Log skin-to-plasma concentration ratio (unitless)") # Table 1 row 'Skin' Kt:pl = 0.21
    lkp_gut <- fixed(log(0.15))
    label("Log gastrointestinal tract-to-plasma concentration ratio (unitless)") # Table 1 row 'Gastrointestinal tract' Kt:pl = 0.15
    lkp_other <- fixed(log(0.0015))
    label("Log rest of body-to-plasma concentration ratio (unitless)") # Table 1 row 'Rest of body' Kt:pl = 0.0015

    # ----------------------------------------------------------------
    # Brain permeability-surface area product.
    # ----------------------------------------------------------------
    ps_brain <- fixed(0.0849)
    label("Brain permeability-surface area product PS (mL/min)")
    # Table 1 footnote row 'PS (mL/min)', Monkey column. Scaled from the fitted mouse value as PS_i = PS_mouse * (W_i / W_mouse)^0.67.

    # ----------------------------------------------------------------
    # Drug-specific constants.
    # ----------------------------------------------------------------
    fu <- fixed(0.0081)
    label("Fraction of DPT unbound in cynomolgus monkey plasma (unitless)")
    # Results 'Plasma Protein Binding of DPT in Five Species': monkey plasma protein binding 99.19 +/- 0.03%, so fu = 1 - 0.9919
    rbp <- fixed(1)
