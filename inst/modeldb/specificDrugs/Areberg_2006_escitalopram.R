Areberg_2006_escitalopram <- function() {
  ...
  ini({
    ...
    lka      <- fixed(log(3.6)); label("Absorption rate constant (1/h)")                      # Areberg 2006 Table 3 (theta_5, 'fixed'; Results: 'k_a had to be fixed in order to run the model successfully ... set to 3.6 h^-1 ... based on modeling the data from each subject separately')
    lcl      <- log(19);         label("Apparent oral clearance at the cohort-mean CYP2C19 ratio (L/h)")  # Areberg 2006 Table 3 (theta_2, 19 L/h, RSE 7%) / Results 'The average subject would have values for V/F and CL/F of ... 19 L/h'
    lvc      <- log(800);        label("Apparent central volume of distribution at the cohort-mean weight (L)")  # Areberg 2006 Table 3 (theta_1, 8.0*10^2 L, RSE 6%) / Results 'The average subject would have values for V/F ... of 8.0*10^2 L'
    lvp      <- log(470);        label("Apparent peripheral volume of distribution (L)")            # Areberg 2006 Table 3 (theta_3, 4.7*10^2 L, RSE 8%)
    lq       <- log(98);         label("Apparent inter-compartmental clearance (L/h)")            # Areberg 2006 Table 3 (theta_4, 98 L/h, RSE 13%)
    ltlag    <- log(0.93);       label("Absorption lag time (h)")                                  # Areberg 2006 Table 3 (theta_6, 0.93 h, RSE 1%)
