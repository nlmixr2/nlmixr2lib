AbdRahman_2020_chloroquine_plasma <- function() {
  description <- paste(
    "Semi-mechanistic population PK/PD model of chloroquine and its active",
    "metabolite desethylchloroquine, driven by PLASMA drug concentrations,",
    "in 24 healthy adults inoculated with blood-stage Plasmodium vivax in a",
    "volunteer infection study (Abd-Rahman 2020). PK is a joint",
    "parent-metabolite model: chloroquine follows a two-compartment",
    ...
    "319.8, desethylchloroquine MW 291.8 g/mol). PD is a delayed-effect",
    "(effect-compartment) parasite-clearance model: chloroquine in the effect",
    "compartment equilibrates with plasma at rate ke0 and drives a sigmoid",
    "Emax parasite-killing rate kkill; the log10 P. vivax parasitaemia grows",
    "at a net rate kgrow and declines by kkill. EC50 in plasma was fixed to",
    "one sixth of the whole-blood value and the Hill coefficient was fixed at",
    "2.5 because no subject recrudesced. See the companion whole-blood model",
    "modellib('AbdRahman_2020_chloroquine_wholeblood').",
    sep = " "
  )
  reference <- paste(
  ...
  compartmentData <- list(
    ...
    central = list(analyte = "chloroquine", units = "umol", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "chloroquine", units = "umol", specimen = "plasma", verified = TRUE),
    central_dcq = list(analyte = "desethylchloroquine", units = "umol", specimen = "plasma", verified = TRUE),
    peripheral1_dcq = list(analyte = "desethylchloroquine", units = "umol", specimen = "plasma", verified = TRUE),
    effect = list(
      analyte = "chloroquine",
      units = "umol/L",
  ...
  covariatesDataExcluded <- list(
    WT = list(
      ...
      notes = paste(
        ...
        "as a power function; not retained (narrow body-weight range; 95% CI",
        "of the effect within the +/-20% clinical-relevance region).",
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "LC-MS/MS in plasma and whole blood (plasma LLOQ 1.35 ug/L for",
      "chloroquine and 1.3 ug/L for desethylchloroquine); P. vivax",
      "parasitaemia by 18S rRNA qPCR (limit of detection 10 parasites/mL).",
      "PK was fit separately to plasma and to whole blood data; this file is",
      "the plasma model. Modelling used Monolix 4.3.3 with left-censoring of",
      "below-limit data. Structural PK estimates are Abd-Rahman 2020 Table 2",
      "(plasma columns); PD estimates are Table 3."
    )
  )

  ini({
    # =========================================================================
    # Chloroquine PK -- Abd-Rahman 2020 Table 2, plasma-sample columns.
    ...
    # =========================================================================
    lka     <- log(0.943); label("Chloroquine first-order absorption rate constant ka (1/h)")                 # Table 2 plasma row 'k a (1/h) = 0.943 (95% CI 0.551-1.335)'
    lcl     <- log(54.6);  label("Apparent chloroquine clearance CL_CQ/F (L/h)")                              # Table 2 plasma row 'CL CQ/F (L/h) = 54.6 (95% CI 49.7-59.6)'
    lvc     <- log(2930);  label("Apparent chloroquine central volume Vc_CQ/F (L)")                           # Table 2 plasma row 'V c CQ/F (L) = 2930 (95% CI 2558-3302)'
    lq      <- log(47.2);  label("Apparent chloroquine intercompartmental clearance Q1_CQ/F (L/h)")           # Table 2 plasma row 'Q 1 CQ/F (L/h) = 47.2 (95% CI 38.8-55.6)'
    lvp     <- log(4700);  label("Apparent chloroquine peripheral volume Vp1_CQ/F (L)")                       # Table 2 plasma row 'V p1 CQ/F (L) = 4700 (95% CI 4053-5347)'
    lfdepot <- fixed(log(1)); label("Chloroquine relative bioavailability Frel (fraction)")                   # Table 2 plasma row 'F rel (%) = 100 (fixed)'; typical F fixed to 100%, only its IIV estimated

    # =========================================================================
    # Desethylchloroquine PK -- Abd-Rahman 2020 Table 2, plasma-sample columns.
    ...
    # modelling").
    # =========================================================================
    lcl_dcq <- log(37.6);  label("Apparent desethylchloroquine clearance CL_DCQ/F (L/h)")                     # Table 2 plasma row 'CL DCQ/F (L/h) = 37.6 (95% CI 32.9-42.3)'
    lvc_dcq <- log(40.0);  label("Apparent desethylchloroquine central volume Vc_DCQ/F (L)")                  # Table 2 plasma row 'V c DCQ/F (L) = 40.0 (95% CI 8.6-71.4)'
    lq_dcq  <- log(36.3);  label("Apparent desethylchloroquine intercompartmental clearance Q1_DCQ/F (L/h)")  # Table 2 plasma row 'Q 1 DCQ/F (L/h) = 36.3 (95% CI 28.7-43.9)'
    lvp_dcq <- log(2840);  label("Apparent desethylchloroquine peripheral volume Vp1_DCQ/F (L)")              # Table 2 plasma row 'V p1 DCQ/F (L) = 2840 (95% CI 2134-3546)'
    ...
    # whole-blood model; EC50 and ke0 are plasma-specific.
    # =========================================================================
    ...
    lec50  <- fixed(log(0.047)); label("Chloroquine effect-site EC50 in plasma, EC50_CQ_plasma (umol/L)")     # Table 3 row 'EC 50CQ plasma (umol/L) = 0.047 (fixed)'; fixed to one sixth of the whole-blood EC50 (plasma:whole-blood ratio ~1:6)
    lhill  <- fixed(log(2.5));   label("Hill coefficient of the chloroquine killing sigmoid in plasma (unitless)") # Table 3 row 'Hill CQ plasma = 2.5 (fixed)'
    lke0   <- log(0.0212); label("Chloroquine effect-compartment equilibration rate ke0 in plasma (1/h)")     # Table 3 row 'k e0CQ plasma (1/h) = 0.0212 (95% CI 0.0171-0.0253)'
    ...
    # =========================================================================
    # Inter-individual variability. PK: Table 2 plasma omega estimates
    # (standard deviations), squared for nlmixr2 variances. There is no
    # plasma IIV on Vc_DCQ. PD: Table 3 omega estimates squared; the baseline
    # PLbase random effect is NORMAL (added directly, not log-normal) per
    # Methods ("except for the log10-transformed baseline parasitemia, for
    # which a normal distribution was assumed"). EC50 and ke0 IIV were fixed.
    # =========================================================================
    etalka     ~ 0.438244   # Table 2 plasma 'omega k a = 0.662'; variance = 0.662^2
    etalfdepot ~ 0.036864   # Table 2 plasma 'omega F rel = 0.192'; variance = 0.192^2
    etalcl     ~ 0.007056   # Table 2 plasma 'omega CL CQ/F = 0.084'; variance = 0.084^2
    etalvc     ~ 0.032041   # Table 2 plasma 'omega Vc CQ/F = 0.179'; variance = 0.179^2
    etalcl_dcq ~ 0.038809   # Table 2 plasma 'omega CL DCQ/F = 0.197'; variance = 0.197^2
    ...
    etalec50   ~ fixed(0.09)       # Table 3 'omega EC 50CQ plasma = 0.3', not estimated; variance = 0.3^2
    etalke0    ~ fixed(0.01)       # Table 3 'omega k e CQ plasma = 0.1', not estimated; variance = 0.1^2
    ...
    # =========================================================================
    propSd     <- 0.243; label("Proportional residual error on plasma chloroquine concentration (fraction)")            # Table 2 plasma row 'epsilon prop CQ = 0.243 (95% CI 0.219-0.267)'
    propSd_dcq <- 0.347; label("Proportional residual error on plasma desethylchloroquine concentration (fraction)")    # Table 2 plasma row 'epsilon prop DCQ = 0.347 (95% CI 0.314-0.380)'
    addSd_parasitemia_log10 <- 0.378; label("Additive residual error on log10 parasitaemia (log10 parasites/mL)")       # Table 3 row 'epsilon add plasma = 0.378 (95% CI 0.365-0.391)'
  })
