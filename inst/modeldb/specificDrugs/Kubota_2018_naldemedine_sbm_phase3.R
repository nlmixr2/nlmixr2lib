Kubota_2018_naldemedine_sbm_phase3 <- function() {
  description <- paste0(
    "Landmark binomial logistic-regression exposure-EFFICACY model relating ",
    "the steady-state daily AUC of naldemedine to the probability of being a spontaneous-bowel-movement (SBM) responder ",
    ...
    "prob = 1 / (1 + exp(-(a + b * AUCss))) with a = -0.537 and b = 0.0194. ",
    "This is the better-determined of the two efficacy fits: the 95% CI of the slope excludes zero. The relationship is nonetheless shallow -- Kubota 2018 concludes that halving or doubling the 0.2 mg exposure changes the predicted responder proportion only minimally, which is the basis for the paper's conclusion that no covariate-driven dose adjustment is required. ",
  ...
  reference <- paste(
    ...
    "Logistic-regression coefficients are Table V(a), row '1314V9231 and 1315V9232 (Phase 3)';",
  ...
  units <- list(
    ...
    concentration = "prob_sbm_responder (probability of a spontaneous bowel movement (SBM) responder, 0-1; also logit_sbm_responder)"
  )
  ...
  population <- list(
    ...
    n_subjects = 993L,
    n_studies = 2L,
    n_observations = "993 binary a spontaneous bowel movement (SBM) responder records, one per subject",
    ...
    regions = "Global",
    notes = paste(
      "In these two pivotal 12-week phase 3 studies an SBM responder is a subject with 9 or more positive response weeks out of 12 AND 3 positive response weeks out of the last 4, where a positive response week has both 3 or more SBMs and an increase of 1 or more SBM from baseline. A subject with insufficient response data was treated as a non-responder. Observed responder frequencies (Table IV): placebo 187/548 (34.1%), 0.2 mg 237/445 (53.3%). The two studies were pooled because they shared design and duration.",
      ...
      "the placebo group who also had EFFICACY data, so placebo subjects who",
  ...
  ini({
    # ==================================================================
    # Kubota 2018 Table V(a), row '1314V9231 and 1315V9232 (Phase 3)'. The model is the paper's Eq. (3):
    #
    ...
    logit_ref <- -0.537; label("Logit of the a spontaneous bowel movement (SBM) responder probability at zero naldemedine exposure, i.e. the placebo arm (unitless logit)")  # Table V(a), row '1314V9231 and 1315V9232 (Phase 3)', parameter a = -0.537 (95% CI -0.700 to -0.375)
    e_auc_logit <- 0.0194; label("Log-odds of a spontaneous bowel movement (SBM) responder per 1 ng*h/mL increase in naldemedine steady-state daily AUC (unitless logit)")  # Table V(a), row '1314V9231 and 1315V9232 (Phase 3)', parameter b = 0.0194 (95% CI 0.0114 to 0.0274)
    ...
    addSd_prob_sbm_responder <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    # Linear predictor in the untransformed steady-state daily AUC.
    logit_sbm_responder <- logit_ref + e_auc_logit * AUC_NALD

    prob_sbm_responder <- expit(logit_sbm_responder)

    # ----- Observation -----
    prob_sbm_responder ~ add(addSd_prob_sbm_responder)
  })
}
