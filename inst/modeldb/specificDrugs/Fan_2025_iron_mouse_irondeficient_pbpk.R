Fan_2025_iron_mouse_irondeficient_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 14-compartment, perfusion-limited; NONMEM ADVAN15).",
    "Preclinical (mouse, iron-deficient diet).",
    ...
    "iron-deficient diet column of Table 2; organ volumes and blood flows are",
  ...
  ini({
    # ---- Estimated iron-disposition parameters: Table 2, iron-deficient diet column ----
    # RSE% from Table 2 is given after each value.
    lcl <- log(0.124e-4); label("Unavoidable physiologic iron loss clearance from plasma (L/h)")  # Table 2 CLloss = 0.124 x 10^-4 L/h (RSE 6.19%)
    lkp_adipose <- log(0.186); label("Fat (adipose)-to-plasma partition coefficient (unitless)")  # Table 2 KPfat (RSE 0.042%)
    lkp_bone    <- log(3.635); label("Bone-to-plasma partition coefficient (unitless)")  # Table 2 KPbon (RSE 0.35%)
    lkp_brain   <- log(1.075); label("Brain-to-plasma partition coefficient (unitless)")  # Table 2 KPbra (RSE 0.045%)
    lkp_gut     <- log(5.213); label("Gut-to-plasma partition coefficient (unitless)")  # Table 2 KPgut (RSE 0.14%)
    lkp_heart   <- log(10.08); label("Heart-to-plasma partition coefficient (unitless)")  # Table 2 KPhea (RSE 0.092%)
    lkp_kidney  <- log(6.541); label("Kidney-to-plasma partition coefficient (unitless)")  # Table 2 KPkid (RSE 0.045%)
    lkp_liver   <- log(9.38); label("Liver-to-plasma partition coefficient (unitless)")  # Table 2 KPliv (RSE 0.10%)
    lkp_lung    <- log(17.86); label("Lung-to-plasma partition coefficient (unitless)")  # Table 2 KPlun (RSE 0.023%)
    lkp_muscle  <- log(1.288); label("Muscle-to-plasma partition coefficient (unitless)")  # Table 2 KPmus (RSE 0.07%)
    lkp_other   <- log(4.93); label("Remainder (rest of body)-to-plasma partition coefficient (unitless)")  # Table 2 KPrem (RSE 0.079%)
    lkp_skin    <- log(0.101); label("Skin-to-plasma partition coefficient (unitless)")  # Table 2 KPski (RSE 0.042%)
    lkp_spleen  <- log(10.13); label("Spleen-to-plasma partition coefficient (unitless)")  # Table 2 KPspl (RSE 0.06%)
    lq_bone_rbc <- log(0.434e-3); label("Erythropoietic iron utilisation flow, bone to red cells (L/h)")  # Table 2 QE (RSE 0.29%)
    lmtt_rbc <- log(41.53); label("Mean red-blood-cell lifespan (h)")  # Table 2 TRBC (RSE 0.067%)
