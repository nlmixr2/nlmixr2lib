Echterhof_2026_phage_human_pbpk <- function() {
  description <- paste(
    "Human.",
    ...
    "densities are replaced with the human column of Table S3. This is the",
    "configuration behind the paper's first-in-human dose projections: it was",
    "qualified against digitised clinical data for the CRISPR-enhanced",
    "Escherichia coli phage LBP-EC01 (Kim 2024, Lancet Infect Dis) dosed at",
    "10^11 PFU every 12 h, giving rBias -106 percent and rRMSE 13.4 percent",
    "overall, and then used for the Monte Carlo trial simulations of 10^7 to",
    "10^12 PFU daily as single, Q24H, Q12H and continuous-infusion regimens.",
    "Twelve organs (lung, muscle, bone, liver,",
  ...
  covariateData <- list(
    WT = list(
      ...
      notes              = paste(
        ...
        "Table S3 tabulates 70 kg as the generic human, and Results 'Scaling to",
        "humans' states that active clearance was allometrically scaled by body",
        "mass 'assuming a human median mass of 70 kg'. Time-invariant."
      ),
      source_name        = "BW"
    ),
    DOSE_PHAGE_PFU = list(
  ...
  population <- list(
    ...
    weight_median  = "70 kg (Results 'Scaling to humans': active clearance scaled to an assumed human median mass of 70 kg; the same value is tabulated in Table S3)",
    sex_female_pct = NA_real_,
    disease_state  = paste(
      "Qualification data are from patients treated with the CRISPR-enhanced",
      "Escherichia coli phage LBP-EC01 (Kim 2024); the trial simulations assume",
      "a generic 70 kg adult."
    ),
    dose_range     = paste(
      "Qualification: 10^11 PFU intravenously every 12 h, given as a rapid",
      "5 min push (Methods 'External validation'). Trial simulation: daily",
      "doses of 10^7 to 10^12 PFU as single dose, Q24H, Q12H or continuous",
      "infusion (Methods 'Clinical trial simulation')."
    ),
    regions        = NA_character_,
    notes          = paste(
      "NO human data were fitted. Every estimated parameter comes from the CD-1",
      "mouse fit; the human enters only through the Table S3 anatomy and",
      "phagocyte densities, with active clearance scaled by the L/h/kg unit to",
      "70 kg. The comparison data are DIGITISED from Kim et al. (Lancet Infect",
      "Dis 2024) using WebPlotDigitizer, so no demographic breakdown is",
      "recoverable. Simulations assume phage disposition is independent of",
      "haematocrit and that phage does not distribute into the red-blood-cell",
      "space (Methods 'Clinical trial simulation'), i.e. the blood compartment",
      "of this model is the plasma space. The mouse fit was naive-pooled with",
      "no random effects, so no IIV is encoded here."
    )
  )

  ini({
    # ==================================================================
    # Human anatomy and physiology -- Supplemental Table S3, "Human" column.
    ...
    CO_C <- fixed(16.5) ; label("Cardiac output coefficient (L/h/kg^0.75)")                      # Table S3 CO, Human
    ...
    fq_brain            <- fixed(0.114)    ; label("Fraction of cardiac output to brain (unitless)")            # Table S3 F Q_Brn, Human
    fq_liver            <- fixed(0.046)    ; label("Fraction of cardiac output to liver, hepatic artery (unitless)")  # Table S3 F Q_Lvr, Human
    fq_spleen           <- fixed(0.01375)    ; label("Fraction of cardiac output to spleen (unitless)")           # Table S3 F Q_Spn, Human
    fq_small_intestine  <- fixed(0.13575)    ; label("Fraction of cardiac output to small intestine (unitless)")  # Table S3 F Q_SI, Human
    fq_large_intestine  <- fixed(0.022625)   ; label("Fraction of cardiac output to large intestine (unitless)")  # Table S3 F Q_LI, Human
    fq_kidney           <- fixed(0.175)    ; label("Fraction of cardiac output to kidney (unitless)")           # Table S3 F Q_Kid, Human
    fq_muscle           <- fixed(0.191)    ; label("Fraction of cardiac output to muscle (unitless)")           # Table S3 F Q_Msc, Human
    fq_skin             <- fixed(0.058)    ; label("Fraction of cardiac output to skin (unitless)")             # Table S3 F Q_Ski, Human
    fq_bone             <- fixed(0.042)   ; label("Fraction of cardiac output to bone (unitless)")             # Table S3 F Q_Bon, Human
    fq_stomach          <- fixed(0.022625) ; label("Fraction of cardiac output to stomach (unitless)")          # Table S3 F Q_Sto, Human

    # Organ volume as a fraction of body weight (unitless). Carcass closes the
    # balance against 1, so no F_V_Crs row is needed.
    fv_lung             <- fixed(0.008)   ; label("Lung volume as a fraction of body weight (unitless)")            # Table S3 F V_Lun, Human
    fv_brain            <- fixed(0.02)   ; label("Brain volume as a fraction of body weight (unitless)")           # Table S3 F V_Brn, Human
    fv_liver            <- fixed(0.0257)   ; label("Liver volume as a fraction of body weight (unitless)")           # Table S3 F V_Lvr, Human
    fv_spleen           <- fixed(0.00257)    ; label("Spleen volume as a fraction of body weight (unitless)")          # Table S3 F V_Spn, Human (row captioned "Fraction of BW as Lung" -- a copy-paste label error; the row is spleen, see Errata)
    fv_small_intestine  <- fixed(0.0091)   ; label("Small-intestine volume as a fraction of body weight (unitless)") # Table S3 F V_SI, Human
    fv_large_intestine  <- fixed(0.0053)   ; label("Large-intestine volume as a fraction of body weight (unitless)") # Table S3 F V_LI, Human
    fv_kidney           <- fixed(0.0044)   ; label("Kidney volume as a fraction of body weight (unitless)")          # Table S3 F V_Kid, Human
    fv_muscle           <- fixed(0.4)    ; label("Muscle volume as a fraction of body weight (unitless)")          # Table S3 F V_Msc, Human
    fv_skin             <- fixed(0.0371)   ; label("Skin volume as a fraction of body weight (unitless)")            # Table S3 F V_Ski, Human
    fv_bone             <- fixed(0.02)     ; label("Bone volume as a fraction of body weight (unitless)")            # Table S3 F V_Mar, Human
    fv_stomach          <- fixed(0.0021)    ; label("Stomach volume as a fraction of body weight (unitless)")         # Table S3 Fr_V_Sto, Human
    fv_blood            <- fixed(0.079) ; label("Blood volume as a fraction of body weight (unitless)")           # Table S3 F V_Pla, Human

    # Vascular (capillary blood) fraction of each organ's volume (unitless).
    bv_lung             <- fixed(0.3867)   ; label("Vascular fraction of lung volume (unitless)")             # Table S3 F V_Lun_Bld, Human
    bv_brain            <- fixed(0.04)  ; label("Vascular fraction of brain volume (unitless)")            # Table S3 F V_Brn_Bld, Human
    bv_liver            <- fixed(0.11)  ; label("Vascular fraction of liver volume (unitless)")            # Table S3 F V_Lvr_Bld, Human
    bv_spleen           <- fixed(0.3)  ; label("Vascular fraction of spleen volume (unitless)")           # Table S3 F V_Spn_Bld, Human
    bv_small_intestine  <- fixed(0.3)  ; label("Vascular fraction of small-intestine volume (unitless)")  # Table S3 F V_SI_Bld, Human
    bv_large_intestine  <- fixed(0.3)  ; label("Vascular fraction of large-intestine volume (unitless)")  # Table S3 F V_LI_Bld, Human
    bv_kidney           <- fixed(0.36)  ; label("Vascular fraction of kidney volume (unitless)")           # Table S3 F V_Kid_Bld, Human
    bv_muscle           <- fixed(0.01)  ; label("Vascular fraction of muscle volume (unitless)")           # Table S3 F V_Msc_Bld, Human
    bv_skin             <- fixed(0.08)  ; label("Vascular fraction of skin volume (unitless)")             # Table S3 F V_Ski_Bld, Human
    bv_remainder        <- fixed(0.01)  ; label("Vascular fraction of carcass volume (unitless)")          # Table S3 F V_Crs_Bld, Human
    bv_bone             <- fixed(0.04)   ; label("Vascular fraction of bone volume (unitless)")             # Table S3 F V_Bon_Bld, Human
    bv_stomach          <- fixed(0.3)  ; label("Vascular fraction of stomach volume (unitless)")          # Table S3 F V_Sto_Bld, Human

    # Tissue-resident phagocyte density (cells/g). Sets each organ's MPS
    # carrying capacity.
    m_blood             <- fixed(4170000)  ; label("Phagocyte density in blood (cells/g)")             # Table S3 M Pla, Human
    m_lung              <- fixed(24800000) ; label("Phagocyte density in lung (cells/g)")              # Table S3 M Lun, Human
    ...
    m_liver             <- fixed(21800000) ; label("Phagocyte density in liver (cells/g)")             # Table S3 M Lvr, Human
    m_spleen            <- fixed(1.18e8) ; label("Phagocyte density in spleen (cells/g)")            # Table S3 M Spn, Human
    m_small_intestine   <- fixed(1710000)  ; label("Phagocyte density in small intestine (cells/g)")   # Table S3 M SI, Human
    m_large_intestine   <- fixed(11200000)  ; label("Phagocyte density in large intestine (cells/g)")   # Table S3 M LI, Human
    m_kidney            <- fixed(2210000)  ; label("Phagocyte density in kidney (cells/g)")            # Table S3 M Kid, Human
    m_remainder         <- fixed(2630000)  ; label("Phagocyte density in carcass (cells/g)")           # Table S3 M Crs, Human
    m_muscle            <- fixed(773000)  ; label("Phagocyte density in muscle (cells/g)")            # Table S3 M Msc, Human
    m_skin              <- fixed(3910000)  ; label("Phagocyte density in skin (cells/g)")              # Table S3 M Ski, Human
    m_bone              <- fixed(45300000)  ; label("Phagocyte density in bone (cells/g)")              # Table S3 M Bon, Human
    m_stomach           <- fixed(936000)  ; label("Phagocyte density in stomach (cells/g)")           # Table S3 M Sto, Human
