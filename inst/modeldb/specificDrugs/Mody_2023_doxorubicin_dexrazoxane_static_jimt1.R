Mody_2023_doxorubicin_dexrazoxane_static_jimt1 <- function() {
  ...
  ini({
    ...
    # 106 for the DOX curve and 106 for the DEX curve).
    ...
    limax_dox <- fixed(log(0.988))  ; label("Maximal fractional inhibition of viability by DOX, Imax,DOX (unitless)")  # Table 1: 0.988 (3.7% RSE)
    lec50_dox <- fixed(log(0.214)) ; label("DOX concentration for half-maximal inhibition IC50,DOX (uM)")            # Table 1: 214 nM = 0.214 uM (18.5% RSE)

    limax_dexrazoxane <- fixed(log(0.74))  ; label("Maximal fractional inhibition of viability by DEX, Imax,DEX (unitless)")  # Table 1: 0.74 (6.77% RSE)
    lec50_dexrazoxane <- fixed(log(97.5))  ; label("DEX concentration for half-maximal inhibition IC50,DEX (uM)")             # Table 1: 97.5 uM (23.3% RSE)
    ...
    lpsi <- log(1.11) ; label("DOX-DEX interaction parameter psi (unitless; >1 antagonistic, <1 synergistic, =1 additive)")  # Results / Figure 2 caption: psi = 1.11
