Dasti_2025_mrna1273_qsp <- function() {
  description <- paste(
    "QSP (multiscale mRNA-vaccine immune response, tissue layer). Dasti 2025",
    "model of the humoral response to the Moderna mRNA-1273 COVID-19",
    "mRNA vaccine in adults. Three physiological",
  ...
  reference <- paste(
    ...
    "initial values from Table S2, parameters from Table S3 and the mRNA-1273 estimates from Table S9,",
    ...
    "(Figure_reproduction/Figure 7/{model_equations,Parameters,simulation}.m);",
  ...
  population <- list(
    ...
    notes         = paste(
      ...
      "same spike antigen as BNT162b2, so the published implementation reuses",
      "the BNT162b2 mRNA molecular weight when converting the mass dose to",
      "pmol. Sample sizes of the source studies are listed in SI Table S8.",
      sep = " "
    )
  )
  ...
  ini({
    ...
    kactNT       <- fixed(305.593943); label("Maximum naive helper T-cell activation rate (1/day)") # Table S9 (305.5939), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kactMT       <- fixed(637.5926644); label("Maximum memory helper T-cell activation rate (1/day)") # Table S9 (637.5927), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kprolAT      <- fixed(6.239953041); label("Maximum activated helper T-cell proliferation rate (1/day)") # Table S9 (6.2400), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    ...
    CCN          <- fixed(96.66083585); label("Carrying capacity of 1 functional T cell for naive B-cell activation (unitless)") # Table S9 (96.6608), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kactMB       <- fixed(14.25252128); label("Maximum memory B-cell activation rate (1/day)") # Table S9 (14.2525), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kprolABN     <- fixed(9.78725703); label("Maximum proliferation rate of activated B cells derived from naive B cells (1/day)") # Table S9 (9.7873), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kprolABM     <- fixed(2.911196668); label("Maximum proliferation rate of activated B cells derived from memory B cells (1/day)") # Table S9 (2.9112), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kln2blPC     <- fixed(23.88807008); label("Plasma-cell migration rate, lymph node to blood (1/day)") # Table S9 (23.8881), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kdtSP        <- fixed(0.06506866486); label("Short-lived plasma-cell death rate (1/day)") # Table S9 (0.0651), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    kdtLP        <- fixed(0.006996046844); label("Long-lived plasma-cell death rate (1/day)") # Table S9 (0.0070), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    delayB       <- fixed(0.0008119979953); label("Activated B-cell migration rate into the germinal centre (1/day)") # Table S9 (8.1200e-04), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
    ...
    kdegAb       <- fixed(0.1424413132); label("Antibody elimination rate in blood (1/day)") # Table S9 (0.1424), the column headed 'Calibration with Pfizer over 60 y.o. data'; that header is transposed with the Moderna header -- see vignette Errata
