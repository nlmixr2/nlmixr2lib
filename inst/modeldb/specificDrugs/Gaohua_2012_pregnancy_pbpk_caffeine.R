Gaohua_2012_pregnancy_pbpk_caffeine <- function() {
  description <- paste(
    ...
    "with gestational-age-dependent maternal physiology and hepatic CYP1A2",
  ...
  compartmentData <- list(
    depot    = list(analyte = "caffeine", units = "mg", specimen = "administration site", verified = FALSE),
    lung     = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    adipose  = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    bone     = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    brain    = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    heart    = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    kidney   = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    gut      = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    liver    = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    muscle   = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    skin     = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    spleen   = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    preg     = list(analyte = "caffeine", units = "mg", specimen = "tissue", verified = FALSE),
    arterial = list(analyte = "caffeine", units = "mg", specimen = "blood cell", verified = FALSE),
    venous   = list(analyte = "caffeine", units = "mg", specimen = "blood cell", verified = FALSE)
  )
  ...
  population <- list(
    ...
    disease_state  = paste(
      ...
      "model file extracts the caffeine case and is paired with",
      "Gaohua_2012_pregnancy_pbpk_metoprolol and",
      "Gaohua_2012_pregnancy_pbpk_midazolam."
    ),
    dose_range     = "Caffeine 150 mg oral single dose (Brazier 1983 protocol; Gaohua 2012 Fig 2, Table 5)",
  ...
  ini({
    ...
    # Absorption parameters (Gaohua 2012 Table 3, caffeine column).
    lka       <- fixed(log(2.18));     label("First-order absorption rate constant (1/h)")                 # Gaohua 2012 Table 3 (caffeine ka = 2.18 1/h)
    fa        <- fixed(1.0);           label("Fraction available for absorption from dosage form")         # Gaohua 2012 Table 3 (caffeine fa = 1)
    fg        <- fixed(1.0);           label("Gut availability (fraction surviving gut wall first pass)")  # Gaohua 2012 Table 3 (caffeine Fg = 1)
    ...
    fu        <- fixed(0.681);         label("Fraction unbound in plasma (basal, held constant over GA)")  # Gaohua 2012 Table 3 (caffeine fu = 0.681)
    bp        <- fixed(0.98);          label("Blood:plasma concentration ratio (basal, held constant)")    # Gaohua 2012 Table 3 (caffeine B:P = 0.98)
    ...
    lcl_int_h <- fixed(log(8));        label("Basal hepatic intrinsic clearance CL_int,H0 (L/h)")          # Gaohua 2012 Table 3 (caffeine CL_int,H = 8 L/h)
    ...
    a_cyp1a2  <- fixed(1.0);           label("Fractional contribution of CYP1A2 to CL_int,H")              # Gaohua 2012 Table 3 (caffeine A_1A2 = 1)
    a_cyp2d6  <- fixed(0.0);           label("Fractional contribution of CYP2D6 to CL_int,H")              # Gaohua 2012 Table 3 (caffeine A_2D6 = 0)
    a_cyp3a4  <- fixed(0.0);           label("Fractional contribution of CYP3A4 to CL_int,H")              # Gaohua 2012 Table 3 (caffeine A_3A4 = 0)
    ...
    kp_adipose <- fixed(0.191);   label("Tissue:plasma partition coefficient, adipose")     # Gaohua 2012 Table 4 (caffeine adipose 0.191)
    kp_bone    <- fixed(0.374);   label("Tissue:plasma partition coefficient, bone")        # Gaohua 2012 Table 4 (caffeine bone 0.374)
    kp_brain   <- fixed(0.606);   label("Tissue:plasma partition coefficient, brain")       # Gaohua 2012 Table 4 (caffeine brain 0.606)
    kp_gut     <- fixed(0.578);   label("Tissue:plasma partition coefficient, gut")         # Gaohua 2012 Table 4 (caffeine gut 0.578)
    kp_heart   <- fixed(0.583);   label("Tissue:plasma partition coefficient, heart")       # Gaohua 2012 Table 4 (caffeine heart 0.583)
    kp_kidney  <- fixed(0.597);   label("Tissue:plasma partition coefficient, kidney")      # Gaohua 2012 Table 4 (caffeine kidney 0.597)
    kp_liver   <- fixed(0.57);    label("Tissue:plasma partition coefficient, liver")       # Gaohua 2012 Table 4 (caffeine liver 0.57)
    kp_lung    <- fixed(0.62);    label("Tissue:plasma partition coefficient, lung")        # Gaohua 2012 Table 4 (caffeine lung 0.62)
    kp_muscle  <- fixed(0.622);   label("Tissue:plasma partition coefficient, muscle")      # Gaohua 2012 Table 4 (caffeine muscle 0.622)
    kp_skin    <- fixed(0.6);     label("Tissue:plasma partition coefficient, skin")        # Gaohua 2012 Table 4 (caffeine skin 0.6)
    kp_spleen  <- fixed(0.591);   label("Tissue:plasma partition coefficient, spleen")      # Gaohua 2012 Table 4 (caffeine spleen 0.591)
    kp_preg    <- fixed(0.606);   label("Tissue:plasma partition coefficient, fetoplacental unit") # Gaohua 2012 Table 4 footnote (assumed = brain)
