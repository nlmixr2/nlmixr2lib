Zhou_2025_brentuximab <- function() {
  ...
  ini({
    ...
    lcl  <- log(0.0208); label("ADC clearance (L/h)")                                     # Zhou 2025 Table S1: 0.0208 (16.3% RSE)
    lvc  <- log(2.54);   label("ADC central volume (L)")                                   # Zhou 2025 Table S1: 2.54 (0.7% RSE)
    lq   <- log(0.0192); label("ADC inter-compartmental clearance to peripheral 1 (L/h)") # Zhou 2025 Table S1: 0.0192 (14.2% RSE)
    lvp  <- log(97.1);   label("ADC peripheral volume 1 (L)")                              # Zhou 2025 Table S1: 97.1 (18.9% RSE)
    lq2  <- log(0.0865); label("ADC inter-compartmental clearance to peripheral 2 (L/h)") # Zhou 2025 Table S1: 0.0865 (7.3% RSE)
    lvp2 <- log(3.39);   label("ADC peripheral volume 2 (L)")                              # Zhou 2025 Table S1: 3.39 (15.8% RSE)
    ...
    lcl_mmae   <- log(0.794);    label("MMAE clearance (L/h)")                                    # Zhou 2025 Table S2: 0.794 (2.0% RSE)
    lvc_mmae   <- log(20.1);     label("MMAE central volume (L)")                                   # Zhou 2025 Table S2: 20.1 (0.7% RSE)
    lq_mmae    <- log(0.628);    label("MMAE inter-compartmental clearance (L/h)")                 # Zhou 2025 Table S2: 0.628 (1.2% RSE)
    lvp_mmae   <- log(2.74);     label("MMAE peripheral volume (L)")                               # Zhou 2025 Table S2: 2.74 (0.7% RSE)
    lkd_mmae   <- log(0.0186);   label("MMAE binding rate constant (1/h)")                         # Zhou 2025 Table S2: 0.0186 (0.4% RSE)
    lalfm_mmae <- log(0.00462);  label("Decay rate of ADC->MMAE proteolytic-conversion fraction (1/h)") # Zhou 2025 Table S2: 0.00462 (0.7% RSE)
    lklag_mmae <- log(60.8);     label("Lag-compartment empty rate constant (1/h)")              # Zhou 2025 Table S2: 60.8 (1.3% RSE)
