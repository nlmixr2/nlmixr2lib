LopezArguello_2023_imipenem_qsp <- function() {
  ...
  compartmentData <- list(
    ...
    nperi  = list(analyte = "imipenem", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_IPM_MGL = list(
      description = "Static extracellular imipenem concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    notes = paste(
      ...
      paste("Imipenem was the backbone drug of all five datasets and Table S1 reports a single set of",
  ...
  ini({
    ...
    rate_influx_scaled <- 74.5; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, imipenem: 74.5 (SE 29.0%)
    # The studied extracellular concentration enters as the CONC_IPM_MGL covariate
    # (Table 1: imipenem studied at 2 mg/L = 2x the MIC of 1 mg/L).
    ...
    k2_1a <- 2210;      label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, imipenem PBP1a: 2210
    k2_1b <- 1710;      label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, imipenem PBP1b: 1710
    k2_2 <- fixed(500); label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, imipenem PBP2: 500, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_3 <- 990;        label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, imipenem PBP3: 990
    k2_4 <- 189;        label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, imipenem PBP4: 189
    k2_5 <- 335;        label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, imipenem PBP5/6: 335
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_IPM_MGL
