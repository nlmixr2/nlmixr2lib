Sun_2025_maribavir_immunosuppressant_increase <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent increase in immunosuppressant drug concentration ",
    "level as a function of the maribavir AUC on the DAY OF THE EVENT, in ",
    "allogeneic hematopoietic cell transplant (HCT) recipients with a ",
    "first asymptomatic cytomegalovirus infection treated with maribavir ",
    "400 mg orally twice daily (Sun 2025, n = 238, the AURORA maribavir ",
    "arm). Note that this is the DAY-OF-EVENT exposure metric (AUCday), ",
    "not the steady-state metric: Sun 2025's abstract reports that ",
    "STEADY-STATE exposures were not significantly associated with any ",
    "adverse event other than nausea and vomiting, whereas all fourteen ",
    "day-of-event relationships in Figure S3 are significant. A ",
    "day-of-event AUC is partly a consequence of when in the treatment ",
    "course the event happened, so it is the weaker causal claim of the ",
    "two. This is the rarest of the fourteen endpoints -- the intercept ",
    "of -5.45 puts the reference probability near 0.004 -- and ",
    "correspondingly the exposure slope has the widest relative ",
    "uncertainty (+0.0531, SE 0.0239, p = 0.0264). There is no PK layer ",
  ...
  units <- list(
    ...
    concentration = "prob_immunosuppressant_increase (probability of a treatment-emergent increase in immunosuppressant drug concentration, 0-1; also logit_immunosuppressant_increase)"
  )
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0531 per 10 ug*h/mL (odds",
        "ratio 1.05 (1.01-1.11), p = 0.0264)."
      ),
    ...
    HCT_PRIOR = list(
      ...
      notes              = paste(
        ...
        "form carried here.",
        "Coefficient +1.89. Note that the sign is OPPOSITE to this covariate's",
        "effect in the companion vomiting model (-2.05), which is a genuine",
        "difference between endpoints and not a transcription slip."
      ),
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary raised immunosuppressant concentration records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel f (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -5.45; label("Logit of the raised immunosuppressant concentration probability at zero maribavir exposure with all risk factors at their reference level (unitless logit)")  # Sun 2025 Figure S3 panel f (Supporting Information file s002), Intercept -5.45 (SE 0.908), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0531; label("Log-odds of raised immunosuppressant concentration per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel f (Supporting Information file s002), Estimate 0.0531 (SE 0.0239), odds ratio 1.05 (1.01-1.11), p = 0.0264

    # ----- Risk-factor effects on the logit -----
    e_hct_prior_logit <- 1.89; label("Log-odds of raised immunosuppressant concentration for a prior HCT versus transplant-naive (unitless logit)")  # Sun 2025 Figure S3 panel f (Supporting Information file s002), Prior HCT treatment Estimate 1.89 (SE 0.815), odds ratio 6.65 (1.35-32.8), p = 0.0201
    ...
    addSd_prob_immunosuppressant_increase <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_immunosuppressant_increase <- logit_ref +
      e_auc_logit * auc10 +
      e_hct_prior_logit * HCT_PRIOR

    prob_immunosuppressant_increase <- expit(logit_immunosuppressant_increase)

    # ----- Observation -----
    prob_immunosuppressant_increase ~ add(addSd_prob_immunosuppressant_increase)
  })
}
