Wei_2026_cefquinome_enrofloxacin_lung_tmic <- function() {
  description <- paste0(
    ...
    "carries the fit driven by the %T>MIC (combine) index computed from lung PK data. ",
    ...
    "Wei 2026's Emax = 0.094 onto the canonical `e0` (effect at zero exposure) and Wei 2026's ",
    "E0 = -5.994 onto the canonical `emax` (maximal drug effect), and writes the algebraically ",
    "identical E = e0 - (e0 - emax) * Ce^N / (Ce^N + EC50^N). No value is altered by the remapping. ",
    "Parameters from Wei 2026 Table 6, lung / %T>MIC (combine) column: e0 = 0.094, emax = -5.994 ",
    "log10 CFU/mL, EC50 = 50.059, Hill N = 1.364 (R^2 = 0.888, AIC = 17.319). ",
    ...
    "typical-value simulation. Sibling models from the same paper: Wei_2026_cefquinome_enrofloxacin_plasma_cmaxmic, Wei_2026_cefquinome_enrofloxacin_plasma_aucmic, Wei_2026_cefquinome_enrofloxacin_plasma_tmic, Wei_2026_cefquinome_enrofloxacin_lung_cmaxmic and Wei_2026_cefquinome_enrofloxacin_lung_aucmic."
  )
  reference <- paste(
    ...
    "PK/PD'. Parameter estimates: Table 6, lung / %T>MIC (combine) column.",
  ...
  covariateData <- list(
    FTMIC_CEFQ = list(
      description        = "Cefquinome PK/PD index: the percentage of a 24 h dosing interval during which the lung cefquinome concentration exceeds MIC(combine), the MIC of cefquinome measured in the presence of the average enrofloxacin lung concentration achieved over 72 h",
      units              = "percent",
      type               = "continuous",
      ...
      source_name        = "%T>MIC (combine) (Wei 2026 Table 5, lung block; Materials and methods, 'Analysis of antibacterial effects and fitting of PK/PD')"
    )
  )
  ...
  ini({
    ...
    # matrix: lung   index: %T>MIC (combine)
    # =================================================================
    ...
    #   e0   <- Wei 2026's Emax = 0.094   (effect at zero exposure)
    #   emax <- Wei 2026's E0   = -5.994  (maximal drug effect)
    ...
    e0 <- 0.094
    label("Difference in lung K. pneumoniae load from the blank control at zero exposure (log10 CFU/mL)")  # Wei 2026 Table 6, lung block, %T>MIC (combine) column, "E max" = 0.094 (defined in Materials and methods as the blank-control change)
    emax <- -5.994
    label("Maximal difference in lung K. pneumoniae load from the blank control (log10 CFU/mL; negative = reduction)")  # Wei 2026 Table 6, lung block, %T>MIC (combine) column, "E 0" = -5.994 (defined in Materials and methods as the maximum antibacterial effect)
    lec50 <- log(50.059)
    label("Log %T>MIC (combine) index producing 50% of the maximal antibacterial effect EC50 (percent)")  # Wei 2026 Table 6, lung block, %T>MIC (combine) column, EC 50 = 50.059
    lhill <- log(1.364)
    label("Log Hill coefficient N defining the steepness of the index-effect curve (unitless)")  # Wei 2026 Table 6, lung block, %T>MIC (combine) column, Hill's slope = 1.364

    # =================================================================
    # Residual error
    # =================================================================
    # Wei 2026 Table 6 reports the correlation coefficient (R^2 = 0.888)
    # and the AIC (17.319) of this fit and gives no residual standard
