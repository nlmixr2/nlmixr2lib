Gaohua_2015_lung_pbpk_pyrazinamide <- function() {
  description <- paste(
    "PBPK (permeability-limited lung, 25 ODEs; Simcyp V14 R1). Pyrazinamide",
  ...
  population <- list(
    ...
    notes       = paste(
      ...
      "simulation used 400 North European Caucasian subjects (age 25-55, 50%",
      "female) dosed 1000 mg once daily for 5 days, compared with Conte et",
      "al. 1999 (Supplementary Table S1).  These are virtual subjects",
  ...
  ini({
    ...
    # The compartment pH values (ELF 6.6, mass 6.69, blood 7.4; Methods "pH")
    # are not parameters of this file: pyrazinamide is neutral, so its
    # Henderson-Hasselbalch unionised fraction is 1 at every pH and no pH term
    # enters its equations.  They are parameters of the isoniazid and
    # ethambutol files, which do ionise.
    ...
    # ================= Compound file: pyrazinamide =================
    # Supplementary Table S2 unless noted.
    bp       <- fixed(0.63);  label("Blood:plasma concentration ratio")               # Table S2 "B/P"
    fup      <- fixed(0.9);   label("Unbound fraction in plasma")                     # Table S2 "fu p" (main plasma binding protein albumin)
    lka      <- fixed(log(0.9));  label("First-order absorption rate constant (1/h)") # Table S2 "ka" (0.9 1/h)
    lfdepot  <- fixed(log(0.8));  label("Fraction of the dose absorbed (unitless)")   # Table S2 "fa" (0.8)
    lvc      <- fixed(log(0.46)); label("Systemic distribution volume (L/kg)")        # Table S2 "V ss" (0.46 L/kg)
    lcl_renal   <- fixed(log(0.11)); label("Renal clearance (L/h)")                   # Table S2 "CL R" (0.11 L/h)
    # NOT PUBLISHED.  Table S2 reports the metabolic route as CLu,int = 0.5
    ...
    # The value below is back-solved from the AUC of the paper's OWN predicted
    # mean plasma profile (Supplementary Figure 2B, 1500 mg single dose,
    # digitised: AUCinf 345.7 mg*h/L, total CL 3.47 L/h, minus the published
    # renal 0.11 L/h).  Figure-derived, not a published number, and not a
    # validation target.
    lcl_nonren  <- fixed(log(3.36)); label("Non-renal clearance (L/h)")               # figure-derived: back-solved from Supplementary Figure 2B; see the note above

    peffLung <- fixed(0.0138e-4); label("Lung effective permeability (cm/s)")            # Table S2 "Lung effective permeability" (0.0138 x 10^-4 cm/s); Results 'Data sources': QSAR-predicted 13.8 x 10^-7 cm/s, fni = 1 at pH 7.4 (neutral)
    fuMass   <- fixed(0.985); label("Unbound fraction in the lung tissue mass")            # Table S2 "Lung fu mass"; Methods reports the same value for pyrazinamide
  ...
  model({
    ...
    # Pyrazinamide is neutral (Table S2 "Compound Type"), so it is fully
    # unionised at every compartment pH; the paper quotes fni = 1 at pH 7.4.
    fniFluid <- 1
    fniMass  <- 1
    fniBlood <- 1
