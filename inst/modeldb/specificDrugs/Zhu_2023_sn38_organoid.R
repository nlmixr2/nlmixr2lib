Zhu_2023_sn38_organoid <- function() {
  description <- paste(
    ...
    "term describing the 96 h concentration-viability relationship of SN-38,",
    "the active metabolite of irinotecan. A vehicle-control organoid and a",
    "treated organoid grow in parallel; the reported readout is cell viability,",
    "the ratio of treated to control organoid volume. This is the in vitro half",
    "of the Zhu 2023 in vitro-to-in vivo translation: the same Emax / EC50 /",
    "Hill parameters are carried into the human irinotecan minimal-PBPK PK/PD",
    "model, see modellib('Zhu_2023_irinotecan')."
  )
  reference <- paste(
  ...
  covariateData <- list(
    CONC_SN38_UM = list(
      description = paste(
        "Static SN-38 concentration applied to the culture medium of the 96 h",
        "tumour-organoid drug-sensitivity assay."
      ),
      units = "uM",
      type = "continuous",
      ...
      notes = paste(
        ...
        "grid; Figure 2B spans roughly 0.01-100 umol/L."
      ),
      source_name = "C"
    )
  )
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "for 7 organoid lines for SN-38 (patients 1-7). No demographic",
      "characteristics of the donor patients are reported."
    )
  )
  ...
  ini({
    ...
    # Table 2.
    lkgrow <- fixed(log(0.03)); label("Natural organoid growth rate kg (1/h)")            # Zhu 2023 Table 2, kg column (0.03 for patients 1-7)
    ...
    lemax <- log(0.046);      label("Maximum organoid killing rate Emax (1/h)")           # Zhu 2023 Table 4, Emax_SN = 0.046 (geometric mean of Table 2 patients 1-7)
    lec50 <- log(9.2);        label("SN-38 concentration at half-maximal killing EC50 (umol/L)")  # Zhu 2023 Table 4, EC50_SN = 9.2 umol/L (geometric mean of Table 2)
    lhill <- log(0.32);       label("Hill coefficient of the killing term (unitless)")    # Zhu 2023 Table 4, hill_SN = 0.32 (geometric mean of Table 2)
