Han_2025_midazolam_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, dynamic age-dependent; custom Phoenix WinNonlin 8.4",
    "implementation). Midazolam disposition across the entire human age",
    "range, from preterm neonates through adults to the oldest old. Fifteen",
    "perfusion-limited tissues plus a five-segment gut lumen / gut wall",
    "absorption cascade; CYP3A4 elimination in liver and in the duodenal,",
    "jejunal and ileal wall. The same structure is shared by the fentanyl,",
    ...
    "unbound fraction, intrinsic clearance and permeability differ. Organ",
    "volumes, organ blood flows, hepatic microsomal protein, CYP3A4",
    "activity and plasma protein binding are all driven by AGE, so one",
    "model covers paediatric, adult and geriatric populations. Midazolam is",
    "the only one of the four drugs the paper dosed orally, and the only one",
    "for which an effective permeability is reported. Han 2025 build their",
    "1000 virtual individuals by varying Peff, fu,b, CLint,l, CLint,i, Rb and",
    "PBSF over 0.67-1.5x the ideal value; that sweep is carried here as",
    "exponential IIV of variance 0.05415 on the same eight control-stream",
    "parameters (see the vignette Errata for the derivation)."
  )
  reference <- paste(
    ...
    "S6-S7 and Eq 18-22; validation targets from Table S12. The deposited",
  ...
  covariateData <- list(
    ...
    ROUTE_IV = list(
      ...
      notes              = paste(
        ...
        "the canonical's founding paper. Dose into `stomach` when",
        "ROUTE_IV = 0 and into `venous` when ROUTE_IV = 1."
      ),
  ...
  population <- list(
    ...
    n_subjects     = 1000L,
    n_studies      = 85L,
    age_range      = "gestational age 24 weeks (preterm neonates) to 91 years",
    weight_range   = "0.571 kg (preterm neonates) to 85 kg (adults)",
    disease_state  = paste(
      "Healthy volunteers plus surgical / intensive-care patients receiving",
      "midazolam for sedation or premedication. Han 2025 Table S8 lists 85",
      "midazolam data sets digitised from the published literature (23",
      "paediatric, 52 adult, 10 geriatric); Table S12 gives the observed and",
      "predicted AUC0-t and Cmax for each."
    ),
    dose_range     = paste(
      "0.075-1 mg/kg oral, 5-7.5 mg oral fixed dose, 5 ug/kg to 0.5 mg/kg",
      "intravenous, and a 259 ug/kg/h 148.8 h neonatal infusion"
    ),
    ...
    notes          = paste(
      ...
      "of variance 0.05415 on the eight corresponding control-stream",
      "parameters; rxode2::zeroRe() recovers the typical-value predictor.",
  ...
  ini({
    # ================= MIDAZOLAM drug-specific parameters =================
    # All from Han 2025 Table S1, midazolam column. Kt:p values were taken
    # from rat tissue:plasma ratios [Table S1 footnote, ref 22] assuming the
    # human tissue:free-plasma ratios equal the rat ones. NOTE: the deposited
    # Supplementary File S2 script carries every one of these Kt:p values
    # divided by 1.84 (e.g. lung 2.506 vs 4.61); the Table S1 values are the
    # ones that reproduce the paper's own predicted Cmax and AUC (see the
    # vignette's Errata section), so Table S1 is used here.
    kp_lung    <- fixed(4.61);    label("midazolam lung:plasma partition coefficient Kt:p (unitless)")      # Table S1, Lung
    kp_kidney  <- fixed(4.72);    label("midazolam kidney:plasma partition coefficient Kt:p (unitless)")    # Table S1, Kidney
    kp_heart   <- fixed(4.31);    label("midazolam heart:plasma partition coefficient Kt:p (unitless)")     # Table S1, Heart
    kp_liver   <- fixed(4.10);    label("midazolam liver:plasma partition coefficient Kt:p (unitless)")     # Table S1, Liver
    kp_muscle  <- fixed(1.33);    label("midazolam muscle:plasma partition coefficient Kt:p (unitless)")    # Table S1, Muscle
    kp_skin    <- fixed(1.44);    label("midazolam skin:plasma partition coefficient Kt:p (unitless)")      # Table S1, Skin
    kp_adipose <- fixed(9.23);    label("midazolam adipose:plasma partition coefficient Kt:p (unitless)")   # Table S1, Adipose
    kp_brain   <- fixed(3.38);    label("midazolam brain:plasma partition coefficient Kt:p (unitless)")     # Table S1, Brain
    kp_spleen  <- fixed(3.38);    label("midazolam spleen:plasma partition coefficient Kt:p (unitless)")    # Table S1, Spleen
    ...
    # The Phoenix control stream does: tvClintg1/2/3 = 34 / 134.6 / 78.51 at
    # CLint,l = 0.389. Those are exactly proportional to the three nmol
    # contents (34.0 / 0.389 / 9.7 = 9.0107 mg microsomal protein per nmol
    # CYP3A, reproducing the jejunal and ileal values to four figures), i.e.
    # 111 pmol CYP3A4 per mg hepatic microsomal protein. That derived constant
    # is what supplies the gut CLint of the three opioid siblings, whose
    # Table S1 rows report a hepatic CLint only.
    lcl_gw1    <- fixed(log(34.0));   label("midazolam duodenal wall CYP3A4 intrinsic clearance (log mL/min)")  # Phoenix control stream tvClintg1 = 34
    lcl_gw2    <- fixed(log(134.6));  label("midazolam jejunal wall CYP3A4 intrinsic clearance (log mL/min)")   # Phoenix control stream tvClintg2 = 134.6
    lcl_gw3    <- fixed(log(78.51));  label("midazolam ileal wall CYP3A4 intrinsic clearance (log mL/min)")     # Phoenix control stream tvClintg3 = 78.51
    ...
    # 0.05413, which reproduces both published CV% values in Table S12
    # (42.6 simulated vs 41.8 published, oral 7.5 mg; 27.3 vs 26.4,
    ...
    etalpeff      ~ fixed(0.05415);  label("IIV on effective intestinal permeability (variance, log scale)")
    ...
    propSd     <- fixed(0.10);    label("Proportional residual error placeholder, midazolam (fraction)")    # not reported by Han 2025
  })
  ...
  model({
    ...
    #    unbound fraction of Eq 17. Midazolam is albumin-bound (Han 2025
    #    reports no pKa/logP for it and gives an albumin ontogeny in
    #    Eq 13-14); the three opioid siblings are alpha1-acid-glycoprotein
    #    bound and set bind_alb <- 0.
    # =====================================================================
    bind_alb <- 1

    # Individual values of the eight random-effect parameters. At eta = 0
    ...
    # Intestinal-wall CYP3A4. The adult typical values come from the Phoenix
    # control stream (see the lcl_gw1/2/3 comment in ini()); Eq 11 scales them
    # down in paediatrics.
    ...
    # via 1e6. The deposited S2 script computes the same quantity as
    # "Cpre = Cvenous / 0.6".
    Cc <- c_venous / bpr * 1e6
    Cc ~ prop(propSd)
  })
