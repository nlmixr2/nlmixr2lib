Lee_2023_tylosin_Apleuropneumoniae_infected <- function() {
  description <- paste0(
    ...
    "against Actinobacillus pleuropneumoniae (challenge isolate BA2000013) in plasma drawn from co-infected pigs after a ",
    ...
    "PK/PD index AUC24h/MIC. Parameters from Lee 2023 Table 3, column 'A. pleuropneumoniae / Infected': ",
    "Emax = 7.34 log10 CFU/mL, EC50 = 1.33 h, E0 = 2.76 log10 CFU/mL, gamma = 1.86. ",
    "The parameterisation was confirmed numerically against the paper's own independently printed ",
    "rows: Emax - E0 reproduces exactly (4.58), and solving E = 0 and E = -3 returns ",
    "AUC24h/MIC = 1.013 and 2.666 h versus the published 1.03 and 2.54 h. ",
  ...
  ini({
    ...
    # column: A. pleuropneumoniae / Infected
    # =============================================================
    ...
    le0 <- log(2.76)
    label("Change in bacterial count in the drug-free control over 24 h E0 (log10 CFU/mL)")  # Lee 2023 Table 3, A. pleuropneumoniae / Infected, E 0 = 2.76 +/- SD
    lemax <- log(7.34)
    label("Maximum drug effect Emax, the span from control growth to maximum kill (log10 CFU/mL)")  # Lee 2023 Table 3, A. pleuropneumoniae / Infected, E max = 7.34 +/- SD
    lec50 <- log(1.33)
    label("AUC24h/MIC producing half the maximum effect EC50 (h)")  # Lee 2023 Table 3, A. pleuropneumoniae / Infected, EC 50 = 1.33 +/- SD
    lhill <- log(1.86)
    label("Hill coefficient gamma, steepness of the AUC24h/MIC effect curve (unitless)")  # Lee 2023 Table 3, A. pleuropneumoniae / Infected, gamma = 1.86 +/- SD
