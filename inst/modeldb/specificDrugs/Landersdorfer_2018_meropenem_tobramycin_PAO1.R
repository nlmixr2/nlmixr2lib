Landersdorfer_2018_meropenem_tobramycin_PAO1 <- function() {
  ...
  population <- list(
    ...
    notes            = paste(
      ...
      "Sibling model: Landersdorfer_2018_meropenem_tobramycin_PAOmutS."
    )
  )
  ...
  ini({
    ...
    log10prb_ri  <- -8.53; label("Log10 proportion of MEM-resistant/TOB-intermediate bacteria (seeds RI subpop)") # Table S3: Log10PRB_RI = -8.53 (SE 3.3%) for PAO1
    log10prb_ir  <- -3.08; label("Log10 proportion of MEM-intermediate/TOB-resistant bacteria (seeds IR subpop)") # Table S3: Log10PRB_IR = -3.08 (SE 3.6%) for PAO1

    # --- Killing by meropenem (Table S3) ---
    kmax_mem_ss  <- 3.77;  label("Maximum meropenem killing rate constant, double-susceptible (1/h)")    # Table S3: Kmax_MEM_SS = 3.77 (SE 18.6%) for PAO1
    kmax_mem_ri  <- 0.652; label("Maximum meropenem killing rate constant, MEM-resistant/TOB-int (1/h)") # Table S3: Kmax_MEM_RI = 0.652 (SE 14.9%) for PAO1
    kmax_mem_ir  <- 3.92;  label("Maximum meropenem killing rate constant, MEM-int/TOB-resistant (1/h)") # Table S3: Kmax_MEM_IR = 3.92 (SE 17.2%) for PAO1
    ...
    kc50_ir_mem  <- 61.5;  label("Meropenem KC50, MEM-intermediate/TOB-resistant (mg/L)")                # Table S3: KC50_MEM_IR = 61.5 (SE 15.5%) for PAO1
    ...
    kmax_tob_ss  <- 7.03;  label("Maximum tobramycin killing rate constant, double-susceptible (1/h)")     # Table S3: Kmax_TOB_SS = 7.03 (SE 16.7%) for PAO1
    kmax_tob_ri  <- 0.624; label("Maximum tobramycin killing rate constant, MEM-resistant/TOB-int (1/h)")  # Table S3: Kmax_TOB_RI = 0.624 (SE 26.8%) for PAO1
