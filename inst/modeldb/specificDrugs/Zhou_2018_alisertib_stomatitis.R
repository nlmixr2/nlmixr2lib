Zhou_2018_alisertib_stomatitis <- function() {
  description <- paste0(
    "Landmark logistic-regression exposure-SAFETY model for CTCAE grade 2 or ",
    "higher stomatitis as a function of the log of the time-averaged daily ",
    ...
    "n = 593). Stomatitis is the mucositis endpoint the Abstract refers to as ",
    "mucositis. There is no PK layer and no ODE: the exposure metric is ",
    ...
    "significant predictor at p < 0.0001. This is the STEEPEST of the three ",
    "alisertib exposure-safety relationships, with roughly twice the log-AUC ",
    "slope of the neutropenia and diarrhoea models. Both coefficients are ",
    ...
    "B, which is exactly logit-linear in log(AUC). The recovered model ",
    ...
    "Zhou_2018_alisertib_neutropenia and Zhou_2018_alisertib_diarrhea."
  )
  reference <- paste(
    ...
    "relationship in Figure 7 panel B; model-predicted incidences quoted in",
  ...
  covariateData <- list(
    AUC_ALIS = list(
      ...
      notes = paste(
        ...
        "Figure 7 panel B plots the observed range out to about 200",
  ...
  population <- list(
    ...
    notes = paste0(
      ...
      "and 3 of Zhou 2018. Grade 2 was chosen as the stomatitis cut-off, ",
      "versus grade 3 for neutropenia, on the differential clinical relevance ",
      "of each event to tolerability and quality of life."
    )
  )
  ...
  ini({
    ...
    # panel B is the only place the fitted relationship appears. Both
    # values below were recovered by digitising that curve at 400 dpi
    # from the publisher PDF (page 13) and regressing logit(p) on
    # log(AUC): the residual SD is 0.0122 on the logit scale over 440
    # extracted points spanning AUC 8.9 to 200 umol*h/L/day, which is
    ...
    # 0.073 / 0.125 / 0.064 at the three exposures Zhou 2018 quotes in
    # its Results text, against the printed 0.07 / 0.13 / 0.065 -- every
    ...
    logit_ref <- -6.485; label("Logit of the grade 2 or higher stomatitis probability at a time-averaged alisertib AUC of 1 umol*h/L/day (unitless logit)")  # digitised from Zhou 2018 Figure 7 panel B; not tabulated in the source
    e_auc_logit <- 1.434; label("Change in the log-odds of grade 2 or higher stomatitis per unit increase in the natural log of the time-averaged alisertib AUC (unitless logit)")  # digitised from Zhou 2018 Figure 7 panel B; not tabulated in the source
    ...
    addSd_prob_stomatitis_grade2 <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    logit_stomatitis_grade2 <- logit_ref + e_auc_logit * log(AUC_ALIS)
