Leber_2023_everolimus_sheep <- function() {
  ...
  ini({
    ...
    lcl     <- log(130);          label("Apparent oral clearance (L/h)")                   # Leber 2023 Supplementary Table 2 (final adsorption model)
    lvc     <- fixed(log(3100));  label("Apparent central volume of distribution (L)")     # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    lka     <- fixed(log(0.232)); label("First-order oral absorption rate constant (1/h)")   # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    ltlag   <- fixed(log(4.14));  label("Absorption lag time (h)")                         # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    ...
    lclmax_hemoadsorption <- log(3.23);    label("Maximum CytoSorb hemoadsorption clearance at zero saturation (L/h)")  # Leber 2023 Supplementary Table 2
    lamax_hemoadsorption  <- log(0.0163);  label("Maximum drug amount adsorbable by the CytoSorb cartridge (mg)")        # Leber 2023 Supplementary Table 2
