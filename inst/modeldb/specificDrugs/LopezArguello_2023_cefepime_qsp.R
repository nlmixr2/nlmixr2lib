LopezArguello_2023_cefepime_qsp <- function() {
  ...
  compartmentData <- list(
    ...
    nperi  = list(analyte = "Cefepime", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_FEP_MGL = list(
      description = "Static extracellular cefepime concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; cefepime MIC 1 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "cefepime at a static extracellular concentration of 2 mg/L (2x the MIC of 1 mg/L); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 1.66; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, cefepime: 1.66 (SE 39.4%)
    # The studied extracellular concentration enters as the CONC_FEP_MGL covariate
    # (Table 1: cefepime studied at 2 mg/L = 2x the MIC of 1 mg/L).
    ...
    k2_1a <- 396;        label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, cefepime PBP1a: 396
    k2_1b <- 68.7;       label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, cefepime PBP1b: 68.7
    k2_2 <- 51.7;        label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, cefepime PBP2: 51.7
    k2_3 <- fixed(1000); label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, cefepime PBP3: 1000, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_4 <- 252;         label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, cefepime PBP4: 252
    k2_5 <- 0.291;       label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, cefepime PBP5/6: 0.291
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_FEP_MGL
