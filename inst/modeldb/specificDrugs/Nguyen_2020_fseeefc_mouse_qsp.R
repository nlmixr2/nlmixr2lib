Nguyen_2020_fseeefc_mouse_qsp <- function() {
  description <- paste(
    "Preclinical (mouse). QSP. Myostatin / activin A / ActRIIB",
    "systems-pharmacology model for the follistatin-Fc fusion protein",
    "FS-EEE-Fc (Nguyen 2020), mouse parameterisation.",
    ...
    "pituitary.",
    "System parameters from Table S3 (mouse), compound parameters from",
    "Table S4, fitted to serum PK after a 1 mg/kg IV dose in CD1 mice",
    "(Figure S5); the muscle parameters were fitted to muscle-mass gain",
    "(from body weight) in C57 mice dosed IV at 1-50 mg/kg twice weekly",
    "for 4 weeks (Figure S6A). No FSH sub-model was parameterised for the",
    "mouse. Table S4 gives no absorption rate, so dosing is intravenous",
    "into central. Deterministic; ligand and receptor states start at the",
  ...
  population <- list(
    species = "mouse (CD1 for PK; C57 for muscle PD)",
    n_subjects = NA,
    n_studies = 2,
    weight_range = "reference body weight 0.028 kg (Table S4)",
    dose_range = "1 mg/kg IV single dose (PK); 1, 3, 10 and 50 mg/kg IV twice weekly for 4 weeks (muscle PD)",
    notes = paste(
      "In-house FS-EEE-Fc studies (Supplementary Methods; PD study published",
      "by Iskenderian 2018). Temporal body-weight change was the surrogate",
      "for muscle-mass change. Doses in mg (mg/kg x 0.028 kg)."
    )
  )

  ini({
    # ---- System physiology (Table S3 mouse) --------------------------------------
    vplasma <- fixed(0.00094); label("Plasma volume (L)") # Table S3 mouse: 0.00094
    vmuscle <- fixed(0.00147); label("Interstitial volume of muscle (L)") # Table S3 mouse: 0.00147
    vpituitary <- fixed(1.26e-07); label("Interstitial volume of anterior pituitary (L)") # Table S3 mouse: 1.26e-07
    lf_muscle <- fixed(8.61e-05); label("Lymph flow of muscle (L/h)") # Table S3 mouse: 8.61e-05
    lf_pituitary <- fixed(2.13e-08); label("Lymph flow of pituitary (L/h)") # Table S3 mouse: 2.13e-08
    lf_other <- fixed(0.000574); label("Lymph flow of other tissues (L/h)") # Table S3 mouse: 0.000574

    # ---- Myostatin (Table S3 mouse) ------------------------------------------------
    ...
    ksyn_ppmyo <- fixed(0.00483); label("Synthesis rate of myostatin-propeptide complex in muscle (nmol/h)") # Table S3 mouse: 0.00483 (unit printed 1/hr; enters the muscle balance as an amount rate)
    kcleave_ppmyo_muscle <- fixed(0.0356); label("Cleavage rate of myostatin-propeptide complex in muscle (1/h)") # Table S3 mouse: 0.0356
    kcleave_ppmyo_plasma <- fixed(0.0349); label("Cleavage rate of myostatin-propeptide complex in plasma (1/h)") # Table S3 mouse: 0.0349
    kdeg_ppmyo <- fixed(0.347); label("Degradation rate of myostatin-propeptide complex, all spaces (1/h)") # Table S3 mouse: 0.347 (footnote a: same in all spaces)
    kdeg_myo <- fixed(0.347); label("Degradation rate of mature myostatin, all spaces (1/h)") # Table S3 mouse: 0.347 (footnote a: same in all spaces)
    sigma_v_myo <- fixed(0.7); label("Vascular reflection coefficient of myostatin species (unitless)") # Table S3 mouse: 0.7 (footnote a: same in other tissues)
    sigma_is_myo <- fixed(0.2); label("Lymphatic reflection coefficient of myostatin species (unitless)") # Table S3 mouse: 0.2 (footnote a)
    kon_myo_actriib <- fixed(1.328); label("Myostatin-ActRIIB association rate constant (1/(nM*h))") # Table S3 mouse: 1.328 (Sako 2010)
    koff_myo_actriib <- fixed(0.124); label("Myostatin-ActRIIB dissociation rate constant (1/h)") # Table S3 mouse: 0.124 (Sako 2010)

    # ---- Activin A (Table S3 mouse) ------------------------------------------------
    ...
    ksyn_ppact_pituitary <- fixed(1.53e-07); label("Synthesis rate of activin A-propeptide complex in pituitary (nmol/h)") # Table S3 mouse: 1.53e-07
    ksyn_ppact_plasma <- fixed(4.09e-06); label("Synthesis rate of activin A-propeptide complex in plasma (nmol/h)") # Table S3 mouse: 4.09e-06
    ksyn_ppact_muscle <- fixed(0.000235); label("Synthesis rate of activin A-propeptide complex in muscle (nmol/h)") # Table S3 mouse: 0.000235
    kcleave_ppact <- fixed(3113); label("Cleavage rate of activin A-propeptide complex, all producing spaces (1/h)") # Table S3 mouse: 3113
    kdeg_ppact <- fixed(1.39); label("Degradation rate of activin A-propeptide complex, all spaces (1/h)") # Table S3 mouse: 1.39 (same in all spaces, as Table 1)
    kdeg_act <- fixed(2.08); label("Degradation rate of mature activin A, all spaces (1/h)") # Table S3 mouse: 2.08 (same in all spaces, as Table 1)
    sigma_v_act <- fixed(0.7); label("Vascular reflection coefficient of activin species (unitless)") # Table S3 mouse: 0.7 (footnote b: same in pituitary and other tissues)
    sigma_is_act <- fixed(0.2); label("Lymphatic reflection coefficient of activin species (unitless)") # Table S3 mouse: 0.2 (footnote b)
    kon_act_actriib <- fixed(14.9); label("Activin A-ActRIIB association rate constant (1/(nM*h))") # Table S3 mouse: 14.90 (Sako 2010)
    koff_act_actriib <- fixed(0.533); label("Activin A-ActRIIB dissociation rate constant (1/h)") # Table S3 mouse: 0.533 (Sako 2010)

    # ---- ActRIIB (Table S3 mouse) --------------------------------------------------
    bl_actriib <- fixed(0.138); label("Total ActRIIB concentration in muscle and in pituitary (nM)") # Table S3 mouse: 0.138 nM, printed once for all species

    # ---- FS-EEE-Fc compound parameters (Table S4 mouse) ------------
    mw_drug <- fixed(150000); label("Molecular weight of FS-EEE-Fc (g/mol)") # Table S4 mouse MW_Drug = 150000
    vother <- fixed(0.04); label("Volume of other tissues (L)") # Table S4 mouse V_other = 0.040 L
    kdeg_drug <- fixed(0.005); label("Degradation rate of free FS-EEE-Fc, all spaces (1/h)") # Table S4 mouse k_deg_drug = 0.005
    kdeg_md <- fixed(0.005); label("Degradation rate of FS-EEE-Fc-myostatin complex (1/h)") # Table S4 mouse k_deg_MD = 0.005 (assumed ~ k_deg_drug)
    kdeg_ad <- fixed(0.005); label("Degradation rate of FS-EEE-Fc-activin A complex (1/h)") # Table S4 mouse k_deg_AD = 0.005 (assumed ~ k_deg_drug)
    kon_drug_myo <- fixed(0.6); label("FS-EEE-Fc-myostatin association rate constant (1/(nM*h))") # Table S4 mouse kon_drug-Myo = 0.6 (footnote b)
    koff_drug_myo <- fixed(0.0045); label("FS-EEE-Fc-myostatin dissociation rate constant (1/h)") # Table S4 mouse koff_drug-Myo = 0.0045 (from in vitro Kd)
    kon_drug_act <- fixed(0.6); label("FS-EEE-Fc-activin A association rate constant (1/(nM*h))") # Table S4 mouse kon_drug-Act = 0.6 (footnote b)
    koff_drug_act <- fixed(0.0037); label("FS-EEE-Fc-activin A dissociation rate constant (1/h)") # Table S4 mouse koff_drug-Act = 0.0037 (from in vitro Kd)
    sigma_v_drug <- fixed(0.6); label("Vascular reflection coefficient of drug and drug complexes (unitless)") # Table S4 mouse sigma_m_V_drug = 0.6 (footnote a: same in pituitary and other tissues)
    sigma_is_drug <- fixed(0.2); label("Lymphatic reflection coefficient of drug and drug complexes (unitless)") # Table S4 mouse sigma_m_IS_drug = 0.2 (footnote h: same in pituitary and other tissues)

    # ---- Muscle pharmacodynamics (Table S4 mouse) -----------------------------
    vmax_muscle <- fixed(1.84); label("Maximal rate of muscle-mass increase (%/h)") # Table S4 mouse V_max_muscle = 1.84
    h_muscle <- fixed(0.788); label("Hill coefficient of the ActRIIB occupancy-muscle link (unitless)") # Table S4 mouse h_muscle = 0.788
    ro50_muscle <- fixed(1.0); label("Muscle ActRIIB occupancy giving half-maximal effect (%)") # Table S4 mouse RO_50_muscle = 1.0
    kdeg_muscle <- fixed(0.011); label("First-order loss rate of the muscle-mass change (1/h)") # Table S4 mouse k_deg_muscle = 0.011
  ...
  model({
    ...
    # starts (as the deposited human code does at the first dose).
