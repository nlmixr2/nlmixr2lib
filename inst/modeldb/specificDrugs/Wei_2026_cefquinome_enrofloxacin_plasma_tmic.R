Wei_2026_cefquinome_enrofloxacin_plasma_tmic <- function() {
  description <- paste0(
    ...
    "carries the fit driven by the %T>MIC (combine) index computed from plasma free drug PK data. ",
    ...
    "Wei 2026's Emax = 0.108 onto the canonical `e0` (effect at zero exposure) and Wei 2026's ",
    "E0 = -5.940 onto the canonical `emax` (maximal drug effect), and writes the algebraically ",
    "identical E = e0 - (e0 - emax) * Ce^N / (Ce^N + EC50^N). No value is altered by the remapping. ",
    "Parameters from Wei 2026 Table 6, plasma free drug / %T>MIC (combine) column: e0 = 0.108, emax = -5.940 ",
    "log10 CFU/mL, EC50 = 35.748, Hill N = 1.540 (R^2 = 0.953, AIC = 14.695). ",
    "The parameterisation was confirmed against a value Wei 2026 printed independently of Table 6: solving E = -3 returns an index of 37.061 against the published 3 log10 CFU/mL target of 37.062. This is the fit Wei 2026 selected as its primary result (highest R^2 and lowest AIC of the six), and the basis for its conclusion that cefquinome behaves as a time-dependent agent in this combination. ",
    ...
    "covariate FTMIC_CEFQ. That covariate carries the PK/PD index as an ALREADY-FORMED RATIO rather ",
    ...
    "typical-value simulation. Sibling models from the same paper: Wei_2026_cefquinome_enrofloxacin_plasma_cmaxmic, Wei_2026_cefquinome_enrofloxacin_plasma_aucmic, Wei_2026_cefquinome_enrofloxacin_lung_cmaxmic, Wei_2026_cefquinome_enrofloxacin_lung_aucmic and Wei_2026_cefquinome_enrofloxacin_lung_tmic."
  )
  reference <- paste(
    ...
    "PK/PD'. Parameter estimates: Table 6, plasma free drug / %T>MIC (combine) column.",
  ...
  units <- list(
    time = "h",
    dosing = "percent (cefquinome %T>MIC (combine) index, supplied as a covariate)",
    concentration = "log10 CFU/mL (observation)"
  )

  depends <- c("FTMIC_CEFQ")

  covariateData <- list(
    FTMIC_CEFQ = list(
      description        = "Cefquinome PK/PD index: the percentage of a 24 h dosing interval during which the free plasma cefquinome concentration exceeds MIC(combine), the MIC of cefquinome measured in the presence of the average free enrofloxacin plasma concentration achieved over 72 h",
      units              = "percent",
      type               = "continuous",
      ...
      source_name        = "%T>MIC (combine) (Wei 2026 Table 5, plasma free drug block; Materials and methods, 'Analysis of antibacterial effects and fitting of PK/PD')"
    )
  )
  ...
  ini({
    ...
    # matrix: plasma free drug   index: %T>MIC (combine)
    # =================================================================
    ...
    #   e0   <- Wei 2026's Emax = 0.108   (effect at zero exposure)
    #   emax <- Wei 2026's E0   = -5.940  (maximal drug effect)
    ...
    e0 <- 0.108
    label("Difference in lung K. pneumoniae load from the blank control at zero exposure (log10 CFU/mL)")  # Wei 2026 Table 6, plasma free drug block, %T>MIC (combine) column, "E max" = 0.108 (defined in Materials and methods as the blank-control change)
    emax <- -5.940
    label("Maximal difference in lung K. pneumoniae load from the blank control (log10 CFU/mL; negative = reduction)")  # Wei 2026 Table 6, plasma free drug block, %T>MIC (combine) column, "E 0" = -5.940 (defined in Materials and methods as the maximum antibacterial effect)
    lec50 <- log(35.748)
    label("Log %T>MIC (combine) index producing 50% of the maximal antibacterial effect EC50 (percent)")  # Wei 2026 Table 6, plasma free drug block, %T>MIC (combine) column, EC 50 = 35.748
    lhill <- log(1.540)
    label("Log Hill coefficient N defining the steepness of the index-effect curve (unitless)")  # Wei 2026 Table 6, plasma free drug block, %T>MIC (combine) column, Hill's slope = 1.540

    # =================================================================
    # Residual error
    # =================================================================
    # Wei 2026 Table 6 reports the correlation coefficient (R^2 = 0.953)
    # and the AIC (14.695) of this fit and gives no residual standard
