Zhou_2018_alisertib_diarrhea <- function() {
  description <- paste0(
    "Landmark logistic-regression exposure-SAFETY model for CTCAE grade 2 or ",
    "higher diarrhoea as a function of the log of the time-averaged daily AUC ",
    "of the Aurora A kinase inhibitor alisertib, in adult cancer patients ",
    "given 5 to 200 mg/day on the 7-days-on, 14-days-off schedule (Zhou 2018, ",
    "n = 594). There is no PK layer and no ODE: the exposure metric is ",
    ...
    "C, which is exactly logit-linear in log(AUC). The recovered model ",
    "reproduces all three probabilities the paper prints in its Results text ",
    "to within the printed rounding -- see the vignette. The endpoint is ",
    "named with the register's American spelling, diarrhea, although the ",
    "source publication spells it diarrhoea throughout. Companion models for ",
    "the other two mechanism-related antiproliferative toxicities are ",
    "Zhou_2018_alisertib_neutropenia and Zhou_2018_alisertib_stomatitis."
  )
  reference <- paste(
    ...
    "relationship in Figure 7 panel C; model-predicted incidences quoted in",
  ...
  units <- list(
    ...
    concentration = "prob_diarrhea_grade2 (probability of grade 2 or higher diarrhoea, 0-1; also logit_diarrhea_grade2)"
  )
  ...
  covariateData <- list(
    AUC_ALIS = list(
      ...
      notes = paste(
        ...
        "Figure 7 panel C plots the observed range out to about 200",
        "umol*h/L/day. Reference exposures quoted in Results: 15.63 (Western",
        "patients at 50 mg twice daily), 23.72 (East Asian patients at 50 mg",
        "twice daily) and 14.23 (East Asian patients at the 30 mg twice daily",
        "regional recommended phase 2 dose).",
  ...
  population <- list(
    ...
    notes = paste0(
      ...
      "and 3 of Zhou 2018. Grade 2 was chosen as the diarrhoea cut-off, versus ",
      "grade 3 for neutropenia, on the differential clinical relevance of each ",
      "event to tolerability and quality of life."
    )
  )
  ...
  ini({
    ...
    # panel C is the only place the fitted relationship appears. Both
    # values below were recovered by digitising that curve at 400 dpi
    # from the publisher PDF (page 13) and regressing logit(p) on
    # log(AUC): the residual SD is 0.0049 on the logit scale over 479
    # extracted points spanning AUC 2.4 to 123 umol*h/L/day, which is
    # what confirms the assumed logit-linear-in-log(AUC) form rather
    # than assuming it.
    #
    # The digitisation is self-validating. Without using any printed
    # number, the recovered coefficients predict probabilities of
    # 0.131 / 0.167 / 0.124 at the three exposures Zhou 2018 quotes in
    # its Results text, against the printed 0.13 / 0.17 / 0.12 -- every
    ...
    logit_ref <- -3.752; label("Logit of the grade 2 or higher diarrhoea probability at a time-averaged alisertib AUC of 1 umol*h/L/day (unitless logit)")  # digitised from Zhou 2018 Figure 7 panel C; not tabulated in the source
    e_auc_logit <- 0.677; label("Change in the log-odds of grade 2 or higher diarrhoea per unit increase in the natural log of the time-averaged alisertib AUC (unitless logit)")  # digitised from Zhou 2018 Figure 7 panel C; not tabulated in the source
    ...
    addSd_prob_diarrhea_grade2 <- fixed(0.001); label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    logit_diarrhea_grade2 <- logit_ref + e_auc_logit * log(AUC_ALIS)
    prob_diarrhea_grade2 <- expit(logit_diarrhea_grade2)

    prob_diarrhea_grade2 ~ add(addSd_prob_diarrhea_grade2)
  })
}
