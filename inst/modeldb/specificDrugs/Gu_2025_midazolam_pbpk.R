Gu_2025_midazolam_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). midazolam disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. midazolam is",
    "a short-acting benzodiazepine metabolism predominantly mediated by hepatic and intestinal CYP3A (Supplementary Section 2.8).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; midazolam parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "midazolam", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    ...
    lung           = list(analyte = "midazolam", units = "mg", specimen = "tissue", verified = TRUE),
    ...
    arterial       = list(analyte = "midazolam", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "midazolam", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "3 and 7.5 mg single oral",
    notes          = paste(
  ...
  ini({
    ...
    fu_p       <- fixed(0.031);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [3] Gertz 2011
    bpr        <- fixed(0.55);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp [20] Yang 2020
    fu_g       <- fixed(1);    label("Fraction unbound in the intestinal wall (unitless)")  # Table S2 fu,g assumed (footnote c)
    ...
    lcl_renal  <- fixed(log(0.00142));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 1.42 mL/min [35] Patel 1990
    lcl_liver  <- fixed(log(0.5468));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 546.8 mL/min [35] Patel 1990

    # ---- Absorption ----
    # Peff is tabulated, so Eq S15 (ka,i = 2 x Peff / ri) gives a distinct
    # absorption rate constant per intestinal segment.
    lpeff      <- fixed(log(0.04));  label("Effective permeability coefficient Peff,A-B (log cm/min)")  # Table S2 Peff,A-B 0.04 cm/min [20] Yang 2020

    # ---- Intestinal-wall intrinsic clearance (Supplementary Table S2) ----
    lclg_duodenum <- fixed(log(0.0268));  label("Duodenal wall intrinsic clearance CLGW1,int (log L/min)")  # Table S2 CLGW1,int 26.8 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)
    lclg_jejunum  <- fixed(log(0.1063));  label("Jejunal wall intrinsic clearance CLGW2,int (log L/min)")  # Table S2 CLGW2,int 106.3 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)
    lclg_ileum    <- fixed(log(0.062));  label("Ileal wall intrinsic clearance CLGW3,int (log L/min)")  # Table S2 CLGW3,int 62 mL/min; footnote d (scaled from hepatic metabolic parameters by intestinal:hepatic CYP3A content)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(3.89    ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(1.37    ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(1.27    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(3.79    ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(1.2     ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(1.52    ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(0.75    ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.86    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(1.31    ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(1.09    ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(1.52    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
