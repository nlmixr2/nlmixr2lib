Kubota_2018_naldemedine_gi_moderate_phase2b <- function() {
  description <- paste0(
    ...
    "prob = 1 / (1 + exp(-(a + b * AUCss))) with a = -2.75 and b = 0.0254. ",
    "The 95% CI of the slope includes zero. Kubota 2018 cautions more broadly that the phase 2b safety parameters could not be estimated properly because of the small sample size; prefer the phase 3 models for prediction. No SEVERE gastrointestinal event occurred in this study, so unlike the phase 3 analysis there is no severe-only companion model. ",
  ...
  reference <- paste(
    ...
    "Logistic-regression coefficients are Table V(b), rows '1107V9221 (Phase 2b)' / 'Moderate, Severe';",
  ...
  population <- list(
    ...
    regions = "Global",
    notes = paste(
      "The endpoint is the occurrence of a treatment-emergent adverse event in the gastrointestinal-disorders system organ class, dichotomised at the MODERATE-or-greater severity threshold. Observed frequencies (Table IV): placebo 2/61 (3.3%), 0.1 mg 1/9 (11.1%), 0.2 mg 3/9 (33.3%), 0.4 mg 1/10 (10.0%).",
  ...
  ini({
    # ==================================================================
    # Kubota 2018 Table V(b), rows '1107V9221 (Phase 2b)' / 'Moderate, Severe'. The model is the paper's Eq. (3):
    #
