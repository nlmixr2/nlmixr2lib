Lee_2023_tylosin_Apleuropneumoniae_healthy <- function() {
  description <- paste0(
    ...
    "against Actinobacillus pleuropneumoniae (challenge isolate BA2000013) in plasma drawn from healthy pigs after a ",
    ...
    "PK/PD index AUC24h/MIC. Parameters from Lee 2023 Table 3, column 'A. pleuropneumoniae / Healthy': ",
    "Emax = 7.19 log10 CFU/mL, EC50 = 1.21 h, E0 = 2.84 log10 CFU/mL, gamma = 2.76. ",
    "The parameterisation was confirmed numerically against the paper's own independently printed ",
    "rows: Emax - E0 reproduces exactly (4.35), and solving E = 0 and E = -3 returns ",
    "AUC24h/MIC = 1.037 and 2.057 h versus the published 0.98 and 1.97 h. ",
  ...
  compartmentData <- list(
    bact = list(analyte = "Actinobacillus pleuropneumoniae", units = "CFU/mL", specimen = "plasma", verified = TRUE)
  )
  ...
  population <- list(
    ...
    notes               = paste0(
      ...
      "non-compartmental PK for healthy pigs (Table 2): Cmax 5.79 ug/mL, Tmax 0.25 h, ",
      "T1/2 1.99 h, AUC 13.33 h*ug/mL, Cl/F 1416.19 mL/h/kg. The paper reports four independent ",
  ...
  ini({
    ...
    # column: A. pleuropneumoniae / Healthy
    # =============================================================
    ...
    le0 <- log(2.84)
    label("Change in bacterial count in the drug-free control over 24 h E0 (log10 CFU/mL)")  # Lee 2023 Table 3, A. pleuropneumoniae / Healthy, E 0 = 2.84 +/- SD
    lemax <- log(7.19)
    label("Maximum drug effect Emax, the span from control growth to maximum kill (log10 CFU/mL)")  # Lee 2023 Table 3, A. pleuropneumoniae / Healthy, E max = 7.19 +/- SD
    lec50 <- log(1.21)
    label("AUC24h/MIC producing half the maximum effect EC50 (h)")  # Lee 2023 Table 3, A. pleuropneumoniae / Healthy, EC 50 = 1.21 +/- SD
    lhill <- log(2.76)
    label("Hill coefficient gamma, steepness of the AUC24h/MIC effect curve (unitless)")  # Lee 2023 Table 3, A. pleuropneumoniae / Healthy, gamma = 2.76 +/- SD
