Kim_2016_piperacillin <- function() {
  description <- paste(
    "Two-compartment population PK model for piperacillin in 33 Korean adult",
    "inpatients with acute infections (Kim 2016), sampled at steady state after",
    "the fourth 1-h IV infusion of 2 g or 4 g piperacillin (with tazobactam)",
    "every 8 h. Zero-order IV input into the central compartment, first-order",
    "elimination, an additive non-renal-plus-renal clearance linear in",
    "Cockcroft-Gault creatinine clearance, and a linear body-weight effect on the",
    "central volume; the intercompartmental clearance and the peripheral volume",
    "were fixed.",
    sep = " "
  )
  reference <- paste(
    ...
    "Structural and covariate equations: Results section 2 ('the final model was",
    "as follows') and the Table 2 structural-model header rows.",
    "All parameter estimates: Table 2, piperacillin columns.",
  ...
  # samples assayed by LC-MS/MS) and Results section 2 (two-compartment
  ...
  covariateData <- list(
    CRCL = list(
      ...
      notes = paste(
        ...
        "TVCL (L/h) = 2.90 + 4.03 * CLcr/47 (Kim 2016 Results section 2 and",
        "Table 2 structural-model header). Adding CLcr on CL lowered the",
        "objective function value by 10.387 points. Cohort CLcr was",
    ...
    WT = list(
      ...
      notes = paste(
        ...
        "estimated): TVV1 (L) = 19.50 * weight/60 (Kim 2016 Results section 2",
        "and Table 2 structural-model header). The paper states that V1 was",
        "'adjusted for median body weight of 60 kg with a linear relationship'",
        "a priori, following its reference [16], rather than being selected by",
        "the covariate search. Cohort weight was 58.17 +/- 10.08 kg, range",
        "36.30-75.40 (Table 1)."
      ),
      source_name = "weight"
    )
  ...
  ini({
    # ===== Structural parameters (Kim 2016 Table 2, piperacillin columns) =====
    ...
    # Table 2 theta1 = 2.90 L/h (RSE 42.41%; bootstrap 2.80, 95% CI 0.01-5.26)
    lcl_renal <- log(4.03)
    label("Renal clearance arm at CLcr = 47 mL/min (L/h)")
    # Table 2 theta5 = 4.03 L/h (RSE 32.75%; bootstrap 3.99, 95% CI 1.82-7.89)

    lvc <- log(19.50)
    label("Central volume V1 at 60 kg (L)")
    # Table 2 theta2 = 19.50 L (RSE 14.82%; bootstrap 19.31, 95% CI 15.15-29.62)
    ...
    # Table 2 Q = 2.29 L/h (no RSE, no bootstrap: fixed)
    lvp <- fixed(log(3.76))
    label("Peripheral volume V2 (L)")
    # Table 2 V2 = 3.76 L (no RSE, no bootstrap: fixed)
    ...
    # decisive check is the residual row in the same block: sigma = 0.399 with
    # RSE 10.78% on roughly 33 x 4 = 132 observations, and sqrt(2/132) = 12.31%
    # is the floor a variance-scale RSE column could not beat. 10.78% is below
    # that floor, so the block is printed on the SD scale; a variance reading
    # would also make the proportional residual error 63.2%, implausible for the
    ...
    etalcl ~ 0.077841 # Table 2 'omega CL' = 0.279 (RSE 24.73%; bootstrap 0.265, 95% CI 0.146-0.613); 0.279^2
    etalvc ~ 0.032041 # Table 2 'omega V1' = 0.179 (RSE 84.36%; bootstrap 0.177, 95% CI 0.001-0.722); 0.179^2

    # ===== Residual error (Kim 2016 Table 2: proportional only) =====
    # Methods section 4: additive, proportional and combined error models were
    # tested; Results section 2: 'Residual variability was most effectively
    # explained by a proportional error model.'
    propSd <- 0.399
    label("Proportional residual error (fraction)")
    # Table 2 sigma, proportional error = 0.399 (RSE 10.78%; bootstrap 0.390, 95% CI 0.297-0.472)
  })

  model({
    # ----- Individual PK parameters (Kim 2016 Results section 2 equations) ----
    # TVCL (L/h) = 2.90 + 4.03 * CLcr/47, with a single log-normal eta on the
    ...
    # ----- Output -----
    # Total plasma piperacillin concentration: dose in mg, vc in L -> mg/L,
