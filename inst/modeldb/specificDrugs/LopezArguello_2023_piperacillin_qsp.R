LopezArguello_2023_piperacillin_qsp <- function() {
  ...
  compartmentData <- list(
    npbp1a = list(analyte = "piperacillin-bound PBP 1a", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp1b = list(analyte = "piperacillin-bound PBP 1b", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp2  = list(analyte = "piperacillin-bound PBP 2", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp3  = list(analyte = "piperacillin-bound PBP 3", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp4  = list(analyte = "piperacillin-bound PBP 4", units = NA_character_, specimen = "bile", verified = FALSE),
    npbp56 = list(analyte = "piperacillin-bound PBPs 5/6", units = NA_character_, specimen = "bile", verified = FALSE),
    nperi  = list(analyte = "free piperacillin in periplasm", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_PIP_MGL = list(
      description = "Static extracellular piperacillin concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
      reference_category = "n/a -- studied at 8 mg/L (2x the MIC of 4 mg/L)",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; piperacillin MIC 4 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "piperacillin at a static extracellular concentration of 8 mg/L (2x the MIC of 4 mg/L); no dosing events",
    notes = paste(
      ...
      "The 15 drugs were split into five estimation datasets of four drugs each with imipenem as the shared backbone drug; the Noise, Fini and residual-error parameters in this file are those of dataset 3 (IPM, ATM, PIP, CAR) (Table S2).",
  ...
  ini({
    ...
    rate_influx_scaled <- 0.301; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, piperacillin: 0.301 (SE 81.6%)
    # The studied extracellular concentration enters as the CONC_PIP_MGL covariate
    # (Table 1: piperacillin studied at 8 mg/L = 2x the MIC of 4 mg/L).
    ...
    k2_1a <- 456;         label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, piperacillin PBP1a: 456
    k2_1b <- 489;         label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, piperacillin PBP1b: 489
    k2_2 <- 628;          label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, piperacillin PBP2: 628
    k2_3 <- fixed(10000); label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, piperacillin PBP3: 10000, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_4 <- 642;          label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, piperacillin PBP4: 642
    k2_5 <- 6.82;         label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, piperacillin PBP5/6: 6.82
    ...
    # --- Non-suppressible background gel-band fraction (Table S2, dataset 3 (IPM, ATM, PIP, CAR)) ---
    noise_1a <- 0.0659; label("Background gel-band fraction not suppressible by drug, PBP1a (unitless)") # Table S2 NOISE_1a = 0.0659
    noise_1b <- 0.0775; label("Background gel-band fraction not suppressible by drug, PBP1b (unitless)") # Table S2 NOISE_1b = 0.0775
    noise_2 <- 0.157;   label("Background gel-band fraction not suppressible by drug, PBP2 (unitless)") # Table S2 NOISE_2 = 0.157
    noise_3 <- 0.168;   label("Background gel-band fraction not suppressible by drug, PBP3 (unitless)") # Table S2 NOISE_3 = 0.168
    noise_4 <- 0.0763;  label("Background gel-band fraction not suppressible by drug, PBP4 (unitless)") # Table S2 NOISE_4 = 0.0763
    noise_5 <- 0.274;   label("Background gel-band fraction not suppressible by drug, PBP5/6 (unitless)") # Table S2 NOISE_5 = 0.274

    # --- Random multiplier on the initial number of PBP molecules (Table S2, dataset 3 (IPM, ATM, PIP, CAR)) ---
    fini_1a <- 0.943; label("Multiplier on the initial number of PBP1a molecules (unitless)") # Table S2 Fini,1a = 0.943
    fini_1b <- 0.972; label("Multiplier on the initial number of PBP1b molecules (unitless)") # Table S2 Fini,1b = 0.972
    fini_2 <- 0.981;  label("Multiplier on the initial number of PBP2 molecules (unitless)") # Table S2 Fini,2 = 0.981
    fini_3 <- 0.95;   label("Multiplier on the initial number of PBP3 molecules (unitless)") # Table S2 Fini,3 = 0.95
    fini_4 <- 0.898;  label("Multiplier on the initial number of PBP4 molecules (unitless)") # Table S2 Fini,4 = 0.898
    fini_5 <- 1;      label("Multiplier on the initial number of PBP5/6 molecules (unitless)") # Table S2 Fini,5 = 1

    # --- Residual error: SD = SDin + SDsl * Y (Fig. S8 lines 111-116; Table S2, dataset 3 (IPM, ATM, PIP, CAR)) ---
    addSd_PBP1a <- 4.73; label("Additive residual SD, PBP1a (molecules/cell)") # Table S2 SDin1a = 4.73
    addSd_PBP1b <- 3.05; label("Additive residual SD, PBP1b (molecules/cell)") # Table S2 SDin1b = 3.05
    addSd_PBP2 <- 5.58;  label("Additive residual SD, PBP2 (molecules/cell)") # Table S2 SDin2 = 5.58
    addSd_PBP3 <- 9.68;  label("Additive residual SD, PBP3 (molecules/cell)") # Table S2 SDin3 = 9.68
    addSd_PBP4 <- 1.59;  label("Additive residual SD, PBP4 (molecules/cell)") # Table S2 SDin4 = 1.59
    addSd_PBP56 <- 156;  label("Additive residual SD, PBP5/6 (molecules/cell)") # Table S2 SDin5 = 156

    propSd_PBP1a <- 0.233; label("Proportional residual SD, PBP1a (fraction)") # Table S2 SDsl1a = 0.233
    propSd_PBP1b <- 0.187; label("Proportional residual SD, PBP1b (fraction)") # Table S2 SDsl1b = 0.187
    propSd_PBP2 <- 0.131;  label("Proportional residual SD, PBP2 (fraction)") # Table S2 SDsl2 = 0.131
    propSd_PBP3 <- 0.0436; label("Proportional residual SD, PBP3 (fraction)") # Table S2 SDsl3 = 0.0436
    propSd_PBP4 <- 0.38;   label("Proportional residual SD, PBP4 (fraction)") # Table S2 SDsl4 = 0.38
    propSd_PBP56 <- 0.106; label("Proportional residual SD, PBP5/6 (fraction)") # Table S2 SDsl5 = 0.106
  })
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_PIP_MGL
