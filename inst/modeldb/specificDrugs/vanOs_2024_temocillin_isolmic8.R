vanOs_2024_temocillin_isolmic8 <- function() {
  description <- paste0(
    "In vitro (Escherichia coli ISOL MIC8, temocillin MIC 8 mg/L). ",
  ...
  reference <- paste0(
    ...
    "Pharmacodynamic parameter estimates: main-text Table 3, column ISOL MIC8. ",
  ...
  compartmentData <- list(
    ...
    bact_susceptible = list(
      analyte = "Escherichia coli ISOL MIC8 (susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_less_susceptible = list(
      analyte = "Escherichia coli ISOL MIC8 (less susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_resistant = list(
      analyte = "Escherichia coli ISOL MIC8 (resistant subpopulation growing on agar with 32 mg/L temocillin)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    )
  )
  ...
  population <- list(
    species = "in vitro (Escherichia coli ISOL MIC8); pharmacokinetics from critically ill adults",
    n_subjects = 1L,
    n_studies = 1L,
    strain = paste0(
      "ISOL MIC8 (skin swab; ESBL genes: none reported; temocillin MIC 8 mg/L, determined in triplicate ",
    ...
    notes = paste0(
      ...
      "estimated separately for the two experimental systems (Bmax,STK 8.71 and Bmax,HFIM ",
      "10.1 log10 cfu/mL for this strain); log10bmax below carries the HFIM value because that ",
  ...
  ini({
    ...
    # ---- Bacterial growth ---------------------------------------------------
    kg_s   <- 1.51; label("Growth rate constant of the susceptible subpopulation (1/h)")      # Table 3 (ISOL MIC8): k_g,S 1.51, %RSE 7.9
    kg_ls  <- 0.720; label("Growth rate constant of the less susceptible subpopulation (1/h)") # Table 3 (ISOL MIC8): k_g,LS 0.720, %RSE 19.6
    kg_res <- 0.510; label("Growth rate constant of the resistant subpopulation (1/h)")       # Table 3 (ISOL MIC8): k_g,RES 0.510, %RSE 4.5

    log10bmax <- 10.1; label("Maximum bacterial density (log10 cfu/mL)") # Table 3 (ISOL MIC8): Bmax,HFIM 10.1, %RSE 1.3; the static time-kill value Bmax,STK was 8.71 (%RSE 0.9)
    ...
    emax_s  <- 2.86; label("Maximum drug effect rate constant, susceptible subpopulation (1/h)")             # Table 3 (ISOL MIC8): Emax,S 2.86, %RSE 4.5
    ec50_s  <- 3.75; label("Unbound concentration at half-maximal effect, susceptible subpopulation (mg/L)") # Table 3 (ISOL MIC8): EC50,S 3.75, %RSE 4.9
    hill_s  <- 3.51; label("Hill coefficient, susceptible subpopulation")                                 # Table 3 (ISOL MIC8): H,S 3.51, %RSE 19.0

    emax_ls <- 1.01; label("Maximum drug effect rate constant, less susceptible subpopulation (1/h)")             # Table 3 (ISOL MIC8): Emax,LS 1.01, %RSE 22.6
    ec50_ls <- 17.1; label("Unbound concentration at half-maximal effect, less susceptible subpopulation (mg/L)") # Table 3 (ISOL MIC8): EC50,LS 17.1, %RSE 10.6
    hill_ls <- 1.79; label("Hill coefficient, less susceptible subpopulation")                                 # Table 3 (ISOL MIC8): H,LS 1.79, %RSE 28.3

    klin_res <- 0.0137; label("Linear drug effect rate constant, resistant subpopulation (L/(mg*h))") # Table 3 (ISOL MIC8): k_lin,RES 0.0137, %RSE 5.7

    # ---- Initial bacterial population sizes ---------------------------------
    log10cfu0_s   <- 5.79; label("Fitted initial size of the susceptible subpopulation (log10 cfu/mL)")      # Table 3 (ISOL MIC8): cfu_t0,S 5.79, %RSE 1.5
    log10cfu0_ls  <- 2.09; label("Fitted initial size of the less susceptible subpopulation (log10 cfu/mL)") # Table 3 (ISOL MIC8): cfu_t0,LS 2.09, %RSE 5.6
    log10cfu0_res <- 0.156; label("Fitted initial size of the resistant subpopulation (log10 cfu/mL)")       # Table 3 (ISOL MIC8): cfu_t0,RES 0.156, %RSE 87.8
    ...
    # the log10 scale."
    addSd_CFUtotal <- 0.555; label("Additive residual SD on the log10 total bacterial count (log10 cfu/mL)")     # Table 3 (ISOL MIC8): RUV total 0.555, %RSE 4.5
    addSd_CFUres   <- 0.655; label("Additive residual SD on the log10 resistant bacterial count (log10 cfu/mL)") # Table 3 (ISOL MIC8): RUV RES 0.655, %RSE 8.6
  })
  ...
  model({
    ...
    # log10(1e6 / (10^5.79 + 10^2.09)) log10 cfu/mL.
