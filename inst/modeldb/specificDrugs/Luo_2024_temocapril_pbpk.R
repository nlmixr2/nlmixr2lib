Luo_2024_temocapril_pbpk <- function() {
  description <- paste(
    "PBPK (semi-mechanistic, custom WinNonlin 8.1 implementation). Joint",
    "temocapril + temocaprilat disposition in healthy adults and in liver",
    "cirrhosis (Child-Pugh A/B/C). Temocapril is an inactive ester prodrug",
    "hydrolysed by hepatic CES1 to the active diacid temocaprilat.",
    "Temocaprilat is eliminated by BOTH renal and biliary routes, with",
    "biliary clearance twice the renal clearance (Section 3.1.5); biliary",
    "excretion is MRP2-mediated and is scaled in cirrhosis by the Table 1",
    "MRP2 content ratio. The semi-PBPK circuit is stomach, three",
  ...
  reference <- paste(
    ...
    "and Eq 1-6; validation targets from Table 8.",
    sep = " "
  )
  ...
  ini({
    # ================= TEMOCAPRIL (parent prodrug) =================
    # Luo 2024 Table 2. Clearances converted mL/min -> L/min.
    lcl_int_h            <- fixed(log(5.3597));       label("temocapril hepatic CES1 intrinsic clearance (log L/min)")  # Table 2: CLint 5359.7 mL/min [ref 57]
    lcl_int_k            <- fixed(log(0.1102));       label("temocapril renal intrinsic clearance (log L/min)")  # Table 2: CLint,K 110.2 mL/min [ref 59]
    lvc                  <- fixed(log(15.398));       label("temocapril systemic apparent volume of distribution (log L)")  # Table 2: Vsys 15.398 L [ref 58] (footnote g)
    lka                  <- fixed(log(0.065));        label("temocapril gut-lumen absorption rate constant (log 1/min)")  # Table 2: ka 0.065 1/min [ref 58] (footnote g)
    kp_liver             <- fixed(2.82);              label("temocapril liver:plasma partition coefficient KL:P (unitless)")  # Table 2 KL:P (Rodgers-Rowland, footnote d)
    kp_gut               <- fixed(3.17);              label("temocapril gut:plasma partition coefficient KG:P (unitless)")  # Table 2 KG:P (Rodgers-Rowland, footnote d)
    kp_kidney            <- fixed(2.47);              label("temocapril kidney:plasma partition coefficient KK:P (unitless)")  # Table 2 KK:P (Rodgers-Rowland, footnote d)
    bpr                  <- fixed(1);                 label("temocapril blood:plasma concentration ratio Rb (unitless)")  # Table 2: Rb 1 (footnote e, assumed)
    fu_p                 <- fixed(0.3);               label("temocapril fraction unbound in plasma (unitless)")  # Table 2 fu,b 0.3 with Rb 1 -> fu,p 0.30 via Eq S8 [ref 60]; Section 2.3: no binding-protein data for temocapril so albumin binding assumed (pka < 7.4)

    # ================= TEMOCAPRILAT (active metabolite) =================
    # Luo 2024 Table 2, temocaprilat row.
    lcl_int_k_temat      <- fixed(log(0.94984));      label("temocaprilat renal intrinsic clearance (log L/min)")  # Table 2: CLint,K 949.84 mL/min [ref 64]; Section 3.1.5
    lvc_temat            <- fixed(log(58.535));       label("temocaprilat systemic apparent volume of distribution (log L)")  # Table 2: Vsys 58.535 L [ref 63] (footnote f)
    lk12_temat           <- fixed(log(0.00184));      label("temocaprilat central -> peripheral1 rate K12 (log 1/min)")  # Table 2: K12 0.00184 1/min [ref 63] (footnote f)
    lk21_temat           <- fixed(log(0.000078));     label("temocaprilat peripheral1 -> central rate K21 (log 1/min)")  # Table 2: K21 0.000078 1/min [ref 63] (footnote f)
    kp_liver_temat       <- fixed(0.289);             label("temocaprilat liver:plasma partition coefficient KL:P (unitless)")  # Table 2 KL:P (Rodgers-Rowland, footnote d)
    kp_gut_temat         <- fixed(0.322);             label("temocaprilat gut:plasma partition coefficient KG:P (unitless)")  # Table 2 KG:P (Rodgers-Rowland, footnote d)
    kp_kidney_temat      <- fixed(0.251);             label("temocaprilat kidney:plasma partition coefficient KK:P (unitless)")  # Table 2 KK:P (Rodgers-Rowland, footnote d)
    bpr_temat            <- fixed(1);                 label("temocaprilat blood:plasma concentration ratio Rb (unitless)")  # Table 2: Rb 1 (footnote e, assumed)
    fu_p_temat           <- fixed(0.025);             label("temocaprilat fraction unbound in plasma (unitless)")  # Table 2 fu,b 0.025 with Rb 1 -> fu,p 0.025 via Eq S8 [ref 60]
    lcl_int_bile_temat   <- fixed(log(1.89968));      label("temocaprilat biliary intrinsic clearance (log L/min)")  # Table 2: 1899.68 mL/min [ref 65] (footnote b, bile intrinsic clearance of temocaprilat); Section 3.1.5 CLbile,m = 2.0 x CLint,K
    ...
    propSd               <- fixed(0.10);  label("Proportional residual error placeholder, temocapril (fraction)")  # not reported by Luo 2024
    propSd_temat         <- fixed(0.10);  label("Proportional residual error placeholder, temocaprilat (fraction)")  # not reported by Luo 2024
  })
  ...
  model({
    ...
    r_mrp2  <- cp_0 * 1 + cp_lc * 0.54                                          # Table 1 MRP2 content ratio 1 -> 0.54
    ...
    #    temocapril binds mainly to albumin (Section 2.3).
    # ============================================================
    ...
    fu_p_i_temat  <- 1 / (1 + (1 - fu_p_temat) * r_alb / fu_p_temat)
    fu_b_i_temat  <- fu_p_i_temat / bpr_temat
    v_sys_temat   <- exp(lvc_temat) * (fu_p_i_temat / fu_p_temat)
    cl_int_k_temat <- exp(lcl_int_k_temat) * r_gfr
    cl_int_bile_temat <- exp(lcl_int_bile_temat) * r_mrp2   # MRP2-mediated biliary route (Section 3.1.5)
    k12_temat <- exp(lk12_temat)
    k21_temat <- exp(lk21_temat)
    ...
    c_wall_duo_temat <- wall_duodenum_temat / v_duo
    c_wall_jej_temat <- wall_jejunum_temat  / v_jej
    c_wall_ile_temat <- wall_ileum_temat    / v_ile
    c_pv_temat       <- portal_vein_temat   / v_pv
    c_liver_temat    <- liver_temat         / v_liver
    c_kid_temat      <- kidney_temat        / v_kid
    c_sys_temat      <- central_temat       / v_sys_temat
    eff_wall_duo_temat <- c_wall_duo_temat * bpr_temat / kp_gut_temat
    eff_wall_jej_temat <- c_wall_jej_temat * bpr_temat / kp_gut_temat
    eff_wall_ile_temat <- c_wall_ile_temat * bpr_temat / kp_gut_temat
    eff_liver_temat    <- c_liver_temat    * bpr_temat / kp_liver_temat
    eff_kid_temat      <- c_kid_temat      * bpr_temat / kp_kidney_temat
    ...
    #    temocaprilat: S5/S6 without absorption, S13 liver, S14 kidney, S16-S17 (2-cmt).
    # ============================================================
    ...
    # ---- temocaprilat: no lumen states (it is not absorbed from the gut lumen) ----
    d/dt(wall_duodenum_temat) <- q_duo * c_sys_temat - q_duo * eff_wall_duo_temat
    d/dt(wall_jejunum_temat)  <- q_jej * c_sys_temat - q_jej * eff_wall_jej_temat
    d/dt(wall_ileum_temat)    <- q_ile * c_sys_temat - q_ile * eff_wall_ile_temat

    d/dt(portal_vein_temat) <- q_duo * eff_wall_duo_temat + q_jej * eff_wall_jej_temat +
                            q_ile * eff_wall_ile_temat - q_pv * c_pv_temat

    d/dt(liver_temat) <- q_pv * c_pv_temat + q_ha * c_sys_temat + hydrolysis -
                      q_liver * eff_liver_temat
                      - cl_int_bile_temat * fu_b_i_temat * eff_liver_temat

    d/dt(kidney_temat) <- q_kid * c_sys_temat -
                       (q_kid + fu_b_i_temat * cl_int_k_temat) * eff_kid_temat

    d/dt(central_temat) <- q_liver * eff_liver_temat + q_kid * eff_kid_temat -
                        (q_ha + q_kid) * c_sys_temat - q_gut * c_sys_temat
                        + k21_temat * peripheral1_temat - k12_temat * central_temat
    d/dt(peripheral1_temat) <- k12_temat * central_temat - k21_temat * peripheral1_temat

    # Table 2 also reports an oral bioavailability F = 0.65 for this drug
    # (Table 2: F 0.65 [ref 61]).
    ...
    #    mg/L = ug/mL, matching the AUC unit (ug*h/mL) of Table 8.
    # ============================================================
    Cc <- c_sys
    Cc_temat <- c_sys_temat
