Solana_2014_omeprazole <- function() {
  ...
  population <- list(
    ...
    notes = "ABSTRACT-ONLY EXTRACTION: the full-text Therapeutic Drug Monitoring article (DOI 10.1097/FTD.0000000000000033) was not available when this model was built; all structural and parameter information was taken from the PubMed abstract (PMID 24365987). The numeric covariate ranges (per-patient age, weight, sex distribution) are not enumerated in the abstract and are recorded here as NA / narrative; they are not load-bearing for simulation because the only covariate used by the model is body weight via allometric scaling. The infusion duration is not stated in the abstract and is assumed to be a short IV bolus-like infusion in the vignette simulation; users with the full text can adjust the infusion rate accordingly."
  )
