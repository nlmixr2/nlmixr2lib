Granda_2024_kynurenicacid_pbpk <- function() {
  description <- paste(
    "PBPK (mechanistic kidney, 35 states).",
    "Individualised prediction of kynurenic acid renal clearance from",
    ...
    "The biomarker leg of the workflow: for each subject the unbound",
    "intrinsic secretory clearance is fitted so that the model",
    "reproduces that subject's MEASURED kynurenic acid renal clearance",
    "(Table 3). The fitted value is then carried into the tenofovir and",
    "oseltamivir carboxylate models via a drug-specific scalar.",
  ...
  ini({
    ...
    fup  <- fixed(0.07)
    label("Unbound fraction in plasma")  # Granda 2024 Table 2 (footnote a: literature average across binding methodologies; Methods fixes it at 7% for all subjects)
    bp   <- fixed(1)
    label("Blood-to-plasma concentration ratio")  # Granda 2024 Table 2 (footnote d)
    papp <- fixed(1)
    label("Apparent passive permeability (10^-6 cm/s)")  # Granda 2024 Table 2 (footnote d)
    secscalar <- fixed(1)
    label("Biomarker-to-drug secretory-clearance scalar (unitless)")  # Granda 2024 no scalar: kynurenic acid IS the biomarker whose secretory clearance is being estimated, so the biomarker-to-drug scalar is unity by construction
