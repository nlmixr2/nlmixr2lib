Chen_2025_hemoporfin_patient_rating <- function() {
  description <- paste0(
    "Binomial logistic exposure-response model for the probability that the ",
    "patient rates the clinical outcome of hemoporfin photodynamic therapy ",
    "as 'good' or 'excellent' 8 weeks after treatment, in 24 Chinese ",
    "pediatric patients aged 7-14 years with port-wine stain (Chen 2025, ",
    "NCT03125057). The probability is ",
    "expit(-12.9 + 0.981 * AUC_HEMO) where AUC_HEMO is the individual ",
    ...
    "(slope p = 0.0148), and it has the steepest exposure slope of the ",
    "five: each 1 h*ug/mL increase in AUC(0-30min) multiplies the odds of a ",
    "high patient rating by exp(0.981) = 2.67. There is no PK layer and no ",
    "ODE -- exposure enters as a covariate column. No between-subject ",
    "random effect and no residual error are estimated (Bernoulli ",
    "likelihood). The 30-minute exposure window is not arbitrary: light ",
    "irradiation is applied 10-30 minutes after the start of the 20-minute ",
    "infusion, so AUC(0-30min) is the drug exposure actually available for ",
    "photoactivation. Companion models: Chen_2025_hemoporfin (population ",
    "PK) and four sibling endpoints in the Chen_2025_hemoporfin_* family."
  )
  reference <- paste(
  ...
  ini({
    # ==================================================================
    # Chen 2025 Table S1 (Data S1), row block "Probability of patients
    # rating the clinical outcome as 'good' or 'excellent'". Model form
    # is Equation 3:
    ...
    # ----- Logit intercept -----
    logit_ref <- -12.9 ; label("Logit of the probability of a high patient rating at AUC_HEMO = 0 h*ug/mL (unitless logit)")  # Table S1, beta0 = -12.9 (95% CI -25.6 to -4.31, p = 0.0130)

    # ----- Exposure effect on the logit -----
    e_auc_hemo_logit <- 0.981 ; label("Log-odds of a high patient rating per 1 h*ug/mL increase in hemoporfin AUC(0-30min) (mL/(ug*h))")  # Table S1, beta1 = 0.981 mL*ug^-1*h^-1 (95% CI 0.308 to 1.95, p = 0.0148). Results 3.5 states the corresponding odds ratio as exp(0.981) = 2.67.
    ...
    addSd_prob_patient_rating <- fixed(0.001) ; label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    # ----- Linear predictor (Chen 2025 Equation 3, Table S1) -----
    logit_patient_rating <- logit_ref + e_auc_hemo_logit * AUC_HEMO

    prob_patient_rating <- expit(logit_patient_rating)

    # ----- Observation -----
    prob_patient_rating ~ add(addSd_prob_patient_rating)
  })
}
