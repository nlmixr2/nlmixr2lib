Yamamoto_2017_acetaminophen_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Acetaminophen disposition in plasma and",
    ...
    "pH-dependent factors. The plasma side is the empirical two-compartment",
    ...
    "CNS data, so the CNS profiles are genuine predictions. Carries an",
    "enterohepatic-recirculation compartment: the paper's 'Fraction'",
    "parameter (Table 2, 0.693) is the fraction of eliminated drug returned",
    "to plasma, switched on 120 min after the dose."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "acetaminophen", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "acetaminophen", units = "ng", specimen = "tissue", verified = TRUE),
    gallbladder = list(analyte = "acetaminophen", units = "ng", specimen = "bile", verified = TRUE),
    brain_vascular = list(analyte = "acetaminophen", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "acetaminophen", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "acetaminophen", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "acetaminophen", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "acetaminophen", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "acetaminophen", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "acetaminophen", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "acetaminophen", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "15 mg/kg (10 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 16 animals, 15 mg/kg (10 min infusion), data",
      "from reference 24 (Westerhout 2012). Plasma and brain microdialysis",
      "(brain extracellular fluid and CSF) samples were collected after",
      "femoral-vein administration; see Supplementary Material S2 for surgery",
      "and bioanalysis. Concentrations in the CNS compartments are unbound",
      "concentrations."
    )
  )
  ...
  ini({
    ...
    lcl <- log(15.8); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(49.5); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(33.8); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(363); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1

    # Enterohepatic recirculation (Table 2 'Fraction' row). The control
    # stream S3 gates it on time: FRAC is 0 before 120 min and THETA(14)
    # after, so the recycled peak appears only in the terminal phase.
    f_ehc <- fixed(0.693); label("Fraction of eliminated drug reabsorbed by enterohepatic circulation (unitless)")  # Table 2 Fraction
    t_ehc <- fixed(120); label("Time at which enterohepatic recirculation starts (min)")  # control stream S3 $PK
    ...
    etalvp ~ 0.268  # Table 2 CV 51.8 percent
    ...
    propSd <- 0.237; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 0.5 and molecular weight = 151;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(1.1e-4); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(4.6e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(12); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(29); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(32); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    PHF1 <- fixed(1); label("pH-dependent factor, brain extracellular fluid to microvasculature (unitless)")  # Table 5 PHF1
    PHF2 <- fixed(1); label("pH-dependent factor, lateral-ventricle CSF to microvasculature (unitless)")  # Table 5 PHF2
    PHF3 <- fixed(1); label("pH-dependent factor, third + fourth ventricle CSF to microvasculature (unitless)")  # Table 5 PHF3
    PHF4 <- fixed(1); label("pH-dependent factor, brain extracellular to intracellular fluid (unitless)")  # Table 5 PHF4
    PHF5 <- fixed(1); label("pH-dependent factor, brain intracellular to extracellular fluid (unitless)")  # Table 5 PHF5
    PHF6 <- fixed(1); label("pH-dependent factor, brain intracellular fluid to lysosome (unitless)")  # Table 5 PHF6
    PHF7 <- fixed(1); label("pH-dependent factor, lysosome to brain intracellular fluid (unitless)")  # Table 5 PHF7
    ...
    BF <- fixed(1.1); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
  ...
  model({
    ...
    # Enterohepatic recirculation is switched on t_ehc minutes after
    # the dose (control stream S3 $PK).
    if (t < t_ehc) {
      frac_ehc <- 0
    } else {
      frac_ehc <- f_ehc
    ...
    d/dt(central) <- -(cl / vc) * central -
      ...
      (cl / vc) * frac_ehc * gallbladder
    d/dt(peripheral1) <- (q / vc) * central - (q / vp) * peripheral1
    d/dt(gallbladder) <- (cl / vc) * central -
      (cl / vc) * frac_ehc * gallbladder -
      (cl / vc) * (1 - frac_ehc) * gallbladder
