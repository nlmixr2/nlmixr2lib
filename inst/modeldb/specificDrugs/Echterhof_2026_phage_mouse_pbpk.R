Echterhof_2026_phage_mouse_pbpk <- function() {
  description <- paste(
    "Preclinical (mouse, Swiss-Webster CD-1, male, 5-7 weeks, 22-30 g).",
    "PBPK (whole-body, permeability-limited, Monolix 2024R1 SAEM, naive-pooled)",
    "for the biodistribution of three intravenously administered lytic",
    "Pseudomonas aeruginosa bacteriophages (Luz24 63 nm, PAML-31-1 227 nm,",
    "OMKO1 400 nm) labelled with 125I-Sulfo-SHPP. This is the species the model",
    "was FITTED to; the rat and human siblings",
    "(modellib('Echterhof_2026_phage_rat_pbpk'),",
    "modellib('Echterhof_2026_phage_human_pbpk')) reuse these estimates with",
    "species anatomy substituted. Twelve organs (lung, muscle, bone, liver,",
  ...
  covariateData <- list(
    WT = list(
      ...
      notes              = paste(
        ...
        "Table S1 gives per-phage mean mouse weights of 27.2 g (Luz24),",
        "34.5 g (OMKO1) and 34.4 g (PAML-31-1); Methods 'In vivo studies' gives",
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "1, 5, 15 and 30 min. Because concentration data were a single TERMINAL",
      "endpoint per animal, inter-animal variability was not estimable and the",
      "fit was naive-pooled with no random effects -- hence no IIV is encoded in",
      "this file (Methods 'Pharmacokinetic analysis'). Tissues were washed in",
      "PBS, weighed and counted for 125I; results expressed as %ID/g, except",
      "stomach contents which were modelled as %ID."
    )
  )

  ini({
    # ==================================================================
    # Mouse anatomy and physiology -- Supplemental Table S3, "Mouse" column.
    ...
    CO_C <- fixed(16.5) ; label("Cardiac output coefficient (L/h/kg^0.75)")                      # Table S3 CO, Mouse
    ...
    fq_brain            <- fixed(0.033)    ; label("Fraction of cardiac output to brain (unitless)")            # Table S3 F Q_Brn, Mouse
    fq_liver            <- fixed(0.021)    ; label("Fraction of cardiac output to liver, hepatic artery (unitless)")  # Table S3 F Q_Lvr, Mouse
    fq_spleen           <- fixed(0.011)    ; label("Fraction of cardiac output to spleen (unitless)")           # Table S3 F Q_Spn, Mouse
    fq_small_intestine  <- fixed(0.105)    ; label("Fraction of cardiac output to small intestine (unitless)")  # Table S3 F Q_SI, Mouse
    fq_large_intestine  <- fixed(0.0175)   ; label("Fraction of cardiac output to large intestine (unitless)")  # Table S3 F Q_LI, Mouse
    fq_kidney           <- fixed(0.091)    ; label("Fraction of cardiac output to kidney (unitless)")           # Table S3 F Q_Kid, Mouse
    fq_muscle           <- fixed(0.159)    ; label("Fraction of cardiac output to muscle (unitless)")           # Table S3 F Q_Msc, Mouse
    fq_skin             <- fixed(0.058)    ; label("Fraction of cardiac output to skin (unitless)")             # Table S3 F Q_Ski, Mouse
    fq_bone             <- fixed(0.0267)   ; label("Fraction of cardiac output to bone (unitless)")             # Table S3 F Q_Bon, Mouse
    fq_stomach          <- fixed(0.017625) ; label("Fraction of cardiac output to stomach (unitless)")          # Table S3 F Q_Sto, Mouse

    # Organ volume as a fraction of body weight (unitless). Carcass closes the
    # balance against 1, so no F_V_Crs row is needed.
    fv_lung             <- fixed(0.0073)   ; label("Lung volume as a fraction of body weight (unitless)")            # Table S3 F V_Lun, Mouse
    fv_brain            <- fixed(0.0165)   ; label("Brain volume as a fraction of body weight (unitless)")           # Table S3 F V_Brn, Mouse
    fv_liver            <- fixed(0.0549)   ; label("Liver volume as a fraction of body weight (unitless)")           # Table S3 F V_Lvr, Mouse
    fv_spleen           <- fixed(0.005)    ; label("Spleen volume as a fraction of body weight (unitless)")          # Table S3 F V_Spn, Mouse (row captioned "Fraction of BW as Lung" -- a copy-paste label error; the row is spleen, see Errata)
    fv_small_intestine  <- fixed(0.0253)   ; label("Small-intestine volume as a fraction of body weight (unitless)") # Table S3 F V_SI, Mouse
    fv_large_intestine  <- fixed(0.0109)   ; label("Large-intestine volume as a fraction of body weight (unitless)") # Table S3 F V_LI, Mouse
    fv_kidney           <- fixed(0.0167)   ; label("Kidney volume as a fraction of body weight (unitless)")          # Table S3 F V_Kid, Mouse
    fv_muscle           <- fixed(0.384)    ; label("Muscle volume as a fraction of body weight (unitless)")          # Table S3 F V_Msc, Mouse
    fv_skin             <- fixed(0.1653)   ; label("Skin volume as a fraction of body weight (unitless)")            # Table S3 F V_Ski, Mouse
    fv_bone             <- fixed(0.03)     ; label("Bone volume as a fraction of body weight (unitless)")            # Table S3 F V_Mar, Mouse
    fv_stomach          <- fixed(0.006)    ; label("Stomach volume as a fraction of body weight (unitless)")         # Table S3 Fr_V_Sto, Mouse
    fv_blood            <- fixed(0.029029) ; label("Blood volume as a fraction of body weight (unitless)")           # Table S3 F V_Pla, Mouse

    # Vascular (capillary blood) fraction of each organ's volume (unitless).
    bv_lung             <- fixed(0.5)   ; label("Vascular fraction of lung volume (unitless)")             # Table S3 F V_Lun_Bld, Mouse
    bv_brain            <- fixed(0.03)  ; label("Vascular fraction of brain volume (unitless)")            # Table S3 F V_Brn_Bld, Mouse
    bv_liver            <- fixed(0.31)  ; label("Vascular fraction of liver volume (unitless)")            # Table S3 F V_Lvr_Bld, Mouse
    bv_spleen           <- fixed(0.17)  ; label("Vascular fraction of spleen volume (unitless)")           # Table S3 F V_Spn_Bld, Mouse
    bv_small_intestine  <- fixed(0.24)  ; label("Vascular fraction of small-intestine volume (unitless)")  # Table S3 F V_SI_Bld, Mouse
    bv_large_intestine  <- fixed(0.24)  ; label("Vascular fraction of large-intestine volume (unitless)")  # Table S3 F V_LI_Bld, Mouse
    bv_kidney           <- fixed(0.24)  ; label("Vascular fraction of kidney volume (unitless)")           # Table S3 F V_Kid_Bld, Mouse
    bv_muscle           <- fixed(0.04)  ; label("Vascular fraction of muscle volume (unitless)")           # Table S3 F V_Msc_Bld, Mouse
    bv_skin             <- fixed(0.03)  ; label("Vascular fraction of skin volume (unitless)")             # Table S3 F V_Ski_Bld, Mouse
    bv_remainder        <- fixed(0.04)  ; label("Vascular fraction of carcass volume (unitless)")          # Table S3 F V_Crs_Bld, Mouse
    bv_bone             <- fixed(0.1)   ; label("Vascular fraction of bone volume (unitless)")             # Table S3 F V_Bon_Bld, Mouse
    bv_stomach          <- fixed(0.24)  ; label("Vascular fraction of stomach volume (unitless)")          # Table S3 F V_Sto_Bld, Mouse

    # Tissue-resident phagocyte density (cells/g). Sets each organ's MPS
    # carrying capacity.
    m_blood             <- fixed(1.8e5)  ; label("Phagocyte density in blood (cells/g)")             # Table S3 M Pla, Mouse
    m_lung              <- fixed(1.71e6) ; label("Phagocyte density in lung (cells/g)")              # Table S3 M Lun, Mouse
    ...
    m_liver             <- fixed(1.71e6) ; label("Phagocyte density in liver (cells/g)")             # Table S3 M Lvr, Mouse
    m_spleen            <- fixed(1.71e6) ; label("Phagocyte density in spleen (cells/g)")            # Table S3 M Spn, Mouse
    m_small_intestine   <- fixed(1.8e5)  ; label("Phagocyte density in small intestine (cells/g)")   # Table S3 M SI, Mouse
    m_large_intestine   <- fixed(1.8e5)  ; label("Phagocyte density in large intestine (cells/g)")   # Table S3 M LI, Mouse
    m_kidney            <- fixed(1.8e5)  ; label("Phagocyte density in kidney (cells/g)")            # Table S3 M Kid, Mouse
    m_remainder         <- fixed(1.8e5)  ; label("Phagocyte density in carcass (cells/g)")           # Table S3 M Crs, Mouse
    m_muscle            <- fixed(1.8e5)  ; label("Phagocyte density in muscle (cells/g)")            # Table S3 M Msc, Mouse
    m_skin              <- fixed(1.8e5)  ; label("Phagocyte density in skin (cells/g)")              # Table S3 M Ski, Mouse
    m_bone              <- fixed(1.8e5)  ; label("Phagocyte density in bone (cells/g)")              # Table S3 M Bon, Mouse
    m_stomach           <- fixed(1.8e5)  ; label("Phagocyte density in stomach (cells/g)")           # Table S3 M Sto, Mouse
