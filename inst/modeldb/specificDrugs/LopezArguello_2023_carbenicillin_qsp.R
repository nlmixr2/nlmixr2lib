LopezArguello_2023_carbenicillin_qsp <- function() {
  ...
  compartmentData <- list(
    npbp1a = list(analyte = "carbenicillin-PBP 1a complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp1b = list(analyte = "carbenicillin-PBP 1b complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp2  = list(analyte = "carbenicillin-PBP 2 complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp3  = list(analyte = "carbenicillin-PBP 3 complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp4  = list(analyte = "carbenicillin-PBP 4 complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    npbp56 = list(analyte = "carbenicillin-PBP 5/6 complex", units = NA_character_, specimen = "administration site", verified = FALSE),
    nperi  = list(analyte = "carbenicillin in periplasm", units = NA_character_, specimen = "bile", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_CAR_MGL = list(
      description = "Static extracellular carbenicillin concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
      reference_category = "n/a -- studied at 96 mg/L (2x the MIC of 48 mg/L)",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; carbenicillin MIC 48 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "carbenicillin at a static extracellular concentration of 96 mg/L (2x the MIC of 48 mg/L); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 0.148; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, carbenicillin: 0.148 (SE 44.5%)
    # The studied extracellular concentration enters as the CONC_CAR_MGL covariate
    # (Table 1: carbenicillin studied at 96 mg/L = 2x the MIC of 48 mg/L).
    ...
    k2_1a <- 3030;        label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, carbenicillin PBP1a: 3030
    k2_1b <- 9520;        label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, carbenicillin PBP1b: 9520
    k2_2 <- 984;          label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, carbenicillin PBP2: 984
    k2_3 <- fixed(10000); label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, carbenicillin PBP3: 10000, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_4 <- 1580;         label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, carbenicillin PBP4: 1580
    k2_5 <- 1270;         label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, carbenicillin PBP5/6: 1270
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_CAR_MGL
