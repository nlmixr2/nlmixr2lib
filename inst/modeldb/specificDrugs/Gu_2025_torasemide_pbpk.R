Gu_2025_torasemide_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). torasemide disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. torasemide is",
    "a loop diuretic about 20% renal excretion with the remaining 80% hepatically metabolised (Supplementary Section 2.4).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; torasemide parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "torasemide", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    liver          = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    kidney         = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    spleen         = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    lung           = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    heart          = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    brain          = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    muscle         = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    skin           = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    adipose        = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    other          = list(analyte = "torasemide", units = "mg", specimen = "tissue", verified = TRUE),
    arterial       = list(analyte = "torasemide", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "torasemide", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  ini({
    ...
    fu_p       <- fixed(0.01);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [12] Knauf 1998
    bpr        <- fixed(1);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp assumed (footnote c)
    # Table S2 gives fu,g = 1 assumed (footnote c), but torasemide has no
    ...
    lcl_renal  <- fixed(log(0.0064));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 6.4 mL/min [31] Schwartz 1993
    lcl_liver  <- fixed(log(0.0366));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 36.6 mL/min [31] Schwartz 1993

    # ---- Absorption ----
    # ka is tabulated directly (Eq S15 not needed) and applies to all
    # three absorbing segments.
    lka        <- fixed(log(0.07));  label("First-order intestinal absorption rate constant ka (log 1/min)")  # Table S2 ka 0.07 1/min footnote b (simulated by WinNonlin)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(0.028   ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(0.015   ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(0.02    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(0.027   ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(0.016   ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(0.019   ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(0.019   ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.017   ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(0.019   ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(0.019   ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(0.019   ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
