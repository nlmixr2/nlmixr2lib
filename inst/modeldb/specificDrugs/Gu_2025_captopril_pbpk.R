Gu_2025_captopril_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). captopril disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. captopril is",
    "a angiotensin-converting-enzyme inhibitor predominantly hepatic metabolism with renal excretion of metabolites and 40-50% of unchanged drug (Supplementary Section 2.5).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; captopril parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "captopril", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    liver          = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    kidney         = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    spleen         = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    lung           = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    heart          = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    brain          = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    muscle         = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    skin           = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    adipose        = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    other          = list(analyte = "captopril", units = "mg", specimen = "tissue", verified = TRUE),
    arterial       = list(analyte = "captopril", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "captopril", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "10-100 mg single oral",
    notes          = paste(
  ...
  ini({
    ...
    fu_p       <- fixed(0.73);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [14] Rasool 2021
    bpr        <- fixed(1);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp [14] Rasool 2021
    # Table S2 gives fu,g = 1 assumed (footnote c), but captopril has no
    ...
    lcl_renal  <- fixed(log(0.37));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 370 mL/min [14] Rasool 2021
    lcl_liver  <- fixed(log(0.455));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 455 mL/min [14] Rasool 2021

    # ---- Absorption ----
    # ka is tabulated directly (Eq S15 not needed) and applies to all
    # three absorbing segments.
    lka        <- fixed(log(0.099));  label("First-order intestinal absorption rate constant ka (log 1/min)")  # Table S2 ka 0.099 1/min footnote b (simulated by WinNonlin)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(0.39    ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(0.97    ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(1.32    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(1.85    ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(0.99    ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(10.08   ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(1.08    ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(1.04    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(1.2     ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(1.2     ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(1.08    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
