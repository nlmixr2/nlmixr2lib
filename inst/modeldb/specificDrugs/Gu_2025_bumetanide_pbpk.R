Gu_2025_bumetanide_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). bumetanide disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. bumetanide is",
    "a loop diuretic eliminated by renal excretion (Supplementary Section 2.3).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; bumetanide parameters from Supplementary",
  ...
  compartmentData <- list(
    stomach        = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    duodenum       = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    jejunum        = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    ileum          = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    cecum          = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    colon          = list(analyte = "bumetanide", units = "mg", specimen = "administration site", verified = TRUE),
    wall_stomach   = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum  = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum   = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum     = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum     = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon     = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    liver          = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    kidney         = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    spleen         = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    lung           = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    heart          = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    brain          = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    muscle         = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    skin           = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    adipose        = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    other          = list(analyte = "bumetanide", units = "mg", specimen = "tissue", verified = TRUE),
    arterial       = list(analyte = "bumetanide", units = "mg", specimen = "whole blood", verified = TRUE),
    venous         = list(analyte = "bumetanide", units = "mg", specimen = "whole blood", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "1-5 mg single oral and 1-5 mg intravenous (2-3 min infusion)",
    notes          = paste(
  ...
  ini({
    ...
    fu_p       <- fixed(0.03);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [10] Donovan 2018
    bpr        <- fixed(0.55);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp [10] Donovan 2018
    # Table S2 gives fu,g = 1 assumed (footnote c), but bumetanide has no
    ...
    lcl_renal  <- fixed(log(0.146));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 146 mL/min [30] Cook 1988
    lcl_liver  <- fixed(log(0.062));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 62 mL/min [30] Cook 1988

    # ---- Absorption ----
    # ka is tabulated directly (Eq S15 not needed) and applies to all
    # three absorbing segments.
    lka        <- fixed(log(0.014));  label("First-order intestinal absorption rate constant ka (log 1/min)")  # Table S2 ka 0.014 1/min footnote b (simulated by WinNonlin)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote e (method of [37])
    kp_adipose   <- fixed(0.05    ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(0.1     ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(0.07    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(0.22    ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(0.14    ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(0.07    ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(0.06    ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.17    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(0.11    ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(0.28    ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(0.07    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
