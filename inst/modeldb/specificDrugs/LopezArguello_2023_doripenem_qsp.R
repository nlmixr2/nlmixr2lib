LopezArguello_2023_doripenem_qsp <- function() {
  ...
  covariateData <- list(
    ...
    CONC_DOR_MGL = list(
      description = "Static extracellular doripenem concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
      reference_category = "n/a -- studied at 2 mg/L (2x the MIC of 1 mg/L)",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; doripenem MIC 1 mg/L (CLSI broth microdilution)",
    ...
    dose_range = "doripenem at a static extracellular concentration of 2 mg/L (2x the MIC of 1 mg/L); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 40.9; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, doripenem: 40.9 (SE 50.7%)
    # The studied extracellular concentration enters as the CONC_DOR_MGL covariate
    # (Table 1: doripenem studied at 2 mg/L = 2x the MIC of 1 mg/L).
    ...
    k2_1a <- 49.3;      label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, doripenem PBP1a: 49.3
    k2_1b <- 677;       label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, doripenem PBP1b: 677
    k2_2 <- fixed(500); label("Second-order acylation rate constant, PBP2 (1e-3 /min; FIXED)") # Table S1, doripenem PBP2: 500, footnote a (fixed after sensitivity analyses of the highest-affinity target)
    k2_3 <- 1350;       label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, doripenem PBP3: 1350
    k2_4 <- 878;        label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, doripenem PBP4: 878
    k2_5 <- 85.7;       label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, doripenem PBP5/6: 85.7
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_DOR_MGL
