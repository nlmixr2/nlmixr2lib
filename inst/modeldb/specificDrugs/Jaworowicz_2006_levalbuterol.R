Jaworowicz_2006_levalbuterol <- function() {
  ...
  ini({
    ...
    lka         <- log(6.28)
    label("Adult (>= 12 years) absorption rate constant ka (1/h)")        # Table 2: Ka adults = 6.28 1/hr (%SEM 7.8)
    ...
    lcl         <- log(59.1)
    label("Apparent clearance intercept at WT = 74.8 kg (L/h)")             # Table 2: CL/F = 59.1 L/hr (%SEM 14.6); Equation 1 intercept
    e_wt_cl     <- 0.477
    label("Linear-additive WT effect on CL/F (L/h per kg above 74.8 kg)")   # Table 2: slope = 0.477 L/hr/kg (%SEM 26.4); Equation 1 slope
    ...
    lq          <- log(100)
    label("Inter-compartmental clearance Q (L/h)")                          # Table 2: Q = 100 L/hr (%SEM 9.7); no IIV reported
