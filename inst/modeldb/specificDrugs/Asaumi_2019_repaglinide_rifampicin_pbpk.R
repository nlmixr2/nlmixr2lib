Asaumi_2019_repaglinide_rifampicin_pbpk <- function() {
  description <- paste(
    "PBPK (five-unit tandem liver, extended clearance concept, segregated-flow",
    "intestine for rifampicin; Napp). Rifampicin-repaglinide drug-drug",
    "interaction in healthy adults. The rifampicin perpetrator model (saturable",
    "OATP1B uptake, UGT autoinduction) is coupled to a repaglinide victim model",
    "with enterohepatic circulation; rifampicin competitively inhibits OATP1B",
    "uptake (in vivo Ki,u 0.17 uM) and induces OATP1B (Emax 2.32), CYP2C8",
    "(Emax 2.55) and CYP3A (Emax 4.57) by turnover models (EC50,u 0.0639 uM).",
    "Repaglinide hepatic metabolism is 80.5% CYP2C8 and 19.5% CYP3A. Predicts the",
    "net inhibition (single dose) and induction (repeated dosing) AUC ratios.",
    "Victim beta fixed at 0.2 (0.5 / 0.8 sets in the file comments).",
  ...
  # in umol/L. Repaglinide: doses in ug, states in ug/L = ng/mL.
  units <- list(
    time = "h",
    dosing = "umol (rifampicin); ug (repaglinide)",
    concentration = "ng/mL (Cc, repaglinide); umol/L (Cc_rif)"
  )
  ...
  compartmentData <- list(
    central = list(analyte = "repaglinide", units = "ng/mL", specimen = "whole blood", verified = TRUE),
    is_liver1 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    is_liver2 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    is_liver3 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    is_liver4 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    is_liver5 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    int_liver1 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    int_liver2 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    int_liver3 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    int_liver4 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    int_liver5 = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    muscle = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    skin = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    adipose = list(analyte = "repaglinide", units = "ng/mL", specimen = "tissue", verified = TRUE),
    ehc1 = list(analyte = "repaglinide", units = "ug", specimen = "bile", verified = TRUE),
    ehc2 = list(analyte = "repaglinide", units = "ug", specimen = "bile", verified = TRUE),
    ehc3 = list(analyte = "repaglinide", units = "ug", specimen = "bile", verified = TRUE),
    gut_lumen = list(analyte = "repaglinide", units = "ug", specimen = "administration site", verified = TRUE),
  ...
  population <- list(
    ...
    dose_range = "rifampicin 600 mg PO single or once daily for 5-7 days; repaglinide 50 ug-4 mg single PO",
    notes = paste(
      "Clinical DDI data from the literature (Kim 2016, Yoshikado 2017, Bidstrup",
      "2004, Hatorp 2003, Niemi 2000); mean profiles / AUC ratios of healthy",
      "volunteers. The enterocyte CYP3A state is integrated, as in the",
      "Supplementary Model Code, but does not act on repaglinide (FaFg = 1)."
    )
  )
  ...
  ini({
    ...
    # ------------------------------------------------------------------
    # Repaglinide (victim). Estimated in this paper: Table S2A, beta = 0.2
    # column, 'representative values' from the Kim 2016 control profile.
    # beta = 0.5 / 0.8 sets: Tlag 0.19 / 0.20 h, ka 3.5 / 3.5 1/h,
    # SF Kp 1.4 / 1.4, fB CLint,all 0.80 / 0.80 L/h/kg.
    # ------------------------------------------------------------------
    ltlag <- log(0.18)
    label("Repaglinide lag time in intestinal absorption (h)") # Table S2A Repaglinide beta 0.2 Tlag 0.18 +/- 0.01
    lka <- log(3.6)
    label("Repaglinide absorption rate constant (1/h)") # Table S2A Repaglinide beta 0.2 ka 3.6 +/- 0.6
    sfkp <- 1.4
    label("Repaglinide common scaling factor on in silico Kp values (unitless)") # Table S2A Repaglinide beta 0.2 SF Kp 1.4 +/- 0.3
    fbclint_all <- 0.80
    label("Repaglinide unbound-blood overall hepatic intrinsic clearance fB*CLint,all (L/h/kg)") # Table S2A Repaglinide beta 0.2 fB CLint,all 0.80 +/- 0.02
    # Fixed repaglinide parameters -- Table 1 'Repaglinide' column.
    lkbile <- fixed(log(1.39))
    label("Repaglinide transit rate constant in enterohepatic circulation (1/h)") # Table 1 Repaglinide kbile 1.39
    fafg <- fixed(1)
    label("Repaglinide intestinal availability FaFg (unitless)") # Table 1 Repaglinide FaFg 1
    lkp_muscle <- fixed(log(0.112))
    label("Repaglinide muscle-to-blood partition coefficient (unitless)") # Table 1 Repaglinide Kp,muscle 0.112
    lkp_skin <- fixed(log(0.479))
    label("Repaglinide skin-to-blood partition coefficient (unitless)") # Table 1 Repaglinide Kp,skin 0.479
    lkp_adipose <- fixed(log(0.149))
    label("Repaglinide adipose-to-blood partition coefficient (unitless)") # Table 1 Repaglinide Kp,adipose 0.149
    fb <- fixed(0.0109)
    label("Repaglinide unbound fraction in blood (unitless)") # Table 1 Repaglinide fB 0.0109
    fh <- fixed(0.0123)
    label("Repaglinide unbound fraction in hepatocytes (unitless)") # Table 1 Repaglinide fH 0.0123
    rdif <- fixed(0.392)
    label("Repaglinide Rdif = PSdif,inf / PSact,inf (unitless)") # Table 1 Repaglinide Rdif 0.392
    beta <- fixed(0.2)
    label("Repaglinide beta, rate-determining fraction of hepatic clearance (unitless)") # Table 1 Repaglinide beta 0.2/0.5/0.8 (sensitivity values); 0.2 used here
    gamma <- fixed(0.578)
    label("Repaglinide gamma = PSdif,inf / PSdif,eff (unitless)") # Table 1 Repaglinide gamma 0.578
    fbile <- fixed(0.21)
    label("Repaglinide fraction of hepatocyte elimination that is biliary excretion (unitless)") # Table 1 Repaglinide fbile 0.21
    f_oatp1b <- fixed(1)
    label("Repaglinide fraction of active hepatic uptake mediated by OATP1B (unitless)") # Methods: 'f OATP1B values were set to be unity'
    fm_cyp2c8 <- fixed(0.805)
    label("Repaglinide fraction of hepatic metabolism by CYP2C8 (unitless)") # Table 1 Repaglinide fm 0.805 (CYP2C8); Supplementary Text Eq. 1 with alpha 2.99
    fm_cyp3a4 <- fixed(0.195)
    label("Repaglinide fraction of hepatic metabolism by CYP3A (unitless)") # Table 1 Repaglinide fm 0.195 (CYP3A) = 1 - fm,CYP2C8
    ki_oatp_rif <- fixed(0.17)
    label("Rifampicin unbound Ki for OATP1B-mediated repaglinide uptake (umol/L)") # Methods: 0.17 uM = 0.60 x 0.19 / 0.65
  })
  ...
  model({
    ...
    bile1 <- fbile * fh * cl_int_h * int_liver1
    elim1 <- (ind1 * (1 - fbile) + fbile) * fh * cl_int_h * int_liver1
    ...
    bile2 <- fbile * fh * cl_int_h * int_liver2
    elim2 <- (ind2 * (1 - fbile) + fbile) * fh * cl_int_h * int_liver2
    ...
    bile3 <- fbile * fh * cl_int_h * int_liver3
    elim3 <- (ind3 * (1 - fbile) + fbile) * fh * cl_int_h * int_liver3
    ...
    bile4 <- fbile * fh * cl_int_h * int_liver4
    elim4 <- (ind4 * (1 - fbile) + fbile) * fh * cl_int_h * int_liver4
    ...
    bile5 <- fbile * fh * cl_int_h * int_liver5
    elim5 <- (ind5 * (1 - fbile) + fbile) * fh * cl_int_h * int_liver5
