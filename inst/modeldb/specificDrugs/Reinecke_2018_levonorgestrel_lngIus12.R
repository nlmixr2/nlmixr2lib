Reinecke_2018_levonorgestrel_lngIus12 <- function() {
  description <- paste(
    "LNG-IUS 12 (Kyleena) arm of the Reinecke 2018 final comprehensive",
    ...
    "LNG-IUS 20 and LNG-IUS 8 arms because all three devices, plus the",
  ...
  population <- list(
    ...
    n_subjects     = 1306L,
    n_studies      = 1L,
    studies        = "Phase 3 study 310442 (levonorgestrel and SHBG concentrations plus residual-content measurements over the full 5 years of intended treatment)",
    age_range      = "18-35 years (median 27 years)",
    weight_range   = "39-160 kg (median 65 kg)",
    sex_female_pct = 100,
    disease_state  = "Healthy premenopausal women using a levonorgestrel-releasing intrauterine system for contraception",
    dose_range     = "Single insertion of one LNG-IUS 12 (Kyleena) device; nominal release 12 ug/day, indicated for up to 5 years",
    notes          = paste(
      "Demographics and observation counts from Table 1, whose product",
      "labels are offset by one row relative to their values: the",
      "1306-subject / study-310442 row carries no label but belongs to",
      "LNG-IUS 12, because the Data section assigns study 89532 to LNG-IUS",
      "20 and says LNG-IUS 12 and LNG-IUS 8 both came from study 310442.",
      "Contributing observations: 2253 LNG concentrations, 2374 SHBG",
      "concentrations and 854 residual-content measurements. The loaded",
      "reservoir content is not stated in the text; the y-intercept of the",
      "Supplemental Figure S1b residual-content plot is approximately",
      "19.5 mg, and that value reproduces the Table 4 in-vivo release rates",
      "to within 1%. Nineteen LNG and 45 SHBG measurements taken after",
      "device removal were excluded across the IUS data before the",
  ...
  ini({
    # ------------------------------------------------------------------
    # In-vivo release from the LNG-IUS 12 reservoir. Table S3a gives the
    ...
    # ------------------------------------------------------------------
    lc12 <- log(298)     ; label("Zero-order release rate C12 (ng/h)")                          # Table S2, final comprehensive model, C12 LNG-IUS 12 = 298 (291-305) ng/h
    lc13 <- log(0.139)   ; label("Second release process coefficient C13 (dimensionless)")      # Table S2, final comprehensive model, C13 LNG-IUS 12 = 0.139 (0.101-0.177)
    lt1  <- log(9100)    ; label("Time parameter T1 for the drop in release process 2 (h)")     # Table S2, final comprehensive model, T1 LNG-IUS 12 = 9100 (5470-12 700)

    # Absolute bioavailability on the logit scale: TF = 5.09 gives
    # F = 0.994, matching the value Table S2 prints in parentheses.
    logitfdepot <- 5.09  ; label("Logit-transformed absolute bioavailability of the LNG-IUS 12 reservoir (logit units)")  # Table S2, final comprehensive model, F LNG-IUS 12 = 5.09 (4.20-5.98), i.e. 0.994 (0.985-0.997)
    ...
    addSd_iusResidual <- sqrt(0.189)    ; label("Additive residual error SD on residual device content (mg)")                      # Table S2, final comprehensive model, SIGMA LNG-IUS 12 res. content = 0.189 (0.153-0.225) as a variance; sqrt = 0.435 mg
  })
  ...
  model({
    ...
    # LNG-IUS 12 release branch of Table S3a.
    input1 <- c12
