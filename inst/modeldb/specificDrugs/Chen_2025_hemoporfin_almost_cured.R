Chen_2025_hemoporfin_almost_cured <- function() {
  description <- paste0(
    ...
    "therapy as category I (almost cured, at least 90% improvement in ",
    "port-wine-stain colour blanching) 8 weeks after treatment, in 24 ",
    "Chinese pediatric patients aged 7-14 years (Chen 2025, NCT03125057). ",
    "The probability is expit(-29.7 + 1.98 * AUC_HEMO) where AUC_HEMO is ",
    "the individual hemoporfin AUC(0-30min) in h*ug/mL derived from the ",
    "companion population PK model (Chen 2025 Table S1). This endpoint did ",
    "NOT reach statistical significance (slope p = 0.104, 95% CI 0.372 to ",
    "5.63) and the non-significance is the paper's reported result, not a ",
    "transcription gap: of the five endpoints screened, only the ",
    "investigator- and patient-rating endpoints were significant. The ",
    "coefficients are extracted because Table S1 reports point estimates ",
    "for all five, but the very large intercept and slope reflect near-",
    "separation in a 24-patient sample and should not be extrapolated. ",
    "There is no PK layer and no ODE -- exposure enters as a covariate ",
    "column. No between-subject random effect and no residual error are ",
    "estimated (Bernoulli likelihood). Companion models: ",
  ...
  units <- list(
    ...
    concentration = "prob_almost_cured (probability of expert-panel category I, 0-1; also logit_almost_cured)"
  )
  ...
  population <- list(
    ...
    notes          = paste0(
      ...
      "is category I. Only the pediatric trial contributed efficacy data -- ",
  ...
  ini({
    # ==================================================================
    # Chen 2025 Table S1 (Data S1), row block "Probability of being
    # almost cured (category I)". Model form is Equation 3:
    ...
    #
    # NEITHER coefficient is significant at the 5% level. The magnitudes
    # (intercept -29.7, slope 1.98) and the extremely wide confidence
    # intervals are the signature of near-separation in 24 subjects; the
    # fitted curve is effectively a step function over the observed
    # exposure range. Retained for completeness because Table S1 prints
    # point estimates, but see the vignette's Assumptions and deviations
    # before using this model for prediction.
    # ==================================================================

    # ----- Logit intercept -----
    logit_ref <- -29.7 ; label("Logit of the probability of expert-panel category I at AUC_HEMO = 0 h*ug/mL (unitless logit)")  # Table S1, beta0 = -29.7 (95% CI -81.4 to -7.13, p = 0.0888; not significant)

    # ----- Exposure effect on the logit -----
    e_auc_hemo_logit <- 1.98 ; label("Log-odds of expert-panel category I per 1 h*ug/mL increase in hemoporfin AUC(0-30min) (mL/(ug*h))")  # Table S1, beta1 = 1.98 mL*ug^-1*h^-1 (95% CI 0.372 to 5.63, p = 0.104; not significant)
    ...
    addSd_prob_almost_cured <- fixed(0.001) ; label("Placeholder additive residual SD on the typical-value event probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })

  model({
    # ----- Linear predictor (Chen 2025 Equation 3, Table S1) -----
    logit_almost_cured <- logit_ref + e_auc_hemo_logit * AUC_HEMO

    prob_almost_cured <- expit(logit_almost_cured)

    # ----- Observation -----
    prob_almost_cured ~ add(addSd_prob_almost_cured)
  })
}
