Suri_2018_brentuximab <- function() {
  ...
  ini({
    ...
    e_bsa_adc_vc      <- 1.27;   label("Power exponent of (BSA / 1.865) on ADC V1 (unitless)")              # Suri 2018 Table S1: 1.27 (4.9% RSE)
    ...
    lclm  <- log(0.577);   label("MMAE clearance (CLM, L/hr)")                                              # Suri 2018 Table S3 (sup 8): 0.577 (1.2% RSE)
    lvcm  <- log(16.0);    label("MMAE central volume (VM, L)")                                             # Suri 2018 Table S3 (sup 8): 16.0 (1.4% RSE)
    lqm   <- log(2.65);    label("MMAE inter-compartmental clearance (QM, L/hr)")                           # Suri 2018 Table S3 (sup 8): 2.65 (1.2% RSE)
    lvpm  <- log(14.2);    label("MMAE peripheral volume (VMP, L)")                                         # Suri 2018 Table S3 (sup 8): 14.2 (1.1% RSE)
    lkd   <- log(0.00069); label("MMAE binding rate constant (Kd, 1/hr)")                                   # Suri 2018 Table S3 (sup 8): 0.00069 (1.6% RSE) — supplement table column header reads "Kd 1/h"; main paper page 994 mistakenly prints the unit as "L/h" (see vignette Errata section)
    lalfm <- log(2.64);    label("Decay rate of ADC->MMAE proteolytic-conversion fraction (ALFM, 1/hr)")    # Suri 2018 Table S3 (sup 8): 2.64 (1.0% RSE) — main paper page 994 mistakenly prints the unit as "L/h" (see vignette Errata section)
    lklag <- log(15.7);    label("Lag-compartment empty rate constant (Klag, 1/hr)")                        # Suri 2018 Table S3 (sup 8): 15.7 (1.0% RSE) — main paper page 994 mistakenly prints the unit as "L/h" (see vignette Errata section)
    ...
    e_bsa_mmae_cl     <- 2.81;   label("Power exponent of (BSA / 1.865) on MMAE CL (unitless)")             # Suri 2018 Table S3: 2.81 (6.4% RSE)
    e_bsa_mmae_vc     <- 0.89;   label("Power exponent of (BSA / 1.865) on MMAE VM (unitless)")             # Suri 2018 Table S3: 0.89 (9.5% RSE)
    e_alb_mmae_cl     <- 0.982;  label("Power exponent of (ALB / 36.81) on MMAE CL (unitless)")             # Suri 2018 Table S3: 0.982 (3.2% RSE)
    e_tbili_mmae_cl   <- -0.1;   label("Power exponent of (TBILI / 7.78) on MMAE CL (unitless)")            # Suri 2018 Table S3: -0.1 (7.5% RSE)
    e_creat_mmae_cl   <- -0.143; label("Power exponent of (CREAT / 72.4) on MMAE CL (unitless)")            # Suri 2018 Table S3: -0.143 (10.2% RSE)
    ...
    etalclm  ~ 0.1664         # Suri 2018 Table S3: MMAE CL 42.5% CV; omega^2 = log(0.425^2 + 1) = 0.1664
    etalvcm  ~ 0.3725         # Suri 2018 Table S3: MMAE V1 66.7% CV; omega^2 = log(0.667^2 + 1) = 0.3725
    ...
    CmmaepropSd  <- 0.423; label("Proportional residual error on MMAE Cmmae (fraction)")                    # Suri 2018 Table S3: 42.3% CV (0.3% RSE)
  })
  ...
  model({
    ...
    cl_mmae <- exp(lclm + etalclm) *
               nbsa^e_bsa_mmae_cl *
               nalb^e_alb_mmae_cl *
               ntbili^e_tbili_mmae_cl *
               ncreat^e_creat_mmae_cl
    vc_mmae <- exp(lvcm + etalvcm) * nbsa^e_bsa_mmae_vc
    qm      <- exp(lqm)
    vp_mmae <- exp(lvpm)
    kd      <- exp(lkd)
    alfm    <- exp(lalfm)
    ...
    # k10 * adc_central and decays exponentially with rate ALFM. tad()
    ...
    # and ng/mL for ADC; ug/mL = mg/L). Cmmae (umol/L) * MW_MMAE_Da
    # (~718.0) = Cmmae in ng/mL (paper uses ng/mL and pg/mL for MMAE).
