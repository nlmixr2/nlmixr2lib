Nguyen_2020_domagrozumab_qsp <- function() {
  description <- paste(
    "QSP. Myostatin / activin A / ActRIIB systems-pharmacology model (Nguyen",
    "2020) parameterised for the anti-myostatin monoclonal antibody",
    "domagrozumab (PF-06252616) in healthy adults.",
    ...
    "pituitary.",
    "Compound PK and myostatin-binding parameters were fitted to digitised",
    "domagrozumab PK data (Figure S4); the antibody does not bind activin",
    "(koff for activin set arbitrarily high). The muscle and FSH",
    "parameters are the adnectin and ACE-031 estimates, used by the authors",
    "to forward-predict domagrozumab muscle-volume change (Table 3). Table",
    "2 gives no absorption rate for domagrozumab, so the model supports",
    "intravenous dosing into central only (the tabulated s.c.",
    "bioavailability of 0.75 cannot be used without ka). Deterministic;",
    "ligand, receptor and FSH states start at the drug-free steady state.",
    sep = " "
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "domagrozumab (free, not ligand-bound)", units = "mg", specimen = "plasma", verified = TRUE),
    drug_muscle = list(analyte = "domagrozumab (free)", units = "nM", specimen = "tissue", verified = TRUE),
    drug_pituitary = list(analyte = "domagrozumab (free)", units = "nM", specimen = "tissue", verified = TRUE),
    drug_other = list(analyte = "domagrozumab (free)", units = "nM", specimen = "tissue", verified = TRUE),
    md_plasma = list(analyte = "domagrozumab-myostatin complex", units = "nM", specimen = "plasma", verified = TRUE),
    md_muscle = list(analyte = "domagrozumab-myostatin complex", units = "nM", specimen = "tissue", verified = TRUE),
    md_other = list(analyte = "domagrozumab-myostatin complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_plasma = list(analyte = "domagrozumab-activin A complex", units = "nM", specimen = "plasma", verified = TRUE),
    ad_muscle = list(analyte = "domagrozumab-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_pituitary = list(analyte = "domagrozumab-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
    ad_other = list(analyte = "domagrozumab-activin A complex", units = "nM", specimen = "tissue", verified = TRUE),
  ...
  population <- list(
    ...
    n_subjects = NA,
    n_studies = 1,
    disease_state = "healthy adult volunteers (domagrozumab phase 1, Bhattacharya 2018)",
    dose_range = "10 mg/kg for the muscle-volume forward prediction (Table 3)",
    notes = paste(
      "PK parameters were estimated by fitting the QSP model to digitised",
      "aggregate domagrozumab PK data (Figure S4); subject counts are not",
      "reported. Table 2 prints F = 0.75 (from Bhattacharya 2018) but no ka,",
      "so only intravenous administration is encoded. Doses in mg."
    )
  )
  ...
  ini({
    ...
    # ---- domagrozumab compound parameters (Table 2, domagrozumab column) ------------
    mw_drug <- fixed(150000); label("Molecular weight of domagrozumab (g/mol)") # Table 2, domagrozumab column MW_Drug = 150,000
    vother <- fixed(3.36); label("Volume of other tissues (L)") # Table 2, domagrozumab column V_other = 3.36 L
    kdeg_drug <- fixed(0.0014); label("Degradation rate of free domagrozumab, all spaces (1/h)") # Table 2, domagrozumab column k_deg_drug = 0.0014
    kdeg_md <- fixed(0.0014); label("Degradation rate of domagrozumab-myostatin complex (1/h)") # Table 2, domagrozumab column k_deg_MD = 0.0014 (assumed ~ k_deg_drug)
    kdeg_ad <- fixed(0.0014); label("Degradation rate of domagrozumab-activin A complex (1/h)") # Table 2, domagrozumab column k_deg_AD = 0.0014 (assumed ~ k_deg_drug)
    kon_drug_myo <- fixed(0.389); label("domagrozumab-myostatin association rate constant (1/(nM*h))") # Table 2, domagrozumab column kon_drug-Myo = 0.389
    koff_drug_myo <- fixed(0.00153); label("domagrozumab-myostatin dissociation rate constant (1/h)") # Table 2, domagrozumab column koff_drug-Myo = 0.00153
    kon_drug_act <- fixed(0.6); label("domagrozumab-activin A association rate constant (1/(nM*h))") # Table 2, domagrozumab column kon_drug-Act = 0.6 (footnote i)
    koff_drug_act <- fixed(600000.0); label("domagrozumab-activin A dissociation rate constant (1/h)") # Table 2, domagrozumab column koff_drug-Act = 6 x 10^5 (footnote g: arbitrary high value, negligible activin binding)
    sigma_v_drug <- fixed(0.98); label("Vascular reflection coefficient of drug and drug complexes (unitless)") # Table 2, domagrozumab column sigma_m_V_drug = 0.98
    sigma_is_drug <- fixed(0.2); label("Lymphatic reflection coefficient of drug and drug complexes (unitless)") # Table 2, domagrozumab column sigma_m_IS_drug = 0.2 (footnote h: same in pituitary and other tissues)

    # ---- Muscle and FSH pharmacodynamics (Table 2, adnectin / ACE-031 columns)
    vmax_muscle <- fixed(0.005); label("Maximal rate of muscle-volume increase (%/h)") # Table 2 V_max_muscle = 0.005 (adnectin estimate; used to forward-predict domagrozumab muscle volume, Table 3)
