Inoue_2025_valemetostat_thrombocytopenia <- function() {
  description <- paste0(
    "Bayesian logistic-regression exposure-safety model for CTCAE grade ",
    ">= 3 thrombocytopenia in adults with relapsed/refractory peripheral ",
    ...
    "exposure slope (supplementary Table S7). The exposure term is linear ",
    "in the standardized exposure zE = (CSSU_VALE - 17.7) / 13.7 ng/mL. ",
  ...
  reference <- paste(
    ...
    "Exposure-response parameters are from supplementary Table S7;",
  ...
  units <- list(
    ...
    concentration = "prob_plt_decrease (probability of CTCAE grade >= 3 thrombocytopenia, 0-1; also logit_plt_decrease)"
  )
  ...
  covariateData <- list(
    ...
    PLT = list(
      description = "Baseline platelet count -- the laboratory value corresponding to this model thrombocytopenia endpoint.",
      units = "10^9 cells/L",
      type = "continuous",
      reference_category = NULL,
      notes = "Centred at 181.9 and scaled by 103.55 inside model(). Centre is the Figure S8 reference subject value; scale source: free-fit from the Figure S8C panel; Table 1 ER-safety SD 104 agrees.",
      source_name = "Baseline platelets, 109 cells/L"
    ),
    AAG = list(
  ...
  population <- list(
    ...
    sex_female_pct = 35.9,
    race_ethnicity = c(`Asian, Japanese` = 30.3, `Asian, non-Japanese` = 8.4, White = 45.4, Other = 15.9),
  ...
  ini({
    ...
    # Table S7. The table note states the reporting convention: 'main
    ...
    logit_ref <- log(0.222 / (1 - 0.222))
    label("Logit of the CTCAE grade >= 3 thrombocytopenia probability for the reference subject (unitless logit)")
    # Inoue 2025 Table S7, 'Population mean' 0.222 (0.108, 0.387); Rhat 1.00, ESS-bulk 14,848

    # ----- Exposure effect -----
    e_cssu_logit <- log(2.14)
    label("Log-odds of CTCAE grade >= 3 thrombocytopenia per 13.7 ng/mL increase in unbound valemetostat Cavgtte (unitless logit)")
    # Inoue 2025 Table S7, 'Unbound Cavg up to thrombocytopenia (Cavgtte)' 2.14 (0.858, 5.69); Rhat 1.00, ESS-bulk 14,476

    # ----- Covariate effects on the logit intercept (b1) -----
    e_age_logit <- log(1.01)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for a 11.85 years increase in age (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Age, years' OR 1.01 (0.701, 1.47); Rhat 1.00, ESS-bulk 22,437
    e_wt_logit <- log(0.841)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for a 15.57 kg increase in body weight (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Weight, kg' OR 0.841 (0.567, 1.23); Rhat 1.00, ESS-bulk 22,114
    e_plt_logit <- log(0.390)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for a 103.55 10^9 cells/L increase in baseline platelets (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Baseline platelets, 109 cells/L' OR 0.390 (0.222, 0.615); Rhat 1.00, ESS-bulk 20,313
    e_aag_logit <- log(1.25)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for a 62.16 mg/dL increase in AAG (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'AAG, mg/dL' OR 1.25 (0.845, 1.86); Rhat 1.00, ESS-bulk 17,913
    e_ldh_logit <- log(1.21)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for a 0.5803 log-unit increase in log baseline LDH (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'log baseline LDH, log U/L' OR 1.21 (0.849, 1.73); Rhat 1.00, ESS-bulk 21,673
    e_sexf_logit <- log(0.789)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for female sex vs male reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Sex: Female' OR 0.789 (0.414, 1.46); Rhat 1.00, ESS-bulk 22,578
    e_race_asian_oth_logit <- log(1.14)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for Asian non-Japanese vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Race/Country: Asian non-Japanese' OR 1.14 (0.519, 2.54); Rhat 1.00, ESS-bulk 22,048
    e_race_white_logit <- log(0.782)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for White vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Race/Country: White' OR 0.782 (0.405, 1.52); Rhat 1.00, ESS-bulk 18,363
    e_race_other_logit <- log(0.782)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for other race/country vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Race/Country: Other' OR 0.782 (0.361, 1.65); Rhat 1.00, ESS-bulk 21,389
    e_hepimp_logit <- log(1.10)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for mild or moderate hepatic impairment vs normal reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'NCI-ODWG hepatic function: Mild or Moderate impairment' OR 1.10 (0.565, 2.08); Rhat 1.00, ESS-bulk 23,778
    e_ecog_ge1_logit <- log(1.15)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for ECOG PS 1+ vs ECOG PS 0 reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'ECOG PS: 1+' OR 1.15 (0.635, 2.08); Rhat 1.00, ESS-bulk 23,096
    e_tumtp_ptcl_logit <- log(0.803)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for PTCL vs ATLL reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Patient type: PTCL' OR 0.803 (0.406, 1.61); Rhat 1.00, ESS-bulk 17,710
    e_tx_hct_logit <- log(1.09)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for prior transplant vs none (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Prior transplants: Yes' OR 1.09 (0.568, 2.07); Rhat 1.00, ESS-bulk 24,762
    e_line_3l_logit <- log(0.927)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for two prior regimens (third line) vs one prior regimen reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Number of prior regimens: 2' OR 0.927 (0.459, 1.83); Rhat 1.00, ESS-bulk 18,275
    e_line_4l_plus_logit <- log(1.39)
    label("Log-odds shift on the CTCAE grade >= 3 thrombocytopenia logit for more than two prior regimens (fourth line or later) vs one prior regimen reference (unitless logit)")
    # Inoue 2025 Table S7 main effect, 'Number of prior regimens: >2' OR 1.39 (0.769, 2.54); Rhat 1.00, ESS-bulk 18,849
    ...
    # Inoue 2025 Table S7 interaction effect, 'Age, years' -0.115 (-0.547, 0.316); Rhat 1.00, ESS-bulk 19,526
    e_wt_slope <- 0.0203
    label("Shift in the unbound-exposure slope for a 15.57 kg increase in body weight (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Weight, kg' 0.0203 (-0.453, 0.516); Rhat 1.00, ESS-bulk 19,366
    e_plt_slope <- 0.0484
    label("Shift in the unbound-exposure slope for a 103.55 10^9 cells/L increase in baseline platelets (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Baseline platelets, 109 cells/L' 0.0484 (-0.492, 0.550); Rhat 1.00, ESS-bulk 19,846
    e_aag_slope <- -0.355
    label("Shift in the unbound-exposure slope for a 62.16 mg/dL increase in AAG (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'AAG, mg/dL' -0.355 (-0.772, 0.0419); Rhat 1.00, ESS-bulk 19,223
    e_ldh_slope <- 0.400
    label("Shift in the unbound-exposure slope for a 0.5803 log-unit increase in log baseline LDH (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'log baseline LDH, log U/L' 0.400 (0.0299, 0.815); Rhat 1.00, ESS-bulk 18,300
    e_sexf_slope <- 0.360
    label("Shift in the unbound-exposure slope for female sex vs male reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Sex: Female' 0.360 (-0.320, 1.04); Rhat 1.00, ESS-bulk 19,925
    e_race_asian_oth_slope <- 0.0551
    label("Shift in the unbound-exposure slope for Asian non-Japanese vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Race/Country: Asian non-Japanese' 0.0551 (-0.795, 0.925); Rhat 1.00, ESS-bulk 22,497
    e_race_white_slope <- -0.349
    label("Shift in the unbound-exposure slope for White vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Race/Country: White' -0.349 (-1.05, 0.337); Rhat 1.00, ESS-bulk 21,427
    e_race_other_slope <- 0.0551
    label("Shift in the unbound-exposure slope for other race/country vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Race/Country: Other' 0.0551 (-0.724, 0.867); Rhat 1.00, ESS-bulk 21,078
    e_hepimp_slope <- -0.309
    label("Shift in the unbound-exposure slope for mild or moderate hepatic impairment vs normal reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'NCI-ODWG hepatic function: Mild or Moderate impairment' -0.309 (-0.996, 0.360); Rhat 1.00, ESS-bulk 21,340
    e_ecog_ge1_slope <- 0.354
    label("Shift in the unbound-exposure slope for ECOG PS 1+ vs ECOG PS 0 reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'ECOG PS: 1+' 0.354 (-0.305, 1.03); Rhat 1.00, ESS-bulk 20,467
    e_tumtp_ptcl_slope <- -0.425
    label("Shift in the unbound-exposure slope for PTCL vs ATLL reference (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Patient type: PTCL' -0.425 (-1.19, 0.307); Rhat 1.00, ESS-bulk 18,120
    e_tx_hct_slope <- -0.170
    label("Shift in the unbound-exposure slope for prior transplant vs none (unitless logit)")
    # Inoue 2025 Table S7 interaction effect, 'Prior transplants: Yes' -0.170 (-0.863, 0.522); Rhat 1.00, ESS-bulk 23,510
    e_line_3l_slope <- -0.271
    ...
    # Inoue 2025 Table S7 interaction effect, 'Number of prior regimens: 2' -0.271 (-1.03, 0.459); Rhat 1.00, ESS-bulk 20,718
    e_line_4l_plus_slope <- -0.205
    ...
    # Inoue 2025 Table S7 interaction effect, 'Number of prior regimens: >2' -0.205 (-0.850, 0.438); Rhat 1.00, ESS-bulk 21,739
    ...
    addSd_prob_plt_decrease <- fixed(0.001)
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_plt_decrease <- logit_ref + expo_effect + cov_logit + cov_slope * zexpo
