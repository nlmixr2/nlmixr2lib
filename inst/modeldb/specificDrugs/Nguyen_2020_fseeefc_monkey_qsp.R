Nguyen_2020_fseeefc_monkey_qsp <- function() {
  description <- paste(
    "Preclinical (cynomolgus monkey). QSP. Myostatin / activin A / ActRIIB",
    "systems-pharmacology model for the follistatin-Fc fusion protein",
    "FS-EEE-Fc (Nguyen 2020), monkey parameterisation used to project",
    "human PK.",
    ...
    "pituitary.",
    "System parameters from Table S3 (monkey), compound parameters from",
    "Table S4, fitted simultaneously to serum PK after IV and SC doses of",
    "0.3, 3 and 30 mg/kg. PK only: no muscle or FSH sub-model was",
    "parameterised for the monkey. Deterministic; ligand and receptor",
  ...
  compartmentData <- list(
    depot = list(analyte = "FS-EEE-Fc", units = "mg", specimen = "administration site", verified = TRUE),
  ...
  population <- list(
    species = "cynomolgus monkey (male)",
    n_subjects = NA,
    n_studies = 1,
    sex_female_pct = 0,
    weight_range = "reference body weight 6.2 kg (Table S4)",
    dose_range = "0.3, 3 and 30 mg/kg IV and SC",
    notes = paste(
      "In-house study (Supplementary Methods); serum sampled to 1008 h;",
      "LLOQ 19.5 ng/mL in 100% serum. Doses in mg (mg/kg x 6.2 kg)."
    )
  )

  ini({
    # ---- System physiology (Table S3 monkey) --------------------------------------
    vplasma <- fixed(0.187); label("Plasma volume (L)") # Table S3 monkey: 0.187
    vmuscle <- fixed(0.426); label("Interstitial volume of muscle (L)") # Table S3 monkey: 0.426
    vpituitary <- fixed(4.72e-06); label("Interstitial volume of anterior pituitary (L)") # Table S3 monkey: 4.72e-06
    lf_muscle <- fixed(0.01972); label("Lymph flow of muscle (L/h)") # Table S3 monkey: 0.01972
    lf_pituitary <- fixed(1.26e-07); label("Lymph flow of pituitary (L/h)") # Table S3 monkey: 1.26e-07
    lf_other <- fixed(0.037); label("Lymph flow of other tissues (L/h)") # Table S3 monkey: 0.037

    # ---- Myostatin (Table S3 monkey) ------------------------------------------------
    ...
    ksyn_ppmyo <- fixed(0.0996); label("Synthesis rate of myostatin-propeptide complex in muscle (nmol/h)") # Table S3 monkey: 0.0996 (unit printed 1/hr; enters the muscle balance as an amount rate)
    kcleave_ppmyo_muscle <- fixed(0.0578); label("Cleavage rate of myostatin-propeptide complex in muscle (1/h)") # Table S3 monkey: 0.0578
    kcleave_ppmyo_plasma <- fixed(0.00503); label("Cleavage rate of myostatin-propeptide complex in plasma (1/h)") # Table S3 monkey: 0.00503
    kdeg_ppmyo <- fixed(0.347); label("Degradation rate of myostatin-propeptide complex, all spaces (1/h)") # Table S3 monkey: 0.347 (footnote a: same in all spaces)
    kdeg_myo <- fixed(0.347); label("Degradation rate of mature myostatin, all spaces (1/h)") # Table S3 monkey: 0.347 (footnote a: same in all spaces)
    sigma_v_myo <- fixed(0.7); label("Vascular reflection coefficient of myostatin species (unitless)") # Table S3 monkey: 0.7 (footnote a: same in other tissues)
    sigma_is_myo <- fixed(0.2); label("Lymphatic reflection coefficient of myostatin species (unitless)") # Table S3 monkey: 0.2 (footnote a)
    kon_myo_actriib <- fixed(1.328); label("Myostatin-ActRIIB association rate constant (1/(nM*h))") # Table S3 monkey: 1.328 (Sako 2010)
    koff_myo_actriib <- fixed(0.124); label("Myostatin-ActRIIB dissociation rate constant (1/h)") # Table S3 monkey: 0.124 (Sako 2010)

    # ---- Activin A (Table S3 monkey) ------------------------------------------------
    ...
    ksyn_ppact_pituitary <- fixed(6.36e-07); label("Synthesis rate of activin A-propeptide complex in pituitary (nmol/h)") # Table S3 monkey: 6.36e-07
    ksyn_ppact_plasma <- fixed(0.0063); label("Synthesis rate of activin A-propeptide complex in plasma (nmol/h)") # Table S3 monkey: 0.0063
    ksyn_ppact_muscle <- fixed(0.00865); label("Synthesis rate of activin A-propeptide complex in muscle (nmol/h)") # Table S3 monkey: 0.00865
    kcleave_ppact <- fixed(3113); label("Cleavage rate of activin A-propeptide complex, all producing spaces (1/h)") # Table S3 monkey: 3113
    kdeg_ppact <- fixed(1.39); label("Degradation rate of activin A-propeptide complex, all spaces (1/h)") # Table S3 monkey: 1.39 (same in all spaces, as Table 1)
    kdeg_act <- fixed(2.08); label("Degradation rate of mature activin A, all spaces (1/h)") # Table S3 monkey: 2.08 (same in all spaces, as Table 1)
    sigma_v_act <- fixed(0.7); label("Vascular reflection coefficient of activin species (unitless)") # Table S3 monkey: 0.7 (footnote b: same in pituitary and other tissues)
    sigma_is_act <- fixed(0.2); label("Lymphatic reflection coefficient of activin species (unitless)") # Table S3 monkey: 0.2 (footnote b)
    kon_act_actriib <- fixed(14.9); label("Activin A-ActRIIB association rate constant (1/(nM*h))") # Table S3 monkey: 14.90 (Sako 2010)
    koff_act_actriib <- fixed(0.533); label("Activin A-ActRIIB dissociation rate constant (1/h)") # Table S3 monkey: 0.533 (Sako 2010)

    # ---- ActRIIB (Table S3 monkey) --------------------------------------------------
    bl_actriib <- fixed(0.138); label("Total ActRIIB concentration in muscle and in pituitary (nM)") # Table S3 monkey: 0.138 nM, printed once for all species

    # ---- FS-EEE-Fc compound parameters (Table S4 monkey) ------------
    mw_drug <- fixed(150000); label("Molecular weight of FS-EEE-Fc (g/mol)") # Table S4 monkey MW_Drug = 150000
    lka <- fixed(log(0.027)); label("Subcutaneous absorption rate constant (1/h)") # Table S4 monkey k_a = 0.027
    lfdepot <- fixed(log(0.7)); label("Subcutaneous bioavailability (fraction)") # Table S4 monkey F = 0.7
    vother <- fixed(0.54); label("Volume of other tissues (L)") # Table S4 monkey V_other = 0.54 L
    kdeg_drug <- fixed(0.008); label("Degradation rate of free FS-EEE-Fc, all spaces (1/h)") # Table S4 monkey k_deg_drug = 0.008
    kdeg_md <- fixed(0.008); label("Degradation rate of FS-EEE-Fc-myostatin complex (1/h)") # Table S4 monkey k_deg_MD = 0.008 (assumed ~ k_deg_drug)
    kdeg_ad <- fixed(0.008); label("Degradation rate of FS-EEE-Fc-activin A complex (1/h)") # Table S4 monkey k_deg_AD = 0.008 (assumed ~ k_deg_drug)
    kon_drug_myo <- fixed(0.6); label("FS-EEE-Fc-myostatin association rate constant (1/(nM*h))") # Table S4 monkey kon_drug-Myo = 0.6 (footnote b)
    koff_drug_myo <- fixed(0.0045); label("FS-EEE-Fc-myostatin dissociation rate constant (1/h)") # Table S4 monkey koff_drug-Myo = 0.0045 (from in vitro Kd)
    kon_drug_act <- fixed(0.6); label("FS-EEE-Fc-activin A association rate constant (1/(nM*h))") # Table S4 monkey kon_drug-Act = 0.6 (footnote b)
    koff_drug_act <- fixed(0.0037); label("FS-EEE-Fc-activin A dissociation rate constant (1/h)") # Table S4 monkey koff_drug-Act = 0.0037 (from in vitro Kd)
    sigma_v_drug <- fixed(0.84); label("Vascular reflection coefficient of drug and drug complexes (unitless)") # Table S4 monkey sigma_m_V_drug = 0.84 (footnote a: same in pituitary and other tissues)
    sigma_is_drug <- fixed(0.2); label("Lymphatic reflection coefficient of drug and drug complexes (unitless)") # Table S4 monkey sigma_m_IS_drug = 0.2 (footnote h: same in pituitary and other tissues)
