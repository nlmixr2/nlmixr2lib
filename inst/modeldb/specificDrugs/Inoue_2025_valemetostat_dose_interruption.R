Inoue_2025_valemetostat_dose_interruption <- function() {
  description <- paste0(
    ...
    "with relapsed/refractory peripheral T-cell lymphoma treated with the ",
    "oral EZH1/EZH2 dual inhibitor valemetostat (Inoue 2025, n = 251, 200 ",
    "mg once daily). The probability of the event is expit(a0 + <exposure ",
    ...
    "effects on the exposure slope (supplementary Table S10). The ",
    "exposure term is linear in the standardized exposure zE = (CSSU_VALE ",
    "- 17.7) / 13.7 ng/mL. Continuous covariates enter centred and ",
    "scaled; binary covariates enter raw. There is no PK layer and no ",
    "ODE: the individual exposure metric is supplied as the CSSU_VALE ",
    "data column, computed by the companion population PK model ",
  ...
  reference <- paste(
    ...
    "Exposure-response parameters are from supplementary Table S10;",
  ...
  units <- list(
    ...
    concentration = "prob_dose_interruption (probability of dose interruption due to a TEAE, 0-1; also logit_dose_interruption)"
  )
  ...
  ini({
    ...
    # Table S10. The table note states the reporting convention: 'main
    ...
    logit_ref <- log(0.413 / (1 - 0.413))
    label("Logit of the dose interruption due to a TEAE probability for the reference subject (unitless logit)")
    # Inoue 2025 Table S10, 'Population mean' 0.413 (0.254,0.590); Rhat 1.00, ESS-bulk 14,309

    # ----- Exposure effect -----
    e_cssu_logit <- log(1.41)
    label("Log-odds of dose interruption due to a TEAE per 13.7 ng/mL increase in unbound valemetostat Cavgtte (unitless logit)")
    # Inoue 2025 Table S10, 'Unbound Cavg up to dose interruption (Cavgtte)' 1.41 (0.583,3.39); Rhat 1.00, ESS-bulk 14,657

    # ----- Covariate effects on the logit intercept (b1) -----
    e_age_logit <- log(1.18)
    label("Log-odds shift on the dose interruption due to a TEAE logit for a 11.85 years increase in age (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Age, years' OR 1.18 (0.873,1.60); Rhat 1.00, ESS-bulk 21,949
    e_wt_logit <- log(1.02)
    label("Log-odds shift on the dose interruption due to a TEAE logit for a 15.57 kg increase in body weight (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Weight, kg' OR 1.02 (0.735,1.42); Rhat 1.00, ESS-bulk 19,262
    e_aag_logit <- log(1.08)
    label("Log-odds shift on the dose interruption due to a TEAE logit for a 62.16 mg/dL increase in AAG (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'AAG, mg/dL' OR 1.08 (0.789,1.48); Rhat 1.00, ESS-bulk 20,384
    e_ldh_logit <- log(0.912)
    label("Log-odds shift on the dose interruption due to a TEAE logit for a 0.5803 log-unit increase in log baseline LDH (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'log baseline LDH, log U/L' OR 0.912 (0.664,1.25); Rhat 1.00, ESS-bulk 20,678
    e_sexf_logit <- log(0.757)
    label("Log-odds shift on the dose interruption due to a TEAE logit for female sex vs male reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Sex: Female' OR 0.757 (0.435,1.30); Rhat 1.00, ESS-bulk 23,161
    e_race_asian_oth_logit <- log(1.17)
    label("Log-odds shift on the dose interruption due to a TEAE logit for Asian non-Japanese vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Race/Country: Asian non-Japanese' OR 1.17 (0.568,2.46); Rhat 1.00, ESS-bulk 21,897
    e_race_white_logit <- log(1.22)
    label("Log-odds shift on the dose interruption due to a TEAE logit for White vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Race/Country: White' OR 1.22 (0.667,2.22); Rhat 1.00, ESS-bulk 17,713
    e_race_other_logit <- log(0.718)
    label("Log-odds shift on the dose interruption due to a TEAE logit for other race/country vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Race/Country: Other' OR 0.718 (0.354,1.39); Rhat 1.00, ESS-bulk 19,122
    e_hepimp_logit <- log(1.65)
    label("Log-odds shift on the dose interruption due to a TEAE logit for mild or moderate hepatic impairment vs normal reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'NCI-ODWG hepatic function: Mild or Moderate impairment' OR 1.65 (0.942,2.98); Rhat 1.00, ESS-bulk 20,792
    e_ecog_ge1_logit <- log(1.06)
    label("Log-odds shift on the dose interruption due to a TEAE logit for ECOG PS 1+ vs ECOG PS 0 reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'ECOG PS: 1+' OR 1.06 (0.632,1.76); Rhat 1.00, ESS-bulk 18,512
    e_tumtp_ptcl_logit <- log(1.21)
    label("Log-odds shift on the dose interruption due to a TEAE logit for PTCL vs ATLL reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Patient type: PTCL' OR 1.21 (0.650,2.25); Rhat 1.00, ESS-bulk 17,288
    e_tx_hct_logit <- log(1.30)
    label("Log-odds shift on the dose interruption due to a TEAE logit for prior transplant vs none (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Prior transplants: Yes' OR 1.30 (0.735,2.31); Rhat 1.00, ESS-bulk 24,150
    e_line_3l_logit <- log(0.955)
    label("Log-odds shift on the dose interruption due to a TEAE logit for two prior regimens (third line) vs one prior regimen reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Number of prior regimens: 2' OR 0.955 (0.524,1.74); Rhat 1.00, ESS-bulk 18,840
    e_line_4l_plus_logit <- log(0.997)
    label("Log-odds shift on the dose interruption due to a TEAE logit for more than two prior regimens (fourth line or later) vs one prior regimen reference (unitless logit)")
    # Inoue 2025 Table S10 main effect, 'Number of prior regimens: >2' OR 0.997 (0.579,1.71); Rhat 1.00, ESS-bulk 18,495
    ...
    # Inoue 2025 Table S10 interaction effect, 'Age, years' -0.0980 (-0.456,0.260); Rhat 1.00, ESS-bulk 20,609
    e_wt_slope <- -0.0335
    label("Shift in the unbound-exposure slope for a 15.57 kg increase in body weight (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Weight, kg' -0.0335 (-0.453,0.389); Rhat 1.00, ESS-bulk 16,079
    e_aag_slope <- -0.296
    label("Shift in the unbound-exposure slope for a 62.16 mg/dL increase in AAG (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'AAG, mg/dL' -0.296 (-0.618,0.00803); Rhat 1.00, ESS-bulk 18,202
    e_ldh_slope <- 0.0347
    label("Shift in the unbound-exposure slope for a 0.5803 log-unit increase in log baseline LDH (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'log baseline LDH, log U/L' 0.0347 (-0.301,0.385); Rhat 1.00, ESS-bulk 19,098
    e_sexf_slope <- 0.233
    label("Shift in the unbound-exposure slope for female sex vs male reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Sex: Female' 0.233 (-0.377,0.845); Rhat 1.00, ESS-bulk 21,337
    e_race_asian_oth_slope <- -0.0826
    label("Shift in the unbound-exposure slope for Asian non-Japanese vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Race/Country: Asian non-Japanese' -0.0826 (-0.932,0.763); Rhat 1.00, ESS-bulk 22,411
    e_race_white_slope <- 0.407
    label("Shift in the unbound-exposure slope for White vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Race/Country: White' 0.407 (-0.277,1.09); Rhat 1.00, ESS-bulk 17,729
    e_race_other_slope <- -0.0284
    label("Shift in the unbound-exposure slope for other race/country vs Asian Japanese reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Race/Country: Other' -0.0284 (-0.745,0.708); Rhat 1.00, ESS-bulk 18,423
    e_hepimp_slope <- 0.0635
    label("Shift in the unbound-exposure slope for mild or moderate hepatic impairment vs normal reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'NCI-ODWG hepatic function: Mild or Moderate impairment' 0.0635 (-0.528,0.668); Rhat 1.00, ESS-bulk 20,647
    e_ecog_ge1_slope <- 0.373
    label("Shift in the unbound-exposure slope for ECOG PS 1+ vs ECOG PS 0 reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'ECOG PS: 1+' 0.373 (-0.215,0.986); Rhat 1.00, ESS-bulk 20,463
    e_tumtp_ptcl_slope <- -0.322
    label("Shift in the unbound-exposure slope for PTCL vs ATLL reference (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Patient type: PTCL' -0.322 (-1.00,0.364); Rhat 1.00, ESS-bulk 17,748
    e_tx_hct_slope <- -0.284
    label("Shift in the unbound-exposure slope for prior transplant vs none (unitless logit)")
    # Inoue 2025 Table S10 interaction effect, 'Prior transplants: Yes' -0.284 (-0.917,0.361); Rhat 1.00, ESS-bulk 21,687
    e_line_3l_slope <- 0.499
    ...
    # Inoue 2025 Table S10 interaction effect, 'Number of prior regimens: 2' 0.499 (-0.173,1.25); Rhat 1.00, ESS-bulk 20,565
    e_line_4l_plus_slope <- -0.0711
    ...
    # Inoue 2025 Table S10 interaction effect, 'Number of prior regimens: >2' -0.0711 (-0.678,0.543); Rhat 1.00, ESS-bulk 20,408
    ...
    addSd_prob_dose_interruption <- fixed(0.001)
  ...
  model({
    ...
    # ----- Linear predictor -----
    logit_dose_interruption <- logit_ref + expo_effect + cov_logit + cov_slope * zexpo
