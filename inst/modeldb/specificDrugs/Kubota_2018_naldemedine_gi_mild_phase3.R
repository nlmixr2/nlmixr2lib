Kubota_2018_naldemedine_gi_mild_phase3 <- function() {
  description <- paste0(
    ...
    "prob = 1 / (1 + exp(-(a + b * AUCss))) with a = -1.72 and b = 0.0106. ",
    "This is the best-determined of the safety fits: the 95% CI of the slope excludes zero, if only barely. ",
  ...
  reference <- paste(
    ...
    "Logistic-regression coefficients are Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Mild, Moderate, Severe';",
  ...
  population <- list(
    ...
    n_subjects = 993L,
    n_studies = 2L,
    n_observations = "993 binary a gastrointestinal disorder of mild or greater severity records, one per subject",
    ...
    regions = "Global",
    notes = paste(
      "The endpoint is the occurrence of a treatment-emergent adverse event in the gastrointestinal-disorders system organ class, dichotomised at the MILD-or-greater severity threshold. Observed frequencies (Table IV): placebo 75/548 (13.7%), 0.2 mg 94/445 (21.1%).",
  ...
  ini({
    # ==================================================================
    # Kubota 2018 Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Mild, Moderate, Severe'. The model is the paper's Eq. (3):
    #
    ...
    logit_ref <- -1.72; label("Logit of the a gastrointestinal disorder of mild or greater severity probability at zero naldemedine exposure, i.e. the placebo arm (unitless logit)")  # Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Mild, Moderate, Severe', parameter a = -1.72 (95% CI -1.95 to -1.51)
    e_auc_logit <- 0.0106; label("Log-odds of a gastrointestinal disorder of mild or greater severity per 1 ng*h/mL increase in naldemedine steady-state daily AUC (unitless logit)")  # Table V(b), rows '1314V9231 and 1315V9232 (Phase 3)' / 'Mild, Moderate, Severe', parameter b = 0.0106 (95% CI 0.000712 to 0.0202)
