Sun_2025_maribavir_headache <- function() {
  description <- paste0(
    "Binomial logistic-regression exposure-SAFETY model for ",
    "treatment-emergent headache as a function of the maribavir AUC on ",
    "the DAY OF THE EVENT, in allogeneic hematopoietic cell transplant ",
    ...
    "claim of the two. The graft T-cell-depletion modality is retained, ",
    "with the three fitted levels differing in SIGN. The source's fourth ",
    "level, 'Not reported', is DELIBERATELY OMITTED from this model file: ",
    "it has one subject, and its published estimate of -12.7 with a ",
    "standard error of 1460 and p = 0.993 is the signature of complete ",
    "separation rather than an estimate. See the covariate notes and the ",
    "vignette Assumptions and deviations section. The exposure slope is ",
    "+0.0513 per 10 ug*h/mL. There is no PK layer and no ODE: the ",
    "exposure metric is supplied as a data column, derived in the source ",
    "analysis from the companion population PK model packaged as ",
  ...
  covariateData <- list(
    AUC_MBV_DAY = list(
      ...
      notes              = paste(
        ...
        "order of magnitude.",
        "In this model the exposure enters with slope 0.0513 per 10 ug*h/mL (odds",
        "ratio 1.05 (1.02-1.08), p < 0.001)."
      ),
      source_name        = "AUCday of maribavir (increment of 10 h.ug/mL)"
    ),
    HCT_TCD_ALEMTUZUMAB = list(
      description        = "Graft T-cell depletion with in-vivo alemtuzumab for the current transplant.",
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no T-cell depletion)",
      notes              = paste(
        "9 of 238 subjects (3.8%). One of three indicators decomposing the T-cell",
        "depletion modality; all three are 0 for an unmanipulated graft (135 of",
        "238, 56.7%). The three are kept separate rather than pooled into one",
        "'any depletion' flag because their estimated effects differ in SIGN.",
        "Coefficient +1.77, the only significant level of the set."
      ),
      source_name        = "T-Cell depletion modality, Alemtuzumab administration (in vivo)"
    ),
    HCT_TCD_ATG = list(
      description        = "Graft T-cell depletion with in-vivo anti-thymocyte globulin for the current transplant.",
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no T-cell depletion)",
      notes              = paste(
        "74 of 238 subjects (31.1%), the commonest depletion modality in the cohort.",
        "Coefficient -0.882, not significant -- and OPPOSITE in sign to the",
        "alemtuzumab level, which is why the three modalities are carried as",
        "separate indicators rather than pooled into one depletion flag."
      ),
      source_name        = "T-Cell depletion modality, Anti-thymocyte globulin administration (in vivo)"
    ),
    HCT_TCD_EXVIVO = list(
      description        = "Ex-vivo graft T-cell depletion for the current transplant.",
      units              = "(binary)",
      type               = "binary",
      reference_category = "0 (no T-cell depletion)",
      notes              = paste(
        "19 of 238 subjects (8.0%).",
        "Coefficient +0.863, not significant."
      ),
      source_name        = "T-Cell depletion modality, Ex-vivo T-cell depletion"
    )
  )
  ...
  population <- list(
    ...
    n_subjects     = 238L,
    n_studies      = 1L,
    n_observations = "238 binary headache records, one per patient",
  ...
  ini({
    # ================================================================
    # Sun 2025 Figure S3 panel l (Supporting Information file s002).
    #
    ...
    # ----- Logit intercept -----
    logit_ref <- -3.33; label("Logit of the headache probability at zero maribavir exposure with all risk factors at their reference level (unitless logit)")  # Sun 2025 Figure S3 panel l (Supporting Information file s002), Intercept -3.33 (SE 0.482), p < 0.001

    # ----- Exposure effect -----
    e_auc_logit <- 0.0513; label("Log-odds of headache per 10 ug*h/mL increase in maribavir AUC on the day of the event (unitless logit)")  # Sun 2025 Figure S3 panel l (Supporting Information file s002), Estimate 0.0513 (SE 0.0147), odds ratio 1.05 (1.02-1.08), p < 0.001

    # ----- Risk-factor effects on the logit -----
    e_hct_tcd_alemtuzumab_logit <- 1.77; label("Log-odds of headache for in-vivo alemtuzumab T-cell depletion versus none (unitless logit)")  # Sun 2025 Figure S3 panel l (Supporting Information file s002), T-Cell depletion modality, Alemtuzumab administration (in vivo) Estimate 1.77 (SE 0.791), odds ratio 5.84 (1.24-27.5), p = 0.0256
    e_hct_tcd_atg_logit <- -0.882; label("Log-odds of headache for in-vivo anti-thymocyte-globulin T-cell depletion versus none (unitless logit)")  # Sun 2025 Figure S3 panel l (Supporting Information file s002), T-Cell depletion modality, Anti-thymocyte globulin administration (in vivo) Estimate -0.882 (SE 0.609), odds ratio 0.414 (0.125-1.37), p = 0.1477
    e_hct_tcd_exvivo_logit <- 0.863; label("Log-odds of headache for ex-vivo T-cell depletion versus none (unitless logit)")  # Sun 2025 Figure S3 panel l (Supporting Information file s002), T-Cell depletion modality, Ex-vivo T-cell depletion Estimate 0.863 (SE 0.621), odds ratio 2.37 (0.702-8.00), p = 0.1644
  ...
  model({
    ...
    logit_headache <- logit_ref +
      e_auc_logit * auc10 +
      e_hct_tcd_alemtuzumab_logit * HCT_TCD_ALEMTUZUMAB +
      e_hct_tcd_atg_logit * HCT_TCD_ATG +
      e_hct_tcd_exvivo_logit * HCT_TCD_EXVIVO
