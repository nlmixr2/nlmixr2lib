Li_2025_doxycycline_duck_ff20 <- function() {
  ...
  ini({
    ...
    # Li 2025 Methods, "PK and PD analyses" and Table 2 (florfenicol 20 mg/kg)
    # =================================================================
    ...
    # emax = (Li 2025 "E0" row). This orientation is confirmed numerically:
    # substituting Table 2 returns R = 3.000 at Ce = 39.19 h (AUC24h/MIC),
    # R = 3.000 at Ce = 1.72 (Cmax/MIC) and R = 3.000 at Ce = 51.66 (%T>MIC),
    # each being the paper's own reported "Decrease in 3 Log10 CFU/mL" row.
    e0 <- -0.53
    label("Change in bacterial count in the untreated model group over 24 h, as a reduction (log10 CFU/mL; negative = growth)")  # Li 2025 Table 2 row "E max (Log 10 CFU/mL)" = 0.53, which the Methods define as the change in the drug-free model group; sign flipped because Results report the model group GREW by 0.04-0.93 log10 CFU/mL. Kept on the natural scale because it is a signed net change

    lemax <- log(3.98)
    label("Log maximum antibacterial effect over 24 h Emax (log10 CFU/mL reduction)")  # Li 2025 Table 2 row "E 0 (Log 10 CFU/mL)" = 3.98, which the Methods define as the maximum antibacterial effect

    lec50 <- log(8.83)
    label("Log AUC24h/MIC producing 50% of the maximal effect EC50 (h)")  # Li 2025 Table 2 row "EC 50", AUC24h/MIC column = 8.83

    lhill <- log(0.86)
    label("Log Hill coefficient N of the exposure-response relationship (unitless)")  # Li 2025 Table 2 row "Hill's slope", AUC24h/MIC column = 0.86
    ...
    # exposure-response fit (only R^2 = 0.918 for the AUC24h/MIC sigmoid), so
