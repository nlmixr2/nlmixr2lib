Geldof_2008_fluvoxamine_sert_ecf_rat <- function() {
  description <- paste(
    ...
    "to fluvoxamine BRAIN EXTRACELLULAR FLUID (ECF) concentration, fit by",
    "Geldof et al. (2008, Br J Pharmacol 154:1369-1378). This is the second of",
    "the three PK-PD models reported side by side in Table 3 of that paper; the",
    "plasma- and brain-tissue-driven variants are",
    "Geldof_2008_fluvoxamine_sert_plasma_rat and",
    "Geldof_2008_fluvoxamine_sert_brain_rat. Occupancy is related to the ECF",
    "concentration by a hyperbolic Bmax (Emax) function,",
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "(Discussion p1377).",
      "Relating occupancy to ECF rather than plasma concentration lowered the",
      "objective function by 12.2 points and reduced both the residual error",
      "and the IIV on EC50, which the authors attribute to the non-linear brain",
      "distribution (Results p1374).",
      "Measured brain ECF concentrations were below the 1 ng/mL LOQ over a",
      "substantial part of the concentration-effect curve; the ECF EC50",
      "(0.22 ng/mL) lies below that LOQ, so the lower limb of the curve is",
      "informed by model-predicted concentrations back-calculated from the much",
      "higher brain tissue concentrations through the brain distribution model",
      "(paper Results p1374 and Figure 3b)."
    )
  )
  ...
  ini({
    ...
    # ------------------------------------------------------------------
    lemax <- log(94.9); label("Maximum SERT occupancy Bmax (% of control [3H]citalopram labelling)")  # Geldof 2008 BJP Table 3, ECF column: Bmax = 94.9 % (CV 1.1%)
    lec50 <- log(0.22); label("Brain ECF fluvoxamine concentration at half-maximal SERT occupancy EC50 (ng/mL)")  # Geldof 2008 BJP Table 3, ECF column: EC50 = 0.22 ng/mL (CV 10.6%)

    etalec50 ~ 0.25  # Geldof 2008 BJP Table 3, ECF column: omega^2(eta_EC50) = 0.25 (CV 38.0%)

    addSd <- 5.5588; label("Additive residual SD on SERT occupancy (percentage points)")  # Geldof 2008 BJP Table 3, ECF column: sigma^2(eps1ij) = 30.9 (CV 27.1%); SD = sqrt(30.9) = 5.5588
  })
  ...
  model({
    ...
    Cecf   <- cdb  # brain ECF (deep brain) fluvoxamine concentration (ng/mL); the PD driver
    Cbrain <- bt   # total brain tissue fluvoxamine concentration (ng/mL); diagnostic output

    # Hyperbolic Bmax model (paper Methods p1373) driven by the ECF concentration.
    sertOccupancy <- emax * Cecf / (ec50 + Cecf)
