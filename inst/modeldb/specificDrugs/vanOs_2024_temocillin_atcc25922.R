vanOs_2024_temocillin_atcc25922 <- function() {
  description <- paste0(
    "In vitro (Escherichia coli ATCC 25922, temocillin MIC 16 mg/L). ",
  ...
  reference <- paste0(
    ...
    "Pharmacodynamic parameter estimates: main-text Table 3, column ATCC 25922. ",
  ...
  compartmentData <- list(
    ...
    bact_susceptible = list(
      analyte = "Escherichia coli ATCC 25922 (susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_less_susceptible = list(
      analyte = "Escherichia coli ATCC 25922 (less susceptible subpopulation)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    ),
    bact_resistant = list(
      analyte = "Escherichia coli ATCC 25922 (resistant subpopulation growing on agar with 32 mg/L temocillin)",
      units = "cfu/mL", specimen = "not applicable", verified = TRUE
    )
  )
  ...
  population <- list(
    species = "in vitro (Escherichia coli ATCC 25922); pharmacokinetics from critically ill adults",
    n_subjects = 1L,
    n_studies = 1L,
    strain = paste0(
      "ATCC 25922 (reference strain; ESBL genes: none reported; temocillin MIC 16 mg/L, determined in triplicate ",
    ...
    notes = paste0(
      ...
      "estimated separately for the two experimental systems (Bmax,STK 8.89 and Bmax,HFIM ",
      "10.0 log10 cfu/mL for this strain); log10bmax below carries the HFIM value because that ",
  ...
  ini({
    ...
    # ---- Bacterial growth ---------------------------------------------------
    kg_s   <- 1.29; label("Growth rate constant of the susceptible subpopulation (1/h)")      # Table 3 (ATCC 25922): k_g,S 1.29, %RSE 5.5
    kg_ls  <- 0.797; label("Growth rate constant of the less susceptible subpopulation (1/h)") # Table 3 (ATCC 25922): k_g,LS 0.797, %RSE 7.5
    kg_res <- 0.583; label("Growth rate constant of the resistant subpopulation (1/h)")       # Table 3 (ATCC 25922): k_g,RES 0.583, %RSE 3.8

    log10bmax <- 10.0; label("Maximum bacterial density (log10 cfu/mL)") # Table 3 (ATCC 25922): Bmax,HFIM 10.0, %RSE 0.7; the static time-kill value Bmax,STK was 8.89 (%RSE 0.9)
    ...
    emax_s  <- 2.06; label("Maximum drug effect rate constant, susceptible subpopulation (1/h)")             # Table 3 (ATCC 25922): Emax,S 2.06, %RSE 3.3
    ec50_s  <- 3.56; label("Unbound concentration at half-maximal effect, susceptible subpopulation (mg/L)") # Table 3 (ATCC 25922): EC50,S 3.56, %RSE 1.9
    hill_s  <- fixed(10); label("Hill coefficient, susceptible subpopulation")                                 # Table 3 (ATCC 25922): H,S 10, FIX

    emax_ls <- 0.777; label("Maximum drug effect rate constant, less susceptible subpopulation (1/h)")             # Table 3 (ATCC 25922): Emax,LS 0.777, %RSE 9.2
    ec50_ls <- 17.1; label("Unbound concentration at half-maximal effect, less susceptible subpopulation (mg/L)") # Table 3 (ATCC 25922): EC50,LS 17.1, %RSE 9.1
    hill_ls <- 1.68; label("Hill coefficient, less susceptible subpopulation")                                 # Table 3 (ATCC 25922): H,LS 1.68, %RSE 12.1

    klin_res <- 0.00867; label("Linear drug effect rate constant, resistant subpopulation (L/(mg*h))") # Table 3 (ATCC 25922): k_lin,RES 0.00867, %RSE 9.8

    # ---- Initial bacterial population sizes ---------------------------------
    log10cfu0_s   <- 5.75; label("Fitted initial size of the susceptible subpopulation (log10 cfu/mL)")      # Table 3 (ATCC 25922): cfu_t0,S 5.75, %RSE 1.1
    log10cfu0_ls  <- 1.30; label("Fitted initial size of the less susceptible subpopulation (log10 cfu/mL)") # Table 3 (ATCC 25922): cfu_t0,LS 1.30, %RSE 12.1
    log10cfu0_res <- 0.237; label("Fitted initial size of the resistant subpopulation (log10 cfu/mL)")       # Table 3 (ATCC 25922): cfu_t0,RES 0.237, %RSE 46.4
    ...
    # the log10 scale."
    addSd_CFUtotal <- 0.403; label("Additive residual SD on the log10 total bacterial count (log10 cfu/mL)")     # Table 3 (ATCC 25922): RUV total 0.403, %RSE 4.6
    addSd_CFUres   <- 0.614; label("Additive residual SD on the log10 resistant bacterial count (log10 cfu/mL)") # Table 3 (ATCC 25922): RUV RES 0.614, %RSE 7.0
  })
  ...
  model({
    ...
    # log10(1e6 / (10^5.75 + 10^1.30)) log10 cfu/mL.
