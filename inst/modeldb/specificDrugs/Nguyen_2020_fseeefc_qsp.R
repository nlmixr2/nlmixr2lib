Nguyen_2020_fseeefc_qsp <- function() {
  description <- paste(
    "QSP. Myostatin / activin A / ActRIIB systems-pharmacology model for the",
    "engineered follistatin-Fc fusion protein FS-EEE-Fc in humans (Nguyen 2020),",
    "used to project the human efficacious dose for Duchenne muscular dystrophy.",
    "Drug disposition follows the Shah & Betts (2012) platform antibody model",
    "reduced to plasma plus the interstitial space of muscle, anterior pituitary",
    "and 'other tissues', linked by lymph flow with vascular and lymphatic",
    "reflection coefficients. Latent (propeptide-bound) and mature myostatin are",
    "made in muscle and cleaved in muscle and plasma; latent and mature activin",
    "A are made in plasma, muscle and pituitary. The drug binds both mature",
    "ligands (drug-ligand complexes are cleared like free drug); the free",
    "ligands bind the ActRIIB receptor in muscle and in the pituitary. Total",
    "muscle ActRIIB occupancy drives an indirect-response muscle-volume change",
    "(%) and pituitary ActRIIB occupancy drives plasma FSH. System parameters",
    "are from Table 1, compound parameters from Table 2 (FS-EEE-Fc column,",
    "projected from cynomolgus-monkey PK), and the muscle / FSH",
  ...
  paper_specific_compartments <- c(
    "drug_muscle", "drug_pituitary", "drug_other",
    "md_plasma", "md_muscle", "md_other",
    "ad_plasma", "ad_muscle", "ad_pituitary", "ad_other",
    "myo_plasma", "myo_muscle", "myo_other",
    "ppmyo_plasma", "ppmyo_muscle", "ppmyo_other",
    "act_plasma", "act_muscle", "act_pituitary", "act_other",
    "ppact_plasma", "ppact_muscle", "ppact_pituitary", "ppact_other",
    "actriib_muscle", "myo_actriib_muscle", "act_actriib_muscle",
    "actriib_pituitary", "act_actriib_pituitary",
    "muscle_growth", "fsh"
  )
  ...
  population <- list(
    ...
    n_studies = 0,
    disease_state = paste(
      "Projection for boys with Duchenne muscular dystrophy; FS-EEE-Fc had no",
      "clinical data. The muscle pharmacodynamics were calibrated on",
      "healthy-volunteer anti-myostatin adnectin (BMS-986089) data and the FSH",
      "pharmacodynamics on ACE-031 data in healthy postmenopausal women."
    ),
    weight_range = "single reference body weight of 71 kg (deposited code)",
    dose_range = "1-30 mg/kg weekly subcutaneous (Figure 4c-d); 3-5 mg/kg weekly projected efficacious",
    notes = paste(
      "No subjects were fitted for FS-EEE-Fc in humans. The human PK",
      "parameters were projected from a cynomolgus-monkey fit (Table S4) by",
      "simple allometry of V_other (exponent 1.12); the rate constants were",
      "carried over unchanged. Doses are in mg (mg/kg x 71 kg in the authors'",
      "code)."
    )
  )
  ...
  ini({
    ...
    # ---- FS-EEE-Fc compound parameters (Table 2, FS-EEE-Fc column) ------------
    mw_drug <- fixed(150000); label("Molecular weight of FS-EEE-Fc (g/mol)") # Table 2 MW_Drug = 150,000
    lka <- fixed(log(0.027)); label("Subcutaneous absorption rate constant (1/h)") # Table 2 k_a = 0.027 (footnote a: allometric scaling from monkey)
    lfdepot <- fixed(log(0.7)); label("Subcutaneous bioavailability (fraction)") # Table 2 F = 0.7 (footnote a)
    vother <- fixed(8.28); label("Volume of other tissues (L)") # Table 2 V_other = 8.28 L (footnote a; 0.54 L monkey x (71/6.2)^1.12)
    kdeg_drug <- fixed(0.008); label("Degradation rate of free FS-EEE-Fc, all spaces (1/h)") # Table 2 k_deg_drug = 0.008 (footnote a)
    kdeg_md <- fixed(0.006); label("Degradation rate of FS-EEE-Fc-myostatin complex (1/h)") # Table 2 k_deg_MD = 0.006 (deposited code uses k_deg_drug = 0.008; see vignette)
    kdeg_ad <- fixed(0.006); label("Degradation rate of FS-EEE-Fc-activin A complex (1/h)") # Table 2 k_deg_AD = 0.006 (deposited code uses k_deg_drug = 0.008; see vignette)
    kon_drug_myo <- fixed(0.6); label("FS-EEE-Fc-myostatin association rate constant (1/(nM*h))") # Table 2 kon_drug-Myo = 0.6
    koff_drug_myo <- fixed(0.0045); label("FS-EEE-Fc-myostatin dissociation rate constant (1/h)") # Table 2 koff_drug-Myo = 0.0045 (footnote d: Kd 7.5 pM)
    kon_drug_act <- fixed(0.6); label("FS-EEE-Fc-activin A association rate constant (1/(nM*h))") # Table 2 kon_drug-Act = 0.6 (footnote i)
    koff_drug_act <- fixed(0.0037); label("FS-EEE-Fc-activin A dissociation rate constant (1/h)") # Table 2 koff_drug-Act = 0.0037 (footnote d: Kd 6.1 pM)
    sigma_v_drug <- fixed(0.98); label("Vascular reflection coefficient of drug and drug complexes (unitless)") # Table 2 sigma_m_V_drug = 0.98 (footnote f; footnote h: same in pituitary and other tissues)
    sigma_is_drug <- fixed(0.2); label("Lymphatic reflection coefficient of drug and drug complexes (unitless)") # Table 2 sigma_m_IS_drug = 0.2 (footnote h)

    # ---- Muscle and FSH pharmacodynamics (Table 2, adnectin / ACE-031 columns)
    vmax_muscle <- fixed(0.005); label("Maximal rate of muscle-volume increase (%/h)") # Table 2 V_max_muscle = 0.005 (adnectin fit; carried to FS-EEE-Fc per Methods and deposited code)
