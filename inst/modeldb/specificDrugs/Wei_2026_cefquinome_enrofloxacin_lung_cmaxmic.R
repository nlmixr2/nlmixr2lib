Wei_2026_cefquinome_enrofloxacin_lung_cmaxmic <- function() {
  description <- paste0(
    ...
    "carries the fit driven by the Cmax/MIC (combine) index computed from lung PK data. ",
    ...
    "Wei 2026's Emax = 0.077 onto the canonical `e0` (effect at zero exposure) and Wei 2026's ",
    "E0 = -5.976 onto the canonical `emax` (maximal drug effect), and writes the algebraically ",
    "identical E = e0 - (e0 - emax) * Ce^N / (Ce^N + EC50^N). No value is altered by the remapping. ",
    "Parameters from Wei 2026 Table 6, lung / Cmax/MIC (combine) column: e0 = 0.077, emax = -5.976 ",
    "log10 CFU/mL, EC50 = 555.674, Hill N = 0.819 (R^2 = 0.880, AIC = 17.757). ",
    ...
    "typical-value simulation. Sibling models from the same paper: Wei_2026_cefquinome_enrofloxacin_plasma_cmaxmic, Wei_2026_cefquinome_enrofloxacin_plasma_aucmic, Wei_2026_cefquinome_enrofloxacin_plasma_tmic, Wei_2026_cefquinome_enrofloxacin_lung_aucmic and Wei_2026_cefquinome_enrofloxacin_lung_tmic."
  )
  reference <- paste(
    ...
    "PK/PD'. Parameter estimates: Table 6, lung / Cmax/MIC (combine) column.",
  ...
  covariateData <- list(
    CMAXMIC_CEFQ = list(
      description        = "Cefquinome PK/PD index: the peak lung concentration of cefquinome divided by MIC(combine), the MIC of cefquinome measured in the presence of the average enrofloxacin lung concentration achieved over 72 h",
      units              = "unitless",
      type               = "continuous",
      ...
      source_name        = "Cmax/MIC (combine) (Wei 2026 Table 5, lung block; Materials and methods, 'Analysis of antibacterial effects and fitting of PK/PD')"
    )
  )
  ...
  ini({
    ...
    # matrix: lung   index: Cmax/MIC (combine)
    # =================================================================
    ...
    #   e0   <- Wei 2026's Emax = 0.077   (effect at zero exposure)
    #   emax <- Wei 2026's E0   = -5.976  (maximal drug effect)
    ...
    e0 <- 0.077
    label("Difference in lung K. pneumoniae load from the blank control at zero exposure (log10 CFU/mL)")  # Wei 2026 Table 6, lung block, Cmax/MIC (combine) column, "E max" = 0.077 (defined in Materials and methods as the blank-control change)
    emax <- -5.976
    label("Maximal difference in lung K. pneumoniae load from the blank control (log10 CFU/mL; negative = reduction)")  # Wei 2026 Table 6, lung block, Cmax/MIC (combine) column, "E 0" = -5.976 (defined in Materials and methods as the maximum antibacterial effect)
    lec50 <- log(555.674)
    label("Log Cmax/MIC (combine) index producing 50% of the maximal antibacterial effect EC50 (unitless)")  # Wei 2026 Table 6, lung block, Cmax/MIC (combine) column, EC 50 = 555.674
    lhill <- log(0.819)
    label("Log Hill coefficient N defining the steepness of the index-effect curve (unitless)")  # Wei 2026 Table 6, lung block, Cmax/MIC (combine) column, Hill's slope = 0.819

    # =================================================================
    # Residual error
    # =================================================================
    # Wei 2026 Table 6 reports the correlation coefficient (R^2 = 0.880)
    # and the AIC (17.757) of this fit and gives no residual standard
