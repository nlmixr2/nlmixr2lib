Gaohua_2012_pregnancy_pbpk_metoprolol <- function() {
  description <- paste(
    "PBPK (whole-body, 14-compartment pregnancy p-PBPK adapted from the",
    "Simcyp Simulator version 11 full-PBPK platform). Metoprolol (predominantly",
    "CYP2D6 substrate; minor CYP3A4 contribution) disposition in healthy",
    "Caucasian women aged 20-40 years, with gestational-age-dependent",
    "maternal physiology and hepatic CYP2D6 / CYP3A4 activity. The 14 ODE compartments are arterial blood, venous blood,",
  ...
  compartmentData <- list(
    ...
    lung     = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    adipose  = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    bone     = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    brain    = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    heart    = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    kidney   = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    gut      = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    liver    = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    muscle   = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    skin     = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    spleen   = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    preg     = list(analyte = "metoprolol", units = "mg", specimen = "tissue", verified = FALSE),
    arterial = list(analyte = "metoprolol", units = "mg", specimen = "blood cell", verified = FALSE),
    venous   = list(analyte = "metoprolol", units = "mg", specimen = "blood cell", verified = FALSE)
  )
  ...
  population <- list(
    ...
    disease_state  = paste(
      ...
      "model file extracts the metoprolol case and is paired with",
      "Gaohua_2012_pregnancy_pbpk_caffeine and",
      "Gaohua_2012_pregnancy_pbpk_midazolam."
    ),
    dose_range     = "Metoprolol 10 mg IV and 100 mg oral single doses (Hogstedt 1985 protocol; Gaohua 2012 Fig 3, Table 5)",
  ...
  ini({
    ...
    # Absorption parameters (Gaohua 2012 Table 3, metoprolol column).
    lka       <- fixed(log(1.45));     label("First-order absorption rate constant (1/h)")                 # Gaohua 2012 Table 3 (metoprolol ka = 1.45 1/h)
    fa        <- fixed(0.88);          label("Fraction available for absorption from dosage form")         # Gaohua 2012 Table 3 (metoprolol fa = 0.88)
    fg        <- fixed(0.99);          label("Gut availability (fraction surviving gut wall first pass)")  # Gaohua 2012 Table 3 (metoprolol Fg = 0.99)
    ...
    fu        <- fixed(0.879);         label("Fraction unbound in plasma (basal, held constant over GA)")  # Gaohua 2012 Table 3 (metoprolol fu = 0.879)
    bp        <- fixed(1.127);         label("Blood:plasma concentration ratio (basal, held constant)")    # Gaohua 2012 Table 3 (metoprolol B:P = 1.127)
    ...
    lcl_int_h <- fixed(log(195));      label("Basal hepatic intrinsic clearance CL_int,H0 (L/h)")          # Gaohua 2012 Table 3 (metoprolol CL_int,H = 195 L/h)
    ...
    a_cyp1a2  <- fixed(0.0);           label("Fractional contribution of CYP1A2 to CL_int,H")              # Gaohua 2012 Table 3 (metoprolol A_1A2 = 0)
    a_cyp2d6  <- fixed(0.93);          label("Fractional contribution of CYP2D6 to CL_int,H")              # Gaohua 2012 Table 3 (metoprolol A_2D6 = 0.93)
    a_cyp3a4  <- fixed(0.07);          label("Fractional contribution of CYP3A4 to CL_int,H")              # Gaohua 2012 Table 3 (metoprolol A_3A4 = 0.07)
    ...
    kp_adipose <- fixed(0.407);   label("Tissue:plasma partition coefficient, adipose")     # Gaohua 2012 Table 4 (metoprolol adipose 0.407)
    kp_bone    <- fixed(1.161);   label("Tissue:plasma partition coefficient, bone")        # Gaohua 2012 Table 4 (metoprolol bone 1.161)
    kp_brain   <- fixed(1.775);   label("Tissue:plasma partition coefficient, brain")       # Gaohua 2012 Table 4 (metoprolol brain 1.775)
    kp_gut     <- fixed(2.524);   label("Tissue:plasma partition coefficient, gut")         # Gaohua 2012 Table 4 (metoprolol gut 2.524)
    kp_heart   <- fixed(2.643);   label("Tissue:plasma partition coefficient, heart")       # Gaohua 2012 Table 4 (metoprolol heart 2.643)
    kp_kidney  <- fixed(2.475);   label("Tissue:plasma partition coefficient, kidney")      # Gaohua 2012 Table 4 (metoprolol kidney 2.475)
    kp_liver   <- fixed(3.379);   label("Tissue:plasma partition coefficient, liver")       # Gaohua 2012 Table 4 (metoprolol liver 3.379)
    kp_lung    <- fixed(1.553);   label("Tissue:plasma partition coefficient, lung")        # Gaohua 2012 Table 4 (metoprolol lung 1.553)
    kp_muscle  <- fixed(2.683);   label("Tissue:plasma partition coefficient, muscle")      # Gaohua 2012 Table 4 (metoprolol muscle 2.683)
    kp_skin    <- fixed(1.36);    label("Tissue:plasma partition coefficient, skin")        # Gaohua 2012 Table 4 (metoprolol skin 1.36)
    kp_spleen  <- fixed(2.732);   label("Tissue:plasma partition coefficient, spleen")      # Gaohua 2012 Table 4 (metoprolol spleen 2.732)
    kp_preg    <- fixed(1.775);   label("Tissue:plasma partition coefficient, fetoplacental unit") # Gaohua 2012 Table 4 footnote (assumed = brain)
