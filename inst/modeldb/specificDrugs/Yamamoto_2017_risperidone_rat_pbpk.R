Yamamoto_2017_risperidone_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Risperidone disposition in plasma and",
    ...
    "CNS data, so the CNS profiles are genuine predictions. The paper notes",
    "that risperidone plasma data were insufficient to support the",
    "two-compartment disposition they appeared to follow, so a",
    "one-compartment model was retained; the resulting plasma",
    "misspecification propagates into a modest under-prediction of brain ECF",
    "and cisterna-magna concentrations (Figure 2)."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "risperidone", units = "ng", specimen = "plasma", verified = TRUE),
    brain_vascular = list(analyte = "risperidone", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "risperidone", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "risperidone", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "risperidone", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "risperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "risperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "risperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "risperidone", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "2 mg/kg (20 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 16 animals, 2 mg/kg (20 min infusion), data",
  ...
  ini({
    ...
    lcl <- log(886); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(43100); label("Central volume of distribution (mL)")  # Table 2 V_PL
    ...
    etalcl ~ 0.526  # Table 2 CV 72.5 percent
    etalvc ~ 0.288  # Table 2 CV 53.7 percent
    ...
    propSd <- 0.472; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional
    addSd <- 0.0244; label("Additive residual error (ng/mL)")  # Table 2 sigma_plasma additive

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 2.5 and molecular weight = 410;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(8.2e-3); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(2.9e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(1.3); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(1.3); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(1.5); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    BF <- fixed(0.49); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
