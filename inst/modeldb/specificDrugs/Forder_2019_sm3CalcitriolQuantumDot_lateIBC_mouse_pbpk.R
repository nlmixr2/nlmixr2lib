Forder_2019_sm3CalcitriolQuantumDot_lateIBC_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of quantum dots conjugated with calcitriol and the SM3 anti-MUC1 antibody (SM3 CalQD) in mice with late-stage (metastatic) inflammatory breast cancer (IBC).",
    "Seven flow-limited, well-stirred compartments: the healthy six plus a `tumor` compartment for delocalized metastatic tumor tissue in parallel with the other organs (Figure 1 right; Eq. 14-16, as implemented in the supplementary MATLAB code).",
    "All volumes and flows are fixed physiology for a 22 g mouse (Table 1);",
    "the only fitted parameters are the kidney, liver, spleen, lung and tumor",
    ...
    "reported, so the model is for typical-value simulation only. The plasma equation follows the authors' supplementary MATLAB code (which produced Table 3) rather than the printed Eq. 14; the code form creates 1-4 percent spurious mass transiently. See the vignette Errata.",
    sep = " "
  )
  reference <- paste(
  ...
  compartmentData <- list(
    plasma = list(
      analyte = "SM3 anti-MUC1 antibody + calcitriol-conjugated quantum dot",
      units = "nM",
      specimen = "plasma",
      verified = TRUE
    ),
  ...
  population <- list(
    ...
    sex_female_pct = 100,
    disease_state = "late-stage (metastatic) inflammatory breast cancer (SUM149 xenograft)",
    dose_range = paste(
      "Single intravenous injection of SM3 CalQD; the model code starts plasma at",
    ...
    notes = paste(
      ...
      "Schaefer 2012 (Forder 2019 Table 2, late-stage tumor column): kidney 11.57, liver 18.12, lung 7.46, spleen 17.92.",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, Late-stage tumor, SM3 CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(46.6); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, SM3 CalQD
    lkp_liver <- log(72.8); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, SM3 CalQD
    lkp_spleen <- log(72.3); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, SM3 CalQD
    lkp_lung <- log(30.1); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, SM3 CalQD
    lkp_tumor <- log(98.1); label("Log tumor:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, SM3 CalQD
  ...
  model({
    ...
    # Supplementary code Tsm3QD_MW(), which produced Table 3: plasma receives
    # q_liver * liver / kp_liver PLUS q_spleen * spleen / kp_spleen. The printed
    # Eq. 14 instead returns (q_liver + q_spleen) * liver / kp_liver, as in the
    # healthy model. The code form is kept because the Table 3 late-stage
    # partition coefficients reproduce the Table 2 data only under it; it adds
    # 1-4 percent spurious mass during the first hours (net creation
    # q_spleen * (spleen / kp_spleen - liver / kp_liver) vanishes once the
    # spleen and liver equilibrate). See the vignette Errata.
    q_total <- q_kidney + q_liver + q_spleen + q_lung + q_other + q_tumor # Eq. 15

    d/dt(plasma) <- (q_kidney * kidney / kp_kidney +
      q_liver * liver / kp_liver +
    ...
    d/dt(tumor) <- q_tumor / v_tumor * (plasma - tumor / kp_tumor) # Eq. 16
