Jeon_2014_piperacillin <- function() {
  ...
  covariateData <- list(
    ...
    DAY_AFTER_INJURY = list(
      ...
      type = "continuous",
      reference_category = NULL,
      notes = "Source column DAI. Continuous integer days since the burn injury (cohort mean 12.8 days, range 2-68 per Jeon 2014 Table 1). Paper-specific covariate (NOT a registered canonical in inst/references/covariate-columns.md) by the maintainers' decision (2026-06-07): treated as a paper-specific column inside this single model only. checkModelConventions() will note the absence of a canonical entry, which is expected. Used as an additive linear effect on CL: TVCL = exp(lcl) * (CRCL / 132) + e_dai_cl * DAY_AFTER_INJURY with e_dai_cl = -0.0874 L/h per day (CL decreases as the hypermetabolic phase resolves). A future continuous-days-since-burn covariate canonical (suggested name POSTBURN_DAYS, proposed for the Han 2013 fluconazole model) may motivate promoting this concept when a second burn-cohort extraction reuses it."
    )
  )
