Saleh_2023_phenobarbital_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, ICR).",
    "Nine-compartment CNS PBPK model predicting unbound phenobarbital",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 10 mg/kg intraperitoneal dose. The CNS structure is the mouse",
    ...
    "0.0121 (estimated by the authors; Table V). The plasma PK model is the",
  ...
  ini({
    ...
    lcl <- log(0.02); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(11.2); label("Central compartment volume (mL)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(232); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(1.4); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(7.14); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.7); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.0121); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (estimated)
    ...
    kpuu_lv <- fixed(0.0121); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.0121); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    # Table IV reports all interindividual variances as 0 for phenobarbital;
    ...
    propSd <- sqrt(0.005); label("Proportional residual error (fraction)")  # Table IV: 0.005 as variance
  })
