Forder_2019_controlQuantumDot_lateIBC_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of unconjugated control quantum dots (ConQD) in mice with late-stage (metastatic) inflammatory breast cancer (IBC).",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, Late-stage tumor, ConQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(41.7); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, ConQD
    lkp_liver <- log(49.3); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, ConQD
    lkp_spleen <- log(88.0); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, ConQD
    lkp_lung <- log(27.3); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, ConQD
    lkp_tumor <- log(24.9); label("Log tumor:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, ConQD
