Bonate_2004_apomine <- function() {
  description <- paste(
    ...
    "in elapsed time (CL = CL0 + CLmax * time^n / (cl_t50^n + time^n);",
  ...
  covariateData <- list(
    ...
    DIS_CANCER = list(
      ...
      notes              = paste(
        ...
        "parameters (CLmax, cl_t50, n) and distribution parameters (Q, Vp)",
  ...
  ini({
    ...
    # on cl_t50 per paper Results paragraph 2).
    # CL(t) = CL0 + CLmax * t^n / (cl_t50^n + t^n)
    # -------------------------------------------------------------------
    lcl_time_max <- log(0.320) ; label("Maximum induction-driven increment in CL/F (L/h)")                                            # Bonate 2004 Table 3: CLmax = 320 mL/h (SE 85.3; variance too small to estimate)
    lcl_t50   <- log(46.4)  ; label("Time to 50% maximal auto-induction (h)")                                                      # Bonate 2004 Table 3: cl_t50 = 46.4 h (SE 17.6; BSV 88%)
    lcl_time_hill  <- log(6.40)  ; label("Hill (shape) exponent for the auto-induction sigmoid (unitless)")                             # Bonate 2004 Table 3: Shape parameter (n) = 6.40 (SE 1.76; variance too small to estimate)
    ...
    etalcl_t50 ~ 0.5594                                                                                                            # Bonate 2004 Table 3: BSV cl_t50 = 88% -> omega^2 = log(0.88^2 + 1) = 0.5594
  ...
  model({
    ...
    #    ~1.4% of CLmax for the typical cl_t50 = 46.4 h and n = 6.40, so
    ...
    cl    <- cl0 + cl_time_max * time^cl_time_hill / (cl_t50^cl_time_hill + time^cl_time_hill)
