Han_2025_sufentanil_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, dynamic age-dependent; custom Phoenix WinNonlin 8.4",
    "implementation). Sufentanil disposition across the human age range,",
    "from young children through adults to the elderly. Fifteen",
    ...
    "effective permeability for sufentanil (Table S1 gives '/'), and every one",
    "of the 12 clinical data sets in Table S11 is an intravenous bolus or",
  ...
  reference <- paste(
    ...
    "S6-S7 and Eq 18-22; clinical data sets in Table S11; validation",
    "targets from Table S15. The deposited",
  ...
  compartmentData <- list(
    lung          = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    kidney        = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    heart         = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    liver         = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    muscle        = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    skin          = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    adipose       = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    brain         = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    venous        = list(analyte = "sufentanil", units = "mg", specimen = "whole blood",  verified = TRUE),
    spleen        = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    arterial      = list(analyte = "sufentanil", units = "mg", specimen = "whole blood",  verified = TRUE),
    other         = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    stomach       = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    duodenum      = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    jejunum       = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    ileum         = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    cecum         = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    colon         = list(analyte = "sufentanil", units = "mg", specimen = "administration site",  verified = TRUE),
    wall_stomach  = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    wall_duodenum = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    wall_jejunum  = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    wall_ileum    = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    wall_cecum    = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE),
    wall_colon    = list(analyte = "sufentanil", units = "mg", specimen = "tissue", verified = TRUE)
  )
  ...
  population <- list(
    ...
    n_subjects     = 1000L,
    n_studies      = 12L,
    age_range      = "4.8 years (children) to 77 years",
    weight_range   = "18.8 kg (children) to 87 kg (adults)",
    disease_state  = paste(
      "Children under elective anaesthesia and surgery, adults in general, abdominal and neurosurgery, and elderly aortocoronary-bypass and neurosurgery patients.",
      "Han 2025 Table S11 lists 12 sufentanil data sets digitised from the",
      "published literature (3 paediatric, 5 adult, 4 geriatric);",
      "Table S15 gives the observed and predicted AUC0-t for each."
    ),
    dose_range     = paste(
      "1.74-15 ug/kg intravenous bolus, a 150 ug fixed dose, and 2-30 ug/kg/min infusions over 0.5-7 min"
    ),
    ...
    notes          = paste(
      ...
      "Table S15 reports observed and predicted AUC0-t only; no Cmax column is given for sufentanil.",
  ...
  ini({
    # ================= SUFENTANIL drug-specific parameters =================
    # All from Han 2025 Table S1, sufentanil column. The Kt:p values of fentanyl,
    # alfentanil and sufentanil were calculated by the method of Table S1
    # reference [21] from pKa 8 and logP 3.95.
    kp_lung    <- fixed(3.57);        label("sufentanil lung:plasma partition coefficient Kt:p (unitless)")      # Table S1, Lung
    kp_kidney  <- fixed(2.69);        label("sufentanil kidney:plasma partition coefficient Kt:p (unitless)")    # Table S1, Kidney
    kp_heart   <- fixed(2.43);        label("sufentanil heart:plasma partition coefficient Kt:p (unitless)")     # Table S1, Heart
    kp_liver   <- fixed(2.95);        label("sufentanil liver:plasma partition coefficient Kt:p (unitless)")     # Table S1, Liver
    kp_muscle  <- fixed(1.75);        label("sufentanil muscle:plasma partition coefficient Kt:p (unitless)")    # Table S1, Muscle
    kp_skin    <- fixed(8.31);        label("sufentanil skin:plasma partition coefficient Kt:p (unitless)")      # Table S1, Skin
    kp_adipose <- fixed(114.55);      label("sufentanil adipose:plasma partition coefficient Kt:p (unitless)")   # Table S1, Adipose
    kp_brain   <- fixed(5.41);        label("sufentanil brain:plasma partition coefficient Kt:p (unitless)")     # Table S1, Brain
    kp_spleen  <- fixed(1.60);        label("sufentanil spleen:plasma partition coefficient Kt:p (unitless)")    # Table S1, Spleen
    kp_stomach <- fixed(5.70);        label("sufentanil stomach:plasma partition coefficient Kt:p (unitless)")   # Table S1, Stomach
    kp_gut     <- fixed(5.70);        label("sufentanil intestine:plasma partition coefficient Kt:p (unitless)") # Table S1, Intestine (all five gut-wall segments)
    kp_other   <- fixed(0.001);   label("sufentanil rest-of-body:plasma partition coefficient Kt:p (unitless)")  # Table S1, ROB (footnote a: assumed, not calculated)
    lbpr       <- fixed(log(0.74));   label("sufentanil blood:plasma concentration ratio Rb (log, unitless)")         # Table S1, Rb [ref 11]
    lfu_p      <- fixed(log(0.075));  label("sufentanil fraction unbound in adult plasma (log, unitless)")            # Table S1, fu,p [ref 11]
    lcl_int_h  <- fixed(log(0.934));  label("sufentanil CYP3A4 hepatic intrinsic clearance (log mL/min/mg microsomal protein)")  # Table S1, CLint [ref 20]; back-calculated from in vivo hepatic clearance via Eq S7 (Table S1 footnote)
    # Table S1 reports "/" for the sufentanil effective permeability: Han 2025
    # never simulate an oral sufentanil dose, and every data set in Table S11 is
    ...
    peff       <- fixed(0);       label("sufentanil effective intestinal permeability Peff (cm/min)")           # Table S1: not reported (intravenous administration only)
    ...
    # mg-equivalents are 87.4036 / 346.0154 / 201.8252. The sufentanil values
    # below are that constant times the Table S1 sufentanil CLint,l of 0.934:
    # DERIVED, not tabulated. See the vignette Errata.
    lcl_gw1    <- fixed(log(81.634962));  label("sufentanil duodenal wall CYP3A4 intrinsic clearance (log mL/min)")  # 87.4036 x 0.934
    lcl_gw2    <- fixed(log(323.178384));  label("sufentanil jejunal wall CYP3A4 intrinsic clearance (log mL/min)")   # 346.0154 x 0.934
    lcl_gw3    <- fixed(log(188.504737));  label("sufentanil ileal wall CYP3A4 intrinsic clearance (log mL/min)")     # 201.8252 x 0.934
    ...
    #
    # Peff carries no random effect in this file: Table S1 reports no sufentanil
    ...
    propSd     <- fixed(0.10);    label("Proportional residual error placeholder, sufentanil (fraction)")    # not reported by Han 2025
  })
  ...
  model({
    ...
    #    level of plasma ALB (or AAG)"). Sufentanil is a lipophilic base
    #    (Table S1 pKa 8, logP 3.95) and binds predominantly to
    ...
    # via 1e6, matching the ng/mL*min AUC units of Table S15.
    Cc <- c_venous / bpr * 1e6
    Cc ~ prop(propSd)
  })
}
