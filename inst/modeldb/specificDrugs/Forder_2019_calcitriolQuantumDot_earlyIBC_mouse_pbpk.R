Forder_2019_calcitriolQuantumDot_earlyIBC_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of calcitriol-conjugated quantum dots (CalQD) in mice with early-stage inflammatory breast cancer (IBC).",
  ...
  compartmentData <- list(
    ...
    tumor = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE),
    breast = list(analyte = "calcitriol-conjugated quantum dot", units = "nM", specimen = "tissue", verified = TRUE)
  )
  ...
  population <- list(
    ...
    notes = paste(
      ...
      "Schaefer 2012 (Forder 2019 Table 2, late-stage tumor column): kidney 15.15, liver 20.15, lung 7.59, spleen 21.46.",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, Early-stage tumor, CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(91.2); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
    lkp_liver <- log(121.2); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
    lkp_spleen <- log(129.1); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
    lkp_lung <- log(45.7); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
    lkp_tumor <- log(45.5); label("Log tumor:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
    lkp_breast <- log(1.71); label("Log breast:plasma partition coefficient (log unitless)") # Table 3, Early-stage tumor, CalQD
