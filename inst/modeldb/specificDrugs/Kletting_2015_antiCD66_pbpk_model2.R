Kletting_2015_antiCD66_pbpk_model2 <- function() {
  description <- paste(
    ...
    "spleen numbers through granulocyte-pool ratios. Model 2 constrains the blood antigen number to the liver plus spleen sum weighted by each organ's measured over calculated volume (S1 Text Eq 9), compensating for the hepatomegaly and splenomegaly common in acute leukaemia.",
    ...
    "shipped values are the cohort means of Table 1 for Model 2. Amounts",
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "SDs of the individual estimates for Model 2 were AgL 0.27, AgS 0.30, exL 0.091, exS 0.038, fRM 0.24 %, cRM 0.34, lambda_db 1.7e-5 /min, r_im 0.098 and V 0.63 L;",
  ...
  ini({
    # ---- Fitted parameters, Table 1 cohort means (Model 2) ---------------
    ag0Liver <- fixed(0.33) ; label("Total CD66 antigen number in the liver, Ag0,L (nmol)")      # Table 1
    ag0Spleen <- fixed(0.25) ; label("Total CD66 antigen number in the spleen, Ag0,S (nmol)")    # Table 1
    exLiver <- fixed(0.226) ; label("Liver fraction of the extravascular delay compartment, exL (unitless)")    # Table 1
    exSpleen <- fixed(0.098) ; label("Spleen fraction of the extravascular delay compartment, exS (unitless)") # Table 1
    fRm <- fixed(0.0073) ; label("Fraction of total plasma flow reaching the red marrow, fRM (unitless)")      # Table 1 reports fRM in per cent
    cRm <- fixed(1.20) ; label("Individual correction of the L2-L4 to whole red marrow scaling factor, cRM (unitless)") # Table 1
    ...
    rIm <- fixed(0.806) ; label("Probability that one antibody arm is immunoreactive, r_im (unitless)")  # Table 1
    vSerum <- fixed(3.00) ; label("Total serum volume, V (L)")                                        # Table 1
    # ---- Model 2 organ-size weights (S1 Text Eq 9) -----------------------
  ...
  model({
    ...
    # Eq 9 (Model 2): the same constraint, each organ weighted by its measured
    # over calculated volume. See the Model 1 sibling for why the printed
    # '* 0.9' is encoded as '/ 0.9'.
    ag0Plasma <- (ag0Liver * ratioVolLiver + ag0Spleen * ratioVolSpleen) /
      agBloodOrganFrac
