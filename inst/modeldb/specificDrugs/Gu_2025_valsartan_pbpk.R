Gu_2025_valsartan_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 24-state, custom Phoenix WinNonlin 8.4",
    "implementation). valsartan disposition in healthy adults and in",
    "chronic heart failure across NYHA classes II, III and IV. valsartan is",
    "a angiotensin II receptor antagonist eliminated by biliary and renal excretion (Supplementary Section 2.6).",
  ...
  reference <- paste(
    ...
    "from Supplementary Table S1; valsartan parameters from Supplementary",
  ...
  ini({
    ...
    fu_p       <- fixed(0.05);    label("Fraction unbound in plasma (unitless)")  # Table S2 fu,plasma [26] Poirier 2009
    bpr        <- fixed(1);    label("Blood:plasma concentration ratio Rb (unitless)")  # Table S2 Rbp assumed (footnote c)
    # Table S2 gives fu,g = 1 assumed (footnote c), but valsartan has no
    ...
    lcl_renal  <- fixed(log(0.0103));  label("Renal plasma clearance CLrenal (log L/min)")  # Table S2 CLrenal 10.3 mL/min [32] Flesch 1997
    lcl_liver  <- fixed(log(0.0262));  label("Hepatic plasma clearance CLliver (log L/min)")  # Table S2 CLliver 26.2 mL/min [32] Flesch 1997

    # ---- Absorption ----
    # ka is tabulated directly (Eq S15 not needed) and applies to all
    # three absorbing segments.
    lka        <- fixed(log(0.008));  label("First-order intestinal absorption rate constant ka (log 1/min)")  # Table S2 ka 0.008 1/min footnote b (simulated by WinNonlin)

    # ---- Tissue:plasma partition coefficients (Supplementary Table S2) ----
    # footnote f (method of [38])
    kp_adipose   <- fixed(0.045   ); label("Adipose:plasma partition coefficient (unitless)")  # Table S2 KAdipose:plasma
    kp_liver     <- fixed(0.14    ); label("Liver:plasma partition coefficient (unitless)")  # Table S2 KLiver:plasma
    kp_muscle    <- fixed(0.19    ); label("Muscle:plasma partition coefficient (unitless)")  # Table S2 KMuscle:plasma
    kp_lung      <- fixed(0.33    ); label("Lung:plasma partition coefficient (unitless)")  # Table S2 KLung:plasma
    kp_kidney    <- fixed(0.14    ); label("Kidney:plasma partition coefficient (unitless)")  # Table S2 KKidney:plasma
    kp_stomach   <- fixed(0.16    ); label("Stomach:plasma partition coefficient (unitless)")  # Table S2 Kstomach:plasma
    kp_other     <- fixed(0.01    ); label("Rest-of-body:plasma partition coefficient (unitless)")  # Table S2 KROB:plasma
    kp_brain     <- fixed(0.1     ); label("Brain:plasma partition coefficient (unitless)")  # Table S2 KBrain:plasma
    kp_heart     <- fixed(0.12    ); label("Heart:plasma partition coefficient (unitless)")  # Table S2 KHeart:plasma
    kp_spleen    <- fixed(0.16    ); label("Spleen:plasma partition coefficient (unitless)")  # Table S2 KSpleen:plasma
    kp_skin      <- fixed(0.14    ); label("Skin:plasma partition coefficient (unitless)")  # Table S2 KSkin:plasma
    kp_gut       <- fixed(0.16    ); label("Intestinal-wall:plasma partition coefficient (unitless)")  # Table S2 Kgut:plasma
