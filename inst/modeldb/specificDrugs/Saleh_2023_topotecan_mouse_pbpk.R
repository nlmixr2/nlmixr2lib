Saleh_2023_topotecan_mouse_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK3.0 CNS physiologically-based model, mouse version).",
    "Preclinical (mouse, CD1 nude).",
    "Nine-compartment CNS PBPK model predicting unbound topotecan",
    "concentrations in brain extracellular fluid (brain ECF) after a",
    "single 4 mg/kg intravenous dose. The CNS structure is the mouse",
    ...
    "0.21 (taken from ref [57]; Table V). The plasma PK model is the",
  ...
  compartmentData <- list(
    central              = list(analyte = "topotecan", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1          = list(analyte = "topotecan", units = "ng", specimen = "plasma", verified = TRUE),
    brain_vascular       = list(analyte = "topotecan", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf            = list(analyte = "topotecan", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_cell_membrane  = list(analyte = "topotecan", units = "ng", specimen = "tissue", verified = TRUE),
    brain_icf            = list(analyte = "topotecan", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome       = list(analyte = "topotecan", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv         = list(analyte = "topotecan", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv        = list(analyte = "topotecan", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm         = list(analyte = "topotecan", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas        = list(analyte = "topotecan", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range   = "4 mg/kg IV (single dose; Table IV)",
    notes        = paste(
      "Unbound plasma and microdialysis brain-ECF concentration-time",
      "data for topotecan came from Stewart lab, St. Jude Children's Research Hospital",
  ...
  ini({
    ...
    lcl <- log(1.41); label("Central clearance (mL/min)")  # Table IV
    lvc <- log(8.41); label("Central compartment volume (mL)")  # Table IV
    lq  <- log(0.486); label("Intercompartmental clearance (mL/min)")  # Table IV
    lvp <- log(18.36); label("Peripheral compartment volume (mL)")  # Table IV

    # ---- Drug physicochemical properties: Table I ----
    mwt  <- fixed(421); label("Molecular weight (g/mol)")  # Table I
    logp <- fixed(1.84); label("Lipophilicity, logP (unitless)")  # Table I (ALOGPS)
    pka  <- fixed(8); label("Acidic ionization constant (unitless)")  # Table I (Chemaxon)
    pkb  <- fixed(9.75); label("Basic ionization constant (unitless)")  # Table I (Chemaxon)

    # ---- Drug biological properties: Tables II and V ----
    fu_plasma <- fixed(0.3); label("Unbound fraction in plasma (unitless)")  # Table II
    kpuu_ecf  <- fixed(0.21); label("Kp,uu,BBB: unbound brain-ECF-to-plasma ratio (unitless)")  # Table V (ref [57])
    ...
    kpuu_lv <- fixed(0.21); label("Kp,uu at the BCSFB, lateral ventricles (unitless)")  # assumed = Kp,uu,BBB
    kpuu_cm <- fixed(0.21); label("Kp,uu at the BCSFB, cisterna magna (unitless)")  # assumed = Kp,uu,BBB
    ...
    etalcl ~ 0.011  # Table IV IIV on central clearance (variance)

    # ---- Residual unexplained variability: Table IV (reported as VARIANCES) ----
    propSd <- sqrt(0.051); label("Proportional residual error (fraction)")  # Table IV: 0.051 as variance
  })
