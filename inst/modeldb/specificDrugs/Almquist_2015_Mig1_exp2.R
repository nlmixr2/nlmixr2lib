Almquist_2015_Mig1_exp2 <- function() {
  description <- paste(
    ...
    "EXPERIMENT 2 (4% -> 1.5% glucose; a biological replicate of experiment",
    "1). Nuclear Mig1 (observed as Mig1-GFP light intensity) is produced at a",
    "glucose-proportional rate and removed at a rate modulated by a lumped,",
    "unmeasured adaptation component X; the circuit adapts perfectly, so",
    "nuclear Mig1 returns to its basal level regardless of the post-shift",
    "glucose level. Cell-to-cell variability is carried by a full 3x3",
    "log-normal random-effect block on the basal Mig1 level and the two rate",
    "constants. One of four per-experiment fits;",
    "see modellib('Almquist_2015_Mig1_exp1') (4% -> 1.5%),",
  ...
  population <- list(
    ...
    notes          = paste(
      ...
      "four (s = 38.1 x 10^3) and the largest eta-shrinkage (S1 Table); the",
      "paper notes that two of its shrinkage percentages approach the 20-30%",
      "advisory limit but judges the set as a whole feasible. It is also the",
      "experiment where the standard two-stage approach failed most badly,",
      "overestimating Var[k2] by a factor of 88 when all cells were retained",
      "(Table 4)."
    )
  )
  ...
  ini({
    # =====================================================================
    # Fixed effects -- Almquist 2015 Table 2, column "Exp 2".
    ...
    # Almquist 2015 Table 2, Exp 2: Ms = 3.36 x 10^3 (RSE 1%).
    ...
    lkout_mig1 <- log(0.00473)
    label("First-order loss rate constant of nuclear Mig1, k2 (1/s)")
    # Almquist 2015 Table 2, Exp 2: k2 = 0.00473 /s (RSE 6%).
    ...
    # Almquist 2015 Table 1, Exp Nr 2, column "To": 1.5%.

    # =====================================================================
    # Random effects -- Almquist 2015 Table 3, Exp Nr 2 (the covariance
    ...
    # Substituting the Table 2 Exp 2 values
    #   w11 = 0.0712, w12 = 0.0447, w13 = -0.0377,
    #   w22 = 0.144,  w23 = 0.193,  w33 = 0.439
    # reproduces every printed entry of Table 3 Exp Nr 2 exactly:
    #   Omega11 = 0.0084888 (Table 3: 0.0085)
    #   Omega21 = -0.0008393 (Table 3: -840 x 10^-6)
    #   Omega22 = 0.0579850 (Table 3: 0.058)
    #   Omega31 = -0.0165503 (Table 3: -0.017)
    #   Omega32 = 0.0847270 (Table 3: 0.085)
    #   Omega33 = 0.1927210 (Table 3: 0.19)
    # and the implied correlations -0.038 (eta1,eta2), -0.41 (eta1,eta3) and
    # 0.80 (eta2,eta3) match the Table 3 correlation block. The construction
    # Omega = U %*% t(U) guarantees positive definiteness.
    # =====================================================================

    etalrbase + etalkout_mig1 + etalkout_adapt ~
      c( 0.0084888200,
        -0.0008393000, 0.0579850000,
        -0.0165503000, 0.0847270000, 0.1927210000)

    # =====================================================================
    # Residual error -- Almquist 2015 Table 2, row "s", Exp 2.
    # =====================================================================

    addSd_mig1 <- sqrt(38.1e3)
    label("Additive residual SD on nuclear Mig1 intensity (a.u.)")
    # Almquist 2015 Table 2, Exp 2: s = 38.1 x 10^3 (RSE 6%). The paper
    # defines the observation model as y_t = Mig1(t) + e_t with
    # "e_t ~ N(0, s), with s denoting the VARIANCE of the measurement
    # error", so the nlmixr2 additive SD is sqrt(s) = 195.2 a.u.
