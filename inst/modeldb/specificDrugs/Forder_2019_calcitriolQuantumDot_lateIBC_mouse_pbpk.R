Forder_2019_calcitriolQuantumDot_lateIBC_mouse_pbpk <- function() {
  description <- paste(
    ...
    "single intravenous dose of calcitriol-conjugated quantum dots (CalQD) in mice with late-stage (metastatic) inflammatory breast cancer (IBC).",
  ...
  ini({
    ...
    # ================================================================
    # Partition coefficients -- Table 3, Late-stage tumor, CalQD column
    # (relative residual sum of squares fit; no uncertainty reported)
    # ================================================================
    lkp_kidney <- log(74.1); label("Log kidney:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, CalQD
    lkp_liver <- log(98.3); label("Log liver:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, CalQD
    lkp_spleen <- log(105.0); label("Log spleen:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, CalQD
    lkp_lung <- log(37.1); label("Log lung:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, CalQD
    lkp_tumor <- log(23.8); label("Log tumor:plasma partition coefficient (log unitless)") # Table 3, Late-stage tumor, CalQD
