Yamamoto_2017_atenolol_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Atenolol disposition in plasma and nine",
    ...
    "pH-dependent factors. The plasma side is the empirical one-compartment",
    ...
    "CNS data, so the CNS profiles are genuine predictions. One-compartment",
    "plasma model with no interindividual variability: Table 2 reports every",
    "atenolol omega as NA. See the vignette Errata for the two places where",
    "the supplementary control stream S4 disagrees with Table 2."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    central = list(analyte = "atenolol", units = "ng", specimen = "plasma", verified = TRUE),
    brain_vascular = list(analyte = "atenolol", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "atenolol", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "atenolol", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "atenolol", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "atenolol", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "atenolol", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "atenolol", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "atenolol", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "10 mg/kg (1 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 5 animals, 10 mg/kg (1 min infusion), data",
      "from reference 25 (de Lange 1994). Plasma and brain microdialysis (brain",
      "extracellular fluid and CSF) samples were collected after femoral-vein",
      "administration; see Supplementary Material S2 for surgery and",
      "bioanalysis. Concentrations in the CNS compartments are unbound",
      "concentrations."
    )
  )
  ...
  ini({
    ...
    lcl <- log(7.13); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(256); label("Central volume of distribution (mL)")  # Table 2 V_PL
    ...
    propSd <- 0.486; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 0.2 and molecular weight = 266;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(5.7e-5); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(3.5e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(40); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(82); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(110); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    PHF7 <- fixed(0.004); label("pH-dependent factor, lysosome to brain intracellular fluid (unitless)")  # Table 5 PHF7
    ...
    BF <- fixed(0.92); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
