Nielsen_2011_erythromycin <- function() {
  description <- "In vitro (Streptococcus pyogenes M12 NCTC P1800). Semimechanistic PKPD model of erythromycin time-kill kinetics; two-stage bacterial life-cycle (proliferating drug-sensitive S and non-growing drug-insensitive R) with sigmoidal Emax killing of S via an effect compartment; first-order drug elimination (ke set per in vitro kinetic-system flow rate); drug-specific degradation kdeg fixed at zero. Parameter values are from the combined static and dynamic estimation in Table 3."
  reference <- paste(
  ...
  ini({
    ...
    # =============================================================
    # Drug-specific PK parameters (erythromycin)
    # =============================================================
    # Methods: "For the other drugs, the kdeg was set to zero."
    # Erythromycin is one of these "other drugs" (no detectable
    ...
    # half-life (Table 1 dynamic-erythromycin column); override via
    ...
    label("Log in vitro kinetic-system elimination rate ke (1/h; default ln(2)/1.7 = human half-life; override per experiment)")  # Table 1: simulated human half-life for erythromycin = 1.7 h

    # =============================================================
    # Drug-specific PD parameters (erythromycin; Table 3)
    # =============================================================
    lemax <- log(2.46)
    label("Log maximum killing rate Emax (1/h)")                                          # Table 3 "Static and dynamic" column: Emax ERY = 2.46 (RSE 13%)
    lec50 <- log(0.0336)
    label("Log effect-compartment concentration for 50% of Emax (mg/L)")                  # Table 3 "Static and dynamic" column: EC50 ERY = 0.0336 (RSE 44%)
    lhill <- log(0.652)
    label("Log Hill (sigmoidicity) exponent gamma on the killing function (unitless)")    # Table 3 "Static and dynamic" column: gamma ERY = 0.652 (RSE 20%)
    lke0  <- log(1.02)
    label("Log effect-compartment delay rate ke0 (1/h)")                                  # Table 3 "Static and dynamic" column: ke0 ERY = 1.02 (RSE 21%)
