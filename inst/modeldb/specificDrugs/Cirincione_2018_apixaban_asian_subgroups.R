Cirincione_2018_apixaban_asian_subgroups <- function() {
  description <- "Population PK model for apixaban in subjects with nonvalvular atrial fibrillation (Cirincione 2018, stage 2 final ad hoc model): as the stage 2 final model, but with the single Asian-race clearance effect stratified into separate Japanese, Korean and other-Asian effects."
  ...
  covariateData <- list(
    ...
    SEXF = list(
      ...
      notes = "Fractional-change effect on the nonrenal clearance arm; female subjects have 22.7 percent lower CL_NR/F than male in this ad hoc fit. Source column SEX is coded 1 = male, 2 = female and the control stream derives 'FEM=0; IF (SEX.EQ.2) FEM=1', matching the canonical orientation with no sign inversion.",
      source_name = "SEX"
    ),
    RACE_JAPANESE = list(
      ...
      units = "(binary)",
      type = "binary",
      reference_category = "0 (non-Asian, i.e. RACE_JAPANESE = RACE_KOREAN = RACE_ASIAN_OTH = 0)",
      notes = "Fractional-change effect on total apparent clearance, -15.1 percent relative to a non-Asian subject. Japanese subjects were 9.14 percent of the analysis dataset (Table 2). Derived in the control stream from the four-level ASIAN column as 'JPN=0; IF (ASIAN.EQ.1) JPN=1'. The three Asian indicators are mutually exclusive.",
      source_name = "ASIAN"
    ),
    RACE_KOREAN = list(
      ...
      type = "binary",
      reference_category = "0 (non-Asian, i.e. RACE_JAPANESE = RACE_KOREAN = RACE_ASIAN_OTH = 0)",
      notes = "Fractional-change effect on total apparent clearance, +3.25 percent relative to a non-Asian subject. Korean subjects were 1.57 percent of the analysis dataset (Table 2) and are the only Asian subgroup whose effect is not statistically distinguishable from zero (0.0325 +/- 0.0753). Derived in the control stream as 'KOR=0; IF (ASIAN.EQ.2) KOR=1'.",
      source_name = "ASIAN"
    ),
    RACE_ASIAN_OTH = list(
      description = "Other-Asian race indicator (Asian heritage other than Japanese or Korean)",
      units = "(binary)",
      type = "binary",
      reference_category = "0 (non-Asian, i.e. RACE_JAPANESE = RACE_KOREAN = RACE_ASIAN_OTH = 0)",
      notes = "Fractional-change effect on total apparent clearance, -4.27 percent relative to a non-Asian subject. Other-Asian subjects were 4.15 percent of the analysis dataset (Table 2). Derived in the control stream as 'OTHRASIA=0; IF (ASIAN.EQ.3) OTHRASIA=1'. The dominant reference grouping here is non-Asian (predominantly White, 83.22 percent of the dataset) rather than an Asian subgroup, which differs from the osimertinib-program models that also use this canonical.",
      source_name = "ASIAN"
    ),
    DIS_NVAF = list(
  ...
  population <- list(
    ...
    race_asian_detail = c(Japanese = 9.14, Korean = 1.57, `Other Asian` = 4.15),
    ...
    notes = "Ad hoc refinement of the stage 2 final model, fit to the same 11,968 concentrations to support global development by resolving the single Asian-race clearance effect into Japanese, Korean and other-Asian effects. OFV -5,900.112 vs -5,888.403 for the stage 2 final model. Typical reference subject used for the paper's covariate displays: non-Asian male with NVAF, aged 65 years, 70 kg, cCrCL 80 mL/min, no concomitant CYP3A4/P-gp inhibitor. Median covariates used for the paper's typical Japanese NVAF predictions (Figure 3): 64.1 kg, 70 years, cCrCL 65.1 mL/min."
  )

  ini({
    # Structural parameters: Table 3, 'stage 2 final ad hoc model' column.
    lka <- log(0.471)
    ...
    # Covariate effects: Table 3, 'stage 2 final ad hoc model' column;
    # functional forms from Table S2 Eqs. [1a]-[1e], which are written for
    # exactly this ad hoc parameterisation, and Supplemental File S2.
    e_dosetime_evening_ka <- -0.436
    ...
    e_age_cl_nonren <- -0.414
    label("Power exponent on (AGE / 65) for CLNR/F (theta14, unitless)")
    e_sexf_cl_nonren <- -0.227
    label("Fractional change in CLNR/F for female vs male (theta15, unitless)")
    e_race_japanese_cl <- -0.151
    label("Fractional change in total CL/F for Japanese vs non-Asian (theta24, unitless)")
    e_race_korean_cl <- 0.0325
    label("Fractional change in total CL/F for Korean vs non-Asian (theta25, unitless)")
    e_race_asian_oth_cl <- -0.0427
    label("Fractional change in total CL/F for other Asian vs non-Asian (theta16, unitless)")
    e_dis_nvaf_cl <- -0.145
    ...
    e_conmed_cyp3a4_pgp_inh_cl <- -0.140
    ...
    logitifdepot50 <- -0.327
    label("Logit of the fractional reduction in relative bioavailability at a 50-mg dose (I50, theta9, unitless)")

    # IIV: Table 3, 'stage 2 final ad hoc model' column. Exponential on the
    # microconstants of the ADVAN4/TRANS1 parameterisation, diagonal OMEGA
    # ('$OMEGA DIAGONAL(5)' in Supplemental File S2).
    etalka ~ 0.264 # Table 3, stage 2 final ad hoc, 'omega2-ka' = 0.264 (51.4 %CV)
    etalkel ~ 0.0961 # Table 3, stage 2 final ad hoc, 'omega2-k' = 0.0961 (31.0 %CV)
    etalvc ~ 0.0290 # Table 3, stage 2 final ad hoc, 'omega2-Vc/F' = 0.0290 (17.0 %CV)
    etalk21 ~ 0.241 # Table 3, stage 2 final ad hoc, 'omega2-k21' = 0.241 (49.1 %CV)
    etalk12 ~ 1.55 # Table 3, stage 2 final ad hoc, 'omega2-k12' = 1.55 (124.5 %CV)

    # Residual error: Table 3, 'stage 2 final ad hoc model' column. The
    # source fits log-transformed observations with an additive error on the
    # log scale and SIGMA fixed to 1, so these thetas ARE the log-scale SDs.
    expSdHvJapan <- 0.31
    label("Log-scale residual SD for phase I healthy volunteers and the Japan NVAF / Japan ACS phase II studies (theta21)")
    expSdAppraise1 <- 0.667
    label("Log-scale residual SD for the APPRAISE-1 phase II study (theta22)")
    expSdAristotle <- 0.457
  ...
  model({
    ...
    # Total apparent clearance, Table S2 Eq. [1d]. The three mutually
    # exclusive Asian indicators replace the single Asian effect of the
    # stage 2 final model; all three zero selects the non-Asian reference.
    cl <- (cl_renal + cl_nonren) *
      (1 + e_race_asian_oth_cl * RACE_ASIAN_OTH) *
      (1 + e_race_japanese_cl * RACE_JAPANESE) *
      (1 + e_race_korean_cl * RACE_KOREAN) *
