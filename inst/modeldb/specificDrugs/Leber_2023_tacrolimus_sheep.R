Leber_2023_tacrolimus_sheep <- function() {
  ...
  compartmentData <- list(
    ...
    adsorbed       = list(analyte = "tacrolimus", units = "mg", specimen = "not applicable", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "Tacrolimus 10-12 mg orally twice daily, individually titrated to a whole-blood target of 6-8 ng/mL (Table 1). Given in combination with mycophenolate mofetil 1 g twice daily and prednisolone 10 mg (group 1).",
  ...
  ini({
    ...
    lcl     <- log(132);       label("Apparent oral clearance (CL/F, L/h)")                     # Leber 2023 Supplementary Table 2 (final adsorption model)
    lvc     <- fixed(log(6330)); label("Apparent central volume of distribution (V1/F, L)")     # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    lka     <- fixed(log(4.8));  label("First-order oral absorption rate constant (KA, 1/h)")   # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    ltlag   <- fixed(log(15.3)); label("Absorption lag time (ALAG, h)")                         # Leber 2023 Supplementary Table 1 (fixed in the step-2 adsorption model)
    ...
    lclmax_hemoadsorption <- log(4.02);   label("Maximum CytoSorb hemoadsorption clearance at zero saturation (CLmax, L/h)")  # Leber 2023 Supplementary Table 2
    lamax_hemoadsorption  <- log(0.040);  label("Maximum drug amount adsorbable by the CytoSorb cartridge (Amax, mg)")        # Leber 2023 Supplementary Table 2
    ...
    #   CL/F : 22.9% CV -> log(0.229^2 + 1) = 0.0511122
    #   V1/F : 41.1% CV -> log(0.411^2 + 1) = 0.1560811
    etalcl ~ fixed(0.0511122)   # Leber 2023 Supplementary Table 1 (IIV CL/F, 22.9% CV)
    etalvc ~ fixed(0.1560811)   # Leber 2023 Supplementary Table 1 (IIV V/F, 41.1% CV)

    # Residual error on the pre-adsorber (inlet) samples, which are the
    # systemic whole-blood concentrations this model predicts.
    propSd <- 0.156; label("Proportional residual error, pre-adsorber samples (fraction)")  # Leber 2023 Supplementary Table 2 (Prop. error pre, 15.6% CV)
    propSd_Cpostfilter <- 0.088; label("Proportional residual error, post-adsorber samples (fraction)")  # Leber 2023 Supplementary Table 2 (Prop. error post, 8.8% CV)
  })
  model({
    ka     <- exp(lka)
    cl     <- exp(lcl + etalcl)
