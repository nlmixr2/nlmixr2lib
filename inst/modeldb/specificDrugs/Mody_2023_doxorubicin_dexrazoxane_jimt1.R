Mody_2023_doxorubicin_dexrazoxane_jimt1 <- function() {
  ...
  ini({
    ...
    lkgrow    <- log(0.0171)       ; label("First-order cell growth rate constant kg (1/h)")                                # Table 2: 0.0171 (1.02% RSE)

    lkmax_dox <- log(0.0705)    ; label("Maximal DOX killing rate constant Smax,DOX (1/h)")                              # Table 2: 0.0705 (15.6% RSE)
    lkc50_dox <- log(0.315)    ; label("DOX concentration for half-maximal killing rate SC50,DOX (uM)")                 # Table 2: 0.315 (18.5% RSE)
    lktr_dox  <- log(0.0703)     ; label("DOX cell-death transit rate constant 1/tau_DOX (1/h)")                          # Table 2: 0.0703 (9.9% RSE); tau_DOX = 14.2 h

    lkmax_dexrazoxane <- log(0.0482) ; label("Maximal DEX killing rate constant Smax,DEX (1/h)")                         # Table 2: 0.0482 (20.2% RSE)
    lkc50_dexrazoxane <- log(21.2) ; label("DEX concentration for half-maximal killing rate SC50,DEX (uM)")            # Table 2: 21.2 (18.8% RSE)
    lktr_dexrazoxane  <- log(0.0335)  ; label("DEX cell-death transit rate constant 1/tau_DEX (1/h)")                     # Table 2: 0.0335 (11.9% RSE); tau_DEX = 29.9 h

    # Table 2 prints psi only to one significant figure, as "~2"
    # (%RSE 16.3); no more precise value exists in the paper or its
    # supplement. Encoded as the printed value. psi = 1 recovers the
    # single-agent DOX model of Eqs 3a-3e.
    lpsi <- log(2) ; label("DOX-DEX interaction parameter psi (unitless; >1 antagonistic, <1 synergistic, =1 additive)")  # Table 2: ~2 (16.3% RSE)
