Okada_2025_rocatinlimab <- function() {
  ...
  ini({
    ...
    lcl     <- log(0.230);   label("Linear (nonspecific) clearance for a 70 kg AD/UC patient with albumin 44 g/L, non-psoriasis (L/day)")
    lvc     <- log(3.30);    label("Central volume of distribution for a 70 kg subject (L)")
    lvp     <- log(2.82);    label("Peripheral volume of distribution (L)")
    lq      <- log(0.775);   label("Intercompartmental clearance (L/day)")
    lvmax   <- log(0.968);   label("Maximum velocity of saturable Michaelis-Menten clearance for a 70 kg patient (mg/day)")
    lkm     <- log(0.289);   label("Michaelis-Menten constant of rocatinlimab serum concentration (ug/mL)")
    lkdes   <- log(0.00439); label("First-order rate of decline of Vmax over time (1/day)")
    lka     <- log(0.312);   label("First-order absorption rate after subcutaneous administration (1/day)")
    lfdepot <- log(0.855);   label("Subcutaneous bioavailability (fraction)")
