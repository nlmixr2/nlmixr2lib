Fukae_2024_valemetostat_dose_reduction <- function() {
  description <- paste0(
    "Bayesian logistic-regression exposure-safety model for DOSE ",
    "REDUCTION due to a treatment-emergent adverse event in adults ",
    ...
    "This is the endpoint Fukae 2024 used to set the UPPER limit of ",
    "its target exposure range, on the reasoning that maintaining the ",
    "initial dose after an adverse event represents acceptance of a ",
    "warranted risk, so the probability of a permanent reduction is ",
    "the sharper tolerability criterion. The probability of the event ",
    "is expit(mu + bE*zE + x'b1 + x'b2*zE) where zE is the ",
    ...
    "(Fukae 2024 Methods equation, Table 3). All candidate covariate ",
    "effects were regularized via normal-mixture spike-and-slab priors ",
    "(spike Normal(0, 0.1), slab Normal(0, 2.5), mixing weight ",
    "Beta(1, 1)); only the intercept and the exposure slope carried ",
    "weakly informative unregularized priors. The exposure effect is ",
    "the second steepest in the paper (odds ratio 1.79 per ",
    "250 ng*h/mL, 95% credible interval 1.06-3.08, excluding 1). ",
    "Fitted with CmdStanR 0.4.0 (Hamiltonian Monte Carlo, no-U-turn ",
    "sampler). There is no PK layer and no ODE: individual unbound ",
    "AUCss is supplied as data from the companion population PK model ",
    ...
    "Fukae_2024_valemetostat_dose_interruption; five companion ",
  ...
  units <- list(
    ...
    concentration = "prob_dose_reduction (probability of Dose reduction due to TEAE, 0-1; also logit_dose_reduction)"
  )
  ...
  population <- list(
    ...
    n_subjects     = 102L,
    n_studies      = 2L,
    n_observations = "100 evaluable binary dose-reduction records (one per patient; 2 of the 102 patients had a missing outcome, Fukae 2024 Table 1)",
    ...
    notes          = "Observed outcome: 10 of 102 patients (9.8%) had a dose reduction due to a TEAE, 90 (88.2%) did not, and 2 (2.0%) were missing (Fukae 2024 Table 1). Fukae 2024 selected this endpoint as the unique safety criterion for the upper limit of its region of practical equivalence."
  )

  ini({
    # ==================================================================
    # All values are posterior medians from Fukae 2024 Table 3, column
    # "Dose reduction due to TEAE". The Table 3 footnote states the reporting
    ...
    logit_ref             <- log(0.0774 / (1 - 0.0774)) ; label("Logit of the dose reduction due to TEAE probability for the reference patient (unitless logit)")  # Fukae 2024 Table 3, "Population mean" 0.0774 (0.0374, 0.137); ESS 6510, Rhat 1.00

    # ----- Exposure slope on the logit (beta*_E) -----
    e_aucu_logit          <- log(1.79)     ; label("Log-odds of dose reduction due to TEAE per 250 ng*h/mL increase in unbound valemetostat AUCss (unitless logit)")  # Fukae 2024 Table 3, OR 1.79 (1.06, 3.08); ESS 6036, Rhat 1.00

    # ----- Covariate effects on the logit intercept (beta*_1) -----
    e_age_logit           <- log(0.996)    ; label("Log-odds shift on the dose reduction due to TEAE logit per 10-year increase in age (unitless logit)")  # Fukae 2024 Table 3, OR 0.996 (0.827, 1.18); ESS 8884, Rhat 1.00
    e_ldh_logit           <- log(0.960)    ; label("Log-odds shift on the dose reduction due to TEAE logit per 300 U/L increase in baseline LDH (unitless logit)")  # Fukae 2024 Table 3, OR 0.960 (0.283, 1.12); ESS 2590, Rhat 1.00
    e_wt_logit            <- log(0.978)    ; label("Log-odds shift on the dose reduction due to TEAE logit per 20 kg increase in body weight (unitless logit)")  # Fukae 2024 Table 3, OR 0.978 (0.789, 1.17); ESS 7984, Rhat 1.00
    e_sexf_logit          <- log(1.01)     ; label("Log-odds shift on the dose reduction due to TEAE logit for female sex vs male reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.01 (0.776, 1.31); ESS 9932, Rhat 1.00
    e_hepimp_mild_logit   <- log(1.09)     ; label("Log-odds shift on the dose reduction due to TEAE logit for mild hepatic impairment vs normal reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.09 (0.702, 1.90); ESS 5851, Rhat 1.00
    e_race_white_logit    <- log(0.972)    ; label("Log-odds shift on the dose reduction due to TEAE logit for White race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.972 (0.615, 1.43); ESS 5369, Rhat 1.00
    e_race_black_logit    <- log(0.887)    ; label("Log-odds shift on the dose reduction due to TEAE logit for Black race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.887 (0.0220, 1.79); ESS 3414, Rhat 1.00
    e_region_usa_logit    <- log(0.948)    ; label("Log-odds shift on the dose reduction due to TEAE logit for United States vs Japan enrollment reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.948 (0.256, 1.30); ESS 3918, Rhat 1.00

    # ----- Covariate effects on the exposure slope (beta*_2) -----
    e_age_slope           <- log(1.02)     ; label("Shift in the unbound-AUCss exposure slope per 10-year increase in age (unitless logit)")  # Fukae 2024 Table 3, OR 1.02 (0.873, 1.20); ESS 7547, Rhat 1.00
    e_ldh_slope           <- log(0.986)    ; label("Shift in the unbound-AUCss exposure slope per 300 U/L increase in baseline LDH (unitless logit)")  # Fukae 2024 Table 3, OR 0.986 (0.815, 1.18); ESS 12093, Rhat 1.00
    e_wt_slope            <- log(0.946)    ; label("Shift in the unbound-AUCss exposure slope per 20 kg increase in body weight (unitless logit)")  # Fukae 2024 Table 3, OR 0.946 (0.500, 1.14); ESS 3927, Rhat 1.00
    e_sexf_slope          <- log(1.04)     ; label("Shift in the unbound-AUCss exposure slope for female sex vs male reference (unitless logit)")  # Fukae 2024 Table 3, OR 1.04 (0.823, 1.41); ESS 5181, Rhat 1.00
    e_hepimp_mild_slope   <- log(0.949)    ; label("Shift in the unbound-AUCss exposure slope for mild hepatic impairment vs normal reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.949 (0.653, 1.30); ESS 7401, Rhat 1.00
    e_race_white_slope    <- log(0.957)    ; label("Shift in the unbound-AUCss exposure slope for White race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.957 (0.536, 1.59); ESS 9057, Rhat 1.00
    e_race_black_slope    <- log(0.999)    ; label("Shift in the unbound-AUCss exposure slope for Black race vs Asian reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.999 (0.394, 2.60); ESS 11847, Rhat 1.00
    e_region_usa_slope    <- log(0.982)    ; label("Shift in the unbound-AUCss exposure slope for United States vs Japan enrollment reference (unitless logit)")  # Fukae 2024 Table 3, OR 0.982 (0.618, 1.53); ESS 7089, Rhat 1.00

    # ----- No between-subject variability, no residual error -----
    # See Fukae_2024_valemetostat_orr_central.R for the full rationale.
    addSd_prob_dose_reduction <- fixed(0.001) ; label("Placeholder additive residual SD on the typical-value dose reduction due to TEAE probability; the source likelihood is Bernoulli (no source residual)")  # not from source; see vignette Assumptions and deviations
  })
  ...
  model({
    ...
    logit_dose_reduction <- logit_ref +
                    e_aucu_logit * zauc +
                    cov_logit +
                    cov_slope * zauc
