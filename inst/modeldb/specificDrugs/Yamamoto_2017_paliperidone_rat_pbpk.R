Yamamoto_2017_paliperidone_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Paliperidone disposition in plasma and",
  ...
  compartmentData <- list(
    central = list(analyte = "paliperidone", units = "ng", specimen = "plasma", verified = TRUE),
    peripheral1 = list(analyte = "paliperidone", units = "ng", specimen = "tissue", verified = TRUE),
    brain_vascular = list(analyte = "paliperidone", units = "ng", specimen = "plasma", verified = TRUE),
    brain_ecf = list(analyte = "paliperidone", units = "ng", specimen = "brain ISF", verified = TRUE),
    brain_icf = list(analyte = "paliperidone", units = "ng", specimen = "tissue", verified = TRUE),
    brain_lysosome = list(analyte = "paliperidone", units = "ng", specimen = "tissue", verified = TRUE),
    brain_csf_lv = list(analyte = "paliperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_tfv = list(analyte = "paliperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_cm = list(analyte = "paliperidone", units = "ng", specimen = "CSF", verified = TRUE),
    brain_csf_sas = list(analyte = "paliperidone", units = "ng", specimen = "CSF", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range = "0.5 mg/kg (20 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 21 animals, 0.5 mg/kg (20 min infusion), data",
      "from reference 9 (Yamamoto 2017, Pharm Res). Plasma and brain",
  ...
  ini({
    ...
    lcl <- log(196); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(26400); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(61.5); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(3580); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    ...
    etalcl ~ 0.176  # Table 2 CV 42.0 percent
    etalvc ~ 0.226  # Table 2 CV 47.5 percent
    ...
    propSd <- 0.227; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = 1.8 and molecular weight = 426;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(1.8e-3); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    Daq <- fixed(2.8e-4); label("Aqueous diffusivity coefficient (cm^2/min)")  # Table 4 aqueous diffusivity coefficient
    ...
    AFout1 <- fixed(3.0); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(3.7); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(4.7); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    BF <- fixed(0.91); label("Brain-tissue binding factor (unitless)")  # Table 5 BF
