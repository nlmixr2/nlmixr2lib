Yamamoto_2017_methotrexate_rat_pbpk <- function() {
  description <- paste(
    "PBPK (LeiCNS-PK1.0 comprehensive CNS physiologically-based model).",
    "Preclinical (rat, male Wistar). Methotrexate disposition in plasma and",
    ...
    "whose three asymmetry factors were derived from three DIFFERENT Kp,uu",
    "values (Table 4: Kp,uu,brainECF 0.018, Kp,uu,CSFLV 0.0066, Kp,uu,CSFCM",
    "0.0024), because methotrexate CSF concentrations differ between the",
    "lateral ventricle and the cisterna magna. No total-brain binding factor",
    "is reported (Table 4 Kp = NA; Table 5 BF = NA), so no total-brain output",
    "is produced."
  )
  reference <- paste(
  ...
  compartmentData <- list(
    ...
    peripheral1 = list(analyte = "methotrexate", units = "ng", specimen = "tissue", verified = TRUE),
    peripheral2 = list(analyte = "methotrexate", units = "ng", specimen = "tissue", verified = TRUE),
  ...
  population <- list(
    ...
    dose_range = "40 and 80 mg/kg (10 min infusion), intravenous",
    notes = paste(
      "Study design from Table 1: 23 animals, 40 and 80 mg/kg (10 min",
      "infusion), data from reference 23 (Westerhout 2014). Plasma and brain",
  ...
  ini({
    ...
    lcl <- log(8.04); label("Plasma clearance (mL/min)")  # Table 2 CL_PL
    lvc <- log(28.0); label("Central volume of distribution (mL)")  # Table 2 V_PL
    lq <- log(28.5); label("Intercompartmental clearance to peripheral 1 (mL/min)")  # Table 2 Q_PL_PER1
    lvp <- log(111); label("Peripheral 1 volume of distribution (mL)")  # Table 2 V_PER1
    lq2 <- log(3.33); label("Intercompartmental clearance to peripheral 2 (mL/min)")  # Table 2 Q_PL_PER2
    lvp2 <- log(83.5); label("Peripheral 2 volume of distribution (mL)")  # Table 2 V_PER2
    ...
    etalcl ~ 0.140  # Table 2 CV 37.4 percent
    etalq2 ~ 0.181  # Table 2 CV 42.5 percent
    etalvc ~ 0.163  # Table 2 CV 40.4 percent
    ...
    propSd <- 0.151; label("Proportional residual error (fraction)")  # Table 2 sigma_plasma proportional
    addSd <- 5400; label("Additive residual error (ng/mL)")  # Table 2 sigma_plasma additive

    # ---- Drug-specific CNS parameters (Table 4) ----
    # Transmembrane permeability and aqueous diffusivity are the in silico
    # predictions of Eqs. 2 and 1 from logP = -1.9 and molecular weight = 454;
    # the tabulated values are used directly rather than re-deriving them.
    P0 <- fixed(6.1e-7); label("Transmembrane permeability (cm/min)")  # Table 4 transmembrane permeability
    ...
    AFout1 <- fixed(46445.5); label("Asymmetry factor out of brain extracellular fluid (unitless)")  # Table 4 AFout1
    AFout2 <- fixed(4.7e5); label("Asymmetry factor out of lateral-ventricle CSF (unitless)")  # Table 4 AFout2
    AFout3 <- fixed(1028201); label("Asymmetry factor out of third + fourth ventricle CSF (unitless)")  # Table 4 AFout3
    ...
    PHF1 <- fixed(1.3); label("pH-dependent factor, brain extracellular fluid to microvasculature (unitless)")  # Table 5 PHF1
    PHF2 <- fixed(1.3); label("pH-dependent factor, lateral-ventricle CSF to microvasculature (unitless)")  # Table 5 PHF2
    PHF3 <- fixed(1.3); label("pH-dependent factor, third + fourth ventricle CSF to microvasculature (unitless)")  # Table 5 PHF3
    PHF4 <- fixed(1.3); label("pH-dependent factor, brain extracellular to intracellular fluid (unitless)")  # Table 5 PHF4
    PHF5 <- fixed(2.5); label("pH-dependent factor, brain intracellular to extracellular fluid (unitless)")  # Table 5 PHF5
    PHF6 <- fixed(2.5); label("pH-dependent factor, brain intracellular fluid to lysosome (unitless)")  # Table 5 PHF6
    PHF7 <- fixed(250); label("pH-dependent factor, lysosome to brain intracellular fluid (unitless)")  # Table 5 PHF7

    # ---- Rat CNS physiology (Table 3); identical for all ten drugs ----
    # Table 3 reports volumes in uL; converted to mL to match the mL/min
    # flows and the ng / (ng/mL) dosing and concentration units. The total
    # brain volume V_TOT is omitted: Table 4 reports Kp = NA and Table 5
    # BF = NA for this compound, so no total-brain output is produced.
