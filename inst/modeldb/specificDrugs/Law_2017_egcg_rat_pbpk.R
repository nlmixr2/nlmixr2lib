Law_2017_egcg_rat_pbpk <- function() {
  description <- paste(
    "PBPK (whole-body, 13 blood-flow-limited tissue compartments).",
    "Preclinical (rat). Single-catechin model for epigallocatechin gallate",
    "(EGCg) in rats (Law et al. 2017). Thirteen perfusion-limited",
    "compartments (lung, kidney, muscle, brain, liver, spleen, gut, bone,",
    "skin, heart, adipose, rest of body, blood split into a one-third",
    "arterial and a two-thirds venous pool) with first-order oral input into",
    "gut tissue after a lag time, renal elimination from the kidney, and an",
  ...
  ini({
    ...
    # Table 3 (kac EGCg = 0.003 /min per kg^-0.3)
    lka <- fixed(log(0.003)); label("Absorption rate coefficient kac into gut tissue (/min per kg^-0.3)")
    # Table 3 prints a RANGE for EGCg, 'F 0.0003-0.038'; 0.038 is the upper
    # bound and is the value consistent with the Zhu et al. (2000) 2500 mg/kg
    # calibration simulation of Figure 4 - see vignette Errata
    lfdepot <- fixed(log(0.038)); label("Empirical oral bioavailability factor F (unitless)")
    # Table 3 (tlag EGCg = 10 min)
    ltlag <- fixed(log(10)); label("Oral absorption lag time (min)")
    # Table 3 (Rt EGCg = 3 min)
    lmtt_bile <- fixed(log(3)); label("Bile-duct sub-compartment residence time Rt (min)")
    # Table 3 (krac EGCg = 0.67)
    lkreab <- fixed(log(0.67)); label("Colonic reabsorption rate coefficient krac (/min per kg^-0.3)")
    # Table 3 (kfc EGCg = 0.13)
    lkfec <- fixed(log(0.13)); label("Faecal transport rate coefficient kfc (/min per kg^-0.3)")
    # Table 3 (CLbc EGCg = 9.13) (9.13 mL/min per kg^0.66 converted to L/min)
    lcl_nonren <- fixed(log(9.13 / 1000)); label("Biliary clearance coefficient CLbc (L/min per kg^0.66)")
    # Table 3 (CLrc EGCg = 0.36) (0.36 mL/min per kg^0.66 converted to L/min)
    lcl_renal <- fixed(log(0.36 / 1000)); label("Renal clearance coefficient CLrc (L/min per kg^0.66)")

    # Table 1 (BLPLR EGCg = 0.91, rat)
    ...
    # Table 2 (adipose EGCg = 0.2)
    lkp_adipose <- fixed(log(0.2)); label("Tissue/blood partition coefficient, adipose (unitless)")
    # Table 2 (bone EGCg = 1.62)
    lkp_bone <- fixed(log(1.62)); label("Tissue/blood partition coefficient, bone (unitless)")
    # Table 2 (brain EGCg = 3.15)
    lkp_brain <- fixed(log(3.15)); label("Tissue/blood partition coefficient, brain (unitless)")
    # Table 2 (gut EGCg = 2.04)
    lkp_gut <- fixed(log(2.04)); label("Tissue/blood partition coefficient, gut (unitless)")
    # Table 2 (heart EGCg = 1.27)
    lkp_heart <- fixed(log(1.27)); label("Tissue/blood partition coefficient, heart (unitless)")
    # Table 2 (kidney EGCg = 1.43)
    lkp_kidney <- fixed(log(1.43)); label("Tissue/blood partition coefficient, kidney (unitless)")
    # Table 2 (liver EGCg = 1.5)
    lkp_liver <- fixed(log(1.5)); label("Tissue/blood partition coefficient, liver (unitless)")
    # Table 2 (lung EGCg = 1.7)
    lkp_lung <- fixed(log(1.7)); label("Tissue/blood partition coefficient, lung (unitless)")
    # Table 2 (muscle EGCg = 1.02)
    lkp_muscle <- fixed(log(1.02)); label("Tissue/blood partition coefficient, muscle (unitless)")
    # Table 2 (skin EGCg = 1.79)
    lkp_skin <- fixed(log(1.79)); label("Tissue/blood partition coefficient, skin (unitless)")
    # Table 2 (spleen EGCg = 0.98)
    lkp_spleen <- fixed(log(0.98)); label("Tissue/blood partition coefficient, spleen (unitless)")
    # Table 2 (other EGCg = 1)
    ...
    # deviations section. Source: Results (34.9% MAPE, Figure 4, Zhu et al. 2000
    # calibration).
    propSd <- fixed(0.349); label("Proportional residual magnitude, from the reported MAPE (fraction)")
  })
