Wei_2026_cefquinome_enrofloxacin_plasma_aucmic <- function() {
  description <- paste0(
    ...
    "carries the fit driven by the AUC72h/MIC (combine) index computed from plasma free drug PK data. ",
    ...
    "Wei 2026's Emax = 0.006 onto the canonical `e0` (effect at zero exposure) and Wei 2026's ",
    "E0 = -4.000 onto the canonical `emax` (maximal drug effect), and writes the algebraically ",
    ...
    "log10 CFU/mL, EC50 = 441.48, Hill N = 1.508 (R^2 = 0.867, AIC = 17.377). ",
    "The parameterisation was confirmed against a value Wei 2026 printed independently of Table 6: solving E = -3 returns an index of 915.967 against the published 3 log10 CFU/mL target of 915.967, an exact match to six significant figures. ",
    ...
    "typical-value simulation. Sibling models from the same paper: Wei_2026_cefquinome_enrofloxacin_plasma_cmaxmic, Wei_2026_cefquinome_enrofloxacin_plasma_tmic, Wei_2026_cefquinome_enrofloxacin_lung_cmaxmic, Wei_2026_cefquinome_enrofloxacin_lung_aucmic and Wei_2026_cefquinome_enrofloxacin_lung_tmic."
  )
  reference <- paste(
    ...
    "PK/PD'. Parameter estimates: Table 6, plasma free drug / AUC72h/MIC (combine) column.",
  ...
  covariateData <- list(
    AUCMIC_CEFQ = list(
      description        = "Cefquinome PK/PD index: the cumulative area under the free plasma concentration-time curve over the 72 h treatment course divided by MIC(combine), the MIC of cefquinome measured in the presence of the average free enrofloxacin plasma concentration achieved over 72 h",
      units              = "h",
      type               = "continuous",
      ...
      source_name        = "AUC72h/MIC (combine) (Wei 2026 Table 5, plasma free drug block; Materials and methods, 'Analysis of antibacterial effects and fitting of PK/PD')"
    )
  )
  ...
  ini({
    ...
    # matrix: plasma free drug   index: AUC72h/MIC (combine)
    # =================================================================
    ...
    #   e0   <- Wei 2026's Emax = 0.006   (effect at zero exposure)
    #   emax <- Wei 2026's E0   = -4.000  (maximal drug effect)
    ...
    e0 <- 0.006
    label("Difference in lung K. pneumoniae load from the blank control at zero exposure (log10 CFU/mL)")  # Wei 2026 Table 6, plasma free drug block, AUC72h/MIC (combine) column, "E max" = 0.006 (defined in Materials and methods as the blank-control change)
    emax <- -4.000
    label("Maximal difference in lung K. pneumoniae load from the blank control (log10 CFU/mL; negative = reduction)")  # Wei 2026 Table 6, plasma free drug block, AUC72h/MIC (combine) column, "E 0" = -4.000 (defined in Materials and methods as the maximum antibacterial effect)
    lec50 <- log(441.48)
    label("Log AUC72h/MIC (combine) index producing 50% of the maximal antibacterial effect EC50 (h)")  # Wei 2026 Table 6, plasma free drug block, AUC72h/MIC (combine) column, EC 50 = 441.48
    lhill <- log(1.508)
    label("Log Hill coefficient N defining the steepness of the index-effect curve (unitless)")  # Wei 2026 Table 6, plasma free drug block, AUC72h/MIC (combine) column, Hill's slope = 1.508

    # =================================================================
    # Residual error
    # =================================================================
    # Wei 2026 Table 6 reports the correlation coefficient (R^2 = 0.867)
    # and the AIC (17.377) of this fit and gives no residual standard
