LopezArguello_2023_avibactam_qsp <- function() {
  ...
  compartmentData <- list(
    ...
    nperi  = list(analyte = "Avibactam", units = NA_character_, specimen = "administration site", verified = FALSE)
  )
  ...
  covariateData <- list(
    ...
    CONC_AVI_MGL = list(
      description = "Static extracellular avibactam concentration applied in the PBP-binding assay. Scales the rate of net influx and PBP access (Eq 1: Rate_Influx/access = Rate_Influx/access,scaled x C_drug).",
      units = "mg/L",
      type = "continuous",
  ...
  population <- list(
    ...
    n_subjects = 1L,
    n_studies = 1L,
    disease_state = "Wild-type P. aeruginosa PAO1; avibactam MIC not determined (beta-lactamase inhibitor)",
    ...
    dose_range = "avibactam at a static extracellular concentration of 4 mg/L (a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors); no dosing events",
    notes = paste(
  ...
  ini({
    ...
    rate_influx_scaled <- 9.85; label("Scaled rate of net influx and PBP access (drug molecules/min per mg/L)") # Table 1, avibactam: 9.85 (SE 30.9%)
    # The studied extracellular concentration enters as the CONC_AVI_MGL covariate
    # (Table 1: avibactam studied at 4 mg/L = a fixed concentration within the clinically relevant range; MIC not determined for beta-lactamase inhibitors).
    ...
    k2_1a <- 0.163; label("Second-order acylation rate constant, PBP1a (1e-3 /min)") # Table S1, avibactam PBP1a: 0.163
    k2_1b <- 23;    label("Second-order acylation rate constant, PBP1b (1e-3 /min)") # Table S1, avibactam PBP1b: 23
    k2_2 <- 15.1;   label("Second-order acylation rate constant, PBP2 (1e-3 /min)") # Table S1, avibactam PBP2: 15.1 -- shared estimate with avibactam (Table S1 footnote b: the apparent acylation rate constant for PBP2 by relebactam was shared with the estimate of avibactam)
    k2_3 <- 0.923;  label("Second-order acylation rate constant, PBP3 (1e-3 /min)") # Table S1, avibactam PBP3: 0.923
    k2_4 <- 26.3;   label("Second-order acylation rate constant, PBP4 (1e-3 /min)") # Table S1, avibactam PBP4: 26.3
    k2_5 <- 24.5;   label("Second-order acylation rate constant, PBP5/6 (1e-3 /min)") # Table S1, avibactam PBP5/6: 24.5
  ...
  model({
    ...
    rate_influx <- CELLS_INTACT * rate_influx_scaled * CONC_AVI_MGL
