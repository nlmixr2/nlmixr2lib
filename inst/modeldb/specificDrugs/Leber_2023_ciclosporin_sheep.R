Leber_2023_ciclosporin_sheep <- function() {
  ...
  compartmentData <- list(
    ...
    adsorbed       = list(analyte = "ciclosporin", units = "mg", specimen = "not applicable", verified = TRUE)
  )
  ...
  population <- list(
    ...
    dose_range     = "Ciclosporin (cyclosporin A) 600-1,000 mg orally twice daily, individually titrated to a whole-blood target of 80-100 ng/mL (Table 1). Given in combination with mycophenolate mofetil 1 g twice daily and prednisolone 10 mg (group 2).",
  ...
  ini({
    ...
    #   CL/F : 43.8% CV -> log(0.438^2 + 1) = 0.1755017
    etalcl ~ fixed(0.1755017)   # Leber 2023 Supplementary Table 1 (IIV CL/F, 43.8% CV)

    # Residual error on the pre-adsorber (inlet) samples, which are the
    # systemic whole-blood concentrations this model predicts.
    propSd <- 0.337; label("Proportional residual error, pre-adsorber samples (fraction)")  # Leber 2023 Supplementary Table 2 (Prop. error pre, 33.7% CV)
    propSd_Cpostfilter <- 0.29; label("Proportional residual error, post-adsorber samples (fraction)")  # Leber 2023 Supplementary Table 2 (Prop. error post, 29% CV)
  })
  model({
    ka <- exp(lka)
    cl <- exp(lcl + etalcl)
