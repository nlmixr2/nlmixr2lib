Allegaert_2015_paracetamol <- function() {
  ...
  ini({
    ...
    e_birthcontrol_cl_gluc  <- 1.46   ; label("Multiplicative scalar on glucuronide-formation CL for oral-contraceptive use (CONMED_BIRTHCONTROL = 1, applied on top of the OCC effect)") # TH12 FINAL = 1.46E+00
  ...
  model({
    ...
    # for Q2, CONMED_BIRTHCONTROL = 0 for CL_gluc) and to the corresponding `e_*`
    ...
    m_bc_cl_gluc   <- 1 + (e_birthcontrol_cl_gluc - 1) * CONMED_BIRTHCONTROL
