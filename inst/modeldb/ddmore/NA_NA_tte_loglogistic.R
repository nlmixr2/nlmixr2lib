NA_NA_tte_loglogistic <- function() {
  ...
  population <- list(
    ...
    notes          = "200 simulated patients; 142 (71%) had Competing Event 2. Competing Event 2 is right-censored only (CENSORING = 1; exact event times are observed). For the other three events in the same bundle see NA_NA_tte_gompertz.R, NA_NA_tte_gompertz_ev2.R, and NA_NA_tte_lognormal.R. Source: BAST Inc Limited, 'BAST approach to parametric time-to-event (PTTE) modelling', 12 July 2017 (BAST_PTTE_modelling.pdf); base-distribution discussion § 2.4.1 / Figure 2-4 (selected log-normal in text); covariate-test result Table 2-5 (no covariate retained); final-fit executable Executable_runCOMPEV2_005.mod (log-logistic) and listing Output_simulated_runCOMPEV2_005.res."
  )
