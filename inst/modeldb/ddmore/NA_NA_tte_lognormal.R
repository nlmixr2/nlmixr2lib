NA_NA_tte_lognormal <- function() {
  ...
  population <- list(
    ...
    notes          = "200 simulated patients; 36 (18%) had Competing Event 1. Competing Event 1 is interval-censored: exact event times are unknown, only that the event occurred between two scheduled assessment visits (BAST guiding document § 2.2.1). For the other three events in the same bundle see NA_NA_tte_gompertz.R, NA_NA_tte_gompertz_ev2.R, and NA_NA_tte_loglogistic.R. Source: BAST Inc Limited, 'BAST approach to parametric time-to-event (PTTE) modelling', 12 July 2017 (BAST_PTTE_modelling.pdf); base-distribution selection § 2.4.1 / Figure 2-3; covariate selection § 2.4.2 / Table 2-4; final-fit listing Output_simulated_runCOMPEV1_101.res."
  )
