Hansson_2013c_sunitinib <- function() {
  ...
  covariateData <- list(
    DOSE_MG = list(
      description        = "Current administered sunitinib daily dose (mg) carried as a time-varying data column. Set to 0 during off-cycles (4 weeks on / 2 weeks off in the Hansson 2013 GIST cohort) or for placebo subjects so the derived AUC = DOSE_MG / CLI becomes 0.",
      units              = "mg",
      type               = "continuous",
      reference_category = NULL,
      notes              = "DDMORE-bundle simulated dataset reports DOSE_MG = 50 (treated, on-cycle) or 0 (off-cycle / placebo) at every record. The .mod feeds DOSE_MG into AUC = DOSE_MG / CLI in $PK at every event call, producing a per-cycle daily-AUC equivalent (mg*h/L). For typical-cohort vignette simulations the value is held at 50 mg during the 4-week on-cycles.",
      source_name        = "DOSE"
    ),
    CLI = list(
    ...
    EC50_SVEGFR3 = list(
      ...
      units              = "mg*h/L (matches the auc = DOSE_MG / CLI exposure summary)",
      type               = "continuous",
      reference_category = NULL,
