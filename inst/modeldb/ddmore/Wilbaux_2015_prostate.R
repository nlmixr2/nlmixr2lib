Wilbaux_2015_prostate <- function() {
  ...
  population <- list(
    n_subjects = 223L,
    n_studies = 1L,
    age_range = "adults with metastatic castration-resistant prostate cancer (mCRPC); detailed age distribution not extracted (the PMC text quoted only the n = 223 cohort summary)",
  ...
  ini({
    ...
    etalk1 + etalk2 + etalq501 + etalq502 + etalkoutts + etalsflv + etalkinpsa + etalkoutpsa + etalpsa0 ~ c(
      # NOTE: the published OMEGA block is not jointly positive-definite
      # (min eigenvalue -2.4e-3, a rounding artifact in the source table),
      # so chol() fails when simulating IIV. Adjusted to the nearest
      # positive-definite matrix (Higham 2002; Matrix::nearPD); changes are
      # confined to the 3rd-4th significant figure.
      0.72312683,                                                              # OMEGA(2,2)  -- var Kc
      -0.69776323, 1.8304282,                                                  # OMEGA(3,2..3)
      -0.63801387, -0.63402578, 4.7600016,                                     # OMEGA(4,2..4)
      -0.79918618, -0.22834371, 2.9300197, 2.8402711,                          # OMEGA(5,2..5)
      0.35093715, 2.2498825, -2.6499929, -4.2399065, 20.60003,                 # OMEGA(6,2..6)
      0.23695966, 0.039926237, -0.75099542, -0.95394083, 3.4300193, 0.78601244, # OMEGA(7,2..7)
      0.13061185, 0.97328423, -1.8099523, -1.4494292, -0.60380932, -0.097378333, 2.6011624, # OMEGA(8,2..8)
      0.31191379, 0.044942129, -2.2899899, -1.1598737, 1.8400416, 0.57802675, 0.096959764, 1.530057, # OMEGA(9,2..9)
      0.11518768, 0.69334397, -1.3100204, -1.2502733, -0.30209155, 0.15294107, 2.299425, 0.034674021, 2.4002728 # OMEGA(10,2..10)
    )
