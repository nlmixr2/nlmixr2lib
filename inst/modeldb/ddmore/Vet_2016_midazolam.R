Vet_2016_midazolam <- function() {
  ...
  covariateData <- list(
    ...
    ORGF = list(
      ...
      notes              = "Decomposed inside `model()` into mutually exclusive binary indicators `orgf1`, `orgf2`, `orgf3`, `orgf_ge4` that select per-stratum typical CL values (Vet 2016 .mod $PK lines 51-55: `IF (ORGF.EQ.0) TVCL = THETA(1) * ...`, `IF (ORGF.EQ.1) TVCL = THETA(7) * ...`, etc., with a final `IF (ORGF.GT.3.5) TVCL = THETA(10) * ...` collapsing the 4-and-5-organ strata). The Vet 2016 source dataset records ORGF in {0, 1, 2, 3, 4, 5}; the canonical decomposition treats >=4 as a single combined stratum to match the source's parameterization.",
      source_name        = "ORGF"
    ),
    OCC = list(
  ...
  population <- list(
    ...
    disease_state  = "Critically ill paediatric patients receiving continuous IV midazolam in the paediatric intensive care unit (PICU). Inflammation (CRP) and number of failing organs (ORGF, 0..>=4) are the two retained covariates.",
  ...
  ini({
    ...
    # ORGF=0 typical CL (FIXED at .mod $THETA line 78 "1.6 FIX") for a 5 kg child with
    # CRP = 32 mg/L. Per-stratum CL values for ORGF >= 1 are encoded as additive shifts
    # `e_orgf<k>_cl` on the log scale below; the reference category is ORGF = 0.
    lcl <- fixed(log(1.60))   ; label("Typical CL for ORGF = 0, WT = 5 kg, CRP = 32 mg/L (L/h, FIXED)") # .lst line 455 TH 1 = 1.60E+00 (THETA(1) FIX in .mod)
    ...
    # Per-stratum CL shifts on the log scale relative to the ORGF = 0 reference. Each
    # value is `log(THETA(k) / THETA(1))` for k = 7, 8, 9, 10, with THETA(1) = 1.6 (the
    # ORGF = 0 typical CL). The numerical shifts are pre-computed below to avoid an
    # arithmetic step in `ini()`; the .lst line 455 source THETAs are quoted in each
    # comment for traceability.
    e_orgf1_cl    <- -0.2154  ; label("Additive shift in log(CL) when ORGF = 1 (unitless)")     # log(1.29  / 1.60) ; .lst TH 7  = 1.29E+00
    e_orgf2_cl    <- -0.5141  ; label("Additive shift in log(CL) when ORGF = 2 (unitless)")     # log(0.957 / 1.60) ; .lst TH 8  = 9.57E-01
    e_orgf3_cl    <- -0.6420  ; label("Additive shift in log(CL) when ORGF = 3 (unitless)")     # log(0.842 / 1.60) ; .lst TH 9  = 8.42E-01
    e_orgf_ge4_cl <- -0.8585  ; label("Additive shift in log(CL) when ORGF >= 4 (unitless)")   # log(0.678 / 1.60) ; .lst TH 10 = 6.78E-01
  ...
  model({
    # 1. Decompose the integer-valued ORGF column into mutually exclusive binary
    # indicators that select the per-stratum CL shift. Reference category is
    # ORGF = 0 (all four indicators evaluate to 0; no shift applied).
    orgf1    <- (ORGF == 1)
    orgf2    <- (ORGF == 2)
    orgf3    <- (ORGF == 3)
    orgf_ge4 <- (ORGF >= 4)
    ...
    # 3. Individual PK parameters with allometric WT scaling, the per-stratum ORGF
    # CL shift, the CRP power effect on CL, and the IOV multiplexing on log-CL.
