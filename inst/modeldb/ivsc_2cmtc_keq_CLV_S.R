ivsc_2cmtc_keq_CLV_S <- function() {
  ...
  ini({
    ...
    lksynS <- 0.1  ; label("Target Synthesis rate (1/d)") 
    lkeS   <- 0.1  ; label("Target elimination rate (1/d)")
    lkeDS  <- 0.4  ; label("Internalisation rate (1/d)")
    lkeqS  <- 0.35 ; label("")
