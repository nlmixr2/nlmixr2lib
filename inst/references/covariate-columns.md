...
- **Description:** Per-subject latent mixture-model class indicator from popPK absorption-mixture papers that resolve a lagged-absorption subpopulation from a no-lag subpopulation. 1 = subject classified to the lagged-absorption subpopulation (estimable lag time and, in some papers, distinct absorption rate constant ka); 0 = subject classified to the no-lag subpopulation (lag time fixed at 0). Not a measured clinical covariate -- the mixture assignment is the per-subject latent-class index from a NONMEM `$MIXTURE` block. Founding example: Bonate 2004 apomine (Methods "Base model development", equation (1.4), and Results paragraph 1). Second-precedent example: Frymoyer 2013 mycophenolic acid (Methods / Population PK model building paragraph 2).
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (no-lag subpopulation; the majority class in Frymoyer 2013 at 91 %, the minority class in Bonate 2004 at 3 % -- the reference category encodes "no lag", not "minority").
- **Source aliases:** `MIXTURE` -- NONMEM `$MIXTURE` block class index. In Bonate 2004 component 1 = lagged-absorption Group 1 with P = 0.970 and component 2 = no-lag Group 2 with P = 0.030, so the binary indicator is `MIX_LAGGED_ABS = as.integer(MIXTURE == 1)`. In Frymoyer 2013 component 1 (P1) = no-delay subpopulation with P = 0.91 and component 2 (P2) = delayed-absorption subpopulation with P = 0.09, so the binary indicator is `MIX_LAGGED_ABS = as.integer(MIXTURE == 2)`. Map paper-specific component numbering to the canonical "1 = lagged" semantics per paper.
- **Example models:**
  - `Bonate_2004_apomine.R` (gates ka between two subpopulation-specific typical values and gates the lag time on the depot compartment between Group 1 = `exp(ltlag)` and Group 2 = 0; the shared `etalka` log-normal IIV scales whichever group's ka is active for the subject).
  - `Frymoyer_2013_mycophenolic_acid.R` (gates only the lag time on the depot compartment between Group 2 = `exp(ltlag) = 1.96 h` and Group 1 = 0; ka, F and disposition parameters are shared between the two classes per Frymoyer 2013 "Allowing other absorption parameters (ka and F) to vary for the two subpopulations by using an extended mixture model did not improve the overall model or predictions").
- **Notes:** The population probability of `MIX_LAGGED_ABS = 1` is the estimated mixture fraction of the lagged-absorption subpopulation per paper: 0.970 for Bonate 2004 (Table 3 with P1 = -3.47 -> P(Group 1) = 1 / (1 + exp(P1)) = 0.970) and 0.09 for Frymoyer 2013 (Table 3, P2 = 0.09, RSE 4.0 %; identified 8 of 132 subjects). For typical-value simulation set `MIX_LAGGED_ABS` to whichever class dominates the paper of interest; for population simulation, draw `MIX_LAGGED_ABS ~ Bernoulli(P_lagged)` with `P_lagged` taken from each paper's reported mixture fraction. Scope: specific because the binary semantics are tied to the lagged-vs-no-lag absorption-mixture dichotomy; future popPK papers that share this dichotomy may extend this entry, while mixture indicators from unrelated dichotomies (e.g., fast vs slow elimination subpopulations) should register a new canonical name (`MIX_FAST_ELIM`, etc.). Originally ratified canonically on 2026-06-04 alongside the Bonate 2004 apomine extraction; extended on 2026-06-13 with the Frymoyer 2013 mycophenolic acid extraction (lag-only mixture; shared ka and F across classes).
