...
- **Type:** continuous
- **Scope:** general
- **Reference category:** n/a — used with power scaling `(FFM / ref)^exponent`. Reference values observed: 45 kg (Aguiar 2021, Crohn's disease cohort median).
- **Source aliases:** none known.
- **Example models:** `Aguiar_2021_ustekinumab.R` (reference 45 kg; power exponents 0.598 on CL, 0.590 on Vc, 0.586 on Vp).
- **Notes:** Frequently estimated from height, weight, and sex via the Janmahasatian et al. 2005 semi-mechanistic model (Clin Pharmacokinet. 2005;44(10):1051-1065). Document the FFM-derivation method in `covariateData[[FFM]]$notes`. FFM is preferred over total body weight when scaling monoclonal-antibody PK because mAb distribution is largely confined to extracellular fluid; muscle / lean tissue tracks extracellular volume better than total weight in heavier patients.
