...
- **Example models:** `Morris_2011_telapristone.R` (selects the telapristone CL/F typical value between the high-CL anchor `exp(lcl_pop1) = 11.6 L/h` and the low-CL anchor `exp(lcl_pop2) = 3.34 L/h`; the shared IIV `etalcl ~ 0.200` is applied multiplicatively to whichever typical the mixture-class selects).
