...
- **Description:** Arterial blood partial pressure of carbon dioxide, a routinely-measured blood-gas variable. Elevated in hypercapnia and respiratory acidosis (chronic obstructive pulmonary disease, restrictive lung disease, hypoventilation, severe asthma exacerbation). Used in population-PK analyses of theophylline and other drugs whose hepatic metabolism is sensitive to acid-base status / pulmonary disease severity. Typically captured at study admission via an automatic blood-gas analyzer on an arterial puncture sample; document baseline-vs-time-varying status in `covariateData[[PACO2]]$notes` per model.
- **Units:** mmHg
- **Type:** continuous
- **Scope:** general
- **Reference category:** n/a -- used as a continuous covariate in exponential / log-linear effect forms `exp(e_paco2_param * PACO2)` or centered `exp(e_paco2_param * (PACO2 - ref))`. Reference values observed: 42.5 mmHg (Yano 1993 stable chronic airway obstruction cohort mean; also the laboratory normal-range midpoint of 35-45 mmHg in adults).
- **Source aliases:**
  - `PaCO2` -- the standard physiological abbreviation; used directly in Yano 1993 Table I and Eqs 7-8.
- **Example models:** `Yano_1993_theophylline.R` (log-linear effect on both CL and Vd of theophylline: `CL = exp(lcl + e_hepimp_cl * HEPIMP + e_paco2_cl * PACO2 + etalcl)` with `e_paco2_cl = 0.0233` per mmHg, and `Vd = exp(lvc + e_paco2_vc * PACO2 + etalvc)` with `e_paco2_vc = 0.00934` per mmHg; both estimated by forward-selection likelihood-ratio testing).
- **Notes:** Companion to arterial PaO2, arterial blood pH, serum bicarbonate, and base excess in respiratory-physiology covariate panels (not yet registered; future models retaining these as final covariates should propose them as new canonicals at extraction time). Yano 1993 found PaCO2 to be the most informative blood-gas covariate when entered in a forward-selection competition against PaO2, pH, hematocrit, and albumin. General scope because PaCO2 is a routinely-measured laboratory variable suitable for any model where pulmonary disease severity or acid-base status modulates drug disposition. Ratified canonically on 2026-06-07 alongside the Yano 1993 theophylline extraction.
...
### HEPIMP (**canonical for hepatic-impairment indicator (binary; classification scheme paper-specific)**)
- **Description:** Binary hepatic-impairment indicator: 1 = any hepatic impairment (mild, moderate, or severe under whatever classification scheme the source paper applies), 0 = normal hepatic function. The classification scheme that defines "impairment" is paper-specific and must be documented in per-model `covariateData[[HEPIMP]]$notes`. This canonical's semantic is "some hepatic dysfunction vs. none"; the underlying decoder (NCI ODWG groups, transaminase cutoff, clinician judgment, Child-Pugh class, etc.) is allowed to vary across source papers, paralleling the design of `HEPIMP_SEV` and `HEPIMP_MODSEV`. Schemes commonly encountered:
  - **NCI ODWG groups 2-4 vs group 1** (Ramalingam SS et al., J Clin Oncol 2010;28:4507): group 1 = normal, group 2 = mild (TBILI <= ULN and AST > ULN, or TBILI > 1-1.5 x ULN), group 3 = moderate (TBILI > 1.5-3 x ULN), group 4 = severe (TBILI > 3 x ULN). Source papers typically pool groups 2-4 versus group 1 because the impaired-liver subgroups are individually small. Used by `Lin_2024_casirivimab.R`, `Lu_2019_polatuzumab.R`.
  - **Transaminase cutoff + clinician judgment**: HEPIMP = 1 if AST or ALT exceeds a paper-specified threshold (e.g., > 30 IU/L in Yano 1993) OR if the treating physician documents hepatic dysfunction. Used by `Yano_1993_theophylline.R`.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (normal hepatic function under whatever scheme the model applies).
- **Source aliases:**
  ...
  - `HF` -- Yano 1993's binary hepatic dysfunction column (1 = AST or ALT > 30 IU/L OR clinician diagnosis; 0 otherwise); same orientation as the canonical, no value transformation. Used in `Yano_1993_theophylline.R`.
  - `HEP_IMP` -- retired canonical name; replaced by `HEPIMP` for consistency with the `HEPIMP_MILD` family.
- **Example models:** `Lu_2019_polatuzumab.R` (NCI ODWG scheme; multiplicative effect on FRAC_NS = 1.19, applied as `1.19^HEPIMP`). `Yano_1993_theophylline.R` (transaminase + clinician-judgment scheme; log-linear effect on theophylline CL: `CL = exp(lcl + e_hepimp_cl * HEPIMP + e_paco2_cl * PACO2)` with `e_hepimp_cl = -0.525`, i.e., hepatic dysfunction reduces theophylline CL by ~40.8% relative to subjects with normal hepatic function at the same PaCO2).
- **Notes:** Companion to `HEPIMP_MILD`, `HEPIMP_SEV`, `HEPIMP_MODSEV`, and `HEPIMP_MOD_MISSING` for models that resolve hepatic impairment at finer granularity. Use this canonical when the source paper dichotomises hepatic-function status as "any impairment vs. normal" under any classification scheme; document the scheme in `covariateData[[HEPIMP]]$notes`. The semantic ("any vs. none") is invariant across schemes; the decoder is allowed to vary because a value of 1 in one paper's scheme is not always commensurate with a value of 1 in another's (e.g., NCI ODWG group 2 ~ TBILI x ULN cut points vs Yano's AST/ALT > 30 IU/L cutoff). Models that need to combine cohorts with different schemes must reclassify the source data to a single scheme before pooling -- this canonical does NOT promise that HEPIMP = 1 is comparable across studies. Scheme-broadening ratified on 2026-06-07 alongside the Yano 1993 theophylline extraction; the earlier NCI-ODWG-only description (which dated to the introduction of HEPIMP) is the historical first instance and remains a valid scheme under the broadened semantic.
