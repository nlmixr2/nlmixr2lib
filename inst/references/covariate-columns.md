...
### MIX_FAST_ELIM (**canonical for binary mixture-model class indicator: fast-eliminator vs slow-eliminator subpopulation**)
- **Description:** Per-subject latent mixture-model class indicator for a bimodal-clearance acetylator-phenotype-like subpopulation split. 1 = subject classified to the fast eliminator subpopulation (higher apparent clearance); 0 = subject classified to the slow eliminator subpopulation (lower apparent clearance, typical reference category). Founding example is Wilkins 2011 isoniazid in South African pulmonary tuberculosis patients: a two-class NONMEM mixture on apparent oral clearance characterising the N-acetyltransferase-2 (NAT2) polymorphism that drives bimodal isoniazid elimination (typical CL_fast/F = 21.6 L/h, typical CL_slow/F = 9.70 L/h, both at the 70 kg reference weight). Not a measured clinical covariate -- the mixture assignment is the per-subject posterior latent-class index from the source paper's mixture-model NONMEM run; an explicit phenotyping assay (NAT2 genotyping, caffeine probe, etc.) would be required to assign the indicator from external data.
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (slow eliminator subpopulation; majority class at 86.8 % of the Wilkins 2011 cohort).
- **Source aliases:** `$MIX class assignment` -- NONMEM `$MIX` block class index. The Wilkins 2011 paper labels the two classes "fast eliminator" (component-1 typical CL/F = 21.6 L/h) and "slow eliminator" (component-2 typical CL/F = 9.70 L/h); the canonical binary indicator is `MIX_FAST_ELIM = as.integer($MIX == fast component)`.
- **Example models:** `Wilkins_2011_isoniazid.R` (gates the typical apparent CL/F via the power-form covariate effect `e_mix_fast_elim_cl` on CL/F: `cl = exp(lcl + e_mix_fast_elim_cl * MIX_FAST_ELIM + etalcl) * (WT/70)^0.75 * (1 + e_hiv_pos_cl * HIV_POS)` with `lcl = log(9.70)` (slow-eliminator reference) and `e_mix_fast_elim_cl = log(21.6/9.70) approx 0.8005`).
- **Notes:** The population probability of `MIX_FAST_ELIM = 1` is the estimated mixture fraction `P_fast` (Wilkins 2011 Table 2 row "Proportion of fast eliminators in population (P_fast)" = 0.132, RSE 23.2 %); it is registered as the `p_fast` parameter in the source paper's NONMEM control stream. For typical-value simulation set `MIX_FAST_ELIM = 0` (dominant slow-eliminator phenotype). For population simulation, draw `MIX_FAST_ELIM ~ Bernoulli(p_fast)` per subject. Wilkins 2011 Discussion notes the true acetylator distribution is trimodal (slow / intermediate / fast NAT2 phenotypes) but the data could not differentiate intermediate from fast eliminators, so the binary indicator pools intermediate and fast into a single fast-eliminator class. Distinct from `MIX_PDI` (mechanism-of-action mixture), `MIX_LARGE_BASE` (baseline-tumor-size mixture), `MIX_LARGE_RUV` (residual-error-magnitude mixture), `MIX_LAGGED_ABS` (absorption-kinetics mixture), and `MIX_HIGH_VP` (peripheral-volume mixture): `MIX_FAST_ELIM` is specifically a clearance-magnitude mixture driven by the acetylator-phenotype polymorphism. Future popPK extractions that share the same fast-vs-slow CL bimodality may extend this entry; CL mixtures from unrelated dichotomies (e.g., renal-clearance-pathway saturation, dose-dependent kinetic switches) should register a new canonical name. Ratified canonically on 2026-06-13 alongside the Wilkins 2011 isoniazid extraction.
