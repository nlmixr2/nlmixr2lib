...
- **Source aliases:**
  ...
  - `FORM` (Lacy 2018; per-dose-occasion indicator paired with the Study XL184-010 tablet-vs-capsule crossover) -- decompose into `FORM_CAPSULE = as.integer(FORM == "capsule")`. Reference 0 = tablet (Cabometyx 60 mg the approved RCC / CRPC formulation).
...
- **Notes:** Scoped specific because the complement reference category is paper-defined (solution for Hennig itraconazole, tablet for Gupta lenvatinib and Lacy 2018). Sibling to `FORM_TABLET` (Kyhl 2016 / Tikiso 2021 tablet vs solution) under the `FORM_*` family. Future formulation-comparison models that need a capsule indicator should reuse this canonical, extending the example list and documenting the comparator in per-model notes.
