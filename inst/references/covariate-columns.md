...
- **Source aliases:**
  ...
  - `CYP2C9` -- Kleideiter 2017 (paper Table 13 stratification: extensive metabolizers, poor and intermediate metabolizers, unknown). When paired with `CYP2C9_PM_IM`, CYP2C9_EM = 1 means confirmed EM; CYP2C9_PM_IM = 1 means confirmed PM/IM; both = 0 means unknown phenotype (the typical-value reference in Kleideiter 2017).
...
- **Notes:** Pairs with `CYP2C9_EM` for the three-level "unknown / EM / PM-IM" stratification used in Kleideiter 2017, where the unknown-phenotype reference (both indicators = 0) is the most common category. PM and IM are pooled because the Kleideiter 2017 covariate analysis (only 38.3% of subjects with known CYP2C9 phenotype) did not separately resolve them; downstream papers that distinguish PM from IM should register a separate `CYP2C9_PM` canonical and split this group. Ratified canonically on 2026-05-25 alongside the Kleideiter 2017 cebranopadol extraction.
...
- **Example models:** `Hennig_2006_itraconazole.R` (Hennig 2006 Table II final estimates: capsule `ka` 0.09 h^-1 vs solution `ka` 0.96 h^-1 and capsule relative bioavailability 0.55 vs solution 1; the `etalfdepot` IIV applies only to the capsule arm), `Hennig_2007_itraconazole.R` (selects between `lka_cap` and `lka_sol` typical-value absorption rate constants and applies `f(depot) <- (1 - FORM_CAPSULE) + FORM_CAPSULE * fdepot` so the relative bioavailability `F_rel = 0.817` is applied only to the capsule arm), `Gupta_2016_lenvatinib.R` (relative bioavailability of capsule vs tablet is 0.896; F1 fixed to 1 for the tablet reference; the `etalfcap` IIV (30.2% CV) applies only to the capsule arm), `Kleideiter_2017_cebranopadol.R` (paired with `FORM_TABLET` to encode the three-level cebranopadol formulation stratification: tablet reference, oral solution, liquid-filled capsule; multiplicative effects on `ka` (log shift `log(2.09 / 0.864) = 0.883`), `klag` (log shift `log(0.077 / 0.087) = -0.122`), and bioavailability (factor 1.174) for capsules relative to the tablet reference).
- **Notes:** Scoped specific because the complement reference category is paper-defined (solution for Hennig itraconazole, tablet for Gupta lenvatinib and Kleideiter 2017 cebranopadol). Sibling to `FORM_TABLET` (Kyhl 2016 / Tikiso 2021 tablet vs solution; Kleideiter 2017 cebranopadol tablet reference) under the `FORM_*` family. Future formulation-comparison models that need a capsule indicator should reuse this canonical, extending the example list and documenting the comparator in per-model notes.
