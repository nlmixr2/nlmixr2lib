...
- **Description:** 1 = subject (or dose record) received intravenous (IV) administration, 0 = administered via a non-IV route (typically subcutaneous (SC), but also oral (PO) or intraperitoneal (IP) depending on the source study). Per-subject or per-dose-record covariate flagging the dosing route when a population analysis pools cohorts that differ by route, with covariate effects on PK parameters that capture route-specific disposition behaviour.
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (non-IV; usually SC, sometimes oral or IP depending on the source paper -- see Example models).
- **Source aliases:**
  ...
  - "IV" vs "oral" administration route -- used in `Koolen_2010_docetaxel.R` (per-dose-record indicator flagging IV docetaxel vs oral docetaxel, switching the central volume of distribution between V_central_iv = 9.8 L and V_central_po = 44 L).
- **Example models:**
...
- **Notes:** This is the per-subject (or per-dose-record) covariate-equation indicator, distinct from the dosing-event `cmt` column that names the target compartment. When simulating, set `ROUTE_IV = 1` for IV cohorts and dose into the central compartment; set `ROUTE_IV = 0` for the non-IV reference (SC, oral, IP, etc.) and dose into the depot. Scope: specific because the set of parameters that differ by route is paper-specific (Yu 2022 carries route-specific exponential effects on disposition parameters; Zierhut 2008, Wang 2021, and Fiedler-Kelly 2019 carry route-specific PK observation residual SDs; Fiedler-Kelly 2019 and Koolen 2010 additionally carry a route-specific central volume of distribution). The 0-reference category itself is paper-specific (SC in Yu 2022 / Zierhut 2008 / Wang 2021 / Fiedler-Kelly 2019; oral in Koolen 2010); the canonical name encodes the direction of the IV-flagging contrast, not the identity of the comparator route.
