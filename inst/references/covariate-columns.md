...
### DIS_HF_OR_CARDSURG (**canonical for cardiac failure OR cardiac surgery pooled indicator**)
- **Description:** 1 = subject has cardiac failure OR has undergone (or is undergoing the postoperative period of) cardiac surgery, 0 = neither. Pooled binary indicator capturing a low-cardiac-output / diminished hepatic and renal perfusion state. Source papers pool the two contributing conditions because the underlying physiological perturbation (reduced cardiac output -> reduced hepatic and renal blood flow -> reduced clearance of hydrophilic, renally-eliminated drugs) is shared and the per-condition counts are typically small in paediatric intensive-care cohorts. Reference category 0 includes subjects with normal cardiac function and no cardiac-surgical history (or with non-cardiac surgery only).
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (no cardiac failure AND no cardiac surgery).
- **Source aliases:**
  - `HEART` -- Hawwa 2013 ranitidine paper's NONMEM column name; defined in Methods (page 5 final equation block) as a single 0/1 indicator combining "cardiac failure" and "cardiac surgery" subgroups. Table 1 reports the marginal counts (14 with cardiac failure, 10 with cardiac surgery, 17 unique subjects in the union out of 78).
- **Example models:** `Hawwa_2013_ranitidine.R` (multiplicative reduction of clearance: TVCL = 32.1 * (WT/70)^0.75 * 0.463^HEART, i.e., 53.7% reduction when HEART = 1; OFV decrease 12.618, P < 0.001; the paper attributes the effect to reduced hepatic and renal blood flow in proportion to reduced cardiac output).
- **Notes:** Specific scope kept because the pooled cardiac-failure-OR-cardiac-surgery semantic is paper-specific (Hawwa 2013 pools the two because the cohort has 14 cardiac-failure plus 10 cardiac-surgery subjects with 7-subject overlap = 17 unique, and the per-group counts are too small to estimate separate effects). Distinct from `RACHS1` (integer 1-6 paediatric cardiac surgery risk category, requires a known surgical-risk stratification) and from `DIS_HF_OR_LF_SEV` (pools severe heart failure with severe liver failure on the basis of shared end-stage end-organ effects on hepatic clearance; Hawwa's HEART is broader because it includes any cardiac-surgery patient regardless of HF severity, and narrower because it excludes liver failure). A future paper with sufficient counts to separate the two axes should register sibling canonicals (e.g., `DIS_HF` for cardiac failure and `CARDSURG` for cardiac surgery indicator) and compute the OR inline in the model body, rather than reuse this pooled indicator. Ratified canonically on 2026-06-17 alongside the Hawwa 2013 ranitidine extraction.
