...
### MECH_VENT (**canonical for invasive mechanical-ventilation treatment-status indicator**)
- **Description:** 1 = the subject was receiving invasive (positive-pressure) mechanical ventilation during the modeled period (endotracheal intubation or tracheostomy with ventilator support); 0 = no invasive mechanical ventilation (room air, supplemental oxygen via nasal cannula / face mask, or non-invasive positive-pressure support without intubation). Treatment-status flag rather than a measured oxygenation value; captures the structural physiology of positive-pressure ventilation (increased intrathoracic pressure, reduced venous return, reduced cardiac output, altered renal blood flow and antidiuretic-hormone secretion, extracellular-fluid redistribution and oedema) that perturbs the PK of drugs distributing into the extracellular volume in critically ill cohorts. Per-subject indicator in the source data; typically time-fixed at the subject level for the PK sampling window (the cohort was classified at enrolment as ventilated or not), though future papers that resolve session-level changes in ventilation status would use a time-varying form and the per-model `covariateData[[MECH_VENT]]$notes` would document the time resolution.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (no invasive mechanical ventilation).
- **Source aliases:**
  - `VENT` -- used in `Conil_2007_ceftazidime.R` (binary indicator on V2 in a 2-compartment ceftazidime popPK model). Conil 2007 Methods records the indicator at study entry for the 50-patient burn-ICU cohort (16 of 50 ventilated).
- **Example models:** `Conil_2007_ceftazidime.R` (chained multiplicative effect on the peripheral volume V2: `TVV2 = 2.69 * (1 + 1.43*SEXF) * (1 + 2.44*MECH_VENT) * (1 + 0.00414*CRCL)`; ventilated patients have a 2.44-fold larger V2 contribution from the MECH_VENT term, reflecting the paper's hypothesis that PEEP-driven antidiuretic-hormone secretion increases extracellular fluid and peripheral drug distribution).
- **Notes:** Family is the same as `HEMODIAL` and `CRRT_STATUS` (binary treatment-status indicators with general scope). Distinct from `OXYSUP_LOW` / `OXYSUP_HIGH` (decomposed binary indicators from a 4-level ordered oxygen-supplementation categorical, where Lin 2024 pooled the rare mechanical-ventilation cases into `OXYSUP_HIGH`). Use `MECH_VENT` only when the source paper retains invasive mechanical ventilation as its own structural binary, separate from any oxygen-support stratification; source papers that pool mechanical ventilation with high-flow oxygen (e.g., Lin 2024 casirivimab) should continue to use `OXYSUP_HIGH` rather than switching. General scope because invasive mechanical ventilation is a widely studied structural ICU population-PK covariate; a future paper that retains it with the same orientation can extend `Example models` without registering a new canonical. Ratified canonically on 2026-06-17 alongside the Conil 2007 ceftazidime extraction.
...
- **Notes:** Decomposed indicator from a 4-level ordered categorical (no oxygen / low-flow / high-flow / mechanical ventilation). Use with the parallel `OXYSUP_HIGH` indicator. For source papers that split invasive mechanical ventilation from high-flow oxygen (e.g., Conil 2007 ceftazidime burn-ICU cohort), encode mechanical ventilation under the separate `MECH_VENT` canonical rather than extending the `OXYSUP_*` family.
