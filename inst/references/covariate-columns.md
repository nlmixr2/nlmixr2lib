...
- **2026-05-07** -- Added `TERM_BIRTH` (specific scope under a new `Pregnancy / hormonal status` H2 section; binary term-vs-preterm birth indicator), `CONMED_BIRTHCONTROL` (specific scope under the same new `Pregnancy / hormonal status` H2 section; binary oral-contraceptive use indicator; full-word name preferred over the `BC_USE` form initially proposed for the Allegaert 2015 extraction), and `URINE_FLOW` (specific scope under `Renal / hepatic function`; continuous instantaneous urine flow rate, mL/h, with centering reference 100 mL/h and a `URINE_FLOW == 0` sentinel-zero rule that gates the linear effect on renal CL; full-word name preferred over the bare `UF` source abbreviation initially proposed) canonical entries while extracting `Allegaert_2015_paracetamol.R` (DDMODEL00000267). Source aliases mapped: `TERM` -> `TERM_BIRTH` (same orientation, no transformation), `BC` -> `CONMED_BIRTHCONTROL` (same orientation, no transformation), `UF` -> `URINE_FLOW` (rename from source abbreviation to canonical full-word form). The Allegaert 2015 BMC Anesthesiol PDF is not on disk in this worktree; covariate semantics, units, and reference values are taken from the bundle's `Executable_OriginalModelCode.mod` `$INPUT` comments and `$PK` block. Operator confirmed (sidecar response 047-001) the full-word canonical names `CONMED_BIRTHCONTROL` / `URINE_FLOW` over the originally-proposed `BC_USE` / `UF` forms; the change log entry from the prior extraction commit (`ced840f2`) is superseded by this one.
