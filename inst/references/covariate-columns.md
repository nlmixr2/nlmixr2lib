...
- **Description:** Ideal body weight in kg, typically derived from height and sex using the Devine formula or its variants. Time-fixed at baseline unless the source paper states otherwise. Used in size-normalisation of clearance / dose-rate / volume in adult popPK models where the source paper reports IBW as the preferred size descriptor over total body weight (e.g., when overweight subjects pull clearance scaling away from the typical pattern, or when distribution volume scales with lean rather than total mass).
- **Units:** kg
- **Type:** continuous
- **Scope:** general
- **Reference category:** n/a -- used either as a linear ratio `(IBW / ref)` for clearance / dose-rate normalisation, or as a power scalar `(IBW / ref)^exponent` for volume / clearance allometric-style scaling. Reference values observed: 60 kg (Holford 1992 tacrine adult Alzheimer's population mean IBW), 62 kg (Shoji 2011 pregabalin pooled-cohort mean IBW).
- **Source aliases:**
  - `IBW` -- standard abbreviation used directly in Holford 1992 and Shoji 2011.
- **Example models:** `Holford_1992_tacrine.R` (reference 60 kg; tacrine "clearance" relative to IBW = 60 enters as the dose-rate normalisation factor `(60 / IBW)` driving the tacrine effect-compartment input; the Holford-Peace Devine variant is documented in `covariateData[[IBW]]$notes`), `Shoji_2011_pregabalin.R` (reference 62 kg; power-of-ratio scaling on CL/F (exponent 0.354) and V/F (exponent 0.819) per Shoji 2011 Table 3 final model; the IBW formula is the Devine-family variant cited as reference [12] of the paper).
- **Notes:** Promoted from `specific` to `general` on 2026-06-13 when Shoji 2011 pregabalin became the second adult-popPK model to ratify the name. Per-model `covariateData[[IBW]]$notes` should record the formula the source paper used. The Holford-Peace 1992 variant is: men IBW (kg) = 52 + 0.75 * (height_cm - 152); women IBW (kg) = 49 + 0.67 * (height_cm - 152). The classic Devine 1974 formula is: men 50 + 2.3 * (height_in - 60); women 45.5 + 2.3 * (height_in - 60). Other variants (Robinson 1983, Miller 1983, Hamwi 1964) exist; the per-paper choice should be recorded so a user simulating against IBW can match the source's derivation. When the source dataset supplies IBW pre-computed, the column name is typically `IBW` directly. When only `HT` + `SEXF` are provided, the user must compute IBW externally using the source-paper formula before passing it to the model.
