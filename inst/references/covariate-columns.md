...
### DOSE_RTV_MGKG (**canonical for per-administration ritonavir dose per kg body weight**)
- **Description:** Per-administration (per-record) ritonavir dose in milligrams per kg of body weight, supplied as a covariate column on every record (observation and dose). Time-fixed within a given dose regimen; when a subject's regimen changes across study occasions, the column carries the corresponding mg/kg value for each occasion's records. Used to drive a linear deviation on relative bioavailability of co-administered protease-inhibitor / antiretroviral drugs (lopinavir and ritonavir itself in the Zhang 2013 LPV/r-RIF integrated model) of the form `F = BIO * (1 + slp * (DOSE_RTV_MGKG - DOSE_RTV_MGKG_STD))` where `DOSE_RTV_MGKG_STD` is the paper's per-population reference dose (1.5 mg/kg adults, 2.9 mg/kg children in Zhang 2013 Methods).
- **Units:** mg/kg
- **Type:** continuous
- **Scope:** specific
- **Reference category:** n/a -- enters as a centred linear-deviation regressor. Reference values observed: 1.5 mg/kg (Zhang 2013 adults' median ritonavir dose without rifampicin), 2.9 mg/kg (Zhang 2013 children's median ritonavir dose without rifampicin), 14 mg/kg (super-boosted children LPV:RTV 1:1 regimen with antitubercular therapy), 6 mg/kg (children doubled-dose LPV/r regimen with antitubercular therapy).
- **Source aliases:**
  - `DoseRTV` -- Zhang 2013 Methods symbol in the bioavailability deviation equation `F = BIO * (1 + SLP * (DoseRTV - DoseRTV-STD))`; also appears in the upstream Zhang 2012 adults-only precursor (doi:10.1111/j.1365-2125.2011.04154.x) and the Zhang 2012 children-only precursor (doi:10.3851/IMP1939) as the same concept.
- **Example models:** `Zhang_2013_lopinavir_ritonavir.R` (centred linear-deviation effect on relative bioavailability per Zhang 2013 Table 2 'Slope of RTV dose effect on bioavailability' row: LPV F slope = 0.019 / mg/kg for children only with reference 2.9 mg/kg (lopinavir-adults slope was 'not supported'); RTV F slope = 0.46 / mg/kg for adults with reference 1.5 mg/kg and 0.026 / mg/kg for children with reference 2.9 mg/kg).
- **Notes:** Specific scope because the absolute slope coefficient is intrinsic to ritonavir's first-pass metabolism / P-glycoprotein self-inhibition saturation in the LPV/r co-administered setting and is not transferable to non-ritonavir-co-formulated antiretrovirals. Member of the `DOSE_<DRUG>_<UNIT>` family (siblings `DOSE_PHT_MGKGD` for phenytoin mg/kg/day, `DOSE_IND` for indacaterol ug/day). Computed by the user as the per-administration ritonavir dose (mg) divided by current body weight (kg). Operator-approved on 2026-06-17 (sidecar-001 q1 = A) alongside the Zhang 2013 LPV/r-RIF integrated popPK extraction.
