...
### CONMED_CHARCOAL (**canonical for concomitant activated-charcoal coadministration indicator**)
- **Description:** 1 = subject received activated charcoal (medicinal-charcoal binder) for gastrointestinal-luminal drug binding during the modeled period; 0 = no activated charcoal. Treats activated charcoal as a coadministered intervention rather than a decontamination process per the operator-ratified convention (registered alongside the Cooper 2015 sertraline-overdose extraction). Per-subject indicator in the source data; in Cooper 2015 it is treated as time-fixed at the subject level even though the SDAC bolus is administered at a single 1.5-4 h post-overdose timepoint -- the paper does not time-resolve the charcoal dose within subject and the f_CL-char effect on apparent clearance is applied uniformly across the subject's full observation record.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (no activated charcoal).
- **Source aliases:**
  - `SDAC` -- single-dose activated charcoal; used in `Cooper_2015_sertraline.R` (binary indicator on apparent clearance in a 1-compartment sertraline-overdose popPK model; per-subject time-fixed). The "single-dose" dose-resolution distinction is documented per-model in `covariateData[[CONMED_CHARCOAL]]$source_name` and `$notes` rather than carving a sibling `MDAC` canonical.
- **Example models:** `Cooper_2015_sertraline.R` (multiplicative factor on apparent CL: `cl <- exp(lcl + etalcl) * (1 + (e_charcoal_cl - 1) * CONMED_CHARCOAL)` with `e_charcoal_cl = 1.92`, so subjects who received SDAC have 92% higher apparent CL than non-SDAC subjects; per-subject time-fixed indicator; the paper reports 7/28 patients received SDAC at 1.5-4 h post-overdose, treated as a per-subject indicator without time-resolution).
- **Notes:** Ratified canonically on 2026-06-17 alongside the Cooper 2015 sertraline-overdose extraction. Single-dose vs multi-dose activated charcoal (SDAC vs MDAC) administration regimens are not split into separate canonicals -- they are distinguished per-model in `covariateData[[CONMED_CHARCOAL]]$source_name` (e.g., source alias `SDAC` for single-dose, `MDAC` for a hypothetical future multi-dose paper) and `$notes` (which documents the dose-resolution convention used in the source paper). When a future paper time-resolves the charcoal dose within subject (transitioning from 0 to 1 at the time of charcoal administration), the indicator itself remains binary; the per-model notes should document the time resolution. Conceptually adjacent to `HEMODIAL` (extracorporeal renal solute removal) and `CRRT_STATUS` (continuous renal replacement) as a treatment-status indicator that captures a removal-class intervention, but classified under the `CONMED_<drug>` family per the operator response to the Cooper 2015 sidecar (the chemical agent is a coadministered gastrointestinal-luminal binder; the procedure is its administration).
