...
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (Viperidae). Sanhajariya 2018 Table A1 fixes F1(Viperidae) = 1 and estimates F1(Elapidae) = 0.569 relative to that reference -- elapid bites deliver ~57 % of the venom dose of viperid bites on average in the pooled human meta-analysis.
- **Source aliases:** `Viperidae` / `Elapidae` family-name strings used in Sanhajariya 2018 Section 2.2.3 Discussion and inferred for each of the 24 cited primary studies in Table 6 (9 elapid-bite studies, 16 viper-bite studies). No explicit NONMEM column name is disclosed in Appendix Table A1; the binary encoding is the canonical reconstruction.
- **Example models:** `Sanhajariya_2018_snake_venom.R` (Sanhajariya 2018 Table A1 covariate model: `f(central) <- exp(lfdepot + e_snakefamily_elapid_fdepot * SNAKEFAMILY_ELAPID + etalfdepot)` with `lfdepot = log(1)` fixed and `e_snakefamily_elapid_fdepot = log(0.569)`).
- **Notes:** Specific scope because the two-family split is tied to the Sanhajariya 2018 meta-analysis dataset structure; the underlying 24 primary studies in Table 6 split cleanly into elapid (9) and viperid (16) groups, and the sparse pooled data did not support a per-species or per-genus stratification (Section 2.2.3 prose: 'it is possible that some of this variability could be further explained by the snake species, but these data were not available'). The Atractaspididae family is named as the third medically important snake family in Sanhajariya 2018 Section 1 prose but does not appear in any human or animal popPK study in the paper, so a sibling `SNAKEFAMILY_ATRACTASPID` is not yet warranted. Future snake-envenoming popPK extractions that distinguish additional families (e.g., Atractaspididae, or a Colubrid family for back-fanged colubrids) should register sibling canonicals (`SNAKEFAMILY_ATRACTASPID`, `SNAKEFAMILY_COLUBRID`) rather than overload this binary indicator. F1 in the Sanhajariya 2018 model also carries an enormous BSV (CV 275 %, RSE 7 %) that absorbs unknown per-bite injected venom mass on top of the family-level multiplier; downstream simulations should treat the covariate as a typical-value family scaling and the eta on F as the per-bite stochastic component. Ratified canonically on 2026-05-30 alongside the Sanhajariya 2018 snake-venom extraction.
