...
- **Example models:** `Ji_2018_tacrolimus.R` (combinational categorical effect on tacrolimus CL/F that depends on both recipient and donor CYP3A5 status: multiplier 2.314 when the recipient is an expresser and the donor is an expresser (REDE), 1.523 when the recipient is an expresser and the donor is a nonexpresser (REDN), and 1.0 otherwise (RNDE / RNDN reference, which Ji 2018 merged because the two estimated effects were similar)).
- **Notes:** Sibling canonical to `CYP3A5_EXPR` -- the same expresser/nonexpresser semantics applied to the graft donor instead of the recipient. Use this canonical whenever a solid-organ-transplant popPK model parameterises CL/F (or F) on the donor's metabolic genotype as well as the recipient's; the existing `CYP3A5_EXPR` canonical's "subject" is by convention the recipient (the patient receiving the dose), and prior transplant models in nlmixr2lib (`Bergmann_2014_tacrolimus.R`, `Storset_2014_tacrolimus.R`) parameterise on the recipient genotype only because they did not have donor genotype data. The donor-genotype effect is biologically meaningful for graft-metabolised drugs such as tacrolimus, where the transplanted organ's intrinsic CYP3A5 activity contributes to first-pass and systemic metabolism: in Ji 2018 the donor's expresser status further increases CL/F over and above the recipient's own genotype, but only when the recipient is itself an expresser. Future papers that report the donor genotype in a NONMEM `$INPUT` column (e.g., `D_CYP3A5`, `DON_CYP3A5`, `CYP3A5D`) should record their source name here. The expresser-equals-1 orientation is preserved (same as `CYP3A5_EXPR`); papers that report a donor `*3/*3` indicator should invert (`CYP3A5_EXPR_DONOR = 1 - source_indicator`) and document the inversion in `notes`. Ratified canonically on 2026-05-20 alongside the Ji 2018 LDLT tacrolimus extraction.
