...
### CONMED_CAVITD (**canonical for concomitant calcium + vitamin D supplementation indicator**)
- **Description:** 1 = subject is on daily oral calcium + vitamin D supplementation alongside the modelled drug during the observation period; 0 = not on calcium + vitamin D supplementation. Per-subject baseline indicator in osteoporosis / bisphosphonate trials, where calcium + vitamin D coadministration is standard supportive therapy and meaningfully modifies bone-turnover biomarker dynamics independent of the bisphosphonate itself.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (no concomitant calcium + vitamin D supplementation).
- **Source aliases:**
  - "supplemental therapy" / "supplemental calcium and vitamin D therapy" -- Pillai 2004 narrative + Table 4 legend, referring to the daily oral calcium + vitamin D coadministration design feature distinguishing studies MF4361 / MF4411 (supplements) from study MF9853 (no supplements).
- **Example models:** `Pillai_2004_ibandronate.R` (drives the multiplicative effect on the K-PD virtual-effect-compartment equilibration rate KDE: `kde = exp(lkde_no_supp + e_cavitd_kde * CONMED_CAVITD)` with `lkde_no_supp = log(0.112)` and `e_cavitd_kde = log(0.014 / 0.112) = -2.079`, giving an 8-fold slower KDE when supplements are coadministered; also gates the multiplicative `VITD = 1 - VIT * [1 - exp(-KVIT * t)]` suppression term on uCTX so that VITD = 1 when `CONMED_CAVITD = 0` and contributes up to a 31.4% additional uCTX suppression at long times when `CONMED_CAVITD = 1`).
- **Notes:** General scope because calcium + vitamin D coadministration is standard supportive therapy alongside any bone-resorption-active drug (bisphosphonates, denosumab, romosozumab, teriparatide) and the indicator is mechanistically generic across that drug class. Distinct from a hypothetical `CONMED_CALCIUM` (calcium alone) or `CONMED_VITD` (vitamin D alone) sub-canonical -- the combined daily oral calcium + vitamin D regimen is the canonical bisphosphonate / denosumab supportive-therapy package and is treated as a single binary. Source papers that quantitatively separate calcium vs vitamin D effects should register the sub-canonicals at that time rather than overloading this combined indicator. The per-model `covariateData[[CONMED_CAVITD]]$notes` should document the supplementation regimen actually used in the source study (e.g., daily oral calcium 500 mg + vitamin D 400 IU). Founding example `Pillai_2004_ibandronate.R`. Ratified canonically on 2026-06-17 alongside the Pillai 2004 ibandronate extraction.
