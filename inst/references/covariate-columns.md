...
### CTP_SCORE (**canonical for continuous Child-Turcotte-Pugh score**)
- **Description:** Composite Child-Turcotte-Pugh score, integer 5-15 points, sum of five subscores (total bilirubin, serum albumin, INR / prothrombin time, ascites severity, hepatic encephalopathy severity). Class A = 5-6 points (well-compensated cirrhosis), Class B = 7-9 points (significant functional compromise), Class C = 10-15 points (decompensated cirrhosis). Used by popPK / popPD models that encode hepatic-impairment severity as a continuous power-form or linear covariate on disposition parameters, rather than as a dichotomized indicator (`HEPIMP_SEV`, `HEPIMP_MODSEV`).
- **Units:** points (integer-valued)
- **Type:** continuous
- **Scope:** general
- **Reference category:** n/a -- used with power-form effects `(CTP_SCORE / ref)^exponent` or linear-deviation forms `(1 + e * (CTP_SCORE - ref))`. Reference values observed: 5 (Chi 2018, the Class A border value); use the cohort-mean or paper-specified reference in `covariateData[[CTP_SCORE]]$notes`.
- **Source aliases:**
  - `CTP` -- used in `Chi_2018_propofol.R` (paper Methods 'Pharmacokinetics' final regression model; 5-14 points across the enrolled cohort).
  - `Child-Pugh score`, `Child-Turcotte-Pugh score` -- prose aliases in narrative source text.
- **Example models:** `Chi_2018_propofol.R` (power-form effect on peripheral volume: `V2 = 52.9 * (CTP_SCORE / 5)^(-0.848)`; reference 5 = Class A border value; lower CTP_SCORE associated with larger V2 in the 32-patient liver-transplant cohort).
- **Notes:** Time-fixed per subject in popPK models (the score is calculated at study enrolment from the patient's baseline labs and clinical findings). Distinct from `HEPIMP_SEV` (binary Class C indicator) and `HEPIMP_MODSEV` (binary Class B+C indicator): use `CTP_SCORE` when the source paper encodes hepatic-impairment severity as a continuous regressor rather than a dichotomized indicator. The score is paper-independent in its construction (the five subscore weights are standardised internationally), so scope is general. If a future model encodes only the dichotomized class membership, use one of the `HEPIMP_*` canonicals instead.
