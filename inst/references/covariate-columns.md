...
### CONMED_NNRTI (**canonical for concomitant non-nucleoside reverse transcriptase inhibitor (NNRTI) class indicator (efavirenz or nevirapine)**)
- **Description:** 1 = subject is receiving a CYP3A4-inducing non-nucleoside reverse transcriptase inhibitor (NNRTI) -- specifically efavirenz (EFV) or nevirapine (NVP) -- as the third agent in a combination antiretroviral regimen; 0 = subject is on a non-NNRTI comparator regimen specified by the source paper (typically a protease-inhibitor-based regimen). Efavirenz and nevirapine are both CYP3A4 (and CYP2B6) inducers; the pooled class indicator is used by HIV popPK models that find the EFV-vs-NVP induction magnitudes are not statistically distinguishable and pool them into a single class-level binary effect on the apparent clearance of a co-administered substrate drug.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (no concomitant NNRTI; typically a boosted protease-inhibitor regimen).
- **Source aliases:** none standardized; source datasets typically encode the column as `NNRTI`, `IND`, or as a derived `max(EFV, NVP)` indicator. Document the source-column name per-model in `covariateData[[CONMED_NNRTI]]$source_name`.
- **Example models:** `Crommentuyn_2005_lopinavir.R` (multiplicative effect on apparent lopinavir clearance: `cl *= (1 + e_nnrti_cl * CONMED_NNRTI)` with `e_nnrti_cl = 0.39` for the +39% induction effect reported in Crommentuyn 2005 Results page 7 / Table 2 row IND = 1.39; the paper tested separate factors for EFV (1.41) and NVP (1.52) and found no improvement in fit over a pooled class-level NNRTI factor).
- **Notes:** Distinct from the sibling drug-specific `CONMED_EFV` (which captures efavirenz alone with a different paper-defined comparator regimen). Use `CONMED_NNRTI` when the source paper pools EFV and NVP into a single class-level indicator and reports a single coefficient; use `CONMED_EFV` (and a hypothetical `CONMED_NVP`) when the source paper reports separate EFV / NVP effects. A future popPK model that estimates separate drug-specific effects should register a new `CONMED_NVP` canonical for the nevirapine arm rather than overload `CONMED_NNRTI`. Per-model `covariateData[[CONMED_NNRTI]]$notes` should document the exact NNRTIs counted in the `= 1` category (Crommentuyn 2005 counts efavirenz and nevirapine only; etravirine, rilpivirine, and doravirine were not represented in the 2001-2004 cohort and were not tested). Ratified canonically on 2026-06-10 alongside the Crommentuyn 2005 lopinavir extraction; class scope (general) because the EFV-or-NVP pooled indicator is mechanism-driven and not paper-specific.
...
- **Description:** Per-subject (time-fixed within an evaluated regimen) ritonavir AUC over a 12-hour dosing interval (q12h, BID ritonavir), used as a co-medication exposure covariate driving boosted-protease-inhibitor (lopinavir, atazanavir, darunavir, etc.) clearance in popPK models that account for ritonavir's CYP3A4-inhibition effect on a BID regimen. In Crommentuyn 2005 the value is computed per-subject as DOSE_RTV / CL_RTV using individual Bayesian CL_RTV estimates from the Kappelhoff et al. 2005 ritonavir popPK model (Crommentuyn 2005 reference [19], Br J Clin Pharmacol 59:174-82), and feeds the lopinavir CL/F inverse-saturable form `cl = exp(lcl) * (auc50 / (auc50 + CONMED_RTV_AUC_12h)) * IND` with `auc50 = 2.26 mg*h/L` (Crommentuyn 2005 Methods Equations 1-2; Table 2).
- **Units:** `mg*h/L` (document per-model via `covariateData[[CONMED_RTV_AUC_12h]]$units` if a different exposure unit is reported).
- **Type:** continuous
- **Scope:** specific
- **Reference category:** n/a -- enters via the inverse-saturable form `auc50 / (auc50 + CONMED_RTV_AUC_12h)`. Reference value observed: 3.58 mg*h/L (Crommentuyn 2005 cohort median across 122 HIV-1-infected adults on BID LPV/r; range 0.85-18.77).
- **Source aliases:**
  - `AUC12h` -- printed name in Crommentuyn 2005 (Methods Equations 1-2; Results page 6; Table 2 row 3). The paper writes the subscript `12h` to denote the BID dosing-interval AUC; the column name is registered with the `12h` token in the column name to keep the q12h-BID convention visible without relying on subscripts.
- **Example models:** `Crommentuyn_2005_lopinavir.R` (lopinavir CL/F inverse-saturable dependence on CONMED_RTV_AUC_12h: `cl = exp(lcl) * (auc50 / (auc50 + CONMED_RTV_AUC_12h)) * IND` with `auc50 = 2.26 mg*h/L` and `IND = 1 + 0.39 * CONMED_NNRTI`).
- **Notes:** Specific scope -- column meaning is tied to ritonavir as the booster drug AND to the q12h-BID dosing-interval AUC convention. Sibling of `AUC_RTV` (which is the q24h once-daily form, Dickinson 2009 cohort median 7.52 mg*h/L). The two canonicals capture chemically-identical ritonavir AUC but at different dosing-interval scopes; never overload them. For simulation users without observed per-subject ritonavir AUC, the Crommentuyn 2005 cohort median 3.58 mg*h/L reproduces typical-value behaviour (the value at which the inverse-saturable form gives CL/F = 5.73 L/h in the lopinavir + ritonavir steady-state simulation, matching the paper's reported per-cohort-median CL/F). Ratified canonically on 2026-06-10 alongside the Crommentuyn 2005 lopinavir extraction.
