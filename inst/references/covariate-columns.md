...
- **Source aliases:**
  ...
  - Time-varying docetaxel plasma concentration carried directly into the Puisset 2007 PD fit from the upstream Baille 1997 / Bruno 1996 Bayesian individual posthoc profiles (Puisset 2007 has no NM-TRAN-style single-letter source-column name; the value is the canonical `CP_MGL`) -- used in `Puisset_2007_docetaxel.R`.
- **Example models:** `Netterberg_2017_docetaxel.R` (linear drug effect on the proliferation rate of the Friberg myelosuppression chain: `(1 - SL * CP_MGL)` with `SL = 19.27 (mg/L)^-1` after Kloft 2006's `THETA(3)/808*1000` MW-808 conversion; CP_MGL supplied per event row from an upstream docetaxel popPK simulation), `Puisset_2007_docetaxel.R` (linear drug effect `Slope * CP_MGL` on the proliferating compartment of the Friberg myelosuppression chain; the upstream docetaxel PK in Puisset 2007 was Baille 1997 / Bruno 1996 individual Bayesian posthoc profiles supplied externally and not encoded in the model file).
...
### PRIOR_CHEMO_LINES_GE2 (**canonical for >= 2 prior chemotherapy lines indicator**)
- **Description:** 1 = subject received at least two prior chemotherapy regimens (lines) before the analyzed treatment (i.e., the analyzed treatment is the third line or later), 0 = subject received zero or one prior chemotherapy lines (i.e., the analyzed treatment is the first or second line). Time-invariant within a subject (records pretreatment history at baseline). Captures cumulative bone-marrow exposure to cytotoxic chemotherapy at a clinically meaningful threshold that distinguishes more-vs-less heavily pretreated cancer patients; relevant to myelosuppression PD analyses where the bone-marrow proliferating-pool reserve is depleted by extensive prior cytotoxic therapy.
- **Units:** (binary)
- **Type:** binary
- **Scope:** general
- **Reference category:** 0 (0 or 1 prior chemotherapy lines).
- **Source aliases:**
  - `PTT2` -- used in `Puisset_2007_docetaxel.R` (Puisset 2007 NM-TRAN convention; values 0 = patient received less than two lines of chemotherapy before docetaxel, 1 = patient received at least two lines). The companion `PTT1` column in the same paper encodes "first-line or not" and was not retained in the final covariate model; it would correspond to the canonical `LINE_1L` if a future paper required it.
- **Example models:** `Puisset_2007_docetaxel.R` (multiplicative effect on the Friberg drug-effect slope: `slope = ... * e_prior_chemo_lines_ge2_slope^PRIOR_CHEMO_LINES_GE2` with `e_prior_chemo_lines_ge2_slope = 1.69` -- patients with >= 2 prior chemotherapy lines have a 69% higher Slope, i.e., greater neutropenia sensitivity, consistent with cumulative bone-marrow depletion).
- **Notes:** Distinct from `LINE_1L` (which uses a different threshold: 1L vs 2L+; collapses 2L and 3L+ together) and from `PRIOR_ANTICANCER` (which is a 0-vs-any threshold inclusive of radiotherapy / surgery / hormonal therapy and counts any prior anticancer modality). `PRIOR_CHEMO_LINES_GE2` is specifically the >= 2 chemotherapy-line threshold among cytotoxic-systemic-drug therapy lines, capturing the "heavily pretreated" cohort in oncology trials. Pairs naturally with `LINE_1L` when a paper distinguishes 0 / 1 / >=2 prior lines as a three-level construct (Puisset 2007 reports both PTT1 and PTT2 as candidate covariates; only PTT2 was retained in the final model). When a future paper reports a continuous prior-line count, register a parallel canonical (e.g. `PRIOR_CHEMO_LINES_COUNT`) rather than overloading this one.
...
### STUDY_TOULOUSE (**canonical for Puisset 2007 Institut Claudius-Regaud (Toulouse) site indicator**)
- **Description:** 1 = subject enrolled at the Institut Claudius-Regaud (Toulouse) site of the Puisset 2007 docetaxel pooled PK/PD analysis; 0 = Hopital Cochin (Paris) site. The two single-centre studies differed in docetaxel HPLC bioanalytical procedure (interday-CV 15.7% in Toulouse vs 6.0% in Paris) and in PK sampling schedule (4 sampling times in Toulouse vs 3 in Paris), and the corresponding centre effect on both docetaxel CL (+70%) and the Friberg drug-effect slope (+82%) is acknowledged by the authors as a between-centre analytical bias rather than a clinical PD covariate.
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (Hopital Cochin, Paris site, n=55).
- **Source aliases:** `CEN` -- used in `Puisset_2007_docetaxel.R` (Puisset 2007 NM-TRAN convention; Methods page 291: "CEN = 0 or = 1 if data corresponded to Paris or Toulouse, respectively").
- **Example models:** `Puisset_2007_docetaxel.R` (multiplicative effect on the Friberg drug-effect slope: `slope = ... * e_study_toulouse_slope^STUDY_TOULOUSE` with `e_study_toulouse_slope = 1.82` -- Toulouse subjects have an 82% higher Slope; the Puisset 2007 PK companion analysis also retains a 1.7-fold CEN effect on CL).
- **Notes:** Specific scope because the contrast is tied to the Puisset 2007 / Tran 2006 / Puisset 2007 dual-centre French docetaxel program (Institut Claudius-Regaud + Hopital Cochin). The paper explicitly attributes the centre effect to analytical bias between the two HPLC assays plus the coarser PK sampling schedule in Toulouse rather than a generalisable clinical PD covariate (Puisset 2007 Discussion, page 293: "differences at the analytical level (i.e., HPLC procedure) between the two centres were responsible for a bias in the determination of docetaxel CL"). For typical-value simulation outside the Puisset 2007 cohort, leave `STUDY_TOULOUSE = 0` (Paris reference) since the effect is a study-level artefact. Subject-level / time-fixed; set once from the trial identifier on each subject record.
