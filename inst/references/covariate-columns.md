...
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (saline control; no kainate-induced SE).
- **Source aliases:**
  - Paper-specific "rat group" indicator -- used in `Syvanen_2011_verapamil_rat.R` (the paper's "rat group" categorical covariate, with 1 = kainate-treated and 0 = saline-treated; the canonical column has identical semantics).
- **Example models:** `Syvanen_2011_verapamil_rat.R` (multiplicative theta^COVARIATE form on `vbr1`: `vbr1 = ... * exp(e_dis_postse_kainate_vbr1 * DIS_POSTSE_KAINATE + ...)` with `e_dis_postse_kainate_vbr1 = log(1.32) = 0.2776` (32% increase in fast-exchange brain compartment volume in post-SE kainate rats vs saline controls). The paper screened the post-SE state as a covariate on `Qin` and `Qout` as well but did not retain either; the only retained effect is on `Vbr1`, which the paper's Discussion interprets as increased intracellular brain distribution rather than altered BBB transport).
- **Notes:** Scope: specific because the only on-disk source is the Syvanen 2011 rat PET kainate-vs-saline study at a single 7-day post-induction interval and the canonical column meaning is intrinsically tied to that experimental design. The per-paper SE-induction protocol and post-induction interval MUST be documented in `covariateData[[DIS_POSTSE_KAINATE]]$notes` because the underlying P-gp / brain-distribution biology evolves over the post-SE timecourse (acute SE -- early post-SE [days 1-7] -- pre-epileptic [weeks 2-3] -- chronic spontaneous epilepsy [weeks 3+]); a paper that uses a different post-induction interval represents a different biological state and may warrant a successor canonical (e.g., `DIS_POSTSE_KAINATE_28D` for a 4-week-post-induction state) rather than overloading this one. Distinct from [[SEIZURE_ACUTE]] (an acute focal-seizure-activity indicator capturing transient pilocarpine-evoked limbic seizures DURING the modelled observation window, used in `Clinckers_2008_MHD_rat.R`) -- `DIS_POSTSE_KAINATE` captures a chronic disease state established BEFORE the modelled observation window, with no active seizures during sampling. Distinct also from any human-epilepsy disease-state indicator: this canonical is rat-preclinical-only by construction; a future human-epilepsy popPK paper should register a separately scoped canonical (e.g., `DIS_EPILEPSY` for chronic human epilepsy or `DIS_DRE` for drug-resistant epilepsy). Ratified canonically on 2026-06-03 alongside the Syvanen 2011 (R)-[11C]verapamil rat PET extraction.
