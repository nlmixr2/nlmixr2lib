...
### BIRTHCONTROL (**canonical for oral-contraceptive use indicator**)
...
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (no oral contraceptive). Effect form in Allegaert 2015 is multiplicative: `CL_glucuronide *= theta_BIRTHCONTROL` when `BIRTHCONTROL == 1`, with `theta_BIRTHCONTROL = 1.46` (estrogen-driven UGT2B7 induction).
- **Source aliases:**
  - `BC` (Allegaert 2015 NONMEM column; same orientation, no transformation) -- used in `Allegaert_2015_paracetamol.R`.
- **Example models:** `Allegaert_2015_paracetamol.R`.
- **Notes:** Specific scope because the canonical encoding pools all oral contraceptive types (combined / progestin-only) into a single binary; future models that need to distinguish formulations should register a finer-grained canonical (e.g., `BIRTHCONTROL_COMBINED`, `BIRTHCONTROL_PROGESTIN`). The full-word canonical name was chosen over a shorter `BC_USE` form for clarity in source traces.
...
### CAPSULE (**canonical for capsule-vs-solution oral formulation indicator**)
- **Description:** 1 = capsule formulation, 0 = oral solution. Per-record (formulation-fixed at each dose) binary covariate flagging the formulation when a population analysis pools capsule and oral-solution arms and tests formulation as a covariate on absorption rate (`ka`) and relative bioavailability (`fdepot`).
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (oral solution; `fdepot` fixed to 1 for the solution arm in Hennig 2006).
- **Source aliases:**
  - `PREP` (1 = capsules, 0 = oral solution) -- used in `Hennig_2006_itraconazole.R` (the source NMTRAN `$INPUT` column from `HENNIG S 2006 Clin Pharmacokin Example_Metabolite(Itraconazole)_CTL File.ctl`).
- **Example models:** `Hennig_2006_itraconazole.R` (Hennig 2006 Table II final estimates: capsule `ka` 0.09 h^-1 vs solution `ka` 0.96 h^-1 and capsule relative bioavailability 0.55 vs solution 1; the `etalfdepot` IIV applies only to the capsule arm).
- **Notes:** Specific scope because the "capsule vs oral solution" contrast is tied to Hennig 2006's itraconazole formulation-comparison design. Distinct from `TABLET` (Kyhl 2016 nalmefene tablet vs solution) and the `FORM_*` family (drug-product-version indicators); future capsule-vs-solution comparisons should extend this entry rather than register a new canonical.

### CAPSULE
- **Description:** 1 = capsule formulation, 0 = oral solution. Per-dose-record (within-subject in cross-over designs) categorical covariate selecting between two formulation-specific absorption rate constants and applying a relative-bioavailability factor to the capsule arm.
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (oral solution; the typical-value reference whose relative bioavailability is fixed to 1).
- **Source aliases:**
  - `PREP` -- used in `Hennig_2007_itraconazole.R` (Hennig 2007 NONMEM `$INPUT` column with PREP = 1 = capsule and PREP = 0 = oral solution; same orientation as the canonical, no transformation).
- **Example models:** `Hennig_2007_itraconazole.R` (selects between `lka_cap` and `lka_sol` typical-value absorption rate constants and applies `f(depot) <- (1 - CAPSULE) + CAPSULE * fdepot` so the relative bioavailability `F_rel = 0.817` is applied only to the capsule arm).
- **Notes:** Specific scope because the capsule-vs-oral-solution contrast is paper-specific (different formulation-comparison studies use different reference categories and drive different parameter splits). Distinct from `TABLET` (Kyhl 2016 nalmefene tablet vs solution -- a different solid dosage form vs solution contrast) and from the `FORM_*` family (drug-product-version indicators). When a future capsule-vs-solution study reuses this canonical, extend the example-model list rather than register a new name.
...
- **2026-05-08** -- Added `TRIG` (general scope, under `Cardiometabolic / target biomarkers`; serum triglyceride concentration with linear-deviation effect form, reference 5.3 mmol/L from the Archary 2019 lamivudine equation centring), `EARLY_ART` (specific scope, under `Infectious disease (HIV)`; early-vs-delayed antiretroviral-treatment-initiation arm indicator from the MATCH trial), and `DAY14` (specific scope, under `Occasion / period (IOV)`; within-subject day-14-post-ART landmark indicator that gates a step change in typical-value PK parameters between day 1 and day 14 of treatment) canonical entries while extracting `Archary_2019_abacavir.R` and `Archary_2019_lamivudine.R` (Br J Clin Pharmacol 2019 doi:10.1111/bcp.13998). The DAY14 covariate is the operator-approved (sidecar response-001 Q2 = 2i) representation of the source paper's day-1-vs-day-14 step change in CL/F (abacavir) and ka (lamivudine) -- the source attributes the step to nutritional rehabilitation + hepatic auto-induction over the first ~2 weeks of treatment in severely malnourished HIV-infected children. Source aliases mapped: `EARLY` -> `EARLY_ART` (Archary 2019 prose; same orientation, no transformation).
- **2026-05-08** -- Added `CAPSULE` (specific scope under `Formulation / assay / study`; binary capsule-vs-oral-solution formulation indicator with capsule = 1) canonical entry while extracting `Hennig_2006_itraconazole.R` (Hennig 2006 Clin Pharmacokinet 45(11):1099-1114). Source alias `PREP` from the supplied `.ctl` `$INPUT` mapped to `CAPSULE` (no value transformation; same orientation 1 = capsules / 0 = oral solution as the canonical). Distinct from `TABLET` (Kyhl 2016 nalmefene tablet-vs-solution) and the `FORM_*` family because the capsule-vs-solution contrast is a dosage-form contrast specific to Hennig 2006's itraconazole formulation comparison.
...
- **2026-05-07** -- Added `TERM_BIRTH` (specific scope under a new `Pregnancy / hormonal status` H2 section; binary term-vs-preterm birth indicator), `BIRTHCONTROL` (specific scope under the same new `Pregnancy / hormonal status` H2 section; binary oral-contraceptive use indicator; full-word name preferred over the `BC_USE` form initially proposed for the Allegaert 2015 extraction), and `URINE_FLOW` (specific scope under `Renal / hepatic function`; continuous instantaneous urine flow rate, mL/h, with centering reference 100 mL/h and a `URINE_FLOW == 0` sentinel-zero rule that gates the linear effect on renal CL; full-word name preferred over the bare `UF` source abbreviation initially proposed) canonical entries while extracting `Allegaert_2015_paracetamol.R` (DDMODEL00000267). Source aliases mapped: `TERM` -> `TERM_BIRTH` (same orientation, no transformation), `BC` -> `BIRTHCONTROL` (same orientation, no transformation), `UF` -> `URINE_FLOW` (rename from source abbreviation to canonical full-word form). The Allegaert 2015 BMC Anesthesiol PDF is not on disk in this worktree; covariate semantics, units, and reference values are taken from the bundle's `Executable_OriginalModelCode.mod` `$INPUT` comments and `$PK` block. Operator confirmed (sidecar response 047-001) the full-word canonical names `BIRTHCONTROL` / `URINE_FLOW` over the originally-proposed `BC_USE` / `UF` forms; the change log entry from the prior extraction commit (`ced840f2`) is superseded by this one.
...
- **2026-05-08** -- Added `TCHOL` (general-scope, total serum cholesterol concentration; placed under `Cardiometabolic / target biomarkers` directly after `HDLC`) canonical entry while extracting `Archary_2018_lopinavir.R`. The unqualified `CHOL` source alias is the Archary 2018 NONMEM column abbreviation; `TC` is an alternative lipid-panel abbreviation. The `T` prefix follows the existing `TBILI` (total bilirubin) and `TPRO` (total serum protein) "T = total" convention, distinguishing total cholesterol from the HDL fraction (`HDLC` already registered). Reference value 3 mmol/L (Archary 2018 paper-reference / cohort-mean baseline; equivalent to ~116 mg/dL).
- **2026-05-08** -- Added `CONMED_AZOLE` (general-scope binary indicator for concomitant azole antifungal therapy; placed under `Concomitant / prior medication` after `STATIN`) canonical entry while extracting `Kirubakaran_2022_tacrolimus.R` (BJCP 89(4):1162-1175, doi:10.1111/bcp.15566). Source alias `AZOLE` mapped. Time-varying per subject; the Kirubakaran 2022 dataset carries the indicator forward for one week after azole discontinuation to accommodate the long half-life of itraconazole. Distinct from the existing `CONMED_*` family (IBD-immunosuppressant indicators) because azoles are CYP3A4 / P-gp inhibitors and the canonical's mechanistic relevance is calcineurin-inhibitor-style drug-drug-interaction modelling.
- **2026-05-08** -- Added `CAPSULE` (specific-scope, capsule-vs-oral-solution formulation indicator placed under `Formulation / assay / study` next to `TABLET`) canonical entry while extracting `Hennig_2007_itraconazole.R` (Hennig 2007 BJCP 63(4):438-450). Source alias mapped: `PREP` -> `CAPSULE` (same orientation). Distinct from `TABLET` (Kyhl 2016 nalmefene tablet vs solution -- a different solid dosage form vs solution contrast) and from the `FORM_*` family (drug-product-version indicators); future capsule-vs-solution comparison studies should extend the example-models list rather than register a new canonical.
