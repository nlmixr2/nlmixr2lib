...
### CONMED_RIF (**canonical for concomitant rifampicin co-administration at full CYP3A4 induction**)
- **Description:** 1 = subject is on concomitant rifampicin (typically daily 600 mg oral rifampicin) and has reached the post-induction equilibrium state on the perpetrator drug's enzyme-induced expression; 0 = subject is not on rifampicin or is still inside the pre-induction lag window (rifampicin-specific induction kinetics generally require several days of daily dosing for full effect). Time-varying per subject because rifampicin co-administration starts and the induction state is reached only after a paper-specific lag (typically 3-7 days of daily dosing).
- **Units:** (binary)
- **Type:** binary
- **Scope:** specific
- **Reference category:** 0 (no rifampicin co-administration or pre-induction lag).
- **Source aliases:**
  - `RIF` -- used in `Svensson_2014_bedaquiline.R` (paper-defined indicator switching on at day 3 of 600 mg daily rifampicin co-administration; the 3-day lag was selected by NONMEM objective-function search over 1-8 day candidates and chosen as the best fit per the paper's Methods).
- **Example models:** `Svensson_2014_bedaquiline.R` (multiplicative factor on apparent CL_BDQ and CL_M2: `cl_eff = cl_base * 4.78^CONMED_RIF`; 4.78-fold induction of apparent clearance at full induction).
- **Notes:** Distinct from `CONMED_RIF_LPVR4` (which is a compound indicator for rifampicin + super-boosted LPV/r 4:4 used by Tikiso 2021 pediatric abacavir popPK and carries a non-CL effect). The general `CONMED_RIF` covers any popPK study of a CYP3A4 substrate where rifampicin is used as a perpetrator at standard doses with the full-induction lag handled by the data-supplier. The exact lag-to-full-induction (3, 7, 14 days, etc.) must be documented in `covariateData[[CONMED_RIF]]$notes` per model. Future popPK extractions that hold rifampicin co-administration as a binary covariate at full induction should reuse this canonical; if a future model needs a paper-specific time-decaying induction trajectory rather than a step indicator, register a separate canonical (`CONMED_RIF_INDUCTION_TIME`). Specific scope because the post-induction-lag definition is paper-specific; promote to general when a second model legitimately ratifies the same step-indicator semantics. Ratified canonically on 2026-05-21 alongside the Svensson 2014 bedaquiline DDI extraction.
...
- **Type:** continuous
- **Scope:** specific
- **Reference category:** n/a (continuous). In the Schipani 2011 nevirapine model the count is decomposed in `model()` into mutually-exclusive heterozygous (`count == 1`) and homozygous (`count == 2`) indicators, each multiplied by an independently estimated CL/F shift (-0.5 L/h for GT, -1.3 L/h for TT relative to the GG reference).
- **Source aliases:**
...
- **Type:** continuous
- **Scope:** specific
- **Reference category:** n/a (continuous). In the Schipani 2011 nevirapine model the count is decomposed in `model()` into a heterozygous indicator (`count == 1`) multiplied by an estimated CL/F shift (-1.4 L/h relative to the TT reference); the homozygous indicator (`count == 2`) is not estimated because no 983CC homozygotes have been reported in any published cohort.
- **Source aliases:**
...
- **Example models:** `Schipani_2011_nevirapine.R` (additive linear shift on CL/F: `cl = exp(lcl) + e_983tc_cl * (SNP_CYP2B6_RS28399499_C_COUNT == 1) + ...`; 983TC heterozygotes have approximately 40% lower CL/F than the TT reference; Schipani 2011 Table 2).
- **Notes:** Distribution in the Schipani 2011 European HIV-positive cohort (Table 1, n = 275): 983TT 97%, 983TC 3%, 983CC 0%. The 983T>C variant is also known as CYP2B6*18 and is found primarily in African and African-admixed populations (allele frequency ~6-8% in West Africans, ~0% in Europeans and East Asians); the Schipani 2011 cohort included 33% Black-ethnicity subjects, which explains the observed 3% heterozygote frequency. No 983CC homozygotes have been described in the published literature as of the 2011 report (Schipani 2011 Results paragraph 4), so models that include the variant typically estimate only a heterozygous shift. Often co-tested with 516G>T (linkage-disequilibrium block: subjects homozygous for both 516TT and 983TC are CYP2B6 "slow metabolizers" of efavirenz / nevirapine). The count-form encoding follows the same rationale as `SNP_CYP2B6_RS3745274_T_COUNT`. Ratified canonically on 2026-05-21 alongside the Schipani 2011 nevirapine extraction.
