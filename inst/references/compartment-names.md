...
### abs_site (**canonical extravascular absorption-site space**)
- **Type:** compartment
- **Role:** Absorption-site / distribution space that sits downstream of an extravascular dosing `depot` and upstream of `central`, holding drug that has left the administration site but has not yet reached the systemic circulation. The state carries an **amount** (nmol, mg) and has no volume of its own, so concentrations are not formed from it. Use it when a paper resolves the extravascular absorption step as a mechanistic space in which the drug undergoes more than simple first-order transfer -- binding to a receptor pool at quasi-steady state (FcRn in the founding examples), local degradation, more than one competing route onward to plasma, and in particular **back-uptake from plasma** into the same space. That last feature is what distinguishes it from every transit / depot canonical: flux runs in both directions between `abs_site` and `central`, so the space is not a pure feed-forward absorption chain.
- **Source aliases:**
  - `ABS` -- `$MODEL NCOMP=4 COMP=(DEPOT,DEFDOSE) COMP=(ABS) COMP=(CENTRAL) COMP=(PERIPH)` in Jung 2023 Codes S8 / S9; the `$DES` comments name `A(2)` the "absorption site".
  - "distribution space" -- Ngo 2020's own prose name for the same state (Ngo 2020 Figure 1a).
- **Example models:** `Jung_2023_HL2351.R` (founding example: free drug in `abs_site` equilibrates with FcRn by the QSS quadratic, is degraded at `kdeg`, moves onward to plasma either at the fractal rate `kf` or by FcRn-mediated recycling at `ka2`, and is taken back up from plasma at `kup`), `Ngo_2020_HL2351.R` (the same space in the upstream base model).
- **Notes:** Registered 2026-09-04 (PMC9867137), following the precedent by which `presystemic` was itself ratified. Three alternatives were considered and rejected, recorded here so they are not re-opened. `depot2` / `depot_sc` would misdescribe the state as a **second dosing route**, whereas it is strictly sequential -- downstream of the one depot that actually receives the dose. `presystemic` was rejected because that canonical is scoped to gut-wall / portal **first-pass metabolism** on the oral route, where the defining feature is competing metabolite-formation pathways; widening it to cover an extravascular FcRn-recycling space would give one name two mechanisms and lose the ability to search the register for either. `isf` was rejected because it is registered for body-fluid distribution **volumes** in mechanistic mAb models -- it carries a concentration in a physiologic volume -- while `abs_site` carries a bare amount, and neither Jung 2023 nor Ngo 2020 calls the space interstitial fluid. Naming this state `peripheral1` is a specific known error: it collides with the true peripheral compartment and silently builds one fewer ODE state than the paper published (the defect fixed in `Ngo_2020_HL2351.R` when this entry was founded).
...
- **Notes:** Source NONMEM idiom is `$MODEL COMP=(CUMHAZ)` with `DADT(<cumhaz>) = HAZARD`. The pre-2026-06-19 register carried a separate `cumHazard` canonical for single-hazard models (used in `Zecchin_2016_survival.R`); the 2026-06-19 audit collapsed it into this entry under the maintainers' rule that single-hazard models drop any suffix and that all cumulative-hazard compartment names are uniformly lowercase.
...
- **Notes:** Distinct from `brain_csf` (the cerebrospinal-fluid compartment) and from the bare `ecf` (the general brain / tumor extracellular-fluid compartment used by simpler microdialysis models without an explicit CSF compartment, e.g., `Campagne_2019_cyclophosphamide_mouse.R`). Pre-2026-06-24 `brain_ecf` was a deprecated alias of `brain_csf`; the maintainers restored the name 2026-06-24 under the role-based `brain_<region>` namespace so SBPK papers that distinguish parenchymal ECF from CSF can encode both. Existing pre-2026-06-24 models using `brain_csf` for the parenchymal extracellular compartment (e.g., Syvanen 2012) are NOT retrofitted.
...
Adopted 2026-08-06 with the first lactation model in the library (`Wattanakul_2024_primaquine.R` / `Wattanakul_2024_primaquine_motherinfant.R`; PMC11078975; ratified by the maintainers).
...
### bile_transit (**canonical bile-duct transit chain**)
- **Type:** compartment
- **Role:** Catenary chain of bile-duct sub-compartments imposing a fixed transport **delay** between hepatic biliary secretion and arrival of drug in the gut lumen, in the Bischoff et al. (1971) / Harrison and Gibaldi (1977) enterohepatic-recycling form. Numbered members `bile_transit1`, `bile_transit2`, ... are in series and each holds an **amount**. With a per-sub-compartment mean residence time `mtt_bile`, `d/dt(bile_transit1) <- ram - bile_transit1 / mtt_bile` and `d/dt(bile_transit_j) <- (bile_transit_(j-1) - bile_transit_j) / mtt_bile`, where `ram` is the hepatic biliary secretion rate; the flux delivered downstream to `gut_lumen` is `bile_transit_n / mtt_bile`. The total delay imposed by the chain is `n * mtt_bile`, so the chain length and the per-compartment residence time are not separately identifiable from a plasma profile alone and both are taken from the source. Chain length is the paper's; 3 is common.
- **Source aliases:**
  - `R_1` / `R_2` / `R_3` -- bile-duct sub-compartment states of Law et al. (2017) Appendix eq A5, written there on the rate scale (`Rt * dR_j/dt = R_(j-1) - R_j`).
- **Example models:** `Law_2017_egcg_human_pbpk.R` (founding example; n = 3 with `mtt_bile` 0.03 h), `Law_2017_egc_human_pbpk.R`, `Law_2017_ec_human_pbpk.R`, `Law_2017_egcg_rat_pbpk.R`, `Law_2017_ecg_rat_pbpk.R`, `Law_2017_ec_rat_pbpk.R` (the rat siblings use `mtt_bile` 3.00 / 0.30 / 2.00 min).
- **Notes:** `bile_transit<n>` is a blessed chain prefix in `compartmentRegex`; the bare `bile_transit` is reserved for a model that lumps the whole bile-duct delay into one state. Distinct from `a_bile`, which is a **cumulative record** of biliary output and is a sink rather than a delay -- a model needing both a delay chain and a cumulative biliary total can carry both. Distinct from `gallbladder`, which is a storage organ with **gated** emptying producing a discrete secondary peak, rather than a continuously-draining delay. Do **not** use the bare absorption-chain prefix `transit<n>` for this role: a model with enterohepatic recycling normally also carries an oral absorption chain or `depot`, and the two would collide. The paired parameter is `lmtt_bile` (see `parameter-names.md`); the downstream luminal state is `gut_lumen`, from which drug either returns to the perfused `gut` tissue at `kreab` or leaves by faecal transit. Registered by the maintainers' ruling 2026-09-21 (PMC5464336) rather than left in `paper_specific_compartments`, because an n-sub-compartment bile-duct delay is a recurring literature idiom (Bischoff 1971; Harrison and Gibaldi 1977; Ploeger 2000 glycyrrhizic acid) that a future enterohepatic-recycling extraction will hit again.
...
- **Example models:** `Poels_2025_elranatamab_qsp.R`.
- **Notes:** Founding example Poels 2025 (Supplementary Eqs 7, 30-35), adapting Chen et al. (2019) Clin Transl Sci 12:600-608. **Deliberately generic rather than `il6_*`.** Poels 2025 models "a pro-inflammatory cytokine (e.g., IL-6)" and calibrates it against observed IL-6; naming the states `il6_*` would overclaim, because IL-6 is the calibration data rather than the modelled species, and it would leave the registered `il6` canonical -- which means actual IL-6 -- competing for the name. Record the IL-6 calibration provenance in the model's source-trace comments. Naming ruling by the maintainers (PMC12402305).
...
- **Notes:** The `cfb` suffix follows the `das28` / `das28cfb` precedent for marking a change-from-baseline variant, and the lowercase run-together form follows `das28` / `d_updrs`. Note the deliberate distinction from the metabolite-suffix token `vas` (canonical vascular-pool suffix, Ayyar 2024 PBPK) -- the full `druglikingvascfb` name avoids that collision, which is why a bare `vas` observation variable must never be used for a VAS score. Walsh 2024 bounds the logit transformation at -1 to 52 (not 0 to 100) because the period-corrected bipolar score is a change from a neutral 50, so 52 is just above the largest attainable increase.
...
- **Notes:** Ratified 2026-09-03 alongside the Ravva 2015 varenicline craving extraction, as a family-conforming member of the clinical-score PD output family (`pasi`, `das28`, `cows`, `desiretousevas`, `tmccfb`) under the maintainers' standing ruling that family-conforming names need no separate ruling and only new-family mints are referred to them. Lowercase run-together instrument name, per that family. Distinct from `desiretousevas`, which is a single-item unipolar VAS for opioid craving; the Smoking Urges Scale is a five-item questionnaire and its endpoint is the item mean. The related Minnesota Nicotine Withdrawal Scale (MNWS) endpoints are NOT covered by this name -- an MNWS model needs its own canonical (the MNWS Question-1 "urge to smoke" item is an ordered category 0-4, not a 0-100 reading).
...
### adsorbed (**canonical cumulative extracorporeal-adsorption load**)
- **Type:** compartment
- **Role:** Cumulative amount of drug adsorbed onto an extracorporeal sorbent cartridge during hemoadsorption / hemoperfusion, integrating the adsorption flux (`d/dt(adsorbed) <- cl_hemoadsorption * Cc`). A terminal sink holding an amount, but unlike the purely diagnostic `a_<process>` accumulators its value FEEDS BACK into the dynamics: in a saturable-adsorption model the cartridge load is what drives the sorbent toward exhaustion, so the adsorption clearance is a decreasing function of `adsorbed` (`CLmax * (1 - adsorbed / Amax)`) and the state is load-bearing. Not a biological matrix -- `compartmentData[[...]]$specimen` is `"not applicable"`.
- **Source aliases:**
  - `A_CytoSorb`, `ACytosorb(t)` -- Leber 2023 Eq. 2 notation for the drug amount already adsorbed on the cartridge.
- **Example models:** `Leber_2023_tacrolimus_sheep.R`, `Leber_2023_ciclosporin_sheep.R`, `Leber_2023_mycophenolateMofetil_sheep.R`, `Leber_2023_everolimus_sheep.R`, `Leber_2023_methylprednisolone_sheep.R` (founding examples; all five share the linear-decrease saturable sub-model `CL_CytoSorb(t) = CLmax * (1 - adsorbed / Amax)`).
- **Notes:** Registered by the maintainers' ruling 2026-09-05 (PMC10623319) under the bare role name `adsorbed` rather than an `a_`-prefixed form, alongside the registered bare sink `urine`. Device-generic rather than CytoSorb-specific -- any sorbent-cartridge hemoperfusion model (cytokine adsorbers, activated-charcoal columns, immunoadsorption columns) accumulates the same quantity, so the name deliberately carries no brand. Paired with the time-varying `HEMOADSORB_ACTIVE` covariate, which gates the adsorption arm off when no cartridge is in circuit, and with `lclmax_hemoadsorption` / `lamax_hemoadsorption`. Distinct from `urine` / `a_bile` / `a_feces`, which are physiological excretion sinks, and from the `complex` TMDD state, which holds a reversibly bound species in equilibrium with free drug rather than an irreversibly captured cumulative load. Because the state is cumulative, toggling `HEMOADSORB_ACTIVE` back on does not represent a fresh cartridge; a model simulating a cartridge change must reset it explicitly.
---
...
Ratified 2026-09-02 (PMC13274749).
...
- **Source aliases:** none.
- **Example models:** anticoagulant PD models, e.g. `Liesenfeld_2006_dabigatran_aPTT.R`.
- **Notes:** **Mixed case is deliberate and must not be normalised.** `aPTT` is a specific laboratory term whose published spelling carries the lower-case `a` for *activated*, distinguishing it from PTT; it is not a camelCase concatenation of words. The 2026-09-10 pass that renamed the other 15 camelCase canonicals to snake_case (`walkDist` -> `walk_dist`, `viralLoad` -> `viral_load`, ...) left this one alone by the maintainers' decision. Do not "fix" it to `aptt` or `APTT`. The name is also carried by a model filename, so a rename would move a file as well.
...
- **Notes:** Distinct from the bpm-valued heart-rate observable `HR` used by `Feng_2012_higenamine.R`, `Hwang_2023_carvedilol.R` and the `Langdon_2010_PF00821385_*.R` pair -- do NOT use `f_hr` for a model whose endpoint is beats per minute, and do not use `HR` for a normalised endpoint, because the residual-error magnitude and `Emax` are on incomparable scales. The `f` prefix carries its usual register meaning of "fraction" (`fdepot`, `fm`, `ffo`). Residual error follows the standard per-output rule: `addSd_f_hr`. Back-transforming to bpm requires the per-individual baseline and maximal delta-HR, which are data-normalisation constants rather than fitted parameters, so a model observing `f_hr` is not directly comparable to one observing `HR`.

### d_hr (**canonical change-from-baseline heart rate**)
- **Type:** compartment
- **Role:** Change from baseline in heart rate, in beats per minute (`d_hr(t) = HR(t) - HR_baseline`), a signed quantity. Used as the observation variable in concentration-DeltaHR exposure-response models -- most often the companion analysis that establishes the first Garnett assumption ("no effect of drug on heart rate") underpinning a concentration-QTc analysis, where the endpoint of interest is the drug-induced *change* rather than the absolute rate.
- **Source aliases:** `DeltaHR`, `d_hr` -- translate to `d_hr`.
...
- **Notes:** There is a second, model-mechanical reason to prefer `d_hr` over `HR` for a change-from-baseline endpoint: a C-DeltaHR model typically needs the subject's BASELINE heart rate simultaneously, as the covariate column `HR`, and an observation variable named `HR` would shadow that column inside `model()`. Distinct from `f_hr` (the same excursion normalised to each individual's own maximum, unitless and bounded in `[0, 1]`) and from `HR` (absolute beats per minute). Residual error follows the standard per-output rule (`addSd` for a single-output model, `addSd_d_hr` when multi-output). Note the deliberate asymmetry with `QTc` / `QTcF`, which are used as the observation name in change-from-baseline concentration-QTc models (`Darpo_2014_racSotalol_QTcF.R`, `Mukker_2026_tuvusertib_QTcF.R`) rather than a `dQTcF` sibling: that predates this entry and there is no covariate-shadowing pressure there, since those models take baseline QTc as `QTC_BL` rather than `QTc`.

### d_rr (**canonical change-from-baseline RR interval**)
- **Type:** compartment
- **Role:** Change from baseline in the RR interval -- the time elapsing between two consecutive R waves on an electrocardiogram -- in milliseconds (`d_rr(t) = RR(t) - RR_baseline`), a signed quantity. Used as the observation variable in concentration-DeltaRR exposure-response models, i.e. the heart-rate companion analysis of a concentration-QTc study fitted on the reciprocal (interval) scale rather than the beats-per-minute scale. A positive `d_rr` is RR lengthening, that is heart-rate *slowing*; the sign convention is therefore opposite to `d_hr`.
- **Source aliases:** `DeltaRR`, `RR` (bare, when the source labels the delta endpoint by the interval name -- e.g. the Patel 2020 Figure S2 y-axis) -- translate to `d_rr`.
...
- **Source aliases:** `hERG inhibition`, `% hERG block`, `tail-current inhibition` -- translate to `herg_inh` and express as a FRACTION (0-1), not a percentage, so that `imax` is comparable across models.
...
- **Source aliases:** `fold increase in accumulation`, `relative accumulation`, `E` (Agarwal 2011 Methods Equation 3) -- translate to `vbl_accum` and express as a FOLD CHANGE relative to control, not a percentage and not an absolute pmol/mg protein, so that `e0`, `emax` and `ic50` are comparable across models.
...
- **Source aliases:** `K`, `K+`, `serum_K`, `POTAS` -- translate to `serum_k` when assembling input data; document the source-paper symbol in the model file's `description`.
...
### crcl, crclObs (**canonical creatinine clearance**)
- **Type:** compartment
- **Role:** Creatinine-clearance PD output / renal-function state, in mL/min. Used in models that put a creatinine-clearance TIME COURSE on a state or algebraic observable rather than consuming a single baseline value -- gestational renal adaptation, renal-function recovery after injury, ontogeny of renal elimination. Distinct from the covariate column `CRCL`, which is the time-fixed observed BASELINE renal function used to scale parameters; this compartment is the modelled trajectory. `crclObs` is the canonical sibling name for the **observed / model-predicted** creatinine clearance when a model separates it from the integrated state; see the `uacr` / `uacrObs` entry for the rationale. Use the sibling only when the two genuinely differ; when the state IS the observation, observe `crcl`.
- **Distinct from `egfr`.** Creatinine clearance and estimated glomerular filtration rate are not interchangeable and are registered separately for the same reason the covariate columns `CRCL` and `EGFR` are kept apart: `crcl` is a Cockcroft-Gault-style clearance in absolute mL/min scaled by body weight, whereas `egfr` is a body-surface-area-normalised estimate in mL/min/1.73 m^2 from a different equation family (MDRD / CKD-EPI). The founding example is explicit that its Cockcroft-Gault CrCL is a *surrogate* for GFR rather than a GFR estimate. Choose the name the source paper's own equation computes; do not translate one into the other.
- **Source aliases:** `CRCL`, `CrCl`, `ClCr`, `CLCR`, `CRCLPRED` -- translate to `crcl`. The observed-value symbol of a source paper that separates it from the predicted trajectory (e.g. a `CrCL_obs` in a residual-error equation) -- translate to `crclObs`.
- **Example models:** `vanHasselt_2014_crcl_pregnancy.R` (founding example; additive Anderson-Holford hyperbolic rise of maternal creatinine clearance across gestational age, structural values fixed from an upstream meta-analysis with only the random effects estimated). The sibling `vanHasselt_2014_cefazolin_semiphysiological.R` embeds the same trajectory as an algebraic intermediate named `crcl`, which drives clearance through the ratio `crcl / crcl_ega0`.
- **Notes:** Lowercase, matching the lowercase-state / uppercase-covariate split already established twice in this family (`uacr` vs the `UACR` column, `egfr` vs the `EGFR` column) rather than the clinical abbreviation's conventional `CrCL` capitalisation. Ratified by the maintainers (PMC4397420, 2026-09-21) over a mixed-case `CrCL` appealing to the `QTc` / `aPTT` / `P1NP` precedent; the two immediate renal siblings were weighted more heavily, so that the state sits one namespace away from -- not one case-fold away from -- the `CRCL` covariate column. A model whose creatinine-clearance trajectory is a pure algebraic function of a covariate (as in the founding example, where it is a closed-form function of gestational age) still uses this name; the register does not require the state to be integrated.
...
### log10_viral_load (**canonical log10 viral-load PD output**)
- **Type:** compartment
- **Role:** Log10-transformed viral load, the observed output of a viral-kinetics model. The linear-scale state is `virus` (or a model-specific extracellular pool such as `vextra`); this is the log10 of it, usually with a small epsilon floor to keep the transform finite once the virus is driven toward clearance.
- **Source aliases:** none.
- **Example models:** `Canini_2015_danoprevir.R`, `Canini_2018_setrobuvir.R`, `Dodds_2021_covid19_viralkinetic.R`, `Wang_2018_daclatasvir_asunaprevir.R`, `PiresdeMello_2018_zika_FAV_HFIM.R`, `PiresdeMello_2018_zika_FAV_IFN_RBV.R`.
- **Notes:** Spelled out rather than the earlier `Vlog10`, which was used by four models without ever being registered and reads as a variable name rather than a quantity. Distinct from `viral_load`, which is the LINEAR-scale output, and from `d_viral_load`, which is a change from baseline. Do NOT name this `Cc` -- see the naming rule under `central`; two of these models did, and the drug concentration went unnamed.

### log_car (**canonical log CAR-T cell-count PD output**)
- **Type:** compartment
- **Role:** Log-transformed CAR-T cell count per microlitre of blood, the observed output of a CAR-T cellular-kinetics model. The linear state is the total CAR-T cell pool; this is the log of its per-uL concentration, with an epsilon floor so the transform stays finite before expansion and after contraction.
- **Source aliases:** none.
- **Example models:** `Minucci_2024_CART_qsp.R`.
- **Notes:** Named for the `log_cfu` family shape (`log_<entity>`) rather than for the transform base, since CAR-T papers report both ln and log10 and the base is documented per model. A single-example canonical: if a second CAR-T model registers a different convention, reconcile then rather than assuming this one generalises.
...
### d_updrs (**canonical change-from-baseline total UPDRS score**)
- **Type:** compartment
...
### bcvacfb (**canonical change-from-baseline best-corrected visual acuity**)
- **Type:** compartment
- **Role:** Change from baseline in best-corrected visual acuity PD output, used as the modelled endpoint in ophthalmology disease-progression and treatment-effect models that fit the change score directly rather than the absolute acuity. Companion to `bcva`, which holds the absolute acuity; use `bcvacfb` whenever the paper's own structural equation targets the change from baseline, whether or not the same model also carries an absolute-acuity state.
- **Source aliases:** `Effect`, the Zhang 2025 supplement's notation in `Effect = K x Time`; `deltaBCVA`, `d_bcva` -- translate to `bcvacfb`.
- **Example models:** `Zhang_2025_bietti_crystalline_dystrophy_mbma.R`.
- **Notes:** **`<state>cfb` is the library-wide change-from-baseline convention** -- maintainers' ruling of 2026-09-11 (PMC11997835). The register previously carried two competing forms: this `cfb` suffix (`das28cfb`, `druglikingvascfb`, `tmccfb`) and a `d_<instrument>` prefix (`d_updrs`, `d_hr`, `d_rr`, `d_viral_load`). The suffix governs; the surviving `d_`-prefixed names are scheduled for conversion to it in a separate follow-up, and a new change-score endpoint must be named `<state>cfb` rather than `d_<state>`. This entry was itself minted as `d_bcva` and renamed under that ruling. The distinction once drawn between "a change score that is the model's only output" and "a change-score variant of a state that also exists in absolute form" is NOT a naming criterion -- Zhang 2025 is algebraic with no absolute-acuity state of its own and still takes the suffix. The scale is LogMAR (higher = worse vision), so a positive `bcvacfb` is vision loss; note that the covariate register's `SCORE_BCVA` is defined on the ETDRS-letter scale instead (higher = better) and the two are not interchangeable without conversion.

### epm_openarm_time (**canonical elevated-plus-maze open-arm time PD output**)
- **Type:** compartment
- **Role:** Time spent on the OPEN arms of an elevated plus-maze (EPM) during a fixed test period, in seconds -- the standard rodent read-out of unconditioned anxiety and the modelled PD endpoint for anxiolytic drugs. The EPM is a raised plus-shaped apparatus with two walled (closed) and two unwalled (open) arms; an anxious animal avoids the open arms, and an anxiolytic increases the time spent on them, so the endpoint rises with drug effect. The measurement is an ABSOLUTE duration over a fixed trial, not a change score and not a percentage of the trial, so it takes no `cfb` suffix and is not interchangeable with a percentage-of-time variant (a future model reporting the percentage should register a sibling name rather than reuse this one). The trial length is a per-paper design constant and belongs in the model's `population$notes`, not in the name.
- **Source aliases:** `time spent on the open arms`, `time spent in open arms`, `OT`; Yang 2017 writes the modelled quantity simply as `E` in its equation 4.
- **Example models:** `Yang_2017_buagafuran_rat.R` (founding example; direct sigmoid Emax of open-arm time on buagafuran brain concentration in male Wistar rats, 5 min trial).
- **Notes:** Bounded below by 0 and above by the trial length, but founding models fit it on the natural scale with an additive residual rather than on a logit-bounded scale, because the observed values sit far from the ceiling (about 0.6-14 s of a 300 s trial in Yang 2017). Distinct from the **number of open-arm entries** and from the **total arm entries**, which the same EPM experiment records alongside the time and which index locomotor activity rather than anxiety; a model of either needs its own canonical name. Also distinct from closed-arm time, which is not simply the complement because time on the central platform is counted in neither.
...
- **Type:** compartment
- **Role:** Absolute R-R interval length -- the time elapsing between two consecutive R waves on an electrocardiogram -- in milliseconds, as a model PD output. Used as the observation variable in concentration-effect models whose endpoint is the R-R interval itself rather than its change from baseline: a drug that speeds the heart SHORTENS `rr`, so the sign convention is the reciprocal of a beats-per-minute endpoint (`hr = 60000 / rr` with `rr` in ms). May be an algebraic Emax / sigmoid Emax output anchored on a fitted drug-free baseline, or an indirect-response turnover state.
- **Source aliases:** `RR`, `R-R interval length`, `RR interval` -- translate to `rr`.
- **Example models:** `Tylutki_2018_amitriptyline_RR.R` (founding example; sigmoid Emax shortening of the R-R interval by total plasma amitriptyline, from a baseline of 995.3 ms to a plateau of 500.8 ms with EC50 0.4 umol/L and Hill 1.5, fitted to a pooled literature dataset dominated by acute-overdose case reports).
- **Notes:** Lowercase bare name, following the absolute vital-sign PD-output family `hr` / `map` / `sbp` (`Dings_2026_cafedrine_theodrenaline_ephedrine.R`) rather than the mixed-case `QTc` / `QTcF` interval family. That family's own register entry documents its casing as a deliberate asymmetry that predates the lowercase convention, whereas `hr` is the direct domain sibling: `rr` is its reciprocal-scale partner exactly as `d_rr` is the partner of `d_hr`, and keeping `(hr, rr)` mutually consistent is the same argument the maintainers accepted when ruling on `(d_hr, d_rr)`. The lowercase form also keeps the predicted output distinct from any future uppercase `RR` covariate column carrying an observed baseline R-R, the shadowing problem `d_hr`'s entry describes for `HR`. **Distinct from `d_rr`**, the change-from-baseline RR interval: an absolute `rr` is on the order of 500-1000 ms whereas a `d_rr` is on the order of tens of ms, so a residual-error magnitude, baseline or plateau fitted against one must never be read against the other, and the two are not interconvertible without the individual's absolute baseline. Residual error follows the standard per-output rule (`addSd` for a single-output model, `addSd_rr` when multi-output). Distinct also from `QTcP`, the population-corrected QT interval, which consumes an R-R interval as a regressor rather than observing it.
...
### walk_dist (**canonical 6-minute walk distance**)
- **Type:** compartment
...
- **Type:** compartment
- **Role:** Change from baseline in forced vital capacity percent predicted, used as the modelled endpoint by models whose structural equation targets the change score directly rather than the absolute percent-predicted value. Companion to `fvcpp`, which holds the absolute value. In a model-based meta-analysis of placebo-controlled trials the modelled quantity is usually the PLACEBO-CORRECTED change from baseline -- the active-minus-placebo difference, the paper's "double delta" -- which is a further contrast on top of the change score; whether a given model's output is the plain or the placebo-corrected change must be stated in that model's `description` and cannot be read off the name.
- **Source aliases:** `dFVC%pred`, `change from baseline %predicted FVC`, `D%predicted FVC`, `DD %pred FVC` -- translate to `fvcppcfb`.
- **Example models:** `Chan_2017_ipf_fvc_mbma.R` (single `fvcppcfb` output; the PLACEBO-CORRECTED change from baseline for 15 IPF treatment regimens, as a per-treatment Emax times a shared `1 - exp(-kel * time)` onset times a power term in the arm's baseline `FVC_PCTPRED`. The absolute trajectory is not recoverable from this model because Chan 2017 estimated the placebo arm nonparametrically and did not tabulate it).
- **Notes:** Formed by applying the library-wide `<state>cfb` change-from-baseline convention (maintainers' ruling of 2026-09-11, PMC11997835; see `bcvacfb`) to the registered `fvcpp` state, so the name is determined by two existing rulings rather than newly minted. Dimensionless, **signed**, and typically NEGATIVE in untreated fibrosing-ILD cohorts because FVC declines -- so a treatment benefit is a less-negative value, and sign-sensitive validation assertions must not assume positivity. For a placebo-corrected output the sign flips meaning again: there a POSITIVE value is benefit, because the placebo decline has already been subtracted. Distinct from `fvcpp` (absolute) and `fvcz` (Z-score standardised); converting between the change score and the absolute state requires the arm's own baseline, which is carried as the covariate `FVC_PCTPRED`.
...
### migraine_days (**canonical monthly migraine-day count**)
- **Type:** compartment
...
- **Notes:** Companion covariates are `PKK` (time-varying prekallikrein exposure driver), `PKK_BL` (per-subject baseline PKK), and `HAERATE_BL` (per-subject baseline attack rate) documented in `inst/references/covariate-columns.md`. Sibling count-outcome PD-output canonicals include `ms_headache_days`, `migraine_days`, `cel_count`, `seizure_count`, and `score` (each paper-specific by disease domain).
...
- **Notes:** Member of the per-interval event-count PD-output family; siblings are `hae_attacks`, `ms_headache_days`, `migraine_days`, `cel_count`, and `score`. The underlying source likelihood in this family is frequently negative-binomial rather than Poisson (overdispersed counts); rxode2 exposes `llikNbinomMu()` but no negative-binomial *endpoint*, so these models declare `~ pois(...)` and expose the overdispersion as a model variable `ovdp` for downstream negative-binomial sampling via `rnbinom(mu, size = 1/ovdp)`. Distinct from `cumhaz` and the repeated-time-to-event Weibull hazards in `Lindauer_2017_lacosamide_seizure.R`, which model the *timing* of seizure events rather than a per-day count.

### thrombocytopenia_count (**canonical per-period thrombocytopenia-event-count PD output**)
- **Type:** compartment
- **Role:** Count of thrombocytopenia events accumulated over a prophylaxis period, used as the observation variable of a count-outcome exposure-response model. The state holds the mean count `lambda` at each observation row (an algebraic product of a baseline count and a drug effect -- no ODE); the observation model is `thrombocytopenia_count ~ pois(lambda)`. Thrombocytopenia is defined by the source paper as a platelet count decreased by at least 25% from baseline or an absolute platelet count below 150,000/uL.
- **Source aliases:** `thrombocytopenia counts` (Patel 2025 Table S5 row group heading).
- **Example models:** `Patel_2025_radamts13_count_thrombocytopenia.R` (founding example; sigmoid Emax reduction of the baseline count by average plasma ADAMTS13 activity in patients with congenital thrombotic thrombocytopenic purpura).
- **Notes:** Member of the per-interval event-count PD-output family; siblings are `seizure_count`, `hae_attacks`, `ms_headache_days`, `migraine_days`, `cel_count`, `ldh_elevation_count`, and `score`. Unlike `seizure_count`, which is a **daily** rate that a longer-interval model is expected to scale by `NDAYS`, the counts here are fitted directly per prophylaxis period (roughly 6 months) because that is the interval the source paper's Poisson model was estimated on; there is no daily rate to scale. Distinct from `cumhaz` in `Patel_2025_radamts13_rtte_thrombocytopenia.R`, which models the *timing* of the same clinical events via a repeated time-to-event hazard rather than a per-period count -- the two are separate analyses of the same endpoint in the same paper and are deliberately not unified.

### ldh_elevation_count (**canonical per-period elevated-LDH-event-count PD output**)
- **Type:** compartment
- **Role:** Count of elevated lactate dehydrogenase (LDH) events accumulated over a prophylaxis period, used as the observation variable of a count-outcome exposure-response model. The state holds the mean count `lambda`; the observation model is `ldh_elevation_count ~ pois(lambda)`. Elevated LDH is defined by the source paper as LDH raised to more than 1.5 times the baseline value or more than 1.5 times the upper limit of normal, and is used as a marker of microangiopathic hemolytic anemia (MAHA).
- **Source aliases:** `elevated LDH counts` (Patel 2025 Table S5 row group heading).
- **Example models:** `Patel_2025_radamts13_count_ldh.R` (founding example; log-linear reduction of the baseline count by average plasma ADAMTS13 activity in patients with congenital thrombotic thrombocytopenic purpura).
- **Notes:** Member of the per-interval event-count PD-output family; sibling of `thrombocytopenia_count`, with which it shares a cohort, an exposure driver and a fitting interval but not a drug-effect form (the source authors retained a sigmoid Emax for thrombocytopenia and a linear slope for LDH, so the two are separate fits and separate model files). Named for the *event* (an LDH elevation crossing a threshold) rather than for the analyte, so it is distinct from a hypothetical `ldh` canonical holding a continuous LDH concentration -- register that separately if a paper models the LDH time course itself. Distinct from `cumhaz` in `Patel_2025_radamts13_rtte_ldh.R`, which models the timing of the same events.

### viral_load (**canonical viral load**)
- **Type:** compartment
- **Role:** Viral load PD output (virology).
- **Source aliases:** none.
- **Example models:** `Koloskoff_2025_ganciclovir.R`.

### d_viral_load (**canonical change-from-baseline log viral-load PD output**)
- **Type:** compartment
- **Role:** Change from baseline in log-transformed plasma viral load, used as the observation variable of a direct (algebraic, non-ODE) exposure-response model. Negative values are antiviral suppression; add the subject's baseline log viral load to recover an absolute level. This is the change-from-baseline sibling of the absolute PD state `viral_load`, on the same pattern as `ddbp` / `dbp`: use `viral_load` when the model integrates a viral-dynamics ODE and observes the load itself, and `d_viral_load` when the model predicts the treatment-induced delta directly, which is what a short-course antiviral monotherapy exposure-response analysis reports.
...
### djak2v617f (**canonical change-from-baseline JAK2 V617F allele-burden PD output**)
- **Type:** compartment
- **Role:** Change from baseline in the JAK2 V617F variant allele burden of a patient with a Philadelphia-chromosome-negative myeloproliferative neoplasm, in PERCENTAGE POINTS on the linear scale, used as the observation variable of a direct (algebraic, non-ODE) exposure-response model. NEGATIVE values are the desired direction: they mean the neoplastic clone carrying the driver mutation has shrunk as a fraction of hematopoiesis. This is a molecular / disease-modification endpoint, not a hematologic one -- it measures depletion of the mutant clone itself rather than normalisation of the blood counts the clone drives.
- **Source aliases:** `JAK2V617F change from baseline`, `JAK2 V617F allele burden reduction from baseline`, `JAK2reduction from baseline` (the symbol used in Qin 2025 Equations 5 and 6).
- **Example models:** `Qin_2025_ropeginterferon_jak2_week24.R` (founding example; linear regression of the week-24 change on the weeks 0-24 average ropeginterferon alfa-2b concentration and baseline body weight, both uncentred), `Qin_2025_ropeginterferon_jak2_week52.R` (the week-52 sibling, exposure only).
- **Notes:** Not an ODE state -- the founding models have no `d/dt()` at all -- so a model using `djak2v617f` needs no `compartmentData` entry for it. Follows the `d<quantity>` change-from-baseline output shape founded by `ddbp` and extended by `d_viral_load` and `dmcp1`; a future model of the ABSOLUTE allele burden, or of a clonal-dynamics ODE, should register a sibling `jak2v617f` rather than overload this name. **The scale is percentage POINTS, not a fraction and not a per-cent change from baseline**: a patient going from a 60% to a 40% allele burden has `djak2v617f = -20`, not -0.20 and not -33. Because the endpoint is a change, a model predicting it needs the patient's baseline allele burden only if an absolute post-treatment burden is wanted; that baseline is a separate covariate column. Name carries the specific variant (V617F) because allele burden is variant-specific and the same patient may carry other JAK2 abnormalities, such as exon 12 mutations, that are assayed separately.

### hct (**canonical hematocrit PD output**)
- **Type:** compartment
- **Role:** Hematocrit, the packed red-cell VOLUME FRACTION of whole blood, carried as the observed state of an indirect-response or turnover pharmacodynamic model. Use it whenever a model integrates hematocrit itself rather than a red-cell count or a hemoglobin concentration -- for example an erythrocytosis model in which drug inhibits red-cell production, or an erythropoiesis-stimulating-agent model in which drug stimulates it.
- **Source aliases:** `HCT`, `Hct`, `haematocrit`, `PCV` (packed cell volume).
- **Example models:** `Qin_2025_ropeginterferon_hct.R` (founding example; ropeginterferon alfa-2b inhibits hematocrit production through an Imax function, with one `transit1` compartment interposed and a single rate constant `ktr` governing both the transit and the loss transfer, so the model has no separate `kout`).
- **Notes:** **Carry it as a FRACTION in 0-1, not as a per cent**, and say so in `units$concentration`; source papers use both conventions and mix them within a single table -- Qin 2025 Table 1 prints an A20-202 median of `45.7` against a range of `[0.421, 64.1]` in the same cell -- so the scale must be pinned per model rather than inferred. Distinct from three neighbours that are easy to confuse it with: `erythrocytes` (and its `erythrocytes<n>` age-transit chain) is a red-cell COUNT, not a volume fraction; `hb` / `thb` are hemoglobin CONCENTRATIONS; and `prob_anemia` is a threshold-crossing probability rather than any measured value. A model that needs both hematocrit and hemoglobin should carry both states rather than deriving one from the other, because their ratio (the mean corpuscular hemoglobin concentration) is not constant under treatment. Unlike `circ_plt` and `circ_wbc`, hematocrit has no `circ_<celltype>` form: it is not a circulating-cell count and does not belong to the Friberg myelosuppression family.
...
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences a severe (CTCAE grade >= 3) peripheral neuropathy adverse event, ascertained from investigator-reported adverse-event terms rather than from a nerve-conduction or laboratory measurement, in a static landmark exposure-safety logistic model. Peripheral neuropathy is the dose-limiting class toxicity of microtubule-inhibitor antibody-drug conjugates, so this is the characteristic exposure-safety endpoint of the auristatin (monomethyl auristatin E, MMAE) family.
- **Source aliases:** none.
...
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences a CTCAE grade >= 2 corneal epitheliopathy (keratopathy) adverse event, in a static landmark exposure-safety logistic model. Corneal epitheliopathy is the characteristic ocular toxicity of antibody-drug conjugates whose payload reaches the rapidly dividing corneal limbal epithelium, and is managed clinically by dose modification rather than by dose adjustment at baseline.
- **Source aliases:** none.
- **Example models:** `Babel_2026_telisotuzumab_corneal.R` (binomial logistic exposure-safety model for telisotuzumab vedotin; the steepest of that paper's conjugate-driven relationships, log-odds 4.21 per e-fold of conjugate average serum concentration, nominal p = 3.69e-8).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. The grade >= 2 rather than grade >= 3 threshold is not a stylistic choice in the founding model: Babel 2026 states that grade >= 3 events "were not evaluated in the exposure-safety analyses due to a very small number of events (N = 2)", so grade >= 2 is the finest threshold the data support. A future source that does fit a grade >= 3 model must register `prob_corneal_epitheliopathy_grade3` separately rather than reuse this name, for the same threshold-is-part-of-the-definition reason recorded under `prob_peripheral_neuropathy_grade3`.
...
### prob_chr (**canonical complete-hematologic-response probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient with a myeloproliferative neoplasm achieves a COMPLETE HEMATOLOGIC RESPONSE, a composite endpoint requiring several blood counts to fall below their thresholds simultaneously, in a static landmark exposure-response logistic model.
- **Source aliases:** `CHR`, `complete hematologic response`, `complete haematologic response`.
- **Example models:** `Qin_2025_ropeginterferon_chr_week24.R` (founding example; LINEAR logistic regression on the weeks 0-24 average ropeginterferon alfa-2b concentration, a significant relationship), `Qin_2025_ropeginterferon_chr_week52.R` (the week-52 sibling, fitted with an EMAX logistic form because the relationship had gone flat by that landmark).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. **The component thresholds are NOT part of the name and must be recorded per model**, because response criteria differ by disease and by protocol; in the founding paper complete hematologic response means hematocrit < 45% without phlebotomy in the previous 3 months AND white blood cell count < 10 x 10^9/L AND platelet count <= 400 x 10^9/L. Because the endpoint is a composite of three thresholds, it saturates earlier than any single count does, which is why the founding paper's week-52 model needed a saturable form; a user comparing landmarks should expect the exposure slope to flatten with time. Distinct from the continuous hematologic PD states a semi-mechanistic turnover model carries: this is a threshold-crossing probability with no time dimension, and the same paper's `Qin_2025_ropeginterferon_hct` / `_plt` / `_wbc` models describe the underlying counts themselves.

### prob_alt_increase (**canonical alanine-aminotransferase-increase probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences a treatment-related adverse event of increased ALANINE AMINOTRANSFERASE, as an investigator-reported adverse-event term, in a static landmark exposure-safety logistic model.
- **Source aliases:** `ALT increase`, `increased alanine aminotransferase`, `ALT elevation`.
- **Example models:** `Qin_2025_ropeginterferon_alt_increase.R` (founding example; binomial logistic exposure-safety regression on the dose-titration-phase average ropeginterferon alfa-2b concentration, odds ratio 1.123 per ng/mL).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Paired with `prob_ast_increase`; keep the two distinct rather than pooling them into a single transaminase canonical, because papers routinely fit them as separate endpoints with different intercepts and slopes -- in the founding paper the AST slope is steeper and the AST intercept lower, so the two fitted curves cross. **The CTCAE severity threshold is NOT part of the name** and must be recorded in the model's `description`; the founding model pools all grades, of which only one event reached grade 3. Distinct from the covariate `ALT`, which carries a measured baseline aminotransferase activity in U/L as a model input rather than a predicted event probability.

### prob_ast_increase (**canonical aspartate-aminotransferase-increase probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences a treatment-related adverse event of increased ASPARTATE AMINOTRANSFERASE, as an investigator-reported adverse-event term, in a static landmark exposure-safety logistic model.
- **Source aliases:** `AST increase`, `increased aspartate aminotransferase`, `AST elevation`.
- **Example models:** `Qin_2025_ropeginterferon_ast_increase.R` (founding example; binomial logistic exposure-safety regression on the dose-titration-phase average ropeginterferon alfa-2b concentration, odds ratio 1.154 per ng/mL).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. See `prob_alt_increase` for the shared conventions and for why the transaminase pair is kept as two canonicals rather than one. Distinct from the covariate `AST`, which carries a measured baseline aminotransferase activity in U/L as a model input.
...
### prob_hivrna_decr05log (**canonical greater-than-0.5-log10 HIV-1 RNA decline probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a person living with HIV-1 achieves a decrease of MORE THAN 0.5 log10 copies/mL in plasma HIV-1 RNA from a stated baseline to a stated landmark visit, in a static logistic exposure-efficacy model. A *decline-from-baseline* responder endpoint, in contrast to the *absolute-level* endpoint `prob_hivrna_lt50`; the two answer different questions and a heavily treatment-experienced population with a high baseline load can clear the decline bar while remaining far above any absolute-suppression threshold.
- **Source aliases:** none.
- **Example models:** `Parasrampuria_2025_temsavir_resp05log_day8.R` (binomial logistic exposure-efficacy model of the Day 1 to Day 8 responder rate during fostemsavir functional monotherapy in the phase 3 BRIGHTE randomised cohort; the linear predictor is an Emax function of the model-derived steady-state temsavir trough plus a log10 baseline-viral-load slope and a baseline-CD4+-below-20-cells/mm3 shift -- founding example).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape, with the threshold carried in the name as `prob_hivrna_lt50` prescribes for its own sibling series. In the token `decr05log`, `decr` marks the endpoint as a DECREASE from baseline rather than an absolute level, `05` denotes the 0.5 log10 magnitude, and `log` marks the log10 scale -- so `decr1log` is the 1.0 log10 sibling. Registering the decline family separately from `prob_hivrna_lt<n>` is the same distinction the register already makes between `viral_load` (absolute) and `d_viral_load` (change from baseline). The name carries no commitment to a particular landmark visit -- Day 8 in the founding model -- which belongs in the per-model `description` and `population$n_observations`. The founding model exposes the output with a small fixed placeholder residual so the nlmixr2 observation machinery accepts it; the source analysis uses an exact Bernoulli likelihood and estimates no residual error.

### prob_hivrna_decr1log (**canonical greater-than-1-log10 HIV-1 RNA decline probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a person living with HIV-1 achieves a decrease of MORE THAN 1.0 log10 copies/mL in plasma HIV-1 RNA from a stated baseline to a stated landmark visit, in a static logistic exposure-efficacy model. The stricter sibling of `prob_hivrna_decr05log`; a 1 log10 decline is the conventional bar for demonstrable antiviral activity in a short functional-monotherapy period.
- **Source aliases:** none.
- **Example models:** `Parasrampuria_2025_temsavir_resp1log_day8.R` (binomial logistic exposure-efficacy model of the Day 1 to Day 8 responder rate during fostemsavir functional monotherapy in the phase 3 BRIGHTE randomised cohort, structurally identical to its 0.5 log10 sibling but separately fitted; this is the endpoint the paper carries into its dosing-scenario simulations -- founding example).
- **Notes:** See `prob_hivrna_decr05log` for the shared naming and encoding conventions. Kept as a separate canonical rather than a parameterised threshold because the two thresholds are separately fitted models with different coefficients, on the same precedent as `prob_hyperglycemia` / `prob_hyperglycemia_grade2`. In the founding paper the 1 log10 model's baseline-CD4+ effect is roughly half the magnitude of the 0.5 log10 model's (-0.781 vs -1.27) and less precisely estimated, which is the kind of divergence a single shared canonical would hide.
...
### prob_almost_cured (**canonical near-complete-lesion-clearance probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a port-wine-stain lesion is "almost cured" -- expert-panel category I, i.e. at least 90 percent fading of the treated area -- in a static landmark exposure-response logistic model. Scored by a three-member blinded expert panel (one dermatologist, two plastic surgeons) from photographs of the target lesion taken before and 8 weeks after hemoporfin photodynamic therapy, graded by colour blanching from baseline into four categories: I (improvement >= 90 percent), II (>= 60 and < 90 percent), III (>= 20 and < 60 percent), IV (< 20 percent).
- **Source aliases:** none.
- **Example models:** `Chen_2025_hemoporfin_almost_cured.R` (founding example; Chen 2025 Table S1, beta0 = -29.7, beta1 = 1.98 per h*ug/mL, p = 0.104 -- NOT significant).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape, validated by `conv$probOutputRegex` rather than by enumeration; this entry is documentation of what the endpoint means, not a gate. Chen 2025 fitted each endpoint as a SEPARATE univariate frequentist logistic regression in R 3.4.2 (Methods 2.9, Equation 3), independently of the NONMEM population PK run, with the single regressor `AUC_HEMO` (registered in `covariate-columns.md`) -- hemoporfin AUC(0-30min) over the photodynamic-therapy treatment window, supplied as a per-subject covariate column derived from the population PK model's empirical Bayes estimates. The model carries no PK layer and no ODE state, so the probability is algebraic and is evaluated once per subject. Founding models expose a small placeholder additive residual so the nlmixr2 observation machinery accepts the model; the source likelihood is Bernoulli and estimates no residual error. The strictest of the three expert-panel endpoints and therefore the rarest event, which is why its intercept is far out at -29.7 with a 95 percent CI spanning -81.4 to -7.13: only a handful of the 24 pediatric patients reached category I, so the fit is near-separation and the coefficients are not usefully identified. Do not read the large positive slope as a strong exposure effect. Nested inside [[prob_significant_improvement]] and [[prob_any_improvement]]; the three are cumulative thresholds on one ordinal scale, not competing risks, so a downstream user must not treat them as mutually exclusive.

### prob_significant_improvement (**canonical significant-lesion-improvement probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a port-wine-stain lesion achieves "significant improvement" -- expert-panel category I or II combined, i.e. at least 60 percent fading of the treated area. Scored by a three-member blinded expert panel (one dermatologist, two plastic surgeons) from photographs of the target lesion taken before and 8 weeks after hemoporfin photodynamic therapy, graded by colour blanching from baseline into four categories: I (improvement >= 90 percent), II (>= 60 and < 90 percent), III (>= 20 and < 60 percent), IV (< 20 percent).
- **Source aliases:** none.
- **Example models:** `Chen_2025_hemoporfin_significant_improvement.R` (founding example; Chen 2025 Table S1, beta0 = -2.09, beta1 = 0.169 per h*ug/mL, p = 0.505 -- NOT significant).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape, validated by `conv$probOutputRegex` rather than by enumeration; this entry is documentation of what the endpoint means, not a gate. Chen 2025 fitted each endpoint as a SEPARATE univariate frequentist logistic regression in R 3.4.2 (Methods 2.9, Equation 3), independently of the NONMEM population PK run, with the single regressor `AUC_HEMO` (registered in `covariate-columns.md`) -- hemoporfin AUC(0-30min) over the photodynamic-therapy treatment window, supplied as a per-subject covariate column derived from the population PK model's empirical Bayes estimates. The model carries no PK layer and no ODE state, so the probability is algebraic and is evaluated once per subject. Founding models expose a small placeholder additive residual so the nlmixr2 observation machinery accepts the model; the source likelihood is Bernoulli and estimates no residual error. The middle cumulative threshold of the expert-panel scale: strictly between [[prob_almost_cured]] (category I) and [[prob_any_improvement]] (categories I+II+III). Its exposure slope is positive but flat and far from significant.

### prob_any_improvement (**canonical any-lesion-improvement probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a port-wine-stain lesion achieves any improvement -- expert-panel categories I, II or III combined, i.e. at least 20 percent fading of the treated area. Scored by a three-member blinded expert panel (one dermatologist, two plastic surgeons) from photographs of the target lesion taken before and 8 weeks after hemoporfin photodynamic therapy, graded by colour blanching from baseline into four categories: I (improvement >= 90 percent), II (>= 60 and < 90 percent), III (>= 20 and < 60 percent), IV (< 20 percent).
- **Source aliases:** none.
- **Example models:** `Chen_2025_hemoporfin_any_improvement.R` (founding example; Chen 2025 Table S1, beta0 = 1.56, beta1 = -0.0700 per h*ug/mL, p = 0.790 -- NOT significant).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape, validated by `conv$probOutputRegex` rather than by enumeration; this entry is documentation of what the endpoint means, not a gate. Chen 2025 fitted each endpoint as a SEPARATE univariate frequentist logistic regression in R 3.4.2 (Methods 2.9, Equation 3), independently of the NONMEM population PK run, with the single regressor `AUC_HEMO` (registered in `covariate-columns.md`) -- hemoporfin AUC(0-30min) over the photodynamic-therapy treatment window, supplied as a per-subject covariate column derived from the population PK model's empirical Bayes estimates. The model carries no PK layer and no ODE state, so the probability is algebraic and is evaluated once per subject. Founding models expose a small placeholder additive residual so the nlmixr2 observation machinery accepts the model; the source likelihood is Bernoulli and estimates no residual error. The most permissive cumulative threshold of the expert-panel scale, and the only one of the five Chen 2025 endpoints with a NEGATIVE exposure slope (-0.0700). That slope is not significant and should be read as noise around zero, not as a real inverse exposure-response; 66.7 percent of the pediatric cohort reached this endpoint, so there is little contrast left to regress on. This endpoint is the one the paper compares against the adult phase III label rate of 89.7 percent.

### prob_investigator_rating (**canonical high-investigator-efficacy-rating probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that the treating investigator rates the clinical outcome of hemoporfin photodynamic therapy as "excellent" or "good" (versus "moderate" or "unsatisfied") 8 weeks after therapy, in a static landmark exposure-response logistic model. A subjective clinician-reported global impression, deliberately distinct from the photograph-based expert-panel categories.
- **Source aliases:** none.
- **Example models:** `Chen_2025_hemoporfin_investigator_rating.R` (founding example; Chen 2025 Table S1, beta0 = -10.3, beta1 = 0.794 per h*ug/mL, p = 0.0223 -- SIGNIFICANT; odds ratio exp(0.794) = 2.21 per h*ug/mL).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape, validated by `conv$probOutputRegex` rather than by enumeration; this entry is documentation of what the endpoint means, not a gate. Chen 2025 fitted each endpoint as a SEPARATE univariate frequentist logistic regression in R 3.4.2 (Methods 2.9, Equation 3), independently of the NONMEM population PK run, with the single regressor `AUC_HEMO` (registered in `covariate-columns.md`) -- hemoporfin AUC(0-30min) over the photodynamic-therapy treatment window, supplied as a per-subject covariate column derived from the population PK model's empirical Bayes estimates. The model carries no PK layer and no ODE state, so the probability is algebraic and is evaluated once per subject. Founding models expose a small placeholder additive residual so the nlmixr2 observation machinery accepts the model; the source likelihood is Bernoulli and estimates no residual error. One of only two Chen 2025 endpoints with a significant exposure-response, and jointly with [[prob_patient_rating]] the basis of the paper's central claim that higher hemoporfin exposure yields better outcomes. Distinct from [[prob_orr_investigator]], which is an investigator-assessed ONCOLOGY overall-response endpoint with RECIST-style objective criteria; this one is a subjective four-level satisfaction rating dichotomised at its top two levels. Sibling of [[prob_patient_rating]]: same rating scale and same dichotomisation, different rater, fitted as two separate models -- keep them distinct, because the paper's argument rests on investigator and patient agreeing independently.

### prob_patient_rating (**canonical high-patient-efficacy-rating probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that the patient (or their legal representative) rates the clinical outcome of hemoporfin photodynamic therapy as "excellent" or "good" (versus "moderate" or "unsatisfied") 8 weeks after therapy, in a static landmark exposure-response logistic model. A patient-reported global impression of treatment benefit.
- **Source aliases:** none.
...
### aaa_size (**canonical abdominal aortic aneurysm size**)
- **Type:** compartment
...
- **Type:** compartment
- **Role:** Probability that a patient with axial spondyloarthritis meets the Assessment of SpondyloArthritis International Society 20% (ASAS20) response criterion at a single protocol-specified assessment visit. A per-subject binary efficacy outcome modelled by logistic regression, not a longitudinal responder trajectory.
- **Source aliases:** none.
- **Example models:** `Bhatnagar_2024_upadacitinib_asas20_as.R` and `Bhatnagar_2024_upadacitinib_asas20_nraxspa.R` (week-14 response in ankylosing spondylitis and in non-radiographic axial spondyloarthritis; `logit(p) = alpha + beta_trt * ON_TREATMENT`, fitted separately in each population).
- **Notes:** A probability output in `[0, 1]`, not a concentration or an amount. Follows the `prob_<endpoint>` output-naming shape founded by `prob_roc`. Sibling of `prob_asas40`, registered alongside it from the same source paper. Constant in `time`: the source models a single visit's binary outcome, so the output carries no time dimension even though the underlying clinical criterion is a change from baseline. Unlike most members of this family the founding models carry NO exposure term at all -- the source selected a pooled treatment-effect model over linear and nonlinear exposure models on AIC -- so a user simulating this output drives it with the treatment indicator rather than with a concentration or AUC. The founding models expose the output with a small placeholder additive residual so the nlmixr2 likelihood machinery accepts the forward-simulation model; the source analysis maximises a Bernoulli likelihood on the observed 0/1 indicator.

### prob_asas40 (**canonical ASAS40 response probability output**)
- **Type:** compartment
- **Role:** Probability that a patient with axial spondyloarthritis meets the Assessment of SpondyloArthritis International Society 40% (ASAS40) response criterion at a single protocol-specified assessment visit: at least 40% improvement and an absolute improvement of at least two units on a 0-10 numerical rating scale from baseline in at least three of the four ASAS domains, with no worsening in the remaining domain. A per-subject binary efficacy outcome modelled by logistic regression.
- **Source aliases:** none.
...
### prob_easi75 (**canonical EASI-75 response probability output**)
- **Type:** compartment
- **Role:** Probability that an atopic-dermatitis patient achieves EASI-75, i.e. at least a 75% reduction from that patient's own baseline in the Eczema Area and Severity Index, the primary efficacy criterion of essentially every moderate-to-severe AD trial. Unlike the `prob_asas20` / `prob_asas40` pair this output is genuinely LONGITUDINAL: the founding model describes how the responder rate rises from approximately zero at randomisation to a plateau over the double-blind period, with Weeks 12 and 16 the conventional read-out visits.
- **Source aliases:** `P_ij` -- `Serrano_2026_atopicDermatitis_placebo_mbma.R` (Serrano 2026 Methods 2.4: "P_ij is the probability of achieving EASI-75 response").
- **Example models:** `Serrano_2026_atopicDermatitis_placebo_mbma.R` (STUDY-ARM EASI-75 placebo responder probability in a model-based meta-analysis of 41 placebo arms; `logit(P) = Ebase + Em_i * (1 - exp(-k_i * t))`, with topical-corticosteroid permission and arm-mean baseline EASI acting on the plateau `Em_i`).
- **Notes:** A probability output in `[0, 1]`, not a concentration or an amount. Follows the `prob_<endpoint>` output-naming shape founded by `prob_roc`, with the 75% threshold carried in the name in the manner of `prob_seizure50` and `prob_hivrna_lt50`. **The threshold is load-bearing**: AD trials routinely also report EASI-50 and EASI-90, and a model fitted to one of those is a different endpoint that should register a sibling canonical (`prob_easi50`, `prob_easi90`) rather than overload this one -- Serrano 2026's own Discussion proposes exactly such a joint EASI-50 / 75 / 90 model as future work. **Distinct from the covariate `SCORE_EASI`**, which is the EASI score itself on its 0-72 scale; this output is a responder probability derived from a 75% relative reduction in that score, so a model carrying both is normal (the founding model uses arm-mean `SCORE_EASI` as a covariate on `prob_easi75`). The grain is per-model and must be documented: the founding model is an MBMA whose output is a STUDY-ARM proportion, whereas a subject-level exposure-response extraction would expose the same name as an individual response probability. The founding model exposes the output with a small placeholder additive residual so the nlmixr2 likelihood machinery accepts the forward-simulation model; the source's own residual is the binomial sampling variance of an arm proportion, `P*(1-P)/N`, which depends on arm size and so cannot be carried in `ini()`.
...
- **Source aliases:** none.
- **Example models:** `Knebel_2012_istradefylline_nausea.R` (unbounded power model in steady-state istradefylline exposure, additive on the logit scale; `logit(p) = BNS0 + SLOP * AUC^power`), `Sun_2025_maribavir_nausea.R` (frequentist binomial logistic exposure-safety model, LINEAR on the logit in the maribavir AUC on the day of the event, odds ratio 1.07 per 10 ug*h/mL; the observed event proportion rises from 8.3% to 55.0% across day-of-event AUC quartiles).
...
### prob_cmv_clearance_wk8 (**canonical confirmed-CMV-clearance-at-week-8 probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a transplant recipient achieves confirmed clearance of plasma cytomegalovirus DNA at week 8 of antiviral treatment, in a static landmark exposure-efficacy logistic model. Confirmed clearance means two consecutive undetectable plasma CMV DNA measurements; it is the registrational primary endpoint of the maribavir phase 3 programme.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_cmv_clearance_wk8.R` (binomial logistic exposure-efficacy model on steady-state maribavir AUC; the EXPOSURE term is not significant, p = 0.503, and the dominant predictor is instead a treatment-emergent resistance mutation, odds ratio 0.0475)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. An EFFICACY probability, so its exposure slope is expected to be positive or flat, unlike the safety members of the family. Paired with `prob_cmv_clearance_wk16`, the maintained-response endpoint, which is a STRICTER nested criterion: every week-16 responder is also a week-8 responder, so the two must not be treated as independent outcomes. Distinct from a modelled viral-load state in a semi-mechanistic CMV kinetics model: this is the probability of crossing a categorical response definition, not a DNA concentration.

### prob_cmv_clearance_wk16 (**canonical maintained-CMV-clearance-without-tissue-invasive-disease probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a transplant recipient achieves confirmed clearance of plasma cytomegalovirus DNA with no clinical findings of tissue-invasive CMV disease at week 8, AND maintains that state through week 16, in a static landmark exposure-efficacy logistic model. The registrational key secondary endpoint of the maribavir phase 3 programme.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_cmv_clearance_wk16.R` (binomial logistic exposure-efficacy model on steady-state maribavir AUC; the exposure term is not significant and its point estimate is NEGATIVE, p = 0.247)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. A STRICTER nested criterion than `prob_cmv_clearance_wk8`: it adds both the absence of tissue-invasive disease and an eight-week maintenance requirement, so it is a subset of the week-8 endpoint and the two are not independent outcomes. The name carries `wk16` because the maintenance horizon is the defining feature; a trial using a different maintenance window should register its own sibling rather than reuse this one.

### prob_dysgeusia (**canonical dysgeusia adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports dysgeusia (taste disturbance) as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model. Dysgeusia is the characteristic and dose-limiting tolerability signal of maribavir.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_dysgeusia.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.07 per 10 ug*h/mL; reference-patient probability 0.11 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. Sibling of `prob_nausea`, `prob_dizziness` and the rest of the single-preferred-term family. In the founding model the observed event proportion rises monotonically across day-of-event AUC quartiles from 3.3% to 50.0%, the steepest observed gradient of the fourteen endpoints in that paper.

### prob_vomiting (**canonical vomiting adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports vomiting as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
...
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_diarrhea.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.06 per 10 ug*h/mL; reference-patient probability 0.047 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. Completes the gastrointestinal-tolerability trio with `prob_nausea` and `prob_vomiting`. Distinct from a modelled gut-transit or stool-frequency state in a semi-mechanistic gastrointestinal model.

### prob_neutropenia (**canonical neutropenia adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports neutropenia as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_neutropenia.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.05 per 10 ug*h/mL; reference-patient probability 0.27 at zero exposure, the highest reference probability of that paper's fourteen safety endpoints)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. **Distinct from `prob_anc_decrease`**, which is the LABORATORY-derived CTCAE grade >= 3 neutropenia endpoint assessed from absolute-neutrophil-count values. The two describe the same toxicity through different ascertainment routes and sit at materially different event rates, so a model must use whichever matches its source's endpoint definition. Distinct also from the circulating-neutrophil ODE state of myelosuppression models such as the Friberg semi-mechanistic family: this is a threshold-crossing probability, not a cell count.

### prob_neutropenia_grade3 (**canonical severe-neutropenia adverse-event probability output**)
- **Type:** compartment
...
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences diarrhea of CTCAE grade 2 or worse, in a static landmark exposure-safety logistic model. The severity-qualified sibling of `prob_diarrhea`.
- **Source aliases:**
...
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports thrombocytopenia as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_thrombocytopenia.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.05 per 10 ug*h/mL; reference-patient probability 0.028 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. **Distinct from `prob_plt_decrease`**, the LABORATORY-derived CTCAE grade >= 3 endpoint assessed from platelet counts -- the same ascertainment distinction that separates `prob_neutropenia` from `prob_anc_decrease`. Distinct also from a modelled platelet-count state in a platelet-dynamics model.

### prob_anemia_ae (**canonical anemia adverse-event-term probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports anemia as a treatment-emergent adverse event, at ANY severity and ascertained from the investigator-reported adverse-event term, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_anemia.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.07 per 10 ug*h/mL; reference-patient probability 0.021 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. **The `_ae` suffix is load-bearing and exists because of a name that was already taken.** `prob_anemia` is registered for a LABORATORY-derived CTCAE grade >= 3 anemia endpoint assessed from hemoglobin values; this canonical is the ungraded investigator-reported adverse-event term, which sits at a different event rate. Conflating them would silently mix a severe-lab endpoint with an any-grade AE endpoint. Note the resulting asymmetry with `prob_neutropenia` / `prob_anc_decrease` and `prob_thrombocytopenia` / `prob_plt_decrease`, where the bare name went to the AE term and the qualified name to the laboratory endpoint: the ordering in this family reflects which endpoint was registered first, not a rule, so ALWAYS read the Role field rather than inferring the ascertainment route from the name.

### prob_pyrexia (**canonical pyrexia adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports pyrexia (fever) as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_pyrexia.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.05 per 10 ug*h/mL; reference-patient probability 0.037 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. In transplant and infectious-disease cohorts pyrexia is frequently a manifestation of the underlying infection rather than of drug toxicity, so an exposure-pyrexia association should not be read as a drug effect without a confounding analysis.

### prob_headache (**canonical headache adverse-event probability output**)
- **Type:** compartment
...
- **Type:** compartment
- **Role:** Probability (0..1) that a patient is a RESPONDER, defined by the source as achieving at least a 50% reduction in seizure frequency from baseline during the maintenance phase of an adjunctive antiepileptic-drug trial. A static landmark logistic regression of that dichotomised outcome on a scalar exposure metric plus baseline covariates.
- **Source aliases:** `PR` -- the abbreviation used in the Gidal 2018 figure legends for "probability of response".
- **Example models:** `Gidal_2018_eslicarbazepine_response.R` (founding example; adjunctive eslicarbazepine acetate in adults with focal-onset seizures. The exposure term is a POWER function of the average steady-state concentration centred at the population median, `0.735 * (CAV / 10205)^0.609`, so the curve is concave and most of the benefit is realised below the median concentration).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. The `_response` token is the epilepsy literature's standard "responder rate" endpoint and is deliberately NOT a synonym for `prob_orr_central` / `prob_orr_investigator`, which are oncology tumour-response endpoints adjudicated by imaging review: the underlying measurement (a patient-kept seizure diary dichotomised at a 50% reduction) and the therapeutic area are different. It is also distinct from the companion CONTINUOUS seizure-frequency outputs `lnssf` and `seizweek`, which model the count itself rather than the probability of crossing the 50% threshold; a paper may fit both, and Gidal 2018 does, reporting that the two agreed closely. Because the source likelihood is Bernoulli and estimates no residual error, the founding model exposes a small placeholder additive residual.

### dsod (**canonical change-from-baseline serum sodium PD output**)
- **Type:** compartment
- **Role:** Change from baseline in serum (or plasma) sodium concentration, in mmol/L, output by an exposure-response model fitted directly on the difference rather than on the absolute concentration. Negative values are a fall in sodium (the hyponatraemia direction).
...
- **Type:** compartment
- **Role:** Natural logarithm of the standardized seizure frequency plus a small offset, `ln(SSF + 0.33)`, where SSF is the number of focal-onset seizures standardized to a 28-day (4-week) period. This is the scale the source model is fitted and residual-tested on, so it is the observation variable; the natural-scale `ssf` is derived from it by inverting the transform.
- **Source aliases:** `ln(std seiz freq + 0.33)` -- the left-hand side of Gidal 2018 Appendix S1 Eq. E-7.
- **Example models:** `Gidal_2018_eslicarbazepine_ssf.R` (founding example; adjunctive eslicarbazepine acetate in adults with focal-onset seizures. Baseline plus an exclusive placebo-or-Emax switch, with an additive residual SD of 0.16 on this transformed scale and the paper's goodness-of-fit plots drawn as measured versus predicted ln(SSF)).
- **Notes:** The 0.33 offset is part of the transform the authors fitted, not a nuisance constant -- it keeps the logarithm finite for patients with zero seizures during maintenance -- so it must travel with the name. Registered on the LOG scale because that is where the residual error lives; registering only a natural-scale `ssf` would misplace the error model. Distinct from `seizweek`, which is a mean count per WEEK on the natural scale under a Poisson likelihood; the same paper fits both, on different time bases and with different EC50 estimates (3,530 versus 9,450 ng/mL), and they are not interchangeable. Also distinct from `prob_response`, which dichotomises the same underlying diary data at a 50% reduction.

### seizweek (**canonical mean weekly seizure count PD output**)
- **Type:** compartment
- **Role:** Mean number of seizures per week -- the rate parameter lambda of a Poisson (here zero-inflated Poisson) count model for seizure diary data collected in weekly epochs. Natural count scale, not log-transformed.
- **Source aliases:** `lambda_ij` -- Gidal 2018 Appendix S1 Eqs. E-11 and E-12.
- **Example models:** `Gidal_2018_eslicarbazepine_weekly_seizures.R` (founding example; adjunctive eslicarbazepine acetate in adults with focal-onset seizures. Carries a TIME COURSE, unlike the paper's other efficacy models: the count falls from baseline by up to 56%, of which a time-driven placebo component supplies 39% and an Emax term in the average steady-state concentration the remaining 61%. The model also derives `seizweek_marg = (1 - f_zeroinfl) * seizweek`, the marginal expectation under the zero-inflation mixture, because rxode2 has no zero-inflated Poisson likelihood).
- **Notes:** Names the Poisson RATE, so a model carrying zero inflation must say which of the rate and the marginal expectation it is reporting -- the founding model exposes both and flags the ambiguity in its vignette. Distinct from `lnssf`, which is the same clinical signal on a 28-day basis and a logarithmic scale under an additive residual; a paper may fit both and Gidal 2018 does. Because the source likelihood is Poisson and estimates no residual error, the founding model exposes a small placeholder additive residual so the nlmixr2 observation machinery accepts the model.

### prob_renal_disorder (**canonical renal-disorder adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports a renal disorder as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model. A grouped term covering investigator-reported renal impairment, acute kidney injury and related preferred terms rather than a single preferred term.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_renal_disorder.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.08 per 10 ug*h/mL; reference-patient probability 0.032 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. Distinct from the RENAL-FUNCTION COVARIATES (`CRCL`, `EGFR`, `RENALFUNC_REL`), which are inputs describing a patient's kidney function; this is an OUTPUT, the probability of a reported renal adverse event. A model needing a continuous renal trajectory should model creatinine or eGFR as a state, not use this canonical.

### prob_gvhd (**canonical acute-graft-versus-host-disease adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that an allogeneic haematopoietic-cell-transplant recipient reports acute graft-versus-host disease as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_gvhd.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.06 per 10 ug*h/mL; reference-patient probability 0.035 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. Meaningful only in allogeneic-transplant cohorts. Distinct from the `AGVHD_SKIN` / `AGVHD_LIVER` / `AGVHD_INTESTINE` COVARIATE family, which are organ-involvement inputs describing a patient's existing aGvHD status; this is an OUTPUT, the probability of a new treatment-emergent aGvHD event. A paper modelling organ-specific aGvHD probability should register organ-suffixed siblings rather than reuse this composite.

### prob_infection (**canonical invasive-fungal-or-bacterial-infection probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient reports an invasive fungal or bacterial infection as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_infection.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.07 per 10 ug*h/mL; reference-patient probability 0.027 at zero exposure)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an INVESTIGATOR-REPORTED adverse-event term with no severity threshold, so it sits at a higher event rate than a laboratory-defined or CTCAE-graded endpoint for the same toxicity and is subsumed by (not a competing risk against) a composite such as `prob_teae_grade3` or `prob_sae`. The name is deliberately broad because the founding source pools fungal and bacterial invasive infection into one endpoint. A paper separating them, or covering viral infection, should register narrower siblings (`prob_infection_fungal`, `prob_infection_bacterial`) rather than overloading this canonical. In immunosuppressed transplant cohorts an exposure-infection association is heavily confounded by the degree of immunosuppression, which is itself a determinant of antiviral exposure duration.

### prob_immunosuppressant_increase (**canonical raised-immunosuppressant-concentration adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient records an increase in the measured blood concentration of a concomitant immunosuppressant (typically a calcineurin inhibitor such as tacrolimus or ciclosporin) as a treatment-emergent adverse event, in a static landmark exposure-safety logistic model. The endpoint is a drug-drug-interaction signal: it flags the modelled drug raising a co-administered immunosuppressant's exposure.
- **Source aliases:** none.
- **Example models:** `Sun_2025_maribavir_immunosuppressant_increase.R` (frequentist binomial logistic exposure-safety model on the maribavir AUC on the DAY OF THE EVENT, odds ratio 1.05 per 10 ug*h/mL; reference-patient probability 0.0043 at zero exposure, the rarest of that paper's fourteen safety endpoints)
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. Registered on first sighting as a well-formed member of the explicitly extensible adverse-event-term family, not as a new modelling concept. The endpoint is an investigator-reported adverse-event term. Unlike the other members of the family this is a PHARMACOKINETIC interaction signal rather than a toxicity: it reports that a concomitant drug's concentration rose, not that the patient came to harm. Interpret alongside the relevant `CONMED_*` covariates (e.g. `CONMED_CSA`) recording which immunosuppressant was co-administered. A model quantifying the interaction itself should use a mechanistic two-drug PK model rather than this probability output.

### prob_sae (**canonical any-serious-adverse-event probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient experiences ANY treatment-emergent serious adverse event, pooled across all preferred terms, in a static landmark exposure-safety logistic model. Seriousness is the regulatory criterion (death, life-threatening, hospitalisation, disability, congenital anomaly or other medically important event), not a CTCAE severity grade.
- **Source aliases:** none.
...
- **Source aliases:** `clinical remission`, `CR`.
- **Example models:** `Moein_2025_etrolizumab_induction_clinrem.R` (founding example; landmark binomial logistic regression at end of the 14-week induction phase, exposure slope 0.0259 log-odds per ug/mL and NOT significant, P = 0.426 -- at induction prior anti-TNF status rather than exposure drives the outcome), `Moein_2025_etrolizumab_maintenance_clinrem.R` (the end-of-maintenance sibling, where the same endpoint DOES show a significant exposure slope of 0.147 log-odds per ug/mL, P = 0.00544).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. **The composite's component criteria are NOT part of the name and must be recorded per model**, because remission definitions differ by indication and by protocol: in the founding paper clinical remission means a Crohn's Disease Activity Index score < 150 AND a stool-frequency mean daily score <= 3 AND an abdominal-pain mean daily score <= 1, with no worsening in either subscore versus baseline, averaged over the 7 days before the visit. The symptom-based counterpart of the two endoscopic canonicals `prob_endoimp` and `prob_endorem`, which score mucosal damage directly; a paper reporting all three is reporting three genuinely different constructs and each belongs in its own model. The landmark is likewise not part of the name -- induction and maintenance fits of the same endpoint share this canonical and are distinguished by the model filename, which matters because the exposure relationship in the founding paper is absent at induction and present at maintenance. Distinct from the covariate `SCORE_CDAI`, which carries the measured activity index as a model input rather than the probability of crossing its remission threshold.

### prob_endoimp (**canonical endoscopic-improvement probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient with inflammatory bowel disease achieves ENDOSCOPIC IMPROVEMENT, defined relative to that patient's own baseline endoscopic score, in a static landmark exposure-response logistic model.
- **Source aliases:** `endoscopic improvement`, `endoscopic response`.
- **Example models:** `Moein_2025_etrolizumab_induction_endoimp.R` (founding example; landmark binomial logistic regression at end of induction, exposure slope 0.0682 log-odds per ug/mL, not significant at P = 0.061), `Moein_2025_etrolizumab_maintenance_endoimp.R` (the end-of-maintenance sibling, slope 0.234 log-odds per ug/mL, P = 0.00012).
- **Notes:** A probability output in `[0, 1]`. Follows the `prob_<endpoint>` shape. **Record the improvement criterion per model**: in the founding paper it is a >= 50% reduction from the patient's baseline Simple Endoscopic Score for Crohn's Disease. The distinction from `prob_endorem` is load-bearing and is why the two are separate canonicals rather than one: improvement is a RELATIVE, within-patient change from baseline, whereas remission is an ABSOLUTE threshold on the same instrument. A patient with severe baseline disease can therefore meet the improvement criterion without coming close to the remission criterion, the two endpoints have materially different event rates in the same trial, and they are fitted as separate models on the same analysis set. Distinct from the covariate `SCORE_SESCD`, which carries the measured baseline endoscopic score as a model input.

### prob_endorem (**canonical endoscopic-remission probability output**)
- **Type:** compartment
- **Role:** Probability (0..1) that a patient with inflammatory bowel disease achieves ENDOSCOPIC REMISSION, defined as an absolute threshold on an endoscopic severity instrument, in a static landmark exposure-response logistic model.
...
- **Type:** compartment
- **Role:** Non-absorbed drug held in the intestinal lumen, as an **amount** rather than a concentration. Drains by reabsorption at `kr` and, where the model resolves it, by faecal transit out of the body at `kF`. Two input routes are registered, both of which deposit drug that has already reached the systemic circulation back into the lumen: (a) **biliary / hepatic output** (classical enterohepatic circulation), and (b) **direct transporter-mediated exsorption from the central compartment across the gut wall**, the mechanism established for the fluoroquinolones by charcoal-block studies and invoked whenever the fraction of dose recovered in faeces is far larger than the fraction recovered in bile. Distinct from `gallbladder`, which models a storage organ with delayed, gated emptying producing a discrete secondary peak -- the gut lumen is a continuously-draining reservoir with no emptying delay. Also distinct from `gut` / `a_gut`, which are the perfused gut *tissue* with their own volume, blood flow, lymph flow, and partition coefficient.
- **Source aliases:**
...
- **Type:** compartment
- **Role:** Bare-case alias of the registered observation `aaa_size` (Sherer 2012 AAA).
- **Source aliases:** none.
- **Example models:** `Sherer_2012_AAA.R`.
...
### prob_moaas0 ... prob_moaas5, prob_moaas_le0 ... prob_moaas_le4 (**canonical MOAA/S sedation-score category and cumulative probability outputs**)
- **Type:** compartment
- **Role:** Probabilities (0..1) attached to the Modified Observer's Assessment of Alertness/Sedation (MOAA/S) scale, a six-level ordinal sedation score running 5 (fully alert, responds readily to name spoken in a normal tone) down to 0 (does not respond to a painful trapezius squeeze). `prob_moaas<k>` is the probability of scoring exactly `k`; `prob_moaas_le<k>` is the cumulative probability of scoring `k` or lower, i.e. being at least that deeply sedated. Output by an ordered-categorical (proportional-odds) PD model in which a drug term raises every cumulative logit, so that deeper sedation becomes more likely as exposure rises.
...
- **Notes:** Ratified 2026-09-03 alongside the Ravva 2015 varenicline craving extraction (PMC4832970; ratified by the maintainers). Spelled with the full `placebo` token rather than an abbreviated `pbo`, matching the existing `fpg_placebo` / `hba1c_placebo` / `skit_pla` register entries; no `_pbo` suffix exists anywhere in the registers. Composes the canonical `depot` role with the `placebo` qualifier the same way `depot_im` composes it with a dosing route.
...
### m6 (**canonical paper-named M6 metabolite suffix**)
- **Type:** metabolite-suffix
- **Role:** A source paper's "M6" metabolite, whatever chemical entity that designation names in the paper at hand. Used as the metabolite suffix on the `central_m6` compartment, the `lcl_m6` / `lvc_m6` parameters, the `Cc_m6` observation, the `fm_m6` fraction-metabolised and the `propSd_m6` / `addSd_m6` residuals in joint parent + metabolite popPK models. The entity currently carrying it is ivacaftor-M6, one of the two main metabolites of the CFTR potentiator ivacaftor (which is substantially metabolised by CYP3A4); Vonk 2025 fixes the fraction of ivacaftor clearance forming it at 43% and states it has "only 1/50th of ivacaftor's activity". As in the sibling `m1` entry, `cl_m6` and `vc_m6` there are APPARENT values, CL/(F*fm) and V/(F*fm), because no intravenous formulation exists and the metabolite volume was not estimable from the sparse paediatric data.
- **Source aliases:** `M-6` -- the hyphenated form used in some ivacaftor literature and regulatory documents.
- **Example models:** `Vonk_2025_ivacaftor.R` (doi:10.1002/bcp.70131; ivacaftor + M1 + M6 joint popPK in children with cystic fibrosis, a two-compartment parent driving two one-compartment metabolites).
- **Notes:** Sixth member of the generic numbered-metabolite suffix family alongside `m1`, `m2`, `m3`, `m4` and `m8`, following the same convention of lowercasing a source paper's `M<n>` designation. Because `M<n>` designations are per-programme and carry no cross-paper chemical meaning, this token is shared by unrelated metabolites of unrelated drugs and the specific entity is recorded per model, in the model file's `description` and `compartmentData`, not in the suffix. Distinct from `m6g` (morphine-6-glucuronide) -- the suffix matcher compares with `endsWith(name, "_m6")` vs `endsWith(name, "_m6g")` so there is no collision.
...
### m11 (**canonical paper-named M11 metabolite suffix**)
- **Type:** metabolite-suffix
- **Role:** A source paper's "M11" metabolite, whatever chemical entity that designation names in the paper at hand, following the same generic convention as `m1`, `m2`, `m3`, `m4`, `m5` and `m8`. Used as the metabolite suffix on the `central_m11` compartment, the `lcl_m11` / `lvc_m11` parameters, the covariate effects `e_wt_cl_m11` / `e_wt_vc_m11`, the `Cc_m11` observation and the `propSd_m11` / `addSd_m11` residuals. Drug contexts registered so far: in `Zhou_2025_fruquintinib_poppk.R`, M11 is the major circulating metabolite of the VEGFR-1/-2/-3 inhibitor fruquintinib, accounting for approximately 17% of total plasma radioactivity after a radiolabelled dose, with approximately 7.25% of the fruquintinib dose metabolized to it; it is ten-fold less potent than the parent against VEGFR-2 kinase and is therefore explicitly *not* considered an active metabolite.
- **Source aliases:** none.
- **Example models:** `Zhou_2025_fruquintinib_poppk.R` (doi:10.1002/jcph.70001; founding example -- joint one-compartment parent + one-compartment M11 popPK with the formation flux scaled by a fixed fraction metabolized).
- **Notes:** Seventh member of the generic numbered-metabolite suffix family, following the same convention of lowercasing a source paper's `M<n>` designation. Because `M<n>` designations are per-programme and carry no cross-paper chemical meaning, this token is expected to be shared by unrelated metabolites of unrelated drugs and the specific entity is recorded per model, in the model file's `description` and `compartmentData`, not in the suffix. Distinct from `m1` -- the suffix matcher compares with `endsWith(name, "_m11")` versus `endsWith(name, "_m1")`, and because `"central_m11"` also ends with `"_m11"` and not `"_m1"` there is no collision, but a model carrying BOTH an M1 and an M11 must be read with that longest-match care in mind. The same fruquintinib M11 entity is already named in this library's covariate register as `CP_FRUQUINTINIB_M11_NGML`, the time-varying PD driver of the companion concentration-QTc models `Zhou_2025_fruquintinib_QTcP_M11.R` and `Zhou_2025_fruquintinib_QTcF_M11.R` (doi:10.1002/jcph.70051), which are a different paper by the same first author; those PD models consume M11 concentration as a data column, whereas this suffix names the ODE state that generates it.
...
### norflox (**canonical norfloxacin metabolite / sibling-drug suffix**)
- **Type:** metabolite-suffix
- **Role:** Norfloxacin species suffix. Norfloxacin is both a marketed fluoroquinolone antibacterial in its own right and one of the two main metabolites of pefloxacin, formed by N-demethylation, so the suffix serves parent + metabolite extractions in which pefloxacin is the parent (`urine_norflox`, `Aurine_norflox`, `logitfm_norflox`, `addSd_Aurine_norflox`) and would equally serve a combination model in which norfloxacin is a sibling drug. Directly parallels the registered `cipro` suffix, which carries ciprofloxacin in exactly that dual parent-drug / metabolite role.
- **Source aliases:**
  - `NOR` / `FM_NOR` / `URINO` -- Bulitta 2019 Table 3 row labels and supplementary Figure S3 SADAPT-TRAN state and parameter names.
- **Example models:** `Bulitta_2019_pefloxacin.R` (founding example; doi:10.3390/pharmaceutics11070323, formation fraction of pefloxacin non-renal clearance estimated from cumulative urinary norfloxacin).
- **Notes:** Spelled `norflox` rather than `nor` on the `norfluox` (norfluoxetine) / `norcloz` (norclozapine) precedent, where the `nor` prefix is always carried together with enough of the parent stem to identify the compound. A bare `nor` would be ambiguous across the whole N-demethyl family. Note that in the founding example norfloxacin plasma concentrations were **not** measured, so no metabolite clearance or volume is identifiable and the suffix appears only on urinary-excretion states and the formation fraction -- there is no `central_norflox` or `Cc_norflox` in that model.

### noxpeflox (**canonical pefloxacin N-oxide metabolite suffix**)
- **Type:** metabolite-suffix
- **Role:** Pefloxacin N-oxide, the second of the two main metabolites of pefloxacin. Drives `urine_noxpeflox`, `Aurine_noxpeflox`, `logitfm_noxpeflox` and `addSd_Aurine_noxpeflox`.
- **Source aliases:**
  - `NOX` / `FM_NOX` / `URIDE` -- Bulitta 2019 Table 3 row labels and supplementary Figure S3 SADAPT-TRAN state and parameter names.
...
### tic (**canonical ticlopidine sibling-drug suffix**)
- **Type:** metabolite-suffix
- **Role:** Ticlopidine (thienopyridine antiplatelet agent, and a potent mechanism-based inactivator of CYP2B6) sibling-drug suffix, used when ticlopidine is the co-administered PERPETRATOR of a drug-drug interaction rather than the substrate of interest, so the bare canonical names stay with the victim drug. Drives `depot_tic` / `transit1_tic` .. `transit4_tic` / `gut_tic` / `portal_tic` / `liver_tic` / `central_tic` / `peripheral1_tic` compartments, the `lka_tic` / `lvc_tic` / `lq_tic` / `lvp_tic` / `lclint_liver_tic` / `clint_gut_tic` PK parameters, the `Cc_tic` concentration output and the `propSd_tic` residual on ticlopidine plasma concentration.
- **Source aliases:**
  - `TIC` -- Ashraf 2018 Table 1 and Table 2 subscript (`f u,TIC`, `CL INT,H,TIC`, `V C,TIC`).
  - `T` -- the single-letter prefix the deposited NONMEM control stream puts on every ticlopidine quantity (`TCLINTH`, `TV1`, `TKA`, `TDEPOT`, `TPV`, `TLIVER`). Not registered as a suffix in its own right because a bare `_t` is not self-describing.
  - `TICLO` -- the control stream's phase-indicator data item; see the `CONMED_TICLOPIDINE` covariate canonical, which is the registered name for that column.
- **Example models:** `Ashraf_2018_s_ketamine_ticlopidine.R` (ticlopidine as the CYP2B6 mechanism-based inhibitor raising S-ketamine exposure about twofold; its own two-compartment semi-PBPK disposition is solved as coupled ODE states because the portal-vein ticlopidine concentration is what drives the mechanistic-static interaction term; doi:10.1002/psp4.12346).
- **Notes:** Follows the sibling-drug suffix pattern established by `rtv` (ritonavir), `val` (valsartan) and `enz` (enzalutamide): the substrate whose PK the paper set out to characterise keeps the unsuffixed names, and the co-administered drug is suffixed. Carried as ODE states rather than as a time-varying concentration covariate for the same reason as the `prob` / `rif` members of the "Transporter-DDI perpetrator drug suffixes" family -- the perpetrator's own disposition has to be solved because the inhibition term reads its concentration at a site (the portal vein) that is not observed. Ticlopidine is also a CYP2B6 substrate, so it is in principle a victim of its own inhibition; Ashraf 2018 could not support an auto-inhibition model and describes ticlopidine metabolism as first-order, which is what the founding model encodes. Distinct from the `cloca` / `h4` clopidogrel metabolite suffixes: clopidogrel is a chemically related thienopyridine but a different drug.
...
- **Source aliases:**
  - `M1`, `kM1`, `keM1`, `METABO1` -- Bertin 2026 subscript notation in Table 2, Fig. 2 and the `$MODEL` block of the control stream, where the two metabolites are numbered rather than named in parameter subscripts.
- **Example models:** `Bertin_2026_levosimendan.R` (doi:10.1007/s40262-025-01591-4; founding example -- joint levosimendan + OR-1855 + OR-1896 popPK in critically ill adults, neonates and infants on VA-ECMO, with OR-1855 fed through a transit compartment and exchanging reversibly with OR-1896).
- **Notes:** The token lowercases the compound's development code and drops the hyphen, following the `m11` / `m1cis` convention for paper-designated metabolites and the `deaq` / `cloca` convention of compressing a chemical name into a short token. The leading `or` is the Orion Pharma code prefix, not an English word, and the digits are part of the compound's identity rather than a chain index -- so this token is NOT a member of the numbered `m<n>` family and must not be shortened to `m1`, which is a generic per-paper designation that carries no cross-paper chemical meaning. Pairs with [[or1896]]; a model that carries only one of the two still uses the specific token.

### or1896 (**canonical OR-1896 levosimendan metabolite suffix**)
- **Type:** metabolite-suffix
- **Role:** OR-1896, the pharmacologically active metabolite of levosimendan, formed by N-acetyltransferase-2 acetylation of OR-1855 and reversibly deacetylated back to it. It reproduces levosimendan's own haemodynamic effects -- calcium sensitisation of cardiac troponin C and ATP-dependent potassium-channel vasodilation -- and because it is formed slowly and eliminated with a 70-80 hour half-life, it is the species responsible for the sustained inotropic effect that persists for up to a week after a levosimendan infusion stops. Carries `central_or1896`, the `Cc_or1896` observation, the `kmet_or1896` formation rate constant, the `kel_or1896` elimination rate constant, the `e_child_kmet_or1896` covariate effect and the `expSd_or1896` residual.
- **Source aliases:**
