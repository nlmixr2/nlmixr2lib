...
- **Type:** compartment
- **Role:** Epithelial lining fluid of the lung airspace -- the apical aqueous layer sampled by bronchoalveolar lavage, and the site-of-action matrix for anti-infectives treating pneumonia. Two roles share the name because they are the same anatomical space approached from opposite directions. (1) INHALATION route: the airspace that inhaled or intratracheally instilled drug is deposited into before it distributes into the lung tissue and reaches the systemic circulation; the state carries its own physiological volume and its own unbound fraction `fu_elf` (typically derived from plasma `fu` and the plasma/ELF albumin ratio), and exchanges with the lung's `lung_cellular` and `lung_extracellular` pools. (2) SYSTEMIC-PENETRATION route: the airspace that a systemically administered drug penetrates INTO from plasma, encoded as a Sheiner-style effect compartment driven by the (usually unbound) plasma concentration through `kin_elf` with first-order loss `kout_elf`. In role (2) no ELF volume is normally reported, so the state holds a CONCENTRATION rather than an amount, and the steady-state exposure ratio `AUC(elf) / AUC(plasma)` is exactly `kin_elf / kout_elf`.
- **Source aliases:** `ELF` -- Parmar 2023 Figure 2, Table 2 (`V ELF`, `fu ELF`) and supplement equations S22-S26 for role (1); Bian 2024 Figure 2 and Zhang 2019 for role (2).
- **Example models:** `Parmar_2023_spectinamide_1599_mouse_pbpk.R` (role 1); `Bian_2024_lefamulin_original_ppb.R` (role 2; `d/dt(elf) <- kin_elf * Cu - kout_elf * elf`, observed as `Celf`).
- **Notes:** A physiological airspace compartment, not an absorption depot: in role (1) the inhaled or intratracheal dose lands in a separate `depot2` and reaches `elf` by first-order absorption with its own bioavailability, exactly as an oral dose reaches `central` from `depot`. Whole-lung concentration includes the ELF amount (Parmar 2023 supplement S26). Related but distinct from `isf` (generic interstitial fluid) and from the `brain_csf*` namespace; a future inhalation model that resolves regional airway ELF should register `elf_<region>` names rather than overload the bare `elf`. When a paper's ELF layer is a closed-form function of the plasma concentration rather than a compartment, there is no `elf` state at all -- use the algebraic `penratio_elf` / `pwr_penratio_elf` parameters from `parameter-names.md` instead (see `Bian_2024_lefamulin_higher_ppb.R`). The companion `specimen` string for `compartmentData` is `"epithelial lining fluid"`.
