...
- **Notes:** Ratified 2026-09-21 (task `oasweep_PMC5613202` sidecar request-001 q2, answer B). The `_cell` suffix is mandatory and is the whole point of the names: `kpc` and `kcp` are ALREADY canonical in this register for a drug's central-to-peripheral micro-rate constants, and the founding model carries a drug peripheral compartment in the same file, so the unsuffixed symbols would give one name two meanings a few lines apart. Naming the limbs after the biology instead (`lkmob` / `lkhome`, rejected option A) was declined in favour of preserving a one-to-one map onto the source table's row labels, which keeps the source trace checkable by eye; the direction is recorded in each `label()`. Reusing `lktr` for the forward limb (rejected option C) was declined because `ktr` everywhere else in this register is an UNSTIMULATED absorption or maturation transit rate, whereas here the forward limb carries the entire drug effect. Distinguish from the strictly one-way Friberg-style chains (`ktr` into `circ`), which have no return limb at all: the return limb is what gives this family its tolerance and rebound behaviour.
