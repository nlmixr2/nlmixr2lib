...
- **Type:** log-transformed-pk
- **Role:** Extracorporeal renal-replacement-therapy clearance arm contributed by an active hemodialysis (or hemofiltration / hemodiafiltration) session. Paired with the canonical body-CL parameter `lcl` (and the per-time-point `RRT_HEMODIAL_ACTIVE` covariate) to express total apparent clearance as a SINGLE assignment to `cl`: `cl <- exp(lcl + etalcl) * <covariate terms> + RRT_HEMODIAL_ACTIVE * cl_hemodialysis`. Do NOT route the gated sum through a separate `cl_total` variable -- that shape silently disables the dialysis arm in some models; see the encoding rule in the `RRT_HEMODIAL_ACTIVE` entry of `covariate-columns.md` and the mechanical gate `tests/testthat/test-modeldb-active-gate.R`. The `cl_hemodialysis` arm is added to the body baseline only while a session is running.
- **Source aliases:**
...
- **Example models:** `Veinstein_2013_gentamicin.R` (primary `ini()` parameter with IIV; the dialysis arm is estimated as a structural THETA gated by `HEMODIALYSIS`), `Dohmann_2025_piperacillin.R` (additive arm, no IIV), `Tsai_2023_ceftriaxone.R` and `Duke_2024_cefazolin.R` (REPLACEMENT rule -- see Notes).
- **Notes:** **Two composition rules exist, and the value means different things under each -- read the source's own conditional before transcribing.** Under the ADDITIVE rule (the majority, and the one the Role above describes) the estimate is the *increment* dialysis contributes on top of the body baseline. Under the REPLACEMENT rule the source writes a conditional such as the Pmetrics `&IF (HDx.EQ.1) CL=CLHD`, so the estimate is the *total* clearance while a session runs and the body-CL arm (with any covariate acting on it) is switched out entirely; encode it as a single assignment to `cl`: `cl <- (1 - RRT_HEMODIAL_ACTIVE) * <off-therapy arm> + RRT_HEMODIAL_ACTIVE * cl_hemodialysis` (as `Lee_2024_gentamicin_teigen.R` does), never as a separate `cl_total` variable. Reading a replacement-rule estimate as an additive arm double-counts the body clearance. Replacement-rule precedents, both from the same Alice Springs group and both Pmetrics NPAG fits: `Tsai_2023_ceftriaxone.R` (CLHD 8.76 vs CLnHD 0.83 L/h) and `Duke_2024_cefazolin.R` (CLHD 16.36 vs CLnHD 0.40 L/h). Distinct from `lcl_renal` (= residual renal CL, an intrinsic-body component) and `lcl_nonren` (= non-renal intrinsic-body CL). `Liesenfeld_2013_dabigatran.R` derives an equivalent dialysis-arm quantity from the Michaels equation (a function of blood flow rate, dialysate flow rate, and a hemodialyzer mass-transfer-area coefficient) as a derived `cl_dialysis` expression in `model()` rather than a primary `ini()` parameter, so its file does not include `lcl_hemodialysis`. Covariate-effect names on this arm follow the standard shape `e_<cov>_cl_hemodialysis`.

### lclmax_hemoadsorption (**canonical log-transformed maximum extracorporeal-hemoadsorption clearance arm**)
- **Type:** log-transformed-pk
- **Role:** Log-scale MAXIMUM clearance of an extracorporeal hemoadsorption (hemoperfusion) cartridge -- the adsorption clearance at zero cartridge load, before any saturation has occurred. Paired with the canonical body-CL parameter `lcl` and with the per-time-point `HEMOADSORB_ACTIVE` covariate to express total clearance, where the time-varying arm decays from its maximum as the sorbent fills: `cl_hemoadsorption <- HEMOADSORB_ACTIVE * clmax_hemoadsorption * (1 - adsorbed / amax_hemoadsorption)`. The `Leber_2023_*_sheep.R` models compose the total inside the derivative itself -- `d/dt(central) <- ka * depot - (cl + cl_hemoadsorption) * central / vc`, with the same arm feeding `d/dt(adsorbed)` -- and their gate is verified live by `tests/testthat/test-modeldb-active-gate.R`. Do NOT instead route the gated sum through a separate `cl_total` variable and eliminate via `cl_total / vc`: for some model shapes rxode2 solves the linear system analytically from variables named `cl` and `vc` and discards the explicit `d/dt()`, which silently disables the gated arm. See the encoding rule in the `RRT_HEMODIAL_ACTIVE` entry of `covariate-columns.md`. The `max` element is load-bearing and must not be dropped to a bare `lcl_hemoadsorption`: the `ini()` value is the ceiling of a saturating arm, not the arm's typical value, and naming it `lcl_hemoadsorption` would misreport it as a constant clearance.
- **Source aliases:**
...
- **Notes:** Canonical `lkel` adopted 2026-05-28 per the naming audit. Role broadened 2026-09-02 to cover rate-constant-parameterised disposition models that also estimate an explicit volume (maintainers' ruling, PMC13206287); the entry already carried the paper symbol `ke` as an alias, so registering a separate `lk10` would have duplicated it.
...
- **Notes:** Replaces the legacy `ltz` form. Semantically distinct from a lag-time even though both denote a time-shift parameter (maintainers' clarification, 2026-05-28).
...
- **Notes:** The maintainers' ruling was explicit: a double-Weibull release input is the existing Weibull machinery applied twice, so it takes the registered `ra` / `gam1` stems plus numbered second-process partners, rather than a parallel `trel` / `ssrel` family. A third or later process would continue the numbering (`lra3` / `lgam3`). Distinct from `lkamax`, which has no analogue in the release-input form: a release mixture has no asymptotic rate constant, only a fraction split. Prior art with paper-local names: `Bonner_2015_gastric_emptying.R` implements the same double-Weibull mixture as `lgamma1` / `lbeta1` / `lgamma2` / `lbeta2` / `llogit_pr`; it is not retrofitted, per the register's standing rule that existing files are not migrated.
...
- **Source aliases:**
  ...
  - `kb` -- used in `OrtizZacarias_2018_fosfomycin_oral.R` (Table 1, "kb, rate constant biliary elimination").
  - `Kbile`, `k_bi` -- generic spellings.
- **Example models:** `Zhang_2024_f53b_mouse_pbpk.R` (`KbileC` = 0.00001, allometrically scaled terminal biliary elimination from liver to faeces; founding example), `Yang_2025_matrine_pig_pbpk.R` (`kbi` = 0.05835 1/h, liver back into the intestinal lumen, which is what produces the observed two-phase luminal decay), `OrtizZacarias_2018_fosfomycin_oral.R` (`kb` = 0.50 1/h; the first COMPARTMENTAL rather than PBPK example -- see Notes).
- **Notes:** Deliberately **not** `kbm`, and `kbm` must not be widened to cover this case: `kbm` is registered as the biliary-*metabolite* excretion rate constant, moving a **metabolite** out of a **plasma / central** compartment, whereas `lkbile` moves **parent drug** out of **`liver`**. Broadening `kbm` would silently change the meaning of an existing entry that `Hamren_2008_tesaglitazar.R` depends on. The two can coexist in one model: a parent drug excreted in bile via `lkbile` and its glucuronide returned via `kbm` / `kicv`. The **`liver` source compartment is a property of the PBPK founding examples, not a constraint on the canonical**: a classical compartmental model that carries no explicit hepatic state drains parent drug to bile straight out of `central`, and that is still `lkbile`. `OrtizZacarias_2018_fosfomycin_oral.R` is the founding example of that case; the parent-versus-metabolite boundary against `kbm` is unaffected, because the analyte there is still parent drug. When the model has no `liver` state, say so in the parameter's `label()` so a reader is not left looking for one.

### lkreab (**canonical log-transformed intestinal reabsorption rate constant**)
- **Type:** log-transformed-pk
- **Role:** First-order rate constant for the **return leg** of enterohepatic recycling: drug moving out of `gut_lumen` back into the perfused `gut` tissue after biliary delivery, and thence into the systemic circulation (1 / time). The bare counterpart inside `model` is `kreab`. It is the competing route out of `gut_lumen` against faecal transit (`lkfec`), so a model carrying enterohepatic recycling normally carries both. Where the source tabulates a body-weight-scaling coefficient rather than a rate, `ini()` holds the coefficient and `model()` applies the exponent (Law 2017: `kreab = krac * BW^-0.3`).
- **Source aliases:**
  - `krac` / `k_ra` -- Law et al. (2017) Table 3 (rat) and Table 5 (human), Appendix eqs A6-A7; `krac` is the body-weight scaling coefficient of `k_ra`.
  - `kr` -- the spelling used in the `gut_lumen` entry of `compartment-names.md` and hard-coded as a `model()` constant in the `Ramachandran_2023_*_pbpk.R` family (`kr <- 0.17` in `Ramachandran_2023_rifampicin_pbpk.R` only; the ethambutol, isoniazid and pyrazinamide siblings carry `kr <- 0`, i.e. the recycling arm switched off).
- **Example models:** `Law_2017_egcg_rat_pbpk.R` (founding example; `krac` = 0.67 /min per kg^-0.3), `Law_2017_ecg_rat_pbpk.R` (0.41), `Law_2017_ec_rat_pbpk.R` (13.40), `Law_2017_egcg_human_pbpk.R`, `Law_2017_egc_human_pbpk.R`, `Law_2017_ec_human_pbpk.R` (0.18 /h per kg^-0.3).
- **Notes:** Registered by the maintainers' ruling 2026-09-21 (PMC5464336). The concept already existed in the library -- the `gut_lumen` compartment entry names it `kr` and the Ramachandran 2023 models hard-code it -- but had never been given a canonical, so it is named here rather than left to per-paper spelling. Distinct from `lka`, which is the **first-pass** absorption of the administered dose and is normally a different rate reached from `depot` rather than from `gut_lumen`; a model with enterohepatic recycling carries both, and collapsing them would erase the paper's distinction between the initial absorption site and the colonic reabsorption site. Distinct from `lkfec` (the competing exit) and from `lkbile` (the upstream hepatic step that fills the lumen). Pairs with `lmtt_bile` and the `bile_transit<n>` chain when the source imposes a bile-duct transport delay. A model whose source sets the reabsorption rate to exactly zero (three of the four `Ramachandran_2023_*` models do) should keep the `kr <- 0` constant in `model()` rather than migrate to `lkreab`, because `fixed(log(0))` is `-Inf` and not a usable `ini()` entry.
...
- **Notes:** Ratified 2026-09-02 with `lwdist` (same ruling). Not `lkel_exp_kdes`, which is the relaxation rate of a time-varying *elimination rate constant*; `kdist` decays a flux, not a rate constant. Not `lkde` / `lkel` either -- the ordinary first-order elimination of the same model is a separate parameter (`lkel`) and both appear together.
...
- **Notes:** Ratified by the maintainers 2026-09-11 with the Kata 2025 ganciclovir extraction. Substitute the maturing parameter's own stem when the model matures something other than clearance -- the structural sibling `Wu_2024_gfr_maturation.R` matures a glomerular filtration rate and so uses `lgfrbirth` for this role. Distinct from [[cl_time_max]], whose amplitude is a LOG-scale magnitude consumed inside `exp()` and whose driver is time on study rather than the subject's postnatal age.
...
- **Notes:** Ratified by the maintainers 2026-09-11 with the Kata 2025 ganciclovir extraction. Named `matspan` rather than `max` precisely because it is not the maximum the parameter reaches -- naming it `cl_time_max` or `cl_max` would invite a reader to use it as a total clearance, the failure the `cl_exp_component` naming rule warns against. Substitute the maturing parameter's own stem for other parameters; `Wu_2024_gfr_maturation.R` uses `lgfrmax` for the analogous (there allometrically-scaled) plateau.
...
- **Notes:** Ratified by the maintainers 2026-09-21 with the van Hasselt 2014 extraction. Named `matspan` for exactly the reason [[cl_matspan]] is: the paper's own symbol `CrCLMAX` invites a reader to use it as the maximum creatinine clearance, which it is not. Table 3 leaves the unit cell blank for this row; mL/min is forced by the dimensional coherence of eq 4, where it is added to a mL/min baseline.
...
### crcl_hinge (**canonical breakpoint of a piecewise-linear renal-function covariate relationship**)
- **Type:** bare-pk
- **Role:** The segmented-regression KNOT of a piecewise-linear ("reverse hockey stick") renal-function covariate relationship: the value of the covariate `CRCL` at which the rising linear arm meets the flat arm, in the units of the `CRCL` column it acts on (mL/min for a raw creatinine clearance, mL/min/1.73 m^2 for a BSA-normalised eGFR). Clamping the covariate axis at the knot expresses the whole relationship as one expression, `cl <- cl_nonren + e_crcl_cl_renal * min(CRCL, crcl_hinge)`, in which the two arms meet CONTINUOUSLY at `CRCL = crcl_hinge`. `crcl_hinge` is the bare name used inside `model()`; the `ini()` entry is the log form `lcrcl_hinge`, back-transformed as `crcl_hinge <- exp(lcrcl_hinge)`, following the `pna50` / `lpna50` pattern.
- **Source aliases:**
  - `CrCl0`, `eGFR0` -- Li 2017 Equation 7 and Table 5.
- **Example models:** `Li_2017_pomalidomide_crcl.R` (founding example; `CrCl0` = 37.7 mL/min, bootstrap CI 35.027-50.343), `Li_2017_pomalidomide_egfr.R` (`eGFR0` = 26.0 mL/min/1.73 m^2, bootstrap CI 25.01-30.899).
- **Notes:** `hinge` is the segmented-regression / spline term for the knot at which two linear arms meet, and CONTINUITY at that knot is exactly what distinguishes this shape from the register's other breakpoints. It is not spelled `crcl50` -- there is no half-maximal point anywhere in this form, and the `*50` names in this register (`pna50`, `cl_t50`, `ec50`, `dr50`) all denote one. **The plateau is not a separate parameter**: by continuity it is `cl_nonren + e_crcl_cl_renal * crcl_hinge`, and a source that tabulates an intercept, a slope and a breakpoint but no plateau (as Li 2017 Table 5 does) is reporting this continuous hinge. Generalises as `<cov>_hinge` for the same shape on another covariate axis; `Zingmark_2003_clomethiazole.R` already ships this exact role unregistered as `wt50` ("Body weight cap for the piecewise WT-CL relationship"), a candidate to migrate to `wt_hinge`. Distinct from [[tclchange]] and [[tkacut]], which are breakpoints on the TIME axis separating two mutually exclusive arms of a piecewise-CONSTANT relationship, where the two arms need not meet; `crcl_hinge` sits on a covariate axis and joins two arms that do. **Deliberately NOT spelled `crcl_cap`**, which is already in use in the modeldb for a different concept -- an externally-imposed ceiling applied to the covariate before an otherwise-ordinary effect, with no second arm and nothing fitted at the breakpoint (`Tong_2026_vancomycin_thomson.R` and `Tong_2026_vancomycin_goti.R` both carry `crcl_cap <- fixed(150)` style dataset clamps, and `Chen_2023_vancomycin.R` / `Sahota_2015_miridesap.R` use `crcl_capped` locals for the same). A `_hinge` is an ESTIMATED structural parameter that joins two arms; a `_cap` is a preprocessing clamp. Keeping the two spellings apart preserves that distinction.
...
- **Source aliases:**
  - `R0`, `Base`, `BASE`, `BL`, `S0`, `TS0`, `R_baseline` -- common source-paper notations; all translate to `lrbase` without a separate ruling.
...
- **Source aliases:**
  ...
  - `ktransit`, `K13` / `K34` -- Bertin 2026 Table 2 / Sect. 3.2 notation and the corresponding control-stream micro-constants, for a metabolite-formation delay chain rather than an absorption chain.
- **Example models:** `Dings_2026_cafedrine_theodrenaline_ephedrine.R` (`ktr_slow` = 0.107/min over three stages, mean transit time 28.0 min; `ktr_fast` = 1.76/min over four stages, mean transit time 2.27 min), `Bertin_2026_levosimendan.R` (`ktr` = 0.0127 1/h over a single `transit1` stage, delaying the appearance of the levosimendan metabolite OR-1855; the source sets the entry and exit rate constants equal, so one parameter governs both legs).
...
### ke12, ke21 (**canonical two-compartment biophase inter-compartmental rate constants**)
- **Type:** paper-named-param
- **Role:** First-order rate constants (1 / time) of a **two-compartment biophase (effect-site) distribution model**, in which the observed effect is driven by the concentration in a *central* effect compartment (`effect1`) that exchanges not only with the central PK compartment -- at [[ke0]], in both directions -- but also with a *peripheral* effect compartment (`effect2`). `ke12` carries drug from `effect1` to `effect2` and `ke21` carries it back:

  ```
  d effect1 / dt = ke0 * (Cc - effect1) - ke12 * effect1 + ke21 * effect2
  d effect2 / dt = ke12 * effect1 - ke21 * effect2
  ```

  Both states hold a *concentration*, not an amount, exactly as in the one-compartment form `d Ce / dt = ke0 * (Cc - Ce)`. The structure is the Bjornsson / Upton two-compartment biophase used to describe redistribution of an anesthetic within the central nervous system: it decouples the time to peak effect-site concentration from `ke0` alone, so a model can reproduce a fast onset together with a slow offset that a single `ke0` cannot. Inside `model()` the bare names are `ke12` / `ke21`; the log-transformed `lke12` / `lke21` forms carry the `ini()` typical values, since both are strictly positive.
- **Source aliases:**
  - `ke12`, `ke21`, `Ke12`, `Ke21` -- equivalent paper notation; this is the spelling the two-compartment-biophase literature itself uses.
- **Example models:** `BlussevanOudAlblas_2019_propofol_bis.R` (founding example -- `lke12 = log(0.121)`, `lke21 = log(0.172)` alongside `lke0 = log(0.102)` for propofol-induced changes in the Bispectral Index in adolescents; the two-compartment biophase was superior to the one-compartment form at p < 0.001 and reproduces the paper's reported 2.63 min time to peak effect-site concentration, against 3.30 min for the one-compartment biophase with the same `ke0`).
- **Notes:** Deliberately NOT named `k12` / `k21`. Those canonicals are bound to central-to-`peripheral1` exchange of the *pharmacokinetic* disposition model, and a two-compartment biophase model normally carries a two-compartment PK model as well -- so reusing them would put four distinct rate constants into two names. The `e` marker is the same one that distinguishes [[ke0]] from [[kel]], and the digit pair is read against the biophase chain (`effect1` = 1, `effect2` = 2), not against the PK compartment numbering. A one-compartment biophase needs neither name and uses the bare `effect` compartment with `ke0` only; reach for this pair only when the source paper actually estimates a peripheral effect compartment. Note that `ke0` retains its usual double duty in this structure -- it is both the rate constant *into* `effect1` from plasma and the rate constant for drug loss *out of* `effect1` -- so the biophase is still massless and still equilibrates to `Cc` at steady state.
...
- **Example models:** `Hamren_2008_tesaglitazar.R` (kbm = 11.7 1/h: rate at which the acyl glucuronide metabolite is biliary-secreted from the metabolite plasma compartment into the gut compartment, where it then undergoes interconversion back to parent tesaglitazar; founding example), `An_2025_rosuvastatin_ezetimibe.R` (`kbm_ezetimibe` = 0.013 1/h: total ezetimibe leaving the central compartment for the gallbladder, the outbound leg whose gated return is `kehc_ezetimibe`).
...
- **Example models:** `Keunecke_2020_regorafenib_phase1.R`, `Keunecke_2020_regorafenib_phase3.R` (`kehc` = 100 1/h, fixed, so that emptying is "complete and (almost) instantaneous" over the 0.5 h window; doi:10.1111/bcp.14334; founding example), `An_2025_rosuvastatin_ezetimibe.R` (`kehc_ezetimibe` = 1.33 1/h, fixed as the reciprocal of the 0.75 h bile-release duration; the gallbladder there discharges into the GI / absorption compartment rather than into central, so the released drug must be re-absorbed at `ka` before it re-appears in plasma).
...
- **Example models:** `Keunecke_2020_regorafenib_phase1.R`, `Keunecke_2020_regorafenib_phase3.R` (`tmeal1` / `tmeal2` / `tmeal3` = 8 / 12 / 18 h, all fixed; doi:10.1111/bcp.14334; founding example), `An_2025_rosuvastatin_ezetimibe.R` (`tmeal1` / `tmeal2` / `tmeal3` = 4 / 10 / 24 h, all fixed, read against `tad()` rather than absolute time).
- **Notes:** The **time origin is whichever one the source paper defines, and the model must say which**. Keunecke 2020 reports clock times, so its gate is built from a time of day and **any event table used with that model must take midnight as its origin**, placing doses at the intended clock time. An 2025 instead standardises its meals as 4, 10 and 24 h *after an administration*, so `An_2025_rosuvastatin_ezetimibe.R` reads the same three names against `tad()`; that form needs no midnight anchoring, and under once-daily dosing it reproduces the paper's literal `{4, 10, 24}` h indicator for a single-dose event table while recurring correctly after every dose in a multiple-dose one (`tad()` never reaches `tmeal3` = 24 h inside a 24 h interval, so the third window opens only after the final dose). Either way the requirement belongs in the model's `description` and the validation vignette. This is the distinguishing feature against `Kim_2018_tacrolimus.R`, whose `lmtime1` / `lmtime2` carry a single NONMEM-`MTIME`-style emptying window measured *post-dose* for a single-dose study; a multiple-dose model with three meals a day cannot be expressed that way, which is why a time-of-day family is registered separately rather than reusing that name. Do not use `mtime`, which would collide with rxode2's `mtime()` model-event-time function -- the same collision recorded under `tkacut`.
...
- **Example models:** `Keunecke_2020_regorafenib_phase1.R`, `Keunecke_2020_regorafenib_phase3.R` (`dge` = 0.5 h, fixed, matching the paper's assumption that "the duration of transfer of regorafenib from the gallbladder compartment to the central compartment was approximately 30 min"; doi:10.1111/bcp.14334; founding example), `An_2025_rosuvastatin_ezetimibe.R` (`dge` = 0.75 h, fixed, "which approximates the mean duration of GB emptying in healthy subjects"; the reciprocal of this value is also what fixes `kehc_ezetimibe`, so the two are not independent there).
...
### kicv (**canonical interconversion rate constant for reverse-metabolism**)
- **Type:** paper-named-param
- **Role:** First-order rate constant for interconversion of a metabolite back to the species it was formed from (1 / time). That upstream species is usually the parent drug -- the founding acyl-glucuronide case, where gut microbial / brush-border beta-glucuronidases hydrolyse a biliary-excreted conjugate back to the parent aglycone, which is then reabsorbed into systemic circulation, the single rate constant subsuming both the hydrolysis and the reabsorption step because oral PK data cannot separate them. It may equally be an upstream *metabolite*, when a paper models a reversible step between two metabolites neither of which is the parent. In the metabolite-to-metabolite case take the `<canonical>_<metabolite-suffix>` form and let the suffix name the species the rate constant **forms**, so that the forward and reverse constants of a reversible pair read unambiguously as `kmet_<downstream>` and `kicv_<upstream>`.
- **Source aliases:**
  ...
  - `kM2-M1`, `K54` -- Bertin 2026 Table 2 / Fig. 2 notation and the corresponding control-stream micro-constant, for the deacetylation of OR-1896 back to OR-1855.
- **Example models:** `Hamren_2008_tesaglitazar.R` (kicv = 0.79 1/h: rate at which acyl glucuronide accumulated in the gut compartment is hydrolysed and reabsorbed as parent tesaglitazar into the parent's central plasma compartment; founding example), `Bertin_2026_levosimendan.R` (`kicv_or1855` = 0.012 1/h fixed: deacetylation of the active levosimendan metabolite OR-1896 back to its inactive precursor OR-1855, the reverse of `kmet_or1896`; metabolite-to-metabolite case).
- **Notes:** The canonical name `kicv` was chosen specifically to disambiguate from the TMDD `kint` canonical -- the two mechanisms (drug-target complex internalisation vs. interconversion via gut hydrolysis) are semantically distinct, and reusing the same name with paper-specific meaning would create a latent reader-confusion failure mode. Paper-numbered alternatives (`k57`, `k72`-style as in deWinter 2009 MPA / MPAG) are discouraged for new extractions because the numbering is non-portable across papers; prefer the role-based `kbm` / `kicv` pair for any future interconversion popPK model. The Role was widened from "back to its parent drug" to "back to the species it was formed from" on 2026-09-11 with the Bertin 2026 extraction, whose reversible OR-1855 / OR-1896 acetylation is the first metabolite-to-metabolite instance; the mechanism is the same reverse-metabolism concept, so it extends this canonical rather than earning a new one.
...
### k_5fu_form, k_dfcr_form, k_dfur_form, k_gx_form, k_megx_form, k_precursor_5fu_form, k_xyl_form (**canonical formation rate constant, named for its destination**)
- **Type:** paper-named-param
- **Role:** First-order rate constant for formation of a downstream species, named for what it FORMS rather than for the pool it drains. The family shape is `k_<destination>_form`, where `<destination>` is the registered metabolite suffix (or metabolite-suffixed compartment stem) of the species on the receiving side of the flux; the log-transformed primary form is `lk_<destination>_form`. Reached for when a source parameterises a metabolic step as a rate constant on an AMOUNT rather than as a clearance -- typically because the donating pool has no identifiable volume -- and when a cascade carries two or more sequential or parallel formation steps that can only be told apart by their products. Stratum suffixes attach after the family shape (`lk_precursor_5fu_form_ctl` / `lk_precursor_5fu_form_sdt`).
- **Source aliases:**
  - `K23`, `K34` -- Urien 2005 micro-constant notation for the capecitabine cascade steps (`lk_dfur_form`, `lk_5fu_form`).
  - `K12`, `K14`, `K23` -- the lidocaine DDMoRe control stream's transfer notation (`k_megx_form`, `k_xyl_form`, `k_gx_form`).
  - `Ka,Met` -- Kim 2017 Table 2 and equations 1 and 4, the pre-systemic formation rate constant of the 5-FU precursor out of the gut compartment (`lk_precursor_5fu_form`).
  - `KConv` -- Kim 2017 Table 2 and equation 5, the conversion rate constant of the 5-FU precursor to 5-FU (`lk_5fu_form`, the same quantity Urien 2005 calls `K34`).
- **Example models:** `Urien_2005_capecitabine.R` (`k_dfcr_form` derived as `cl_dfcr / vc`, then `k_dfur_form` and `k_5fu_form` along the capecitabine -> 5'-DFCR -> 5'-DFUR -> 5-FU cascade), `NA_NA_lidocaine.R` (`k_megx_form` and `k_xyl_form` as the two parallel routes out of lidocaine central, `k_gx_form` as the onward MEGX -> GX step; the apparent parent elimination is their sum), `Kim_2017_tegafur_rat.R` (`k_precursor_5fu_form`, the pre-systemic first-pass route out of `depot` into `precursor_5fu`, estimated separately per treatment arm, and `k_5fu_form`, the shared conversion of that pool into `central_5fu`).
- **Notes:** The pattern predates this register (in use since the Urien 2005 and DDMoRe lidocaine extractions); formalised on 2026-09-23 alongside the Kim 2017 tegafur extraction, under the maintainers' ruling of 2026-09-21 (PMC6151713). A new member may be added to the heading without a fresh naming ruling so long as `<destination>` is the registered metabolite suffix or compartment stem of the species formed. Distinct from `lcl_form_<metab>`, which is the same concept expressed as a CLEARANCE and is the preferred spelling whenever the donating compartment has a volume, so the flux can be written `cl_form_<metab> * central / vc`. Distinct from `kmet`, which names the rate constant for the pool it drains out of (`central`) and so cannot separate two parallel formation routes. Distinct from `k_presystemic_central_<metab>`, whose source is the `presystemic` compartment holding already-absorbed drug and whose destination is a metabolite `central_<metab>`; `k_precursor_5fu_form` instead drains the dosing `depot` itself as one branch of a competing-route absorption step and lands in an amount-only `precursor_<metab>` pool. Naming the constant for its destination -- rather than by the source paper's `K12` / `K23` digits -- is what keeps the name stable when a state is inserted into the cascade, the same reasoning recorded under `k_presystemic_central`.
...
- **Source aliases:** none.
- **Example models:** `Danielak_2017_clopidogrel.R` (clopidogrel -> H4 active thiol, doi:10.1007/s00228-017-2334-z), `Hadigol_2026_talazoparib_enzalutamide.R` (enzalutamide -> N-desmethyl enzalutamide, `fm <- fixed(0.634)`; doi:10.1002/jcph.70125).
- **Notes:** Distinct from `kmet` (formation rate constant): `fm` is unitless and bounded; `kmet` has rate units. **In a parent-plus-metabolite model fit to plasma observations of both species, `fm` and the metabolite's central volume are not simultaneously identifiable** -- an increase in the volume can be compensated by a decrease in `fm` at unchanged total parent clearance, so only one of the pair may be estimated. A source paper resolves this in one of two ways, and which way it chose determines whether the metabolite's compartment holds a true or an `fm`-scaled amount: either it FIXES `fm` from external information and estimates the volume freely (Hadigol 2026 fixes `fm` to 0.634 from a published enzalutamide PBPK model, having found this fit better than the alternative of assuming equal central volumes for parent and metabolite, so `central_ndmenz` holds a true amount), or it estimates apparent metabolite parameters `CLm/fm` and `Vm/fm` and never names `fm` at all (`Petain_2008_imatinib.R`, `MenonAndersen_2009_imatinib.R`, `Tuo_2025_clobazam.R`, whose metabolite compartments hold `fm`-scaled amounts but still predict the true concentration, because the same `fm` divides clearance and volume). Record which resolution the source used, because the two are not interchangeable when a downstream user changes `fm`.


### fm_125d3, fm_1ohm, fm_25d3, fm_aca, fm_az5104, fm_cyp1a2, fm_cyp2b6, fm_cyp2c8, fm_cyp2c9, fm_cyp3a4, fm_cyp3a5, fm_cysmer, fm_gluc, fm_gx, fm_h4, fm_ko516_frac, fm_m1, fm_m2, fm_m27, fm_m3, fm_m3034, fm_m3g, fm_m5, fm_m6, fm_m6g, fm_megx, fm_morphine, fm_morphine_raw, fm_norflox, fm_noxpeflox, fm_other, fm_sn38, fm_sulf, fm_tfv, fm_ugt, fm_ugt1a1 (**canonical fraction metabolised by a named pathway**)
- **Type:** paper-named-param
...
- **Source aliases:**
  ...
  - `f m,CYP2B6` / `KFM2B6` -- Ashraf 2018 Table 1 / Figure 1 legend notation and the corresponding `$PK` symbol in its deposited NONMEM control stream, for the CYP2B6 fraction of S-ketamine hepatic metabolism.
...
- **Example models:** `Pei_2023_tacrolimus_pbpk.R` (`fm_cyp3a5` 0.55, `fm_cyp3a4` 0.35, `fm_other` 0.10, summing to 1 over the hepatic routes), `Franken_2015_morphine.R` (`fm_m3g` 0.55 and `fm_m6g` 0.10, with the residual route formed inline as `cl_other <- (1 - fm_m3g - fm_m6g) * cl`), `Pejcic_2024_clopidogrel.R` (`fm_h4` 0.12), `Mitra_2026_ziftomenib.R` (`fm_ko516_frac`; see the `_frac` note below), `Jaiswal_2025_dordaviprone.R` (`fm_cyp3a4` fixed at 0.8, the remaining 0.2 being CYP2D6), `Johnson_2025_osimertinib.R` (`fm_az5104` fixed at 0.25, the CYP3A4/5-mediated conversion of osimertinib to its active N-desmethyl metabolite AZ5104; the pathway token is the `az5104` metabolite suffix already registered in `compartment-names.md`. Johnson 2025 fixes rather than estimates the fraction because it is confounded with the metabolite's apparent volume and clearance in a joint parent/metabolite model, so simulated concentrations are insensitive to its value provided the same value is used at simulation as at estimation. The sibling model `Brown_2017_osimertinib.R` carries the same 0.25 as a bare `fmet <- 0.25` constant inside `model()` rather than as a named `ini()` parameter, so its fixed-vs-estimated provenance is not machine-readable), `Asaumi_2019_repaglinide_rifampicin_pbpk.R` / `Asaumi_2019_pioglitazone_rifampicin_pbpk.R` / `Asaumi_2019_glibenclamide_rifampicin_pbpk.R` (`fm_cyp2c8` + `fm_cyp3a4`, or `fm_cyp2c9` + `fm_cyp3a4`, summing to 1 over the hepatic metabolic routes that rifampicin induces; `fm_ugt` 0.759 is rifampicin's own UGT fraction, the route its autoinduction scales), `Padavia_2024_paracetamol.R` (`fm_gluc`, `fm_sulf` and `fm_cysmer` derived in `model()` from the paper's three odds-style `clrat_<metab>` ratios as `clrat_<metab> / (1 + clrat_gluc + clrat_sulf + clrat_cysmer)`, with the residual unchanged-renal route formed inline as `f_unchanged <- 1 / partition_denom`, so all four shares sum to exactly one), `Ashraf_2018_s_ketamine_ticlopidine.R` (`fm_cyp2b6` fixed at 0.63, the CYP2B6 share of S-ketamine hepatic intrinsic clearance; this is the *inhibitable* share in a mechanistic-static DDI model rather than a route that forms a named metabolite, so the complement `1 - fm_cyp2b6` appears inline as the CYP2B6-independent floor of the interaction term instead of as an `fm_other`. The value was estimated by log-likelihood profiling against the in-vitro 0.60 of Palacharla 2018, which the source reports as the corroborating prior).
- **Notes:** A new pathway may be added to the heading without a fresh naming ruling so long as `<pathway>` is lowercase and matches the enzyme or the canonical metabolite suffix it names. Three conventions the family fixes, each of which had already drifted:

  One member was added alongside the Sy 2018 etirinotecan pegol extraction: `fm_sn38`, the fraction of converted irinotecan that becomes SN-38 rather than APC (`Sy_2018_etirinotecanPegol.R`, carried on the log scale as `lfm_sn38` with IIV `etalfm_sn38`). The pathway token is the `sn38` metabolite suffix already registered in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. **This member establishes the "two-route split whose complement is written as a flux ratio" shape.** The two routes out of irinotecan are SN-38 (`fm_sn38` = 0.631) and APC (its complement), so the family's usual "sum to 1" expectation holds; what is unusual is how the complement reaches the ODE. Sy 2018 assume the two metabolites share one aggregate central volume (`V5 = V3`, hence `V5* = V3*`), so writing the APC arm as `(1 - fm_sn38) * kel_irinotecan * central_irinotecan` scaled by the SN-38 volume would double-count: instead the APC formation flux is expressed **relative to the SN-38 arm**, as `r_apc <- (1 - fm_sn38) / fm_sn38`. The parameter is still a fraction metabolised of total irinotecan conversion -- the ratio is a consequence of the shared volume, not a different quantity -- so it stays inside this family rather than moving to `clrat_<metab>`. A transcriber who reads the paper's printed Eq. 9 literally will drop the ratio entirely, at which point `F(irinotecan->SN38)` appears nowhere in the model and its Table 4 estimate of 0.631 +/- 0.017 is unidentifiable; that absence is the tell that the ratio belongs there.
...
Two classes of name collide on the `fm_` prefix but are **not** members of this family and must not be read as fractions metabolised. Both are outside the gate's scope by construction, because it inspects only names bound inside `ini` and `model`:
  ...
  Two members were added 2026-09-09 alongside the Vonk 2025 tezacaftor-ivacaftor extraction: `fm_m1` and `fm_m6`, the fractions of apparent ivacaftor clearance forming ivacaftor-M1 (0.22) and ivacaftor-M6 (0.43), both fixed rather than estimated. Both pathway tokens are registered metabolite suffixes in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. Note the boundary the same extraction illustrates: the paper's sibling tezacaftor model routes *all* parent clearance to a single metabolite, so it carries the bare `fm` (fixed at 1) rather than an `fm_m1` -- the `fm_<pathway>` family is for a split across two or more identified routes.

  One member was added 2026-09-12 alongside the Leding 2026 TBAJ-587 extraction: `fm_m3`, the relative fraction of TBAJ-587 metabolised to M3, carried on the log scale as `lfm_m3` beside the already-registered `fm_m2`. The pathway token is a registered metabolite suffix in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. **This extraction establishes the "anchored at 1" case, which is the family's third member-value shape after "summing to 1" and "fixed at 1 for a sole route".** Leding 2026 fixes *both* `fm_m2` and `fm_m3` at 1, so they sum to 2 rather than 1, because neither true fraction is identifiable without intravenous or direct-metabolite data; Table 1 footnote a records that the metabolite clearances and volumes are "additionally relative to unknown fractions metabolized", so each true fraction is absorbed into its metabolite's apparent CL and V and the `fm` is left as a pure normalisation anchor whose only estimated content is its dose and food covariate effects and (for M2) its IIV. The values summing to more than 1 does **not** move these names out of the family: the discriminator set by the `lclrat_<metab>` / `clrat_<metab>` entry below (spelled `f_clform_<metab>` until 2026-09-11) is whether the formation flux is *subtracted* from the parent, and Code S1's `DADT(2)` retains its full `-K20*A(2) = -CL/V2*A(2)` term while `K25 = CL/V2*FM3` and `K28 = CL/V2*FM2` feed the metabolites without a compensating loss. The parent's single elimination arm is therefore notionally split rather than extended, its total elimination is exactly `CL/V2`, and the denominator each `fm` divides remains total parent clearance -- which is what the `fm_` prefix claims. Contrast `Leegwater_2025_sulfamethoxazole.R`, where the formation arm genuinely sits alongside the elimination arm and so takes `clrat_nasmx` instead. When transcribing this shape, do not "repair" the mass balance by adding a loss term to the parent: that would silently rescale every apparent metabolite parameter in the source's table.

  Four members were added alongside the Ezuruike 2018 ethinylestradiol extraction: `fm_cyp1a2`, `fm_cyp2c8`, `fm_cyp2c9` and `fm_ugt1a1`, each an enzyme token and so addable without a naming ruling. `Ezuruike_2018_ethinylestradiol.R` carries the full six-way split the source's Figure 3b prints (CYP3A4 0.268, CYP2C9 0.125, CYP1A2 0.087, CYP2C8 0.013, UGT1A1 0.063, `fm_other` 0.445 for the unassigned "additional HLM" pathways). **The denominator there is hepatic metabolic clearance, not total clearance**, because the renal arm is carried separately as `lcl_renal`; the six shares consequently sum to exactly 1 over the metabolic routes only. That is the same denominator convention `Jaiswal_2025_dordaviprone.R` and `Pei_2023_tacrolimus_pbpk.R` already use, and it is the right one whenever a model resolves elimination into `lcl_renal` + `lcl_nonren` before splitting the non-renal arm by pathway -- rescaling the shares onto total clearance would make them inconsistent with the `m / hfac` well-stirred factor they multiply. State the denominator in each share's `label` (as that model does: "Share of hepatic metabolic clearance mediated by ...") so a reader cannot mistake one convention for the other.

  Two members were added 2026-09-02 alongside the Keunecke 2020 regorafenib extraction: `fm_m2` (the pre-systemically formed fraction of the dose appearing as regorafenib N-oxide) and `fm_m5` (the fraction of parent clearance forming regorafenib N-oxide N-desmethyl). Both pathway tokens are registered metabolite suffixes in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. Keunecke 2020 holds both on the **logit** scale, so `ini()` carries `logitfm_m2` / `logitfm_m5` and `model()` derives the bare `fm_m2` / `fm_m5` -- see the `logitfm` entry below for that metabolite-suffixed form.

Ten members were added 2026-08-23 during the round-3 consolidation, the first run of this gate over the whole modeldb: `fm_morphine` / `fm_morphine_raw` (Ashraf 2024 codeine), `fm_aca` (Demin 2025), `fm_megx` / `fm_gx` (He 2025 lidocaine), `fm_25d3` / `fm_125d3` (Tuey 2024 cholecalciferol), `fm_m3034` / `fm_m27` (Willmann 2024 elinzanetant) and `fm_1ohm` (Xie 2025 midazolam). Every pathway token was already a registered metabolite suffix in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. An eleventh name the gate caught, `fm_mass` in `Xie_2025_midazolam.R`, was NOT registered: it is `fm_1ohm` converted to a mass basis rather than a share of clearance, so it was renamed `f_mass_1ohm` and put outside the family.

  One member was added 2026-09-14 alongside the Beliveau 2026 tenofovir-alafenamide-implant extraction: `fm_tfv`, the apparent relative bioavailability of implant-derived tenofovir versus oral dosing, which multiplies the tenofovir-alafenamide-to-tenofovir formation flux in `Beliveau_2026_tenofovir_alafenamide_implant.R` and its Weibull-release sibling. The pathway token is the registered `tfv` metabolite suffix in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. **Its value is 9.24, the largest of any member, and that is the "anchored" shape established by Leding 2026 taken one step further.** Beliveau 2026 fixes every tenofovir disposition parameter to the oral literature model of Ji et al., whose `(V/F)TFV` is an oral-*apparent* volume already carrying the oral bioavailability; the single parameter the authors fitted therefore absorbs the whole implant-versus-oral difference in tenofovir availability (the Discussion measures it as a "ca. 10-fold" rise in plasma TFV exposure relative to the oral route) rather than being a true mass fraction. It passes the family's discriminator unchanged: the formation flux is *not* subtracted from the parent -- `d/dt(central)` retains its full `-kel * central` term while the metabolite receives `fm_tfv * kel * central` -- so the denominator remains total parent clearance and the name is `fm_tfv` rather than a `clrat_<metab>`. Do not "repair" the resulting mass imbalance; it is a property of the oral-apparent parameterisation the source inherited, and removing it would silently rescale every tenofovir and tenofovir-diphosphate prediction by a factor of nine. The one naming caveat worth carrying forward: the source's own name for this quantity is `Frel`, and a reader who greps for a bioavailability will not find it under `fm_`; the model file and its vignette both record the mapping explicitly.

Two members were added 2026-09-23 alongside the Bulitta 2019 pefloxacin extraction: `fm_norflox` and `fm_noxpeflox`, the fractions of pefloxacin **non-renal** clearance forming norfloxacin and pefloxacin N-oxide. Both pathway tokens are registered metabolite suffixes in `compartment-names.md`, which is the condition this heading sets for adding one without a naming ruling. Two features of this pair are worth carrying forward. First, the denominator is the **non-renal arm**, not total clearance: the source splits clearance into renal and non-renal components first (`lcl_renal` / `lcl_nonren`) and only then partitions the non-renal arm, so each `fm` divides `cl_nonren` and the two sum to well under 1 (0.352 in healthy volunteers, 0.445 in patients with CF), the balance leaving as unmeasured non-renal elimination. That is the `1 - fm_a - fm_b` residual shape this heading already allows, written implicitly rather than as an `fm_other`, because the source carries no state for the residual route. Second, the values are **group-specific**: each fraction has a healthy-volunteer and a CF typical value, held on the logit scale as `logitfm_norflox` / `logitfm_norflox_cf` and multiplexed by `DIS_CF` inside `model`, so the bare `fm_norflox` that this gate inspects is the per-subject derived value rather than a single estimate. Following the `logitfm` entry below, the `_ind` intermediate is kept on its own line. Enforced mechanically by `.checkFmFamily` in `R/checkModelConventions.R`, which raises an error-severity `fm_family` issue for any `fm_`-prefixed name bound in a model's `ini` or `model` block that is not registered in this heading, so `buildModelDb` fails rather than shipping a new spelling. Adding a pathway means adding it to the heading above -- that is the enumerating step.
...
### lclrat_<metab>, clrat_<metab> (**canonical ratio of a named elimination route's clearance to the parent's unchanged-elimination clearance**)
- **Type:** paper-named-param
- **THIS IS A RATIO, NOT A CLEARANCE.** Unitless and unbounded above, despite the `cl` root. The root is there because the quantity is built from two clearances; the `rat` token is what carries the units. Do not confuse it with the three `cl`-rooted names that ARE clearances in volume/time: `lcl_<metab>` (the parent-to-metabolite FORMATION clearance, e.g. `lcl_gluc` in `vanRongen_2016_acetaminophen.R`), `lcle_<metab>` (the metabolite's OWN elimination clearance, e.g. `lcle_gluc` in the same model), and `lcl_form_<metab>` (formation clearance under the older spelling). A model may legitimately carry `clrat_gluc` and `cl_gluc` side by side -- the first unitless, the second in L/h -- and `Padavia_2024_paracetamol.R` does exactly that.
- **Role:** The clearance of one named elimination route divided by the clearance of the parent's UNCHANGED (non-metabolic) elimination route, in a model where the routes are PARALLEL first-order losses from the parent compartment. Two encodings occur and both belong here:
  - **Elimination-arm parameterised** (the parent's estimated `cl` IS the unchanged arm): the ratio multiplies it, `kform <- clrat_<metab> * cl / vc` alongside `kel <- cl / vc`, so total parent elimination is `(1 + clrat_<metab>) * cl / vc`.
  - **Total-clearance parameterised** (the parent's estimated `cl` is the TOTAL): the ratios form a partition, `fm_<metab> <- clrat_<metab> / (1 + sum(clrat_*))`, and each route's clearance is `cl * fm_<metab>`. The unchanged route's own ratio against itself is the `1` in that denominator.
  The two are the same quantity re-arranged; `cl` means different things beside them, so read which encoding a model uses before reusing a value.
- Use the log form `lclrat_<metab>` in `ini()` when the ratio is estimated (positivity, and mu-referencing for `etalclrat_<metab>`), with the bare `clrat_<metab>` derived in `model()`. Use the bare form directly in `ini()` when the value is fixed. The `<metab>` token must be a registered metabolite suffix in `compartment-names.md`.
- **Source aliases:**
  - `0.4 x CL` -- the Leegwater 2025 Table 3 row "Conversion parent metabolite", and the `K23 = 0.4*CL/V2` line of that paper's supplementary ADVAN5 control stream.
  - `t1` / `t2` / `t3` -- Padavia 2024 Methods 2.4 notation for the three partition parameters. Bare `t1` is NOT usable as a canonical: it reads as a time and already names a local eigenvalue intermediate in `Knebel_2013_atorvastatin.R`.
- **Example models:** `Leegwater_2025_sulfamethoxazole.R` (founding example; `clrat_nasmx <- fixed(0.4)`, elimination-arm parameterised, doi:10.1002/cpt.3421), `Padavia_2024_paracetamol.R` (`lclrat_gluc` / `lclrat_sulf` / `lclrat_cysmer` = the paper's t1 / t2 / t3, total-clearance parameterised, estimated with IIV on two of the three and a gestational-age effect `e_ga_clrat_gluc` on the first, doi:10.1007/s40262-024-01439-3).
- **Notes:** **Deliberately outside the `fm_<pathway>` family, and the distinction is load-bearing.** A `fm_<pathway>` name is by definition a share of TOTAL clearance and its members are expected to sum to 1; this quantity is a ratio to one arm of the total, so the implied share of total clearance is `f/(1+f)` -- 0.4 becomes 0.286 in the founding example. Naming it `fm_nasmx` would overstate the metabolised fraction by 40%. This is the same distinction the register already drew when `Xie_2025_midazolam.R`'s mass-basis quantity was renamed out of the family to `f_mass_1ohm`, and it is the rename that `.checkFmFamily()`'s own diagnostic recommends ("If the parameter is not a fraction metabolised, rename it so it does not claim the `fm_` prefix"). Reach for `fm` / `fm_<pathway>` whenever the paper's parameter really is a share of total clearance (the parent's single elimination arm is then split, not extended); reach for this name only when the route sits ALONGSIDE a separately-parameterised unchanged-elimination arm. Beware that a paper may describe its own parameter in the looser language -- Leegwater 2025's Methods calls 0.4 "40% of the total sulfamethoxazole clearance" while its control stream implements the parallel-arm form; the control stream, not the prose, decides which name applies.

  A paper whose ratios must stay primary cannot be rewritten onto `fm_<pathway>` or onto the pathway clearances. In Padavia 2024 the etas sit on t1 and t2 individually and propagate non-linearly to all four fractions through the shared denominator, so an eta on a derived `fm_` or on a derived `cl_<metab>` cannot reproduce the published random-effects structure. That is why the ratio needs a canonical of its own rather than being folded into a clearance name.

  Spelled `f_clform_<metab>` from its registration on 2026-09-07 until 2026-09-11, when it was renamed here and in `Leegwater_2025_sulfamethoxazole.R`: `form` reads as *formulation* at least as readily as *formation*, and the `cl`-rooted spelling groups the name with the clearance vocabulary it is built from. `git log` is the record of the rename.

### lfprat_<metab>, fprat_<metab> (**canonical ratio of a first-pass formation route's flux to the flux of parent reaching the systemic circulation unchanged**)
- **Type:** paper-named-param
- **THIS IS A RATIO, NOT A FRACTION.** Unitless and unbounded above. A `fprat_` value of 0.953 does NOT mean 95.3% of the oral dose takes that route; the share it implies is `fprat_<metab> / (1 + sum(fprat_*))`, so 0.953 becomes 0.355 in the founding example. Naming such a quantity `fm_<pathway>` or `ffo_<metab>` would overstate the first-pass share by roughly a factor of the denominator, which is the same misreading the `lclrat_<metab>` entry above exists to prevent.
- **Role:** The pre-systemic sibling of `lclrat_<metab>`. It is the odds of one first-pass route against the route that delivers unchanged parent to the systemic circulation, for a model in which an oral dose is partitioned at absorption between bioavailable parent, one or more pre-systemically formed metabolites, and (optionally) a residual lost route. The partition is formed exactly as in the total-clearance encoding of `lclrat_<metab>`: the bioavailable share is `1 / (1 + sum(fprat_*))` and each route's share is `fprat_<metab> / (1 + sum(fprat_*))`, so all shares sum to exactly one by construction and the "sum of bioavailable and first-pass fractions is at most 1" constraint such papers describe is satisfied automatically rather than by a penalty.
- **Distinguished from `lclrat_<metab>` by the DENOMINATOR, and the distinction is load-bearing.** `lclrat_<metab>` is a ratio against the parent's unchanged-ELIMINATION clearance, for parallel first-order losses out of the parent's central compartment; its members describe systemic metabolism. `lfprat_<metab>` is a ratio against the bioavailable-parent route out of the oral DEPOT, before any systemic disposition, and its members describe extraction during absorption. A single model may legitimately carry both, plus genuine `fm_<pathway>` values for the systemic split -- the founding example carries `fm_m3g` / `fm_m6g` for the systemic fractions of morphine clearance alongside `lfprat_m3g` / `lfprat_m6g` for the first pass, and those are different quantities with different denominators that are NOT expected to agree. Reusing the `clrat_` root for a first-pass partition would put two different denominators under one name, so the roots are kept separate.
- Use the log form `lfprat_<metab>` in `ini()` when the ratio is estimated (positivity, and mu-referencing for `etalfprat_<metab>`), with the bare `fprat_<metab>` derived in `model()`. Use the bare form directly in `ini()` when the value is fixed. The `<metab>` token must be a registered metabolite suffix in `compartment-names.md`, with the one exception below.
- **The residual route is `lfprat_other` / `fprat_other`, singular.** A first-pass partition often carries a member that is neither the bioavailable parent nor any measured metabolite -- dose lost to unabsorbed, gut-wall, or otherwise unaccounted routes. That member takes `_other`, which is a route token rather than a metabolite suffix and is therefore exempt from the registered-suffix requirement above. This matches the singular `_other` already registered for `fm_<pathway>` (`fm_other`) and for the partition-coefficient families (`lkp_other`, `kp_other`). A plural `_others` is not a permitted alias.
- **Source aliases:**
  - `theta1` / `theta2` / `theta3` -- Oosten 2017 Table 3 footnote b notation for the three odds parameters (`theta2` is the M3G route, `theta1` the M6G route, `theta3` the residual). Bare `theta<n>` is not usable as a canonical: it is the opaque positional form the conventions forbid elsewhere, it is unsearchable across the registry, and it does not name the quantity's family.
  - `F 1p, met`, `F1p,M3G`, `F1p,M6G` -- the same paper's names for the DERIVED shares, not for these ratios. Those are the `f1p_<metab>` intermediates computed inside `model()`; do not transcribe a printed `F1p` value onto an `fprat_` name.
- **Example models:** `Oosten_2017_morphine.R` (founding example; `lfprat_m3g` = theta2 = 0.953, `lfprat_m6g` = theta1 = 0.170, `lfprat_other` = theta3 = 0.565, reproducing the paper's published `F_oral` 0.372, `F1p,M3G` 0.355 and `F1p,M6G` 0.0631 with a residual lost share of 0.210; estimated with IIV on two of the three, doi:10.1007/s40262-016-0471-7).
- **Notes:** **A paper whose ratios must stay primary cannot be rewritten onto the derived shares.** This is the same argument the `lclrat_<metab>` entry records for `Padavia_2024_paracetamol.R`: in the founding example the etas sit on theta1 and theta3 individually and propagate non-linearly to ALL FOUR shares through the shared denominator, so an eta placed on a derived `f1p_<metab>` or on a derived bioavailability cannot reproduce the published random-effects structure. That is why the ratio needs a canonical of its own rather than being folded into `lfdepot` or into an `fm_` name.

  Note also that the first-pass shares are NOT expected to equal the systemic fractions metabolised, and a reviewer should not treat a mismatch as a transcription error. The founding example's Discussion says so explicitly: the fraction of an oral dose converted to a metabolite during the first pass depends on whether metabolism also occurs in the intestinal wall, on whether total clearance equals hepatic clearance, and on the blood-to-plasma ratio of the parent -- none of which constrain it to the systemic `fm_`. Oosten 2017 estimates `F1p,M3G` = 0.355 against a systemic `fm_m3g` of 0.57, and reports that gap as a finding.
...
- **Notes:** Auto-approved 2026-09-01 as a well-formed member of the documented `lq_<destination>` family alongside the Saporta 2026 meropenem extraction (maintainers' ruling, PMC13041408, that compartment-derived names need no separate ruling). Pairs with `lv_lung`. Distinct from `lq_elf`, which is the CENTRAL-to-ELF clearance; a model resolving both, as Saporta 2026 does, uses the two names together.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2895455). Distinct from every `lcl_<arm>` name (`lcl_renal`, `lcl_nonren`, ...), which are additive arms of the CENTRAL clearance rather than a flow into a separate organ compartment; reading a perfusion flow as a clearance arm would delete the organ compartment entirely. Other perfused organs should take the same `lq_<organ>` shape.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2895455). Paired with `lq_kidney` in the founding example but independent of it -- a model may carry a kidney volume without an explicit perfusion flow, or vice versa.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2895455, in preference to widening the registered `lurprod` / `urprod`). Kept distinct from `urprod` deliberately: `urprod` is scoped to an ESTIMATED zero-order rate of urine production that integrates the canonical `urine_vol` volume state, whereas `uf` is a FIXED physiological flow that moves drug mass between tubular compartments and never integrates a volume. The two are the same physical dimension and could be numerically equal in a given paper, but they occupy different structural roles, and the register's standing warning against giving one name two meanings applies.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2895455). Note the encoding convention in the founding example: the source paper defines a quantity it calls "Reabsorption" that is `Vmax,R / (Km,R + Culf1)` -- a CLEARANCE -- and then multiplies it by `Culf1` in the differential equations, so the mass flux is the usual Michaelis-Menten form.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2895455). A transport `Km` for reabsorption is generally far away from a metabolic `km` for the same drug -- in the founding example the reabsorption `km_reab` of 0.46 mg/mL sits against a metabolic `km` of 0.054 mg/mL in the same model, an eightfold separation -- so never fold them into one name. The reabsorption value is the transporter-affinity one: Felmlee 2010 Discussion places it inside the in-vitro MCT-isoform range of 0.48-1.9 mg/mL and notes that SMCT1, which also carries GHB in the proximal tubule, has a higher affinity still (0.17 and 0.07 mg/mL), close enough that the two transport processes are not separable in vivo.
...
### pcelf (**canonical ELF:plasma partition coefficient**)
- **Type:** paper-named-param
- **Role:** Partition coefficient between the central compartment and the pulmonary epithelial-lining-fluid space, entering the central-to-ELF micro-rate constant as `k_central_elf = (q_elf / vc) * pcelf` against `k_elf_central = q_elf / v_elf`, so at pseudo-equilibrium the ELF:plasma concentration ratio (and hence the ELF:plasma AUC ratio) equals `pcelf` exactly. Unitless, normally in (0, 1]. Inside `model` the bare name is `pcelf`; the log-transformed `lpcelf` form is used in `ini`, since the coefficient is strictly positive. Second member of the `pc<matrix>` family founded by `pcmilk`, and the ELF destination token is the one already carried by the registered `elf` compartment and by `lq_elf` / `lv_elf` / `lcl_elf` / `lq_elf_lung`.
- **Source aliases:**
  - `PC` -- Paiboonvong 2025 Table 2 and Data S1 control streams (`TVPC = THETA(7)`).
- **Example models:** `Paiboonvong_2025_sitafloxacin.R` (founding example -- `lpcelf = log(0.772)` for sitafloxacin in critically ill patients with pneumonia, with 23.3 %CV inter-individual variability).
- **Notes:** Auto-approved as a well-formed destination-token sibling of `pcmilk`, following the same milk-to-ELF hop by which `lq_elf` was registered as the second member of the `lq_<destination>` family founded by `lq_milk`. **Check where the paper puts the unbound fraction before reading a published value as a total-plasma ratio.** In the founding example the authors code `K24 = QELF*fu*PC/VBD` with an unmodified `K42 = QELF/VELF`, so the estimated `pcelf` is the ELF-to-*unbound*-plasma ratio and the ELF-to-total-plasma ratio the model produces is `fu * pcelf` -- 0.486 rather than the 0.772 the prose quotes. Distinct from `ppc`, which is the gain of a Sheiner-style effect compartment (`d Ce / dt = ke0 * (ppc * Cc - Ce)`) with no mass transfer and no compartment volume; use `ppc` when the ELF limb is a driven effect compartment and `pcelf` when it is a real peripheral compartment with `q_elf` and `v_elf`. Distinct also from the physiologic `kp_lung`, which partitions into lung *tissue* rather than into the airspace lining fluid.

### lrelf, relf (**canonical ELF:total-plasma concentration ratio**)
- **Type:** paper-named-param
- **Role:** Ratio of the pulmonary epithelial-lining-fluid (ELF) concentration to the TOTAL plasma concentration in a *direct-response* (instantaneous-equilibrium) plasma-to-ELF link, `Celf <- Cc * relf`. Unitless, normally in (0, 1]. Inside `model()` the bare name is `relf`; the log-transformed `lrelf` form is used in `ini()` since the ratio is strictly positive. Drug-suffixed forms follow the usual pattern (`lrelf_sbt`). Applies only when the ELF limb is algebraic; when it is a real peripheral compartment carrying `q_elf` and `v_elf`, the partition coefficient `pcelf` is the right name.
- **Source aliases:**
  - `PLASMA-ELF ratio` -- Cammarata 2024 Table S6.
  - `R_ELF/plasma` -- Clewe 2015 Table 1.
  - `EPR`, `EPR (1 mg/l)` -- Dimelow 2018 Sect. 2.2.1.1, for the proportional and the power link function respectively.
- **Example models:** `Clewe_2015_rifampicin.R`, `Zhang_2023_amoxicillin.R` (`relf = fixed(0.30)`), `Zhang_2023_aztreonam.R`, `Larson_2026_sulbactam_durlobactam_pediatric_allometry.R` (`lrelf`, `lrelf_sbt`), `Dimelow_2018_avibactam.R` (paired with `lpow_elf`).
- **Notes:** Documented 2026-09-19 with the Dimelow 2018 extraction, for the same reason `lrbase` and `lemax` were documented -- an established convention already in consistent use across several shipped models that had never been written into the register. **The ratio is to TOTAL plasma unless the source says otherwise, and the resulting ELF concentration is then usually treated as entirely unbound**; reading a published penetration ratio as free-plasma-relative when it is total-plasma-relative was measured at 34 percentage points of error on the Zhang 2023 aztreonam case. When `relf` is paired with `lpow_elf` the link is a power function rather than a proportional one, and `relf` then denotes the penetration ratio *at the reference plasma concentration* (1 mg/L in Dimelow 2018) rather than a constant ratio -- the label must say so.

### lpow_elf, pow_elf (**canonical power exponent of a plasma-to-ELF link function**)
- **Type:** paper-named-param
- **Role:** Dimensionless exponent applied to the total plasma concentration in a power-function direct-response plasma-to-ELF link, `Celf <- relf * Cc^pow_elf`, where `relf` is the ELF:plasma penetration ratio at a plasma concentration of 1 unit. `pow_elf = 1` reduces the link to the proportional form; `pow_elf < 1` makes the penetration ratio *fall* as plasma concentration rises, which is the concentration-dependent penetration these analyses are written to characterise. Inside `model()` the bare name is `pow_elf`; `lpow_elf` is used in `ini()` because the exponent is strictly positive and its between-subject variability is reported as a log-normal %CV. The power link is one of the three standard plasma-to-tissue link functions (proportional / power / saturable) enumerated by Dimelow 2018 Sect. 2.2.1.1, so a `pow_<destination>` sibling is expected for other matrices.
- **Source aliases:**
  - `POW` -- Dimelow 2018 Sect. 2.2.1.1 and Table 2.
- **Example models:** `Dimelow_2018_avibactam.R` (founding example -- `lpow_elf = log(0.860)`, 23 %CV; the power link was 22 OFV units better than proportional and 6 better than saturable for avibactam).
- **Notes:** Registered 2026-09-19 with the Dimelow 2018 extraction. Deliberately **not** named `hill_elf` or `h_elf`: `hill` is the shape coefficient of a *sigmoid* Emax relationship and `h_<process>` is the fractal-kinetics heterogeneity exponent, whereas this is a plain power law with no half-saturation term and no saturating plateau. Also distinct from a covariate exponent such as an allometric `e_wt_cl` -- the exponent here applies to the model's own predicted concentration, not to a data column. Pairs one-to-one with `lrelf`; neither is interpretable without the other, because `relf` carries the units `(concentration)^(1 - pow_elf)` and is a bare penetration ratio only at the reference concentration.

### lemax_elf, emax_elf (**canonical maximum attainable ELF concentration of a saturable plasma-to-ELF link**)
- **Type:** paper-named-param
- **Role:** Plateau (maximum attainable) concentration in the pulmonary epithelial lining fluid under a saturable, Michaelis-Menten-shaped direct-response plasma-to-ELF link, `Celf <- emax_elf * Cc / (km_elf + Cc)`, in the model's concentration units. Inside `model()` the bare name is `emax_elf`; `lemax_elf` is used in `ini()`. Because the link passes through the origin (`Celf = 0` at `Cc = 0`) the plateau level and the increment above baseline coincide, which is why this is an `lemax_<output>` member and not an `lrmax_<output>` one. The limiting ELF:plasma penetration ratio as plasma concentration approaches zero is `emax_elf / km_elf`.
- **Source aliases:**
  - `E MAX`, `EMAX` -- Dimelow 2018 Table 2 and Sect. 3.2.
- **Example models:** `Dimelow_2018_ceftazidime.R` (founding example -- `lemax_elf = log(45.4)` mg/L, 24 %CV; the saturable link was 35 OFV units better than proportional and 8 better than power for ceftazidime).
- **Notes:** Registered 2026-09-19 with the Dimelow 2018 extraction as a per-output-suffixed member of the documented `lemax_<output>` family. The bare `lemax` is reserved for a PD *effect increment*; the `_elf` suffix is what marks this as a concentration in a named matrix rather than an effect on an endpoint. Pairs one-to-one with `lkm_elf`. Do not reuse for a saturable *elimination* ceiling -- that is `vmax_<process>`, a rate rather than a concentration.

### lkm_elf, km_elf (**canonical plasma concentration giving half the maximum ELF concentration**)
- **Type:** paper-named-param
- **Role:** Half-saturation constant of the saturable direct-response plasma-to-ELF link, `Celf <- emax_elf * Cc / (km_elf + Cc)`: the TOTAL plasma concentration at which the ELF concentration reaches half of `emax_elf`. Carries concentration units. Inside `model()` the bare name is `km_elf`; `lkm_elf` is used in `ini()`.
- **Source aliases:**
  - `K M`, `KM` -- Dimelow 2018 Table 2 and Sect. 3.2.
- **Example models:** `Dimelow_2018_ceftazidime.R` (founding example -- `lkm_elf = log(71.7)` mg/L, 97 %CV, which the paper notes is comparable to the typical plasma Cmax of a standard 2000 mg dose, so the saturation sits inside the clinical concentration range).
- **Notes:** Registered 2026-09-19 with the Dimelow 2018 extraction as a well-formed member of the documented `km_<process>` disambiguation family (`km_reab`, `km_rbc`, `km_pah`, `km_trans`), using the `elf` destination token already carried by the `elf` compartment and by `lq_elf` / `lv_elf` / `lcl_elf` / `lq_elf_lung` / `pcelf`. Distinct from a metabolic or transporter `km`: nothing is eliminated here, so there is no paired `vmax` -- the paired parameter is the concentration `lemax_elf`. Note that a very large reported %CV on this parameter (97% in the founding example) is characteristic: the half-saturation point of a link fitted over a bounded concentration range is weakly identified, and it is the main reason a cohort simulated from a diagonal OMEGA over-disperses the ELF tails relative to the published percentiles.
...
### lmtt_bile (**canonical log-transformed bile-duct sub-compartment residence time**)
- **Type:** log-transformed-pk
- **Role:** Residence time of **one** sub-compartment of a `bile_transit<n>` bile-duct delay chain, physically bile volume divided by bile flow rate (time). The bare counterpart inside `model` is `mtt_bile`, and the chain equations are `d/dt(bile_transit1) <- ram - bile_transit1 / mtt_bile` and `d/dt(bile_transit_j) <- (bile_transit_(j-1) - bile_transit_j) / mtt_bile`, delivering `bile_transit_n / mtt_bile` to `gut_lumen`.
- **Source aliases:**
  - `Rt` -- Law et al. (2017) Table 3 (rat) and Table 5 (human), Appendix eq A5.
- **Example models:** `Law_2017_egcg_rat_pbpk.R` (founding example; `Rt` = 3.00 min), `Law_2017_ecg_rat_pbpk.R` (0.30 min), `Law_2017_ec_rat_pbpk.R` (2.00 min), `Law_2017_egcg_human_pbpk.R`, `Law_2017_egc_human_pbpk.R`, `Law_2017_ec_human_pbpk.R` (0.03 h).
- **Notes:** Filed under the existing `lmtt_<context>` family by the maintainers' ruling 2026-09-21 (PMC5464336), in preference to a new `lrt_<context>` family. **Read the quantity carefully:** unlike the plain `lmtt` and unlike `lmtt_rbc` / `lmtt_infant`, which name a *total* mean transit time, `lmtt_bile` is the **per-sub-compartment** residence time -- the chain's total delay is `n * mtt_bile`, where `n` is the chain length. A model that reports only a total bile-duct delay should divide by its chain length before populating this parameter, and a model that lumps the delay into a single `bile_transit1` state has `n = 1` so the two readings coincide. The chain length and `mtt_bile` are not separately identifiable from a plasma profile, so both come from the source rather than from a fit. Pairs with `lkreab` (the reabsorption leg the delayed flux feeds) and the `bile_transit<n>` compartment family in `compartment-names.md`.
...
- **Source aliases:**
  - `k f,on` -- Lee 2018 Table 4 and Equation (15), the onset rate constant of the testosterone feedback pool (`kin_moderator1`).
- **Example models:** indirect-response PD models; `Lee_2018_leuprolide_iv_rat.R` and siblings (`kin_moderator1` / `kout_moderator1`, the moderator-pool form -- see the Notes).
- **Notes:** Codified 2026-05-28 per the naming audit. The compartment-suffixed forms `kin_<compartment>` / `kout_<compartment>` (with `lkin_<compartment>` / `lkout_<compartment>` as the log-transformed primaries) are the canonical family for **tissue-exchange rate constants** in semi-physiologic / dosimetry models that parameterise transport as first-order rate constants rather than as clearances: `kin_<tissue>` is central-to-tissue uptake and `kout_<tissue>` is tissue-to-central return, both 1/time. `<compartment>` must be a canonical compartment from `compartment-names.md`. Prefer these role-based names over the source paper's numeric micro-constants (`k12`, `k21`, `k13`, ...) whenever the compartments are anatomically named -- the canonical `k12` / `k21` / `k13` / `k31` entries mean central-to-`peripheral1` / `peripheral2` exchange specifically, so reusing them for organ compartments would silently mislead. Founding examples: `Lindauer_2017_pembrolizumab.R` and `Yamazaki_2008_crizotinib_mouse.R` (`kin_tumor` / `kout_tumor`); `Siebinga_2023_lu177psma617.R` (`kin_salivary_gland`, `kin_kidney`, `kin_liver`, `kin_tumor`, `kin_other` and their `kout_` partners, replacing Siebinga's `k12`/`k21`/`k13`/`k31`/`k14`/`k41`/`k15`/`k51`/`k16`/`k61`); `Nguyen_2026_linezolid.R` (`kin_saliva` / `kout_saliva`, replacing Nguyen's `Kabs` / `Kreabs`, i.e. `K23` / `K32`). The family name carries no commitment to mass balance: Nguyen 2026's saliva state is a DRIVEN (non-depleting) hypothetical effect compartment, so `kin_saliva * central` appears only in the saliva equation and `kout_saliva * saliva` appears in neither the central equation nor anywhere else -- the paired name still records which direction each rate constant describes, which is the point. A second, non-anatomical member of the same suffixed family is the **moderator-pool** form `kin_moderator<n>` / `kout_moderator<n>`, for the production and loss rate constants of a Gabrielsson-Hjorth `moderator<n>` tolerance pool. It is needed whenever one model carries both a biomarker turnover pool and a moderator that gates it, because the bare `kin` / `kout` can then belong to only one of them: `Lee_2018_leuprolide_iv_rat.R` and its three siblings pair `kin_tt` / `kout_tt` (testosterone) with `kin_moderator1` / `kout_moderator1` (Lee 2018's `kf,on` / `kf,off`, the onset and offset rate constants of the feedback pool of its Equation (15)). As with the saliva case, the name commits to nothing about the functional form the rate constant enters in -- Lee 2018's production term is `kin_moderator1 / TT` rather than a bare zero-order rate, which also means `kin_moderator1` carries the pool's units times concentration per time rather than 1/time. Read the model body, not just the name. Distinct from `ktol`, which is the single symmetric turnover rate of a moderator CHAIN whose stages share one constant (`Rognas_2025_bitopertin.R`, `Ahlstrom_2010_nicotinicAcid_rat.R`), and from `smod`, which scales a driver's effect on such a pool rather than naming one of its own legs.
...
- **Notes:** Ratified by the maintainers 2026-09-02 (PMC2825998). Deliberately NOT `kin`: the registered `kin` is the DRUG-FREE baseline production rate of a turnover pool, and reusing it for a quantity that is zero in the absence of drug would invert its meaning -- in the founding model the baseline production is exactly zero (unstimulated IFN-alpha was below the assay LOQ), so there is no `kin` to speak of. Deliberately NOT `emax` either: the maintainers' ratification records that `emax` / `lemax` has no entry in this register at all, and the `emax`-shaped names that do appear in model files are unitless fractional multipliers, so overloading that stem with rate units would mislead. The paper-named `smax_<suffix>` forms already in the registry (`Cao_2025_ferricCarboxymaltose_rat.R` `lsmax_iron`, `Garonzik_2016_daptomycin.R` `smax_s`, `Guiastrennec_2016_gastric_emptying.R` `smax_cckf`) are the multi-arm extension of this bare canonical: a model with a single stimulated pool uses `lsmax` / `smax`, and only a model that must carry two or more stimulated production maxima jointly takes the `_<suffix>` form. Where a paper's `Smax` carries `1/time` units because it scales a first-order rather than a zero-order term, it is a rate CONSTANT and belongs under a `kmax`-style name instead -- see `Mody_2023_doxorubicin_dexrazoxane_mdamb468.R`, which maps "Smax,DOX (1/h)" onto `lkmax_dox` for exactly this reason.
...
- **Notes:** Ratified by the maintainers 2026-09-22 with the Kim 2019 voriconazole extraction (PMC6406770); the same ruling migrated the pre-existing `Abduljalil_2009_clarithromycin.R` spelling `fclp` / `logitfclp` onto this canonical at the same time, so there is no legacy spelling left in the library. Distinct from four neighbours that are all *some* kind of fraction or plateau:
...
### f<analyte> (**canonical co-measured-analyte:parent mass ratio**)
- **Type:** paper-named-param
- **Role:** Unitless, time-invariant mass ratio converting a parent-drug concentration into the concentration of a **second analyte measured in the same matrix** that is not produced by kinetics the model resolves: `C<analyte> <- f<analyte> * Cc`. The log-transform prefix is `lf<analyte>`; IIV, when the source estimates it, is `etalf<analyte>`. Named for the analyte, and the analyte token must be a registered metabolite / sibling-analyte suffix from `R/conventions.R::registeredMetabolites` (so `fsn38`, not `fcontaminant`). Use this when a measured analyte has a component that the paper models as a fixed proportion of the parent rather than as a state with its own formation and elimination -- the classic case being a co-formulated or co-encapsulated contaminant that the bioanalytical assay cannot distinguish from the kinetically formed species.
- **Source aliases:**
  - `fSN38` / `Fraction of SN38 per unit of irinotecan` -- Adiwijaya 2017 Table S7 row label, reported in the mixed unit `ng (SN-38)/ug (irinotecan)`; divide by 1000 to obtain the unitless w/w ratio the parameter carries.
- **Example models:** `Adiwijaya_2017_irinotecan_liposomal.R` (founding example: `lfsn38 = log(9.0e-5)`, the encapsulated-SN-38 to total-irinotecan mass ratio in nanoliposomal irinotecan, with estimated IIV of 0.500 and a manufacturing-site covariate. The measured total SN-38 is the sum of this contaminant term and the kinetically formed unencapsulated SN-38, `Cc_sn38 <- Cu_sn38 + Ce_sn38`; the paper supports the split with in vitro measurement of the contaminant at 0.015% and with the absence of the glucuronide SN-38G in the first 12 h after dosing, which shows the encapsulated fraction is not available for metabolism).
- **Notes:** Distinct from `fm` and `fm_<pathway>`, which are fractions of a *clearance* and therefore describe where eliminated parent mass goes; `f<analyte>` describes what a single plasma sample *contains* and consumes no parent clearance. Distinct from `f<matrix>` (`fsaliva`, `fcortical`), which is the same algebra applied across two sampling **matrices** for one analyte, whereas this is two **analytes** in one matrix -- the two families are deliberately parallel in shape because both encode "a second measurement that is the first one rescaled, with no kinetics of its own". Distinct from `fu` (fraction unbound), which partitions one analyte's concentration into bound and free within the same molecular species. Because the ratio is time-invariant, the contaminant term is exactly parallel to the parent on a log scale; this is what lets it dominate the early profile of the summed observation -- in the founding example the total-SN-38 peak is almost entirely contaminant, and reproducing the paper's published total-SN-38 Cmax is a direct test of this parameter.
...
### lcl_int_g, lcl_int_g_cyp2c9, lcl_int_g_cyp3a4, lcl_int_g_other (**canonical log-transformed gut-wall unbound intrinsic clearance, whole or split by pathway**)
- **Type:** log-transformed-pk
- **Role:** Unbound intrinsic clearance of the gut wall (enterocytes), in volume/time units, feeding the well-stirred "Qgut" relation `egut = fu_gut * cl_int_g / (qgut + fu_gut * cl_int_g)` that turns it into a gut-wall extraction ratio. Use whenever an oral model needs intestinal first-pass loss to RESPOND to something -- a coadministered enzyme modulator, a genotype, a disease state -- because the register's `egut` entry requires that the response be applied to the intrinsic clearance and the ratio re-derived, never applied to the ratio directly (which would let availability leave [0, 1]). The bare `lcl_int_g` carries the whole gut-wall intrinsic clearance; the `lcl_int_g_<pathway>` members split it across named intestinal routes so that a perpetrator can scale only the route it acts on. The family shape follows `fm_<pathway>`: `<pathway>` is a lowercase token naming an intestinal enzyme, with `_other` reserved (singular) for the pooled unmodulated remainder.
- **Source aliases:**
  - `CLint,G`, `CL_int,G`, `CLu,int,G` -- the standard notation in the Yang 2007 "Qgut" literature and in Simcyp / PK-Sim compound tables.
  - `Additional CLint (HIC EL)` -- Ezuruike 2018 Table 4 notation for the intestinal-cytosolic (sulfation) arm assigned after the enzyme-specific arms are accounted for.
  - `TH(8)` -- van den Berg 2021 Table 1 position for the whole-gut value.
- **Example models:** `vandenBerg_2021_uprifosbuvir_pbpk.R` (bare `lcl_int_g` = log(15.5) L/h on an unbound-blood basis, with a coadministered-itraconazole multiplicative factor `e_itra_cl_int_g` = 0.444 applied to it and `egut` re-derived in `model()`), `Ezuruike_2018_ethinylestradiol.R` (three-way split: `lcl_int_g_cyp3a4` = log(2.518), `lcl_int_g_cyp2c9` = log(1.171) and `lcl_int_g_other` = log(5.535) L/h, summing to the 9.224 L/h that the printed `qgut` of 11.74 L/h and gut availability of 0.56 imply, so the baseline `egut` re-derives to exactly the published 0.44; a coadministered CYP3A4 modulator scales only the first arm, and the hypothetical pan-CYP inhibitor also scales the second).
- **Notes:** Registering the bare name is partly a backfill -- it was already shipping in `vandenBerg_2021_uprifosbuvir_pbpk.R` before it had an entry. Two boundaries matter. **(1) Not `clint`.** The registered `clint` is the systemic (hepatic) unbound intrinsic clearance; the gut-wall quantity is a different organ against a different flow (`qgut` rather than hepatic blood flow), so a model that resolves first-pass loss into its two anatomical sites carries both names, exactly as it carries both `eh` and `egut`. **(2) Not `fm_<pathway>`.** A pathway's share of gut-wall intrinsic clearance is NOT a share of total systemic clearance, and gut-wall extraction is a bioavailability term rather than an elimination term, so naming an intestinal route `fm_<enzyme>` would misstate the denominator in exactly the way the `fm_` entry's own `f_mass_1ohm` / `clrat_<metab>` rename precedents warn against. Carrying the split as explicit clearances in flow units avoids the question entirely and keeps every arm individually source-traceable. A new pathway member may be added to this heading without a naming ruling under the same condition the `fm_` family sets: `<pathway>` lowercase and naming the enzyme (or the canonical metabolite suffix) it describes.

### qgut (**canonical composite enterocytic blood flow / permeability term**)
- **Type:** paper-named-param
- **Role:** The "Qgut" term of Yang et al. 2007, a composite of villous blood flow and the drug's effective intestinal permeability, in volume/time units. It is the flow against which gut-wall intrinsic clearance competes in the well-stirred gut relation `egut = fu_gut * cl_int_g / (qgut + fu_gut * cl_int_g)`, and it is the gut-wall analogue of hepatic plasma flow (`fq`) in the well-stirred liver model. Usually fixed at a predicted or published value rather than estimated.
- **Source aliases:**
  - `Qgut`, `Q gut`, `Q_gut` -- the usual compound-table spellings.
- **Example models:** `Ezuruike_2018_ethinylestradiol.R` (`qgut` fixed at 11.74 L/h per Ezuruike 2018 Table 4, Simcyp-predicted from polar surface area and hydrogen-bond donor count), `vandenBerg_2021_uprifosbuvir_pbpk.R` (derived inside `model()` rather than carried in `ini()`, as the harmonic combination `qgut <- (qvi * cl_per) / (qvi + cl_per)` of villous blood flow and permeability clearance -- which is the definition this canonical names).
- **Notes:** Distinct from `fq` (hepatic plasma flow, the liver's analogue) and from any anatomical `q_<organ>` blood-flow parameter of a whole-body PBPK model: `qgut` is NOT a blood flow, it is a flow-and-permeability composite that is always less than or equal to villous blood flow. Pairs with `fu_gut` (the unbound fraction in the enterocyte, commonly assumed to be 1) and with `cl_int_g` / `egut`. A model that prints only the gut availability `FG` and `qgut` can back-calculate `cl_int_g` as `qgut * (1 - FG) / FG` with `fu_gut = 1`; show that arithmetic in the source-trace comment.

### fa (**canonical fraction of the oral dose absorbed from the gut lumen**)
- **Type:** paper-named-param
- **Role:** Fraction of an administered oral dose that crosses the apical enterocyte membrane, i.e. the first of the three factors of oral bioavailability `F = fa * (1 - egut) * (1 - eh)`. Unitless, bounded in [0, 1]. Distinguished from bioavailability itself because a first-pass-resolved oral model needs the absorption step to stay separate from the two metabolic-extraction steps, so that a coadministered enzyme modulator changes only the latter.
- **Source aliases:**
  - `fa`, `f_a`, `Fa`, `F_abs` -- the usual compound-table and PBPK spellings.
- **Example models:** `Ezuruike_2018_ethinylestradiol.R` (`fa` fixed at 0.948 per Ezuruike 2018 Table 4, Simcyp-predicted from polar surface area and hydrogen-bond donor count; Figure 3a of the same paper rounds it to "Fa = 1").
- **Notes:** Distinct from `lfdepot` / `fdepot`, which carry RELATIVE bioavailability (an apparent, often covariate-driven scaling of the whole depot input) rather than the mechanistic absorbed fraction: a model can carry both, with `fdepot` computed in `model()` as the `fa * (1 - egut) * (1 - eh)` product. Distinct also from `fg`, used in `Pei_2023_tacrolimus_pbpk.R` for the fraction of the oral dose absorbed INTO a modelled gut compartment of a whole-body PBPK, where the subsequent gut-wall loss is carried by the gut compartment's own ODE rather than by an extraction ratio. Reserve `fa` for the lumen-to-enterocyte step of the three-factor bioavailability product.
...
### h_<process> (**canonical fractal-kinetics heterogeneity exponent**)
- **Type:** paper-named-param
- **Role:** Heterogeneity (fractal) exponent of a Kopelman fractal-kinetics rate, in which a constant first-order rate constant is replaced by the time-dependent *instantaneous rate coefficient* `rate(t) = q / t^h`. Unitless and conventionally bounded in `[0, 1]`, with `h = 0` recovering ordinary Fick / constant-rate kinetics and larger `h` giving a rate that decays faster with elapsed time. The `<process>` suffix names the process whose rate is made fractal, so a model may carry more than one: `h_abs` for an absorption / release rate, `h_caseum` for anomalous diffusion into a granuloma caseous core. Because `h` scales a rate coefficient rather than being one, it is written bare (no `l` prefix) even when the paper estimates it on a bounded interval.
- **Source aliases:**
  - `h` -- Jung 2023 Eq. (1) and Supplementary Tables S1-S5; `H = THETA(n)` in Codes S3, S5, S7, S9, S11.
  - `h` -- Karakitsios 2025 equation (S15), Table S13.
- **Example models:** `Karakitsios_2025_bedaquiline_mouse_pbpk.R` and `Karakitsios_2025_bedaquiline_human_pbpk.R` (`h_caseum = 0.75`, anomalous diffusion through the necrotic caseum rings; founding usage), `Jung_2023_donepezil_singledose.R`, `Jung_2023_im_depot.R`, `Jung_2023_anakinra.R` (`h_abs`, fractal absorption / release; the founding usage of the `h_abs` member).
- **Notes:** Registered with the Jung 2023 extraction to formalise a naming pattern the library was already using: `h_caseum` / `tau_caseum` shipped with the Karakitsios 2025 bedaquiline models without a register entry, so a later extraction had no way to discover the precedent except by grepping `inst/modeldb/`. The exponent is deliberately NOT folded into `hill`: a Hill coefficient shapes a concentration-response sigmoid, whereas this exponent shapes the *time* dependence of a rate coefficient, so sharing a name would give one canonical two dimensions of meaning -- the pattern rejected for `thres` / `cthres`. Note that a paper may put a random effect on `h`, and the two published forms differ: Jung 2023 Code S11 uses a multiplicative log-normal eta (`H = THETA*EXP(ETA)`) while Code S7 uses an additive one (`H = THETA + ETA`), which can drive individual `h` negative. Reproduce whichever form the source used rather than normalising to the usual `exp(l... + eta)` idiom, and say so in the model file.

### tau_<process> (**canonical fractal-kinetics onset-time regulariser**)
- **Type:** paper-named-param
- **Role:** Small positive constant added to elapsed time in a fractal rate, `rate(t) = q / (t + tau)^h`, so the coefficient is finite at `t = 0`. Units are those of time. Paired 1:1 with the matching `h_<process>`.
- **Source aliases:**
  - `TAU` -- Jung 2023 Code S5 (`TAU = 0.0000001`; `KF = EXP(LOG(THETA(8)) - H*LOG(TIME - TD + TAU))`).
  - `tau` -- Karakitsios 2025 equation (S15) ("critical time tau was fixed to a very small positive constant").
- **Example models:** `Karakitsios_2025_bedaquiline_mouse_pbpk.R` (`tau_caseum = fixed(1e-3)` h), `Jung_2023_donepezil_multipledose.R` (`tau_abs = fixed(1e-7)` h, the authors' own value).
- **Notes:** Include this parameter **only when the source paper specifies it**. Because `h < 1` makes the `t^-h` singularity integrable, an ODE solver reproduces the unregularised form without it: for Jung 2023 Case 1, adding `tau = 1e-7` moves the simulated concentration by about 4 parts per million, so a regulariser invented by the extractor would be an unsourced parameter buying nothing. Of the five Jung 2023 cases only Case 2 -- the multiple-dose model, where the rate is restarted at each new dose -- carries a `TAU`, and only that model records one here.
...
- **Notes:** Ratified 2026-09-02 alongside the Snelder 2019 leuprorelin extraction (PMC6533438; ratified by the maintainers). Deliberately NOT folded into the registered `ki50`: `ki50`'s documented form is *stimulatory*, `rate = rmax * I^hill/(ki50^hill + I^hill)`, so reusing it would silently re-label an Imax-type term as an Emax-type one, and its axis is a biomarker readout rather than a receptor-occupancy fraction. Distinct from `lec50` for the same reason -- the driver here is not a concentration. The concept recurs across the GnRH-agonist downregulation family (Romero 2012 triptorelin, Gries 1999 hCG), so a future extraction of either should reuse this name. The Snelder 2019 Discussion warns that `dr50` (with `kd` and `hill_dr`) is conditional on the assumption `agonist_kd_ratio = 1` and "should not be used outside the context of the model".
...
- **Example models:** `Gao_2012_exenatide_glucose_insulin_rat.R` (`sstim_glucose_insulin = 0.0684 1/mM`, `sstim_insulin_glucose = 0.157 1/nM`; founding example), `Bihorel_2017_LY2510924.R` (`sstim_signal_mob = 5.63`, unitless, the amplitude with which the empirical latent `signal` state stimulates CD34+ cell mobilisation from the precursor pool to blood).
- **Notes:** Ratified 2026-09-02 (PMC3336795). Extended 2026-09-21 (PMC5613202) to cover a LATENT empirical driver as well as a measured biomarker: in `Bihorel_2017_LY2510924.R` the driver is the dimensionless `signal` state built up by `lksig`, whose baseline is exactly 0, so the registered form `(1 + sstim * (driver - driver_baseline))` reduces to `(1 + sstim_signal_mob * signal)` and the algebra is unchanged. Two consequences of that extension are worth stating. First, the factor is UNITLESS whenever the driver is a dimensionless `[0, 1]` signal, which breaks the "units are the reciprocal of the driver's units" rule of thumb above. Second, the `<target>` token in that member is a flux (`mob`, the pool-to-blood mobilisation rate) rather than a compartment stem, because the stimulation acts on a transfer rate rather than on a pool's own turnover; prefer a compartment stem when one applies. The paper's own short names `sglu` / `sins` were declined because they do not say which pool is driver and which is target -- fatal in a feedback loop where both pools are simultaneously both. `lslope_<target>` was declined for the same ambiguity and because it would overload `slope`, which in this register denotes a concentration-response LINE (`lslope0` / `lslope_inf` in `Liesenfeld_2006_dabigatran_ECT.R`) or an assay cross-calibration slope (`cal_slope_<assay>`), neither of which is an indirect-response coefficient. Do NOT use this family for a drug's effect on a pool: a drug-driven stimulation of production or loss is `lsmax` / `lemax` with its own potency term. This family is specifically for a BIOMARKER driving another biomarker's turnover. When the driver's elevation is rectified at zero (the effect switching off below baseline), that gating belongs in `model()` as a `(delta > 0)` factor, not in a second parameter.
...
- **Example models:** `Ravva_2015_varenicline.R` (`slope_placebo = -3.32` and `slope_drug = -0.192`, both on the logit scale of a 0-100 craving score, each with an additive eta), `Hartmann_2026_nintedanib_fvcpp.R` (`slope_placebo = -4.74` percent predicted per year) and `Hartmann_2026_nintedanib_fvcz.R` (`slope_placebo = -0.308` Z-score per year), which use `slope_placebo` WITHOUT a `slope_drug` partner: the drug arm of those two models is a saturable `emax * CTROUGH / (ec50 + CTROUGH)` term rather than a second linear slope. That is the intended reading of the pair rule -- the qualified name is used because a distinct drug-effect term exists and the two must be told apart, not because the drug term is also linear. Both are disease-progression slopes acting on the *derivative* of the endpoint (`d/dt(fvcpp) <- slope_i + dreff`), so the units are endpoint per year and the accompanying eta is additive, exactly as the Notes below predict for a slope whose typical value is negative and small relative to its between-subject SD (-2.46 against 5.55 percent predicted per year, so an individual pediatric slope may take either sign). `vanEsdonk_2018_pregabalin_coldpressor.R` carries the pair in a THIRD shape, on a turnover model rather than an additive one: `slope_placebo = -0.07` s/h drifts the TARGET level of the cold pressor pain-tolerance threshold (`kin = kout * (rbase + slope_placebo * t) * (1 + slope_drug * Cc)`), and `slope_drug = 0.135` 1/(mg/L) is a linear FRACTIONAL stimulation of `kin` rather than an additive increment on the response. The units are the discriminator and are worth checking before assuming the additive form: a `slope_drug` whose units are reciprocal-concentration makes `slope_drug * C` dimensionless, so it must multiply, whereas one carrying endpoint-per-concentration adds. Both readings are in scope for this name -- what it denotes is a LINEAR drug-effect slope, not a particular placement.
- **Notes:** Ratified 2026-09-03 alongside the Ravva 2015 varenicline craving extraction (PMC4832970; the maintainers chose `slope_drug`, with its partner spelled `slope_placebo` to match the full-token placebo convention). The two are registered as a pair because the naming only makes sense as a pair -- a model with a single linear drug slope and no placebo arm should use the plain canonical `slope`. An additive eta is load-bearing rather than incidental when the typical slope is negative but small relative to its IIV: it lets a subject's individual slope take either sign, which is what per-subject IPRED panels of such papers show. When a paper fits several parallel linear models of different endpoints, suffix by endpoint (`slope_drug_<endpoint>`) the same way multi-output residual SDs are suffixed.

### lc0, c0 (**canonical log-transformed and bare baseline plasma concentration of the modelled analyte**)
- **Type:** paper-named-param
- **Role:** Baseline (time-zero, pre-first-dose) plasma concentration of the analyte the model predicts, carried in the model's own `units$concentration`. `lc0` is the log-transformed primary form in `ini()`; inside `model()` the bare name is `c0`. Distinct from `lrbase`, which is the baseline level of a PD / turnover / endogenous *state* in the units of that state and is explicitly not a concentration. Two applications are in use and the choice between them is a structural statement the model file must make explicit in a comment:
  - **Additive offset on the prediction** (`Cc <- central / vc + c0`) when the baseline is a genuinely endogenous pool that is continuously replenished and therefore does not wash out. Example: `Qi_2014_sapropterin.R` (endogenous BH4, `C0 = 16.6 ug/L`).
  - **Initial condition on the central compartment** (`central(0) <- c0 * vc`, with a unit-conversion factor when the concentration and amount units differ) when the baseline is residual drug from prior exposure and must decay with the model's own disposition. Example: `Hard_2017_aripiprazole_lauroxil.R` (`ARI(0) = 0.915 ng/mL`, carrying quantifiable pre-dose aripiprazole from pre-randomization oral tolerability doses).
- **Source aliases:**
  - `C0` -- Qi 2014 Table 3 theta5, Muntau 2017 Table 3.
  - `ARI(0)`, `<ANALYTE>(0)` -- NONMEM `A_0(<cmt>)` initial-amount notation, used by Hard 2017 (CNS Drugs ESM Supplemental Table 7) and its predecessor Hard 2017 (J Clin Psychopharmacol Supplemental Table 2). Both tables report the value in ng/mL even though their abbreviation lists gloss it as an "amount", so read the units column, not the gloss.
- **Example models:** `Qi_2014_sapropterin.R`, `Muntau_2017_sapropterin.R` (additive-offset form), `Hard_2017_aripiprazole_lauroxil.R` (initial-condition form).
- **Notes:** Documented 2026-09-16 alongside the Hard 2017 aripiprazole lauroxil extraction; `lc0` / `c0` was already in consistent use in the two sapropterin models without a register entry. May carry IIV as `etalc0` and may be `fixed()` when the source holds it constant. A very large IIV is common and is not a transcription error -- the quantity is a nuisance term mopping up a handful of quantifiable pre-dose samples, so the log-normal it is fitted with has a long tail (Hard 2017 reports a CV of 1898%). Do not use this name for the `c0` of a QSP cytokine baseline that is a state rather than a plasma concentration of the dosed analyte; those models (`Anbari_2023_atezolizumab_cibisatamab_qsp.R` and siblings) carry a paper-specific `c0` documented in their own files.
...
### lcl_dosemin, lcl_dosespan, lcl_dose50, cl_dosemin, cl_dosespan, cl_dose50 (**canonical dose-dependent apparent-clearance family**)
- **Type:** paper-named-param
- **Role:** The three parameters of an apparent clearance that is a saturating DECREASING function of the administered dose, `cl <- cl_dosemin + cl_dosespan - cl_dosespan * doseAbove / ((cl_dose50 - <truncation>) + doseAbove)`. `cl_dosemin` is the high-dose asymptote and carries clearance units; `cl_dosespan` is the span removed as dose rises, from the low-dose maximum `cl_dosemin + cl_dosespan` down to the asymptote, and carries the same clearance units; `cl_dose50` is the dose removing half the span and carries dose units. Log-transformed in `ini()` as the `l`-prefixed forms, bare inside `model()`. The `cl_` prefix is load-bearing: it keeps a clearance-shape parameter distinct from the PD half-maximal names.
- **Source aliases:**
  - `TVCLmin/F`, `TVCLdelta/F`, `dose50` -- Bihorel 2017 Eq. 1 and Table 2 ("the typical difference between the typical minimum apparent elimination clearance and the typical maximum apparent elimination clearance", "the typical half-inhibitory dose").
- **Example models:** `Bihorel_2017_LY2510924.R` (founding example -- `cl_dosemin = 4.75 L/h`, `cl_dosespan = 12.9 L/h`, `cl_dose50 = 3.60 mg` fixed; apparent clearance falls from 17.65 L/h at doses of 1 mg/day and below to 5.40 L/h at 50 mg/day in an 80.1 kg patient).
- **Notes:** Ratified 2026-09-21 (PMC5613202). This is a nonlinearity on the DOSE axis, not the concentration axis, so it is not a Michaelis-Menten elimination and must not be expressed with `vmax` / `km`; the founding paper chose it precisely because its noncompartmental analysis showed clearance falling with dose while distribution volumes stayed flat, and it is an empirical description of a saturable plus a nonsaturable elimination pathway acting together. `span` follows the `cl_matspan` precedent so the middle parameter cannot be misread as a total clearance -- it is a difference between two clearances, and adding it to `cl_dosemin` rather than estimating the maximum independently is what guarantees the function decreases. Reusing the registered `led50` for the half-effect dose was declined because every prior use of `ed50` is a PD or bioavailability one and the axis-agnostic wording of that entry would have let a PK disposition parameter and a PD potency parameter share a name. Expressing the dose dependence through the `e_<cov>_<param>` covariate-effect grammar was also declined because `cl_dosespan` carries clearance units rather than being a dimensionless coefficient, which is not what that grammar means. The truncation dose (1 mg in the founding model, the lowest dose studied) belongs in `model()` as the rectifier on the dose excess, not in a fourth parameter.

### lkpc_cell, lkcp_cell, kpc_cell, kcp_cell (**canonical reversible precursor-pool to circulating-cell transfer rate constants**)
- **Type:** paper-named-param
- **Role:** The two first-order rate constants (1 / time) of a REVERSIBLE cell exchange between a precursor pool and the circulating compartment of a precursor-dependent indirect-response model. `kpc_cell` moves cells from the pool to blood (`precursor1` to `circ`) and is the limb that carries the drug effect in a mobilisation model; `kcp_cell` returns cells from blood to the pool, representing tissue recapture. Log-transformed in `ini()`, bare inside `model()`. Pairs with `lkout` (elimination of the circulating pool) and `lrbase` (the circulating baseline), and the drug-free initial conditions are `circ(0) <- rbase` and `precursor1(0) <- rbase * (kout + kcp_cell) / kpc_cell`.
- **Source aliases:**
  - `Kpc`, `Kcp` -- Bihorel 2017 Figure 1 and Eq. 5 ("first-order rate of transfer of CD34+ cells from the pool to the response compartment", "from the response back to the pool compartment").
- **Example models:** `Bihorel_2017_LY2510924.R` (founding example -- `kpc_cell = 42.4e-6 1/h`, `kcp_cell = 0.185 1/h` for CD34+ progenitor cells mobilised by the CXCR4 antagonist LY2510924).
- **Notes:** Ratified 2026-09-21 (PMC5613202). The `_cell` suffix is mandatory and is the whole point of the names: `kpc` and `kcp` are ALREADY canonical in this register for a drug's central-to-peripheral micro-rate constants, and the founding model carries a drug peripheral compartment in the same file, so the unsuffixed symbols would give one name two meanings a few lines apart. Naming the limbs after the biology instead (`lkmob` / `lkhome`) was declined in favour of preserving a one-to-one map onto the source table's row labels, which keeps the source trace checkable by eye; the direction is recorded in each `label()`. Reusing `lktr` for the forward limb was also declined because `ktr` everywhere else in this register is an UNSTIMULATED absorption or maturation transit rate, whereas here the forward limb carries the entire drug effect. Distinguish from the strictly one-way Friberg-style chains (`ktr` into `circ`), which have no return limb at all: the return limb is what gives this family its tolerance and rebound behaviour.

### lksig, ksig (**canonical first-order build-up rate of an empirical latent stimulatory signal**)
- **Type:** paper-named-param
- **Role:** First-order rate constant (1 / time) governing the build-up of a dimensionless latent `signal` state that saturates at 1, `d/dt(signal) <- ksig * <gate> * (1 - signal)` with `signal(0) <- 0`. The state stands in for a stimulatory influence whose true driver is unmeasured or unidentifiable, and its magnitude is carried separately by an `sstim_signal_<target>` factor. Log-transformed in `ini()`, bare inside `model()`.
- **Source aliases:**
  - `Kt` -- Bihorel 2017 Eq. 5 and Table 2 ("the first-order signal transit rate, which is null for patients which were not enrolled in Study CXAC").
- **Example models:** `Bihorel_2017_LY2510924.R` (founding example -- `ksig = 0.00804 1/h`, a 124 h time constant giving a signal that is essentially saturated about 20 days after the start of therapy, matching the paper's stated steady state).
- **Notes:** Ratified 2026-09-21 (PMC5613202). Deliberately NOT `lktr`, despite the founding paper calling `Kt` a "signal transit rate": `ktr` in this register is an absorption or maturation transit rate constant moving mass through a chain, whereas this moves no mass at all and the state it fills is a dimensionless switch. Use this only when the paper genuinely leaves the driver unidentified -- when the driver IS measured or dosed, model it (a co-medication K-PD compartment, a time-varying covariate) rather than hiding it behind an empirical signal. The founding authors are explicit that the construct is "largely empirical" and warn against extrapolating it outside the conditions it was fitted in; carry that warning into any model that reuses the family.
