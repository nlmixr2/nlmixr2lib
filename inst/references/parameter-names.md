...
### crcl_cap (**canonical breakpoint of a piecewise-linear renal-function covariate relationship**)
- **Type:** bare-pk
- **Role:** Value of the renal-function covariate `CRCL` above which a piecewise-linear ("reverse hockey stick") covariate-parameter relationship stops rising and goes flat, in the units of the `CRCL` column it clamps (mL/min for a raw creatinine clearance, mL/min/1.73 m^2 for a BSA-normalised eGFR). It clamps the covariate axis so the whole relationship is one expression, `cl <- cl_nonren + e_crcl_cl_renal * min(CRCL, crcl_cap)`, in which the rising arm below the breakpoint and the flat arm above it meet CONTINUOUSLY at `CRCL = crcl_cap`. `crcl_cap` is the bare name used inside `model()`; the `ini()` entry is the log form `lcrcl_cap`, back-transformed as `crcl_cap <- exp(lcrcl_cap)`, following the `pna50` / `lpna50` pattern.
- **Source aliases:**
...
- **Notes:** Names the clamp applied to the COVARIATE axis, which is why it is not spelled `crcl50` -- there is no half-maximal point anywhere in this form, and the `*50` names in this register (`pna50`, `cl_t50`, `ec50`, `dr50`) all denote one. **The plateau is not a separate parameter**: by continuity it is `cl_nonren + e_crcl_cl_renal * crcl_cap`, and a source that tabulates an intercept, a slope and a breakpoint but no plateau (as Li 2017 Table 5 does) is reporting this continuous hinge. Generalises as `<cov>_cap` for the same shape on another covariate axis; `Zingmark_2003_clomethiazole.R` already ships this exact role unregistered as `wt50` ("Body weight cap for the piecewise WT-CL relationship"), a candidate to migrate to `wt_cap`. Distinct from [[tclchange]] and [[tkacut]], which are breakpoints on the TIME axis separating two mutually exclusive arms of a piecewise-CONSTANT relationship, where the two arms need not meet; `crcl_cap` sits on a covariate axis and joins two arms that do.
