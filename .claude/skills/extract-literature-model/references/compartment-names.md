...
1. The blessed chain prefixes: `transit<n>`, `effect<n>`, `precursor<n>`, `lat<n>`, `depot<n>` (for parallel-absorption / IOV models with multiple absorption routes — `depot` + `depot2` is the common idiom), `erythrocytes<n>` / `mch<n>` (paired erythrocyte-age and corpuscular-hemoglobin chains), `moderator<n>` (Gabrielsson–Hjorth tolerance chains), and `caseum<n>` (concentric rings of a TB granuloma's necrotic caseous core).
