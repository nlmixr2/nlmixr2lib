...
- `caseum1`, `caseum2`, … — concentric rings of the **necrotic caseous core** of a tuberculosis granuloma, connected in series (catenary chain) and numbered **outward-to-inward** from the outer caseum edge; each state holds a concentration. This is the shape laser-capture ring-dissection data arrives in (fixed-width annuli, each assayed separately) — put the ring width in the description prose, not in the name. Distinct from `lesion`, the *cellular* macrophage-rich rim, which a model may carry alongside the chain; and from `<tissue>_slab<n>`, which is a method-of-lines numerical mesh. An outer caseum in instantaneous equilibrium with `caseum1` is a derived observable, not a state. Founding example: `Karakitsios_2025_bedaquiline_mouse_pbpk.R`.
...
1. The blessed chain prefixes: `transit<n>`, `effect<n>`, `precursor<n>`, `lat<n>`, `depot<n>` (for parallel-absorption / IOV models with multiple absorption routes — `depot` + `depot2` is the common idiom), `erythrocytes<n>` / `mch<n>` (paired erythrocyte-age and corpuscular-hemoglobin chains), `moderator<n>` (Gabrielsson–Hjorth tolerance chains), `caseum<n>` (concentric rings of a TB granuloma's necrotic caseous core), and `bile_transit<n>` (bile-duct delay chain of enterohepatic-recycling models — Bischoff 1971 / Harrison and Gibaldi 1977; use this, not `transit<n>`, so it cannot collide with an oral absorption chain in the same model).
...
- `prob_<endpoint>` -- event probability in `[0, 1]` output by a landmark (static, no time dimension) exposure-response model, typically a logistic regression of a clinical event on a scalar exposure metric plus baseline covariates: `prob_<endpoint> <- expit(logit_<endpoint>)`. The endpoint token is the paper's clinical endpoint in lowercase snake_case (`prob_roc`, `prob_teae_grade3`, `prob_orr_central`). Do **not** rename a probability to `Cc` -- `Cc` names the central-compartment drug concentration, and renaming would falsify the `units$concentration` metadata. Accepted as an **observation variable only**, never as a compartment: these models carry no ODE state, so the shape does not license `d/dt(prob_x)`.

### `prob_<endpoint>` is shape-validated -- no sidecar, but still document the endpoint

This family is the one exception to the "stop and ask before introducing a new canonical output name" rule. `checkModelConventions()` validates it with `conv$probOutputRegex` (`^prob_[a-z](_?[a-z0-9]+)*$`), **not** against an enumerated list, so a well-formed `prob_<endpoint>` passes the moment you write it. Do not file a sidecar to propose one.

**The absence of a gate is not permission to skip the documentation.** Still append a per-endpoint H3 entry to `inst/references/compartment-names.md` in the same PR as the model file, exactly as you would for any other new canonical output. The entry records what the endpoint *means* clinically and how it differs from a same-named measured state (`prob_anemia` is the probability of crossing a toxicity-grade threshold, not a hemoglobin concentration) -- information the regex cannot carry and a reviewer cannot reconstruct from the name. Enumeration previously drifted to 31 registered endpoints against 9 unregistered ones precisely because the gate, not the value of the documentation, was what people were responding to.

The regex is strict about token shape: `prob_`, `prob_roc_`, `prob__roc` and `prob_ROC` are all **non**-canonical. Operator ruling 2026-09-11.
