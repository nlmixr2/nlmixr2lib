...
- **Shape-validated families are exempt from the sidecar but NOT from the documentation.** A few canonical families are validated by regex rather than by an enumerated list, so a well-formed member passes `checkModelConventions()` the moment you write it and needs no operator approval. `prob_<endpoint>` -- the landmark exposure-response probability output -- is the current example (`conv$probOutputRegex`, `^prob_[a-z](_?[a-z0-9]+)*$`; see `references/compartment-names.md` section "Observation variable"). **The missing gate is not permission to skip the register entry.** Still append the per-endpoint H3 entry to `inst/references/compartment-names.md` in the same PR, as documentation of what the endpoint means clinically and how it differs from a same-named measured state -- the regex cannot carry that, and a reviewer cannot reconstruct it from the name. Operator ruling 2026-09-11.
...
**Table column headers — rename, never `col.names`.** When a `knitr::kable()` needs display headers that differ from the data frame's column names, set them with `dplyr::rename("Display label" = actual_column)` piped immediately before the `kable()` call — **never** with `knitr::kable(..., col.names = c(...))`. `col.names` is *positional*: it binds labels to columns by index, so any change in column order silently transposes the headers. The classic trap is `tidyr::pivot_wider(names_from = PPTESTCD)`, which emits `auclast` before `cmax`, so a `col.names = c("...", "Cmax", "AUC0-24")` vector mislabels both. `dplyr::rename()` binds each label to its column **by name**, so the header is always correct regardless of column order. See `references/vignette-template.md` § "Table column headers".
...
1. Re-confirm the branch is on top of fresh `origin/main` (`git fetch origin && git rebase origin/main` if needed). Then format the new model file with [air](https://posit-dev.github.io/air/) (`air format inst/modeldb/<category>/${STEM}.R`, using the repo's `air.toml`); the `format-check` CI job fails on unformatted R code. If `air` is not on `PATH`, install the version CI pins: `curl -LsSf https://github.com/posit-dev/air/releases/download/0.11.0/air-installer.sh | sh`. Format **before** the render gate below: reformatting afterwards is an edit and means re-running the gates.
...
- **Acquisition / missing data** — any needed file/paper/supplement/upstream model, or a wrong-on-disk PDF → add it to `needs_acquisition.jsonl` and defer the task with a `depends_on` on it (try `scripts/acquire-paper.R` first; OA may auto-acquire). **Always acquire — never skip a genuine model for a missing file, never guess from the abstract when the full text isn't OA.** PK cited as "unpublished data on file" → reuse a published same-drug PK model as the PK layer.
  - **Upstream STRUCTURAL / FRAMEWORK source not on disk** (a semi-mechanistic / PBPK / QSP backbone paper, a predecessor poster, or any cited model whose equations/physiologic constants the current paper inherits) → **acquire it + defer**. Do NOT substitute physiologic constants or equation forms from training-data defaults or a "typical SimCYP entry", do NOT "best-effort", do NOT skip. This is the *strongest* form of the acquire rule (mechanism papers cite each other circularly; silent substitution is unauditable).
  - **Auto-acquire FAILED** (paywall, Cloudflare challenge, supplement endpoint blocked, `acquire-paper.R` exit 2) → this does **NOT** downgrade to a sidecar, to "extract main paper only", or to a skip. Add the item to `needs_acquisition.jsonl` and defer; the operator supplies it in a batch (e.g. via `incoming_pdfs/`). **Acquisition — including a failed auto-acquire and any supplement/upstream source — is NEVER a stop-and-ask question.**
...
- **Scope / encoding** (always "+ note the assumption in vignette Errata") — scope question (which scales / endpoints / scenarios / sub-models / metabolites / components / a survival·TTE or PD sub-model alongside the PK) → ALWAYS full fidelity = the MOST-inclusive option; extract EVERY reported endpoint and sub-model, never ask whether to include one. **If an endpoint's or sub-model's data lives in a supplement/table not on disk, acquire it + defer (per Acquisition above) — never drop the endpoint, never extract a partial subset, never ask "extract or defer this sub-model?".** unreported IIV/RUV (structural values present) → `fixed(0)` + erratum, or typical-value-only if no `fixed(0)` option — never invent variances; a deterministic/mechanistic model with no IIV → encode faithfully (no forced etas); ambiguous residual error (e.g. %σ + a fraction, no ERROR block) → encode BOTH proportional + additive; covariate equation absurd as printed → sensible centered/median-normalized interpretation; undefined reference/centering value → rounded standard (e.g. 70 kg); text vs printed-equation conflict → trust the EQUATION; range-only/figure-only parameters → digitize the figures for ALL needed point estimates; between-occasion variability (BOV) → implement as IOV via nlmixr2's native support.
- **Naming / canonicals** — model file / drug-name / file-stem (incl. a journal name parsed into the drug field) → `author_year_drug` (+ species if relevant); generic INN over trade/dev-code; CamelCase multi-word INNs and author names; printable ASCII only (transliterate accents). Auto-approve well-formed members of these canonical families (no sidecar): `STUDY_<id>`, `DOSE_<drug>_<units>`, `FORM_<drug>_<formulation>`, `CONMED_<INN>`, `SNP_<GENE>_RS<rsid>`, `TUMTP_<type>`, `T_<event>`, `prob_<endpoint>` (regex-validated; still add its per-endpoint H3 documentation entry to `inst/references/compartment-names.md`); effect-site PD driver → `CEFFECT`; a general high-dose switch → `DOSE_HIGH`; count covariate → decomposed binary indicators (not a single integer count); `working_dir: null` infra blocker → fill the standard worktree path and re-dispatch. A GENUINELY-new canonical concept — not a member of a family above and not an alias of an existing register entry (grep first via `scripts/lookup-canonical.R`) — DOES need judgment: file the sidecar.

- **File factoring / model count** — how many `.R` files; whether to split by species / study / base-vs-final; one jointly-fit model vs N separate → **build it as the authors built it** (`references/replicate-author-structure.md`): a single jointly-fit model (e.g. an allometric multi-species, or parent+metabolite, model) is ONE file; N genuinely independent models are N files. Apply this SILENTLY — file factoring / model count is NEVER a sidecar question. (The two narrow exceptions in that reference stand: genuinely infeasible to express in rxode2/nlmixr2, or genuinely ambiguous paper text.)

After both layers, file a sidecar ONLY for what remains: a genuinely ambiguous model STRUCTURE the paper text cannot disambiguate, an unresolvable value conflict, or a brand-new canonical concept. **Acquisition (including a failed auto-acquire and any upstream/framework source), file factoring / model count, and endpoint or sub-model inclusion are NEVER sidecar triggers — the policies above decide them.**
