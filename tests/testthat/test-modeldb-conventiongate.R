...
test_that("the convention-error accumulator starts empty and records errors", {
  ...
  issues <- data.frame(
    model = "Fake_2026_drug",
    category = "fixed_label_redundant",
    severity = "error",
    name = "e_wt_cl",
    message = "m",
    suggestion = "s",
  ...
  expect_error(nlmixr2lib:::.conventionErrorsStopIfAny(), "1 convention error\\(s\\) in 1 model\\(s\\)")
...
test_that("the accumulator aggregates across models rather than stopping at the first", {
  ...
  expect_error(nlmixr2lib:::.conventionErrorsStopIfAny(), "2 convention error\\(s\\) in 2 model\\(s\\)")
...
test_that("reset clears a recorded error", {
  nlmixr2lib:::.conventionErrorsReset()
  nlmixr2lib:::.conventionErrorsAdd(
    "X_2026_y",
    data.frame(
      model = "X_2026_y",
      category = "c",
