...
.namedUnitsModel <- function(units, body = NULL) {
  # a one-compartment oral model whose `units` block is supplied by the test
  f <- function() {
    description <- "A"
    reference <- "R"
    units <- NULL
  ...
  body(f)[[4]] <- bquote(units <- .(units))
  f
}

test_that(".unitsMetaForm() tells the legacy and named units forms apart", {
  expect_equal(nlmixr2lib:::.unitsMetaForm(NULL), "none")
  expect_equal(nlmixr2lib:::.unitsMetaForm(list(time = "h", dosing = "mg", concentration = "mg/L")), "legacy")
  expect_equal(nlmixr2lib:::.unitsMetaForm(list(time = "h", concentration = "mg/L")), "legacy")
  expect_equal(nlmixr2lib:::.unitsMetaForm(list(time = "h", depot = "mg", Cc = "mg/L")), "named")
})

test_that("a consistent named units block passes the units checks", {
  skip_if_not_installed("units")
  good <- .namedUnitsModel(list(time = "h", depot = "mg", Cc = "mg/L"))
  res <- suppressWarnings(checkModelConventions(good, verbose = FALSE))
  expect_equal(nrow(res[res$category %in% c("units", "unit_spelling"), ]), 0)
})

test_that("a named units block must name the time, the dosed compartments and the endpoints", {
  skip_if_not_installed("units")
  bad <- .namedUnitsModel(list(time = "h", depot = "mg"))
  res <- suppressWarnings(checkModelConventions(bad, verbose = FALSE))
  u <- res[res$category == "units" & res$name == "Cc", ]
  expect_equal(nrow(u), 1)
  expect_equal(u$severity, "error")
  expect_match(u$message, "units\\$Cc is missing")
  bad <- .namedUnitsModel(list(depot = "mg", Cc = "mg/L"))
  res <- suppressWarnings(checkModelConventions(bad, verbose = FALSE))
  expect_equal(res[res$category == "units" & res$name == "time", "severity"], "error")
})

test_that("a named units block uses canonical spellings in every token", {
  skip_if_not_installed("units")
  bad <- .namedUnitsModel(list(time = "hr", depot = "mg", Cc = "mg/L"))
  res <- suppressWarnings(checkModelConventions(bad, verbose = FALSE))
  u <- res[res$category == "unit_spelling", ]
  expect_equal(u$name, "units$time")
  expect_match(u$suggestion, "Use 'h'")
  bad <- .namedUnitsModel(list(time = "h", depot = "mcg", Cc = "mcg/mL"))
  res <- suppressWarnings(checkModelConventions(bad, verbose = FALSE))
  u <- res[res$category == "unit_spelling", ]
  expect_equal(sort(u$name), c("units$Cc", "units$depot"))
  expect_true(all(grepl("Use 'ug'", u$suggestion)))
})

test_that("a named units block is validated against the model's arithmetic", {
  skip_if_not_installed("units")
  needsFactor <- .namedUnitsModel(list(time = "h", depot = "mg", Cc = "ng/mL"))
  res <- suppressWarnings(checkModelConventions(needsFactor, verbose = FALSE))
  u <- res[res$category == "units", ]
  expect_equal(u$severity, "warning")
  expect_equal(u$name, "Cc")
  expect_match(u$message, "mg/L \\* 1000 = ng/mL")
  expect_match(u$suggestion, "addUnits\\(\\)")
  inconsistent <- .namedUnitsModel(list(time = "h", depot = "mg", Cc = "mg/L", cl = "mg"))
  res <- suppressWarnings(checkModelConventions(inconsistent, verbose = FALSE))
  u <- res[res$category == "units", ]
  expect_equal(u$severity, "error")
  expect_equal(u$name, "kel")
  expect_match(u$message, "has units mg/L but kel is 1/h")
})

test_that("without the units package a named units block gets an info issue only", {
  good <- .namedUnitsModel(list(time = "h", depot = "mg", Cc = "mg/L"))
  testthat::with_mocked_bindings(
    .unitsAvailable = function() FALSE,
    {
      res <- suppressWarnings(checkModelConventions(good, verbose = FALSE))
      u <- res[res$category == "units", ]
      expect_equal(u$severity, "info")
      expect_match(u$message, "not installed")
    }
  )
})
