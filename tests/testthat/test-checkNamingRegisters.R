test_that("the naming registers are structurally clean", {
  ...
  skip_if(
    !dir.exists(file.path(root, "inst", "references")),
...
test_that("checkNamingRegisters detects each defect class it claims to", {
  ...
  writeLines("x <- 1  # token: real_token", file.path(tmp, "inst", "modeldb", "Real_2020_drug.R"))
...
test_that("checkNamingRegisters does not flag legitimate register patterns", {
  ...
  dir.create(file.path(tmp, "inst", "modeldb"), recursive = TRUE)
  writeLines(
    c("d/dt(central_dox) <- -k * central_dox", "d/dt(igg) <- -kel * igg", "kmet <- exp(lkmet_ntp_peripheral1)"),
...
test_that("checkNamingRegisters enforces the covariate ALL-CAPS convention", {
  ...
  dir.create(file.path(tmp, "inst", "modeldb"), recursive = TRUE)
  writeLines(
    "cl <- exp(lcl) * (1 + e_mixedCase * mixedCase + e * LEGACY_ok)",
    file.path(tmp, "inst", "modeldb", "Real_2020_drug.R")
  )
  ...
  writeLines(register, file.path(tmp, "inst", "references", "covariate-columns.md"))
  issues <- checkNamingRegisters(tmp)
  ...
  expect_equal(issues$name[issues$check == "stale-case-exemption"], "gone_missing")
...
test_that("each register's documented type vocabulary matches the types it uses", {
  ...
  registers <- c("covariate-columns.md", "parameter-names.md", "compartment-names.md")
  ...
  documentedAll <- unique(unlist(lapply(file.path(dir, registers), nlmixr2lib:::.schemaTypes)))
