...
test_that("addSecondAbsorption adds a first-order second path with an F1 split", {
  ...
  .splitLines <- vapply(res$lstExpr,
    function(l) deparse1(l), character(1), USE.NAMES = FALSE)
  ...
  expect_equal(as.character(res$splitInfusionBolus),
...
test_that("addSecondAbsorption lag and transit delays", {
  ...
  expect_error(res0 |> addSecondAbsorption(delay = "transit"),
    regexp = "ntransit|n"
  )
...
test_that("addSecondAbsorption zero-order second path", {
  ...
  expect_equal(rxode2::modelExtract(res, "dur(depot2)"),
    "dur(depot2) <- tk0")
  ...
  expect_equal(rxode2::modelExtract(res, "dur(depot2)"),
    "dur(depot2) <- tk0")
  expect_equal(rxode2::modelExtract(res, "d/dt(depot2)"),
    "d/dt(depot2) <- -ka2 * depot2")
  expect_equal(rxode2::modelExtract(res, "d/dt(central)"),
  ...
  keep <- !vapply(le, function(l) {
    .d <- deparse1(l)
    grepl("~", .d, fixed = TRUE) || grepl("propSd", .d, fixed = TRUE) ||
      grepl("Cc <-", .d, fixed = TRUE) || grepl("f\\(", .d) ||
  ...
  # hour of debugging)
  s <- as.data.frame(rxode2::rxSolve(resNoF, e, params = c(
    lka = log(1.2), lka2 = 0.1, lcl = log(0.1), lvc = log(10),
    ltk0 = 0.1, logitfDepot = logit(0.7)
  )))
  ...
  expect_equal(d$central, c(c1(4) + c2(4), c1(8) + c2(8), c1(48) + c2(48)),
    tolerance = 1e-4
  )
...
test_that("addSecondAbsorption works on a zero-order first path", {
  ...
  expect_equal(as.character(res$splitInfusionBolus),
    "splitInfusionBolus(central, central, depot2)")
  expect_equal(rxode2::modelExtract(res, "f(central)"),
    "f(central) <- fCentral")
...
test_that("convertAbsSequential ties the second lag to the first duration", {
  ...
  expect_error(
    suppressWarnings(res0 |> addZeroOrderAbs()) |>
      addSecondAbsorption(type = "first", delay = "none") |> convertAbsSequential(),
    regexp = "no lag time"
  )
...
test_that("double absorption solves like the simultaneous first-order seed", {
  ...
  pPipe <- c(
    lka = log(1.2), lka2 = log(1.2), lcl = log(0.1), lvc = log(10),
    logitfDepot = logit(0.7), llagDepot2 = log(9), propSd = 0.5
  )
  ...
  eRef <- rxode2::et(time = 0, amt = 100, cmt = "depot1") |>
    ...
    rxode2::et(seq(0, 48, by = 0.5))
  sRef <- as.data.frame(rxode2::rxSolve(ref, eRef, addDosing = TRUE,
    params = c(lka1 = log(1.2), lka2 = log(1.2), lcl = log(0.1),
      lvc = log(10), lgfdepot1 = logit(0.7), ltlag = log(9), propSd = 0.5)))
