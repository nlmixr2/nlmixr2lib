...
Start from an initial dataset that has a previous state and a current state column. For this exercise, we will use a hypthetical adverse event profile switching between "none", "mild", and "moderate" severities.
...
```{r create-model}
mod <- createMarkovModel(statePrior = dMarkov$previous, stateCurrent = dMarkov$current)
cat(mod)
```
...
The model can be subsequently be modified the same as any other `nlmixr2` model, for example to add drug effects, etc.
...
createMarkovTransitionMatrix(statePrior = dMarkov$previous, stateCurrent = dMarkov$current)
createMarkovTransitionMatrix(statePrior = dSim$previous, stateCurrent = dSim$current)
```
