---
title: "Masters_2022_avelumab"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Masters_2022_avelumab}
...
weight-based regimen using a population PK model inherited from
Wilkins 2019 (Ref. 20 in the paper), re-estimated on a pooled dataset
dominated by the JAVELIN Renal 100 / 101 advanced-renal-cell-carcinoma
(aRCC) combination-arm population.
...
with \(I_{\max} = -0.08533\) (a fractional **decrease** at \(t \gg T_{50}\))
and allometric weight scaling with reference weight 80 kg.
...
supplementary material): weight range 44.2–143.0 kg, median 81.5 kg;
for the prior monotherapy sub-population: weight range 30.4–204 kg,
median 70.6 kg. Both regimens dosed avelumab **10 mg/kg IV every 2
weeks**, with the 800 mg flat dose simulated as the alternative.
...
the approved flat **800 mg Q2W**, each given for 13 cycles over 180 days.
...
- **Table 1 θ_Q typo (0.3352 → 0.03352 L/h):** The paper's Table 1
  ...
  `Masters_2022_avelumab.R`. No erratum is posted on the publisher
  site as of 2026-04-20.
- **Reference weight:** Masters 2022 does not explicitly state the
  ...
  weight, so 80 kg is used here as the allometric reference for CL,
  V1, V2, and Q.
...
- **Residual error convention:** The `sigma` column in Table 1 is the
  ...
  additive (2.168 µg/mL) residual components, matching NONMEM's
  default reporting. Used directly as `propSd` and `addSd`.
- **Virtual cohort:** Demographics were simulated as a log-normal
  weight distribution anchored to the aRCC-subpopulation median
  (81.5 kg) and truncated to the reported range (44.2–143.0 kg). The
  paper does not tabulate other baseline covariates for the final
  model because only body weight enters the final structural model.
- **Time units:** The source reports CL and Q in L/h and T50 in days.
  The packaged model keeps time in **days** for consistency with T50;
  CL and Q are converted via ×24.
