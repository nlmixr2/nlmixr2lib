...
interval-censored; the BAST guiding document § 2.4.1 (Figure 2-3)
selected log-normal as the base distribution by AIC, and § 2.4.2
...
covariates (BAST § 2.2.2). Of the 200 simulated patients, 36 (18%)
...
|----------------------------------|------------|----------------------------------------------------------------------------------|
| Hazard form `h(t) = val * pdf(t) / (1 - Phi(num))` | n/a | Executable_runCOMPEV1_101.mod $PK / $DES (`pdf = (1/(fac*lambda*(T+DEL)))*exp(-(num**2)/(2*lambda*lambda))`, `DADT(1) = VAL*pdf/(1-phi(num/lambda))`); BAST guiding doc § 2.4.1 confirms log-normal selected |
...
| η on `lambda` (`OMEGA(1,1)`)     | 0 FIXED    | Output_simulated_runCOMPEV1_101.res FINAL OMEGA(1,1) = 0.00E+00 (placeholder; no estimated IIV) |
| Covariate-selection ΔOFV         | -14.822    | BAST guiding doc § 2.4.2, Table 2-4 (AGE chosen as final covariate model COMPEV1_101) |
...
The BAST guiding document § 2.4.1 (Figure 2-3) reports a Kaplan-Meier
...
The BAST guiding document § 2.4.2 (Table 2-4) reports a +50.9
...
* **Interval-censored event times.** Competing Event 1 events are
  ...
  event times to the next assessment visit (BAST guiding doc § 2.3.3).
