...
8. **`fvcpp` and `fvcz` are declared `paper_specific_compartments`, not
   canonical compartments.** The register carries `fev1pp` (FEV1 percent
   predicted) as the direct sibling, but the standing convention is that a
   compartment role is promoted to canonical only once a *second* independent
   paper uses it. `checkModelConventions()` therefore reports one
   `observation` warning per exposure-response model; it is expected and is not
   a naming error. A second FVC-endpoint extraction is the trigger to promote
   both names.
