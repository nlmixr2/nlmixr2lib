---
title: "Selexipag and ACT-333679 (Krause 2017)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Selexipag and ACT-333679 (Krause 2017)}
...
.mod_meta <- rxode2::rxode(readModelDb("Krause_2017_selexipag"))
```
...
ACT-333679 has its own two-compartment disposition with apparent
central volume V_m/F = 4.65 L and elimination rate constant k_m = 0.49
1/h. Body weight (allometric on V_p/F, CL/F, and V_m/F), total bilirubin
(power on CL/F), sex, and a four-level PAH-comedication categorical
(naive / ERA only / PDE5 inhibitor only / ERA + PDE5 combined; reference
= naive) were retained as statistically significant covariates.
Proportional residual error of 75% (selexipag) and 49% (ACT-333679)
captures phase III sparse-sampling variability.
...
reported by Krause 2017 contained 512 selexipag-treated patients with
at least one usable post-baseline plasma concentration. Patients were
...
68/559 (12%) treated patients had an IMD of 200 ug and 163/559 (29%)
had the maximum allowed IMD of 1,600 ug.

Reference "typical patient" covariate values were 40 years of age, 70 kg
body weight, female, total bilirubin 10 umol/L, creatinine clearance
100 mL/min/1.73 m^2, AST and ALT 20 U/L, PAH-comedication-naive, and
...
parameter estimates"); the covariate-structural equations are from
Table 2 and Methods "Covariate selection (PK)"; the structural-model
layout is from Figure 1 and the Results Population PK paragraph.
...
5 and 25 umol/L comparison strata, and PAH-comedication-naive (reference)
versus the combined ERA + PDE5-inhibitor stratum. Doses are 1,600 ug
selexipag b.i.d. (q12h) -- the maximum allowed maintenance dose, used
to reproduce Krause 2017 Figure 2's steady-state profile.
...
stochastic VPC overlay is generated by solving the full population
model with all random effects active.

```{r simulate-typical, fig.width = 7.5, fig.height = 4.5}
...
profile of selexipag (left panel) and ACT-333679 (right panel) at
steady state on a dose of 1,600 ug b.i.d., for the reference patient.
The colour-coded ranges in the paper correspond to deciles of the
...
combined), and total bilirubin (5 vs 25 umol/L) at the reference dose
of 1,600 ug b.i.d. The replicated panels below stack these one-at-a-time
covariate sweeps; each panel is a typical-value comparison (no random
effects) so only the covariate-driven shift is visible.

```{r figure-3, fig.width = 8.5, fig.height = 9.5}
...
Krause 2017 Table 3 expresses the covariate effects on
steady-state exposure (AUCss) as percentage changes relative to the
reference 70 kg female, 10 umol/L bilirubin, PAH-naive patient. The
table below reproduces the published numbers next to the simulated AUCss
ratios computed from the typical-value cohorts above. Differences within
...
- **Lag-time IIV not represented.** Krause 2017 reports an
  ...
  estimated via Monolix's logit-bounded transformation, individual
  t_lag values restricted to the 0-2 h "gastrointestinal passage time"
  interval (Krause 2017 Methods, "Population PK"). This bounded
  transformation is not directly representable in nlmixr2 / rxode2;
  the packaged model fixes t_lag at the population-typical 0.668 h
  with no IIV. The simulated absorption-phase variability is therefore
  slightly understated; the steady-state predictions (the focus of the
  published validation) are insensitive to this simplification.
- **Diagonal random-effects covariance.** Krause 2017 fits the
  ...
  assumption. Empirical posthoc-eta correlations are not reported in
  the source paper.
- **Rate-constant parameterisation preserved over canonical CL/V/Q/Vp.**
  Krause 2017 parameterises both the selexipag and ACT-333679
  peripheral compartments via first-order rate constants (k_12, k_21,
  k_34, k_43) rather than canonical inter-compartmental clearance and
  peripheral volume. The packaged model keeps the rate-constant form
  (parameter names `lk12`, `lk21`, `lk34_act`, `lk43_act`) so the
  published IIV omegas can be carried verbatim. A canonical-naming
  re-parameterisation would have required a log-normal variance
  translation under an independence assumption that is not warranted
  in this case.
- **Sex contrast is encoded on the male indicator.** Krause 2017 makes
  ...
  reference category and the canonical convention that SEXF = 1
  denotes female.
- **PAH comedication encoded as three orthogonal indicators.** The
  source paper's four-level PAH-comedication categorical
  (naive / ERA only / PDE5I only / ERA + PDE5I) is decomposed into
  three mutually-exclusive binary indicators (`CONMED_ERA`,
  `CONMED_PDE5I`, `CONMED_ERA_PDE5I`) with the naive stratum as the
  reference (all three = 0). The combined stratum carries its own
  categorical coefficient (b = 0.37) rather than the multiplicative
  product of the ERA-only and PDE5I-only effects -- the source paper
  fits four independent categorical levels rather than a class-level
...
- **F (bioavailability) not explicitly modeled.** All parameters are
  apparent (CL/F, V_p/F, V_m/F) because absolute bioavailability is
  not estimated in the source data. Dosing in the simulation is in ug
  and concentrations come out in ng/mL = ug/L given the L volume units.
