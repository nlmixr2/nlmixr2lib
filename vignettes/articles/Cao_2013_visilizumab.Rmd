---
title: "Visilizumab (Cao 2013)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Visilizumab (Cao 2013)}
...
The terminal half-life predicted by the typical-value mPBPK fit corresponds to visilizumab's reported half-life of approximately 2-3 weeks in the underlying Carpenter 2002 study; Cmax and AUC scale linearly with dose because the model is purely linear (no TMDD, no concentration-dependent clearance).
...
- **Model A (clearance from plasma) used by default.** Cao 2013 also reports Model B (clearance from interstitial fluid; CLi = 0.0390 L/hr for visilizumab); Model A is used here for consistency across the 12 nlmixr2lib entries from this paper. Cao 2013 reports a slightly lower objective-function value for Model B in 7 of 10 human mAbs but notes that Model A is more reasonable on the latent constraint sigma1 > sigma2.- **Drug-specific note.** Doses are reported in mg/m^2 (BSA-normalized) in Cao 2013 Figure 5; this model treats dose in mg, so external scaling of mg/m^2 -> mg requires the subject body surface area. The simulation panel below uses 3 mg/kg (~210 mg) IV as an illustrative substitute.
