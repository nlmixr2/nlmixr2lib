---
title: "Clopidogrel (Danielak 2017)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Clopidogrel (Danielak 2017)}
...
* Citation: `r readModelDb("Danielak_2017_clopidogrel")()$reference`
* Description: `r readModelDb("Danielak_2017_clopidogrel")()$description`
* Article: <https://doi.org/10.1007/s00228-017-2334-z> (open access; European Journal of Clinical Pharmacology 2017)

The Danielak 2017 paper develops a joint parent-metabolite population PK model
for oral clopidogrel and its pharmacologically active thiol metabolite H4
(the antiplatelet-active diastereomer that irreversibly inhibits the platelet
P2Y12 receptor). Because H4 is the active species, this paper is the canonical
reference for clopidogrel pharmacogenetic dose-response in CYP2C19 loss-of-
function carriers.

# Population

The model was developed from 63 adult Caucasian patients undergoing elective
coronarography or percutaneous coronary intervention (PCI) at a single
cardiology centre in Poznan, Poland (Danielak 2017 Table 1). All patients were
on chronic oral clopidogrel 75 mg once daily for at least 7 days prior to the
procedure. Demographics: mean age 65.4 +/- 10.5 years, mean weight 79.1 +/-
14.0 kg, mean BMI 28.40 +/- 4.89 kg/m^2 with 67.3% classified as obese
(BMI >= 30); 66.7% male / 33.3% female; 34.9% with diabetes mellitus; 95.2%
on concomitant statin; 73.0% on a concomitant proton-pump inhibitor (mostly
pantoprazole). Exclusion criteria removed patients with acute MI, ongoing
malignancy, liver dysfunction, impaired renal function (serum creatinine
> 2 mg/dL), or treatment with glycoprotein IIb/IIIa antagonists / coumarins /
...
* CYP2C19*17 carriers (ultra-rapid metabolizers, *1/*17 or *17/*17): 25/63 (39.7%)
* CYP3A4*1G carriers: 10/63 (15.8%)
* ABCB1 3435TT homozygotes: 22/63 (34.9%)
* No CYP2C19*2/*2 poor metabolizers were enrolled.

Sampling: 17 subjects provided a full PK profile at 0.5, 1, 2, 3, and 4 h after
dosing; the remaining 46 subjects provided two samples (0.5 + 2 h or 1 + 3 h
post-dose). Sampling was truncated at 4 h post-dose because both clopidogrel
and H4 are rapidly eliminated and most concentrations beyond 4 h are below the
quantitation limit of 0.25 ng/mL.
...
entry in `inst/modeldb/specificDrugs/Danielak_2017_clopidogrel.R`. The table
...
simulations below use a virtual cohort that matches the published genotype
distribution (Table 1): 31.7% CYP2C19*2 carriers, 68.3% non-carriers (pooled
*1/*1 + *1/*17 + *17/*17). Continuous covariates do not enter the final model,
so age / weight / sex / BMI distributions are not used for prediction.
...
Danielak 2017 Figure 4 reports box-and-whisker AUCs of clopidogrel (panel a)
and the active H4 metabolite (panel b) for CYP2C19*2 carriers vs wild-type
homozygotes. The published conclusion is that clopidogrel AUC is unchanged
between groups while H4 AUC is significantly lower in carriers
(mean 8.59 +/- 6.48 vs 13.58 +/- 7.10 ng h/mL, p = 0.004).
...
NCA on the simulated profiles, stratified by CYP2C19*2 carrier status, with
one PKNCA block per analyte (parent + metabolite).
...
Danielak 2017 reports the model-predicted AUC of the active H4 metabolite
stratified by CYP2C19*2 carrier status (Results "Comparison of exposure to
clopidogrel and H4"):
...
The simulated means for H4 are within ~15-20% of the published values (allow
for sampling noise at n = 200 vs n = 20-43 in the original cohort). The
typical-value reduction by carrier status is ~45% (TVFM scales by 1 - 0.45 in
carriers), and the mean reduction in the lognormal simulation is closer to the
paper's 36.7% mean reduction because of the high IIV(FM) = 71.4% CV (a
lognormal mean shifts upward more strongly when the underlying CV is large).
...
* **Residual-error sign-artifact.** Danielak 2017 Table 2 reports the
  proportional residual errors as negative values (-0.45 for clopidogrel,
  -0.66 for H4). The negative sign is a NONMEM parameterisation artifact:
  when the residual model is coded as `Y = F * (1 + THETA(N) * EPS(1))` with
  `EPS(1) ~ N(0, 1)` and `THETA(N)` unconstrained, the optimizer converges
  to either +THETA or -THETA with identical likelihood because the product
  `THETA(N) * EPS(1)` has the same distribution as `-THETA(N) * EPS(1)` when
  `EPS` is symmetric. The magnitude |-0.45| = 0.45 and |-0.66| = 0.66 are
  the proportional residual SDs on the linear-concentration scale (i.e. ~45%
  CV for clopidogrel and ~66% CV for the H4 metabolite). The model file
  encodes the absolute values; the negative-sign source row is documented
  per-parameter in the in-file comment. An alternative interpretation (the
  table reports `log(SIGMA)` on the natural-log scale, giving
  `SIGMA_CLP = exp(-0.45) = 0.638` and `SIGMA_H4 = exp(-0.66) = 0.517`)
  is plausible but does not fit the table's `%RSE` reporting of the residual
  error as straightforwardly as the THETA-on-EPS reading; the magnitudes
  (~45-65% CV under either reading) are similar enough that the typical-
  value simulation and the PKNCA comparison above are unaffected. Future
  re-analysis with the source NONMEM control stream on disk would
  disambiguate the parameterisation.

* **Continuous covariates not retained.** Danielak 2017 tested age, weight,
  BMI, obesity (BMI >= 30), sex, diabetes mellitus, PPI / statin co-medication,
  CYP2C19*17 allele, CYP3A4*1G allele, and ABCB1 3435TT genotype as covariates,
  but only CYP2C19*2 carriage on FM was retained in the final model after the
  backward-elimination step. The virtual cohort in this vignette therefore
  carries only `CYP2C19_S2_CARRIER` as a covariate; demographic columns are
  not needed for prediction. Continuous covariates were assigned the cohort
  mean (age 65.4, weight 79.1 kg, BMI 28.4) in the typical-subject narrative
  but do not enter the model equations.

* **CYP2C19*17 not encoded.** The paper found that CYP2C19*17 on FM and on
  V/F were significant in the initial step but were dropped after CYP2C19*2
  was added (Results page 1626). The vignette does not stratify by *17
  carriage; the simulated non-carrier group is a pool of *1/*1, *1/*17, and
  *17/*17 subjects (consistent with how the paper's final-model predictions
  treat them).

* **Pediatric / non-Caucasian extrapolation not in scope.** The model was fit
  to 63 adult Caucasian patients with stable coronary artery disease. Use in
  pediatric, non-Caucasian, hepatic-impairment, or renal-impairment
  populations would require external validation. The Methods exclusion
  criteria removed renal impairment (serum creatinine > 2 mg/dL) and liver
  dysfunction subjects.

* **CES1 inactive-carboxylic-acid pathway implicit.** Roughly 85% of the
  absorbed clopidogrel dose is inactivated to the inactive carboxylic acid
  metabolite by carboxylesterase 1 (CES1) before reaching the systemic
  circulation (Discussion page 1628). The CES1 path is implicit in the
  parent's total `CL/F` -- this model carries the fraction routed to H4 as
  `FM` and the remaining `(1 - FM)` is the lumped elimination via the
  CES1 carboxylic-acid pathway plus any minor pathways. The CES1 metabolite
  itself is not in the model because it is not pharmacologically active and
  was not measured.

* **AUC validation window.** The published AUCs (Figure 4, Results page 1628)
  appear to be model-predicted F*D/CL_total (clopidogrel) and F*D*FM/CL_h4
  (H4) values, i.e. AUC-to-infinity under the assumption that the dosing
  interval covers the elimination phase. Because clopidogrel kel is ~1.89/h
  and H4 kel is ~51.5/h (half-lives 22 min and 0.8 min), the AUC0-6h
  computed in this vignette is essentially identical to AUCinf (less than 1%
  of the integral lies beyond 6 h post-dose). The agreement with the paper's
  reported means (within ~15-20%) reflects sampling at n = 200 virtual
  subjects vs n = 20-43 carrier / non-carrier groups in the original cohort.

* **Single-dose vs steady-state.** The paper sampled on day 7 or later of
  chronic 75 mg once-daily dosing. Because the elimination half-life is
  short (<= 22 min for the parent), the trough concentration before each
  dose is essentially zero and a single-dose simulation of 75 mg reproduces
  the same Cmax and AUC0-tau as the steady-state pattern. The vignette
  therefore uses a single 75 mg dose for the figures; a steady-state run
  with daily dosing for 7 days produces indistinguishable results.
...
* **M3 BQL handling not reproduced.** The source NONMEM fit used the M3
  ...
  generates continuous predictions without truncation; the user can apply
  a post-hoc `pmax(Cc, 0.25)` or a censoring rule if a BQL-aware downstream
