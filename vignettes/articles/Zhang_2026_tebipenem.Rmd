...
# The bounds below are deliberately NOT set at the observed extremes. Both
# perturbations above are draw-dependent: which participants sample a ka above
# ~50 1/h (and so overshoot on the trapezoid) and which sample a long lag are
# properties of the eta draw, and rxode2 generates those from rxSetSeed() in a
# version-dependent way -- this vignette rendered clean on rxode2 5.1.7 and
# failed CI on 5.1.6 at exactly this line with the old bound of 1.02, which had
# been set at the maximum observed at authoring time.
#
# The MEDIAN is what makes this a structural check: a mis-transcribed clearance,
# dose or molecular-weight ratio moves the ratio by tens of percent and blows
# the median instantly. That assertion is kept tight. The per-participant
# envelope is widened to bracket the documented numerical artefacts with room
# for a different draw, and stays far tighter than any real transcription error.
stopifnot(all(identity_chk$ratio <= 1.06),
          all(identity_chk$ratio >= 0.80),
