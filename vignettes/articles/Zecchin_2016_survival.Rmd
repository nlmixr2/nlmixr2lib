...
* Source bundle: GitHub mirror [`dpastoor/ddmore_scraping/218/`](https://github.com/dpastoor/ddmore_scraping/tree/master/218)
...
available when this model was built; Methods / Table 2 cross-checks were performed
via PMC HTML.
...
patient-level data, the vignette compares the model's typical-value
...
For a time-to-event model the appropriate check is that typical-value
hazard / survival trajectories reproduce the qualitative behaviour
...
A full self-consistency check would re-simulate the bundle's
...
CSV. The cohort built above is a faithful smaller-scale analogue and
Checks 1-3 above are the substitute exercised in this
vignette.
...
* **No publication-PDF cross-check.** The Zecchin 2016 BJCP PDF was not
  ...
  performed against the PMC HTML version (PMC5338128).
