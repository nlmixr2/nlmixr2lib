...
(not available when this model was built). The cohort below is a pragmatic
approximation centred so that reference-subject predictions reproduce the
Table 1 typical values; weight
...
- **No correlation block on IIV.** Quartino 2016 reports IIV on F, CL, Vc,
  ...
  block. If the original NONMEM control stream (Online Resource 4, not
  available when this model was built) used a block omega, the diagonal
  approximation here may underestimate population-level correlation between
  F and CL or between CL and Vc.
...
- **Online Resources 3-13** are referenced throughout the paper for
  ...
  built, so the virtual cohort and validation here rely solely on what the main
