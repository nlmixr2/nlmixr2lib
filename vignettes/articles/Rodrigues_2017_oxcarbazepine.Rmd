---
title: "Rodrigues_2017_oxcarbazepine"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Rodrigues_2017_oxcarbazepine}
...
`COMED_EIAED` indicator. Bioavailability `F` is fixed to 1 per the
...
make_cohort <- function(n, dose_mg_per_kg, eiaed_prevalence,
  ...
  obs <- base |>
    select(id, WT, COMED_EIAED, regimen) |>
...
sim_long <- sim_sd |>
  ...
  mutate(analyte = recode(analyte,
                          Cc     = "OXC",
                          Cc_mhd = "MHD"),
         eiaed_label = ifelse(COMED_EIAED == 1, "EIAED+", "EIAED-"))
...
build_ss_events <- function(wt, eiaed, dose_mg_per_kg_per_day,
  ...
  base <- tibble(id = ids, WT = wt, COMED_EIAED = eiaed,
...
sim_ss <- rxode2::rxSolve(
  ...
  keep   = c("WT", "COMED_EIAED", "cell", "dose_per_kg_day"),
  returnType = "data.frame"
) |>
  dplyr::filter(time > 0)
...
ss <- sim_ss |>
  group_by(id, cell, COMED_EIAED, WT) |>
  summarise(
...
- **EIAED indicator polarity.** Rodrigues 2017 codes `MED = 1` when
  ...
  canonical `COMED_EIAED` column in nlmixr2lib follows the standard
  COMED_* polarity (1 = on EIAED, 0 = off), and the model() block
  applies the source coefficient via `(1 - COMED_EIAED)` so that
