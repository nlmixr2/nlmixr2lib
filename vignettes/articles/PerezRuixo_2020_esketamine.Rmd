...
`readModelDb("PerezRuixo_2020_esketamine")()$population`. `readModelDb()`
returns the model *function*; the trailing `()` evaluates it to the model
object whose `$population` element holds this list.
...
But **1/F_met explains the discrepancy only partially**, for two reasons:
...
but is a covariate- and dose-dependent band, not the single deterministic
`1.41` factor a literal `1/F_met` reading would imply. Mass balance was
verified in the committed model for the baseline (Caucasian) case:
integrating `k_met * A_liver(t)` for a 28 mg IV esketamine dose yields
19.94 mg into the noresketamine pool, matching the expected
...
a convention whose effect on the reported AUC0-24 is modulated by the
