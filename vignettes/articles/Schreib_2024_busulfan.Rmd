...
**Every `rxSolve()` call in this vignette passes `useLinCmt = FALSE`.** That is
now belt and braces rather than a necessity, and it is kept deliberately -- see
below.

Schreib 2024 is a one-compartment model with linear elimination, which is the
structure `rxSolve()` recognises and can rewrite into a closed-form `linCmt()`
solution. That closed form can only carry a **constant** elimination rate
constant, so through rxode2 5.1.6 the rewrite silently substituted `kel` at
`t = 0` for all time and discarded the entire time dependence -- the whole
subject of this paper. The failure was silent, one-sided and large: under the
default flag every interval collapsed to exactly `Dose / (kel(0) * vc)`.

rxode2 5.1.7 closed that hole from the other side. It now refuses the
ODE-to-`linCmt()` conversion when the conversion would change a rate or when
the model carries an exogenous input term, which covers this model, so the
rewrite no longer happens and both flags give the same answer. The check below
solves the same regimen both ways and asserts exactly that: the time
dependence survives, and the two paths agree.

Passing the flag explicitly is still worth doing. It states which solver path
the numbers came from instead of inheriting a package default -- and that
default itself changed in rxode2 5.1.8, from `TRUE` to `FALSE`.
...
only in Table S1 and Figures S4-S9 of the supplement, which was not available
...
* **`useLinCmt = FALSE` on rxode2 5.1.6 and earlier.** The model is
  ...
  ODE-to-`linCmt()` conversion used to replace it with a closed form that can
  only hold a constant elimination rate constant, silently discarding the time
  dependence and under-predicting steady-state exposure per interval by about
  17% (45% in the HLH/XLP group). Rewriting the ODE did not help: an explicit
  rate variable and a clearance-form denominator were both tested and were
  converted identically, so the flag was the only mitigation. rxode2 now
  5.1.7 refuses the conversion when it would change the model
  and the flag is no longer required; this vignette keeps it so that it stays
  correct on older installations. This was always a property of the solver's
...
* **Virtual-cohort distributions.** Age, weight, albumin, infusion duration, sex
  ...
  patient counts appear only in Figure S1, which was not available when this
  model was built. Disease
