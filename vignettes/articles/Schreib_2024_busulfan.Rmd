...
**Every `rxSolve()` call in this vignette passes `useLinCmt = FALSE`.** On a
current rxode2 that is belt-and-braces rather than a requirement, but it keeps
the vignette correct on older installations -- see below.

Schreib 2024 is a one-compartment model with linear elimination, which is
structurally what `rxSolve()` used to recognise and rewrite into a closed-form
`linCmt()` solution by default. That closed form can only carry a **constant**
elimination rate constant, so on rxode2 5.1.7 and earlier the rewrite silently
substituted `kel` at `t = 0` for all time and discarded the entire time
dependence -- which is the whole subject of this paper. Every dosing interval
collapsed to exactly `Dose / (kel(0) * vc)`, an under-prediction reaching 16.5%
by the eighth interval, with no error and no warning.

rxode2 now refuses the conversion when it would change the model
(rxode2 issue 1370), so both flags give the same answer. The check below solves
the same regimen both ways: the two columns must agree, and the AUC must rise
across intervals as clearance falls.
...
tibble::tibble(
  ...
  knitr::kable(
    caption = paste(
      "Both flags now give the same answer, and the AUC rises across intervals",
      "as clearance falls. On rxode2 5.1.7 and earlier the first column was",
      "flat from the second interval on, under-predicting by up to 16.5%."
    )
  )

# Two standing gates. The first is the physiology: clearance falls over the
# course, so exposure must accumulate faster than proportionally. The second
# catches any reintroduction of the constant-kel conversion (rxode2 issue
# 1370), which would flatten the useLinCmt = TRUE column.
...
* **`useLinCmt = FALSE` on rxode2 5.1.7 and earlier.** The model is
  ...
  refuses the conversion when it would change the model (rxode2 issue 1370)
