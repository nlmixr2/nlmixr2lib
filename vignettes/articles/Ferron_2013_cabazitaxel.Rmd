---
title: "Cabazitaxel (Ferron 2013)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Cabazitaxel (Ferron 2013)}
...
three-compartment population PK model from Ferron, Dai and Semiond (2013).
The pooled analysis included 170 patients with advanced solid tumors from
five Phase I-III studies (2,322 measurable plasma concentrations) who
received 10-30 mg/m^2 cabazitaxel as a 1-h IV infusion every 7 or 21 days.
...
tumor-type indicator (TUMTP_BREAST; multiplicative reduction in CL).
...
| Breast cancer effect on CL | Ferron 2013 Table 4, theta7; Eq. 9 | CL multiplied by (1 - 0.543 * TUMTP_BREAST) |
...
| --- | --- |
| `BSA` (m^2) | `BSA` |
| `TT1` (1 = breast cancer, 0 = other) | `TUMTP_BREAST` (1 = breast cancer, 0 = other) |
...
170-patient cohort. The paper does not publish individual covariate values
or correlations.

```{r virtual-pop}
set.seed(2013)
n_subj <- 120  # downsampled from 200 for vignette build budget; VPC bands stable
...
TUMTP_BREAST <- as.integer(TUMTP == "breast")

pop <- data.frame(
  ID        = seq_len(n_subj),
  BSA, TUMTP, TUMTP_BREAST,
  treatment = ifelse(TUMTP_BREAST == 1, "breast cancer", "non-breast cancer")
)
```

## Dosing dataset

Approved TROPIC-label regimen: 25 mg/m^2 IV over 1 hour every 3 weeks. The
absolute mg dose for each subject is `25 * BSA`. Simulate three cycles
followed by an observation tail to capture the long terminal phase
(t1/2 gamma approximately 95 h).
...
intra_cycle <- sort(unique(c(
  ...
  seq(12, 21 * 24, by = 6)               # to next dose (thinned from by=4 for vignette build)
)))
...
ggplot(sim_summary, aes(x = time)) +
  ...
  labs(
    ...
    subtitle = "Median and 90% prediction interval (N = 120 virtual patients)",
...
```{r breast-effect, fig.width = 7, fig.height = 4}
mod_typical <- mod |> rxode2::zeroRe()

typ_pop <- data.frame(
  ID = c(1L, 2L), BSA = c(1.84, 1.84),
  TUMTP_BREAST = c(0L, 1L),
...
beta 1.6 h, gamma 95 h) for a non-breast-cancer patient at the median BSA.
The corresponding values from the typical-subject ODE coefficients are:
...
non-breast-cancer arm should be close to that value; for the breast-cancer
arm the typical CL is 48.5 * (1 - 0.543) = 22.2 L/h.
...
The Ferron 2013 paper does not publish individual covariate values, so the
virtual population approximates Table 3 medians and ranges:

- **BSA distribution**: log-normal centred on the pooled median 1.84 m^2,
  clipped to the observed range 1.30-2.53 m^2.
- **Tumor-type mix**: sampled independently from the pooled Table 3
  ...
  groups collapse into the reference TUMTP_BREAST = 0.
- **No covariate correlations** are imposed (paper does not report them).
- **Dose regimen** uses the approved TROPIC-label dose of 25 mg/m^2
  q3w (Ferron 2013 Methods and Discussion). The model is also valid over
...
- **Interoccasion variability (IOV)** is reported by the paper (CL 19.4%
  CV per cycle, V1 45.3% CV per cycle; Ferron 2013 Table 4) but is omitted
  from this nlmixr2lib implementation for portability, following the
  convention used by other multi-cycle drug-PK models in the package
...
- **No log of CL/V1 correlation block.** The paper does not report a
  ...
  independent in `ini()`. The block-vs-diagonal choice was selected on
  OFV decrease per the paper's Statistical model section.
- **K21 has no IIV** (paper Table 4 footnote: "all intercompartmental
  rate constants except K21").
- **Online Resources 1-4** of the paper (online supplement, including the
  NONMEM control stream of the base model and several sensitivity-analysis
  plots) were not available when this model was built (the Springer
  supplement download returned HTTP 403). All final-model parameter
  values used here come from Table 4 of the main paper, which contains
  the complete set; the supplements would only confirm the base-model
  control stream and the alternative covariate models, not change any
  final-model number.
...
- **Breast-cancer covariate finding** is reported by the paper as
  ...
  reset TUMTP_BREAST = 0 across the cohort.
...
- **Reference category** for TUMTP_BREAST = 0 is "all other tumor types"
  (prostate, gastrointestinal, other). The paper does not split by these
  individual non-breast types.

## Reference

* `r rxode2::rxode(readModelDb("Ferron_2013_cabazitaxel"))$reference`
