...
* **One arm does not reconstruct.** The lung 20+20(Split) arm implies a
  ...
  error. There is not enough information on disk to distinguish the two.
...
* **The published R^2 cannot be reproduced from Table 5.** Recomputing R^2 from
  ...
  does not state the fitted dataset, and the underlying counts are not on disk.
...
* **Supplementary Materials 1-6 were not on disk** for this extraction. The main
  text contains the complete model equation, all six parameter sets, and the
  index and effect data for every arm, so no packaged value depends on them.
