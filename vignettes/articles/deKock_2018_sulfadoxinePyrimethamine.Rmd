...
make_events <- function(wt, waz = 0, ids = 1L, tablets = who_tablets(wt),
  ...
  obs <- tidyr::expand_grid(id = ids, time = times, cmt = c("Cc", "Cc_pyra")) |>
  ...
  stopifnot(nrow(ev) == 2 * n + 2 * n * length(times))
  ev
}
```
...
sim_raw <- rxode2::rxSolve(mod, ev_adult, returnType = "data.frame")

# The event table carries one observation record per endpoint, so every
# (id, time) appears twice -- and rxode2 evaluates BOTH algebraic outputs on
# each of those rows. Collapse to one row per (id, time), but assert first
# that the duplication really is exact, so this is a de-duplication and not a
# silent choice between two different values.
dup <- sim_raw |>
  group_by(id, time) |>
  summarise(
    n = n(),
    spread_cc = diff(range(Cc)),
    spread_pyra = diff(range(Cc_pyra)),
    .groups = "drop"
  )
stopifnot(
  all(dup$n == 2L),
  max(dup$spread_cc) == 0,
  max(dup$spread_pyra) == 0
)
sim_adult <- distinct(sim_raw, id, time, .keep_all = TRUE)
c(raw_rows = nrow(sim_raw), deduplicated_rows = nrow(sim_adult))
```
