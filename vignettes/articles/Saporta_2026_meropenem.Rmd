...
* **`kdig` is not independently identified.** The Results state that the
  ...
  than carrying a parameter the paper never estimated (operator ruling, sidecar
  `oare_PMC13041408` q2).
...
* **The `drc` sigmoid Emax fit of equation 6 is replaced by direct SSE
  ...
  are checkable.
* **`IMMUNE_STATE` is a newly registered canonical covariate.** Ratified with
  this extraction (sidecar `oare_PMC13041408` q3); see
...
* **`kphag` was broadened, and `lv_lung` / `lq_elf_lung` newly registered.** The
  ...
  (sidecar `oare_PMC13041408` q1). `lv_lung` and `lq_elf_lung` are well-formed
