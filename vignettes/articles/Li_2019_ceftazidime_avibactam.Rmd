...
analysis. Following the `replicate-author-structure` policy the extraction is
therefore two model files sharing this one vignette.
...
N_ARM <- 200L # per indication; the skill caps this at 200
...
# robust quantile, never on the extreme -- see the repository CLAUDE.md note on
...
6. **Two model files, one vignette.** The two analytes were fitted separately,
   ...
   extracted as two files per `replicate-author-structure`, and the joint
   PK/PD analysis that couples them lives here in the vignette, which is where
   Li 2019 puts it too.
