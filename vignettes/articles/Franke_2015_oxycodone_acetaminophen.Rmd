...
That is four independent final models. Three of them are packaged here; the
fourth is not, for the reason given under *Errata and known deviations*.
...
**The acetaminophen food-effect model is not packaged.** Franke 2015 states
(Table 5 footnote a) that a *two-compartment* additive-and-proportional error
model was used for acetaminophen in the fed-vs-fasted analysis, and Table 5
reports its `CL/F` (22.9 L/h), `V2/F` (140 L), `Ka` (3.17 1/h), sex factors on
`CL/F` (0.278) and `V2/F` (0.295), and meal factors on `Ka` (0.802 low fat,
0.825 high fat). It does **not** report `Q` or `V3/F` -- neither in the base
model column nor in the final model column, and neither appears in the printed
equations. A two-compartment model cannot be simulated without them, and the
values cannot be borrowed from the pooled fasted acetaminophen model because
that fit's central volume (58.9 L) differs from this one's (140 L) by a factor
of 2.4, so its disposition parameters are not transferable. The model is
therefore left out rather than completed with invented or transplanted numbers.
...
model column. `Franke_2015_oxycodone_food` therefore declares
`addSd <- fixed(0)` and `propSd <- fixed(0)` rather than carrying over the
unrelated 151-subject pooled-fasted estimates. Simulations from that model are
IPRED-only; the interindividual variability, which *is* reported, is intact.
...
* Sex, age and BMI were screened by the authors but retained in none of the
  three packaged models; they are recorded in each model's
  `covariatesDataExcluded` metadata rather than in `covariateData`, and are not
  sampled in the virtual cohorts.
* All parameter values come from the paper's own tables and printed equations.
