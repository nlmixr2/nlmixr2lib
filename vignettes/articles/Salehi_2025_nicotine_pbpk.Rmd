...
code requires. This is a reporting gap that no further source can close, so the
...
baseline saliva volume is reported in any available source. For the pouches and for
...
- **No between-subject variability and no residual-error model** are reported
  in any available source, so none is encoded. The model is deterministic
