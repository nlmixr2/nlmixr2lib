---
title: "Ciclosporin (Frobel 2013)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Ciclosporin (Frobel 2013)}
...
* Citation: `r readModelDb("Frobel_2013_ciclosporin")()$reference`
* Description: `r readModelDb("Frobel_2013_ciclosporin")()$description`
* Article: <https://doi.org/10.1111/bcp.12121> (Br J Clin Pharmacol 2013;76(4):603-615)
...
at the Children's Hospital in Helsinki, Finland (1995-2006); 2 of the
89 eligible patients were excluded for incomplete records. The cohort
was ethnically homogeneous (Caucasian only) and treated under a single
...
entry in `inst/modeldb/specificDrugs/Frobel_2013_ciclosporin.R`. The
...
```{r figure-1, fig.cap = "Replicates Figure 1 of Frobel 2013 (hazard step-function, 0-200 days)."}
...
tendency of the Kaplan-Meier curve (the model has no IIV; per-subject
KM trajectories aren't simulated).

```{r figure-2-top, fig.cap = "Replicates Figure 2 top panel of Frobel 2013 (KM survival, 0-30 days)."}
...
```{r figure-2-bottom, fig.cap = "Replicates Figure 2 bottom panel of Frobel 2013 (KM survival, 0-2920 days)."}
...
across covariate values (longest t_90 around 7-8 days for short
dialysis time / female sex / high baseline weight), as expected for a
base model whose covariate effects fall inside the joint clinical-
relevance bands.
...
* **No covariates in the final model.** Frobel 2013 screened 15
  ...
  covariate modelling (SCM), 10-fold cross-validated SCM (Figure 3),
  and bootstrap-SCM (Figure 4). Three covariates were selected in the
...
* **No PK structure.** The paper's ciclosporin A AUC, daily dose, and
  ...
  covariate screen but were not retained. The packaged TTE model has
  no dosing events and no concentration output; the upstream PK is
  not redistributed here.

* **No IIV / no random effects.** The paper states explicitly: "as
  only one observation was available per individual, random effects
  on the baseline hazard could not be estimated, i.e. the same
  baseline hazard was assumed for all subjects" (Methods,
  "Development of the base model"). The same typical-value hazard is
  used for every simulated subject.

* **No residual error.** The NONMEM run uses `$ESTIMATION METHOD=0
  LIKE`: the likelihood is the survival/event density itself, not an
  observation-error model. The packaged model is intended for forward
  simulation of `hazard`, `cumhaz`, and `sur`.
...
* **`units$concentration` is non-PK.** The TTE output `sur` is a
  ...
  `"probability (the model output `sur` is a survival probability,
  not a drug concentration)"`. The same convention is used by other
  TTE / survival models in the package (e.g.,
  `Zecchin_2016_survival.R`, `NA_NA_tte_gompertz.R`).

* **PKNCA not applicable.** Time-to-event survival models do not
  produce dose-response NCA parameters (Cmax, Tmax, AUC, half-life);
  the validation here is the Figure 1 (hazard) and Figure 2 (Kaplan-
  Meier survival) replication, plus the `t_90` clinical-relevance
  cross-check against Figure 5.

* **Step-function discontinuity handling.** The hazard equation is
  encoded as nested `ifelse()` on time. `rxSolve` integrates the ODE
  with adaptive step (LSODA); the cumulative hazard at the break
  points (5, 8, 25, 100 days) matches the closed-form hand
  calculation to within 1e-4 (see the Figure 2 cross-check). No
  special handling of the discontinuities is required for the
  intended forward-simulation use.
