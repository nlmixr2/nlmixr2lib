...
protein-unbound (Cc) and protein-bound (Cb) phenytoin plasma concentrations
...
PKNCA is run separately on Cc and Cb from the single-dose oral simulation.
...
fraction 0.12 +- 0.03. The simulated single-dose Cc peak above is below the
...
- **Within-pair residual-error correlation collapsed.** The published model
  ...
  a shared common-proportional error EPS3 carried across both Cc and Cb of
  the same plasma sample). The packaged nlmixr2 model collapses these into
  a single proportional SD per output: `propSd_Cc = sqrt(0.136^2 +
  0.174^2)` and `propSd_Cb = sqrt(0.103^2 + 0.174^2)`. The marginal
  proportional residual SD per output is preserved; the within-pair
  correlation between simultaneously-sampled Cc and Cb residuals induced
