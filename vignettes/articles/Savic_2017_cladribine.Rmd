---
title: "Savic_2017_cladribine"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Savic_2017_cladribine}
...
* Citation: `r rxode2::rxode(readModelDb("Savic_2017_cladribine"))$reference`
* Description: `r rxode2::rxode(readModelDb("Savic_2017_cladribine"))$description`
...
* Study 25803 (phase I, n = 16): randomized two-period crossover; 3 mg IV
  infusion over 1 h or 10 mg single oral tablet, with parent cladribine (CdA)
  and metabolite 2-chloroadenine (CAde) plasma and urine sampling.
* Study 26127 (phase I, n = 16): food-effect crossover, single 10 mg oral
  tablet in fed vs fasted sequence.
* Study 26486 (phase I, n = 16): cladribine-and-interferon-beta-1a drug-drug
  interaction study; oral cladribine 10 mg daily for 5 consecutive days
  with subcutaneous IFN beta-1a 8.8/22/44 ug three times weekly.
...
Pooled demographics (source paper Table 3): N = 173, age range
19-65 years (median 40), body weight 48.5-116.1 kg (median 69.2),
65.9 % female, Cockcroft-Gault creatinine clearance 49.6-244.4 mL/min
(median 107.9). The original dataset contained 4790 concentration-time
records; 45 % were excluded (96.5 % of those for BLQ), leaving 2619
plasma and urine records for the population analysis.
...
`inst/modeldb/specificDrugs/Savic_2017_cladribine.R`. Consolidated here for
review:
...
Savic 2017 Figure 2c shows the VPC for a single oral 10 mg dose in fasted
state. We reproduce the typical-value (median) cladribine plasma
...
cladribine clearance of 19 %, 30 %, and 40 % for CLCR = 65, 40, and
20 mL/min, respectively. Reproduce by simulating a 10 mg oral fasted
dose across a CLCR grid.
...
* Renal-impairment scaling: total CL decreases by ~30 % at CLCR =
  40 mL/min (paper Discussion: 30 %). See the renal-impairment table
  above.

# Assumptions and deviations

Several simplifications were made when packaging the Savic 2017 final
model. The full source-paper structure can be recovered by extending
the packaged model where needed.
...
* **Parent-only model.** Savic 2017 jointly fits cladribine and its
  ...
  *corrected for the fraction metabolized* (footnote f), and F_m is
  not separately identifiable in the source data. Without F_m the CAde
...
* **Transit-compartment fed-state absorption simplified to a lag time.**
  Savic 2017 best fit the fed-state absorption with a transit-compartment
  chain (mean transit time MTT = 0.910 h, n = 2.24 compartments). The
  packaged model approximates this with a fed-state-only lag time
  alag(depot) = MTT * FED. The approximation preserves the absorption-
  delay magnitude but loses the gamma-distributed input-rate shape; the
  source paper notes the transit model improved fit over a pure lag-time
  approach, so for downstream simulation needing the precise absorption
  profile users should override with `transit(2.24, 0.910)` gated on
  FED.
* **Phase III-specific 0.319 h lag time omitted.** Table 4 carries a
  ...
  data. This is a study-specific artefact (different sampling /
  dosing logistics in the larger phase III) rather than a
  generalizable patient-level effect; it is not reproduced here.
* **Single proportional residual error magnitude.** Table 4 reports
  four residual-error magnitudes by route/study (IV 20.0 %, oral
  studies 25803 34.7 %, 26127 + 26486 22.8 %, 25643 35.3 %) plus a
  separate BSV of 0.159 on the residual error. The packaged model
  uses the phase III value (35.3 %) as the largest cohort and most
  conservative simulation choice, and omits the BSV-on-residual term.
* **Single shared eta on distribution parameters.** Table 4 reports a
  ...
  four. The packaged model implements this via one `etalq` random
  effect added to each of `lq`, `lq2`, `lvp`, and `lvp2`. Users who
  prefer four diagonal etas (e.g. for sensitivity analysis) can swap
  the shared eta for `etalq ~ 0.0365`, `etalq2 ~ 0.0365`,
  `etalvp ~ 0.0365`, `etalvp2 ~ 0.0365` and re-load the model.
* **Logit-scale BSV on bioavailability.** Savic 2017 Table 4 footnote
  d notes "Variance on a logit scale" for the BSV of F. The packaged
  model implements this by adding `etalogitf` inside the logit
  transform, then applying the inverse logit to keep F bounded in
  (0, 1). The variance 0.223 is on the logit scale, not the linear
  F scale.
...
* **CRCL canonical column.** The source paper's CLCR (raw
  ...
  (CRCL accepts raw Cockcroft-Gault mL/min when the paper does not
  apply BSA normalization). The model uses CRCL directly with
  reference value 105.2 mL/min (the paper's typical patient CLCR =
  6.31 L/h).
* **CONMED_IFNB1A canonical covariate.** Adding this model also
  introduces the new canonical column CONMED_IFNB1A
  (concomitant subcutaneous IFN beta-1a coadministration; binary,
  general scope, reference 0 = no concomitant IFN). The register
  entry is committed alongside this model in
