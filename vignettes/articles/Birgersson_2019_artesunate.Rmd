...
| `etalcl_dha ~ 0.00809` | 0.00809 | `Executable_run1.mod` `$OMEGA(3,3)` (3.CLM_) |
...
Single-dose, dense-sampling NCA per the recipe in `references/pknca-recipes.md`. The treatment grouping variable carries the pregnancy strata so the per-arm Cmax / Tmax / AUC can be summarized side-by-side. Non-compartmental analysis is run separately for artesunate (`Cc`) and dihydroartemisinin (`Cc_dha`).
...
* **No IIV on artesunate or DHA volume of distribution.** The source `.mod` declares `$OMEGA(2,2) 0 FIX` and `$OMEGA(4,4) 0 FIX` (IIV on V2 and V3 fixed at zero). Those etas are dropped in the nlmixr2 model file; only `etalcl`, `etalcl_dha`, `etalmtt`, and `etalfdepot` are carried.
