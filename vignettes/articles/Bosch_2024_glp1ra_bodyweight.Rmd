---
title: "GLP-1RA body composition (Bosch 2024)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{GLP-1RA body composition (Bosch 2024)}
...
* Citation: `r rxode2::rxode2(readModelDb("Bosch_2024_glp1ra_bodyweight"))$reference`
* Description: `r rxode2::rxode2(readModelDb("Bosch_2024_glp1ra_bodyweight"))$description`
* Article: <https://doi.org/10.1002/psp4.13183>
* Supplement S10 (full mrgsolve model code): <https://doi.org/10.1002/psp4.13183>

Bosch 2024 extends the Hall body composition model (Hall 2010) with three
effects layered on energy intake (EI) and physical activity energy
expenditure (PAE):
...
Liraglutide and semaglutide PK are encoded inline as fixed one-compartment
first-order absorption models reproduced from supplement S10 (FDA clinical
pharmacology review, 17 Dec 2018, for liraglutide; Carlsson Petri et al.
2018 for semaglutide). Both PK paths share the body composition machinery
and contribute additively to the total in-vitro-EC50-normalised free
concentration that drives the GLP-1R effect, so a user simulating one
drug doses only that drug's depot compartment.
...
metadata is available via `rxode2::rxode2(readModelDb("Bosch_2024_glp1ra_bodyweight"))$population`.
...
below collects the principal model equations and parameters in one place.
...
models. The Bosch 2024 model is calibrated so the energy intake at baseline
(`ei_init = RMR_init * PAL = 1.6 * RMR`) equals total energy expenditure at
PAL = 1.6.
...
- Mean-study data analysis. Bosch 2024 fit study-mean trajectories, not
  ...
  `rxode2::zeroRe()` to silence the nominal `propSd` residual error.
...
- Kred exposed as a single `ini()` parameter. Bosch 2024 estimated three
  Kred values (non-STEP drug studies = 0.00195 / d; STEP 5 / 8 =
  0.00541 / d; STEP 3 = 0.00924 / d); the model exposes a single
  typical Kred (the non-STEP value, 0.00195 / d) and the user can
  override per simulation if reproducing STEP-specific trajectories.
  The simulation chunk above overrides Kred to the STEP 5 / 8 value
  via `rxode2::ini(lkred = log(0.00541))` before solving. The typical
  Bosch 2024 result range is -15 % to -17 % BW at week 20 for the STEP
  5 / 8 / 3 semaglutide arms.
