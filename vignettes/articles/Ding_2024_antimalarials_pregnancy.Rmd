...
goodness-of-fit plots and prediction-corrected VPCs) was not available when
this model was built. Every value used below comes from the main article text,
...
# and return a badly inflated half-life. The relative floor drops points below
# 1e-6 of the peak, where the integrator has no relative accuracy left.
terminal_summary <- function(time, conc, tail_from, tail_to = Inf) {
  keep <- time >= tail_from & time <= tail_to & conc >= 1e-6 * max(conc)
...
# variable so PKNCA rolls its summaries up per analyte. The filters drop only
# missing values and the integrator noise floor, so the time-zero record is
# retained.
#
...
conc_all <- dplyr::bind_rows(
  ...
  # Per subject and analyte, keep Cc >= 1e-6 * Cmax after the peak: below that the ODE integrator has no relative accuracy left.
...
11. **Supplementary figures not used.** Figures S1-S6 (structure diagrams,
    goodness-of-fit plots, prediction-corrected VPCs) were not available. They
