...
`lambda_z` is fitted over 28-40 h post-dose rather than over PKNCA's
automatically chosen window. For tazobactam the distribution rate constant
(`alpha = 0.43 /h`) is only about 2.4-fold faster than `beta = 0.18 /h`, so a
window that starts too early still carries distribution-phase curvature and
`lambda_z` reads steep: PKNCA's automatic selection began at 13.2 h and returned
3.65 h against a closed-form 3.76 h, a 3.0% bias in the *instrument*, not in the
model (see pattern 11 of the skill's known-failure catalogue). Starting at 28 h
removes it.
...
half_life_check <- function(mod, amt, theta1, theta5, theta2, q, vp) {
  ...
  conc <- sim |>
    dplyr::filter(!is.na(Cc), time >= lambda_z_start) |>
    dplyr::select(id, time, Cc, treatment)
  dose <- ev |>
    dplyr::filter(evid == 1) |>
    dplyr::select(id, time, amt, treatment)
