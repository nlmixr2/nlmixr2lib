...
* **Two convention warnings on the SVR files are deliberate.**
  `checkModelConventions()` reports, for each SVR model, that the single-output
  observation variable `svr` "is not canonical" and that the parameter
  `rx.svr.binom` has no label. Both are accepted rather than worked around.
  `svr` is the paper's own endpoint name and would need registering as a new
  PD-output canonical in `R/conventions.R`; renaming it to `Cc` -- the
  checker's suggestion for concentration outputs -- would be actively
  misleading for a probability. `rx.svr.binom` is generated by rxode2 from the
  `svr ~ dbinom(1, psvr)` endpoint: it is the binomial size, already fixed at
  1 (making the endpoint Bernoulli), and it does not exist in the file's
  `ini()` block, so no `label()` can be attached to it.
