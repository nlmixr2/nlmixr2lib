...
* **The `WAZ` (weight-for-age z-score) functional form is an extraction-side
  ...
  is flagged for maintainer confirmation and does not affect any validation
  above, all of which are run at `WAZ >= -3` where the effect is inert.**
...
* **No between-subject variability on volumes or intercompartmental
  ...
  to both `cl` and `cl_form_desacetylrbn`. (In the extracted text of
