...
elimination pattern. We pass both as time-varying covariates `Cipm`
and `Ctob` on the event table; the rxode2 simulator interpolates
between rows.
...
- **Tobramycin profile within model() is an external covariate.** The
  ...
  can pass arbitrary `Ctob` values in the event table.
- **Imipenem profile within model() is a constant covariate.** The
  ...
  represents `Cipm` as a covariate the user holds constant for the
  duration; users simulating non-constant imipenem PK can pass
  time-varying `Cipm` values in the event table.
...
- **Non-canonical compartment and covariate names.** The bacterial
  ...
  `s2_ir`) and in-vitro drug-input covariates (`Cipm`, `Ctob`) are
