...
this model was built. The model structure encoded here is the
...
fast doubling S2 -> 2*S1 at a fixed rate of 50 /h (file parameter
`lk21`, log-transformed) shared across subpops.
...
**Synergy switch lands at 1.15 mg/L tobramycin.** Compare 24 h log_cfu at
...
- **Supplement not available.** The paper's supplemental PDF (Fig. S2,
  ...
  built. The exact ODE form of the LCGM was therefore inferred
  ...
  3. **Whether the three subpopulations interact via mutation flux
     ...
     low-rate mutation transition; without it, this extension
...
- **Non-canonical compartment and covariate names.** The bacterial
  ...
  systemic popPK / PK-PD. `checkModelConventions()` emits warnings for
  the covariates; they are expected and documented here.
