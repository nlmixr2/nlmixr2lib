---
title: "Sunitinib (Hansson 2013a)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Sunitinib (Hansson 2013a)}
...
parameter values, equations, and the model structure are taken from the DDMORE bundle
...
check plus a self-consistency simulation against the bundle's
shipped simulated dataset; a side-by-side comparison against any
...
not available when this model was built; the model's `population` metadata
...
* **Publication PDF not available.** The Hansson 2013 paper text
  (CPT Pharmacometrics Syst Pharmacol 2013;2:e84,
  doi:10.1038/psp.2013.61) was not available when this model was built. All
...
* **Upstream popPK not packaged.** The drug-exposure summary
  `AUC = DOSE / CLI` consumes `CLI` (individual posthoc clearance)
...
* **Biomarker compartment naming.** The four biomarker states are
  ...
  (`tumorVol`, `cycling_cells`) — paper-named mechanistic states are
...
* **Units `mg` × `pg/mL`.** `units$dosing = "mg"` and
  ...
  PD model has no PK ODE — `DOSE` is consumed only inside
  `auc <- DOSE / CLI`, and the four biomarker states use their own
