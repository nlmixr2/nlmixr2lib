...
one experimental covariate (`HUMAN_SERUM_PCT`, the percentage of human serum) and no
...
below builds an `et()` event table for an arbitrary (DAP, HUMAN_SERUM_PCT)
combination. DAP is inserted as a bolus event into the `dap` compartment
with `amt` interpreted as the initial bath concentration in mg/L; HUMAN_SERUM_PCT is
...
across HUMAN_SERUM_PCT levels because human serum was not shown to affect bacterial
...
daptomycin level (4 mg/L), increasing HUMAN_SERUM_PCT reduces DAP_EF in proportion
to factive(HUMAN_SERUM_PCT). Effective concentrations are:
...
0% human serum), the residual CFU_total is dominated by `r1` + `r2`:
...
is reached by 24 h for all DAP >= 2 mg/L irrespective of HUMAN_SERUM_PCT (Results,
...
fit (Eq 1B, log-ratio area vs daptomycin C / MIC) for USA300 at each HUMAN_SERUM_PCT
...
higher EC50 with increasing HUMAN_SERUM_PCT. The Hill-fit numbers themselves are not
...
- **HUMAN_SERUM_PCT is an in-vitro experimental condition.** `HUMAN_SERUM_PCT`
  is the percent v/v of human serum supplementing the broth -- an
  in-vitro experimental condition rather than a clinical-trial patient
  covariate. It is registered as a canonical covariate under "Preclinical
  experimental conditions" in `inst/references/covariate-columns.md`
  (the register treats data-driven model inputs as covariates,
  including in-vitro experimental conditions). The column was renamed
  from the original short `HS` to the spelled-out canonical
  `HUMAN_SERUM_PCT` on 2026-05-27 to align with the register's
  naming standards.
- **HUMAN_SERUM_PCT treated as a discrete five-level factor, not a continuous
  variable.** factive(HUMAN_SERUM_PCT) is encoded as five fixed estimates -- one per
  experimental HUMAN_SERUM_PCT level (0%, 10%, 30%, 50%, 70%). HUMAN_SERUM_PCT values outside
...
- **Bacterial counts on linear scale internally.** Table 2 reports
  ...
  high-DAP + low-HUMAN_SERUM_PCT regimens.
...
- **`factive(0%)` was not estimated.** The paper estimates factive at
  ...
  `factive = 1` when `HUMAN_SERUM_PCT = 0`; the value is not a `ini()` parameter and
