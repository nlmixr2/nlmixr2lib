...
* **`eta4` (inter-occasion variability) is not encoded.** Wu 2025 Table 2 reports
  ...
  `eta2` and `eta3` only. No supplement or control stream was available when
  this model was built, and the supplement's documented contents -- a
  study-design table and mean concentration-time figures -- would not place an
  eta. Assigning the term to CL, D1 or F would be inventing model
...
* **CFR is not reproduced.** Wu 2025 Table 3 also reports cumulative fraction of
  ...
  pathogen. Those distributions are external to the paper and were not available
  when this model was built, so only the PTA and cut-off columns are validated
  here.
