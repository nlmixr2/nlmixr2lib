...
The van Wijk 2019 publication PDF was not on disk under the literature tree,
...
2. The bundle does **not** ship an `Output_simulated_*.lst` companion run on
   a simulated dataset, so the F.2 self-consistency check substitutes the
...
3. The model is a continuous-environmental-exposure popPK (a regime where
   ...
   F.3 mechanistic-sanity recipe is used instead of PKNCA.
...
* **Mechanistic sanity (F.3)** - simulate the typical-value trajectory at
...
Not performed - the van Wijk 2019 PDF is not on disk under the literature
tree, and the model's continuous-environmental-exposure dosing regime does
...
- **Publication PDF not on disk.** The article
...
- **No `Output_simulated_*.lst` companion run.** The bundle does not ship
  a NONMEM listing on a simulated dataset, so the F.2 self-consistency
