...
* **Delivery times are assumed, not published.** Individual labour durations are
  ...
  infusion.
* **Figure 2 envelope is maintainer-digitised.** The observed ranges (maternal
  ~3-30 pg/mL, cord ~0.6-10 pg/mL) were read off the log axis of Figure 2 by the
  maintainers. They are used only as a validation envelope; **no model parameter
  is derived from them.** All `ini()` values come from Table 1.
