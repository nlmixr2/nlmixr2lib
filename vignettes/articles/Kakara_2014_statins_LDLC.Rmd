---
title: "LDL-cholesterol lowering by statins and co-medications (Kakara 2014)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{LDL-cholesterol lowering by statins and co-medications (Kakara 2014)}
...
* Citation: `r rxode2::rxode(readModelDb("Kakara_2014_atorvastatin"))$reference`
* Description: `r rxode2::rxode(readModelDb("Kakara_2014_atorvastatin"))$description`
* Article: <https://doi.org/10.1111/bcp.12405>

Kakara 2014 developed a single joint indirect-response Imax model for the
LDL-C lowering effects of three statins (atorvastatin, pitavastatin,
...
Kin (32.8 mg/dL/day), one shared Imax (0.567), and a statin-specific ID50
for each drug, plus an additive 10.9% contribution to the synthesis-rate
inhibition fraction when ezetimibe is coadministered.

This nlmixr2lib extraction ships the paper as three sibling model files
(`Kakara_2014_atorvastatin`, `Kakara_2014_pitavastatin`,
`Kakara_2014_rosuvastatin`) so each is self-contained for simulation use.
All three share the structural form and IIV magnitudes; only the ID50
value (and the population-table demographics) differ across files.
...
The full study enrolled 378 patients contributing 2863 LDL-C observations
between November 2009 and October 2011 at a single Japanese centre. The
median dosing period was 362 days for atorvastatin, 437 days for
pitavastatin, and 304 days for rosuvastatin. Only patients with baseline
LDL-C above 60 mg/dL and no prior statin use were enrolled. Ezetimibe was
the only co-medication retained as a covariate in the final model; it was
coadministered only within the rosuvastatin cohort in this dataset.
...
with an age-scaled baseline and an additive ezetimibe contribution to the
synthesis-rate inhibition fraction (Kakara 2014 Methods, "Population
...
log-normal identity `omega^2 = log(1 + CV^2)`. The ID50 IIV is one shared
variance across all three statins in the joint fit; each per-statin
model file therefore carries the same `0.26611` on `etalid50`.
...
Replicating the time course in Kakara 2014 Figure 2 (LDL-C profiles after
statin initiation): the simulated median falls from the age-adjusted
baseline (`~`151 mg/dL atorvastatin, `~`145 mg/dL pitavastatin,
`~`157 mg/dL rosuvastatin) to a new steady state within roughly 4 to 5
weeks.

```{r figure-2-replication}
sim_med <- sim_all |>
...
combination achieves a higher target-attainment rate than rosuvastatin
5 mg monotherapy at every baseline level, matching the paper's central
...
- Kakara 2014 reports the residual variability as `sigma^2 (%) = 15.2`
  ...
  error scale) rather than as a variance `0.152` that would imply a
  39 % CV. The bootstrap 95% confidence interval reported in Table 2
...
- The Kakara 2014 dataset contained no atorvastatin or pitavastatin
  ...
  model files retain the same coefficient so simulations of
  hypothetical atorvastatin + ezetimibe or pitavastatin + ezetimibe
  combinations are supported, but the value is an extrapolation outside
  those cohorts' observed data.
- The simulated cohorts in this vignette use synthetic AGE draws from a
  ...
  the paper's NONMEM Table 3 simulation used a re-sample of the
  cohort's empirical AGE distribution. Median dose-response endpoints
  are robust to this simplification; the 95% prediction interval widths
  are not directly comparable.
- The model has no PK compartment. The user must supply DOSE as a
  time-varying covariate column (mg/day), set to 0 during drug
  holidays. The simulation event tables in this vignette use a single
  constant DOSE level across the observation window because the paper's
...
- The ID50 IIV is one shared variance across all three statins in the
  ...
  files means each independently draws on the same omega; simulating
  the same virtual subject across two model files would NOT preserve
  the per-subject correlation that the joint fit imposed. For the
  paper's reported validations (each statin simulated separately) the
  split-file form is equivalent.
