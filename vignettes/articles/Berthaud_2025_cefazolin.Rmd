...
arm contributing only while a session runs. When rxode2's ODE-to-`linCmt()`
conversion is enabled (`useLinCmt = TRUE`; the default until rxode2 made it
opt-in in September 2026, issue 1389) it recognises the joint presence of `cl`
and `vc` and solves the one-compartment system analytically from that pair,
**discarding the explicit `d/dt()`**. A model that assigns only the
...
packaged model avoids this by assigning the gated sum to `cl` itself, which is
correct under either solve path.
