---
title: "Tislelizumab (Budha 2023)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tislelizumab (Budha 2023)}
...
Simulate tislelizumab (anti-PD-1 IgG4) concentration-time profiles using the
final three-compartment population PK model from Budha et al. (2023). The
pooled analysis included 2596 patients with advanced tumors from 12 studies
(14,473 serum concentration observations).

The structural model is a three-compartment IV model with linear elimination.
Final covariates: body weight, albumin, baseline tumor size, ADA status, and
tumor-type indicators for classical Hodgkin lymphoma and gastric cancer on
CL; and body weight, age, and sex on Vc. Tislelizumab PK were linear across
the 0.5–10 mg/kg dose range, and the approved dose is 200 mg IV every 3
weeks.
...
Approximate the Budha 2023 Table 2 distributions for the pooled 2596-patient
cohort (medians and ranges only — the paper does not publish individual
covariates or covariate correlations).
...
Run PKNCA on the first dosing interval (day 0–21) and a pseudo-steady-state
interval (cycle 10, days 189–210), stratified by ADA status. Terminal
half-life is expected to approximate the paper's reported ≈ 23.8 days
...
The paper reports Vss = 6.42 L (Budha 2023 Results). With the nlmixr2 model,
Vss is simply the sum of the central and peripheral volumes at typical
covariates:
...
- **Tumor size**: log-normal, median 63.3 mm (matched to Budha 2023 Table 2
  median 63.3 mm), clipped to 10–408 mm.
...
- **Tumor-type mix**: sampled independently from Table 2 proportions; only
  cHL (2.7%) and GC (3.9%) get indicator columns — all other types are
  reference.
...
- **Strongest covariates on CL** (Budha 2023 Figures 6–7, not clinically
  meaningful): ±20% range of WT (0.565 exponent) and ALB (-0.457 exponent).
- **cHL patients** have ~19% lower CL (`exp(-0.216) = 0.806`) than the
  reference tumor mix.
- **Shrinkage on V2 and V3 IIV** is high (55.8%, 44.4%). The paper retains
  these etas; this implementation follows the paper.

## Reference

* `r rxode2::rxode(readModelDb("Budha_2023_tislelizumab"))$reference`
