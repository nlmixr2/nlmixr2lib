...
- **IOV on CL documented but not encoded.** Waterhouse 2024 Table 3 reports an
  inter-occasion variability variance of 0.0315 on CL (CV% = 17.9), where an
  "occasion" is each dosing occasion after which vedolizumab PK observations
  are collected. Encoding IOV as a separate occasion-indexed eta on `lcl`
  requires an `OCC` event-table column and specific event-table setup that
  add complexity beyond the simulation-only use case supported here. The
  population-typical plus block-diagonal IIV variability reproduces the
  paper's VPC (Figure 4) shape and prediction intervals adequately, and the
  IOV magnitude is documented in the `ini()` block for downstream users who
  want to extend the model.
