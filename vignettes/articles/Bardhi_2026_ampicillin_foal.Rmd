...
"did not achieve 90%" is required to stay under 95%. Two things justify the band.
...
claim <- function(text, value, achieved) {
  data.frame(
    Claim = text,
    `PTA (%)` = value,
...
knitr::kable(
  ...
  caption = paste(
    ...
    "Pass allows a 5 percentage-point band around the 90% threshold."
  )
)
stopifnot(all(fig3$Pass))
```
...
stopifnot(nrow(age_dir) == 12L * length(MIC_GRID), all(age_dir$young >= age_dir$old))
cat(sprintf(
