...
* **Drug field corrected.** The bibliographic metadata this model was
  ...
  name) -- a metadata error also seen for several other papers. The actual
...
* **Species and model class.** The model was fit to an in-vitro
  ...
  macrophage cell line)." per the package's species-recording
  requirement.
...
* **Baseline values are figure-derived.** Xiang 2018 reports the
  ...
  for NO) are maintainer digitisations of those figure points; users
...
* **Dosing/concentration unit mismatch (lint warning).**
  ...
  documented here per the package's convention for justified deviations.
