...
capture $AUC_{0-\infty}$. At the higher doses the profile still falls below
$10^{-6}$ of its peak, where the solved concentrations are numerical noise, so
those points are dropped before the half-life fit.
...
* **The upstream citation is corrected against Crossref.** Straube's reference
  [5] is "P. M. Glassman and V. R. Muzykantov, *Target-Mediated Exposure
  Enhancement: A Previously Unexplored Limit of TMDD*, Journal of
  Pharmacokinetics and Pharmacodynamics 47 (2020): 573-591". The title and
  authors are right but the pagination is a misprint: Crossref and EuropePMC
  both give 47(5):411-420 for that title (doi:10.1007/s10928-020-09693-1,
  PMID 32488574, PMCID PMC7529850), and 573-591 is the pagination of the
  *Gibiansky* 2008 TMDD-approximation paper in volume 35 of the same journal.
  The `reference` field of the four caplacizumab and linagliptin model files
  therefore cites the Crossref pagination and DOI rather than the pagination as
  printed in the source. This is a bibliographic correction only -- no
  parameter, equation or dose is affected.
