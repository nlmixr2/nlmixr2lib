...
relied on. The per-study re-verification notes live in
...
1. **Secondary-source transcription.** Every parameter comes from the review's
   ...
   field names both. Re-verification notes per study are in
   `refharvest/vanderZeeuw_2026_PMC13272613_cited_primaries.jsonl`.
...
16. **Lee 2024 is not yet extracted.** Its six-level disease-type covariate on
    Vp requires minting new `DIS_<disease>` canonical columns, which is not an
    auto-approved covariate family. All of its parameters are tabulated, so it
