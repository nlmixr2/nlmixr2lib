...
3. **Serum albumin rises 18.9% from `t = 0`, against the paper's "roughly
   ...
   sampling times are not reported, this cannot be settled from the sources that
   were available when this model was built. **No parameter was adjusted to close
   the gap.**
...
5. **IgG molecular weight is back-solved, not printed.** 150000 g/mol is
   ...
   verifiable derivation from published values, not a class-typical substitution.
...
7. **The Figure 5 confidence band is not reproduced.** The paper bootstrapped
   ...
   2018 was not available when this model was built, so only the point-estimate
   curve is drawn.
...
8. **The comparator popPK model is a secondary transcription.** Its parameters
   ...
   omits that paper's covariate model, IIV and residual error. Rebuild from
   Baverel 2018 directly when that paper becomes available. The other nine
   checkpoint inhibitors in Table S3 are deliberately *not* packaged as model
   files.
