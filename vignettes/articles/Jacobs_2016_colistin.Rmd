...
holds DIAL = 0 throughout (no HD session) so the simulated profile matches
...
the same DIAL-gated HD behaviour at the single-dose level: imposing a 4-h HD
...
* **Hemodialysis-clearance values fixed, not estimated.** The paper used
  ...
  in `ini()`) and gated on/off by the `DIAL` covariate. Users who want to
