---
title: "Valitalo_2017_ketorolac"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Valitalo_2017_ketorolac}
...
(half the racemic dose) targeting `central_r` and `central_s`
respectively.
...
(`Cc_r`) and S-ketorolac (`Cc_s`); the cohort label
...
* **Single eta shared across V1, V2, V3 of one enantiomer.** Valitalo
  ...
  encode this as `etalvc_r` and `etalvc_s`, each added once to all
...
* **No depot compartment; user splits the racemic dose 50/50.** The
  ...
  exposes two parallel central compartments (`central_r`,
  `central_s`) without a shared depot. Users of this model must
  supply two simultaneous IV-bolus dose records per administration --
  one with `cmt = central_r` and amt = `0.5 * total racemic mass`,
  and one with `cmt = central_s` at the same amount and time -- as
