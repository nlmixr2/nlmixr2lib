...
selfchk |>
  ...
  knitr::kable(digits = 3, caption = "Median simulated NCA against the closed form.")

# Both sides use the same drawn parameters, so the difference here is pure
# numerical error (trapezoidal AUC on a 15-minute grid, log-linear lambda-z)
# and a tight bound is correct. It is not a cohort-extreme assertion.
stopifnot(
  max(abs(selfchk$`AUC % diff`)) < 5,
  max(abs(selfchk$`t1/2 % diff`)) < 5
)
```
