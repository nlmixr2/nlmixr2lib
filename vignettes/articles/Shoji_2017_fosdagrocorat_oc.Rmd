...
The osteocalcin (OC) K-PD fit used the same 321-patient phase II cohort as
the sister P1NP analysis (NCT01393639); see
[`Shoji_2017_fosdagrocorat_p1np`](Shoji_2017_fosdagrocorat_p1np.html) for the
full demographic summary. For the OC fit Shoji 2017 fixed KDE to the
P1NP-derived estimates (KDE-only re-estimation was unstable for OC data)
and fixed `Imax` to 1 for both drugs (the unconstrained OC estimate was
close to 1).
...
* Placebo: OC = BL + SLP * 8 = 22.2 + 0.0675 * 8 = `r 22.2 + 0.0675 * 8` ng/mL.
...
- **Observation variable naming.** The single-output observation is named
  `OC` (the paper's name) rather than the canonical `Cc`. Same accepted
...
- **Drug arm switching via reparameterization.** Same convention as the
  ...
  `DRUG_PRED = 1`). For OC the typical-value KDE is fixed to the P1NP
...
- **Imax fixed to 1 for both drugs.** The OC fit set `Imax = 1` for both
  ...
  IR; the typical OC %CFB at week 8 for the 15 mg fosdagrocorat arm
  (-16.8% published; ~ -17% simulated) reflects this nearly-complete
  inhibition combined with the slow OC turnover (Kd = 0.939 /week, longer
