...
were not transcribed because the text extracted from the paper does not
provide them.
...
`ini()` entry in
`inst/modeldb/specificDrugs/Hansson_2013_sunitinib_svegfr3_os.R`. The table
below collects them in one place; rates reported in 1/week in the paper
are converted to 1/h via `/24/7` to match `units$time = "h"` (the
