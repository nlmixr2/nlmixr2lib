...
* **Equations S3-S6** (Friden pH-partition correction of brain-homogenate
  ...
  are not - they live in the cited Friden et al. reference, which was not
  available when this model was built. The *outputs* of the correction are
  tabulated (Table S2, parameter rows `brain fu corrected`), so the correction is
  an input-curation step whose results the model consumes rather than a step the
  model performs. No constant has been substituted from outside the available
  sources.
...
* **Residual error.** Main-text Eq. 2 carries an additive noise term `eta`, but
  no variance for it is reported in any available source. Per the standing policy
...
* **The Friden pH-partition constants were not available.** See Scope above.
