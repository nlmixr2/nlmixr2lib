...
the validation strategy reduces to **self-consistency** (model
...
`population` metadata
(`readModelDb("NA_NA_sunitinib")$population`).
...
these are treated as the deposited final estimates.
The `.mod`/`.lst` cross-check that the DDMORE-source extraction protocol
...
The DDMORE bundle ships no simulated dataset, so the standard self-consistency
...
* **No linked publication available.** The bundle for DDMODEL00000231 ships
...
* **No `.lst` cross-check on parameter values.** The DDMORE-source
  ...
  `Output_real_*.lst`. This bundle ships no listing. Per a maintainer
  decision, the MDL parObj
...
* **Two-depot dosing convention.** The MDL declares both `A14`
  ...
  In nlmixr2 the user enters each dose as two simultaneous events
...
* **Compartment naming deviates from canonical.** Several non-PK
  ...
  - `depot_metab`, `central_metab`, `peripheral1_metab` -- metabolite
    ...
    would be infrastructure churn beyond the scope of this model.
