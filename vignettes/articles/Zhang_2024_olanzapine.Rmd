...
- **Supplementary Table S1 was not available when this model was built.** It
  contains the per-drug hypothesis tests (objective-function changes) for the
  30 screened concomitant medications. It holds no model parameter values --
  the final model is fully specified by Equations (6) and (7) and Table 3 -- so
  its absence does not affect the extraction. The stepwise result it documents
  (only aripiprazole retained) is stated in the main text.
