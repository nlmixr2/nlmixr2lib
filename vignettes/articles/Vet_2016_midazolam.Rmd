---
title: "Midazolam (Vet 2016)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Midazolam (Vet 2016)}
...
from which it was extracted. The Vet 2016 publication PDF was not available
when this model was built, so the validation strategy follows the
self-consistency recipe from the package's validation checklist: re-simulate
the bundle's shipped event table with the typical-value model and confirm the
...
was not available when this model was built, so demographic ranges (age range,
weight range, sex balance, region detail beyond "Netherlands SKIC network",
...
publication was not available, the simulated NCA values cannot be compared
...
- **Vet 2016 publication PDF was not available when this model was built**,
  so demographic ranges (age range, weight range, sex
  balance, race / ethnicity, regional enrollment beyond "Netherlands SKIC
  ...
  `population` metadata or the self-consistency check's numeric Css table.
  Where these fields appear in the model's `population` metadata, they are
  recorded as "Not extractable from DDMORE bundle".
  Maintainer follow-up: pull the publication PDF (DOI
...
- **Validation strategy is self-consistency** (per the package's
  validation strategy for DDMORE bundles whose linked publication was not
  available when the model was built). The simulated NCA table above is
  informational; comparison
  against any Vet 2016 per-stratum NCA was not possible.

- **Maintainer-confirmed organ-failure ascertainment criteria.** Vet 2016's
  ...
  could not be quoted from the publication itself because the PDF was not
  available. Each downstream model that consumes `ORG_FAIL_COUNT` should
