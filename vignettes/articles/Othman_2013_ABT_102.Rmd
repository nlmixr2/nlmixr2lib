...
- **Upstream PK provenance.** PK structural parameters (CL/F, V/F, ktr, lag times, Frel) and PK IIV (CV% for CL, V, ktr, lag) come from the **upstream** Othman 2012 popPK paper (J Clin Pharmacol 52:1028-1041). The current paper fixes the PK to empirical-Bayes individual estimates from that fit; the values used here are the population estimates re-reported in the PK/PD-model Results paragraph 1 of Othman 2013. The upstream paper was not available when this model was built; only the values reproduced in Othman 2013 were. PK residual error is not reported in either paper; the packaged model exposes plasma Cc as an internal variable for chained simulation but does not declare a PK observation tilde / residual error.
