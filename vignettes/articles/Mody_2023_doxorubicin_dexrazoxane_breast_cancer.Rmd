...
```{r load-models}
mA <- list(
  jimt1    = readModelDb("Mody_2023_doxorubicin_dexrazoxane_static_jimt1"),
  mdamb468 = readModelDb("Mody_2023_doxorubicin_dexrazoxane_static_mdamb468")
)
mB <- list(
  jimt1    = readModelDb("Mody_2023_doxorubicin_dexrazoxane_jimt1"),
  mdamb468 = readModelDb("Mody_2023_doxorubicin_dexrazoxane_mdamb468")
)
mC <- list(
  jimt1    = readModelDb("Mody_2023_doxorubicin_dexrazoxane_clinical_jimt1"),
  mdamb468 = readModelDb("Mody_2023_doxorubicin_dexrazoxane_clinical_mdamb468")
)
...
static_predict <- function(model, dox, dex, psi = NULL) {
  ...
  out$viability[match(seq_along(dox), out$id)]
}
```
...
```{r layerC-pk}
pk_tobs <- sort(unique(c(seq(0, 24, by = 0.25), seq(24, 504, by = 2))))
...
nca_dox <- nca_one(pk_dox |> filter(time <= 504), 90)
nca_dex <- nca_one(pk_dex |> filter(time <= 504), 900)
...
stopifnot(all(abs(auc_check$`Difference (%)`) < 2))
```
