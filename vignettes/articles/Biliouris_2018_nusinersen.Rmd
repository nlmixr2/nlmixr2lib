...
sections not available with the control stream.
...
the fit dataset). The simulation control stream then re-uses
...
stream (a five-page PDF containing the
...
- **The available source is the simulation control stream, not the full paper text.**
  The five-page PDF contains only the `$PROBLEM SMA MONKEY`
  simulation model (`$SIMULATION (12345678) ONLYSIM SUBPROBLEM=1000`)
  with every `$THETA`, `$OMEGA`, and `$SIGMA` value carrying the
  `FIX` flag. All packaged values are traced verbatim to that
