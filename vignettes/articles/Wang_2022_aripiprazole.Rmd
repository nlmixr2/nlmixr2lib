...
supplement, which was not available when this model was built. See Assumptions
and deviations.
...
- **Supplement not available.** Wang 2022 Tables S1-S4, Figures S1-S3 and
  Appendices S1-S2 were not available when this model was built. Nothing
  needed for either model is in them: Table 1 gives every final popPK estimate
  and both covariate equations, and Table 2 gives both survival parameters.
  Table S4 holds the *base* model (before the phase 3 data were added), which
  the final model supersedes; Tables S2/S3 hold per-study covariate summaries,
  which is why the `population` metadata reports the demographic ranges only
  where the main text states them.
