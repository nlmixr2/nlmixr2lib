...
splenectomy, each with n = 50 subjects (cohort per the package's guidance;
...
* **Cp treated as an exogenous covariate.** The source paper obtains
  ...
  Per the resolution approved by the maintainers, this
...
* **Residual error encoding.** The source paper reports the residual
  ...
  Per the package's validation checklist (NONMEM "additive on
  log-scale" is equivalent to proportional on the linear scale), the
  packaged model encodes this as `PLT ~ prop(propSd)` with
  `propSd = 0.255`.
...
* **Baseline demographic ranges absent from the on-disk source.** The
  text extracted from the Perez-Ruixo 2015 PDF does not tabulate age, weight,
