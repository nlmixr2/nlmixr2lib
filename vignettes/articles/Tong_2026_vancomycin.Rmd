...
`rxSolve()` defaulted to `useLinCmt = TRUE` until rxode2 made the conversion opt-in in September 2026 (issue 1389), which rewrites a recognisably linear
...
of model, and it is necessary: written the literal way, `rxSolve()`'s former default
`useLinCmt = TRUE` rewrite (opt-in since September 2026) drops `peripheral1` and silently solves a
