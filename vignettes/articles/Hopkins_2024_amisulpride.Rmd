...
| `d/dt(effect1) <- kinf_effect1 * fu * fenant * Cc - keff_effect1 * effect1 - kinf_effect2 * effect1` | Supplement, D2 occupancy script `$DES` DADT(6) |
| `d/dt(effect2) <- kinf_effect2 * effect1 - keff_effect2 * effect2` | Supplement, D2 occupancy script `$DES` DADT(7) |
| `rod2 <- 100 * effect2 / (kd + effect2)` | Supplement, D2 occupancy script `$ERROR`: `D2P = (A(7)/(KD + A(7)))*100`; main text Table 2, Distribution Model row |
| `Cc ~ prop(propSd) + add(addSd)` | Supplement, `$ERROR`: `Y = IPRE + IPRE*EPS(1) + EPS(2)` |
| `rod2 ~ add(addSd_rod2)` | Supplement, D2 occupancy script `$ERROR`: `IF (CMT.EQ.8) Y = IPRE + EPS(3)` |
...
make_events <- function(form, dose = 200, n_sub = 1L, times = seq(0, 48, by = 0.1),
  ...
  # every observation returns both Cc and rod2 as columns.
...
stopifnot(nrow(obs) > 0, !anyNA(obs$Cc), !anyNA(obs$rod2), all(obs$Cc >= 0))
```
...
hyst <- solve_arm(tv, "FORM_SEP4199_IR", "IR tablet", times = seq(0, 144, by = 0.25)) |>
  ...
  transmute(time,
            ...
            `D2 receptor occupancy` = rod2 / max(rod2)) |>
...
tmax_ro <- h$time[which.max(h$rod2)]
pk_frac <- h$Cc / max(h$Cc)
ro_frac <- h$rod2 / max(h$rod2)
...
ggplot(md, aes(time, rod2, colour = treatment)) +
...
acc <- pet |>
  ...
  select(treatment, scan, rod2) |>
  pivot_wider(names_from = scan, values_from = rod2) |>
...
dr <- lapply(c(200, 300, 400, 600, 700), function(d) {
  ...
  tibble::tibble(dose = d, rod2_27h = s$rod2[s$time == 27])
}) |>
  bind_rows()
...
paired <- obs |>
  ...
  summarise(cmax = max(Cc), tmax_pk = time[which.max(Cc)],
            tmax_ro = time[which.max(rod2)], .groups = "drop")
...
* **`sex_female_pct` is recorded as 0.** Table 1 reports sex per study rather
  than pooled, and the only pooled column (the N = 37 imaging arm) is 78.4%
  male. The pooled female percentage across all 181 subjects is not published.
