...
- **The functional form of the healthy-adult CL/F factor is inferred, not
  ...
  maintainers. No parameter was tuned to any target.
...
- **`SAMPLE_CAPILLARY` is a newly ratified canonical covariate.** Cleary 2023
  ...
  `inst/references/covariate-columns.md` in the same pull request (maintainer
  ratification). Both matrices used the same validated LC-MS/MS assay, so the
  contrast is the collection site rather than the assay. In this vignette
  roughly 3% of observation records are flagged capillary, concentrated in the
  youngest arm.
...
- **The Mech-PPK model of Table 3 is deferred, not dropped.** It requires the
  ...
  unpublished blood-to-plasma ratio; neither was available when this model was
  built. The ontogeny functions and the ESM Eq. S1-S11 physiology chain have
  been decoded and verified here so the follow-up extraction can proceed
  directly once the upstream paper is available.
