---
title: "Hua_2015_anrukinzumab"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hua_2015_anrukinzumab}
...
sim_sc <- rxSolve(mod, d_sc, returnType = "data.frame") %>%
  left_join(pop_sc %>% distinct(ID, disease), by = c("id" = "ID"))
```
...
```{r simulate-iv}
sim_iv <- rxSolve(mod, d_iv, returnType = "data.frame")
sim_iv <- sim_iv %>%
  left_join(pop_uc %>% distinct(ID, group), by = c("id" = "ID"))
```
