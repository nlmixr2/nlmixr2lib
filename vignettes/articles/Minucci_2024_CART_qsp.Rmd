...
- **Non-canonical covariates.** `FCD8TDP` (drug-product CD8 fraction),
  `TUM_CELLS0` (initial tumor cells), `NDIV` (T-cell expansion factor),
  and `FMEM` (fraction of effector T-cells that become memory) are
  paper-specific covariates without existing canonical entries in
  `inst/references/covariate-columns.md`. Each has a `covariateData`
  entry with an explicit `notes` field documenting the source paper
  provenance. The `TUM_` covariate family in the register currently
  covers volume-based tumor measures (`TUM_VOL`, `TUM_SLD`) rather than
  cell-count-based measures.
