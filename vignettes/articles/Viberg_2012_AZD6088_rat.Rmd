...
* **Typical-value only; IIV magnitudes not reported.** Viberg 2012 Methods
  ...
  disk for this extraction. Per the maintainer-approved encoding, the
...
* **Latency is a non-canonical PD observation name.** The paw-withdrawal-
  ...
  package's paper-named multi-output convention, under which a
  per-output residual-error parameter is named `<errorname>_<output>`.
  `checkModelConventions()` may flag `latency` as outside the canonical
  PK-observation set (`Cc`, `Cc_<metab>`); this is expected and the
  deviation is intentional.

* **Species suffix `_rat`.** AZD6088 has no human
