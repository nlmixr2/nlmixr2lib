...
- **Single residual error term.** The paper used an additive residual error model on the natural log of the observed concentration, which maps to proportional residual error in the linear concentration space (see `inst/references/parameter-names.md` section 'Residual error'). The package model encodes this as `propSd <- sqrt(0.0973) ~= 0.312`; the SD applies on the log scale and equals the proportional CV in linear space to first order.
