...
The coefficient tables (Tables S2 and S6) were not available when this model was
built, so the coefficients are **recovered by inverting the logit** at the three
AUCtau,ss percentiles that main-text Table 4 tabulates. Three points determine a
...
1. **The age-effect centering value is not reported in any available source,
   and 65 years is a back-solve.** Table 2 gives the power exponent (0.699 FIX) but
   ...
   maintainers ratified the 65-year reading; it is recorded here so a reader can
...
3. **The final model's residual error is not tabulated.** Table 2 has no
   ...
   that it was imported rather than estimated. No other value is available.
...
8. **Tables S1-S6 were not available when this model was built.** Table S2
   (exposure-response coefficients within the ibrutinib arm), Table S3
   (covariate subgroups tested), Tables
