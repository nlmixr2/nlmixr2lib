...
are not reported in the text extracted from the PDF; the Canini 2015 paper
cites the underlying Phase 1 design (reference [9]) for baseline
demographics.
...
* **Cohort-specific V0 subsumed by IIV.** The packaged model uses the
  ...
  either post-scale the simulated baseline (multiply `log10_viral_load` at t = 0 by
...
* **Neumann parameters p, beta fixed by convention (p = 1).** Because
  ...
  behaviour (log10_viral_load trajectories) but shifts the interpretation of the
