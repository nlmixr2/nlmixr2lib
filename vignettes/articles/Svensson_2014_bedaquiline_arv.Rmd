...
* **Time-varying CONMED indicators (CONMED_LPV, CONMED_NVP).** Both
  ...
  0/1 time course.
* **Why two model files but one vignette.** Per the package's policy
  of replicating the authors' model structure, the C110 and C117
  ...
  structural parameter values; per the package's one-vignette-per-paper
  convention, the paper still gets a single shared vignette because the
  modelled population, validation strategy, and Table 3 / Figure 1
  comparisons are paper-level facts that span both studies.
