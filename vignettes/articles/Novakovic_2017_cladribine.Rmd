...
The Novakovic 2017 publication is not on disk, so the validation here cannot
...
the per-item baseline category probabilities (the F.3 IRT validation path).
...
- **No on-disk publication.** The Novakovic 2017 publication is not on
  disk, so the side-by-side comparison against the
