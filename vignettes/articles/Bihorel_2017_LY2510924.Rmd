...
Every display equation in this paper is lost by text extraction (it renders as
`formula-not-decoded`). Equations 1-6 were read from a 500 dpi raster render of
...
3. **The published PK claims about low doses are not reproduced by the printed
   ...
   Per standing policy the model file carries the printed values and equations
   exactly; no parameter was tuned toward the figure.
...
* **No published NCA table exists for this paper.** The noncompartmental
  ...
  Res 2014;20:3581-8) and is not on disk, so the NCA section validates the
