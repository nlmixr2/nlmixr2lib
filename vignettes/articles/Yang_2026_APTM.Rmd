...
`hill` needs no unit. `log_cfu = log10(bact + 1)` is log10 CFU/mL. For the two in
...
`e0`, `imax` and `log_cfu` are all log10 CFU/mL.
...
- **Table 3's "Log10CFU/mL drop" rows are the inhibitory term, not the net
  ...
  against Table 3 must compare the term, not `log_cfu`.
