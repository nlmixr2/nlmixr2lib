...
* The publication's underlying dataset (per its Methods; the publication was not on disk when this model was built) is a pooled Bayesian population fit across multiple published human paracetamol PK studies. Per-subject demographics are not exposed by the bundle.
...
1. **No publication on disk.** The Zurlinden & Reisfeld 2016 publication (doi:10.1007/s13318-015-0253-x) was not on disk at extraction time. Parameter values, structural-model equations, and population demographics could not be cross-checked against the publication; everything is sourced from the DDMORE bundle. A more rigorous validation would compare against the publication's Table of posterior summaries and Methods section.
