---
title: "NA_NA_tte_gompertz"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{NA_NA_tte_gompertz}
...
* Citation: `r readModelDb("NA_NA_tte_gompertz")()$reference`
* Description: `r readModelDb("NA_NA_tte_gompertz")()$description`
...
(years), baseline absolute neutrophil count (cells/mm³), number of
prior treatments, diameter of the largest lesion (mm), first-week drug
AUC (µg·h/L), and first-dose Cmax (µg/L) (BAST guiding document
§ 2.2.1, § 2.2.2). The drug, indication, and patient demographics are
unspecified; the bundle is intended for teaching parametric TTE
modelling methodology, not for clinical inference.

For Event 1 specifically, 76/200 (38%) of the simulated patients
experienced the event within the observation window
(BAST guiding document § 2.2.1, Table 2-1).
...
entry in `inst/modeldb/ddmore/NA_NA_tte_gompertz.R`. The table below
collects them in one place.

| Equation / parameter             | Value      | Source location                                                                  |
|----------------------------------|------------|----------------------------------------------------------------------------------|
| Hazard form `h(t) = val * lam_c` | n/a        | Executable_runEV1_201.mod $PK / $DES (`LamC = Lam/1000`, `DADT(1) = VAL*LamC`); BAST guiding doc § 2.4.1 confirms exponential distribution selected for Event 1 |
...
| η on `lam` (`OMEGA(1,1)`)        | 0 FIXED    | Output_simulated_runEV1_201.res FINAL OMEGA(1,1) = 0.00E+00 (placeholder; no estimated IIV) |
| Covariate-selection ΔOFV         | -22.69     | BAST guiding doc § 2.4.2, Table 2-2 (AGE + baseline NEUT chosen as final covariate model EV1_201) |
...
demographics (BAST § 2.2.2 covariate ranges).
...
The BAST guiding document § 2.4.1 (Figure 2-1) reports a Kaplan-Meier
curve overlaid with the candidate parametric distributions; exponential
was selected (AIC 0 versus AIC 2 for Gompertz, 1.999 for Weibull,
1.975 for Lomax, 2.487 for log-logistic, 4.145 for log-normal). The
covariate-VPC stratifications (Figure 2-5 — Figure 2-8) cover the time
window 0 — 400 days. The plot below shows the model's typical-value
...
The model is a TTE survival model, not a PK/PD concentration model — PKNCA
is not the right validation tool. The four checks below exercise the
...
BAST guiding document § 2.4.1 confirms an exponential distribution
...
The BAST guiding document § 2.4.2 (Table 2-2) reports both AGE and
baseline NEUT as significant covariates (covariate-selection ΔOFV
...
baseline neutrophil counts have a lower hazard. The check below
confirms the simulated trajectories shift in the expected directions.
...
shipped `Simulated_event_data.csv` (200 subjects, DVID = 1 records)
...
1. Read `dpastoor/ddmore_scraping/243/Simulated_event_data.csv` (or its
   live-DDMORE-repository counterpart) and filter to `DVID = 1` (Event 1)
   rows.
2. Pass the per-record `AGE` and `NEUT` covariates to `rxSolve(m, ...)`
   with the typical-value parameters (this nlmixr2lib model has no
   IIV, so no `zeroRe()` is needed).
...
* **Distribution mislabeling in the .mod $PROBLEM line.** The .mod
  $PROBLEM line names this a "Gompertz hazard model" but the equation
  `DADT(1) = VAL * LamC` has no `exp(alpha*t)` factor — the realised
  hazard is constant in time, i.e., exponential. The BAST guiding
  document § 2.4.1 (Figure 2-1) confirms exponential was selected for
  Event 1 by AIC. The packaged model preserves the .mod's structure
  faithfully and uses the operator-approved filename
  `NA_NA_tte_gompertz.R`; the description and this Errata note state
...
* **Numerical rescalings preserved.** The .mod uses internal /1000,
  ...
  `model()` so that the `ini()` values match the .lst FINAL THETA
  values one-to-one. The biologically meaningful values are:
  - Baseline hazard rate: 2.80 / 1000 = 0.0028 / day
  - NEUT coefficient:   -1.56 / 10000 = -1.56e-4 per /mm³ above 4133
...
* **No publication-PDF cross-check.** The BAST PTTE 2017 guiding
  ...
  and methodological narrative; no peer-reviewed publication exists.
  The Errata note in the model file's `description` field documents
  this absence.
...
* **Convention warning.** `nlmixr2lib::checkModelConventions()` flags
  ...
  named `cumhaz` (canonical TTE auxiliary-state name registered in
  `R/conventions.R`). The same `units$concentration` warning applies
  to other TTE models in the package (e.g.,
