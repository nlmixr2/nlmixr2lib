...
Reproducing those four numbers exercises **both polarities** of the categorical
covariates at once, and is what pins down the one genuinely ambiguous coding in
the paper (see *Assumptions and deviations*).
...
The footnote is correct, and the paper proves it itself. The covariate-extremes
check above reproduces the published CL/F bounds of 5.17 and 12.1 L/h to three
significant figures **only** under the footnote's coding. Under the other
reading the extremes would be 5.94 and 10.53 L/h, matching neither. The model
file therefore carries the published 0.870 unchanged and raises it to the power
`(1 - RACE_WHITE)`, so the canonical `RACE_WHITE` column keeps its usual
polarity and the inversion is visible at the call site.
