...
No arm exceeds the package's 200-subject-per-arm cohort cap; here each arm
is 25 mice for the VPC, well under the cap.
...
approximation) for each arm using PKNCA. The filter is `!is.na(Cc)`
only -- adding `time > 0` or `Cc > 0` would drop the pre-dose anchor row
that PKNCA needs.
