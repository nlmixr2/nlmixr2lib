...
Finally, assert rather than merely assert in prose that the interval minimum is
attained at an endpoint. An interior minimum would mean `cmin` is not the
trough at all, and it would be wrong by orders of magnitude rather than by a
solver tolerance -- so this check can genuinely go red.

```{r cmin-is-an-endpoint}
endpoint_min <- sim |>
  dplyr::filter(time %in% c(t_last, t_last + tau)) |>
