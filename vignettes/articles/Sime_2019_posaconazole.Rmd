...
- **Table 7 is internally inconsistent at one cell.** For 700 mg q8h x 4 at
  MIC 0.5 mg/L the paper marks the total AUC/MIC 100 target as not attained but
  the unbound fAUC/MIC 0.65 target as attained, even though the authors derived
  0.65 from 100 x 0.0065 precisely so the two would coincide. The packaged model
  attains neither. The transcription above reproduces the paper's marks as
  printed.
...
- **`kon` and `koff` are separately encoded but only their ratio is
  ...
  1e5. The two constants are nonetheless carried separately because that is how
...
- **Known deviation: the model falls a few points short of the paper's
  ...
  every recommended target with a 1.4- to 1.9-fold margin, but the simulated
