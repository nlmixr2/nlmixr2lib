...
13. **Compartment naming.** The 15 states use the canonical `precursor1` for
    the bone-marrow erythroid precursor pool; the reticulocyte quartet
    (`ret_imm_marrow`, `ret_mat_marrow`, `ret_imm_blood`, `ret_mat_blood`),
    the erythrocyte age-transit chain (`erythrocytes1`-`erythrocytes4`), the
    paired corpuscular-hemoglobin chain (`mch1`-`mch4`) and the moderator
    pair (`moderator1`, `moderator2`) are role-based names for states with no
    existing canonical entry. The moderator chain deliberately does not reuse
    the `precursor<n>` prefix that `Ahlstrom_2010_nicotinicAcid_rat.R` uses
    for a Gabrielsson-Hjorth moderator chain, because `precursor1` is already
    taken here by an actual biological precursor pool.
