...
absorption phase. The simulated `aucinf.obs` runs a few percent above the exact
`Dose / CL` because the linear trapezoidal rule over-reads the convex
distribution phase on the paper's sparse early grid -- which is a property of
the study's sampling design, faithfully reproduced, not a model defect. The
`Dose / CL` identity above is the version of this check with the sampling
artefact removed.
