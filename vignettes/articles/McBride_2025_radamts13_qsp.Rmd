...
sources are the Supporting Information S1 (Methods S1 equations,
...
stay settled. This is the steady-state check for endogenous models: it catches
a sign error in any production/loss term, a mistyped rate constant, or a
missing reaction.
...
trough is similarly insensitive (about 6.2% versus 6.4% of normal). Nothing in
the available sources discriminates the two. This file uses the Data S2 value because Data S2 is
...
- **Intercompartmental clearance, Q = 0.456 (Table S1) vs CLD = 0.0456
  ...
  Validation 5 shows the two values cannot be told apart from any available
  source, because peak and trough ADAMTS13 activity are both nearly insensitive
...
- **The `adamts13`-knockout mouse sub-model is not extracted.** Table S1 gives
  ...
  reported in any available source; `ADAMTS13_Model.m` hard-codes
