...
* Units used in the model are specified in a list named by the time unit, the
  dosed compartments and the output variables, for example
  `units <- list(time = "h", depot = "mg", Cc = "ng/mL")`. Use the canonical
  spellings (`"h"`, not `"hr"` or `"hour"`; `"ug"`, not `"mcg"`).
  `checkUnits()` infers the units of every other parameter from these and
  `addUnits()` writes them all into the block, inserting any conversion
  constant the arithmetic needs (see `?addUnits`). The older form
  `units <- list(time = "h", dosing = "mg", concentration = "ng/mL")` is still
  accepted.
