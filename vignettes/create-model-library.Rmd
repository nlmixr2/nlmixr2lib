...
* Units used in the model are specified in a list,
  `units <- list(time = "h", dosing = "mg", concentration = "ng/mL")`, which is
  the form `checkModelConventions()` currently expects. Use the canonical
  spellings (`"h"`, not `"hr"` or `"hour"`; `"ug"`, not `"mcg"`).
  `checkUnits()` infers the unit of every symbol from units given by symbol
  name (`time`, a dosed compartment such as `depot`, an output such as `Cc`),
  and `addUnits()` writes them all into the block in that named form,
  inserting any conversion constant the arithmetic needs (see `?addUnits`).
  The library's models will move to the named form in a separate step.
