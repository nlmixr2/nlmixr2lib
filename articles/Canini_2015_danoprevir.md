...
sim_vpc <- sim |>
  ...
  dplyr::summarise(
    log10_viral_load_Q05 = quantile(log10_viral_load, 0.05, na.rm = TRUE),
    log10_viral_load_Q50 = quantile(log10_viral_load, 0.50, na.rm = TRUE),
    log10_viral_load_Q95 = quantile(log10_viral_load, 0.95, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::mutate(
    day = time / 24,
...
ggplot(sim_vpc, aes(day, log10_viral_load_Q50)) +
  geom_ribbon(aes(ymin = log10_viral_load_Q05, ymax = log10_viral_load_Q95), alpha = 0.25) +
...
baseline_median <- sim |>
  ...
  dplyr::summarise(log10_viral_load_baseline = median(log10_viral_load, na.rm = TRUE),
                   .groups = "drop")

nadir_median <- sim |>
  dplyr::filter(time >= 24) |>
  dplyr::group_by(id, cohort) |>
  dplyr::summarise(log10_viral_load_end = log10_viral_load[which.max(time)], .groups = "drop") |>
  dplyr::group_by(cohort) |>
  dplyr::summarise(log10_viral_load_end_median = median(log10_viral_load_end, na.rm = TRUE),
                   .groups = "drop")
...
decline_tbl <- baseline_median |>
  ...
  dplyr::mutate(
    simulated_decline_log10 = log10_viral_load_baseline - log10_viral_load_end_median,
    pct_diff = 100 * (simulated_decline_log10 - published_decline_log10) /
      published_decline_log10
  ) |>
  dplyr::select(cohort, log10_viral_load_baseline, log10_viral_load_end_median,
                simulated_decline_log10, published_decline_log10, pct_diff) |>
  dplyr::rename(
    "Cohort"                                = cohort,
    "Sim baseline (log10 IU/mL)"            = log10_viral_load_baseline,
    "Sim day-14 median (log10 IU/mL)"       = log10_viral_load_end_median,
...
- **Cohort-specific V0 subsumed by IIV.** The packaged model uses the
  ...
  baseline (multiply `log10_viral_load` at t = 0 by the cohort-specific
  ratio in log10 space) or bias the `etalrbase` distribution.
...
- **Neumann parameters p, beta fixed by convention (p = 1).** Because
  ...
  V0. This does NOT change the observable behaviour (log10_viral_load
