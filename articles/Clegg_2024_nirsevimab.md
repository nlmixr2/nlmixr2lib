...
sim_f6_list <- lapply(ga_groups, function(g) {
  ...
  out_nca <- out |> filter(!is.na(Cc)) |> mutate(ga_group = g$label)
...
- **Figure 6 NCA**: AUC₀₋₃₆₅ and C_max computed via PKNCA on daily
  simulated concentrations; Day-151 concentration derived by direct
  lookup from the same densely sampled time course.
