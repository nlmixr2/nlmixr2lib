...
|:---|:---|:---|
| `bcvacfb <- slope * time` | n/a | Supplement, Data analysis, equation (1) (`Effect = K x Time`) |
...
| `time`         | year                          |
| `slope * time` | LogMAR                        |
| `bcvacfb`      | LogMAR (change from baseline) |
| `addSd`        | LogMAR                        |
...
stopifnot(
  ...
  isTRUE(all.equal(sim_typ$bcvacfb, closed_form, tolerance = 1e-10)),
  # No intercept: change from baseline is identically zero at the baseline visit.
  sim_typ$bcvacfb[t_grid == 0] == 0
)

cat("max |model - 0.0566 * t| =",
    format(max(abs(sim_typ$bcvacfb - closed_form)), digits = 3), "LogMAR\n")
...
# from baseline vs time. `sim` carries the additive residual error; `bcvacfb`
...
- **Observation-variable name.** The observation is named `bcvacfb`: the
  absolute-acuity state `bcva` (`Mulyukov_2018_ranibizumab`) with the
  `cfb` change-from-baseline suffix, exactly as `das28cfb` sits beside
  `das28`. The register carried two competing forms for a change score
