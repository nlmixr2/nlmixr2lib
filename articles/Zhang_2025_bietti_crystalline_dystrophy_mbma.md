...
|:---|:---|:---|
| `d_bcva <- slope * time` | n/a | Supplement, Data analysis, equation (1) (`Effect = K x Time`) |
...
| `time`         | year                          |
| `slope * time` | LogMAR                        |
| `d_bcva`       | LogMAR (change from baseline) |
| `addSd`        | LogMAR                        |
...
stopifnot(
  ...
  isTRUE(all.equal(sim_typ$d_bcva, closed_form, tolerance = 1e-10)),
  # No intercept: change from baseline is identically zero at the baseline visit.
  sim_typ$d_bcva[t_grid == 0] == 0
)

cat("max |model - 0.0566 * t| =",
    format(max(abs(sim_typ$d_bcva - closed_form)), digits = 3), "LogMAR\n")
...
# from baseline vs time. `sim` carries the additive residual error; `d_bcva`
...
- **Observation-variable name.** The observation is named `d_bcva`,
  following the `d_updrs` precedent set by
  `Lee_2011_parkinson_progression` for an algebraic change-from-baseline
  clinical-score endpoint. `d_bcva` is a new entry in
