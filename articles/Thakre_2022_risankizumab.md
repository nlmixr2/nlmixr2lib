...
weight, age, serum albumin, serum creatinine, and baseline
high-sensitivity C-reactive protein (CRP) on clearance, plus body weight
on central volume of distribution. ADA titers on CL and body weight on
V2 were evaluated but did not meet retention criteria and are not in the
final model.
...
pop <- data.frame(
  ...
  CRP   = rlnorm(n_subj, log(6), 1.1)                     # mg/L, skewed right (hs-CRP)
)
```
...
- **CRP (hs-CRP assay):** log-normal around a median of 6 mg/L
  (reference 5.21 mg/L). hs-CRP is highly skewed in PsA.
