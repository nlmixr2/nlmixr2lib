...
worked dimensional analysis. The unit table below shows the units of
every symbol on the right-hand side of each ODE in `model()`; the
left-hand side must equal `[state units]/[time units]` (i.e., the
state’s reporting unit per hour).
...
- **Species and model class.** The model was fit to an in-vitro
  ...
  the package’s species-recording requirement.
...
- **Baseline values are figure-derived.** Xiang 2018 reports the initial
  ...
  maintainer digitisations of those figure points; users who have their
...
- **Dosing/concentration unit mismatch (lint warning).**
  ...
  package’s convention for justified deviations.
