...
the original real data. The original Friberg 2002 publication was not
available when this model was built, so a side-by-side publication-table
comparison of the parameter values is also out of scope here. The
validation in this vignette is therefore the self-consistency check
(confirming that the rxode2 implementation reproduces the bundle’s
NMTRAN trajectories) rather than a publication-table replication.
...
As the mechanistic-sanity check, following the approach used for
DDMORE-sourced models (no PKNCA-amenable PK output, no publication
available for a published-NCA comparison), validation here is restricted
to confirming that the typical-value trajectory matches the canonical
Friberg myelosuppression shape:
...
- **The Friberg 2002 publication was not available when this model was
  ...
  published parameter table or per-cycle WBC profiles is not part of
  this vignette. The validation here is restricted to the
