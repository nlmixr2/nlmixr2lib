...
make_cohort <- function(n, dose_mg, cycle_h, n_cycles, regimen,
  ...
  cov <- tibble::tibble(
    ...
    CREAT       = creat,
    TUMSZ       = tumsz,
    TUMTP_HODGKIN_CLASSICAL   = as.integer(tumtp_chl),
...
- **MMAE central-volume reference category.** The Non-HL effect on MMAE
  ...
  canonical `TUMTP_HODGKIN_CLASSICAL` register entry encodes 1 = HL, so
  the model applies the effect as
  `e_nonhl_vc_mmae^(1 - TUMTP_HODGKIN_CLASSICAL)` to keep the paper’s
  reference (HL) at multiplier 1 while preserving the canonical column
  meaning.
