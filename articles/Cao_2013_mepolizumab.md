...
- Source data digitised from Smith DA et al. *Clin Pharmacokinet.*
  2011;50(4):215-227 (PMID 21348536).

This is the **mepolizumab** entry from the 12-fit Cao 2013 mAb cohort.
...
Cao et al. fit the mPBPK model to mepolizumab plasma concentration
profiles digitised from Smith 2011, anti-IL-5 humanized IgG1
(mepolizumab PK/PD analysis per Smith 2011). Doses: IV (per Smith 2011).
Cao 2013 does not reproduce the underlying Smith 2011 demographics;
...
The packaged metadata (`readModelDb("Cao_2013_mepolizumab")$population`)
...
| `sigma1` (vascular reflection coefficient, tight tissues) | 0.950 | Cao 2013 Table 2, mepolizumab Model A (CV fixed (not estimated)) |
| `sigma2` (vascular reflection coefficient, leaky tissues) | 0.750 | Cao 2013 Table 2, mepolizumab Model A (CV 1.48%) |
| `CLp` (plasma clearance) | 0.00851 L/hr = 0.20424 L/day | Cao 2013 Table 2, mepolizumab Model A (CV 1.50%) |
...
events <- dplyr::bind_rows(
  as.data.frame(make_dose_panel(0.05,  id = 1L)) |> dplyr::mutate(dose_mg_per_kg = 0.05),
  as.data.frame(make_dose_panel(0.5,  id = 2L)) |> dplyr::mutate(dose_mg_per_kg = 0.5),
  as.data.frame(make_dose_panel(5,  id = 3L)) |> dplyr::mutate(dose_mg_per_kg = 5)
)
...
mod <- readModelDb("Cao_2013_mepolizumab")
sim <- rxode2::rxSolve(rxode2::rxode2(mod), events = events,
                       keep = "dose_mg_per_kg") |>
  as.data.frame()
#> rxode2 already building model, waiting for lock file removal
#> lock file: "/tmp/RtmpvuQ6ae/rxode2/rx_1cd74de215b501eb1bdf5094f771a3e2__.rxd/rx_1cd74de215b501eb1bdf5094f771a3e2_.c.lock"
#> .
```
...
sim |>
  ...
  ggplot2::labs(
    ...
    colour = "Dose (mg/kg)",
    title = "Cao 2013 Figure 5 (mepolizumab panel) -- typical-value reproduction",
    caption = "Replicates the mepolizumab panel of Cao 2013 Figure 5 using the packaged Model A mPBPK fit."
  )
```

![](Cao_2013_mepolizumab_files/figure-html/figure-5-1.png)
...
knitr::kable(nca_summary, caption = "Simulated NCA parameters by dose group (Cao 2013 mepolizumab Model A typical-value fit).")
```

| start | end | dose_mg_per_kg | N   | cmax | tmax  | half.life | aucinf.obs |
|------:|----:|---------------:|:----|:-----|:------|:----------|:-----------|
|     0 | Inf |           0.05 | 1   | 1.35 | 0.000 | 18.5      | 17.1       |
|     0 | Inf |           0.50 | 1   | 13.5 | 0.000 | 18.5      | 171        |
|     0 | Inf |           5.00 | 1   | 135  | 0.000 | 18.5      | 1710       |

Simulated NCA parameters by dose group (Cao 2013 mepolizumab Model A
typical-value fit). {.table}

The terminal half-life predicted by the typical-value mPBPK fit
corresponds to mepolizumab’s reported half-life of approximately 2-3
weeks in the underlying Smith 2011 study; Cmax and AUC scale linearly
with dose because the model is purely linear (no TMDD, no
...
- **Model A (clearance from plasma) used by default.** Cao 2013 also
  ...
  for mepolizumab); Model A is used here for consistency across the 12
  ...
  sigma2.- **Drug-specific note.** sigma1 was fixed at 0.950 in Cao 2013
  (CV reported as ‘Not applicable’ in Table 2 footnote c). The
  simulation panel below uses illustrative single-IV doses of 0.05, 0.5,
  and 5 mg/kg.
