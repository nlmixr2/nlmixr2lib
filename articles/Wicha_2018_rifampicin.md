...
- **PAE compartment ODE interpretation.** Wicha 2018 describes the PAE
  ...
  NONMEM code was not available when this model was built, so the
  precise quantitative reproduction requires either the supplement or
  author correspondence. Downstream users who require quantitative
  fidelity to the paper’s Figure 4a should refit the PAE parameters
  against the Gumbo 2007 PAE dataset (Wicha 2018 Methods “PAE model”).
...
- **Residual error sources.** Wicha 2018 is a forward simulation and
  ...
  `addSd_log_cfu` = 1.124 is the combined two-component Svensson 2016
  residual SD on log(CFU/mL). These make the model usable for VPC-style
  stochastic simulation; they are not load-bearing for the EBA
  prediction itself.
