...
## Validation: self-consistency against the DDMORE simulated dataset
...
- **`$ERROR` block residual mapping.** The source `.mod` reports
  log-additive errors on log-IPRED (`Y = LOG(F) + ERR(i)`). Per the
  package’s convention for translating NONMEM `$ERROR` blocks, this maps
  to `Cc ~ prop(propSd)` in nlmixr2 with `propSd = sqrt(SIGMA)`.
