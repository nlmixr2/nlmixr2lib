...
`inst/references/covariate-columns.md`, and minting one is an operator
decision rather than an extractor’s.

It is registered here as **`TUM_JAK2_V617F_VAF`** (operator-ratified), a
continuous somatic variant allele frequency on a 0-100 per cent scale.
The three near neighbours it is deliberately *not* filed under:

- **`SNP_JAK2_RS77375493`.** *JAK2* V617F does have an rsid, so this
  would be mechanically well formed under the auto-approve
  `SNP_<GENE>_RS<rsid>` family. But every registered `SNP_*` column
  carries a germline **genotype** – an allele count or a carrier
