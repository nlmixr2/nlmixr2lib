...
The anemia model is the only one of the three significant
exposure-safety regressions that retained a covariate, so it is a
surface rather than a curve. Figure 5D draws it at the first quartile,
median and third quartile of baseline *JAK2* V617F allele burden, but
Qin 2025 does not print those three quartile values, so the panel below
is drawn at burdens that **are** printed: the two study medians from
Table 1 (61.2% in A20-202 and 77.8% in A19-201) plus a low anchor. This
replicates the panel’s structure and slope, not its three specific
lines.

Note the exposure axis. This model’s `CAV` is the **maintenance-phase**
average, the complement of the titration-phase window used by the two
transaminase models above; it is systematically higher, because it
excludes the period when patients had not yet reached their titrated
dose. The two `CAV` columns are not interchangeable even though they
share a name.
...
![](Qin_2025_ropeginterferon_files/figure-html/fig5cd-1.png)
...
| Cavg, maintenance (ng/mL) | Baseline JAK2 V617F (%) | Model P(anemia) | Closed form |
|---:|---:|---:|---:|
| 0 | 0.0 | 0.0010 | 0.0010 |
| 0 | 61.2 | 0.0119 | 0.0119 |
| 0 | 77.8 | 0.0233 | 0.0233 |
| 20 | 0.0 | 0.0029 | 0.0029 |
| 20 | 61.2 | 0.0358 | 0.0358 |
| 20 | 77.8 | 0.0687 | 0.0687 |
| 40 | 0.0 | 0.0091 | 0.0091 |
| 40 | 61.2 | 0.1033 | 0.1033 |
| 40 | 77.8 | 0.1862 | 0.1862 |

Maintenance-phase anemia probabilities against the closed form. Both
sides evaluate the same fixed coefficients, so this is a pure
numerical-error check and the tolerance is machine precision. {.table
style="width:100%;"}
...
Two further checks pin the two things most easily lost in transcription:
the **scale** of the allele-burden column and the **sign** of both
slopes. Each is arithmetic on fixed published coefficients, so each is
exact.
...
| Quantity                                               |  Value |
|:-------------------------------------------------------|-------:|
| Odds ratio per 10 percentage points of baseline burden |  1.512 |
| Odds ratio at the A19-201 median burden (77.8%) vs 0%  | 24.956 |
| Same, if the 0-1 fraction were supplied by mistake     |  1.033 |

The third row is the failure mode this check exists to catch: a
fraction-scaled column collapses a 25-fold effect to essentially
nothing, and would not otherwise be visible in the output. {.table}
...
- **New register entries.** This extraction registers the PD-output
  ...
  and `djak2v617f` in `inst/references/compartment-names.md`, and the
  covariate canonical `TUM_JAK2_V617F_VAF` in
  `inst/references/covariate-columns.md`. The platelet and white-cell
  states reuse the existing `circ_plt` and `circ_wbc` canonicals rather
  than minting synonyms, and the anemia model’s exposure column reuses
  `CAV`.
- **Seven mutually non-interchangeable `CAV` columns.** All seven E-R
  models name their exposure metric `CAV`, but the averaging windows
  differ and the values are not substitutable: weeks 0-24 and weeks 0-52
  from first dose for the efficacy models, a study-dependent
  titration-phase window (weeks 0-16 in A19-201, weeks 0-6 in A20-202)
  for the transaminase models, and that window’s complement – the
  maintenance phase to end of treatment – for the anemia model. A data
  assembler must build each column separately, per study.

## The eleventh model and its new covariate canonical

Qin 2025’s eleventh model, the **maintenance-phase anemia**
exposure-safety logistic regression (Results 3.4), was deferred on first
extraction and is now packaged as `Qin_2025_ropeginterferon_anemia`. The
delay was a naming decision, not a reporting gap: the model is fully
reported, but its covariate – **baseline *JAK2* V617F allele burden, in
per cent** – had no canonical entry in
`inst/references/covariate-columns.md`, and minting one requires a
maintainer decision.

It is registered here as **`TUM_JAK2_V617F_VAF`** (maintainer-ratified),
a continuous somatic variant allele frequency on a 0-100 per cent scale.
The three near neighbours it is deliberately *not* filed under:

- **`SNP_JAK2_RS77375493`.** *JAK2* V617F does have an rsid, so this
  would be mechanically well formed under the accepted
  `SNP_<GENE>_RS<rsid>` naming family. But every registered `SNP_*`
  column carries a germline **genotype** – an allele count or a carrier
  indicator – whereas this is a somatic **clone fraction** that moves
  under treatment. A value of `61.2` under a `SNP_` name would be
  ambiguous between an allele count and a per cent.
- **`TUM_<GENE>_MUT`** (`TUM_EGFR_MUT`, `TUM_TP53_MUT`, `TUM_IGHV_MUT`,
  …). The right family, but every existing member is a binary 0/1
  presence call. This is their continuous sibling, and founds a
  `TUM_<GENE>_<VARIANT>_VAF` sub-family. The variant token is kept
  because *JAK2* exon 12 mutations are a separate assayable abnormality
  in polycythaemia vera, which Qin 2025’s Discussion names explicitly.
- **`CTDNA`.** Circulating tumour DNA burden in mutant molecules per mL
  of plasma – a different analyte, assay and unit. Worth flagging
  because a fuzzy canonical lookup of “baseline JAK2 V617F allele
  burden” returns `CTDNA` as a confident but wrong match.
...
percentage point, so a column supplied as a 0-1 fraction understates
every covariate contribution 100-fold – silently, since the result is
still a probability in range. At the A19-201 median burden that is the
difference between a 25-fold and a 1.03-fold odds multiplier. The
`anemia-scale-check` chunk above asserts on exactly this. Qin 2025 Table
1’s `JAK2 V617F (%)` row also carries a trap of its own: the Overall
column reads 31.9% because it pools in the 48 phase I healthy
volunteers, who are 0 by construction, so the patient-population summary
is the per-study medians (77.8% and 61.2%) and not the Overall figure.
