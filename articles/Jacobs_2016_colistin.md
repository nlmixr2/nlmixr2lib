...
reference scenario holds RRT_HEMODIAL_ACTIVE = 0 throughout (no HD
session) so the simulated profile matches the paper’s left-most “single
dose without HD” curve.
...
illustrates the same RRT_HEMODIAL_ACTIVE-gated HD behaviour at the
single-dose level: imposing a 4-h HD session starting at t = 8 h on a
typical patient receiving a 1.5 MIU q12h maintenance regimen with a 1.5
MIU reloading dose just after the HD session.
...
- **Hemodialysis-clearance values fixed, not estimated.** The paper used
  ...
  (not in `ini()`) and gated on/off by the `RRT_HEMODIAL_ACTIVE`
  covariate. Users who want to explore alternative dialyser performance
  can override `cl_hd_cms` and `cl_hd_col` in a forked copy of the
  model.
