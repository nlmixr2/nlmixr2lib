...
a real-data fit, a self-consistency check is used instead: re-simulate
using the model’s typical-value parameters and confirm the trajectory
falls inside the distribution of the bundle’s `Simulated_PKPD.txt`
observations. Differences \> 5% on a per-time-point basis are
...
exactly. The qualitative agreement is the acceptance criterion for this
check.
...
- **Bundle simulated dataset is not redistributed.** The DDMORE
  ...
  package does not redistribute the raw file. The self-consistency
  overlay uses a per-time-point summary snapshot computed by the
  maintainers at extraction time from
...
- **Bueno et al. parent model not extracted.** The DDMORE entry’s RDF
  ...
  was not available when this model was built. nlmixr2lib does not
  currently carry the Bueno parent model.
