...
- **New covariate canonical.** No anti-PEG antibody column existed in
  `inst/references/covariate-columns.md` before this extraction. The
  ...
  were therefore registered as a new `ABPEG_<isotype>` family, with
  these models as the founding examples. The isotype is carried in the
  name rather than folded into a single column because papers routinely
  assay both against different cut points and retain only one, as
  Wurthwein 2025 does in ESM Table S6.
