...
- `vmax`: Michaelis-Menten Vmax (when nonlinear / saturable elimination
  is needed). Never `vm`.

Volume parameters always use `vc`, `vp`, `vp2`. Never use generic
numbered volumes like `v`, `v1`, `v2`, `vc1`, `vc2`, etc., even when the
source paper or NONMEM `linCmt()` macro uses that style; the NONMEM
mapping `v1`→`vc`, `v2`→`vp`, `v3`→`vp2` should be applied during model
extraction.
...
For models that report multiple CL terms (e.g., a steady-state arm plus
a time-varying decay arm), use the registered CL component suffixes:
...
Registered metabolite suffixes (defined in
`R/conventions.R::registeredMetabolites`): `mmae`, `dxd`, `sn38`, `dm4`,
`medm4`, `mcmmaf`, `complex`, `ige`, `tab`, `nab`, `dar0`–`dar8`. Add a
new metabolite by editing `R/conventions.R::registeredMetabolites`.
...
- Metabolite-suffixed: `e_<cov>_<param>_<metab>` (e.g., `e_wt_cl_mmae`,
  `e_alb_cl_dxd`).
- Shared exponent (one estimated value applied to two parameters):
  ...
  respectively (Vss = Vc + Vp, so `e_<cov>_vss` becomes
  `e_<cov>_vc_vp`).
