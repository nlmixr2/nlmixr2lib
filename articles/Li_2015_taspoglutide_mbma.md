...
- **Eta naming – `eta_study_<param>`.** Between-trial random effects are
  ...
  package’s MBMA guidance: encode between-study variance as a
  study-level eta, clearly labelled to distinguish it from the popPK
  pattern). The lint flags these as “no matching fixed-effect parameter
