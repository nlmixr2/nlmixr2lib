...
- **One arm does not reconstruct.** The lung 20+20(Split) arm implies a
  ...
  contains a typographical error. There is not enough information in the
  available sources to distinguish the two.
...
- **The published R^2 cannot be reproduced from Table 5.** Recomputing
  ...
  fitted dataset, and the underlying counts are not available. This is
...
- **Supplementary Materials 1-6 were not available** when this model was
  built. The main text contains the complete model equation, all six
  parameter sets, and the index and effect data for every arm, so no
  packaged value depends on them.
