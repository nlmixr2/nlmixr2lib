...
- **V_E/f shared with V_X/f.** Footnote ‡ of Table 3 states V_E/f was
  ...
  model file enforces this by setting `ve <- vx` (same individual
  parameter, including the same `etalvx` IIV).
...
- **Initial total_target uses observed IGE0, not the model SS.** As
  ...
  `inits = c(total_target = pe / cle * ve)` after computing `pe`, `cle`,
  and `ve` for the cohort of interest.
...
#> [10] lotri_1.0.3           qs2_0.2.0             jsonlite_2.0.0       
...
#> [43] data.table_1.18.2.1   glue_1.8.1            Rcpp_1.1.1-1.1       
