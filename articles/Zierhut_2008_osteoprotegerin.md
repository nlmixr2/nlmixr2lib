...
Zierhut et al. (2008) full-text article was not available when this
model was built; cohort-level demographics are therefore not populated
in the model’s `population` metadata. The bundle’s `Vignette_opg.R` (the
...
al. (2008) publication was not available when this model was built, so a
...
## Self-consistency against the DDMORE simulated dataset
...
vignette renders cleanly when the bundle is not present locally.
...
- **Original publication not available.** Zierhut et al. (2008, J.
  Pharmacokinet. Pharmacodyn. 35(4):379-99,
  [doi:10.1007/s10928-008-9093-5](https://doi.org/10.1007/s10928-008-9093-5))
  was not available when this model was built, so a side-by-side
  ...
  self-consistency check against the bundle’s `Simulated_opg.txt` plus
  the mechanistic-sanity checks above. The bundle’s
