...
| `propSd` | 0.2356 | `.lst` SIGMA(1,1) = 0.0555 = 0.2356^2; NONMEM `Y = LOG(F) + EPS(1)` == proportional in linear space (the package’s residual-error convention). |
...
- **`MINIMIZATION TERMINATED DUE TO ROUNDING ERRORS (ERROR=134)`, not
  ...
  The maintainers approved extraction on 2026-05-06 with this caveat
  documented.
...
- **`propSd = 0.2356 = sqrt(0.0555)`.** NONMEM `Y = LOG(F) + EPS(1)`
  ...
  package’s residual-error back-transformation rule gives
