...
- Emax_i: additive linear-scale form
  `Emax_REF + e_ecog_cl_time_max * ECOG_GE1 + e_ipico_cl_time_max * CONMED_IPI_ANY + etaEmax`.
...
| regimen         | cl_time_max | cl_ss_cl_0 |
|:----------------|------------:|-----------:|
...
- **Coadministration regimens beyond the three simulated here**: the
  ...
  2019 by combining the CONMED\_\* indicators (the unmodeled 1 mg/kg q3w
  x 4 induction and 1 mg/kg q12w schedules collapse into the reference
  group on baseline CL, but they would still set CONMED_IPI_ANY = 1 and
  so contribute the IPICO additive effect on Emax). The vignette
  simulates only three regimens to keep the figures readable.
