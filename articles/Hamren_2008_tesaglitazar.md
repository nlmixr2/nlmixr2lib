...
- **No NONMEM control stream available.** The published article is the
  only source available; no `.mod` / `.ctl` / `.lst` is supplied. All
  parameter values are sourced from Table 2 of the main publication. The
  ADVAN6 (general nonlinear kinetics) subroutine was used in the
  original NONMEM VI fit (Methods ‘Data analysis’).
