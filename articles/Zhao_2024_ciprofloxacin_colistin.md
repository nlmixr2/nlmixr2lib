...
fig2a <- do.call(rbind, Map(function(b, mc, ms, s, x) {
  sim_tk(b, mc, ms, x, 0) |>
    dplyr::transmute(time, log_cfu, strain = s, regimen = paste0(x, "x MIC"))
}, grid_cip$BACT, grid_cip$micCip, grid_cip$micCst, grid_cip$strain, grid_cip$cipX))

ggplot(fig2a, aes(time, log_cfu, colour = regimen)) +
...
ggplot(fig2b, aes(time, log_cfu, colour = regimen)) +
  geom_line(linewidth = 0.7) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "grey40") +
  facet_wrap(~strain) +
  labs(x = "Time (h)", y = expression(log[10]~"CFU/mL"),
...
fig2c <- do.call(rbind, Map(function(b, mc, ms, s, xc, xs) {
  sim_tk(b, mc, ms, xc, xs) |>
    dplyr::transmute(time, log_cfu, strain = s,
...
ggplot(fig2c, aes(time, log_cfu, colour = regimen)) +
  geom_line(linewidth = 0.7) +
  geom_hline(yintercept = 1, linetype = "dashed", colour = "grey40") +
  facet_wrap(~strain) +
...
plateau <- gc47$log_cfu[gc47$time == 32]
overshoot <- max(gc47$log_cfu)
...
lm378 <- sim_tk(378, 0.38, 0.5, 0, 1)$log_cfu
lm421 <- sim_tk(421, 1.00, 0.5, 0, 1)$log_cfu
...
tibble::tibble(
  ...
  Result = c(
    sprintf("%.3f vs %.3f (transient peak %.3f at %g h)", plateau, bmax_log10,
            overshoot, gc47$time[which.max(gc47$log_cfu)]),
...
# PKNCA convention: the concentration column is named log_cfu. Here the analyte is
# the unbound ciprofloxacin plasma concentration, not the bacterial count.
sim_nca <- sim_sd |>
  dplyr::filter(!is.na(cipPlasma)) |>
  dplyr::transmute(id, time, log_cfu = cipPlasma, treatment = arm)

sim_nca <- dplyr::bind_rows(
  sim_nca,
  sim_nca |> dplyr::distinct(id, treatment) |> dplyr::mutate(time = 0, log_cfu = 0)
) |>
...
conc_obj <- PKNCA::PKNCAconc(sim_nca, log_cfu ~ time | treatment + id)
...
sim_f4 |>
  ...
  dplyr::summarise(Q10 = quantile(log_cfu, 0.10), Q50 = median(log_cfu),
                   Q90 = quantile(log_cfu, 0.90), .groups = "drop") |>
  ggplot(aes(time, Q50, colour = arm, fill = arm)) +
  geom_ribbon(aes(ymin = Q10, ymax = Q90), alpha = 0.2, colour = NA) +
...
site_check <- tibble::tibble(
  ...
  "log10 CFU/mL at 32 h, ciprofloxacin alone" = c(
    median(sim_f4$log_cfu[sim_f4$arm == "CIP 400 mg q8h alone" & sim_f4$time == 32]),
    median(sim_f5$log_cfu[sim_f5$arm == "CIP 400 mg q8h alone" & sim_f5$time == 32])
  )
)
