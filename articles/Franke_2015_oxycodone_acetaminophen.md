...
That is four independent final models. Three of them are packaged here;
the fourth is not, for the reason given under *Errata and known
deviations*.
...
![](Franke_2015_oxycodone_acetaminophen_files/figure-html/figure-7-1.png)
...
**The acetaminophen food-effect model is not packaged.** Franke 2015
...
equations. A two-compartment model cannot be simulated without them, and
the values cannot be borrowed from the pooled fasted acetaminophen model
because that fit’s central volume (58.9 L) differs from this one’s (140
L) by a factor of 2.4, so its disposition parameters are not
transferable. The model is therefore left out rather than completed with
invented or transplanted numbers.
...
the final model column. `Franke_2015_oxycodone_food` therefore declares
`addSd <- fixed(0)` and `propSd <- fixed(0)` rather than carrying over
the unrelated 151-subject pooled-fasted estimates. Simulations from that
model are IPRED-only; the interindividual variability, which *is*
reported, is intact.
...
- Sex, age and BMI were screened by the authors but retained in none of
  the three packaged models; they are recorded in each model’s
  `covariatesDataExcluded` metadata rather than in `covariateData`, and
  are not sampled in the virtual cohorts.
- All parameter values come from the paper’s own tables and printed
