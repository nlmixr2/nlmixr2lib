...
summaries) was not available when this model was built; this vignette
...
were not available when this model was built; the structural ODE
...
available when this model was built, so a numerical-table comparison is
not possible here. The qualitative directionality nonetheless matches
...
- The Hennig 2015 AAC supplemental material (Fig. S1 model schematic,
  ...
  summary) was not available when this model was built; structural ODE
  topology was inferred from the Methods + Results paragraphs and the
  Hennig 2016 JAC pooled-rifabutin DDI follow-up paper’s NONMEM control
  stream (`run2305.mod`, by the same first author), which uses the same
