...
| `addSd_log_cfu` | SIGMA(1+2) | sqrt(0.422 + 0.0364) \* log(10) ≈ 1.559 | .mod L521-526, .lst SIGMA block lines 877-889 |
...
re-extractable population PK study, and the linked publication was not
available when this model was built. The validation strategy is
therefore **self-consistency**: re-simulate the bundle’s
...
table is not possible because the publication was not available.
...
events_raw <- raw |>
  ...
  # (`CONMED_COL_CC` for colistin, `CONMED_MER_CC` for meropenem).
  # Rename the source-dataset shorthand columns here so the events
  # table satisfies the model's covariate declarations.
  dplyr::rename(CONMED_COL_CC = Ccol, CONMED_MER_CC = Cmer)
...
re-simulation produces `log_cfu` = ln(total bacteria) which corresponds
...
summary_df <- diff_df |>
  ...
  dplyr::summarise(
    ...
    median_log_cfu = stats::median(log_cfu),
...
| cohort          |   n | median_dv | median_log_cfu |  rmse | median_abs_res |
|:----------------|----:|----------:|---------------:|------:|---------------:|
...
Per-cohort self-consistency summary. RMSE / median \|DV - log_cfu\| are
...
ggplot(diff_df, aes(time, log_cfu)) +
...
- **Non-canonical observation variable.** `log_cfu` (ln of total
...
- **Per-replicate residual error collapsed.** The .mod’s `$ERROR` block
  ...
  (`addSd_log_cfu = sqrt(0.422 + 0.0364) * log(10)`).
- **No linked publication available.** The associated paper
  ([doi:10.1093/jac/dkv488](https://doi.org/10.1093/jac/dkv488)) was not
  available when this model was built; PKNCA-style comparison against
  published tables is therefore not possible. Validation reduces to
  self-consistency against the bundle-shipped `Output_simulated_*.lst`.
