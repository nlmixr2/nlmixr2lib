...
algebraic observable `log_cfu` (the log10 total count) as a column
regardless. The PK sections below keep `Cc`, which is the apramycin
plasma concentration – the two are different quantities and now have
different names.
...
sim_invitro <- purrr::pmap_dfr(invitro_conditions, function(condition, BACT, PH_MEDIUM, mic) {
  purrr::map_dfr(xmic_levels, function(xmic) {
    ...
    solve_typical(mod_invitro, ev) |>
      dplyr::transmute(time, logcfu = log_cfu,
                       condition = condition,
