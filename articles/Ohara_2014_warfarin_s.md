...
recTail <- rec |>
  ...
  # Keep gap >= 1e-6 * its peak: below that the ODE integrator has no relative accuracy left.
  dplyr::filter(gap >= 1e-6 * max(gap)) |>
  dplyr::filter(time >= 24 * 40, time <= 24 * 58, gap > 1e-9)
