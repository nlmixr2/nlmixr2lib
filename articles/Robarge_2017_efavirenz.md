...
- **CYP2B6 distribution.** Robarge 2017 Table 1 reports race (71%
  ...
  PDF) carries that breakdown. The simulated cohort uses an approximate
  60% / 30% / 10% normal / intermediate / slow split chosen to populate
  all three multiplier groups in a typical mixed North American
  Caucasian + African-American cohort, not to reproduce the exact
  Robarge 2017 sample. A user with a specific cohort in mind should
  rebuild the `subjects` table with the matching frequencies.
...
- **FM is a new canonical covariate.** Fat mass is the natural
  ...
  same Janmahasatian (2005) formula chain (FM = TBW - FFM). `FM` is
  registered in `inst/references/covariate-columns.md` as paired with
  `FFM`.
