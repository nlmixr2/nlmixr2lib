...
Gupta et al. (2016) characterised amatuximab (MORAb-009)
pharmacokinetics in 199 patients across four clinical studies (two Phase
I dose-escalation studies in advanced mesothelin-expressing tumours, a
Phase II pancreatic cancer study, and a Phase II malignant pleural
mesothelioma \[MPM\] study). The final PK model is a **two-compartment
structure with parallel linear and Michaelis-Menten (saturable)
elimination** from the central compartment. Covariate analysis retained
body weight on the central volume (power effect, reference 70 kg) and
antidrug-antibody (ADA) positivity at a titer threshold \> 64 on linear
clearance (multiplicative effect of 1.49).

This vignette documents the parameter provenance in a source-trace
table, builds a virtual cohort matching Gupta 2016 Table 1, simulates
the 5 mg/kg weekly (without interruption) regimen evaluated in the
paper’s dose selection analysis, and validates the simulated NCA against
the observed Study 003 median Cmin and the published PK characteristics.

- Citation: Gupta A, Hussein Z, Hassan R, Wustner J, Maltzman JD, Wallin
  BA. Population pharmacokinetics and exposure-response relationship of
...
SD (16.4 kg), clipped to the observed range (35-134 kg). ADA positivity
(with the \>64 titer threshold) is drawn as Bernoulli(0.10) as a
...
does not publish the exact ADA-prevalence distribution for the PK
dataset.
...
constant-rate IV infusion for six 21-day cycles (no interruption),
giving 18 doses over ~126 days. We reproduce that schedule, sampling at
each pre-dose trough, end-of-infusion, 24 h, 72 h, and mid-interval for
NCA.
...
typical-value reference pass via
...
![](Gupta_2016_amatuximab_files/figure-html/fig1-1.png)
...
![](Gupta_2016_amatuximab_files/figure-html/fig2-1.png)
...
- **Intermittent vs continuous weekly dosing.** The paper distinguishes
  ...
  simulated weekly-without-interruption regimen used in the
  dose-selection analysis. This vignette implements the **continuous
  weekly** regimen (to match the paper’s Figure 3 dose-evaluation
  simulation).
...
- Gupta A, Hussein Z, Hassan R, Wustner J, Maltzman JD, Wallin BA.
