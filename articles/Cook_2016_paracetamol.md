...
2016 publication PDF was not available when this model was built, so the
validation strategy is a self-consistency check: re-simulate the
bundle’s structural model with the published final estimates and confirm
the trajectory is shape- and magnitude-consistent with the source ODE.
