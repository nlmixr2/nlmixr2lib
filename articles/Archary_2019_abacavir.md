...
- **Year-letter on the file name resolves to 2019, not 2018.** The file
  ...
  to `Archary_2019_abacavir.R` to match the publication year.
- **Two-models-per-paper extraction (maintainer decision).** Archary
  ...
  vignettes in a single PR. The lamivudine half is in
...
- **Day-1-vs-day-14 step encoded via a binary `DAY14` covariate
  ...
  CL/F between day 1 (3.33 L/h per 7 kg) and day 14 (5.86 L/h per 7 kg)
  of ART, which the source attributes to nutritional rehabilitation +
  auto-induction of hepatic metabolism over the first ~2 weeks of
  treatment. The maintainers approved encoding the step as a binary
  `DAY14` covariate (0 = day 1, 1 = day 14) on CL/F via the
  multiplicative shift `(1 + e_day14_cl * DAY14)` with
