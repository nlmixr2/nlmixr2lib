...
identity, so it is gated tightly (a deterministic quantity is asserted
at the accuracy actually achieved).
...
# printed numbers, with no cohort draw, no seed and no RNG anywhere. Nothing
# here varies with the simulated cohort, so the bounds are set tightly at the
...
# draw, so it is asserted on the centre and with a wide band,
# not on any extreme.
