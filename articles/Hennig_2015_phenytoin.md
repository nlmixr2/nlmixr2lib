...
describes protein-unbound (Cc) and protein-bound (Cb) phenytoin plasma
...
`ini()` entry in `inst/modeldb/specificDrugs/Hennig_2015_phenytoin.R`.
...
| `propSd_Cc` (proportional residual SD on Cc) | 0.221 | sqrt(0.136^2 + 0.174^2): assay error PHYu 13.6 percent + common prop 17.4 percent (Table 2) |
...
| Observation: `Cc = central / vc`, `Cb = (peripheral1 / vp) * pub` | n/a | Figure 1 caption |
...
The published study contributed paired Cc / Cb concentrations from
...
build_events <- function(cohort, route = c("PO", "IV"), dose_mg_per_kg) {
  ...
  # it returns Cc and Cb in the output frame regardless. Use Cc here.
  obs <- tibble::tibble(id = cohort$id) |>
    tidyr::crossing(time = obs_grid) |>
    dplyr::mutate(amt = 0, evid = 0, cmt = "Cc", rate = 0)
  dose <- cohort |>
    dplyr::transmute(
      id   = id,
      time = 0,
...
### Typical-value Cc and Cb time courses by weight band
...
sim_typ |>
  ...
  tidyr::pivot_longer(c(Cc_med, Cb_med),
  ...
  labs(x = "Time after dose (h)", y = "Concentration (mg/L)",
       ...
       title = "Typical-value Cc and Cb time courses by weight band",
...
sim_po |>
  dplyr::filter(time <= 24, !is.na(Cc), !is.na(Cb)) |>
  dplyr::group_by(time) |>
  dplyr::summarise(
  ...
  scale_y_log10() +
  labs(x = "Time after dose (h)", y = "Concentration (mg/L)",
       title = "Simulated 5th / 50th / 95th percentile Cc and Cb after PO 5 mg/kg",
...
PKNCA is run separately on Cc and Cb from the single-dose oral
...
nca_input <- sim_po |>
  dplyr::filter(!is.na(Cc), !is.na(Cb)) |>
  dplyr::select(id, time, Cc, Cb, band)
...
conc_obj_cu <- PKNCA::PKNCAconc(
  nca_input |> dplyr::transmute(id, time, Cc = Cc, band),
  Cc ~ time | band + id,
  concu = "mg/L", timeu = "h"
)
...
knitr::kable(summary(nca_cu),
             caption = "Simulated NCA - unbound phenytoin (Cc) after a single 5 mg/kg oral dose.")
```

| Interval Start | Interval End | band | N | Cmax (mg/L) | Tmax (h) | Half-life (h) | AUCinf,obs (h\*mg/L) |
|---:|---:|:---|:---|:---|:---|:---|:---|
| 0 | Inf | \<2 y | 60 | 0.180 \[159\] | 6.00 \[0.300, 72.0\] | 47.0 \[66.6\], n=59 | NC |
| 0 | Inf | \>11 y | 60 | 0.279 \[179\] | 6.00 \[0.300, 72.0\] | 36.0 \[45.5\], n=58 | NC |
| 0 | Inf | 2-11 y | 60 | 0.219 \[184\] | 8.00 \[0.550, 72.0\] | 31.3 \[28.9\], n=57 | NC |

Simulated NCA - unbound phenytoin (Cc) after a single 5 mg/kg oral dose.
{.table}
...
| Interval Start | Interval End | band | N | Cmax (mg/L) | Tmax (h) | Half-life (h) | AUCinf,obs (h\*mg/L) |
|---:|---:|:---|:---|:---|:---|:---|:---|
| 0 | Inf | \<2 y | 60 | 1.49 \[158\] | 6.00 \[0.300, 72.0\] | 47.0 \[66.6\], n=59 | NC |
| 0 | Inf | \>11 y | 60 | 2.26 \[180\] | 6.00 \[0.300, 72.0\] | 36.0 \[45.5\], n=58 | NC |
| 0 | Inf | 2-11 y | 60 | 1.80 \[182\] | 8.00 \[0.550, 72.0\] | 31.3 \[28.9\], n=57 | NC |
...
fraction 0.12 +- 0.03. The simulated single-dose Cc peak above is below
