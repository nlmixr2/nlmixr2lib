...
7.  **The less susceptible outputs carry a 0 log10 baseline; the total
    ...
    `log_cfu` is the untransformed `log10` of the summed state and can
    fall below 0 under strong killing; the figures above clamp it at 0
    for display only, matching the paper’s convention of assigning 1
    CFU/mL to non-quantifiable samples.
