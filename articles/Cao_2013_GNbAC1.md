...
- Source data digitised from Curtin F et al. *Clin Ther.*
  2012;34(11):2268-2278 (PMID 23200102).

This is the **GNbAC1** entry from the 12-fit Cao 2013 mAb cohort. The
...
Cao et al. fit the mPBPK model to GNbAC1 plasma concentration profiles
digitised from Curtin 2012, humanized IgG4 against MSRV envelope protein
(first-in-human single-dose escalation; candidate therapy for
MS-associated endogenous retrovirus). Doses: 0.15, 0.6, 2, 6 mg/kg IV.
Cao 2013 does not reproduce the underlying Curtin 2012 demographics;
consult the source publication for age, sex, and other baseline
characteristics. Cao 2013 used a 70 kg reference body weight when
assigning the human physiological constants (V_p = 2.6 L, ISF = 15.6 L,
lymph flow = 2.9 L/day).

The packaged metadata (`readModelDb("Cao_2013_GNbAC1")$population`)
...
| `sigma1` (vascular reflection coefficient, tight tissues) | 0.915 | Cao 2013 Table 2, GNbAC1 Model A (CV 5.21%) |
| `sigma2` (vascular reflection coefficient, leaky tissues) | 0.831 | Cao 2013 Table 2, GNbAC1 Model A (CV 5.71%) |
| `CLp` (plasma clearance) | 0.00714 L/hr = 0.17136 L/day | Cao 2013 Table 2, GNbAC1 Model A (CV 2.93%) |
...
events <- dplyr::bind_rows(
  as.data.frame(make_dose_panel(0.15,  id = 1L)) |> dplyr::mutate(dose_mg_per_kg = 0.15),
  as.data.frame(make_dose_panel(0.6,  id = 2L)) |> dplyr::mutate(dose_mg_per_kg = 0.6),
  as.data.frame(make_dose_panel(2,  id = 3L)) |> dplyr::mutate(dose_mg_per_kg = 2),
  as.data.frame(make_dose_panel(6,  id = 4L)) |> dplyr::mutate(dose_mg_per_kg = 6)
)
...
mod <- readModelDb("Cao_2013_GNbAC1")
sim <- rxode2::rxSolve(rxode2::rxode2(mod), events = events,
                       keep = "dose_mg_per_kg") |>
  as.data.frame()
```

## Replicate Figure 5 (GNbAC1 panel)
...
sim |>
  ...
  ggplot2::labs(
    ...
    colour = "Dose (mg/kg)",
    title = "Cao 2013 Figure 5 (GNbAC1 panel) -- typical-value reproduction",
    caption = "Replicates the GNbAC1 panel of Cao 2013 Figure 5 using the packaged Model A mPBPK fit."
  )
```

![](Cao_2013_GNbAC1_files/figure-html/figure-5-1.png)
...
knitr::kable(nca_summary, caption = "Simulated NCA parameters by dose group (Cao 2013 GNbAC1 Model A typical-value fit).")
```

| start | end | dose_mg_per_kg | N   | cmax | tmax  | half.life | aucinf.obs |
|------:|----:|---------------:|:----|:-----|:------|:----------|:-----------|
|     0 | Inf |           0.15 | 1   | 4.04 | 0.000 | 21.4      | 61.3       |
|     0 | Inf |           0.60 | 1   | 16.2 | 0.000 | 21.4      | 245        |
|     0 | Inf |           2.00 | 1   | 53.8 | 0.000 | 21.4      | 817        |
|     0 | Inf |           6.00 | 1   | 162  | 0.000 | 21.4      | 2450       |

Simulated NCA parameters by dose group (Cao 2013 GNbAC1 Model A
typical-value fit). {.table}

The terminal half-life predicted by the typical-value mPBPK fit
corresponds to GNbAC1’s reported half-life of approximately 2-3 weeks in
the underlying Curtin 2012 study; Cmax and AUC scale linearly with dose
because the model is purely linear (no TMDD, no concentration-dependent
clearance).
...
- **Model A (clearance from plasma) used by default.** Cao 2013 also
  reports Model B (clearance from interstitial fluid; CLi = 0.0189 L/hr
  for GNbAC1); Model A is used here for consistency across the 12
  ...
  sigma2.- **Drug-specific note.** GNbAC1 is the only IgG4 mAb in the
  Cao 2013 human cohort. The packaged file uses Kp = 0.8 (IgG1 default)
  for consistency with the rest of the cohort; the IgG4 native value is
  0.4 (Cao 2013 Methods, refs 22-23). Cao 2013 Table 2 does not state
  per-drug Kp.
