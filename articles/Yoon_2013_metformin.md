...
| V/F linear-in-deviation form | n/a | Yoon 2013 Results (paragraph on Table S2 covariate screen); reference weight 70 kg per package convention (see Assumptions) |
...
- **Reference weight for V/F.** The paper reports the linear body-weight
  ...
  the Results paragraph following Table III is rendered as an image (it
  does not survive plain-text extraction of the PDF) and does not print
  the reference weight explicitly. This vignette (and the packaged
  model) uses a reference weight of 70 kg per the package’s “sensible
