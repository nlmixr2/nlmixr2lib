...
sample, added to `inst/references/covariate-columns.md` as a member of
the accepted `ASSAY_<method>` family. An integer stratum rather than
five or six paired binary indicators follows the guidance already
recorded in the `ASSAY_CMIA` entry for datasets mixing more than two
assays, and the `STUDY_TACRO_FRANCKE` precedent.
