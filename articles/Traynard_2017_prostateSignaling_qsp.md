...
design <-
  ...
  dplyr::transmute(
    ...
    PI3K_INHIBITED = TR_PI3Ki,
    MEK_INHIBITED = TR_MEKi,
    MTOR_INHIBITED = TR_mTORi,
    P38_INHIBITED = TR_p38i,
    IKK_INHIBITED = TR_IKKai
  )
...
| id | STIM_EGF_NORM | STIM_IGF1_NORM | STIM_IL6_NORM | STIM_TNFA_NORM | STIM_DHT_NORM | PI3K_INHIBITED | MEK_INHIBITED | MTOR_INHIBITED | P38_INHIBITED | IKK_INHIBITED |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
design_long <-
  ...
  dplyr::mutate(
    ...
    `PI3K i` = PI3K_INHIBITED == 1, `IKK i` = IKK_INHIBITED == 1, `MEK i` = MEK_INHIBITED == 1,
    `mTOR i` = MTOR_INHIBITED == 1, `p38 i` = P38_INHIBITED == 1
  ) |>
...
condition_label <-
  ...
  dplyr::transmute(
    ...
    condition = trimws(paste0(
      ...
      ifelse(STIM_DHT_NORM == 1, "DHT ", ""), ifelse(PI3K_INHIBITED == 1, "+PI3Ki ", ""),
      ifelse(MEK_INHIBITED == 1, "+MEKi ", ""), ifelse(MTOR_INHIBITED == 1, "+mTORi ", ""),
      ifelse(P38_INHIBITED == 1, "+p38i ", ""), ifelse(IKK_INHIBITED == 1, "+IKKi ", "")
    ))
  ) |>
...
examples <- tibble::tribble(
  ~id, ~node,
  which(design$STIM_EGF_NORM == 1 & rowSums(design[, c("PI3K_INHIBITED", "MEK_INHIBITED", "MTOR_INHIBITED", "P38_INHIBITED", "IKK_INHIBITED")]) == 0)[1], "ERK1_2",
  which(rowSums(design[, 2:6] == 1) == 0 & rowSums(design[, 7:11]) == 0)[1], "p38",
  which(design$STIM_DHT_NORM == 1 & rowSums(design[, c("PI3K_INHIBITED", "MEK_INHIBITED", "MTOR_INHIBITED", "P38_INHIBITED", "IKK_INHIBITED")]) == 0)[1], "AKT",
  which(design$PI3K_INHIBITED == 1)[1], "RPS6"
) |>
...
control <- dplyr::mutate(
  ...
  PI3K_INHIBITED = 0, MEK_INHIBITED = 0, MTOR_INHIBITED = 0, P38_INHIBITED = 0, IKK_INHIBITED = 0
)
...
inh_map <- c(PI3K = "PI3K_INHIBITED", MEK = "MEK_INHIBITED", mTOR = "MTOR_INHIBITED", p38 = "P38_INHIBITED", IKKa = "IKK_INHIBITED")
...
arm <- function(lab, dht, pi3ki) {
  p <- dplyr::mutate(control, STIM_DHT_NORM = dht, PI3K_INHIBITED = pi3ki)
  p$id <- 1L
...
- **Ligand and inhibitor concentrations are not recoverable from this
  ...
  applied) and the `*_INHIBITED` covariates are binary flags, which is
  exactly the resolution the trained model uses.
