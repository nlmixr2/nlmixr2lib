...
# Cheap regression guard for the disjoint-id property.
...
# Defensive time-zero ensure (PKNCA's AUC interval starts at time zero).
...
- **IBW formula in the virtual cohort.** The source paper references a
  ...
  in the extracted methods text. The cohort builder uses the
  metric-units Devine variant (men 50 + 0.91 \* (HT_cm - 152.4); women
  45.5 + 0.91 \* (HT_cm - 152.4)) to derive IBW from a simulated HT_cm.
  Per-paper IBW is recommended when a user has access to the source
  dataset.
