...
The reference weight is 75 kg; `NON_ECZTRA = 0` and `DILUTION = 0`
define the ECZTRA / undiluted reference.
...
| `e_non_ecztra_cl` (non-ECZTRA on CL) | `0.344` | Table 2 |
| `e_non_ecztra_vc` (non-ECZTRA on V2) | `0.258` | Table 2 |
...
`NON_ECZTRA = 0` (ECZTRA-trial reference) and `DILUTION = 0` (undiluted
...
sim <- rxode2::rxSolve(
  mod, events = events,
  keep = c("WT", "SEXF", "NON_ECZTRA", "DILUTION")
)
```
...
ggplot(vpc, aes(time, Q50)) +
  ...
  labs(
    ...
    caption = "Virtual AD cohort (N = 400); WT median 74.5 kg; NON_ECZTRA = 0, DILUTION = 0."
  ) +
  theme_minimal()
```
...
knitr::kable(typical_summary, digits = 2,
  caption = "Typical-subject steady-state exposure (WT = 75 kg, NON_ECZTRA = 0, DILUTION = 0; IIV zeroed).")
```
...
Typical-subject steady-state exposure (WT = 75 kg, NON_ECZTRA = 0,
...
- The pooled-analysis population spans AD, asthma, and healthy subjects
  ...
  AD-relevant reference combination (`NON_ECZTRA = 0`, `DILUTION = 0`)
  so the simulated profile mirrors the labelled 600-mg-load + 300-mg-Q2W
  SC atopic-dermatitis regimen.
...
- The covariate columns use the canonical `SEXF` / `NON_ECZTRA` /
  `DILUTION` names. Soehoel 2022 writes the latter two `nonECZTRA` and
  `dilution`; the paper’s spellings are recorded in each covariate’s
  `source_name` field, and the values and reference categories are
  unchanged.
