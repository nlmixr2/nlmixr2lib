...
- **Known convention deviation: the exposure-response output names.**
  ...
  warning but not the observation warning. The maintainers’ decision is
  to keep the paper-named forms and accept the warning rather than found
  a PD-output canonical family on two examples; the names may change if
  a canonical family is ratified later. The population PK model
