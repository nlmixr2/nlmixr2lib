...
- **Supplement not available.** Comisar 2025 cites Table S1
  (bioanalytical method validation), Table S2 (covariates tested),
  Figure S1 (observed concentrations) and Figure S2 (external-validation
  VPC). None of these could be obtained when this model was built, and
  none is a parameter source: Table 3 is complete, and every derived
  quantity quoted in the Discussion reproduces from it, so the model
  does not depend on the supplement.
