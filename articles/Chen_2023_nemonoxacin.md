...
- **No IIV on Q.** Table 2 has no `omega^2 Q` row and Eq. 8 prints
  ...
  variance.
- **Supplementary material not available.** The Frontiers supplement
  (Supplementary Tables S1-S9, Figures S1-S13) was not retrievable when
  this model was built. It is not required for the model: the final
  covariate model is printed as Eq. 8 and every parameter estimate is in
  Table 2 of the main text, and the validation targets used above come
  from main-text Tables 1, 3, and 4 plus the Results narrative. The
