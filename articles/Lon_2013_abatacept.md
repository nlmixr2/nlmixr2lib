...
collagen-induced arthritic (CIA) Lewis rats using the final population
PK model from Lon et al. 2013. Abatacept (CTLA-4Ig Fc-fusion protein;
~92 kDa) blocks the CD28 co-stimulatory signal required for T-cell
activation. Lon 2013 characterized abatacept PK in a rat CIA model using
...
cohort whose arm sizes match Lon 2013 exactly (IV n = 5, SC single n =
6, SC multiple n = 6) and whose weights are sampled from a uniform
...
![](Lon_2013_abatacept_files/figure-html/figure-2-1.png)
...
- **Body weight distribution.** Lon 2013 reports only that rats weighed
  ...
  observation windows.
- **Sampling schedule.** Individual sampling times were not published in
  full detail. We use a dense early schedule (every 2-6 h for 3 days,
  twice daily through day 10, daily thereafter) that supports stable NCA
  on both the early-distribution and late-terminal phases.
...
- **PD and disease progression.** Lon 2013 also reports a PD model for
  paw edema (Table 2) built on top of these PK parameters. The PD arm is
  not packaged in this model; `Lon_2013_abatacept()` is PK-only.
...
- **Structural model:** 2-compartment with linear elimination from the
  ...
  residual in `depot`.
- **Units.** Dose in mg and volumes in mL give `central / vc` in mg/mL;
  the observation equation divides by MW (92,000 g/mol) and multiplies
  by 1e6 to report concentrations in umol/L, matching the units in Lon
  2013 Table 1.
- **IIV:** diagonal on CL and V1 only, per Lon 2013 Table 1; no IIV was
  reported on CLD, V2, F, or tau.

### Reference

- Lon HK, Liu D, DuBois DC, Almon RR, Jusko WJ. Modeling
