...
That table also reveals a covariate the published paper never mentions:
**alcohol use** was tested and dropped. The FDA review’s full-model
columns carry Alcohol on N-oxide CL (theta_11 = 0.00354, SE 0.0626) and
on N-oxide Frel (theta_17 = -0.0239, SE 0.0338), both indistinguishable
from zero. Alcohol is consequently absent from the model’s
`covariateData`, since it is not a covariate of the shipped model. See
...
(roflumilast; Nycomed, submitted 2009) reprints the sponsor’s population
PK study report 114/2005 as its **Table 6** (roflumilast parent, printed
page 77; sponsor’s Table 9) and **Table 7** (roflumilast N-oxide,
printed page 79; sponsor’s Table 12). Lahu was the sponsor’s modeller,
so the NDA review and the Clin Pharmacokinet article report the same fit
rather than two analyses.

**All 40 shipped parent and N-oxide values reproduce the FDA “Final With
Race” columns exactly – zero disagreements.** No parameter required an
authority determination and none was altered. The FDA tables add three
things the journal article’s Table I does not carry:

- **Standard errors on every final parameter.** For example parent CL
  10.5 (SE 0.490), N-oxide CL 0.883 (SE 0.0472), N-oxide Vd 65.8 (SE
  1.94), N-oxide Vd weight exponent 1.00 (SE 0.117). The
  food-on-parent-tlag coefficient’s SE of 0.296 against an estimate of
  -0.308 independently confirms the data-poor status the paper’s
  bootstrap flagged.
- **The model-building ladder** – Base / Full / Initial Final / Final
  With Race, with objective function values (parent -9023.112 /
  -9391.524 / -9383.408 / -9397.811; N-oxide -10,310.430 / -10,982.441 /
  -10,974.671 / -11,064.271) and the WAM “drop from base” and “nearness
  to full” statistics. This makes the covariate selection the paper
  describes narratively auditable, and it is what exposes the
  tested-and-dropped covariates recorded in `covariateData` (parent age
  on CL, parent and N-oxide weight, the Frel food and smoking terms, and
  alcohol).
- **Confirmation that the omegas are variances, not SDs.** The reviewer
  quotes inter-subject variability as CV percent, and each value
  reproduces `100 * sqrt(omega^2)` against the tabulated variance:
  parent CL 41.5 percent base to 36.9 percent final (sqrt(0.172),
  sqrt(0.136)); N-oxide CL 44.0 to 38.7 percent (sqrt(0.194),
  sqrt(0.150)); N-oxide Vd 28.5 to 21.2 percent (sqrt(0.0814),
  sqrt(0.0449)). Six of six exact – an external falsifier for the
  variance-scale parameterisation used in `ini()`.
...
consistent with the shipped values (parent CL “reduced by 39.4 percent”
against `e_copd_cl` = -0.394; central volume “increased by nearly
two-fold” against `e_copd_vc` = 1.84; N-oxide volume “decreased by 21
percent” against -0.214; N-oxide clearance “decreased by … 8 percent”
against -0.0785) but supplies no independent digits or SEs. The shipped
COPD block therefore remains sourced solely from Lahu 2010 Table I.
...
- **Sex-coding inversion vs canonical SEXF (resolved; the paper’s prose
  is wrong).** Lahu 2010 equations 6 and 7 use Sex = 1 for male and Sex
  = 0 for female. The canonical SEXF column inverts this (1 = female, 0
  = male). The model() block applies the source coefficients via (1 -
  SEXF) so that the model-equation intercept (when SEXF = 1) is the
  female reference. The paper’s narrative text describes the reference
  population as “male, non-smoking, White, healthy, 40-year-old”
  (Methods page 591 and Results page 599), which would conventionally
  suggest a male-baseline encoding, but the model equations clearly use
  female as the intercept (so a male subject has CL_parent = 10.5 \*
  1.191 = 12.5 L/h, consistent with the paper’s own statement on page
  602 that “clearance of roflumilast was 19 percent greater in men than
  in women”).
  ...
  reference three separate times: “the clearance (theta_3) is 10.5 +/-
  0.490 L/hr for nonsmoking, non-black, non-Hispanic **females**”
  (printed page 77); “the metabolite CL (theta_3) is 0.883 +/- 0.0472
  L/hr for a 40-year-old, 70 kg, nonsmoking **female**” (printed page
  79); and “The bioavailability (FRel) … was fixed to 1 for a
  40-year-old non-black, non-Hispanic **female**” (printed page 80). An
  independent regulatory reviewer working from the sponsor’s source
  report read the intercept exactly as this extraction does, so the
  journal article’s prose – not its equations – is the erroneous side.
  The vignette and downstream simulations use the equation-derived
...
- **No supplement on disk; demographics recovered from the FDA review
  ...
  the FDA NDA 22-522 review’s Table 5 and are tabulated in the
  ...
  reports mean and SD only – individual age and weight minima and maxima
  are still unavailable, so the virtual cohort below continues to use
  approximated ranges. No equivalent demographic table exists for the
  phase II / III COPD extension in either source.
...
- **tPDE4i, adverse-event logistic-regression layer not packaged.** Lahu
  ...
  The FDA NDA 22-522 review was checked as a possible second source for
  the adverse-event coefficients the paper withholds, and **does not
  supply them**. The string “logistic” does not occur anywhere in the
  348-page review. Its exposure-response-for-safety section is a plain
  dose-versus-incidence table (diarrhoea, nausea and weight loss at 0,
  250 and 500 micrograms) with no model and no fitted coefficients; for
  effectiveness the review states outright that the relationship between
  exposure and the primary endpoints “was not characterized” because
  concentrations were not measured in the registration trials. tPDE4i
  appears only as an ANOVA endpoint in a drug-interaction summary
  (sponsor report 252/2008), which reports treatment ratios of mean
  tPDE4i but not the composite’s defining constants. This gap therefore
  stands, and the FDA review does not need to be revisited for it.
