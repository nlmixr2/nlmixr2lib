...
- **Monte Carlo clinical simulations not reproduced.** The paper’s
  ...
  (reference 46) and fosfomycin (reference 47) – none of which was
  available when this model was built, and whose parameters are not
  reproduced in this paper. Those simulations are therefore outside the
  scope of this vignette. A user can reproduce them by overriding
  `d/dt(cpmb)`, `d/dt(cmem)` and `d/dt(cfof)` with the corresponding
