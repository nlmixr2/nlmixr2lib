...
- **SG (the ADC)** — output `Cc` in the model. Two-compartment PK with
  ...
  on CL.
- **Free SN-38 (the released payload)** — output `Csn38`.
  ...
  2002;72:638-647).
- **Total antibody (tAB)** — output `Ctab`. Two-compartment PK with
...
# (cmt = sgCentral and cmt = tabCentral) so that both compartments receive the
...
events <- cov_df |>
  ...
  do({
    ...
    dose_rows <- expand.grid(time = dose_times_h, cmt = c("sgCentral", "tabCentral"),
...
typical_events <- expand.grid(
  time = dose_times_h, cmt = c("sgCentral", "tabCentral"),
  stringsAsFactors = FALSE
)
typical_events$id  <- 1
...
#> ℹ omega/sigma items treated as zero: 'etalclSG', 'etalkrel', 'etalclSN38', 'etalclTAB', 'etalvcTAB'
```
...
sim_tab_summary <- sim_tab |>
  group_by(time) |>
  summarise(
    Q05 = quantile(Ctab, 0.05, na.rm = TRUE),
    Q50 = quantile(Ctab, 0.50, na.rm = TRUE),
    Q95 = quantile(Ctab, 0.95, na.rm = TRUE),
    .groups = "drop"
  )
...
out_bw <- lapply(bw_grid, function(wt) {
  ...
  data.frame(WT = wt, time = s$time, Cc = s$Cc, Ctab = s$Ctab)
})
#> ℹ omega/sigma items treated as zero: 'etalclSG', 'etalkrel', 'etalclSN38', 'etalclTAB', 'etalvcTAB'
#> ℹ omega/sigma items treated as zero: 'etalclSG', 'etalkrel', 'etalclSN38', 'etalclTAB', 'etalvcTAB'
#> ℹ omega/sigma items treated as zero: 'etalclSG', 'etalkrel', 'etalclSN38', 'etalclTAB', 'etalvcTAB'
...
auc_first_cycle <- out_bw |>
  ...
  summarise(
    ...
    AUC_TAB_ug_h_per_mL = trapz(time, Ctab),
    .groups = "drop"
  )
...
dose_first_cycle <- events |>
  filter(evid == 1, cmt == "sgCentral", time < 21 * 24) |>
...
names(sim_sn38_nca)[names(sim_sn38_nca) == "Csn38"] <- "Cc"
...
names(sim_tab_nca)[names(sim_tab_nca) == "Ctab"] <- "Cc"
...
- **Dosing convention.** Each SG infusion event is represented in the
  event table as TWO simultaneous dose rows (one with
  `cmt = "sgCentral"`, one with `cmt = "tabCentral"`) so that the SG and
  tAB compartments receive the same input mass at the same time. This
  matches the source paper’s setup, in which the SG and tAB models were
  fit independently using the SG dose directly.
...
- **Compartment names.** Multi-output ADC simulation requires named
  compartments (`sgCentral`, `sgPeripheral1`, `sn38Central`,
  `sn38Peripheral1`, `tabCentral`, `tabPeripheral1`) outside the
  canonical `central`/`peripheral1` set defined in
...
- **Time reference for the time-dependent CL.** The expression
  `1 - maxRedTAB/100 * (1 - exp(-keffTAB * t))` uses the rxode2
