...
- The supplementary material (model-search summary Table 1,
  ...
  virtual-population-versus-CDC-growth-chart figure) was not available
  when this model was built. It contains no final parameter estimates –
  every value in this model comes from Table 3 of the main text – so the
  gap does not affect the extraction. It does mean the model-search
  detail behind “no allometric factors on peripheral compartment
  parameters” is cited from the main text’s summary of it rather than
  read directly.
