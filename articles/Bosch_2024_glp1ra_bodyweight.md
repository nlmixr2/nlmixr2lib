...
twenty_wk <- sim |>
  ...
  dplyr::transmute(arm,
                   bwkg_at_20wk = round(BW, 2),
                   wtch_pct_at_20wk = round(100 * (BW - 105) / 105, 2))
knitr::kable(
  twenty_wk,
...
indefinitely. This is the steady-state hold check used for endogenous
models. The Bosch 2024 model is calibrated so the energy intake at
baseline (`ei_init = RMR_init * PAL = 1.6 * RMR`) equals total energy
expenditure at PAL = 1.6.
...
ss_sim <- rxode2::rxSolve(mod_typical, events = ss_events,
                          keep = c("arm"))
ss_drift_pct <- 100 * (max(ss_sim$BW) - min(ss_sim$BW)) / step5_subject$WT
