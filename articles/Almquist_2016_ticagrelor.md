...
| Observation model (closed-form quadratic) | derivation in vignette | Almquist 2016 “Observation model” paragraph, p. 317; closed-form derivation in Supplementary Text S2 (not available; the quadratic below is re-derived from the conservation + equilibrium equations in the main text) |
...
- **Observation model (closed-form ex-vivo equilibrium).** Almquist 2016
  ...
  available when this model was built). The quadratic implemented in
...
- **PD model not included.** Almquist 2016 also reports a turnover-
  ...
  parameters live in the supplement (not available) and the PD model is
