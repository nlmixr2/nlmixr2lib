...
| `lcl_time_max` (CLmax) | 320 mL/h = 0.320 L/h | Table 3 |
| `lcl_t50` (t50) | 46.4 h | Table 3 |
| `lcl_time_hill` (induction shape parameter n) | 6.40 | Table 3 |
...
| `etalcl_t50` (BSV t50) | log(0.88^2 + 1) = 0.5594 | Table 3 BSV t50 = 88% |
...
#> ℹ omega/sigma items treated as zero: 'etalcl', 'etalvc', 'etalvp', 'etalcl_t50', 'etalka'
```
...
induction <- hours^cl_time_hill / (cl_t50^cl_time_hill + hours^cl_time_hill)
...
hl_long <- dplyr::bind_rows(
  tibble::tibble(
    ...
    t1_2_alpha = vapply(induction, function(i) {
      cl_t <- cl0_healthy + cl_time_max * i
