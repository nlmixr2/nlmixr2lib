...
2.  **Sign in Eq. 2.** The published Eq. 2 prints
    ...
    increases mitotic cell loss). Following the maintainers’ standing
...
6.  **Body-weight reference.** The published `gamma_wt` = 0.629 (Fixed)
    ...
    tabulate the reference, so a future maintainer may verify against
    the underlying NONMEM control stream if it becomes available.
