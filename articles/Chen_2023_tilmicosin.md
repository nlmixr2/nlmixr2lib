...
ggplot(sims, aes(time, log_cfu, colour = arm)) +
...
reduction_tbl <- do.call(rbind, lapply(names(arm_auc), function(d) {
  ...
  data.frame(
    dose      = as.integer(d),
    simulated = s$log_cfu[s$time == 0] - s$log_cfu[s$time == 96],
...
check1 <- do.call(rbind, lapply(names(arm_auc), function(d) {
  s   <- simulate_arm(arm_auc[[d]], d)
  b   <- s$log_cfu[s$time %in% c(0, 24, 48, 72, 96)]
...
check2 <- c(per_interval_change = -diff(ctl$log_cfu[ctl$time %in% c(0, 24)]),
            e0_parameter = e0,
            log10_cfu_at_96h = ctl$log_cfu[ctl$time == 96])
...
check3 <- c(reduction_at_huge_exposure =
              -diff(simulate_arm(rep(1e6, 4), "sat")$log_cfu[c(1, 49)]),
            emax_parameter = emax)
