...
- **TBW distribution**: the paper’s MCS truncated TBW at 40 kg (Methods,
  ...
  kg.
- **Table S2 (`Additional parameters used in the model`) not
  available**: the paper references a Table S2 in the Results section.
  All parameter values needed to reproduce the model appear in the main
  text (Results paragraph 1 and Methods); the main PDF was used as the
  sole source. Table S2 is inferred from context (Methods) to
