...
| TUMTP=GC on CL | Budha 2023 Eq. 5 / Table 3 (theta14) | Exponential: exp(0.069 \* TUMTP_GASTRIC) |
| TUMTP=cHL on CL | Budha 2023 Eq. 5 / Table 3 (theta15) | Exponential: exp(-0.216 \* TUMTP_HODGKIN_CLASSICAL) |
...
| `TUMTP` (categorical: cHL, GC, NSCLC, …) | split into `TUMTP_HODGKIN_CLASSICAL` and `TUMTP_GASTRIC` indicators; reference = all other tumor types |
...
set.seed(2023)
n_subj <- 150  # downsampled from 500 for vignette build budget; VPC band shape preserved
...
TUMTP_HODGKIN_CLASSICAL <- as.integer(TUMTP == "cHL")
TUMTP_GASTRIC  <- as.integer(TUMTP == "GC")

pop <- data.frame(
  ID        = seq_len(n_subj),
  WT, AGE, ALB, TUMSZ,
  SEXF, ADA_POS,
  TUMTP, TUMTP_HODGKIN_CLASSICAL, TUMTP_GASTRIC,
...
obs_times <- sort(unique(c(
  seq(0, 21, by = 1),                       # intensive profile through cycle 1 (relaxed from by=0.5 for vignette build budget)
...
ggplot(sim_summary, aes(x = time / 7)) +
  ...
  labs(
    ...
    subtitle = "Median and 90% prediction interval (N = 150 virtual patients)",
...
|---:|---:|:---|:---|:---|:---|:---|:---|
| 0 | 21 | ADA-negative | 127 | 560 \[18.7\] | 52.5 \[16.1\] | 1.00 \[1.00, 1.00\] | 19.8 \[8.26\] |
| 0 | 21 | ADA-positive | 23 | 564 \[13.7\] | 54.3 \[12.0\] | 1.00 \[1.00, 1.00\] | 17.4 \[5.80\] |

Cycle 1 NCA summary (days 0-21), stratified by ADA status {.table}
...
conc_ss <- sim |>
  dplyr::filter(time >= 189, time <= 210) |>
  dplyr::rename(ID = id) |>
...
|------:|----:|:-------------|:----|:--------------|:--------------|:--------------|
|     0 |  21 | ADA-negative | 127 | 1160 \[29.5\] | 77.2 \[23.4\] | 39.4 \[38.0\] |
|     0 |  21 | ADA-positive | 23  | 1030 \[23.1\] | 71.7 \[16.6\] | 33.3 \[32.9\] |
