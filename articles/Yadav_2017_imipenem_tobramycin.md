...
| `log_cfu` | log10 CFU/thigh | observation: log10 of total bacterial CFU/thigh (+ 1 CFU floor) |
...
ggplot(exp1_sim, aes(time, log_cfu, color = scenario)) +
  geom_line(linewidth = 0.7) +
  scale_y_continuous(limits = c(0, 9), breaks = seq(0, 8, 2)) +
  labs(x = "Time (h)", y = "log10 CFU/thigh (log_cfu)", color = NULL,
...
ggplot(exp2_sim, aes(time, log_cfu, color = scenario)) +
  geom_line(linewidth = 0.7) +
  scale_y_continuous(limits = c(0, 9), breaks = seq(0, 8, 2)) +
  labs(x = "Time (h)", y = "log10 CFU/thigh (log_cfu)", color = NULL,
...
tob_check <- exp1_sim |>
  ...
  filter(time %in% c(0, 2, 6, 24)) |>
  select(time, scenario, log_cfu) |>
  tidyr::pivot_wider(names_from = scenario, values_from = log_cfu)
...
mono_check <- bind_rows(exp1_sim, exp2_sim) |>
  ...
  select(scenario, time, log_cfu) |>
  tidyr::pivot_wider(names_from = time, values_from = log_cfu,
                     names_prefix = "t_")
...
- **Single observation `log_cfu` carries log10 CFU/thigh, not a drug
