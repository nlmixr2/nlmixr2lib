...
and Figures S1-S12) were available when this model was built. The three
...
# lambda_z skips points below 1e-6 of the peak (no relative accuracy left there).
...
conc_all <- dplyr::bind_rows(
  ...
  # Per subject and analyte, keep Cc >= 1e-6 * Cmax after the peak: below that
  # the ODE integrator has no relative accuracy left. The assay-censored zeros
  # set above are kept.
  dplyr::group_by(treatment, id) |>
  dplyr::filter(time <= time[which.max(Cc)] | Cc == 0 | Cc >= 1e-6 * max(Cc)) |>
  dplyr::ungroup()
