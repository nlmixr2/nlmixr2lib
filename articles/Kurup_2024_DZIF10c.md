...
- **Body-weight distribution.** The paper sampled 1,000 subjects per arm
  ...
  Those microdata were not available, so weight here is lognormal with
...
- **No observed data available.** Individual observed concentrations are
  not public; every figure and table here is a simulation from the
  packaged model compared against the published summary values.
