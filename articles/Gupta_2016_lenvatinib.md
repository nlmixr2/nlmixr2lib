...
n_per_dose <- 40  # downsampled from 100 for vignette build budget; AUC envelopes remain stable
...
make_cohort <- function(n, dose_mg, id_offset = 0L) {
  ...
  cov_tbl <- tibble::tibble(
    ...
    ALB          = alb_gL,
    ALP          = alp_uL,
    CONMED_CYP3A4_IND   = cyp_ind,
    CONMED_CYP3A4_INH   = cyp_inh,
    DIS_HEALTHY  = 0L,
    FORM_CAPSULE = 0L,
...
sim <- rxode2::rxSolve(
  mod, events = events,
  keep = c("treatment", "WT", "ALB", "ALP", "CONMED_CYP3A4_IND",
           "CONMED_CYP3A4_INH", "DIS_HEALTHY", "FORM_CAPSULE")
) |>
  as.data.frame()
...
sd_events <- tibble::tibble(
  id = seq_len(150),  # downsampled from 200 for vignette build budget; VPC band shape unchanged
  WT = pmin(pmax(rnorm(150, 76.7, 19.4), 32.6), 177.5),
  ALB = pmin(pmax(rnorm(150, 39.8, 5.1), 19), 52),
  ALP = pmin(pmax(rlnorm(150, log(77), 0.7), 19), 752),
  CONMED_CYP3A4_IND = 0L, CONMED_CYP3A4_INH = 0L,
...
make_typical_subject <- function(WT, ALB, ALP, CONMED_CYP3A4_IND, CONMED_CYP3A4_INH,
                                 DIS_HEALTHY, FORM_CAPSULE, label) {
  tibble::tibble(
    id = 1L, WT = WT, ALB = ALB, ALP = ALP,
    CONMED_CYP3A4_IND = CONMED_CYP3A4_IND, CONMED_CYP3A4_INH = CONMED_CYP3A4_INH,
...
build_typical_events <- function(cov_row, dose_mg = 24) {
  dose_d <- tibble::tibble(
    ...
    CONMED_CYP3A4_IND = cov_row$CONMED_CYP3A4_IND, CONMED_CYP3A4_INH = cov_row$CONMED_CYP3A4_INH,
...
|:----------|-----------------:|----------------:|-------------------:|------------:|
| 14 mg QD  |               NA |          242.27 |              27.52 |        1.50 |
| 20 mg QD  |               NA |          344.48 |              43.36 |        1.47 |
| 24 mg QD  |               NA |          432.51 |              58.62 |        1.53 |
| 32 mg QD  |               NA |          542.62 |              68.56 |        1.51 |
| 4 mg QD   |               NA |           66.83 |               8.93 |        1.50 |
| 8 mg QD   |               NA |          135.58 |              16.73 |        1.48 |
...
- **New canonicals introduced by this extraction.** Two new entries are
  ...
  scope; alkaline phosphatase continuous concentration) and
  `CONMED_CYP3A4_IND` (general scope; concomitant CYP3A4 inducer
  indicator, sibling to the existing `CONMED_CYP3A4_INH`). The
  `FORM_CAPSULE` entry is extended to acknowledge tablet as a per-paper
  non-capsule reference (the prior Hennig 2006/2007 entries used
