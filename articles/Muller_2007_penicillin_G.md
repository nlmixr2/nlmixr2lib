...
| `e_wt_cl` (exponent of WT/1.195 on CL) | fixed(1.0) | Imputed (maintainer decision); see Errata |
...
- **Body-weight effect on CL – imputed linear scaling at the cohort-mean
  reference (maintainer decision).** Muller 2007 Results page 3724 and
  Figure 3 state that body weight was retained on CL in the final model
  (P \< 0.01, improved fit), but neither the functional form (linear /
  power / allometric) nor the coefficient is printed in the paper. Per
  the maintainers’ decision, the packaged model encodes CL = CL_ref \*
  (WT / 1.195 kg)^1, with the reference weight set to the cohort mean
  (Muller 2007 Table 1) and the exponent fixed at 1. CL at the reference
  subject is 0.103 L/h (Table 2). This is a defensible imputation that
  matches the paper’s qualitative claim “CL increased significantly with
...
- **IIV second-volume term assigned to V1 (central) – Table 2 vs body
  text discrepancy (maintainer decision).** Muller 2007 has an internal
  inconsistency: Table 2 (page 3723) explicitly labels the row
  ...
  maintainers’ decision, the packaged model encodes IIV on V1 (the
  central volume) as `etalvc ~ 0.39`. This matches the parameter
  estimates table; reviewers comparing against the paper’s narrative
  text should expect the table-vs-text discrepancy. A future erratum or
  author-correspondence resolution may flip this to V2; users who want
  the V2-IIV alternative can edit the packaged file to use
  `etalvp ~ 0.39` (with `etalvc` removed) and rebuild.
