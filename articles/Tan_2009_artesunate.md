...
#> ℹ omega/sigma items treated as zero: 'etalcl', 'etalvc', 'etalka', 'etalcl_dihydroart', 'etalvc_dihydroart'
...
- **Unit choice.** The packaged model carries dose in mg, volume in L,
  ...
  values apply directly. Multiplying `Cc` and `Cc_dihydroart` by 1000
  gives ng/mL, which is the unit the paper uses in its narrative.
...
- **No IIV on Q/F or V4/F.** The source paper held the IIV variances on
  ...
  `etalq_dihydroart` and `etalvp_dihydroart`.
