...
is the structure `rxSolve()` recognises and can rewrite into a
closed-form `linCmt()` solution. That closed form can only carry a
**constant** elimination rate constant, so through rxode2 5.1.6 the
rewrite silently substituted `kel` at `t = 0` for all time and discarded
the entire time dependence – the whole subject of this paper. The
failure was silent, one-sided and large: under the default flag every
interval collapsed to exactly `Dose / (kel(0) * vc)`.

rxode2 5.1.7 closed that hole from the other side. It now refuses the
ODE-to-`linCmt()` conversion when the conversion would change a rate or
when the model carries an exogenous input term, which covers this model,
so the rewrite no longer happens and both flags give the same answer.
The check below solves the same regimen both ways and asserts exactly
that: the time dependence survives, and the two paths agree.

Passing the flag explicitly is still worth doing. It states which solver
path the numbers came from instead of inheriting a package default – and
that default itself changed in rxode2 5.1.8, from `TRUE` to `FALSE`.
...
tibble::tibble(
  ...
  knitr::kable(
    caption = paste(
      ...
      "is zero. Before 5.1.7 the converted column did not move after the first",
      "interval, holding kel at its t = 0 value."
    )
  )
```
...
model, so both columns rise as clearance falls and the under-prediction
is zero. Before 5.1.7 the converted column did not move after the first
interval, holding kel at its t = 0 value. {.table}

``` r


# Regression guard, in two parts. The first is the physics and has always held:
# clearance falls with time, so interval AUC must rise. The second used to
# assert the linCmt column was FLAT -- the defect. rxode2 5.1.7 refuses the
# conversion for this model, so the two paths now agree instead, and asserting
# that catches both a reintroduced rewrite and a silent divergence.
stopifnot(trap_ode[8] / trap_ode[1] > 1.15,
          max(abs(trap_default / trap_ode - 1)) < 1e-6)
```
