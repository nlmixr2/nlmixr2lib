...
- **The peripheral transfer is stored as `q` / `vp`, not as the
  ...
  **one-compartment** solution and silently discards `peripheral1`. The
  ...
  `Q/F = K12 * V1/F` and `V2/F = Q/F / K21` are exact, and the
  arithmetic is written out in each `ini()` so the published
  micro-constants remain visible. The simulation chunk asserts the
  simulated terminal slope equals the analytic beta root, which would
  fail if a future edit reintroduced the collapse.
