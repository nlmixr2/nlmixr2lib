...
- Article (the publication this model is attributed to):
...
PDF was not available when this model was built, so external
cross-checks against published tables/figures were not possible. See the
Errata section for how the .lst’s final estimates were used in lieu of a
published parameter table, and how a unit convention in the source
NONMEM model was reconciled against the simulated dataset shipped in the
bundle.
...
the article PDF was not available; the demographics of the bundle’s
`Simulated_dataset.csv` (60 simulated subjects) provide the only
available reference and span body weight 45-128 kg (mean ~78 kg) and raw
measured GFR 19-401 mL/min. The wide GFR range covers severe renal
impairment through to augmented renal clearance, consistent with a
critically ill ICU pneumonia population.
...
not be reconstructed from the available material; the virtual cohort
below mirrors the bundle’s covariate ranges.
...
interval to support PKNCA. This is a two-endpoint model (plasma `Cc` is
`dvid = 1`, ELF `Celf` is `dvid = 2`), so observation rows identify the
endpoint with the `dvid` column rather than a `cmt`. `rxSolve` returns
both `Cc` and `Celf` as columns on every observation row regardless of
which endpoint the row nominates, so a single `dvid = 1` observation
series is enough to recover both trajectories.
...
# Doses go into the ODE state `central` (the plasma compartment, V1).
# Observation rows use the endpoint id `dvid = 1` (plasma). For a
# multi-endpoint model rxSolve maps observation rows to prediction
# endpoints via `dvid`; addressing an observation by `cmt` (the
# algebraic-observable name "Cc"/"Celf" or its auto-injected slot)
# does NOT match the endpoint's internal modeled-compartment id and
# fails translation. `dvid` is the semantic handle for the endpoint
# and is robust to slot renumbering.
dose_rows <- cohort |>
...
obs_rows <- cohort |>
  ...
  transmute(id = id, time = time, evid = 0L, amt = 0, rate = 0,
            dvid = 1L, ss = 0L, ii = 0, WT = WT, CRCL = CRCL)
...
The publication PDF was not available, so the figures below are not
direct figure-by-figure replications of the article. Instead they
provide two kinds of sanity check:
...
ref_events <- dplyr::bind_rows(
  ...
  tibble(id = 1L, time = obs_t, evid = 0L, amt = 0, rate = 0,
         dvid = 1L, ss = 0L, ii = 0, WT = 75, CRCL = 65)
) |>
  dplyr::arrange(id, time, evid)
...
# plus one observation row per ELF observation in the bundle. Doses go
# into the ODE state `central`; ELF observations nominate the ELF
# endpoint with `dvid = 2` (Celf). For a multi-endpoint model rxSolve
# maps observation rows to prediction endpoints by `dvid`; both Cc and
# Celf are returned as columns on every row regardless.
...
|---:|---:|:---|:---|:---|:---|:---|:---|:---|
| 0 | 8 | meropenem 1g q8h IV (SS) | 200 | 118 \[65.8\] | 53.6 \[31.8\] | 3.68 \[228\] | 0.500 \[0.500, 0.500\] | 14.8 \[65.8\] |
...
| Interval Start | Interval End | treatment | N | AUClast (hr\*mg/L) | Cmax (mg/L) | Cmin (mg/L) | Tmax (hr) | Cav (mg/L) |
|---:|---:|:---|:---|:---|:---|:---|:---|:---|
| 0 | 8 | meropenem 1g q8h IV (SS) | 200 | 29.3 \[65.9\] | 7.88 \[60.5\] | 1.17 \[206\] | 1.00 \[0.500, 3.00\] | 3.67 \[65.9\] |
...
The Themans 2019 publication PDF was not available when this model was
built, so a side-by-side comparison against the article’s reported Cmax
/ AUC tables (if any) was not performed. The validation above relies on
(a) self-consistency between the model translation and the bundle’s
`Simulated_dataset.csv` ELF observations and (b) the qualitative
biological plausibility of the simulated trajectories.

## Assumptions and deviations

- **Article PDF not available.** The publication ([Drugs in R&D,
...
- **Journal mismatch with `Model_Accomodations.text`.** The bundle’s
  ...
  submission (“submitted - July 2019, scenario = 4”); the publication
  this model is attributed to is the Drugs in R&D 2019 article. These
  are treated as the same body of work at different stages of journal
  review.
