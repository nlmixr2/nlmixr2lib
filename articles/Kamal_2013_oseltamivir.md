...
eta scale: (etalcl, etalcl_oselcarb) and (etalvp, etalvc_oselcarb).
Residual error is proportional only for OP (40.5 % CV) and combined
additive plus proportional for OC (14.0 % CV + 17.9 ng/mL).
...
| ODE: `d/dt(central_oselcarb) = kel * central - kel_oselcarb * central_oselcarb` | n/a | Figure 1 schematic (‘direct conversion of oseltamivir to OC’); Discussion page 3473 (‘oseltamivir is completely converted to the OC metabolite’) |
...
ggplot(sim_crcl_ss, aes(t_in_interval, Cc_oselcarb, colour = group)) +
...
auc24 <- sim_crcl_ss |>
  group_by(group) |>
  summarise(AUC_0_24 = PKNCA::pk.calc.auc(Cc_oselcarb, t_in_interval,
                                          method = "linear"),
            Cmax     = max(Cc_oselcarb),
            .groups = "drop")
...
ggplot(sim_wt_ss, aes(t_in_interval, Cc_oselcarb, colour = wt_band)) +
  geom_line(linewidth = 0.7) +
  facet_wrap(~ age_band) +
  scale_y_continuous(name = "OC concentration (mg/L)") +
  scale_x_continuous(name = "Time after morning dose at steady state (h)",
...
wt_age_summary <- sim_wt_ss |>
  ...
  summarise(Cmax = max(Cc_oselcarb),
            AUC_0_24 = PKNCA::pk.calc.auc(Cc_oselcarb, t_in_interval, method = "linear"),
            .groups = "drop")
...
sim_vpc_ss <- sim_vpc |>
  ...
  summarise(Q05_oselcarb = quantile(Cc_oselcarb, 0.05),
            Q50_oselcarb = quantile(Cc_oselcarb, 0.50),
            Q95_oselcarb = quantile(Cc_oselcarb, 0.95),
...
conc_oselcarb  <- nca_grid |> select(id, time, Cc_oselcarb, group) |>
  rename(Cc = Cc_oselcarb)
dose_oselcarb  <- nca_grid |>
  group_by(id, group) |>
...
conc_obj_oselcarb <- PKNCA::PKNCAconc(conc_oselcarb, Cc ~ time | group + id)
dose_obj_oselcarb <- PKNCA::PKNCAdose(dose_oselcarb, amt ~ time | group + id)

intervals_oselcarb <- data.frame(
  start = 0, end = 24,
  cmax = TRUE, tmax = TRUE, auclast = TRUE, half.life = TRUE
)

nca_data_oselcarb <- PKNCA::PKNCAdata(conc_obj_oselcarb, dose_obj_oselcarb, intervals = intervals_oselcarb)
nca_res_oselcarb  <- PKNCA::pk.nca(nca_data_oselcarb)

nca_table_oselcarb <- as.data.frame(nca_res_oselcarb$result) |>
  select(group, PPTESTCD, PPORRES) |>
  pivot_wider(names_from = PPTESTCD, values_from = PPORRES)

knitr::kable(nca_table_oselcarb, digits = 3,
...
dose_obj_op <- PKNCA::PKNCAdose(dose_oselcarb, amt ~ time | group + id)
...
- **Residual-error notation.** Kamal 2013 Table 2 labels the residual
  ...
  encoded as `propSd`, `propSd_oselcarb`, and `addSd_oselcarb`
  respectively on the linear concentration scale (mg/L for the OC
  additive SD), and the cross-check at the bounds reproduces the paper’s
  reported combined-error range of “49.8 % to 14.3 % CV for plasma OC
