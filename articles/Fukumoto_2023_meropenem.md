...
- **Screened but unretained covariates.** Age, albumin, serum
  ...
  any of them (the screening table, Supplementary Table S2, was not
  available when this model was built), so they are recorded in the
  model file’s `covariatesDataExcluded` metadata as documentation only.
  In particular the absence of a weight covariate means V1, V2 and Q are
  absolute values for this cohort (median weight 53.7 kg) and should not
  be extrapolated to markedly heavier populations without re-estimation.
- **Supplements not available.** Supplemental Digital Content 1-5
