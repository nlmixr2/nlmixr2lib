...
Steady-state NCA over the final 72-h dosing interval. The treatment
grouping variable goes before `id` in the formula per project
convention. PKNCA reports `cmax`, `tmax`, `cmin`, `cav` and `auclast`
over the interval; the interval is the entire 72-h patch wear period.
