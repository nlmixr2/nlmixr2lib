...
Every display equation in this paper is lost by text extraction (it
renders as `formula-not-decoded`). Equations 1-6 were read from a 500
dpi raster render of the PDF pages, and Table 2 was re-read at the same
resolution.
...
3.  **The published PK claims about low doses are not reproduced by the
    ...
    in the paper. Per standing policy the model file carries the printed
    values and equations exactly; no parameter was tuned toward the
    figure.
...
- **No published NCA table exists for this paper.** The noncompartmental
  ...
  Cancer Res 2014;20:3581-8) and is not on disk, so the NCA section
