...
deliberately not used. What the paper *does* provide is far stronger:
**13 published answer keys**. For each reconstructed case, Table 2 gives
an air concentration and an exposure time that the model must map back
onto the measured left and right cardiac cyanide concentrations. Those
are checked below, followed by the reverse direction – re-running the
paper’s grid search to see whether it returns the published estimates.
