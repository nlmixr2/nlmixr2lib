...
not on disk, so a side-by-side publication-table comparison is also out
of scope here.
...
- **The Kloft 2004 publication is not on disk.** The package metadata
  (description, units, citation, DOI) reflects the publication, but a
  side-by-side comparison against the published parameter table or NCA
  summary is not part of this vignette. The validation here is
