...
|----|----|----|
| `lkel` (KDE fosdagrocorat) | `fixed(log(0.597))` /week | Table 2 OC: “KDE Fosdagrocorat FIX” (from P1NP fit) |
| `dlkel_pred` | `fixed(log(0.535/0.597))` | Table 2 OC: “KDE Prednisone FIX” (from P1NP fit) |
...
| `etalkel` | `omega^2 = 1.5129` (123% CV) | Table 2 OC, “IIV %CV \[g_KDE\]” |
...
- **Drug arm switching via reparameterization.** Same convention as the
  ...
  the P1NP estimates – both `lkel` and `dlkel_pred` are wrapped in
  `fixed()`.
