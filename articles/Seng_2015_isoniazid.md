...
- Citation: Seng K-Y, Hee K-H, Soon G-H, Chew N, Khoo SH, Lee LS-U
  (2015). Population pharmacokinetic analysis of isoniazid,
  acetylisoniazid, and isonicotinic acid in healthy volunteers.
...
- Description: Parent-and-metabolites population PK model with a
  ...
  intermediate / slow) selects between three typical CL/F values;
  creatinine clearance enters the AcINH clearance as a power-law
  covariate.
...
`inst/modeldb/specificDrugs/Seng_2015_isoniazid.R` carries an inline
comment pointing to Seng 2015 Table 2 (final-model column, page 6794).
The table below collates the high-level source mapping for review.
...
A typical-value (no between-subject variability) trajectory per
phenotype illustrates the structural model and the NAT2-driven CL/F
selection.
...
![](Seng_2015_isoniazid_files/figure-html/figure-2-typical-1.png)
...
![](Seng_2015_isoniazid_files/figure-html/figure-4-vpc-1.png)
...
![](Seng_2015_isoniazid_files/figure-html/figure-5-cmax-1.png)
...
- **NAT2 phenotype encoding.** The three-level rapid / intermediate /
  ...
  NAT2_RAPID and NAT2_SLOW (registered in
  `inst/references/covariate-columns.md` alongside this extraction;
  joint reference both = 0 is the intermediate group). Seng 2015’s
  NONMEM source-paper parameterisation has the rapid (fast) acetylator
  group as the structural reference and reports proportional CL/F
  reductions of 0.5 (intermediate) and 0.9 (slow); the packaged model
  reproduces these by storing the three typical-CL/F values explicitly
  (`lcl_fast`, `lcl_int`, `lcl_slow`) and selecting between them by
  indicator algebra in `model()`. A single IIV on CL/F (`etalcl`, 14.4%)
  is shared across all three phenotypes per Seng 2015 Discussion page
  6797 (“the interindividual variability in CL/F cannot be estimated
...
- **Allometric scaling on fixed volumes.** Seng 2015 fixes the AcINH
  ...
  describes “all clearance and volume terms” as allometrically scaled by
  body weight (Results page 6794). The packaged model applies the
...
- **Metabolite mass balance.** Seng 2015 expresses concentrations of INH
  (137 g/mol), AcINH (179 g/mol) and INA (123 g/mol) in mg/L without
  ...
  same mass basis the source paper modelled and reported. Users who need
  molar mass balance can convert post-hoc with the molecular weights
  above.
