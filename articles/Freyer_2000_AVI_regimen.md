...
- [`modellib("Freyer_2000_ifosfamide")`](https://nlmixr2.github.io/nlmixr2lib/reference/modellib.md)
  ...
  and day-2 clearances collapsed to their mean per maintainer
  instruction.
...
maintainers directed a single mean clearance
`(5.6 + 7.95)/2 = 6.775 L/h`.
...
- **Ifosfamide day-1 to day-2 autoinduction of CL is not represented.**
  ...
  mechanism – each day’s samples were fit under its own CL. Per
  maintainer instruction, this extraction collapses the two values to
  their arithmetic mean 6.775 L/h. The consequence is a ~12-15% bias in
  day-1 AUC (see PKNCA comparison above). Users who need the
  day-conditional CL can substitute a piecewise CL in `model()` –
...
- **Correspondence-author track was attempted then closed.** Author
  ...
  peripheral-parameter gap, and the maintainers selected it on
  2026-06-21, but no reply was received before this model was built; the
  maintainers then chose the 1-cmt fallback (with the day-1 / day-2 CL
  averaging for ifosfamide).
