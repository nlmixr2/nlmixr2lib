...
#> 
#> $notes
#> [1] "Baseline body weight, age, and hepatic function were tested as PK covariates and were not retained in the final model (drop in objective function value < 3.84). Demographic detail (age, weight, sex, race) is summarised in Appendix Table S1, which was not available when this model was built; the population fields above therefore record disease state, dosing, and dose-limiting toxicities verbatim from the main text (Analysis Plan and Results). Trial registration: NCT03854227."
```

## Source trace

The per-parameter origin is recorded as an in-file comment next to each
`ini()` entry. Every numeric value below was taken from Table 1 of Guo
et al. (2022, “PF-06939999 PK and PD (SDMA and platelet count) model
parameters”, page 1623). The IIV %CV values from Table 1 were converted
to log-scale variances via the exact log-normal relationship omega^2 =
log(1 + CV^2); the residual error variances were converted to SDs via
sqrt.
...
|:----------|--------:|-------------:|--------------------------:|
| 4 mg q.d. |       4 |         0.85 |                      0.09 |
| 6 mg q.d. |       6 |         0.96 |                      0.20 |
...
|---:|---:|:---|:---|:---|:---|:---|:---|:---|
| 648 | 672 | 4 mg q.d. | 200 | 350 \[48.4\] | 19.1 \[39.5\] | 11.4 \[58.6\] | 6.00 \[6.00, 6.00\] | 14.6 \[48.4\] |
| 648 | 672 | 6 mg q.d. | 200 | 546 \[48.1\] | 29.6 \[38.5\] | 18.0 \[58.8\] | 6.00 \[6.00, 6.00\] | 22.8 \[48.1\] |
| 648 | 672 | 8 mg q.d. | 200 | 740 \[44.5\] | 40.0 \[36.4\] | 24.4 \[53.4\] | 6.00 \[6.00, 6.00\] | 30.8 \[44.5\] |
...
- **Day-1 low-dose bioavailability scaling factor (Table 1, “Scaling
  ...
  in `ini()` are the published values, which were fit *with* the F
  scaling in place; consequently this model will **over-predict cycle 1
  day 1 Cmax / AUC for low-dose cohorts** (the bias is ~1/0.647 ~ 1.55x
  for cycle 1 day 1 only). Steady-state predictions and predictions for
  any dose \> 6 mg are unaffected. Users wanting cycle-1-day-1 fidelity
  should multiply F by 0.647 manually for the qualifying dose events.
...
- **Population demographics (age, weight, sex, race, region)** are
  reported in the source’s Appendix Table S1, which was not available;
  the `population` field in the model file therefore records dosing,
  disease state, and DLT counts verbatim from the main text (Analysis
  Plan and Results sections) and leaves the demographic numerics as
  `NA_*_`.
