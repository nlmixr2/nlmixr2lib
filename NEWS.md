...
* Add Svensson 2017 bedaquiline ([doi:10.1093/jac/dkx317](https://doi.org/10.1093/jac/dkx317)) -- adults with drug-resistant pulmonary tuberculosis (TMC207-C208 Phase IIb trial; PD-only mycobacterial-load decline model with bedaquiline Emax effect on half-life; ratifies new `DIS_TB_XDR` and `TTP_MGIT_BASE` covariate canonicals and the `Cav,W` source alias on `CAV`).
