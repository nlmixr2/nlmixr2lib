...
* Add Velez de Mendizabal 2013 multiple-sclerosis CEL dynamics ([doi:10.1371/journal.pone.0073361](https://doi.org/10.1371/journal.pone.0073361)) -- adults with relapsing-remitting multiple sclerosis (negative-binomial first- and second-order Markov count model on monthly contrast-enhancing-lesion counts with a corticosteroid-administration effect on the first-order Markov coefficient; ratifies new `PPDV` second-order Markov-state covariate canonical and broadens the existing `PDV` and `CONMED_STEROID` canonicals to cover MS lesion counts and time-varying steroid administration).
