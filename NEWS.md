...
* Add Moein 2022 etrolizumab two-compartment population PK model with first-order SC absorption and time-dependent clearance (Maxred × (1 − exp(−Onset × t_since_dose))) in moderately-to-severely active ulcerative colitis (pooled analysis of 646 patients / 6156 concentrations across four phase 1/2/3 studies). Final covariates are body weight on CL/F and Vc (allometric exponents 0.872 and 0.788 relative to 72 kg), serum albumin and CRP on CL (power and exponential models respectively, reference 41 g/L and 4.23 mg/L), ADA titer on CL (exponential), prior anti-TNF use on CL (+4.9%), and disease-extent indicators (`DISEXT_EP`, `DISEXT_OTHER`) on CL. Built to the `extract-literature-model` skill conventions (block-correlated `etalcl` + `etalvc`, diagonal `etalfdepot` on logit-transformed bioavailability, combined `propSd`/`addSd` residual, canonical `WT` / `ALB` / `CRP` / `ADA_TITER` / `PRIOR_TNF` / `DISEXT_EP` / `DISEXT_OTHER` covariate columns) with a companion validation vignette that reproduces the paper's typical-CL time trajectory (Figure S3 analogue), runs PKNCA for single-dose and steady-state NCA over a Q4W regimen, and reproduces the published terminal half-lives (13.0 days single-dose, 17.1 days steady-state) within 1.4%. Registers `CRP`, `ADA_TITER`, `PRIOR_TNF`, `DISEXT_EP`, and `DISEXT_OTHER` as new canonical covariate-column names in the `extract-literature-model` skill. The `PHASE12` study-phase residual-error covariate from the source paper is omitted as a deviation (nlmixr2's residual-error DSL does not support per-observation error magnitudes keyed on a categorical covariate).
