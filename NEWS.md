...
* Add Wang 2018 daclatasvir + asunaprevir ([doi:10.1038/aps.2017.84](https://doi.org/10.1038/aps.2017.84)) -- adults with genotype-1 chronic hepatitis C (integrated MBMA PK + Neumann viral-dynamic model; ratifies new `HCV_GT1B` and `FORM_ASV_LIQUID` covariate canonicals, new `infected` and `virus` compartment canonicals, and new `asv` registered non-parent-drug suffix).
