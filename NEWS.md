...
* Add Chua 2025 mirikizumab two-compartment population PK model with first-order SC absorption in patients with moderately-to-severely active Crohn's disease, replicating the VIVID-1 phase 3 final popPK analysis (n=711, 5318 observations; NCT03926130). Final covariates are body weight on CL, Q, Vc, and Vp; serum albumin and C-reactive protein on CL; and BMI on the logit of subcutaneous bioavailability. Published rate parameters (h^-1) converted to day^-1 for nlmixr2lib time-unit convention. Built to `extract-literature-model` skill conventions (canonical `WT` / `ALB` / `CRP` / `BMI` covariate columns, `etalcl` / `etalvc` / `etalvp` / `etalogitfdepot` block, log-normal IIV converted from %CV via `omega^2 = log(CV^2 + 1)`) with a companion validation vignette that replicates the paper's steady-state IV and SC exposures (Cmax,ss, AUC_tau,ss) via PKNCA and documents the BMI-centering and residual-error assumptions required where the paper did not publish those values. Registers `CRP` (standard C-reactive protein, distinct from `hsCRP`) and `BMI` as new canonical covariate-column names. The SERENITY phase 2 model reported alongside VIVID-1 in the same paper is not replicated in this package.
