...
* Add Siebinga 2023 [177Lu]Lu-PSMA-617 ([doi:10.1002/psp4.12914](https://doi.org/10.1002/psp4.12914)) -- men with low-volume early-stage metastatic prostate cancer receiving two cycles of PSMA-targeted radioligand therapy (six-compartment population PK dosimetry model fit to serial SPECT/CT organ and tumor activity plus blood samples, with saturable salivary-gland binding); ratifies new canonical bare `salivary_gland` compartment, the `lbmax` / `bmax` maximum-binding-capacity names, the `kin_<compartment>` / `kout_<compartment>` tissue-exchange rate-constant family, and the `cal_slope_<assay>` / `cal_int_<assay>` / `cal_bias_<matrix>` assay-calibration parameter canonicals.
