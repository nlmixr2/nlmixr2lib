...
* Add Ribba 2022 circulating tumor DNA and tumor size ([doi:10.3389/fphar.2022.1058220](https://doi.org/10.3389/fphar.2022.1058220)) -- adults with advanced non-small cell lung cancer (OAK study); three Stein bi-exponential models (ctDNA alone, tumor size alone, and a coupled fit) and the library's first ctDNA-modality models. Ratifies the new `CTDNA` covariate canonical, the `growth_ctdna` / `shrink_ctdna` / `ctdna` compartment canonicals, and the `zeta` cross-endpoint decay-rate link parameter; backfills register entries for the pre-existing `kge` / `kse` Stein rate constants.
