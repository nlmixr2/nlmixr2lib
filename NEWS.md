...
* Add Masters 2022 avelumab two-compartment population PK model with time-dependent clearance (Hill-type decrease, Imax = -0.08533, T50 = 99.24 days, gamma = 2.086) in patients with advanced solid tumors, pooled across JAVELIN Renal 100/101 aRCC cohorts and the prior Wilkins 2019 solid-tumor monotherapy dataset (N = 2315). Allometric body-weight scaling on CL, V1, V2 (estimated exponents) and Q (fixed exponent = 1), reference weight 80 kg. Built to `extract-literature-model` skill conventions with structured `covariateData` (WT), `population` metadata, source-trace comments, canonical `etalcl + etalvc + etalvp` block plus diagonal `etalImax`, and combined proportional + additive residual error. The in-file source-trace comment and vignette *Assumptions* section document the Table 1 θ_Q typo (printed 0.3352 L/h corrected to 0.03352 L/h based on the paper's own RSE and 95% CI). Companion validation vignette compares the 800 mg flat-dose and 10 mg/kg weight-based Q2W regimens and runs PKNCA on the third dosing interval.
