...
* Add Kotani 2022 astegolimab two-compartment population PK model in adults with severe asthma from the Zenyatta Phase 2b trial (NCT02918019). Covariate effects: shared allometric exponents on `(CL, Q)` and `(Vc, Vp)`; power effects of baseline eGFR and baseline blood eosinophil count on CL; a `DOSE_70MG` indicator that multiplicatively reduces relative bioavailability by 15.3% in the 70 mg SC Q4W arm. IIV on Frel uses the Box-Cox transformation (Petersson et al. 2009, shape −2.81). Combined additive + proportional residual error. Registers `BEOS` (baseline blood eosinophils) and `DOSE_70MG` as new canonical covariate columns. Built to the `extract-literature-model` skill conventions with per-parameter source-trace comments (all from Table 2) and a validation vignette that reproduces the VPC by dose group and checks terminal half-life against the paper's 19.6 days.
