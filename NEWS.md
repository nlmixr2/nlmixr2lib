...
* SI-unit inline conversions across all 26 affected model files (PR 6b follow-up to the 2026-06-19 register-level SI standardization). Every model file that was previously calibrated against a US-convention concentration now carries an inline conversion line in its `model()` block so the canonical SI input (g/L for ALB/TPRO/CSF_TPRO/IGG/IGM; umol/L for TBILI/DBIL) is converted to the original calibration unit before use in equations. Conversion variables are named `<canonical>_<calibration_unit>` (e.g. `alb_gdL`, `tbili_mgdL`) with an inline comment showing the conversion factor; the structural coefficients calibrated against the original published reference value (e.g. ALB 4.1 g/dL in Fasanmade 2009) stay load-bearing. Models that listed a clinical-chemistry covariate as "screened but not retained" (no equation reference) have their `units` field updated to SI without an inline conversion. Affects 20 ALB g/dL models, 4 TBILI mg/dL models, 1 ALB mmol/L model (de Winter 2009), 1 TPRO mg/dL model (Soy 2015); the Goel 2016 Sonidegib TBILI "fraction of ULN" column is documented as out-of-scope for the SI canonical (different semantic). Mathematical equivalence is by construction: the conversion factor (g/L * 0.1 = g/dL; umol/L / 17.1 = mg/dL; g/L / 66.5 = mmol/L) is applied inline so that SI input of equivalent magnitude (e.g. ALB = 41 g/L = 4.1 g/dL) reduces to the same equation as the prior US-convention input.
* Canonical-register standardization (2026-06-19 audit, disease-prefix renames): renamed `HYPERT` -> `DIS_HYPERT`, `DIAB` -> `DIS_DIAB` (merging the prior parallel `T2DM` canonical into `DIS_DIAB`; per-paper Type-2 detail is now documented in `covariateData[[DIS_DIAB]]$source_name` / `notes`), `CLD_PREM` -> `DIS_CLD_PREM`, `MM` -> `DIS_MM`, `MDSAML` -> `DIS_MDS_AML`, `AMLOAD` -> `DIS_AMYLOID_LOAD`, `AMLIVER` -> `DIS_AMYLOID_LIVER`, and `NWLS` -> `NEW_LESION`. The old canonical names are preserved as `source_aliases` so existing covariate-data CSVs / event tables that use the old column names continue to work for at least one release cycle. Affects `Chen_2022_guselkumab.R`, `Yao_2018_guselkumab.R`, `Overgaard_2019_semaglutide.R`, `Sharma_2018_naltrexone_bupropion.R`, `Lu_2014_sglt_qsp.R`, `Robbie_2012_palivizumab.R`, `Ogasawara_2020_durvalumab.R`, and `Sahota_2015_miridesap.R`.
* Canonical-register standardization (2026-06-19 audit, CONMED + race + phase + typo cleanups): renamed `CYP3A4_INH` -> `CONMED_CYP3A4_INH` and `CYP3A4_IND` -> `CONMED_CYP3A4_IND` (folds these concomitant-medication indicators into the `CONMED_<concept>` family); `RACE_NEAS` -> `RACE_ASIAN_NORTHEAST` (spell out "Northeast" rather than the opaque `NEAS` abbreviation); generic study-phase canonicals `PHASE1` / `PHASE2` -> paper-specific `STUDY_NIPOCALIMAB_PHASE1` / `STUDY_FARLETUZUMAB_PHASE2` (because the two papers pick opposite reference categories, the generic single-token names conflated their encodings); `STDY_VORI` -> `STUDY_VORI` (typo correction: missing vowel); compartment `total_IgG` -> `total_igg` (all-lowercase convention). Old names are preserved as `source_aliases`. Affects `Yassen_2025_asundexian.R`, `Gupta_2016_lenvatinib.R`, `Zingmark_2003_clomethiazole.R`, `Zhou_2021_belimumab.R`, `Farrell_2012_farletuzumab.R`, `Valenzuela_2025_nipocalimab.R`, and `Friberg_2012_voriconazole.R`.
* Canonical-register standardization (2026-06-19 audit, compartment-family normalization): cumulative-hazard family is now uniformly lowercase and single-hazard models drop the suffix - renamed `cumHaz_os` -> `cumhaz_os`, `cumHaz_drop` -> `cumhaz_drop`, and `cumHazard` -> `cumhaz` (consolidated into the existing `cumhaz` entry). Enantiomer-suffix register fixes: `r` -> `r_enant`, `s` -> `s_enant` (avoid single-letter collisions). APAP duplicate-suffix deprecation: Zurlinden 2016 `as` / `ag` (which collided with R reserved words and chemistry symbols) are deprecated in favour of the existing Cook 2016 `apaps` / `apapg` canonicals. Metabolite-suffix readability fixes: `dha` -> `dihydroart`, `endx` -> `endox`, `desrbn` -> `desacetylrbn`, `ndsel` -> `ndmsel`. Affects `Valitalo_2017_ketorolac.R`, `Patel_2017_selumetinib.R`, `Hennig_2015_rifabutin.R`, and eight artesunate / artemether models that carry the `dha` suffix (`Hendriksen_2013`, `Hietala_2010_artemether`, `Hietala_2010_artemether_parasitemia`, `LohyDas_2018`, `Morris_2011_artesunate`, `Mosha_2014`, `Tan_2009`, `Tarning_2012_artemether`).
* Canonical-register standardization (2026-06-19 audit, RRT + HEPIMP family normalization): the renal-replacement-therapy family now follows the `RRT_<MODALITY>_<KIND>` shape - renamed `HEMODIAL` -> `RRT_HEMODIAL_STATUS` (subject-level intermittent-hemodialysis treatment-status indicator), `HEMODIALYSIS` -> `RRT_HEMODIAL_ACTIVE` (per-time-point session gate, also previously known as `DIAL`), and `CRRT_STATUS` -> `RRT_CRRT_STATUS` (continuous / extended RRT). The hepatic-impairment composite indicator `HEPIMP_MOD_MISSING` was renamed to `HEPIMP_MOD_OR_MISSING` for explicit "moderate OR data-missing" pooling semantics. Added a section-header policy for the `ROUTE_*` family: every `ROUTE_<TARGET>` canonical MUST document its reference category in the `Reference category:` field (no renames; documentation discipline only). Old names are preserved as `source_aliases`. Affects `Goti_2018_vancomycin.R`, `Liesenfeld_2013_dabigatran.R`, `Jacobs_2016_colistin.R`, `Veinstein_2013_gentamicin.R`, `Shekar_2014_meropenem.R`, `Han_2013_fluconazole.R`, `Chatelut_1999_interferon_alfa_2b.R`, `Naik_2013_peginesatide.R`, and `Lu_2022_patritumab.R`.
* Canonical-register standardization (2026-06-19 audit, SCORE_ + TUMTP_ + FORM_ family normalizations): 13 clinical-instrument canonicals renamed to the SCORE_ family (`EASI` -> `SCORE_EASI`, `MGADL` -> `SCORE_MGADL`, `BCVA` -> `SCORE_BCVA`, `CDR_SOB` -> `SCORE_CDR_SOB`, `ADAS_COG` -> `SCORE_ADAS_COG`, `MMSE` -> `SCORE_MMSE`, `FAQ` -> `SCORE_FAQ`, `RHPNM` -> `SCORE_RHPNM`, `HIGHAM` -> `SCORE_HIGHAM`, `MAYO_E` -> `SCORE_MAYO_E`, `CDAI` -> `SCORE_CDAI`, `PMAYO` -> `SCORE_PMAYO`, `CALPRO` -> `SCORE_CALPRO`); 4 ambiguous tumor-type abbreviations spelled out (`TUMTP_BC` -> `TUMTP_BREAST`, `TUMTP_GC` -> `TUMTP_GASTRIC`, `TUMTP_CHL` -> `TUMTP_HODGKIN_CLASSICAL`, `TUMTP_OTH` -> `TUMTP_OTHER`); 7 FORM_ outliers normalized to the `FORM_<DRUG>_<FEATURE>` shape (`FORM_P2F2` -> `FORM_ISA_P2F2`, `FORM_DP2` -> `FORM_SAR_DP2`, `FORM_NS0` -> `FORM_LEB_NS0`, `FORM_CHO_PHASE2` -> `FORM_LEB_CHO_PHASE2`, `FORM_APNECUT` -> `FORM_THEO_APNECUT`, `FORM_DORYX_MPC` -> `FORM_DOX_DORYX_MPC`, `FORM_SUBA` -> `FORM_ITR_SUBA`). Old names are preserved as `source_aliases`. Affects 25 model files including `Sathe_2024_sacituzumab.R`, `Zhou_2025_brentuximab.R`, `Quartino_2019_trastuzumab.R`, `Okada_2025_rocatinlimab.R`, `Rosario_2015_vedolizumab.R`, `Abuhelwa_2015_itraconazole.R`, `Zhu_2017_lebrikizumab.R`, `Xu_2019_sarilumab.R`, `Fau_2020_isatuximab.R`, `Hopkins_2017_doxycycline.R`, and others.
* Canonical-register SI-unit standardization (2026-06-19 audit, register-level only): the clinical-chemistry canonicals `ALB`, `TPRO`, `CSF_TPRO`, `IGG`, `IGM`, `TBILI`, `DBIL` are now uniformly **SI** in the register (g/L for proteins/immunoglobulins, umol/L for bilirubin). Enzyme canonicals `AST`, `ALT`, `ALP`, `GGT`, `LDH`, `CPK` were already in U/L (SI) and the Units field was annotated to confirm. Conversion factors for US-convention sources (g/dL, mg/dL) are documented in each canonical's Notes field. **Per-model inline-conversion follow-up:** 94 model files consume one or more of these covariates; each must have an inline conversion line added in their `model()` block (e.g., `tbili_mgdL <- TBILI / 17.1`) AND have a before/after simulation equivalence check run, per the operator's 2026-06-19 directive. The conversion pattern, validation protocol, and full affected-files list are documented in `inst/references/SI-units-followup.md`.
