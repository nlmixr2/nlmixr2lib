...
* Add Kapitanov 2025 dupilumab piPKRO framework ([doi:10.1002/psp4.70160](https://doi.org/10.1002/psp4.70160)) -- healthy adult volunteers (first-in-human single-dose IV data from Li 2015). Packages the physiologically-inspired PKRO (piPKRO) tutorial's Case Study 2 (2-cpt fitted, `Kapitanov_2025_dupilumab_qsp`) and Case Study 3 Approach 2 (3-cpt with skin site-of-action, `Kapitanov_2025_dupilumab_3cmt_qsp`). Ratifies new canonical macroparameter names `lthalf` / `thalf` (log linear-elimination half-life), `ltdist` / `tdist` (log distribution half-time), `lpdist` / `pdist` (log peripheral partition coefficient), and their `_p2`-suffixed second-peripheral variants for 3-compartment piPKRO extensions.
