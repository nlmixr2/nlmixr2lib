...
* Add Budha 2023 tislelizumab three-compartment population PK model (anti-PD-1 IgG4 mAb in 2596 patients with advanced tumors, pooled across 12 studies / 14,473 samples). Final covariates: body weight, albumin, baseline tumor size, ADA status, and tumor-type indicators for classical Hodgkin lymphoma and gastric cancer on CL; body weight, age, and sex on Vc. Adds three canonical covariate columns — `TUMSZ`, `TUMTP_CHL`, `TUMTP_GC` — to `inst/references/covariate-columns.md` (categorical `TUMTP` decomposed into indicators following the `RACE_<GROUP>` pattern). Includes a validation vignette with a source-trace table, simulated 200 mg IV q3w cohort, ADA-stratified PKNCA on cycle 1 and cycle 10, and a Vss cross-check against the paper's reported 6.42 L.
