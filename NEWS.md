...
* Add Yamada 2025 zolbetuximab two-compartment population PK model with zero-order IV input and time-dependent total clearance (`CL(t) = CLss + CLT · exp(-Kdecay · t)`) in patients with locally advanced unresectable or metastatic gastric or gastroesophageal junction (G/GEJ) adenocarcinoma, pooled across eight studies (three phase 1, three phase 2, and SPOTLIGHT / GLOW phase 3). Final covariates are BSA on all clearances and volumes, albumin on CLss and Kdecay, HGB and TBILI on V1, prior gastrectomy on CLss / CLT / V1, sex on CLss and V1, and EOX chemotherapy backbone on V1. Built to `extract-literature-model` skill conventions with a companion validation vignette that replicates Yamada 2025 Figure 3 and compares simulated steady-state exposures against the Table 2 Q2W-vs-Q3W GMRs via PKNCA. Ratifies five canonical covariate columns (`BSA`, `HGB`, `TBILI`, `GAST`, `COMB_EOX`) in `inst/references/covariate-columns.md`.
