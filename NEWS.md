...
* Add Jackson 2022 ixekizumab two-compartment linear population PK model with first-order SC absorption in paediatric plaque psoriasis (IXORA-PEDS, NCT03073200). Allometric weight scaling (reference 58.6 kg) on CL, Q, V2, V3 and a log-linear continuous ADA-titre effect on CL (CL_i = CL * (WT/58.6)^0.989 * (1 + 0.0292 * log_e[ADA titre])). Bioavailability F1 fixed at the adult 0.72. Introduces a new canonical covariate `ADA_TITRE` (continuous reciprocal dilution, ADA-negative encoded as 1 so `log_e(1) = 0` cancels the effect); the register in `inst/references/covariate-columns.md` has been updated accordingly. Includes a companion validation vignette that replicates Jackson 2022 Figure 1 weight-group trough concentrations and Figure 3A ADA-titre CL dependence and runs PKNCA on the Week 8-12 steady-state interval.
