...
* Add Cammarata 2024 sulbactam-durlobactam ([doi:10.1128/aac.00485-24](https://doi.org/10.1128/aac.00485-24)) -- healthy adults, adults with renal impairment including hemodialysis, and adults with cUTI, HABP, VABP, or Acinetobacter bacteremia (combined four-compartment plasma model with the paper's hemodialysis and epithelial-lining-fluid sub-models integrated; ratifies the new `sbt` sibling-drug suffix and the `DIS_HABP` / `DIS_VABP` / `DIS_CUTI` / `DIS_BACTEREMIA` / `DIS_AP` / `REGION_EASTASIA` / `STUDY_SULDUR_PHASE2` / `STUDY_SULDUR_PHASE3` covariate canonicals).
