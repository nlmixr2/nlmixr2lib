...
* Cross-reference Shi 2020 polatuzumab vedotin ([doi:10.1007/s00280-020-04119-8](https://doi.org/10.1007/s00280-020-04119-8); PMID 32770353) ethnicity-sensitivity assessment in the existing `Lu_2019_polatuzumab.R` model file. Shi 2020 re-ran the Lu 2019 popPK model on 18 Asian / 442 non-Asian B-NHL patients and verbatim re-quotes the Asian-race covariate -- "7.1% lower V1 in Asian patients than in non-Asian patients" -- already encoded as `e_asian_vc = 0.929` on acMMAE Vc; no parameter changes, only updated `reference`, `description`, `covariateData[[RACE_ASIAN]]$notes`, and `e_asian_vc` label fields plus a new example-model entry in `inst/references/covariate-columns.md`.
...
* Add Vega-Villa 2013 sodium nitrite ([doi:10.1038/psp.2013.35](https://doi.org/10.1038/psp.2013.35)) -- healthy adult volunteers receiving 48-hour IV infusion (QSP model of NO metabolome and methemoglobin).
* Add Gibiansky 2014 obinutuzumab ([doi:10.1038/psp.2014.42](https://doi.org/10.1038/psp.2014.42)) -- adults with chronic lymphocytic leukemia or non-Hodgkin lymphoma (pooled GAUGUIN, GAUDI, GAUSS, and CLL11 trials; ratifies new `TUMTP_BCL`, `TUMTP_DLBCL`, `TUMTP_MCL` covariate canonicals and `BSIZ` SPPD-mm^2 source-alias on `TUMSZ`).
* Add Kretsos 2014 olokizumab ([doi:10.1038/psp.2014.17](https://doi.org/10.1038/psp.2014.17)) -- adults with mild-to-moderate rheumatoid arthritis (single-dose IV / SC anti-IL-6 mAb; two-compartment PK + effect-compartment fractional sigmoid Imax PD for CRP suppression).
...
* Add Ting 2014 inhaled tobramycin ([doi:10.1038/psp.2013.76](https://doi.org/10.1038/psp.2013.76)) -- cystic fibrosis patients receiving inhaled tobramycin powder (TIP / TOBI Podhaler); ratifies new `FEV1_PCTPRED` covariate canonical for baseline FEV1 percent-predicted.
* Add Gilkey 2015 DiR-labeled block-copolymer nanoparticles ([doi:10.1002/psp4.13](https://doi.org/10.1002/psp4.13)) -- preclinical female BALB/c mice (5-compartment PBPK surrogate model for dexamethasone-loaded nanoparticles in pediatric acute lymphoblastic leukemia therapy).
* Add Le 2015 lampalizumab ([doi:10.1002/psp4.12031](https://doi.org/10.1002/psp4.12031)) -- adults with geographic atrophy secondary to age-related macular degeneration (combined ocular-serum TMDD QSS model for intravitreally administered anti-complement factor D Fab).
* Add Lu 2015 vismodegib ([doi:10.1002/psp4.12039](https://doi.org/10.1002/psp4.12039)) -- adults with advanced solid tumors (predominantly basal cell carcinoma) pooled with healthy volunteers (semi-mechanism-based one-compartment popPK with first-order absorption, first-order elimination of unbound drug, and saturable fast-equilibrium binding to time-varying AAG; ratifies new `FORM_VISMO_PHASEI` covariate canonical for the Phase I dry-blend vs Phase II / commercial wet-granulation capsule formulation contrast).
* Add Ouerdani 2015 pazopanib ([doi:10.1002/psp4.12001](https://doi.org/10.1002/psp4.12001)) -- paired preclinical (female CB-17 SCID mice with CAKI-2 renal-cell carcinoma xenografts) and clinical (adult patients with advanced renal-cell carcinoma; NCT00244764 Phase 2 subset) semi-mechanistic tumour-growth and angiogenesis-inhibition models (ratifies new `AUC_PAZO` and `TUM_VOL` covariate canonicals).
