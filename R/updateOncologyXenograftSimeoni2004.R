...
#' @param drugEffectName,transitRateName character string names of the drug
#'   effect and transit rate (as used in the model block)
...
updateOncologyXenograftSimeoni2004 <- function(object, ncmt,
  ...
  objectDamage <- do.call(
    rxode2::model,
    append(list(objectTumor, append = TRUE), lapply(X = damagedLines, FUN = str2lang))
  )
  objectDamage
}
